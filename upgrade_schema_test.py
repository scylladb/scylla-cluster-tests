...
class TestNonCompoundThriftDynamicTable(Test):
    ...
    def setUp(self):
        cf = CfDef(ks_name, "test_table0", key_validation_class="UTF8Type", comparator_type="LongType", default_validation_class="Int32Type")
...
class TestNonStandardComparator(Test):
    ...
    def runTest(self):
        for key in self.keys:
            assert thrift_client.get(key, ColumnPath("test_table", column=_i64(4)), ConsistencyLevel.ONE).column == Column(_i64(4), _i32(1), 1)
            assert thrift_client.get(key, ColumnPath("test_table", column=_i64(5)), ConsistencyLevel.ONE).column == Column(_i64(5), _i32(2), 2)
        self._insertData()
...
class TestThriftTableConvertedFromDenseToSparse(Test):
    ...
    def setUp(self):
        cf = CfDef(ks_name, "test_table2", key_validation_class="UTF8Type", comparator_type="LongType", default_validation_class="UTF8Type")
        thrift_client.system_add_column_family(cf)
        for key in self.keys:
...
class UpgradeSchemaTest(ClusterTester):
    ...
    def test_upgrade_schema(self):
        ...
        try:
            ...
            pass
        thrift_client.system_add_keyspace(KsDef(ks_name, "org.apache.cassandra.locator.NetworkTopologyStrategy", {"replication_factor": "1"}, cf_defs=[]))
