...
from sdcm.argus_results import send_manager_benchmark_results_to_argus, send_manager_snapshot_details_to_argus, ManagerRestoreBenchmarkResult, ManagerOneOneRestoreBenchmarkResult
...
class ManagerRestoreTests(ManagerTestFunctionsMixIn):
    def test_restore_multiple_backup_snapshots(self):  # noqa: PLR0914
        ...
        for size in snapshot_sizes:
            ...
            self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=snapshot_tag, timeout=180, restore_schema=True, location_list=location_list)
            self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=snapshot_tag, timeout=expected_timeout, restore_data=True, location_list=location_list)
            stress_command = confirmation_stress_template.format(num_of_rows=number_of_rows, keyspace_name=keyspace_name, sequence_start=1, sequence_end=number_of_rows)
    ...
    def test_restore_backup_with_task(self, ks_names: list = None):
        ...
        assert backup_task_status == TaskStatus.DONE, f"Backup task ended in {backup_task_status} instead of {TaskStatus.DONE}"
        soft_timeout = 36 * 60
        hard_timeout = 50 * 60
        with adaptive_timeout(Operations.MGMT_REPAIR, self.db_cluster.data_nodes[0], timeout=soft_timeout):
            self.verify_backup_success(mgr_cluster=mgr_cluster, backup_task=backup_task, ks_names=ks_names, restore_data_with_task=True, timeout=hard_timeout)
    ...
    def test_restore_alternator_backup_with_task(self, delete_tables: list = None):
        ...
        mgr_cluster = self.db_cluster.get_cluster_manager(alternator_credentials=self.alternator.get_credentials(node=self.db_cluster.nodes[0]))
        ...
        hard_timeout = 50 * 60
        with adaptive_timeout(Operations.MGMT_REPAIR, self.db_cluster.data_nodes[0], timeout=soft_timeout):
            self.verify_alternator_backup_success(mgr_cluster=mgr_cluster, backup_task=backup_task, delete_tables=delete_tables, timeout=hard_timeout)
        mgr_cluster.delete()  # remove cluster at the end of the test
...
class ManagerBackupTests(ManagerRestoreTests):
    ...
    def test_backup_rate_limit(self):
        ...
        assert task_status == TaskStatus.DONE, f"Task {backup_task.id} did not end successfully:\n{backup_task.detailed_progress}"
    ...
    def test_enospc_during_backup(self):
        ...
        with ignore_no_space_errors(node=target_node):
            try:
                ...
                assert backup_task.status == TaskStatus.DONE, "The backup failed to run on a node with no free space, while it should have had the room for snapshots due to the previous run"
    ...
    def test_enospc_before_restore(self):
        if is_tablets_feature_enabled(self.db_cluster.nodes[0]):
            ...
            self.log.info("Skipping test_enospc_before_restore due to enabled tablets. For details https://github.com/scylladb/scylla-manager/issues/4276")
            return
        ...
        with ignore_no_space_errors(node=target_node), ignore_stream_mutation_fragments_errors():
            try:
                ...
                restore_task = mgr_cluster.create_restore_task(restore_data=True, location_list=self.locations, snapshot_tag=snapshot_tag)
                final_status = restore_task.wait_and_get_final_status(step=30)

                assert final_status == TaskStatus.ERROR, f"The restore task is supposed to fail, since node {target_node} lacks the disk space to downloadthe snapshot files"
            finally:
                clean_enospc_on_node(target_node=target_node, sleep_time=30)
    ...
    def test_alternator_backup_feature(self):
        ...
        features = {"lsi": test_table_config.get("lsi_name", None), "gsi": test_table_config.get("gsi_name", None), "tags": test_table_config.get("tags", None)}
        ...
        with alternator_backuped_tables(target_node, self.alternator, params=self.params, **features) as tables:
            ...
            with self.subTest("Test restore alternator backup with restore task"):
                ...
                self.alternator.verify_tables_features(node=target_node, tables=tables, wait_for_item_count=test_table_config.get("items", None), **features)
    ...
    def test_no_delta_backup_at_disabled_compaction(self):
        ...
        assert backup_task_1_status == TaskStatus.DONE, f"Backup task ended in {backup_task_1_status} instead of {TaskStatus.DONE}"
        ...
        assert backup_task_2_status == TaskStatus.DONE, f"Backup task ended in {backup_task_2_status} instead of {TaskStatus.DONE}"
        ...
        self.verify_backup_success(mgr_cluster=mgr_cluster, backup_task=backup_task_2, restore_data_with_task=True, timeout=3600)
...
class ManagerRepairTests(ManagerTestFunctionsMixIn):
    ...
    def _test_intensity_and_parallel(self, fault_multiple_nodes):
        ...
        self.create_missing_rows_in_cluster(create_missing_rows_in_multiple_nodes=fault_multiple_nodes, keyspace_to_be_repaired=keyspace_to_be_repaired, total_num_of_rows=29296872)
        ...
        arg_list = [{"intensity": 0.5}, {"intensity": 0.25}, {"intensity": 0.0001}, {"intensity": 2}, {"intensity": 4}, {"parallel": 1}, {"parallel": 2}, {"intensity": 2, "parallel": 1}, {"intensity": 100}, {"intensity": 0}]
    ...
    def test_repair_multiple_keyspace_types(self):
        ...
        assert task_final_status == TaskStatus.DONE, "Task: {} final status is: {}.".format(repair_task.id, str(repair_task.status))
        ...
        for keyspace_name in expected_keyspaces_to_be_repaired:
            ...
            assert keyspace_repair_percentage is not None, "The keyspace {} was not included in the repair!".format(keyspace_name)

            assert keyspace_repair_percentage == 100, "The repair of the keyspace {} stopped at {}%".format(keyspace_name, keyspace_repair_percentage)

        localstrategy_keyspace_percentage = per_keyspace_progress.get(self.LOCALSTRATEGY_KEYSPACE_NAME, None)
        assert localstrategy_keyspace_percentage is None, "The keyspace with the replication strategy of localstrategy was included in repair, when it shouldn't"
...
class ManagerCRUDTests(ManagerTestFunctionsMixIn):
    def test_cluster_crud(self):
        ...
        mgr_cluster.delete()
        mgr_cluster = manager_tool.add_cluster(self.CLUSTER_NAME, db_cluster=self.db_cluster, auth_token=self.monitors.mgmt_auth_token)
...
class ManagerHealthCheckTests(ManagerTestFunctionsMixIn):
    def test_cluster_healthcheck(self):
        ...
        mgr_cluster = self.db_cluster.get_cluster_manager()
        other_host, other_host_ip = [host_data for host_data in self.get_cluster_hosts_with_ips() if host_data[1] != self.get_cluster_hosts_ip()[0]][0]
        sleep = 40
    ...
    def test_healthcheck_change_max_timeout(self):
        ...
        mgr_cluster = self.db_cluster.get_cluster_manager()
        try:
            reconfigure_scylla_manager(manager_node=manager_node, logger=self.log, values_to_update=[{"healthcheck": {"max_timeout": "20ms"}}])
            sleep = 40
...
class ManagerEncryptionTests(ManagerTestFunctionsMixIn):
    ...
    def test_client_encryption(self):
        ...
        manager_node.create_node_certificate(cert_file=manager_node.ssl_conf_dir / TLSAssets.CLIENT_CERT, cert_key=manager_node.ssl_conf_dir / TLSAssets.CLIENT_KEY)
...
class ManagerSuspendTests(ManagerTestFunctionsMixIn):
    def _test_suspend_and_resume_task_template(self, task_type):
        ...
        assert suspendable_task.wait_for_status(list_status=[TaskStatus.RUNNING], timeout=300, step=5), f"task {suspendable_task.id} failed to reach status {TaskStatus.RUNNING}"
        with mgr_cluster.suspend_manager_then_resume(start_tasks=True):
            assert suspendable_task.wait_for_status(list_status=[TaskStatus.STOPPED], timeout=300, step=10), f"task {suspendable_task.id} failed to reach status {TaskStatus.STOPPED}"
        assert suspendable_task.wait_for_status(list_status=[TaskStatus.DONE], timeout=1200, step=10), f"task {suspendable_task.id} failed to reach status {TaskStatus.DONE}"
    ...
    def _test_suspend_with_on_resume_start_tasks_flag_template(self, wait_for_duration):
        ...
        assert suspendable_task.wait_for_status(list_status=[TaskStatus.RUNNING], timeout=300, step=5), f"task {suspendable_task.id} failed to reach status {TaskStatus.RUNNING}"
        with mgr_cluster.suspend_manager_then_resume(start_tasks=False, start_tasks_in_advance=True, duration=f"{suspension_duration}s"):
            assert suspendable_task.wait_for_status(list_status=[TaskStatus.STOPPED], timeout=60, step=2), f"task {suspendable_task.id} failed to reach status {TaskStatus.STOPPED}"
        ...
        if wait_for_duration:
            assert suspendable_task.wait_for_status(list_status=[TaskStatus.DONE], timeout=1200, step=10), (
                f"After the cluster was resumed (while resuming AFTER the suspend duration has passed), task {suspendable_task.id} failed to reach status {TaskStatus.DONE}, but instead stayed in {suspendable_task.status}"
            )
        else:
            assert suspendable_task.status == TaskStatus.STOPPED, f"After the cluster was resumed (while resuming BEFORE the suspend duration has passed), task {suspendable_task.id} failed to stay in status STOPPED"
            time.sleep(suspension_duration + 5)
            assert suspendable_task.status == TaskStatus.STOPPED, (
                f"After the cluster was resumed (while resuming BEFORE the suspend duration has passed), task {suspendable_task.id} failed to stay in status STOPPED after suspension time ended"
            )
    ...
    def _test_suspend_and_resume_without_starting_tasks(self):
        ...
        with mgr_cluster.suspend_manager_then_resume(start_tasks=False):
            ...
            assert current_task_status == TaskStatus.STOPPED, f'Task {current_task_status} did not remain in "{TaskStatus.STOPPED}" status, but instead reached "{current_task_status}" status'
...
class ManagerRollbackTests(ManagerTestFunctionsMixIn):
    ...
    def test_manager_upgrade(self):
        ...
        mgr_cluster = manager_tool.get_cluster(cluster_name=cluster_name) or manager_tool.add_cluster(name=cluster_name, host=selected_host, auth_token=self.monitors.mgmt_auth_token)
...
class ManagerRestoreBenchmarkTests(ManagerTestFunctionsMixIn):
    ...
    def _send_restore_results_to_argus(self, task: RestoreTask, manager_version_timestamp: int, dataset_label: str = None):
    ...
    def test_backup_and_restore_only_data(self, object_storage_method: ObjectStorageUploadMode):
        ...
        task = self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=backup_task.get_snapshot_tag(), timeout=110000, restore_data=True, extra_params=extra_params, object_storage_method=object_storage_method)
    ...
    def test_restore_from_precreated_backup(self, snapshot_name: str, object_storage_method: ObjectStorageUploadMode = None, restore_outside_manager: bool = False):
        ...
        self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=snapshot_data.tag, timeout=600, restore_schema=True, location_list=locations)
        ...
        if restore_outside_manager:
            ...
            task = self.restore_backup_with_task(
                mgr_cluster=mgr_cluster, snapshot_tag=snapshot_data.tag, restore_data=True, timeout=snapshot_data.exp_timeout, location_list=locations, extra_params=extra_params, object_storage_method=object_storage_method
            )
    ...
    def test_restore_data_without_manager(self):
        ...
        assert snapshot_name, "The test requires a pre-created snapshot to restore from. Please provide the 'mgmt_reuse_backup_snapshot_name' parameter."
...
class ManagerOneToOneRestore(ManagerTestFunctionsMixIn):
    ...
    def _send_one_one_restore_results_to_argus(self, bootstrap_duration: int, restore_duration: int) -> None:
        ...
        send_manager_benchmark_results_to_argus(argus_client=self.test_config.argus_client(), result=results, result_table=result_table)
...
class ManagerBackupRestoreConcurrentTests(ManagerTestFunctionsMixIn):
    ...
    def test_backup_benchmark(self):
        ...
        backup_thread = threading.Thread(target=self.manager_backup_and_report, kwargs={"mgr_cluster": mgr_cluster, "label": "Backup during read stress"})

        read_stress_thread = threading.Thread(target=self.run_read_stress_and_report, kwargs={"label": "Read stress during backup"})
        backup_thread.start()
