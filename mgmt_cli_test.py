...
class ManagerRestoreBenchmarkTests(ManagerTestFunctionsMixIn):
    ...
    def test_backup_and_restore_only_data(self, manager_method: ObjectStorageUploadMode):
        ...
        task = self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=backup_task.get_snapshot_tag(),
                                             timeout=110000, restore_data=True, extra_params=extra_params, manager_method=manager_method)
    ...
    def test_restore_from_precreated_backup(self, snapshot_name: str, manager_method: ObjectStorageUploadMode = None, restore_outside_manager: bool = False):
        ...
        self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=snapshot_data.tag, timeout=600,
                                      restore_schema=True, location_list=locations, manager_method=manager_method)
    ...
    def test_restore_benchmark(self):
        ...
        """
        if manager_method := self.params.get('manager_method'):
            manager_method = ObjectStorageUploadMode(manager_method)
        if reuse_snapshot_name := self.params.get('mgmt_reuse_backup_snapshot_name'):
            ...
            self.test_restore_from_precreated_backup(
                reuse_snapshot_name, manager_method=manager_method)
        else:
            self.log.info("Executing test_backup_and_restore_only_data...")
            self.test_backup_and_restore_only_data(manager_method=manager_method)
