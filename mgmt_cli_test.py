...
class StressLoadOperations(ClusterTester, LoaderUtilsMixin):
    ...
    def prepare_run_and_verify_stress_in_threads(
        self, cmd_template: str, keyspace_name: str, num_of_rows: int, stop_on_failure: bool = False
    ) -> None:
        ...
        for loader_index in range(num_of_loaders):
            stress_cmd = cmd_template.format(
...
class ManagerEncryptionTests(ManagerTestFunctionsMixIn):
    def test_client_encryption(self):
        ...
        mgr_cluster.update(
            client_encrypt=True, force_non_ssl_session_port=mgr_cluster.sctool.is_minimum_3_2_6_or_snapshot
        )
        time.sleep(90)
...
||||||| parent of aac40c98c (fix(mgmt_cli_test.py): report restored data size to Argus)
        download_bw, load_and_stream_bw = task.download_bw, task.load_and_stream_bw
        if download_bw:
            results["download bandwidth"] = download_bw
        if load_and_stream_bw:
...
=======
    ...
    def _send_restore_results_to_argus(self, results: dict, manager_version_timestamp: int, dataset_label: str = None):
        ...
        """
        result_table = ManagerRestoreBenchmarkResult(sut_timestamp=manager_version_timestamp)
>>>>>>> aac40c98c (fix(mgmt_cli_test.py): report restored data size to Argus)
        send_manager_benchmark_results_to_argus(
...
>>>>>>> aac40c98c (fix(mgmt_cli_test.py): report restored data size to Argus)
        ...
        if not (self.params.get("mgmt_skip_post_restore_stress_read") or snapshot_data.prohibit_verification_read):
            ...
            self.prepare_run_and_verify_stress_in_threads(
                cmd_template=snapshot_data.cs_read_cmd_template,
