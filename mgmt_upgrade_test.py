...
class ManagerUpgradeTest(ManagerTestFunctionsMixIn, ClusterTester):
    ...
    def write_multiple_rows(self, table_name, key_range, keyspace_name="ks1"):
        with self.db_cluster.cql_connection_patient(self.db_cluster.nodes[0]) as session:
            for num in range(*key_range):
                session.execute(
                    f"insert into {keyspace_name}.{table_name} (key, c1, c2) VALUES ('k_{num}', 'v_{num}', 'v_{num}');"
                )
    ...
    def _create_and_add_cluster(self):
        ...
        mgr_cluster = manager_tool.add_cluster(
            name=ManagerUpgradeTest.CLUSTER_NAME, db_cluster=self.db_cluster, auth_token=self.monitors.mgmt_auth_token
        )
    ...
    def test_upgrade(self):  # noqa: PLR0914
        ...
        with self.subTest(
            "Executing the 'backup list' and 'backup files' commands on a older version backup with newer version manager"
        ):
        ...
        with self.subTest("purging a older version backup"):
            ...
            for i in range(2, 4):
                ...
                assert rerunning_backup_task.status == TaskStatus.DONE, (
                    f"backup {rerunning_backup_task.id} that was rerun again from the start has failed to reach status DONE within expected time limit"
                )
...
def wait_until_task_finishes_return_details(task, wait=True, timeout=1000, step=10):
    ...
    if task.sctool.is_v3_cli:
        ...
        end_time_string = task.sctool.get_table_value(
            parsed_table=task_history, column_name="end time", identifier=latest_run_id
        )
...
def _create_mismatched_details_error_message(previous_task_details, current_task_details, mismatched_details_name_list):
    ...
    for name in mismatched_details_name_list:
        complete_error_description += error_message_format.format(
...
def validate_previous_task_details(task, previous_task_details):
    ...
    for detail_name, current_value in current_task_details.items():
        if isinstance(current_value, datetime):
            ...
            mismatched_details_name_list.append(detail_name)
    complete_error_description = _create_mismatched_details_error_message(
        previous_task_details, current_task_details, mismatched_details_name_list
    )
...
def upgrade_scylla_manager(
    pre_upgrade_manager_version, target_upgrade_server_version, target_upgrade_agent_version, manager_node, db_cluster
):
