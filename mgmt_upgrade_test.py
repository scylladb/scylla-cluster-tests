...
class ManagerUpgradeTest(ManagerTestFunctionsMixIn, ClusterTester):
    ...
    def write_multiple_rows(self, table_name, key_range, keyspace_name="ks1"):
        with self.db_cluster.cql_connection_patient(self.db_cluster.nodes[0]) as session:
            for num in range(*key_range):
                session.execute(f"insert into {keyspace_name}.{table_name} (key, c1, c2) VALUES ('k_{num}', 'v_{num}', 'v_{num}');")
    ...
    def _create_and_add_cluster(self):
        ...
        mgr_cluster = manager_tool.add_cluster(name=ManagerUpgradeTest.CLUSTER_NAME, db_cluster=self.db_cluster, auth_token=self.monitors.mgmt_auth_token)
    ...
    def test_upgrade(self):  # noqa: PLR0914
        ...
        if not target_upgrade_server_version:
            target_upgrade_server_version = get_manager_repo_from_defaults(target_manager_version, distro=manager_node.distro)
        if not target_upgrade_agent_version:
            target_upgrade_agent_version = get_manager_repo_from_defaults(target_manager_version, distro=self.db_cluster.nodes[0].distro)
        ...
        with self.subTest("Creating reoccurring backup and repair tasks"):
            ...
            backup_task = mgr_cluster.create_backup_task(cron=create_cron_list_from_timedelta(minutes=2), location_list=self.locations, keyspace_list=["keyspace1"])
        ...
        with self.subTest("Creating a simple backup with the intention of purging it"):
            ...
            rerunning_backup_task = mgr_cluster.create_backup_task(location_list=self.locations, keyspace_list=["ks1"], retention=2)
            rerunning_backup_task.wait_and_get_final_status(timeout=300, step=20)
            assert rerunning_backup_task.status == TaskStatus.DONE, f"Unknown failure in task {rerunning_backup_task.id}"
        ...
        upgrade_scylla_manager(
            pre_upgrade_manager_version=current_manager_version, target_upgrade_server_version=target_upgrade_server_version, target_upgrade_agent_version=target_upgrade_agent_version, manager_node=manager_node, db_cluster=self.db_cluster
        )
        ...
        with self.subTest("Continuing a older version stopped backup task with newer version manager"):
            ...
            assert pausable_backup_task.status == TaskStatus.DONE, f"task {pausable_backup_task.id} failed to continue after manager upgrade"
        ...
        with self.subTest("Restoring an older version backup task with newer version manager, using a restore task"):
            self.verify_backup_success(mgr_cluster=mgr_cluster, backup_task=backup_task, restore_data_with_task=True, timeout=600)
            self.run_verification_read_stress()

        with self.subTest("Executing the 'backup list' and 'backup files' commands on a older version backup with newer version manager"):
        ...
        with self.subTest("purging a older version backup"):
            ...
            for i in range(2, 4):
                ...
                assert rerunning_backup_task.status == TaskStatus.DONE, f"backup {rerunning_backup_task.id} that was rerun again from the start has failed to reach status DONE within expected time limit"
            per_node_backup_file_paths = mgr_cluster.get_backup_files_dict(snapshot_tag=rerunning_backup_task.get_snapshot_tag())
            for node in self.db_cluster.nodes:
                node_id = node.host_id
                # making sure that the files of the missing table isn't in s3
                assert "cf1" not in per_node_backup_file_paths[node_id]["ks1"], "The missing table is still in s3, even though it should have been purged"
...
def wait_until_task_finishes_return_details(task, wait=True, timeout=1000, step=10):
    ...
    start_time = task.sctool.get_table_value(parsed_table=task_history, column_name="start time", identifier=latest_run_id)
    ...
    if task.sctool.is_v3_cli:
        ...
        end_time_string = task.sctool.get_table_value(parsed_table=task_history, column_name="end time", identifier=latest_run_id)
        end_time = time_string_to_datetime(end_time_string)
    task_details = {"next run": next_run_time, "latest run id": latest_run_id, "start time": start_time, "end time": end_time, "duration": duration}
    return task_details
...
def _create_mismatched_details_error_message(previous_task_details, current_task_details, mismatched_details_name_list):
    ...
    for name in mismatched_details_name_list:
        complete_error_description += error_message_format.format(name, previous_task_details[name], current_task_details[name])
...
def validate_previous_task_details(task, previous_task_details):
    ...
    for detail_name, current_value in current_task_details.items():
        if isinstance(current_value, datetime):
            ...
            mismatched_details_name_list.append(detail_name)
    complete_error_description = _create_mismatched_details_error_message(previous_task_details, current_task_details, mismatched_details_name_list)
    assert not mismatched_details_name_list, f"Task details of {task.id} changed:{complete_error_description}"


def upgrade_scylla_manager(pre_upgrade_manager_version, target_upgrade_server_version, target_upgrade_agent_version, manager_node, db_cluster):
    ...
    assert new_manager_version != pre_upgrade_manager_version, "Manager failed to upgrade - previous and new versions are the same. Test failed!"
...
def _create_stopped_repair_task_with_manual_argument(mgr_cluster, arg_string):
    ...
    repair_task = RepairTask(task_id=task_id, cluster_id=mgr_cluster.id, manager_node=mgr_cluster.manager_node)
