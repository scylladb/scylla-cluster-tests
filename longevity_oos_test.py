...
def ignore_stress_errors():
    with ExitStack() as stack:
        stack.enter_context(EventsSeverityChangerFilter(new_severity=Severity.NORMAL, event_class=CassandraStressEvent, extra_time_to_expiration=60))
        stack.enter_context(EventsSeverityChangerFilter(new_severity=Severity.NORMAL, event_class=CassandraStressLogEvent, extra_time_to_expiration=60))
        yield
...
def ignore_repair_errors():
    with ExitStack() as stack:
        stack.enter_context(EventsSeverityChangerFilter(new_severity=Severity.NORMAL, event_class=DatabaseLogEvent, regex=".*Repair service is disabled. No repairs will be started until it's re-enabled.*", extra_time_to_expiration=60))
        stack.enter_context(EventsSeverityChangerFilter(new_severity=Severity.NORMAL, event_class=NodetoolEvent, regex=".*Repair service is disabled. No repairs will be started until it's re-enabled.*", extra_time_to_expiration=60))
        stack.enter_context(EventsSeverityChangerFilter(new_severity=Severity.NORMAL, event_class=TestFrameworkEvent, regex=".*Failed on waiting until task.*", extra_time_to_expiration=60))
        yield
...
class LongevityOutOfSpaceTest(LongevityTest):
    def tearDown(self):
        ...
        for node in self.db_cluster.nodes:
            ...
            if max_usage >= 98.5:
                TestFrameworkEvent(source=self.__class__.__name__, message=f"Node {node.name} ({node.private_ip_address}) max disk usage was {max_usage:.2f}%.", severity=Severity.ERROR).publish()
    ...
    def run_read_stress(self):
        stress_queue = []
        self.assemble_and_run_all_stress_cmd(stress_queue, self.params.get("stress_cmd_r"), self.params.get("keyspace_num"))
        return all(self.verify_stress_thread(stress) for stress in stress_queue)

    def run_write_stress(self):
        stress_queue = []
        self.assemble_and_run_all_stress_cmd(stress_queue, self.params.get("stress_cmd_w"), self.params.get("keyspace_num"))
        return all(self.verify_stress_thread(stress) for stress in stress_queue)
    ...
    def scale_out(self, nr_nodes=None, rack=None):
        ...
        added_nodes = self.db_cluster.add_nodes(count=nr_nodes, instance_type=instance_type, enable_auto_bootstrap=True, rack=rack)
    ...
    def prepare(self):
        ...
        for node in self.db_cluster.nodes:
            ...
            if usage <= 85:
                TestFrameworkEvent(source=self.__class__.__name__, message=f"Node {node.name} ({node.private_ip_address}) max disk is only {usage:.2f}% after prepare.", severity=Severity.CRITICAL).publish()
    ...
    def test_oos_write(self):
        ...
        with ignore_stress_errors():
            ...
            if result:
                TestFrameworkEvent(source=self.__class__.__name__, message="Writes should have failed, but it succeeded", severity=Severity.ERROR).publish()
    ...
    def test_oos_restart(self):
        ...
        with ignore_stress_errors():
            ...
            if not restarted:
                TestFrameworkEvent(source=self.__class__.__name__, message="No node reached 97% disk usage to restart.", severity=Severity.CRITICAL).publish()
    ...
    def test_oos_repair(self):
        ...
        try:
            ...
            TestFrameworkEvent(source=self.__class__.__name__, message=f"Repair should not finish. Status: {task_final_status}", severity=Severity.CRITICAL).publish()
        ...
        if task_final_status != TaskStatus.DONE:
            progress_full_string = repair_task.progress_string(parse_table_res=False, is_verify_errorless_result=True).stdout
            if task_final_status != TaskStatus.ERROR_FINAL:
                repair_task.stop()
            raise ScyllaManagerError(f"Task: {repair_task.id} final status is: {task_final_status}.\nTask progress string: {progress_full_string}")
    ...
    def test_oos_compaction(self):
        ...
        if self.get_compactions(threshold_node, interval=600) != 0:
            TestFrameworkEvent(source=self.__class__.__name__, message=f"There should be no running compactions on node {threshold_node.name}", severity=Severity.ERROR).publish()
        ...
        if self.get_compactions(threshold_node, interval=interval) == 0:
            TestFrameworkEvent(source=self.__class__.__name__, message=f"There should have been running compactions on node {threshold_node.name} after scale out", severity=Severity.ERROR).publish()
    ...
    def test_oos_secondary_index(self):
        ...
        try:
            ...
            TestFrameworkEvent(source=self.__class__.__name__, message="Index creation should not finish.", severity=Severity.CRITICAL).publish()
        except TimeoutError:
        ...
        try:
            ...
            TestFrameworkEvent(source=self.__class__.__name__, message="Index creation should finish after scale out.", severity=Severity.CRITICAL).publish()
