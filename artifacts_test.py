            ...
            with self.subTest("check if scylla unnecessarily installed a time synchronization service"):
                ...
                try:
                    if is_timesyncd_service_installed:
                        ...
                        self.log.warning(
                            "No time sync service was installed. Passable since it's an offline installation."
                        )
                except AssertionError:
                    full_list = self.node.remoter.run("systemctl list-units --full").stdout.strip()
                    self.log.warning(
                        "Seems that there was an issue with ntp check. Here's the full list of services "
                        "active on the node: %s",
                        full_list,
                    )
                    raise
