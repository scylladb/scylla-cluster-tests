...
BACKENDS = {"aws": ["Ec2Snitch", "Ec2MultiRegionSnitch"], "gce": ["GoogleCloudSnitch"], "azure": ["AzureSnitch"], "docker": ["GossipingPropertyFileSnitch", "SimpleSnitch"]}
...
class ArtifactsTest(ClusterTester):
    ...
    def check_scylla_version_in_housekeepingdb(self, prev_id: int, expected_status_code: str, new_row_expected: bool, backend: str) -> int:
        """
        ...
        row = self.housekeeping.get_most_recent_record(query=f"SELECT id, version, ip, statuscode FROM {self.CHECK_VERSION_TABLE} WHERE uuid = %s", args=(self.node.uuid,))
        ...
        assert public_ip_address == row[2], f"Wrong IP address is saved in '{self.CHECK_VERSION_TABLE}' table: expected {self.node.public_ip_address}, got: {row[2]}"

        # Validate reported node version
        assert row[1] == self.node.scylla_version, f"Wrong version is saved in '{self.CHECK_VERSION_TABLE}' table: expected {self.node.public_ip_address}, got: {row[2]}"

        # Validate expected status code
        assert row[3] == expected_status_code, f"Wrong statuscode is saved in '{self.CHECK_VERSION_TABLE}' table: expected {expected_status_code}, got: {row[3]}"
    ...
    def check_cluster_name(self):
        ...
        self.assertTrue(self.db_cluster.name == yaml_cluster_name, f"Cluster name is not as expected. Cluster name in scylla.yaml: {yaml_cluster_name}. Cluster name: {self.db_cluster.name}")
    ...
    def run_cassandra_stress(self, args: str):
        ...
        if self.params.get("client_encrypt"):
            transport_str = c_s_transport_str(self.params.get("peer_verification"), self.params.get("client_encrypt_mtls"))
    ...
    def check_cqlsh(self):
        ...
        for line in output:
            try:
                ...
                continue
            self.log.debug(host)
            assert "broadcast_address" in host and "host_id" in host, f"system.local: {host}, doesn't have 'broadcast_address' or 'host_id'"
    ...
    def check_housekeeping_service_status(self, backend: str):
        ...
        if status_out.stderr and "TMOUT" not in status_out.stderr:
            ScyllaHousekeepingServiceEvent(message=f"scylla-housekeeping-restart service error: {status_out.stderr}", severity=Severity.ERROR).publish()
            return
        ...
        if (
            ...
            ScyllaHousekeepingServiceEvent(message="scylla-housekeeping-restart service running", severity=Severity.NORMAL).publish()
        else:
            ScyllaHousekeepingServiceEvent(message=f"scylla-housekeeping-restart service is not running: {status}", severity=Severity.ERROR).publish()
    ...
    def verify_snitch(self, backend_name: str):
        ...
        if not self.params.get("use_preinstalled_scylla"):
            ...
            self.actions_log.info("Skipping verifying the snitch due to the 'use_preinstalled_scylla' being set to False")
            return
        ...
        with self.logged_subtest("verify snitch against describecluster output"):
            self.assertTrue(any(snitch_matches_describecluster), msg=f"Expected snitch matches for describecluster to not be empty, but was. Snitch matches: {snitch_matches_describecluster}")

        with self.logged_subtest("verify snitch against scylla.yaml configuration"):
            self.assertTrue(any(snitch_matches_scylla_yaml), msg=f"Expected snitch matches for scylla yaml to not be empty, but was. Snitch matches: {snitch_matches_scylla_yaml}")
    ...
    def verify_docker_latest_match_release(self) -> None:
        ...
        assert latest_digests == release_digests, f"latest != {latest_version}, images digest differs [{latest_digests}] != [{release_digests}]"
        ...
        if pre_create_schema:
            ...
            for i in range(1, keyspace_num + 1):
                ...
                for col_idx in range(col_num):
                    ...
                    columns[cs_key] = "blob"
                self.create_table(
                    name="standard1", keyspace_name=keyspace_name, key_type="blob", read_repair=0.0, columns=columns, scylla_encryption_options=scylla_encryption_options, compaction=compaction_strategy, sstable_size=sstable_size
                )
    ...
    def test_scylla_service(self):  # noqa: PLR0915
            ...
            with self.logged_subtest("check Scylla IO Params"):
                try:
                    if self.node.db_node_instance_type in ["t3.micro"]:
                        self.skipTest(f"{self.node.db_node_instance_type} is not a supported instance - skipping this test.")
                    validator = IOTuneValidator(self.node)
                    validator.validate()
                    send_iotune_results_to_argus(self.test_config.argus_client(), validator.results, self.node, self.params)
                    ...
                    TestFrameworkEvent(source={self.__class__.__name__}, message="Error during IOTune params validation.", severity=Severity.ERROR).publish()

        with self.logged_subtest("verify write cache for NVMe devices"):
            self.verify_nvme_write_cache()

        if backend != "docker" and not self.params.get("nonroot_offline_install") and self.node.db_node_instance_type != "t3.micro":
            ...
            with self.logged_subtest("check if scylla unnecessarily installed a time synchronization service"):
                ...
                try:
                    if is_timesyncd_service_installed:
                        assert not is_ntp_service_installed, "systemd-timesyncd is already installed, yet scylla installed ntp service on top of it"
                        assert not is_chrony_service_installed, "systemd-timesyncd is already installed, yet scylla installed chrony service on top of it"
                    elif not self.params.get("unified_package"):
                        assert is_ntp_service_installed or is_chrony_service_installed, "systemd-timesyncd is not installed on the instance, yet Scylla did not install ntp or chrony services as a replacement"
                    else:
                        ...
                        self.log.warning("No time sync service was installed. Is it possible this is because it's an offline installation?")
                except AssertionError:
                    full_list = self.node.remoter.run("systemctl list-units --full").stdout.strip()
                    self.log.warning("Seems that there was an issue with ntp check. Here's the full list of services active on the node: %s", full_list)
                    raise
        ...
        with self.logged_subtest("check Scylla server after stop/start"):
            ...
            if not self.node.is_nonroot_install and backend != "docker":
                ...
                with self.actions_log.action_scope("Validate version after stop/start"):
                    ...
                    version_id_after_stop = self.check_scylla_version_in_housekeepingdb(prev_id=0, expected_status_code=expected_housekeeping_status_code, new_row_expected=False, backend=backend)
        ...
        with self.logged_subtest("check Scylla server after restart"):
            ...
            if not self.node.is_nonroot_install and backend != "docker":
                ...
                self.check_housekeeping_service_status(backend=backend)
                self.check_scylla_version_in_housekeepingdb(prev_id=version_id_after_stop, expected_status_code=expected_housekeeping_status_code, new_row_expected=True, backend=backend)
    ...
    def run_scylla_doctor(self):
        if self.params.get("client_encrypt") and SkipPerIssues("https://github.com/scylladb/field-engineering/issues/2280", self.params):
            ...
            self.actions_log.info("Scylla Doctor test is skipped for encrypted environment due to issue field-engineering#2280")
            return
