...
class MixedShardRepairDense(MixedShardRepairBase):
    ...
    def setUp(self):
        super().setUp()
        self.populate_data_parallel(size_in_gb=1024, replication_factor=2, blocking=True)

class MixedShardRepairSparse(MixedShardRepairBase):

    def setUp(self):
        super().setUp()
        self.populate_data_parallel_mb(size_in_mb=50, replication_factor=2, blocking=True)
