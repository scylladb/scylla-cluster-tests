...
def estimate_cost(config_files, backend, duration, output_format, output_file):
    ...
    try:
        ...
        unavailable = RunCostEstimate.unavailable().as_dict()
        if output_file:
            with open(output_file, "w", encoding="utf-8") as fobj:
                json.dump(unavailable, fobj)
        if output_format == "json":
            # A caller that asked for JSON parses stdout. Answering a failure in prose would
            # break it on exactly the configs that already have a problem — so keep the
            # contract and say "no estimate" in the format that was asked for.
            click.echo(json.dumps(unavailable))
        else:
    ...
    # The duration drives the whole figure, and it comes from config rather than from anything
    # the reader chose here — so state it. A cost with no timespan attached invites being read
    # as a rate, or as the cost of something shorter than the run actually is.
    lifecycle = "spot" if estimate.is_spot else "on-demand"
    basis = f"{estimate.duration_hours:,.2f} hours at {lifecycle} rates"
    # Only mention the ceiling when the run can actually reach it. With fallback disabled the
    # spot figure is the whole story, and naming a higher number would be noise.
    if estimate.may_fall_back:
        basis += f"; up to ${estimate.on_demand_total:,.2f} if it falls back to on-demand"
    headline = f"Estimated cost of this run: {total} ({basis})"
    if estimate.partial:
