...
def list_images(
    cloud_provider: str, branch: str, version: str, regions: List[str], arch: AwsArchType, output_format: str = "table"  # noqa: PLR0912
):
    if len(regions) == 0:
...
def integration_tests(test, n):
    ...
    sys.exit(pytest.main(["-v", "-m", "integration", f"-n{n}", *(f"unit_tests/{t}" for t in test)]))
