...
@click.option(
    ...
    "--version",
    type=str,
    help="List images by version. Use '-v all' for all versions. OSS format: <4.3> Enterprise format: <enterprise-2021.1>. Mutually exclusive with --branch.",
)
...
def show_jepsen_results(test_id):
    ...
    if jepsen.restore_jepsen_data(test_id):
        click.secho(
            message=f"\nJepsen data restored, starting web server on http://{SCT_RUNNER_HOST}:{jepsen.jepsen_results_port}/",
            fg="green",
        )
