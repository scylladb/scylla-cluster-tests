...
def list_resources(ctx, user, billing_project, test_id, get_all, get_all_running, verbose, backend_type, xcloud_envs):  # noqa: PLR0912, PLR0914, PLR0915
    ...
    def list_resources_on_aws():
        ...
        if elastic_ips_aws:
            aws_table = PrettyTable(["AllocationId", "PublicIP", "TestId", "RunByUser",
                                    "BillingProject", "InstanceId (attached to)"])
            aws_table.align = "l"
        ...
        if security_groups:
            aws_table = PrettyTable(["Name", "Id", "TestId", "RunByUser", "BillingProject"])
            aws_table.align = "l"
            aws_table.sortby = 'Id'
    ...
    def list_resources_on_eks():
        ...
        if eks_clusters:
            eks_table = PrettyTable(["Name", "TestId", "Region", "RunByUser", "BillingProject", "CreateTime"])
            eks_table.align = "l"
        ...
        if load_balancers:
            aws_table = PrettyTable(["Name", "Region", "TestId", "RunByUser", "BillingProject"])
            aws_table.align = "l"
    ...
    def list_resources_on_gke():
        ...
        if gke_clusters:
            gke_table = PrettyTable(["Name", "Region-AZ", "TestId", "RunByUser", "BillingProject", "CreateTime"])
            gke_table.align = "l"
        ...
        if any(docker_resources.values()):
            if docker_resources.get("containers"):
                docker_table = PrettyTable(["Name", "Builder", "Public IP" if get_all_running else "Status",
                                            "TestId", "RunByUser", "BillingProject", "Created"])
            ...
            if docker_resources.get("images"):
                docker_table = PrettyTable(["Name", "Builder", "TestId", "RunByUser", "BillingProject", "Created"])
                docker_table.align = "l"
                docker_table.sortby = "Created"
    ...
    def list_resources_on_azure():
        ...
        if instances:
            azure_table = PrettyTable(["Name", "Region-AZ", "PublicIP", "TestId",
                                      "RunByUser", "BillingProject", "LaunchTime"])
            azure_table.align = "l"
                ...
                if clusters:
                    ...
                    if filtered_clusters:
                        xcloud_table = PrettyTable(["Name", "Environment", "Status",
                                                   "Provider", "TestId", "RunByUser", "BillingProject", "CreatedAt"])
...
@click.option('--billing-project', required=False, type=str, default="", help="Billing project tag filter (currently not applied)")
...
def clean_runner_instances(runner_ip, test_status, backend, user, billing_project, test_id, dry_run, force):
    add_file_logger()
    # Note: clean_sct_runners does not currently support BillingProject; option accepted for CLI parity.
    clean_sct_runners(
