...
from sdcm.sct_runner import AwsSctRunner, GceSctRunner, AzureSctRunner, get_sct_runner, clean_sct_runners, update_sct_runner_tags, list_sct_runners
...
from sdcm.send_email import get_running_instances_for_email_report, read_email_data_from_file, build_reporter, send_perf_email
...
def sct_option(name, sct_name, **kwargs):
    ...
    return click.option(name, type=kwargs.get("type", sct_opt["type"]), help=kwargs.get("help", sct_opt["help"]), multiple=multimple_use)
...
def cloud_provider_option(function=None, default: str | None = DEFAULT_CLOUD, required: bool = True, help: str = "Cloud provider"):
    def actual_decorator(func):
        return click.option("-c", "--cloud-provider", required=required, type=click.Choice(choices=SUPPORTED_CLOUDS, case_sensitive=False), default=default, is_eager=True, help=help)(func)
...
@click.group()
@click.option("--install-bash-completion", is_flag=True, callback=install_callback, expose_value=False, help="Install completion for the current shell. Make sure to have psutil installed.")
@click.option(
...
def cli(ctx):
    ...
    if ctx.invoked_subcommand not in ("update-conf-docs", "conf-docs", "nemesis-list", "create-nemesis-pipelines", "create-nemesis-yaml", "pre-commit"):
        try_auth_with_okta()

        key_store = KeyStore()
        # TODO: still leaving old keys, until we'll rebuild runner images - and reconfigure jenkins
        key_store.sync(keys=["scylla-qa-ec2", "scylla-test", "scylla_test_id_ed25519"], local_path=Path("~/.ssh/").expanduser(), permissions=0o0600)
...
@click.option("-b", "--backend", type=click.Choice(SCTConfiguration.available_backends), help="Backend to use")
@click.option("--clean-runners", is_flag=True, default=False, help="Include SCT runner instances in cleanup (requires --user or --test-id to avoid accidental deletion of all SCT runners)")
@click.pass_context
def clean_resources(ctx, post_behavior, user, test_id, logdir, dry_run, backend, clean_runners):
    ...
    if clean_runners and not user and not test_id:
        click.echo("ERROR: --clean-runners requires --user and/or --test-id, to prevent accidentally deleting all SCT runners", err=True)
        ctx.exit(1)
    ...
    for param in params:
        ...
        if clean_runners:
            ...
            clean_sct_runners(test_status="", test_runner_ip=None, backend=backend, user=param.get("RunByUser"), test_id=param.get("TestId"), dry_run=dry_run, force=True)
...
def list_resources(ctx, user, test_id, get_all, get_all_running, verbose, backend_type, xcloud_envs):  # noqa: PLR0912, PLR0914, PLR0915
    ...
    def list_resources_on_aws():
        ...
        if aws_instances:
            ...
            for instance in aws_instances:
                ...
                aws_table.add_row([name, instance["Placement"]["AvailabilityZone"], instance.get("PublicIpAddress", "N/A") if get_all_running else instance["State"]["Name"], test_id, run_by_user, instance["LaunchTime"].ctime()])
        ...
        if elastic_ips_aws:
            ...
            for eip in elastic_ips_aws:
                ...
                test_id = tags.get("TestId", "N/A")
                run_by_user = tags.get("RunByUser", "N/A")
                aws_table.add_row([eip["AllocationId"], eip["PublicIp"], test_id, run_by_user, eip.get("InstanceId", "N/A")])
    ...
    def list_resources_on_docker():
        ...
        docker_resources = list_resources_docker(tags_dict=params, running=get_all_running, group_as_builder=True, verbose=verbose)

        if any(docker_resources.values()):
            if docker_resources.get("containers"):
                docker_table = PrettyTable(["Name", "Builder", "Public IP" if get_all_running else "Status", "TestId", "RunByUser", "Created"])
    ...
    def list_resources_on_azure():
        ...
        if instances:
            ...
            for instance in instances:
                creation_time = instance.creation_time.isoformat(sep=" ", timespec="seconds") if instance.creation_time else "N/A"
                tags = instance.tags
                test_id = tags.get("TestId", "N/A")
                run_by_user = tags.get("RunByUser", "N/A")
                azure_table.add_row([instance.name, instance.region, instance.public_ip_address, test_id, run_by_user, creation_time])
            ...
            try:
                ...
                if clusters:
                    ...
                    for cluster in clusters:
                        ...
                        cluster_details = api_client.get_cluster_details(account_id=account_id, cluster_id=cluster.get("id"), enriched=True)
                    ...
                    if filtered_clusters:
                        xcloud_table = PrettyTable(["Name", "Environment", "Status", "Provider", "TestId", "RunByUser", "CreatedAt"])
                        ...
                        for cluster in filtered_clusters:
                            ...
                            xcloud_table.add_row([cluster_name, environment, cluster_status, cloud_provider, short_test_id, cluster_user, created_at])

                        click.echo(xcloud_table.get_string(title=f"ScyllaDB Cloud clusters ({environment})"))
                    else:
                        click.secho(f"Nothing found for selected filters in ScyllaDB Cloud ({environment})!", fg="yellow")
                else:
...
@click.option("-br", "--branch", type=str, help="Branch to query images for. Defaults to 'master:latest' Mutually exclusive with --version.")
@click.option("-v", "--version", type=str, help="List images by version. Use '-v all' for all versions. OSS format: <4.3> Enterprise format: <enterprise-2021.1>. Mutually exclusive with --branch.")
@click.option("-r", "--region", "regions", type=CloudRegion(), help="Cloud region to query images in", multiple=True)
@click.option("-a", "--arch", type=click.Choice(AwsArchType.__args__), default="x86_64", help="architecture of the AMI (default: x86_64)")
        ...
        if version is not None:
            match cloud_provider:
                case "aws":
                    ...
                    if output_format == "table":
                        click.echo(create_pretty_table(rows=rows, field_names=version_fields_with_tag_name).get_string(title=f"AWS Machine Images by Version in region {region}"))
                    elif output_format == "text":
                        ami_images_json = images_dict_in_json_format(rows=rows, field_names=version_fields_with_tag_name)
                ...
                case "gce":
                    ...
                    if output_format == "table":
                        click.echo(create_pretty_table(rows=rows, field_names=version_fields).get_string(title="GCE Machine Images by version"))
                    elif output_format == "text":
                ...
                case "azure":
                    ...
                    if output_format == "table":
                        click.echo(create_pretty_table(rows=rows, field_names=version_fields).get_string(title="Azure Machine Images by version"))
                    elif output_format == "text":
            ...
            match cloud_provider:
                case "aws":
                    ...
                    if output_format == "table":
                        click.echo(create_pretty_table(rows=ami_images, field_names=branch_fields_with_tag_name).get_string(title=f"AMI Machine Images for {branch} in region {region}"))
                    elif output_format == "text":
                        ami_images_json = images_dict_in_json_format(rows=ami_images, field_names=branch_fields_with_tag_name)
                        click.echo(ami_images_json)
                case "gce":
                    gce_images = get_gce_images(branch=branch, arch=arch)
                    if output_format == "table":
                        click.echo(create_pretty_table(rows=gce_images, field_names=branch_fields).get_string(title=f"GCE Machine Images for {branch}"))
                    elif output_format == "text":
...
@click.option("-r", "--target-region", "target_regions", type=str, multiple=True, help="Target region(s) to search for equivalents. Can be specified multiple times. If not specified, searches in source region only.")
@click.option("-a", "--target-arch", type=click.Choice(AwsArchType.__args__), help="Target architecture (x86_64 or arm64). If not specified, uses same arch as source AMI.")
@click.option("-o", "--output-format", type=click.Choice(["table", "json", "text"]), default="table", help="Output format: 'table' for human-readable table, 'json' for structured data, or 'text' for AMI IDs only")
def find_ami_equivalent(ami_id: str, source_region: str, target_regions: tuple[str, ...], target_arch: AwsArchType | None, output_format: str):
    ...
    results = find_equivalent_ami(ami_id=ami_id, source_region=source_region, target_regions=target_regions_list, target_arch=target_arch)
    ...
    if output_format == "table":
        ...
        field_names = ["Region", "AMI ID", "Name", "Architecture", "Creation Date", "Name Tag", "Scylla Version", "Build ID", "Owner ID"]
        table = PrettyTable(field_names)
        table.align = "l"

        for result in results:
            table.add_row(
                [result["region"], result["ami_id"], result["name"], result["architecture"], result["creation_date"], result["name_tag"], result["scylla_version"], result["build_id"][:6] if result["build_id"] else "N/A", result["owner_id"]]
            )
        ...
        output = {"source_ami_id": ami_id, "source_region": source_region, "target_arch": target_arch, "results": results}
...
def show_log(test_id, output_format, update_argus: bool):
    ...
    if update_argus:
        try:
            store_logs_in_argus(test_id=test_id, logs=reduce(lambda acc, log: acc[log["type"]].append(log["link"]) or acc, files, defaultdict(list)), update=True)
...
def show_jepsen_results(test_id):
    ...
    if jepsen.restore_jepsen_data(test_id):
        click.secho(message=f"\nJepsen data restored, starting web server on http://{SCT_RUNNER_HOST}:{jepsen.jepsen_results_port}/", fg="green")
...
def show_events(test_id: str, follow: bool = False, last_n: int = None, save_to: str = None):
    ...
    for builder in builders:
        LOGGER.info("Applying action for events.log on builder %s:%s...", builder["builder"]["name"], builder["builder"]["public_ip"])
...
@click.option("-t", "--test", required=False, default=[""], multiple=True, help="Run specific test file from unit-tests directory")
...
def integration_tests(test, n):
    ...
    sys.exit(pytest.main(["-v", "-m", "integration", "--dist", "loadgroup", f"-n{n}", *(f"unit_tests/{t}" for t in test)]))
...
@click.option("-e", "--emails", required=True, type=str, help="Comma separated list of emails. Example a@b.com,c@d.com")
@click.option("-t", "--report-type", required=True, type=click.Choice(choices=["general", "last-7-days-qa", "last-7-days-non-qa"], case_sensitive=False), help="Type of the report")
@click.option("-u", "--user", required=False, type=str, default="", help="User or instance owner. Applicable for last-7-days-* reports")
...
def collect_logs(test_id=None, logdir=None, backend=None, config_file=None):
    ...
    for cluster_type, s3_links in collected_logs.items():
        for link in s3_links:
            ...
            table.add_row([current_cluster_type, create_proxy_argus_s3_url(link).format(collector.test_id, link.split("/")[-1])])
...
def send_email(
    ...
    if not test_results:
        ...
        if test_id:
            test_results.update({"test_id": test_id, "nodes": get_running_instances_for_email_report(test_id, runner_ip), "log_links": list_logs_by_test_id(test_id)})
...
def create_operator_test_release_jobs(branch, username, password, sct_branch, sct_repo, triggers):
    ...
    server = JenkinsPipelines(username=username, password=password, base_job_dir=base_job_dir, sct_branch_name=sct_branch, sct_repo=sct_repo)
    server.create_job_tree(f"{server.base_sct_dir}/jenkins-pipelines/operator", create_freestyle_jobs=triggers, template_context={"release_version": get_latest_scylla_release(product="scylla-enterprise")})
...
def create_qa_tools_jobs(username, password, sct_branch, sct_repo, triggers):
    ...
    server = JenkinsPipelines(username=username, password=password, base_job_dir=base_job_dir, sct_branch_name=sct_branch, sct_repo=sct_repo)
    server.create_job_tree(f"{server.base_sct_dir}/jenkins-pipelines/qa", create_freestyle_jobs=triggers, job_name_suffix="")
...
def create_performance_jobs(username, password, sct_branch, sct_repo, triggers):
    ...
    base_job_dir = ""
    server = JenkinsPipelines(username=username, password=password, base_job_dir=base_job_dir, sct_branch_name=sct_branch, sct_repo=sct_repo)
    server.create_job_tree(f"{server.base_sct_dir}/jenkins-pipelines/performance/{sct_branch}", create_freestyle_jobs=triggers, job_name_suffix="")
...
@click.option("-r", "--regions", type=CloudRegion(cloud_provider="aws"), default=[], help="Cloud regions", multiple=True)
...
def create_runner_image(cloud_provider, region, availability_zone):
    ...
    sct_config = SCTConfiguration()
    sct_runner = get_sct_runner(cloud_provider=cloud_provider, region_name=region, availability_zone=availability_zone, params=sct_config)
...
@click.option("-rm", "--restore-monitor", required=False, type=bool, help="Is the runner for restore monitor purpose or not")
@click.option("-rt", "--restored-test-id", required=False, type=str, help="Test ID of the test that the runner is created for restore monitor")
@click.option("-p", "--address-pool", required=False, type=str, help="ElasticIP pool to use")
def create_runner_instance(cloud_provider, region, availability_zone, instance_type, root_disk_size_gb, test_id, test_name, duration, restore_monitor=False, restored_test_id="", address_pool=None):
    if cloud_provider == "aws":
        assert len(availability_zone) == 1, f"Invalid AZ: {availability_zone}, availability-zone is one-letter a-z."
    add_file_logger()
...
@click.option("-t", "--tags", type=(str, str), help="Space separated key value pair to add as a new tag to the runner", multiple=True)
...
def clean_runner_instances(runner_ip, test_status, backend, user, test_id, dry_run, force):
    add_file_logger()
    clean_sct_runners(test_runner_ip=runner_ip, test_status=test_status, backend=backend, user=user, test_id=test_id, dry_run=dry_run, force=force)
...
@click.option("-f", "--mapping-file", envvar="SCT_MAPPING_FILEPATH", type=click.Path(exists=True), required=True, help="Full path to es index mapping file")
...
@click.argument("runner-ip", type=str)
@click.option("-b", "--backend", type=click.Choice(SCTConfiguration.available_backends), help="Allows to skip making backend detection API calls.")
def fetch_junit(runner_ip, backend):
    ...
    cmd = f'ssh -tt {proxy_command} -i {target_key} -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o ServerAliveInterval=10 {target_username}@{target_ip} \'find ~/sct-results -iname junit.xml | xargs cat\''
...
@click.option("--test-id", type=str, required=False, help="If hdr_folder is not provided, logs will be downloaded from argus using this test_id")
@click.option(
    "--stress-tool", default="cassandra-stress", required=False, type=click.Choice(["cassandra-stress", "scylla-bench", "latte"], case_sensitive=False), help="stress tool name. Supported tools: cassandra-stress|scylla-bench|latte"
)
@click.option("--stress-operation", required=True, type=click.Choice(["READ", "WRITE"], case_sensitive=False), help="Supported stress operations: READ|WRITE")
...
@click.option("--duration-from-start-min", type=int, required=True, help="Time period in minutes in HDR file to investigate, started from start-time ")
@click.option("--error-threshold-ms", type=int, default=10, required=False, help="Error threshold in ms for P99 to consider it as a spike")
...
def hdr_investigate(test_id: str, stress_tool: str, stress_operation: str, throttled_load: bool, start_time: str, duration_from_start_min: int, error_threshold_ms: int, hdr_summary_interval_sec: int, hdr_folder: str) -> None:
    """
    ...
    hdr_summaries = make_hdrhistogram_summary_by_interval(hdr_tags, stress_operation, hdr_folder, start_time_ms, end_time_ms, interval=hdr_summary_interval_sec)
    ...
    for tag in hdr_tags:
        ...
        tag_summaries = [s for s in tag_hdr_summaries if s and "percentile_99" in s and s["percentile_99"] > error_threshold_ms]
