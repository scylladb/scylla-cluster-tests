    ...
    def test_custom_time(self):
        ...
        with self.db_cluster.cql_connection_patient(node=self.db_cluster.nodes[0], user=loader_utils.DEFAULT_USER, password=loader_utils.DEFAULT_USER_PASSWORD) as session:
            ...
            for index, shares in enumerate(self.service_level_shares):
                self.roles.append(create_sla_auth(session=session, shares=shares, index=str(index), attach_service_level=is_enterprise))

            if self.params.get("run_fullscan"):
                self.fullscan_role = create_sla_auth(session=session, shares=self.FULLSCAN_SERVICE_LEVEL_SHARES, index="0")
        ...
        for role in self.roles + [self.fullscan_role]:
            ...
            if role and is_enterprise:
                with adaptive_timeout(Operations.SERVICE_LEVEL_PROPAGATION, node=self.db_cluster.data_nodes[0], timeout=15, service_level_for_test_step="MAIN_SERVICE_LEVEL"):
                    SlaUtils().wait_for_service_level_propagated(cluster=self.db_cluster, service_level=role.attached_service_level)

        self.add_sla_credentials_to_stress_cmds(workload_names=["prepare_write_cmd", "stress_cmd", "stress_read_cmd"], roles=self.roles, params=self.params, parent_class_name=self.__class__.__name__)
