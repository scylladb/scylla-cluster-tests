...
class LongevityBalancerTest(LongevityTest):
    ...
    def run_bigger_load(self):
        ...
        self.hdr_tags.extend(tag for s in stress_queue for tag in s.hdr_tags)
        self.calculate_latencies(row_name='scaled_load_scaled_cluster')
        for stress in stress_queue:
            self.verify_stress_thread(stress)
    ...
    def calculate_latencies(self, row_name: str):
        ...
        sleep(120)  # postpone measure steady state latency to skip c-s start period when latency is high
        latency_calculator_decorator(cycle_name='workload_latencies',
                                     row_name=row_name)(lambda _: sleep(sleep_time))(self)
    ...
    def test_workload_latencies(self):
        ...
        # get steady latency for base load with new cluster configuration
        ...
        # get steady latency for base load with original cluster configuration
        self.calculate_latencies(row_name='base_load_base_cluster_restored')  # new row name is needed to not overwrite
