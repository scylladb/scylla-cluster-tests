...
class MaximumPerformanceSearchTest(PerformanceRegressionTest):
    def test_search_best_read_throughput(self):
        stress_params = {"stress_cmd_tmpl": self.params.get("stress_cmd_r"), "test_name": "Test read"}
    ...
    def test_search_best_write_throughput(self):
        stress_params = {"stress_cmd_tmpl": self.params.get("stress_cmd_w"), "test_name": "Test write"}
        stress_params.update(**self._get_stress_parameters())
        # run a write workload as a preparation
        self.preload_data()
        self.wait_no_compactions_running(n=400, sleep_time=120)
    ...
    def test_search_best_mixed_throughput(self):
        stress_params = {"stress_cmd_tmpl": self.params.get("stress_cmd_m"), "test_name": "Test mixed"}
        stress_params.update(**self._get_stress_parameters())
        # run a write workload as a preparation
        self.preload_data()
        self.wait_no_compactions_running(n=400, sleep_time=120)
    ...
    def run_search_best_performance(
        self,
        stress_cmd_tmpl: str,  # noqa: PLR0914
        stress_num: int,
        stress_num_step: int,
        ...
        while len(self.loaders.nodes) > 0:
            stress_cmd = self._build_stress_command(cmd_tmpl=stress_cmd_tmpl, threads=threads, duration=stress_step_duration)
            ...
            current_result.update({"threads": threads, "n_loaders": len(self.loaders.nodes), "n_process": stress_num, "total_threads": threads * stress_num * len(self.loaders.nodes), "stress_cmd": stress_cmd})
            ...
            if current_result["op rate"] > best_result["op rate"] + deviation:
                ...
                best_result = current_result
                self.log.info("Current throughput: %s larger than best: %s. Increase threads", current_result["op rate"], best_result["op rate"])
                threads += threads_step
            elif current_result["op rate"] >= best_result["op rate"] - deviation and current_result["op rate"] <= best_result["op rate"] + deviation:
        ...
        analyzer = SearchBestThroughputConfigPerformanceAnalyzer(es_index=self._test_index, email_recipients=self.params.get("email_recipients"))
