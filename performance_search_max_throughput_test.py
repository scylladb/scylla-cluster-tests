...
class MaximumPerformanceSearchTest(PerformanceRegressionTest):
    ...
    def run_search_best_performance(  # pylint: disable=too-many-arguments,too-many-locals,too-many-statements
        self,
        stress_cmd_tmpl: str,
        stress_num: int,
        stress_num_step: int,
        ...
        while len(self.loaders.nodes) > 0:
            stress_cmd = self._build_stress_command(
                cmd_tmpl=stress_cmd_tmpl, threads=threads, duration=stress_step_duration
            )
            ...
            if current_result["op rate"] > best_result["op rate"] + deviation:
                ...
                best_result = current_result
                self.log.info(
                    "Current throughput: %s larger than best: %s. Increase threads",
                ...
                and current_result["op rate"] <= best_result["op rate"] + deviation
            ):
