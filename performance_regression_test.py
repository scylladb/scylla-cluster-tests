    ...
    def setUp(self):
        ...
        if self.params.get("run_db_node_benchmarks"):
            ...
            if not all(ready_nodes):
                ...
                HWPerforanceEvent(message="DB cluster passed hardware performance test", severity=Severity.NORMAL).publish()
...
""" % (
    ...
    def display_results(self, results, test_name=""):
        self.log.info(self.str_pattern, "op-rate", "partition-rate", "row-rate", "latency-mean", "latency-median", "l-94th-pct", "l-99th-pct", "l-99.9th-pct", "total-partitions", "total-err")
    ...
    def _workload(self, stress_cmd, stress_num, test_name, sub_type=None, keyspace_num=1, prefix="", debug_message="", save_stats=True):
        ...
        stress_queue = self.run_stress_thread(stress_cmd=stress_cmd, stress_num=stress_num, keyspace_num=keyspace_num, prefix=prefix, stats_aggregate_cmds=False)
    ...
    def run_read_workload(self, nemesis=False):
        ...
        if nemesis:
            ...
            self.db_cluster.add_nemesis(nemesis=self.get_nemesis_class(), tester_obj=self, hdr_tags=stress_queue.hdr_tags)
    ...
    def prepare_mv(self, on_populated=False):
        with self.db_cluster.cql_connection_patient_exclusive(self.db_cluster.nodes[0]) as session:
            ...
            create_materialized_view(session, ks_name, base_table_name, view_name, ["C0"], ["key"], mv_columns=["C0", "key"])
    ...
    def _write_with_mv(self, on_populated):
        ...
        ops_without_mv = self._workload(stress_cmd=base_cmd_w, stress_num=2, sub_type="write_without_mv", test_name=test_name, keyspace_num=1, debug_message="First write cassandra-stress command: {}".format(base_cmd_w))

        # Create MV
        self.prepare_mv(on_populated=on_populated)

        # Start cassandra-stress writes again now with MV
        ops_with_mv = self._workload(stress_cmd=base_cmd_w, stress_num=2, sub_type="write_with_mv", test_name=test_name, keyspace_num=1, debug_message="Second write cassandra-stress command: {}".format(base_cmd_w))

        self.assert_mv_performance(ops_without_mv, ops_with_mv, "Throughput of run with materialized view is more than {} times lower then throughput of run without materialized view".format(self.ops_threshold_prc / 100))
    ...
    def _read_with_mv(self, on_populated):
        ...
        self._workload(stress_cmd=base_cmd_p, stress_num=2, test_name=test_name, prefix="preload-", keyspace_num=1, debug_message="Prepare the test, run cassandra-stress command: {}".format(base_cmd_p), save_stats=False)

        # run a read workload
        ops_without_mv = self._workload(stress_cmd=base_cmd_r, stress_num=2, sub_type="read_without_mv", test_name=test_name, keyspace_num=1, debug_message="First read cassandra-stress command: {}".format(base_cmd_r))

        self.prepare_mv(on_populated=on_populated)

        # If the MV was created on the empty base table, populate it before reads
        if not on_populated:
            self._workload(stress_cmd=base_cmd_w, stress_num=2, test_name=test_name, prefix="preload-", keyspace_num=1, debug_message="Prepare test before second cassandra-stress command: {}".format(base_cmd_w), save_stats=False)

        # run a read workload
        ops_with_mv = self._workload(stress_cmd=base_cmd_r, stress_num=2, sub_type="read_with_mv", test_name=test_name, keyspace_num=1, debug_message="Second read cassandra-stress command: {}".format(base_cmd_r))
    ...
    def _mixed_with_mv(self, on_populated):
        ...
        self._workload(stress_cmd=base_cmd_p, stress_num=2, test_name=test_name, keyspace_num=1, prefix="preload-", debug_message="Prepare the test, run cassandra-stress command: {}".format(base_cmd_p), save_stats=False)

        # run a mixed workload without MV
        ops_without_mv = self._workload(stress_cmd=base_cmd_m, stress_num=2, sub_type="mixed_without_mv", test_name=test_name, keyspace_num=1, debug_message="First mixed cassandra-stress command: {}".format(base_cmd_m))

        self.prepare_mv(on_populated=on_populated)

        # run a mixed workload with MV
        ops_with_mv = self._workload(stress_cmd=base_cmd_p, stress_num=2, sub_type="mixed_with_mv", test_name=test_name, keyspace_num=1, debug_message="Second start of mixed cassandra-stress command: {}".format(base_cmd_p))

        self.assert_mv_performance(ops_without_mv, ops_with_mv, "Throughput of stress run with materialized view is more than {} times lower then throughput of stress run without materialized view".format(self.ops_threshold_prc / 100))
    ...
    def test_write(self):
        ...
        # run a workload
        stress_queue = self.run_stress_thread(stress_cmd=base_cmd_w, stress_num=stress_multiplier, stats_aggregate_cmds=False)
        results = self.get_stress_results(queue=stress_queue)
    ...
    def test_read(self):
        ...
        self.run_fstrim_on_all_db_nodes()
        # run a read workload
        stress_queue = self.run_stress_thread(stress_cmd=base_cmd_r, stress_num=stress_multiplier, stats_aggregate_cmds=False)
        results = self.get_stress_results(queue=stress_queue)
    ...
    def test_mixed(self):
        ...
        self.wait_no_compactions_running(n=240, sleep_time=180)
        self.run_fstrim_on_all_db_nodes()
        stress_queue = self.run_stress_thread(stress_cmd=base_cmd_m, stress_num=stress_multiplier, stats_aggregate_cmds=False)
        results = self.get_stress_results(queue=stress_queue)
    ...
    def test_latency_steady_state(self):
        ...
        object_set = ParallelObject(timeout=None, objects=[[_stress_op, self, _stress_queue] for _stress_op, _stress_queue in stress_operation_mapping.items()], num_workers=len(stress_operation_mapping))
    ...
    def test_mv_write(self):
        ...
        def run_workload(stress_cmd, user_profile):
            ...
            stress_queue = self.run_stress_thread(stress_cmd=stress_cmd, stress_num=1, profile=user_profile, stats_aggregate_cmds=False)
            results = self.get_stress_results(queue=stress_queue)
            self.update_test_details(scylla_conf=True)
            self.display_results(results, test_name=test_name)
            self.check_regression()
        ...
        def get_mv_name(user_profile):
            ...
            if not mv_name:
                assert False, "Failed to recognoze materialized view name from {0}: {1}".format(user_profile, user_profile_yaml)
    ...
    def test_uniform_counter_update_bench(self):
        ...
        """
        base_cmd_r = "scylla-bench -workload uniform -mode counter_update -duration 30m -partition-count 50000000 -clustering-row-count 1 -connection-count 32 -concurrency 512 -replication-factor 3"
    ...
    def test_timeseries_bench(self):
        ...
        """
        cmd_w = "scylla-bench -workload=timeseries -mode=write -replication-factor=3 -partition-count=5000 -clustering-row-count=1000000 -clustering-row-size=200 -concurrency=48 -max-rate=150000 -rows-per-request=5000"
    ...
    def build_histogram(self, workload: str, hdr_tags: list):
        ...
        self.update_hdrhistograms(histogram_name="test_histogram_by_interval", histogram_data=histogram_data_by_interval)
...
class PerformanceRegressionUpgradeTest(PerformanceRegressionTest, UpgradeTest):
    ...
    def run_workload_and_upgrade(self, stress_cmd, sub_type=None):
        ...
        for node in self.db_cluster.nodes:
            ...
            versions_list.append({"base_version": base_version, "base_build_id": base_build_id, "target_version": target_version, "target_build_id": target_build_id, "node_name": node.name})
...
class PerformanceRegressionMaterializedViewLatencyTest(PerformanceRegressionTest):
    ...
    def test_read_mv_latency(self):
        ...
        stress_queue = self.run_stress_thread(stress_cmd=self.params.get("stress_cmd_r"), stress_num=1, stats_aggregate_cmds=False)
