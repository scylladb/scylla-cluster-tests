...
class SstablesValidator(TeardownValidator):
    ...
    def _upload_corrupted_files(node: BaseNode, invalid_sstables_lines):
        ...
        s3_link = upload_remote_files_directly_to_s3(
            node.ssh_login_info, list(corrupted_sstables), s3_bucket=S3Storage.bucket_name, s3_key=f"{node.parent_cluster.uuid}/{node.name}-corrupted-sstables.tar.gz", max_size_gb=30, public_read_acl=True
        )
        return s3_link
    ...
    def _run_nodetool_scrub(self, node: BaseNode, keyspace: str, table: str, timeout=1200):
        ...
        result = node.run_nodetool(sub_cmd="scrub", args=f"--mode VALIDATE --no-snapshot {keyspace} {table}".strip(), timeout=timeout, coredump_on_timeout=False, ignore_status=True)
        if not result.ok:
            ValidatorEvent(message=f"Error running nodetool scrub on node {node.name}: {result.stdout}\n{result.stderr}", severity=Severity.ERROR).publish()
        # sometimes logs might be delayed, so we need to wait for them
        scrub_finish_lines = wait.wait_for(func=lambda: list(finish_scrub_follower), step=10, text="Waiting for 'Finished scrubbing in validate mode' logs", timeout=300, throw_exc=False)
        if not scrub_finish_lines:
            ValidatorEvent(message=f"No scrubbing validation message found in db logs on node: {node.name}", severity=Severity.ERROR).publish()
            return
    ...
    def validate(self):
        for cluster in self.tester.db_clusters_multitenant:
            ...
            try:
                ...
                ValidatorEvent(message=f"Error during nodetool scrub validation: {exc}", severity=Severity.ERROR).publish()
