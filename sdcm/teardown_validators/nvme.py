...
class NvmeValidator(TeardownValidator):
    ...
    def _run_self_tests(self):
        ...
        if not nodes:
            LOGGER.info("No DB nodes available for NVMe self-tests")
            return

        LOGGER.info("Running NVMe self-tests (type=%s) on %d DB nodes", test_type.name, len(nodes))

        # run_self_test_on_all_devices triggers the test on all data disks of a node
        # before polling, so a node takes ~per_node_timeout regardless of disk count
        per_node_timeout = SHORT_SELF_TEST_TIMEOUT if test_type == SelfTestType.SHORT else EXTENDED_SELF_TEST_TIMEOUT
        # Allow extra margin for device discovery and polling overhead
        overall_timeout = per_node_timeout * 2
        with ThreadPoolExecutor(max_workers=min(len(nodes), 10)) as executor:
            futures = {
                executor.submit(run_self_test_on_all_devices, node, test_type, per_node_timeout): node for node in nodes
            }
            try:
