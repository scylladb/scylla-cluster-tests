...
class RackawareValidator(TeardownValidator):  # pylint: disable=too-few-public-methods
    ...
    def validate(self):
        ...
        for region in self.tester.db_cluster.datacenter:
            (non_rack_db_nodes, rack_db_nodes) = self.map_nodes_per_rack_with_and_without_loader(
                db_nodes=db_nodes_per_region[region], loader_nodes=loaders_per_region[region]
            )
            ...
            if not (non_rack_db_nodes or rack_db_nodes):
                ValidatorEvent(
                    message="Rackaware validation. Unable to retrieve node list filtered by loader AZ, or all nodes when loader is absent.",
                    severity=Severity.ERROR,
                ).publish()
                validation_passed.add(False)
                continue

            validation_passed.add(
                self.one_region_validate(non_rack_db_nodes=non_rack_db_nodes, rack_db_nodes=rack_db_nodes)
            )
    ...
    def one_region_validate(self, non_rack_db_nodes, rack_db_nodes):
        ...
        for db_node_ip in non_rack_db_nodes:
            ...
            LOGGER.debug(
                "Node %s. Non-system CQl read amounts are being routed to a node in a loader-less AZ: %s",
                db_node_ip,
                cql_reads,
            )
        ...
        for db_node_ip in rack_db_nodes:
            cql_reads = max(0, self.get_cql_reads(db_node_ip=db_node_ip))
            LOGGER.debug(
                "Node %s. Non-system CQl read amounts are being routed to a node in a loader AZ: %s",
                db_node_ip,
                cql_reads,
            )
        ...
        if not (rack_user_cql_reads or non_rack_user_cql_reads):
            ValidatorEvent(
                message="Rackaware validation. Reads (non-system CQL) initiated by the user were not received",
                severity=Severity.ERROR,
            ).publish()
            return False

        non_system_cql_reads_perc = (
            100 * float(non_rack_user_cql_reads) / (float(rack_user_cql_reads) + float(non_rack_user_cql_reads))
        )
        LOGGER.debug(
            "Non-system CQl read amounts are being routed to a node in a loader-less AZ: %s", non_rack_user_cql_reads
        )
    ...
    def get_cql_reads(self, db_node_ip: str):
        ...
        """
        query = (
            'sum(irate(scylla_cql_reads{instance="%s"} [40s]))-sum(irate(scylla_cql_reads_per_ks{instance="%s"} [40s]))'
