...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    @staticmethod
    def stop_nemesis_on_stress_errors(stress_thread: DockerBasedStressThread) -> None:
        stress_results = super(stress_thread.__class__, stress_thread).get_results()
        stress_errors = []
        for _, _, event in stress_results:
            stress_errors.extend(event.errors)
        if stress_errors:
            errors_str = '\n'.join(stress_errors)
            raise NemesisStressFailure(
                f"During '{stress_thread.stress_cmd}' stress command the following errors occurred:\n{errors_str}")
