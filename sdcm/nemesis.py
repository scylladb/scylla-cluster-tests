...
from sdcm.target_node_lock import run_nemesis, set_running_nemesis, unset_running_nemesis, \
    NEMESIS_TARGET_SELECTION_LOCK, lock_nodes, CantAcquireLockException
...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def disrupt_rolling_restart_cluster(self, random_order=False):
        try:
            with lock_nodes(self.cluster.nodes, self.target_node.running_nemesis, timeout=300):
                self.cluster.restart_scylla(nodes=self.cluster.nodes, random_order=random_order)
