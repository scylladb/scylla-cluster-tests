...
from test_lib.compaction import (
    ...
    get_gc_mode,
    GcMode,
    calculate_allowed_twcs_ttl_borders,
...
class Nemesis:
    ...
    def __init__(self, tester_obj, termination_event, *args, nemesis_selector=None, nemesis_seed=None, **kwargs):
        ...
        self.nemesis_registry = NemesisRegistry(base_class=Nemesis, excluded_list=COMPLEX_NEMESIS)
    ...
    def disrupt_restart_then_repair_node(self):
        with (
            ...
            with self.action_log_scope("Restart the node", target=self.target_node.name):
    ...
    def disrupt_load_and_stream(self):
        ...
        self._prepare_test_table(ks="keyspace_refresh", table="standard1")
        ...
        with self.cluster.cql_connection_patient(self.target_node) as session:
            ...
            for ks in keyspaces:
                ...
                tables = get_db_tables(
                    session=session, keyspace_name=ks, node=self.target_node, with_compact_storage=False
                )
                if to_be_skipped:
    ...
    def modify_table_default_time_to_live(self):
        ...
        self._modify_table_property(
            name="default_time_to_live", val=value, filter_out_table_with_counter=True, keyspace_table=keyspace_table
        )
    ...
    def disrupt_abort_repair(self):
        ...
        with ThreadPoolExecutor(max_workers=1, thread_name_prefix="NodeToolRepairThread") as thread_pool:
            ...
            with (
                ...
                with self.action_log_scope("Abort repair streaming", target=self.target_node.name):
