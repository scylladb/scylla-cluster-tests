...
from sdcm.utils.common import get_db_tables, generate_random_string, reach_enospc_on_node, clean_enospc_on_node, parse_nodetool_listsnapshots, update_authenticator, sleep_for_percent_of_duration, get_views_of_base_table
...
from sdcm.utils.k8s.chaos_mesh import MemoryStressExperiment, IOFaultChaosExperiment, DiskError, NetworkDelayExperiment, NetworkPacketLossExperiment, NetworkCorruptExperiment, NetworkBandwidthLimitExperiment
...
from sdcm.utils.replication_strategy_utils import temporary_replication_strategy_setter, NetworkTopologyReplicationStrategy, ReplicationStrategy, SimpleReplicationStrategy
...
from test_lib.compaction import CompactionStrategy, get_compaction_strategy, get_compaction_random_additional_params, get_gc_mode, GcMode, calculate_allowed_twcs_ttl, get_table_compaction_info
...
class Nemesis(NemesisFlags):
    ...
    def __init__(self, tester_obj, termination_event, *args, nemesis_selector=None, nemesis_seed=None, **kwargs):
        ...
        self.nemesis_registry = NemesisRegistry(base_class=Nemesis, flag_class=NemesisFlags, excluded_list=COMPLEX_NEMESIS)
    ...
    def switch_target_node(self, node: BaseNode):
        self.node_allocator.switch_target_node(old_node=self.target_node, new_node=node, nemesis_name=self.current_disruption)
    ...
    def set_target_node(self, dc_idx: Optional[int] = None, rack: Optional[int] = None, is_seed: Union[bool, DefaultValue, None] = DefaultValue, allow_only_last_node_in_rack: bool = False, current_disruption=None):
        ...
        self.target_node = self.node_allocator.select_target_node(
            nemesis_name=disruption_name, pool_type=self._target_node_pool_type, filter_seed=self.filter_seed, is_seed=is_seed, dc_idx=dc_idx, rack=rack, allow_only_last_node_in_rack=allow_only_last_node_in_rack
        )
    ...
    def _kill_scylla_daemon(self):
        with EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=CassandraStressLogEvent, regex=".*Connection reset by peer.*", extra_time_to_expiration=30):
            self.log.info("Kill all scylla processes in %s", self.target_node)
            with DbNodeLogger(self.cluster.nodes, "kill all scylla processes", target_node=self.target_node), self.action_log_scope(f"pkill -9 scylla on {self.target_node.name} node"):
            ...
            if self.cluster.params.get("use_mgmt") and SkipPerIssues("scylladb/scylla-manager#2813", params=self.tester.params):
    ...
    @staticmethod
    def _handle_start_stop_compaction_results(trigger_and_watcher_futures: dict[str, ParallelObjectResult], allow_trigger_exceptions: bool = True):
        """
    ...
    def _get_random_non_system_ks_cf(self, filter_empty_tables: bool = True) -> tuple[str, str] | None:
        ...
        cf_name separately.
        """
        if ks_cf_list := self.cluster.get_non_system_ks_cf_list(db_node=self.target_node, filter_empty_tables=filter_empty_tables):
    ...
    def _prepare_start_stop_compaction(self) -> StartStopCompactionArgs:
        ...
        return StartStopCompactionArgs(keyspace=ks, columnfamily=cf, timeout=360, target_node=self.target_node, compaction_ops=CompactionOps(cluster=self.cluster, node=self.target_node))
    ...
    def disrupt_start_stop_major_compaction(self):
        ...
        trigger_func = partial(compaction_args.compaction_ops.trigger_major_compaction, keyspace=compaction_args.keyspace, cf=compaction_args.columnfamily)
        watch_func = partial(
        ...
        with (
            DbNodeLogger(self.cluster.nodes, "start and stop major compaction", target_node=self.target_node, additional_info=f"on {ks_cf}"),
            self.action_log_scope(f"start and stop major compaction on {ks_cf} table on {self.target_node.name} node"),
        ):
            results = ParallelObject.run_named_tasks_in_parallel(tasks={"trigger": trigger_func, "watcher": watch_func}, timeout=compaction_args.timeout + 5, ignore_exceptions=True)
    ...
    def disrupt_start_stop_scrub_compaction(self):
        ...
        ks_cf = f"{compaction_args.keyspace}.{compaction_args.columnfamily}"
        with DbNodeLogger(self.cluster.nodes, "start and stop scrub compaction", target_node=self.target_node), self.action_log_scope(f"start and stop scrub compaction on {ks_cf} table on {self.target_node.name} node"):
            results = ParallelObject.run_named_tasks_in_parallel(tasks={"trigger": trigger_func, "watcher": watch_func}, timeout=compaction_args.timeout + 5, ignore_exceptions=True)
    ...
    def disrupt_start_stop_cleanup_compaction(self):
        ...
        ks_cf = f"{compaction_args.keyspace}.{compaction_args.columnfamily}"
        with DbNodeLogger(self.cluster.nodes, "start and stop cleanup compaction", target_node=self.target_node), self.action_log_scope(f"start and stop cleanup compaction on {ks_cf} table on {self.target_node.name} node"):
            results = ParallelObject.run_named_tasks_in_parallel(tasks={"trigger": trigger_func, "watcher": watch_func}, timeout=compaction_args.timeout + 5, ignore_exceptions=True)
    ...
    def disrupt_start_stop_validation_compaction(self):
        ...
        ks_cf = f"{compaction_args.keyspace}.{compaction_args.columnfamily}"
        with DbNodeLogger(self.cluster.nodes, "start and stop validation compaction", target_node=self.target_node), self.action_log_scope(f"start and stop validation compaction on {ks_cf} on {self.target_node.name} node"):
            results = ParallelObject.run_named_tasks_in_parallel(tasks={"trigger": trigger_func, "watcher": watch_func}, timeout=compaction_args.timeout + 5, ignore_exceptions=True)
    ...
    def disrupt_restart_then_repair_node(self):
        with (
            DbEventsFilter(db_event=DatabaseLogEvent.DATABASE_ERROR, line="Can't find a column family with UUID", node=self.target_node),
            DbEventsFilter(db_event=DatabaseLogEvent.BACKTRACE, line="Can't find a column family with UUID", node=self.target_node),
        ):
            with DbNodeLogger(self.cluster.nodes, "restart node", target_node=self.target_node), self.action_log_scope(f"Restart {self.target_node.name} node"):
    ...
    def _check_for_cdc_success_messages(found_cdc_error, cdc_success_msg):
        ...
        if not found_success_info:
            raise CdcStreamsWasNotUpdated(f"After '{found_cdc_error[0]}', messages '{' or '.join(cdc_success_msg)}' were not found")
    ...
    def disrupt_switch_between_password_authenticator_and_saslauthd_authenticator_and_back(self):
        ...
        with self.target_node.remote_scylla_yaml() as scylla_yml:
            ...
            if orig_auth == SASLAUTHD_AUTHENTICATOR:
                ...
                raise UnsupportedNemesis("This nemesis only supports to switch between SaslauthdAuthenticator and PasswordAuthenticator")
    ...
    def disrupt_restart_with_resharding(self):
        if self._is_it_on_kubernetes():
            raise UnsupportedNemesis("Not supported on K8S. Run 'disrupt_nodetool_flush_and_reshard_on_kubernetes' instead")
        ...
        self.log.info(f"Restart node with resharding. New murmur3_partitioner_ignore_msb_bits value: {murmur3_partitioner_ignore_msb_bits}")
    ...
    def replace_full_file_name_to_prefix(self, one_file, ks_cf_for_destroy):
        ...
        try:
            ...
            self.log.debug('File name "{file_name}" is not as expected for Scylla data files. Search files for "{ks_cf_for_destroy}" table'.format(file_name=file_name, ks_cf_for_destroy=ks_cf_for_destroy))
    ...
    def _choose_file_for_destroy(self, ks_cfs, return_one_file=True):
        ...
        files = self.target_node.remoter.sudo("find /var/lib/scylla/data/%s-* -maxdepth 1 -type f" % ks_cf_for_destroy, verbose=False)
        if files.stderr:
            raise NoFilesFoundToDestroy("Failed to get data files for destroy in {}. Error: {}".format(ks_cf_for_destroy, files.stderr))
    ...
    def _destroy_data_and_restart_scylla(self, keyspaces_for_destroy: list = None, sstables_to_destroy_perc: int = 50):
        tables = self.cluster.get_non_system_ks_cf_list(db_node=self.target_node, filter_empty_tables=False, filter_by_keyspace=keyspaces_for_destroy)
        if not tables:
        ...
        try:
            ...
            self.log.debug("SStables amount to destroy (%s percent of all SStables): %s", sstables_to_destroy_perc, sstables_amount_to_destroy)

            destroyed_files = 0
            while sstables_amount_to_destroy > 0:
                file_for_destroy = random.choice(all_files_to_destroy)
                if not (file_group_for_destroy := self.replace_full_file_name_to_prefix(one_file=file_for_destroy, ks_cf_for_destroy=tables)):
                    continue

                with DbNodeLogger(self.cluster.nodes, "remove data", target_node=self.target_node, additional_info=file_group_for_destroy):
                    result = self.target_node.remoter.sudo("rm -f %s" % file_group_for_destroy)
                if result.stderr:
                    raise FilesNotCorrupted("Files were not removed. The nemesis can't be run. Error: {}".format(result))
    ...
    def disrupt_nodetool_drain(self):
        ...
        self.target_node.run_nodetool("status", ignore_status=True, verbose=True, warning_event_on_exception=(Exception,))
    ...
    def disrupt_disable_enable_ldap_authorization(self):
        ...
        ldap_config = {"role_manager": "", "ldap_url_template": "", "ldap_attr_role": "", "ldap_bind_dn": "", "ldap_bind_passwd": ""}
    ...
    def _replace_cluster_node(self, old_node_ip: str | None = None, host_id: str | None = None, timeout: int | float = MAX_TIME_WAIT_FOR_NEW_NODE_UP, rack=0, is_zero_node: bool = False) -> BaseNode:
        ...
        add_node_func_args = {"count": 1, "dc_idx": self.target_node.dc_idx, "enable_auto_bootstrap": True, "rack": rack, "disruption_name": self.current_disruption, **({"is_zero_node": is_zero_node} if is_zero_node else {})}
        ...
        verification_node = random.choice([node for node in self.cluster.nodes if node not in (new_node, self.target_node) and not node.running_nemesis])
    ...
    def _add_and_init_new_cluster_nodes(self, count, timeout=MAX_TIME_WAIT_FOR_NEW_NODE_UP, rack=None, instance_type: str = None, is_zero_node: bool = False) -> list[BaseNode]:
        ...
        add_node_func_args = {"count": count, "dc_idx": self.target_node.dc_idx, "enable_auto_bootstrap": True, "rack": rack, "instance_type": instance_type, "disruption_name": self.current_disruption}
        if is_zero_node:
        ...
        with self.action_log_scope(f"Add {count} new nodes on {rack} rack, {instance_type} type"):
            new_nodes = skip_on_capacity_issues(db_cluster=self.tester.db_cluster)(self.cluster.add_nodes)(**add_node_func_args)
        self.monitoring_set.reconfigure_scylla_monitoring()
    ...
    def _nodetool_decommission(self, add_node=True):
        ...
        with self.action_log_scope(f"Decommission {self.target_node.name} node. is_zero_token_node: {self.target_node._is_zero_token_node}"):
    ...
    def disrupt_nodetool_seed_decommission(self, add_node=True):
        ...
        if not self.target_node.is_seed:
            self.target_node = self.node_allocator.select_target_node(nemesis_name=self.current_disruption, pool_type=self._target_node_pool_type, filter_seed=self.filter_seed, is_seed=True)
    ...
    def _verify_resharding_on_k8s(self, cpus, dc_idx):
        ...
        self.log.info("Update the cpu count to '%s' CPUs to make Scylla start the resharding process on all the nodes 1 by 1", cpus)
        ...
        for node, liveness_probe_failures, resharding_start, resharding_finish in nodes_data:
            ...
            if resharding_time < 120:
                self.log.warning("Resharding was too fast - '%s's (<120s) on the '%s' node. So, nemesis didn't cover the case.", resharding_time, node.name)
            else:
                self.log.info("Resharding took '%s's on the '%s' node. It is enough to cover the case.", resharding_time, node.name)
            ...
            assert not liveness_probe_failures_return, f"There are liveness probe failures: {liveness_probe_failures_return}"
    ...
    def disrupt_terminate_kubernetes_host_then_replace_scylla_node(self):
        if not self.cluster.params.get("k8s_enable_sni") and SkipPerIssues("https://github.com/scylladb/scylla-operator/issues/1124", params=self.tester.params):
    ...
    def _disrupt_kubernetes_then_replace_scylla_node(self, disruption_method):
        ...
        self.log.info(
            ...
            node,
            old_uid,
            [f"{neighbour_scylla_pod.metadata.namespace}/{neighbour_scylla_pod.metadata.name}" for neighbour_scylla_pod in neighbour_scylla_pods],
        )
    ...
    def _get_neighbour_scylla_pods(self, scylla_pod):
        if self.tester.params.get("k8s_tenants_num") < 2:
            return []
        matched_pods = KubernetesOps.list_pods(scylla_pod.k8s_cluster, namespace=None, field_selector=f"spec.nodeName={scylla_pod.pod_spec.node_name}", label_selector="app.kubernetes.io/name=scylla")
    ...
    def _terminate_and_replace_node(self):
        ...
        if self._is_it_on_kubernetes():
            raise UnsupportedNemesis('Use "disrupt_terminate_kubernetes_host_then_replace_scylla_node" instead this one for K8S')
        ...
        new_node = self.replace_node(old_node_ip, host_id, rack=self.target_node.rack, is_zero_node=self.target_node._is_zero_token_node)
        try:
            ...
            def wait_for_old_node_to_removed():
                ...
                if old_node_ip == new_node.ip_address:
                    assert state == "UN", f"New node with the same IP as removed one should be in UN state but was: {state}"
                else:
            ...
            try:
                ...
                for i in range(10):
                    ...
                    with self.cluster.cql_connection_patient(self.target_node, connect_timeout=600) as session:
                        ...
                        session.execute(SimpleStatement(f"DROP TABLE drop_table_during_repair_ks_{i}.standard1"), timeout=300)
            finally:
                thread.result()

    def _major_compaction(self):
        with adaptive_timeout(Operations.MAJOR_COMPACT, self.target_node, timeout=8000), self.action_log_scope(f"Major compaction on {self.target_node.name} node"):
    ...
    def disrupt_load_and_stream(self):
        ...
        if result is not None and result.exit_status == 0:
            map_files_to_node = SstableLoadUtils.distribute_test_files_to_cluster_nodes(nodes=self.cluster.data_nodes, test_data=test_data)
    ...
    def disrupt_nodetool_refresh(self, big_sstable: bool = False):
        ...
        test_data = SstableLoadUtils.get_load_test_data_inventory(column_num, big_sstable=big_sstable, load_and_stream=False)
        ...
        if result is not None and result.exit_status == 0:
            ...
            for node in self.cluster.data_nodes:
                SstableLoadUtils.upload_sstables(node, test_data=test_data[0], table_name="standard1", is_cloud_cluster=self.cluster.params.get("db_type") == "cloud_scylla")
    ...
    def _k8s_fake_enospc_error(self, node):
        ...
        try:
            experiment = IOFaultChaosExperiment(node, duration="300s", error=DiskError.NO_SPACE_LEFT_ON_DEVICE, error_probability=100, methods=["write", "flush"], volume_path="/var/lib/scylla")
    ...
    def disrupt_remove_service_level_while_load(self):
        ...
        with self.cluster.cql_connection_patient(node=self.cluster.nodes[0], user=DEFAULT_USER, password=DEFAULT_USER_PASSWORD) as session:
            ...
            try:
                ...
                role.attach_service_level(ServiceLevel(session=session, name=SERVICE_LEVEL_NAME_TEMPLATE % (removed_shares, random.randint(0, 10)), shares=removed_shares).create())
    ...
    def repair_nodetool_repair(self, node=None, publish_event=True):
        ...
        with adaptive_timeout(Operations.REPAIR, node, timeout=HOUR_IN_SEC * 48), self.action_log_scope(f"Start nodetool repair on {node.name} node"):
    ...
    def run_repair_on_nodes(self, nodes: list, ignore_down_hosts=False, publish_event=True):
        ...
        if not self.cluster.params.get("use_mgmt") and not self.cluster.params.get("use_cloud_manager"):
            for node in nodes:
                try:
                    with adaptive_timeout(Operations.REPAIR, node, timeout=HOUR_IN_SEC * 3), self.action_log_scope(f"nodetool repair -pr on {node.name} node"):
    ...
    def _prepare_test_table(self, ks="keyspace1", table=None):
        ...
        if ks not in test_keyspaces or not table_exist:
            stress_cmd = f"cassandra-stress write n=400000 cl=QUORUM -mode native cql3 -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor={self.tester.reliable_replication_factor})' -log interval=5"
            cs_thread = self.tester.run_stress_thread(stress_cmd=stress_cmd, keyspace_name=ks, stop_test_on_failure=False, round_robin=True)
    ...
    def _nemesis_stress_failure_handler(self, stress_pool, errors):
        ...
        if len(errors) == len(stress_pool.get_results()):
            ...
            raise NemesisStressFailure(f"Aborting '{self.__class__.__name__}' nemesis as stress command failed with the following errors:\n{errors_str}")
    ...
    def disrupt_truncate(self):
        ...
        with self.action_log_scope(f"Truncate {ks_cf} table using cqlsh"):
            self.target_node.run_cqlsh(cmd=f"TRUNCATE {keyspace_truncate}.{table}{truncate_cmd_timeout_suffix}", timeout=truncate_timeout)
    ...
    def disrupt_truncate_large_partition(self):
        ...
        """
        if SkipPerIssues(issues="https://github.com/scylladb/scylladb/issues/20356", params=self.tester.params) and self.tester.params.get("use_zero_nodes"):
        ...
        truncate_cmd_timeout_suffix = self._truncate_cmd_timeout_suffix(truncate_timeout)
        with self.action_log_scope(f"Truncate {ks_cf} table using cqlsh"):
            self.target_node.run_cqlsh(cmd=f"TRUNCATE {ks_name}.{table}{truncate_cmd_timeout_suffix}", timeout=truncate_timeout)
    ...
    def _modify_table_property(self, name, val, filter_out_table_with_counter=False, keyspace_table=None):
        ...
        if not keyspace_table:
            ...
            ks_cfs = self.cluster.get_non_system_ks_cf_list(db_node=self.target_node, filter_out_table_with_counter=filter_out_table_with_counter, filter_out_mv=True)  # not allowed to modify MV

            keyspace_table = random.choice(ks_cfs) if ks_cfs else ks_cfs

        if not keyspace_table:
            raise UnsupportedNemesis("Non-system keyspace and table are not found. ModifyTableProperties nemesis can't be run")

        cmd = "ALTER TABLE {keyspace_table} WITH {name} = {val};".format(keyspace_table=keyspace_table, name=name, val=val)
    ...
    def _add_drop_column_generate_columns_to_add(self, added_columns_info):
        add = []
        columns_to_add = min(self._add_drop_column_max_columns - len(added_columns_info["column_names"]), self._add_drop_column_max_per_add)
        if columns_to_add > 1:
            columns_to_add = random.randrange(1, columns_to_add)
        for _ in range(columns_to_add):
            new_column_name = self._random_column_name(added_columns_info["column_names"].keys(), self._add_drop_column_max_column_name_size)
            new_column_type = CQLTypeBuilder.get_random(added_columns_info["column_types"], allow_levels=10, avoid_types=["counter"], forget_on_exhaust=True)
    ...
    def _add_drop_column(self, drop=True, add=True):
        ...
        if self._add_drop_column_target_table is None:
            return
        added_columns_info = self._add_drop_column_get_added_columns_info(self._add_drop_column_target_table, self._add_drop_column_columns_info)
        ...
        if add:
            ...
            cmd = f"ALTER TABLE {self._add_drop_column_target_table[1]} ADD ( {', '.join(['%s %s' % (col[0], col[1]) for col in add])} );"
            if self._add_drop_column_run_cql_query(cmd, self._add_drop_column_target_table[0]):
    ...
    def verify_initial_inputs_for_delete_nemesis(self):
        ...
        if not (self.tester.partitions_attrs and self.tester.partitions_attrs.max_partitions_in_test_table):
            raise UnsupportedNemesis('This nemesis expects "max_partitions_in_test_table" sub-parameter of data_validation to be set')

    def choose_partitions_for_delete(self, partitions_amount, ks_cf, with_clustering_key_data=False, exclude_partitions=None):
        """
        ...
        if partitions_attrs.partition_range_with_data_validation:
            ...
            available_partitions_for_deletion = list(range(partitions_attrs.partition_end_range + 1, partitions_attrs.max_partitions_in_test_table))
        else:
    ...
    def get_random_timestamp_from_partition(self, ks_cf, pkey, partition_percentage=0.25) -> tuple[int, int]:
        ...
        with self.cluster.cql_connection_patient(node=self.target_node) as session:
            ...
            self.log.debug("[%s_using_timestamp] Partition size: %s, fetching up to %s", self.base_disruption_name, number_of_rows, fetch_limit)
            partition = session.execute(SimpleStatement(f"select pk, ck from {ks_cf} where pk = {pkey} limit {fetch_limit}")).all()
            if not partition:
                message = f"No rows found in partition (pk = {pkey}) after counting {number_of_rows} rows. The partition may have been deleted."
                self.log.error(message)
                raise PartitionNotFound(message)
            delete_mark = partition[-1].ck
            timestamp_result = session.execute(SimpleStatement(f"select writetime(v) from {ks_cf} where pk = {pkey} and ck = {delete_mark}")).one()
    ...
    def _verify_using_timestamp_deletions(self, ks_cf: str, verification_queries: list[tuple[int, int, int]]):
        ...
        with self.cluster.cql_connection_patient(self.target_node, connect_timeout=300) as session:
            for pk, ck, ts in verification_queries:
                result = session.execute(SimpleStatement(f"SELECT pk, ck, writetime(v) FROM {ks_cf} WHERE pk = {pk} AND ck = {ck}")).one()
                assert not result or result.writetime_v != ts, f"USING TIMESTAMP: deletion failed for ({pk}, {ck}), row still exists, timestamp used: {ts}"
                if not mv_not_configured:
                    try:
                        result = session.execute(SimpleStatement(f"SELECT pk, ck, writetime(v) FROM {mv_table_name} WHERE pk = {pk} AND ck = {ck}")).one()
                        assert not result or result.writetime_v != ts, f"USING TIMESTAMP: deletion failed for ({pk}, {ck}), row still exists in MV (!!!), timestamp used: {ts}"
    ...
    def delete_half_partition(self, ks_cf):
        ...
        partitions_for_delete = self.choose_partitions_for_delete(partitions_amount=partitions_amount, ks_cf=ks_cf, with_clustering_key_data=True)
    ...
    def delete_by_range_using_timestamp(self, ks_cf: str):
        ...
        partitions_for_delete = self.choose_partitions_for_delete(partitions_amount=self.tester.partitions_attrs.non_validated_partitions // self.num_deletions_factor, ks_cf=ks_cf, with_clustering_key_data=False)
        if not partitions_for_delete:
        ...
        self.actions_log.info(f"Deleting partitions using timestamp in {ks_cf} table. Partitions count: {len(partitions_for_delete)}, partitions percentage: {partition_percentage}")
        for pkey, _ in partitions_for_delete.items():
            ...
            timestamp, clustering_key = self.get_random_timestamp_from_partition(ks_cf=ks_cf, pkey=pkey, partition_percentage=partition_percentage)
    ...
    def delete_range_in_few_partitions(self, ks_cf, partitions_for_exclude_dict):
        ...
        partitions_for_delete = self.choose_partitions_for_delete(
            partitions_amount=self.tester.partitions_attrs.non_validated_partitions // self.num_deletions_factor, ks_cf=ks_cf, with_clustering_key_data=True, exclude_partitions=partitions_for_exclude
        )
        if not partitions_for_delete:
        ...
        self.actions_log.info(f"Delete same range in the few partitions in {ks_cf} table. Partitions count: {len(partitions_for_delete)}")
        queries = []
        for pkey in partitions_for_delete.keys():
            queries.append(f"delete from {ks_cf} where pk = {pkey} and ck >= {clustering_keys[0]} and ck <= {clustering_keys[-1]}")
    ...
    def disrupt_delete_overlapping_row_ranges(self):
        ...
        self.verify_initial_inputs_for_delete_nemesis()
        ks_cf = "scylla_bench.test"
        partitions_for_delete = self.choose_partitions_for_delete(partitions_amount=self.tester.partitions_attrs.non_validated_partitions // self.num_deletions_factor, ks_cf=ks_cf, with_clustering_key_data=True)
        if not partitions_for_delete:
        ...
        self.actions_log.info(f"Delete random row ranges in few partitions in {ks_cf} table. Partitions count: {len(partitions_for_delete)}")
        queries = []
        for pkey, ckey in partitions_for_delete.items():
    ...
    def modify_table_caching(self):
        ...
        """
        prop_val = dict(keys=random.choice(["NONE", "ALL"]), rows_per_partition=random.choice(["NONE", "ALL", random.randint(1, 10000)]))
    ...
    def toggle_table_gc_mode(self):
        ...
        if not (all_ks_cfs := self.cluster.get_non_system_ks_cf_list(db_node=self.target_node, filter_func=self.cluster.is_ks_rf_one)):
            raise UnsupportedNemesis("Any table with RF != 1 is not found. disrupt_toggle_table_gc_mode nemesis can't run")
        ...
        alter_command_prefix = "ALTER TABLE " if not is_cf_a_view(node=self.target_node, ks=keyspace, cf=table) else "ALTER MATERIALIZED VIEW "
        ...
        self.actions_log.info(f"Toggle table GC mode for {keyspace_table} table from {current_gc_mode.value} to {new_gc_mode.value}")
    ...
    def toggle_table_ics(self):
        ...
        alter_command_prefix = "ALTER TABLE " if not is_cf_a_view(node=self.target_node, ks=keyspace, cf=table) else "ALTER MATERIALIZED VIEW "
        cmd = alter_command_prefix + " {keyspace_table} WITH compaction = {new_compaction_strategy_as_dict};".format(**locals())
        self.log.debug("Toggle table ICS query to execute: {}".format(cmd))
        self.actions_log.info(f"Toggle table ICS for {keyspace_table} table from {cur_compaction_strategy.value} to {new_compaction_strategy.value}")
        try:
            self.target_node.run_cqlsh(cmd)
    ...
    def modify_table_compaction(self):
        ...
        if prop_val["class"] == "TimeWindowCompactionStrategy":
            ...
            self._modify_table_property(name="default_time_to_live", val=str(4300000), filter_out_table_with_counter=True)
    ...
    def modify_table_default_time_to_live(self):
        ...
        ks_cfs = self.cluster.get_non_system_ks_cf_list(db_node=self.target_node, filter_out_table_with_counter=True, filter_out_mv=True)
        ...
        self._modify_table_property(name="default_time_to_live", val=ttl_to_set, filter_out_table_with_counter=True, keyspace_table=keyspace_table)
    ...
    def modify_table_memtable_flush_period_in_ms(self):
        ...
        """
        self._modify_table_property(name="memtable_flush_period_in_ms", val=random.choice([0, random.randint(60000, 200000)]))
    ...
    def modify_table_twcs_window_size(self):
        ...
        self._modify_table_property(name="compaction", val=ks_cs_settings["compaction"], keyspace_table=ks_cs_settings["name"])
        self._modify_table_property(name="default_time_to_live", val=ks_cs_settings["dttl"], keyspace_table=ks_cs_settings["name"])
        self._modify_table_property(name="gc_grace_seconds", val=ks_cs_settings["gc"], keyspace_table=ks_cs_settings["name"])
        ...
        reshape_twcs_records = self.target_node.follow_system_log(patterns=["need reshape. Starting reshape process", "Reshaping", f"Reshape {ks_cs_settings['name']} .* Reshaped"])
        ...
        if not reshape_twcs_records:
            self.log.warning("Log message with sstables for reshape was not found. Autocompaction alreadycompact sstables by timewindows")
        ...
        self.log.info("Number of sstables before: %s and after %s change twcs settings", num_sstables_before_change, num_sstables_after_change)
        ...
        for node in self.cluster.data_nodes:
            ...
            self.log.info("Number of sstables before: %s and after %s change twcs settings on node: %s", num_sstables_before_change, num_sstables_after_change, node.name)
            if num_sstables_before_change > num_sstables_after_change:
                self.log.error("Number of sstables after change settings larger than before")
    ...
    def _manager_backup_and_report(self, object_storage_upload_mode: ObjectStorageUploadMode, label) -> BackupTask:
        ...
        decorated = latency_calculator_decorator(legend="Scylla-Manager Backup", cycle_name=label)(self._run_manager_backup)
    ...
    def disrupt_mgmt_restore(self):  # noqa: PLR0914
        ...
        def choose_snapshot(snapshots_dict, region: str):
            ...
            if self.cluster.nodes[0].is_enterprise:
                ...
                oss_snapshots = [snapshot_key for snapshot_key, snapshot_value in all_snapshots_per_region.items() if snapshot_value["scylla_product"] == "oss"]
                snapshot_tag = random.choice(oss_snapshots)

            snapshot_info = all_snapshots_per_region[snapshot_tag]
            snapshot_info.update({"expected_timeout": snapshot_groups_by_size[chosen_snapshot_size]["expected_timeout"], "number_of_rows": snapshot_groups_by_size[chosen_snapshot_size]["number_of_rows"]})
        ...
        def execute_data_validation_thread(command_template, keyspace_name, number_of_rows):
            ...
            for loader_index in range(number_of_loaders):
                stress_command = command_template.format(num_of_rows=rows_per_loader, keyspace_name=keyspace_name, sequence_start=rows_per_loader * loader_index + 1, sequence_end=rows_per_loader * (loader_index + 1))
                read_thread = self.tester.run_stress_thread(stress_cmd=stress_command, round_robin=True, stop_test_on_failure=False)
        ...
        def _restore_schema(locations: list, cluster_id: str, tag: str) -> None:
            ...
            if dc_under_test_name != dc_from_backup_name:
                self.log.info("DC names mismatch - restoring the schema manually altering cql statements and applying them one by one")
        ...
        skip_issues = ["https://github.com/scylladb/scylla-manager/issues/3829", "https://github.com/scylladb/scylla-manager/issues/4049"]
        is_multi_dc = len(self.cluster.params.region_names) > 1 or (self.cluster.params.get("simulated_regions") or 0) > 1
        ...
        chosen_snapshot_tag, chosen_snapshot_info = choose_snapshot(snapshots_dict=persistent_manager_snapshots_dict[cluster_backend], region=region)
        ...
        try:
            restore_task = mgr_cluster.create_restore_task(restore_data=True, location_list=location_list, snapshot_tag=chosen_snapshot_tag)
            ...
            stress_queue = execute_data_validation_thread(command_template=confirmation_stress_template, keyspace_name=chosen_snapshot_info["keyspace_name"], number_of_rows=chosen_snapshot_info["number_of_rows"])
    ...
    def _delete_existing_backups(self, mgr_cluster):
        ...
        if deleted_tasks:
            self.log.warning("Deleted the following backup tasks before the nemesis starts: %s", ", ".join(deleted_tasks))

    @decorate_with_context_if_issues_open(ignore_take_snapshot_failing, issue_refs=["https://github.com/scylladb/scylla-manager/issues/3389"])
    ...
    def _mgmt_repair_cli(self, ignore_down_hosts=None):
        ...
        if task_final_status != TaskStatus.DONE:
            progress_full_string = mgr_task.progress_string(parse_table_res=False, is_verify_errorless_result=True).stdout
            if task_final_status != TaskStatus.ERROR_FINAL:
                mgr_task.stop()
            raise ScyllaManagerError(f"Task: {mgr_task.id} final status is: {str(task_final_status)}.\nTask progress string: {progress_full_string}")
    ...
    def disrupt_abort_repair(self):
        ...
        def silenced_nodetool_repair_to_fail():
            try:
                ...
                self.target_node.run_nodetool("repair", verbose=True, warning_event_on_exception=(UnexpectedExit, Libssh2UnexpectedExit), error_message="Repair failed as expected. ", publish_event=False, long_running=True, retry=0)
        ...
        with ThreadPoolExecutor(max_workers=1, thread_name_prefix="NodeToolRepairThread") as thread_pool:
            ...
            wait.wait_for(func=repair_streaming_exists, timeout=300, step=1, throw_exc=True, text="Wait for repair starts")

            self.log.debug("Abort repair streaming by storage_service/force_terminate_repair API")

            with (
                DbEventsFilter(db_event=DatabaseLogEvent.DATABASE_ERROR, line="repair's stream failed: streaming::stream_exception", node=self.target_node),
                DbEventsFilter(db_event=DatabaseLogEvent.RUNTIME_ERROR, line="Can not find stream_manager", node=self.target_node),
                ...
                with DbNodeLogger(self.cluster.nodes, "abort repair streaming", target_node=self.target_node), self.action_log_scope(f"Abort repair streaming on {self.target_node.name} node"):
    ...
    def disrupt_validate_hh_short_downtime(self):
        ...
        wait.wait_for(func=target_node_reported_un_by_others, timeout=300, step=5, throw_exc=True, text="Wait for target_node to be seen as UN by others")
    ...
    def _validate_snapshot(self, nodetool_cmd: str, snapshot_content: namedtuple):
        ...
        ks_cf = self.cluster.get_any_ks_cf_list(db_node=self.target_node, filter_empty_tables=False, filter_func=filter_func)
        ...
        if sorted(keyspace_table) != sorted(snapshot_content_list):
            raise AssertionError(f"Snapshot content not as expected. \nExpected content: {sorted(keyspace_table)} \n Actual snapshot content: {sorted(snapshot_content_list)}")
    ...
    def disrupt_snapshot_operations(self):
        ...
        def _few_tables():
            ...
            if not tables:
                ...
                # If not - raise exception to investigate the issue
                raise Exception(f'Something wrong happened, tables for "{selected_keyspace}" keyspace were notfound in the list:\n{ks_cf}')
    ...
    def _disrupt_show_toppartitions(self, allow_new_api: bool):
        self.log.debug("Running 'disrupt_show_toppartitions' method using %s API.", "new and old" if allow_new_api else "old")
    ...
    def get_rate_limit_for_network_disruption(self) -> Optional[str]:
        ...
        query = 'avg(node_network_receive_bytes_total{instance=~".*?%s.*?", device="%s"})' % (self.target_node.ip_address, self.target_node.scylla_network_configuration.device)
        now = time.time()
    ...
    def _disrupt_network_random_interruptions_k8s(self, list_of_timeout_options):
        ...
        match interruption:
            case "delay":
                ...
                experiment = NetworkDelayExperiment(self.target_node, duration, f"{delay_in_msecs}ms", correlation=20, jitter=f"{jitter}ms")
            case "loss":
            ...
            case "corrupt":
                ...
                self.actions_log.info(f"Interruption by corrupting network packets - corrupt_percentage: {corrupt_percentage}%")
            ...
            case "rate":
                ...
                limit = limit_base * 20
                self.actions_log.info(f"Interruption by limiting network bandwidth - rate: {rate_limit}, limit: {limit}")
                experiment = NetworkBandwidthLimitExperiment(self.target_node, duration, rate=rate_limit, limit=limit, buffer=10000)
        with DbNodeLogger(self.cluster.nodes, f"network {interruption} interruption", target_node=self.target_node), self.action_log_scope(f"Network interruption on {self.target_node.name} for {duration}s"):
            experiment.start()
            experiment.wait_until_finished()
    ...
    def disrupt_network_random_interruptions(self):
        ...
        if not self.target_node.scylla_network_configuration or (self.target_node.scylla_network_configuration and not self.target_node.scylla_network_configuration.device):
        ...
        list_of_tc_options = [
            ...
            ("NetworkRandomInterruption_{}pct_corrupt".format(corrupt_percentage), "--corrupt {}%".format(corrupt_percentage)),
            ("NetworkRandomInterruption_{}sec_delay".format(delay_in_secs), "--delay {}s --delay-distro 500ms".format(delay_in_secs)),
        ]
        if rate_limit:
            list_of_tc_options.append(("NetworkRandomInterruption_{}_limit".format(rate_limit), "--rate {}".format(rate_limit)))

        option_name, selected_option = random.choice(list_of_tc_options)
        wait_time = random.choice(list_of_timeout_options)

        self.actions_log.info(f"Network interruption start on {self.target_node.name}, option: {option_name}, wait time: {wait_time}")
        if self.target_node.systemd_version < 256:
            context_manager = EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=CoreDumpEvent, regex=r".*executable=.*networkd.*", extra_time_to_expiration=60)
        else:
    ...
    def _disrupt_network_block_k8s(self, list_of_timeout_options):
        ...
        with DbNodeLogger(self.cluster.nodes, "block network traffic", target_node=self.target_node, additional_info=f"for {duration}sec"):
            experiment.start()
        experiment.wait_until_finished()
        time.sleep(15)
        self.cluster.wait_all_nodes_un()
    ...
    def disrupt_network_block(self):
        ...
        with context_manager:
            self.target_node.traffic_control(None)
            try:
                with DbNodeLogger(self.cluster.nodes, "block network traffic", target_node=self.target_node, additional_info=f"for {wait_time}sec"):
                    self.target_node.traffic_control(selected_option)
                time.sleep(wait_time)
            finally:
                self.target_node.traffic_control(None)
    ...
    def disrupt_remove_node_then_add_node(self):
        ...
        if self.cluster.params.get("db_type") == "cloud_scylla":
            raise UnsupportedNemesis("Skipping this nemesis due the replace node option that supported by Cloud is tested by CloudReplaceNonResponsiveNode nemesis")
        if self._is_it_on_kubernetes():
        ...
        with self.node_allocator.run_nemesis(nemesis_label="RemoveNodeAddNode", node_list=up_normal_nodes) as verification_node:
    ...
    def _remove_node_add_node(self, verification_node, node_to_remove, remove_node_host_id=None):
        ...
        if not remove_node_host_id:
            removed_node_status = self.cluster.get_node_status_dictionary(ip_address=node_to_remove.ip_address, verification_node=verification_node)
        ...
        wait_for(node_operations.is_node_seen_as_down, step=5, timeout=600, throw_exc=True, down_node=node_to_remove, verification_node=verification_node, text=f"Wait other nodes see {node_to_remove.name} as DOWN...")
        ...
        def remove_node():
            ...
            with self.node_allocator.run_nemesis(nemesis_label="RemoveNodeAddNode") as rnd_node:
                self.log.info("Running removenode command on {}, Removing node with the following host_id: {}".format(rnd_node.ip_address, host_id))
                with adaptive_timeout(Operations.REMOVE_NODE, rnd_node, timeout=HOUR_IN_SEC * 48):
                    res = rnd_node.run_nodetool("removenode {}".format(host_id), ignore_status=True, verbose=True, long_running=True, retry=0)
    ...
    def disrupt_network_reject_inter_node_communication(self):
        ...
        InfoEvent(f"{name} {textual_matching_rule} that belongs to inter node communication connections (port=7000 and 7001) will be {textual_pkt_action} for {wait_time}s").publish()
        ...
        with ignore_alternator_client_errors():
            return self._run_commands_wait_and_cleanup(
                self.target_node,
                name=name,
                start_commands=[f"sudo iptables -t filter -A INPUT -p tcp --dport 7000 {matching_rule} -j {pkt_action}", f"sudo iptables -t filter -A INPUT -p tcp --dport 7001 {matching_rule} -j {pkt_action}"],
                cleanup_commands=[f"sudo iptables -t filter -D INPUT -p tcp --dport 7000 {matching_rule} -j {pkt_action}", f"sudo iptables -t filter -D INPUT -p tcp --dport 7001 {matching_rule} -j {pkt_action}"],
                wait_time=wait_time,
            )
    ...
    def disrupt_network_reject_node_exporter(self):
        ...
        InfoEvent(f"{name} {textual_matching_rule} that belongs to node-exporter(port=9100) connections will be {textual_pkt_action} for {wait_time}s").publish()
    ...
    def disrupt_network_reject_thrift(self):
        ...
        InfoEvent(f"{name} {textual_matching_rule} that belongs to Thrift(port=9160) connections will be {textual_pkt_action} for {wait_time}s").publish()
    ...
    def _iptables_randomly_get_random_matching_rule():
        ...
        if match_type == "statistic":
            ...
            if match_type == "random":
                ...
                return f"randomly chosen packet with {probability} probability", f"-m statistic --mode {mode} --probability {probability}"
            elif match_type == "nth":
            ...
            return f"string of {pkts_per_period} very first packets every {period}", f"-m limit --limit {pkts_per_period}/{period}"
        elif match_type == "connbytes":
            bytes_from = random.choice(["100", "200", "400", "800", "1600", "3200", "6400", "12800", "1280000"])
            return f"every packet from connection that total byte counter exceeds {bytes_from}", f"-m connbytes --connbytes-mode bytes --connbytes-dir both --connbytes {bytes_from}"
        return "every packet", ""
    ...
    def _iptables_randomly_get_disrupting_target():
        ...
        if target_type == "REJECT":
            reject_with = random.choice(["icmp-net-unreachable", "icmp-host-unreachable", "icmp-port-unreachable", "icmp-proto-unreachable", "icmp-net-prohibited", "icmp-host-prohibited", "icmp-admin-prohibited"])
            return f"rejected with {reject_with}", f"{target_type} --reject-with {reject_with}"
        return "dropped", f"{target_type}"

    def _run_commands_wait_and_cleanup(self, node, name: str, start_commands: List[str], cleanup_commands: List[str] = None, wait_time: int = 0):
        """
        ...
        for cmd_num, cmd in enumerate(start_commands):
            try:
                ...
                self.log.error(f"{name}: failed to execute start command {cmd} on node {node} due to the following error: {str(exc)}")
        if not cmd_executed:
            return
        for cmd_num, cmd in enumerate(cleanup_commands):
            try:
                ...
                self.log.debug(f"{name}: failed to execute cleanup command {cmd} on node {node} due to the following error: {str(exc)}")
    ...
    @decorate_with_context_if_issues_open(ignore_ipv6_failure_to_assign, issue_refs=["https://github.com/scylladb/scylladb/issues/20387"])
    ...
    def _call_disrupt_func_after_expression_logged(self, log_follower: Iterable[str], disrupt_func: Callable, disrupt_func_kwargs: dict = None, sleep: int = 1, delay: int = 10, timeout: int = 600):
        """
        ...
        with (
            DbEventsFilter(db_event=DatabaseLogEvent.RUNTIME_ERROR, line="This node was decommissioned and will not rejoin", node=self.target_node),
            DbEventsFilter(db_event=DatabaseLogEvent.RUNTIME_ERROR, line="Fail to send STREAM_MUTATION_DONE", node=self.target_node),
            DbEventsFilter(db_event=DatabaseLogEvent.DATABASE_ERROR, line="streaming::stream_exception", node=self.target_node),
            DbEventsFilter(db_event=DatabaseLogEvent.RUNTIME_ERROR, line="got error in row level repair", node=self.target_node),
        ):
    ...
    def start_and_interrupt_decommission_streaming(self):
        ...
        terminate_pattern = self.target_node.raft.get_random_log_message(operation=TopologyOperations.DECOMMISSION, seed=self.nemesis_seed)
        ...
        trigger = partial(self.target_node.run_nodetool, sub_cmd="decommission", timeout=nodetool_decommission_timeout, warning_event_on_exception=(Exception,), long_running=True, retry=0)

        watcher = partial(self._call_disrupt_func_after_expression_logged, log_follower=log_follower, disrupt_func=self.reboot_node, disrupt_func_kwargs={"target_node": self.target_node, "hard": True, "verify_ssh": True}, delay=0)
        full_operations_timeout = nodetool_decommission_timeout + 3600
        with contextlib.ExitStack() as stack:
            for expected_start_failed_context in self.target_node.raft.get_severity_change_filters_scylla_start_failed(terminate_pattern.timeout):
            ...
            with (
                ...
                FailedDecommissionOperationMonitoring(target_node=self.target_node, verification_node=verification_node, timeout=full_operations_timeout),
    ...
    def start_and_interrupt_repair_streaming(self):
        ...
        watcher = partial(self._call_disrupt_func_after_expression_logged, log_follower=log_follower, disrupt_func=self.reboot_node, disrupt_func_kwargs={"target_node": self.target_node, "hard": True, "verify_ssh": True}, delay=1)
    ...
    def start_and_interrupt_rebuild_streaming(self):
        ...
        trigger = partial(self.target_node.run_nodetool, sub_cmd="rebuild", warning_event_on_exception=(Exception,), long_running=True, retry=0, timeout=timeout // 2)
        log_follower = self.target_node.follow_system_log(patterns=["rebuild from dc:"])

        watcher = partial(
            self._call_disrupt_func_after_expression_logged, log_follower=log_follower, disrupt_func=self.reboot_node, disrupt_func_kwargs={"target_node": self.target_node, "hard": True, "verify_ssh": True}, timeout=timeout, delay=1
        )
        with self.action_log_scope(f"Rebuild data after destroy on {self.target_node.name} node"):
    ...
    def disrupt_decommission_streaming_err(self):
        ...
        """
        if self._is_it_on_kubernetes():
            raise UnsupportedNemesis("This nemesis logic is not compatible with K8S approach for handling Scylla member's decommissioning.")
    ...
    def _decommission_nodes(self, nodes_number, rack, is_seed: Optional[Union[bool, DefaultValue]] = DefaultValue, dc_idx: Optional[int] = None, exact_nodes: list[BaseNode] | None = None):
        ...
        try:
            ...
            InfoEvent(f"FinishEvent - ShrinkCluster failed decommissioning a node {self.target_node} with error {str(exc)}").publish()
    ...
    def _double_cluster_load(self, duration: int) -> None:
        ...
        stress_queue = self.tester.run_stress_thread(stress_cmd=self.tester.stress_cmd, stress_num=1, stats_aggregate_cmds=False, duration=duration)
        results = self.tester.verify_stress_thread(thread_pool=stress_queue, error_handler=self._nemesis_stress_failure_handler)
    ...
    def _grow_cluster(self, rack=None):
        ...
        with self.action_log_scope(f"Grow cluster by {add_nodes_number} nodes"):
            if self.cluster.parallel_node_operations:
                new_nodes = self.add_new_nodes(count=add_nodes_number, rack=rack, instance_type=self.tester.params.get("nemesis_grow_shrink_instance_type"))
            else:
                for idx in range(add_nodes_number):
                    # if rack is not specified, round-robin racks to spread nodes evenly
                    rack_idx = rack if rack is not None else idx % self.cluster.racks_count
                    new_nodes += self.add_new_nodes(count=1, rack=rack_idx, instance_type=self.tester.params.get("nemesis_grow_shrink_instance_type"))
    ...
    def _shrink_cluster(self, rack=None, new_nodes: list[BaseNode] | None = None):
        ...
        self.log.debug("Current target_node %s, is zero_node: %s, dc_idx: %s", self.target_node.name, self.target_node._is_zero_token_node, self.target_node.dc_idx)
    ...
    def disrupt_enable_disable_table_encryption_aws_kms_provider_without_rotation(self):
        self._enable_disable_table_encryption(enable_kms_key_rotation=False, additional_scylla_encryption_options={"key_provider": "KmsKeyProviderFactory"})

    @target_all_nodes
    def disrupt_enable_disable_table_encryption_aws_kms_provider_with_rotation(self):
        self._enable_disable_table_encryption(enable_kms_key_rotation=True, additional_scylla_encryption_options={"key_provider": "KmsKeyProviderFactory"})
    ...
    def _enable_disable_table_encryption(self, enable_kms_key_rotation, additional_scylla_encryption_options=None):  # noqa: PLR0914
        ...
        if additional_scylla_encryption_options and additional_scylla_encryption_options.get("key_provider", "N/A") == "KmsKeyProviderFactory":
        ...
        def run_write_scylla_bench_load(write_cmd):
            ...
            with (
                EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=ScyllaBenchEvent, extra_time_to_expiration=30, regex=r".*Error during truncate: seastar::rpc::remote_verb_error \(filesystem error.*"),
                EventsSeverityChangerFilter(
                    new_severity=Severity.WARNING, event_class=DatabaseLogEvent, extra_time_to_expiration=30, regex=".*sstable - Error while linking SSTable.*filesystem error: stat failed: No such file or directory.*"
                ),
        ...
        try:
            ...
            with self.target_node.remote_scylla_yaml() as scylla_yaml:
                user_info_encryption_enabled = scylla_yaml.user_info_encryption and scylla_yaml.user_info_encryption.get("enabled", False)
    ...
    def disrupt_hot_reloading_internode_certificate(self):
        ...
        ssl_files_location = json.loads(self.target_node.get_scylla_config_param("server_encryption_options"))["certificate"]
        ...
        if SkipPerIssues("https://github.com/scylladb/scylladb/issues/7909", params=self.tester.params):
            ...
            context_manager = DbEventsFilter(db_event=DatabaseLogEvent.DATABASE_ERROR, line="error GnuTLS:-34, Base64 decoding error")
        else:
            context_manager = contextlib.nullcontext()

        with context_manager:
            for node in self.cluster.nodes:
                node_system_logs[node] = node.follow_system_log(patterns=[f'messaging_service - Reloaded {{"{ssl_files_location}"}}'])
    ...
    def _k8s_disrupt_memory_stress(self):
        ...
        experiment = MemoryStressExperiment(pod=self.target_node, duration=f"{duration}s", workers=1, size="90%", time_to_reach=f"{time_to_reach_secs}s")
        with DbNodeLogger(self.cluster.nodes, "start memory stress", target_node=self.target_node, additional_info="allocate 90% of available memory"):
            experiment.start()
        experiment.wait_until_finished()

        self.log.info("Try to allocate 100% total memory")
        experiment = MemoryStressExperiment(pod=self.target_node, duration=f"{duration}s", workers=1, size="100%", time_to_reach=f"{time_to_reach_secs}s")
        with (
            DbNodeLogger(self.cluster.nodes, "start memory stress", target_node=self.target_node, additional_info="allocate 100% of total memory"),
    ...
    def disrupt_memory_stress(self):
        ...
        with (
            DbNodeLogger(self.cluster.nodes, "start memory stress", target_node=self.target_node, additional_info="allocate 90% of total memory"),
    ...
    def _run_cdc_stressor_tool(self, ks, table):
        ...
        cdc_reader_thread = self.tester.run_cdclog_reader_thread(stress_cmd=cdc_stressor_cmd, stress_num=1, keyspace_name=ks, base_table_name=table)
    ...
    def _verify_cdc_feature_status(self, keyspace: str, table: str, cdc_settings: dict) -> None:
        ...
        if not cdc_settings["enabled"]:
            assert actual_cdc_settings["enabled"] is False, f"CDC options was not disabled. Current: {actual_cdc_settings} expected: {cdc_settings}"
        else:
            assert actual_cdc_settings == cdc_settings, f"CDC extension settings are differs. Current: {actual_cdc_settings} expected: {cdc_settings}"

    def _add_new_node_in_new_dc(self, is_zero_node=False) -> BaseNode:
        add_node_func_args = {"count": 1, "dc_idx": 0, "enable_auto_bootstrap": True, "disruption_name": self.current_disruption, **({"is_zero_node": is_zero_node} if is_zero_node else {})}
        new_node = skip_on_capacity_issues(db_cluster=self.tester.db_cluster)(self.cluster.add_nodes)(**add_node_func_args)[0]
        with new_node.remote_scylla_yaml() as scylla_yml:
            scylla_yml.rpc_address = new_node.ip_address
            scylla_yml.seed_provider = [SeedProvider(class_name="org.apache.cassandra.locator.SimpleSeedProvider", parameters=[{"seeds": self.tester.db_cluster.seed_nodes_addresses}])]
    ...
    def _verify_multi_dc_keyspace_data(self, consistency_level: str = "ALL"):
        read_cmd = f"cassandra-stress read no-warmup cl={consistency_level} n=100000 -schema 'keyspace=keyspace_new_dc compression=LZ4Compressor' -mode cql3 native compression=lz4 -rate threads=5 -pop seq=1..100000 -log interval=5"
    ...
    def _switch_to_network_replication_strategy(self, keyspaces: List[str]) -> None:
        ...
        for keyspace in keyspaces:
            ...
            if keyspace == "system_auth" and replication_strategy.replication_factors[0] != len(nodes_by_region[region]):
                self.log.warning(f"system_auth keyspace is not replicated on all nodes ({replication_strategy.replication_factors[0]}/{len(nodes_by_region[region])}).")
            with self.action_log_scope(f"Switching {keyspace} replication strategy to Network"):
                network_replication = NetworkTopologyReplicationStrategy(**{dc_name: replication_strategy.replication_factors[0]})
    ...
    def disrupt_add_remove_dc(self) -> None:
        ...
        if self.cluster.test_config.MULTI_REGION:
            raise UnsupportedNemesis("add_remove_dc skipped for multi-dc scenario (https://github.com/scylladb/scylla-cluster-tests/issues/5369)")
        ...
        self.tester.create_keyspace("keyspace_new_dc", replication_factor={initial_dc_name: min(3, len(self.cluster.data_nodes))})
        node_added = False
        with ExitStack() as context_manager:
            ...
            with temporary_replication_strategy_setter(node) as replication_strategy_setter:
                ...
                for keyspace in system_keyspaces + ["keyspace_new_dc"]:
                    ...
                    assert isinstance(strategy, NetworkTopologyReplicationStrategy), "Should have been already switched to NetworkStrategy"
                ...
                with (
                    wait_for_log_lines(
                        node=new_node, start_line_patterns=["rebuild.*started with keyspaces=", "Rebuild starts"], end_line_patterns=["rebuild.*finished with keyspaces=", "Rebuild succeeded"], start_timeout=60, end_timeout=600
                    ),
    ...
    def disrupt_sla_increase_shares_during_load(self):
        ...
        if not (stress_cmds := self.get_cassandra_stress_write_cmds()):
            raise UnsupportedNemesis("SLA nemesis needs cassandra-stress default table 'keyspace1.standard1' is created and prefilled")

        column_definition, dataset_size = self.get_cassandra_stress_definition(stress_cmds)

        prometheus_stats = PrometheusDBStats(host=self.monitoring_set.nodes[0].external_address)
        sla_tests = SlaTests()
        error_events = sla_tests.test_increase_shares_during_load(tester=self.tester, prometheus_stats=prometheus_stats, num_of_partitions=dataset_size, cassandra_stress_column_definition=column_definition)
    ...
    def disrupt_sla_decrease_shares_during_load(self):
        ...
        prometheus_stats = PrometheusDBStats(host=self.monitoring_set.nodes[0].external_address)
        sla_tests = SlaTests()
        error_events = sla_tests.test_decrease_shares_during_load(tester=self.tester, prometheus_stats=prometheus_stats, num_of_partitions=dataset_size, cassandra_stress_column_definition=column_definition)
        self.format_error_for_sla_test_and_raise(error_events=error_events)
    ...
    def disrupt_replace_service_level_using_detach_during_load(self):
        ...
        prometheus_stats = PrometheusDBStats(host=self.monitoring_set.nodes[0].external_address)
        sla_tests = SlaTests()
        error_events = sla_tests.test_replace_service_level_using_detach_during_load(tester=self.tester, prometheus_stats=prometheus_stats, num_of_partitions=dataset_size, cassandra_stress_column_definition=column_definition)
        self.format_error_for_sla_test_and_raise(error_events=error_events)
    ...
    def disrupt_replace_service_level_using_drop_during_load(self):
        ...
        prometheus_stats = PrometheusDBStats(host=self.monitoring_set.nodes[0].external_address)
        sla_tests = SlaTests()
        error_events = sla_tests.test_replace_service_level_using_drop_during_load(tester=self.tester, prometheus_stats=prometheus_stats, num_of_partitions=dataset_size, cassandra_stress_column_definition=column_definition)
    ...
    def disrupt_increase_shares_by_attach_another_sl_during_load(self):
        ...
        prometheus_stats = PrometheusDBStats(host=self.monitoring_set.nodes[0].external_address)
        sla_tests = SlaTests()
        error_events = sla_tests.test_increase_shares_by_attach_another_sl_during_load(tester=self.tester, prometheus_stats=prometheus_stats, num_of_partitions=dataset_size, cassandra_stress_column_definition=column_definition)
    ...
    def disrupt_maximum_allowed_sls_with_max_shares_during_load(self):
        ...
        error_events = sla_tests.test_maximum_allowed_sls_with_max_shares_during_load(
            tester=self.tester, prometheus_stats=prometheus_stats, num_of_partitions=dataset_size, cassandra_stress_column_definition=column_definition, service_levels_amount=MAX_ALLOWED_SERVICE_LEVELS - created_service_levels
        )
    ...
    def disrupt_create_index(self):
        ...
        with self.cluster.cql_connection_patient(self.target_node, connect_timeout=300) as session:
            ...
            column = get_random_column_name(session, ks, cf, filter_out_static_columns=True, filter_out_column_types=["counter"])
            if not column:
                raise UnsupportedNemesis("No column found to create index on")
            try:
                with DbNodeLogger(self.cluster.nodes, "create index", target_node=self.target_node, additional_info=f"on {ks}.{cf}.{column}"), self.action_log_scope(f"Create {ks}.{cf} {column} index"):
            ...
            try:
                ...
                sleep_for_percent_of_duration(self.tester.test_duration * 60, percent=1, min_duration=300, max_duration=2400)
            finally:
                with DbNodeLogger(self.cluster.nodes, "drop_index", target_node=self.target_node, additional_info=f"index: {index_name}"):
    ...
    def disrupt_add_remove_mv(self):
        ...
        with self.node_allocator.nodes_running_nemesis(cql_query_executor_node, self.current_disruption):
            ks_cfs = self.cluster.get_non_system_ks_cf_list(db_node=cql_query_executor_node, filter_empty_tables=True, filter_out_mv=True, filter_out_table_with_counter=True)
            if not ks_cfs:
                raise UnsupportedNemesis("Non-system keyspace and table are not found. nemesis can't be run")
            ...
            with self.cluster.cql_connection_patient(node=cql_query_executor_node, connect_timeout=600) as session:
                ...
                try:
                    ...
                    with (
                        adaptive_timeout(operation=Operations.CREATE_MV, node=self.target_node, timeout=14400) as timeout,
                        self.action_log_scope(f"Wait for {ks_name}.{view_name} materialized view to be built on {self.target_node.name} node"),
                    ):
    ...
    def _disrupt_toggle_audit(self, store: AuditStore):
        ...
        if ComparableScyllaVersion(self.target_node.scylla_version) <= ComparableScyllaVersion("2025.1"):
            raise UnsupportedNemesis("Audit feature log format was changed in Scylla 2025.2 and later. Use old sct-branch for Scylla < 2025.2")
        ...
        try:
            with self.action_log_scope(f"Enable {store} audit on categories: {audit_config.categories} in {keyspaces_for_audit} keyspace"):
    ...
    def disrupt_bootstrap_streaming_error(self):
        ...
        new_node: BaseNode = skip_on_capacity_issues(db_cluster=self.tester.db_cluster)(self.cluster.add_nodes)(
            count=1, dc_idx=self.target_node.dc_idx, enable_auto_bootstrap=True, rack=self.target_node.rack, disruption_name=self.current_disruption
        )[0]
        self.monitoring_set.reconfigure_scylla_monitoring()
        self.actions_log.info(f"Added new node : {new_node.name}")
        terminate_pattern = self.target_node.raft.get_random_log_message(operation=TopologyOperations.BOOTSTRAP, seed=self.nemesis_seed)
        bootstrapabortmanager = NodeBootstrapAbortManager(bootstrap_node=new_node, verification_node=self.target_node, actions_log=self.actions_log)
        ...
        if new_node.db_up() and self.target_node.raft.is_cluster_topology_consistent():
            ...
            with (
                self.node_allocator.run_nemesis(nemesis_label="BootstrapStreaminError", node_list=un_nodes) as verification_node,
    ...
    def disrupt_disable_binary_gossip_execute_major_compaction(self):
        ...
        try:
            self.cluster.wait_for_nodes_up_and_normal(nodes=[self.target_node])
            self.target_node.run_cqlsh("SELECT * FROM system_schema.keyspaces;", num_retry_on_failure=20, retry_interval=3)
        except Exception:
    ...
    def disrupt_serial_restart_elected_topology_coordinator(self):
        ...
        for num_of_restart in range(num_of_restarts):
            ...
            if coordinator_node != self.target_node and coordinator_node.running_nemesis:
                raise UnsupportedNemesis(f"Coordinator node is busy with {coordinator_node.running_nemesis}, Coordinator node was restarted: {num_of_restart}")
            ...
            with self.action_log_scope(f"Start Scylla on old coordinator {coordinator_node.name} node"):
                self.target_node.start_scylla()
            assert self.target_node != new_coordinator_node, f"New coordinator node was not elected while old one {coordinator_node.name} was stopped"
    ...
    def _refuse_connection_from_banned_node(self, use_iptables=False):
        ...
        with self.node_allocator.run_nemesis(nemesis_label=f"{simulate_node_unavailability.__name__}") as working_node, ExitStack() as stack:
            stack.enter_context(node_operations.block_loaders_payload_for_scylla_node(self.target_node, loader_nodes=self.loaders.nodes))
            target_host_id = self.target_node.host_id

            def _finalizer(exc_type, *_):
                if exc_type is not KillNemesis:
                    self._remove_node_add_node(verification_node=working_node, node_to_remove=self.target_node, remove_node_host_id=target_host_id)
            ...
            self.tester.create_table(name=table_name, keyspace_name=keyspace_name, key_type="bigint", columns={"name": "text"})
            ...
            with simulate_node_unavailability(self.target_node):
                ...
                self.actions_log.info(f"Blocked {self.target_node.name} node with {simulate_node_unavailability.__name__}")
                wait_for(node_operations.is_node_seen_as_down, step=5, timeout=600, throw_exc=True, down_node=self.target_node, verification_node=working_node, text=f"Wait other nodes see {self.target_node.name} as DOWN...")
                self.log.debug("Remove node %s : hostid: %s with blocked scylla from cluster", self.target_node.name, target_host_id)
                ...
                if simulate_node_unavailability == node_operations.pause_scylla_with_sigstop:
                    ...
                    working_node.run_nodetool(f"removenode {target_host_id}", retry=0, long_running=True)
                assert node_operations.is_node_removed_from_cluster(removed_node=self.target_node, verification_node=working_node), f"Node {self.target_node.name} with host id {target_host_id} was not removed. See log errors"
            ...
            with self.cluster.cql_connection_exclusive(node=self.target_node) as session:
                ...
                for key in random.sample(range(1, 100001), 1000):
                    try:
                        stmt = SimpleStatement(f"INSERT INTO {keyspace_name}.{table_name} (key, name) VALUES ({key}, 'name{key}');", consistency_level=ConsistencyLevel.QUORUM)
                        session.execute(stmt)
                        self.log.error("Banned query passed to cluster from banned node")
                        raise BannedQueryExecUnexpectedSuccess("Query from banned node was executed succesful with Consistency.QUORUM")
            ...
            with self.cluster.cql_connection_patient(working_node, whitelist_nodes=alive_cluster_nodes) as session:
                ...
                stmt = SimpleStatement(f"SELECT * from {keyspace_name}.{table_name}", consistency_level=ConsistencyLevel.QUORUM)
    ...
    def disrupt_kill_mv_building_coordinator(self):
        ...
        with self.cluster.cql_connection_patient(node=self.target_node, connect_timeout=600) as session:
            if not is_views_with_tablets_enabled(session):
                raise UnsupportedNemesis("MV building coordinator works only with tablets")
            ks_cfs = self.cluster.get_non_system_ks_cf_with_tablets_list(db_node=self.target_node, filter_empty_tables=True, filter_out_mv=True, filter_out_table_with_counter=True)
            if not ks_cfs:
        ...
        with self.node_allocator.run_nemesis(node_list=self.cluster.nodes, nemesis_label="Verification node for MV") as working_node:
            ...
            with self.cluster.cql_connection_patient(node=working_node, connect_timeout=600) as session:
                try:
                    ...
                    drop_materialized_view(session, ks_name, view_name)
                    raise UnsupportedNemesis(f"Skip nemesis because view {ks_name}.{view_name} has already finished building")
            ...
            try:
                ...
                for i in range(num_of_restarts):
                    ...
                    try:
                        ...
                        self.log.debug("Coordinator node is busy with %s, number of coordinator successful restarts: %s", coordinator_node.running_nemesis, i)
                        break
                ...
                with self.cluster.cql_connection_patient(node=working_node, connect_timeout=600) as session:
                    result = list(session.execute(SimpleStatement(f"SELECT * FROM {ks_name}.{view_name} limit 1", fetch_size=10)))
...
def disrupt_method_wrapper(method, is_exclusive=False):  # noqa: PLR0915
    ...
    def argus_finalize_nemesis_info(nemesis: Nemesis, method_name: str, start_time: int, nemesis_event: DisruptionEvent):
        ...
        try:
            ...
            if nemesis_event.severity == Severity.ERROR:
                argus_client.finalize_nemesis(name=method_name, start_time=start_time, status=NemesisStatus.FAILED, message=nemesis_event.full_traceback)
            elif nemesis_event.is_skipped:
                argus_client.finalize_nemesis(name=method_name, start_time=start_time, status=NemesisStatus.SKIPPED, message=nemesis_event.skip_reason)
            else:
                argus_client.finalize_nemesis(name=method_name, start_time=start_time, status=NemesisStatus.SUCCEEDED, message="")
        except Exception:
        ...
        try:
            if hasattr(args[0].tester, "data_validator") and args[0].tester.data_validator:
                if not (keyspace := args[0].tester.data_validator.keyspace_name):
                    DataValidatorEvent.DataValidator(severity=Severity.NORMAL, message="Failed fo get keyspace name. Data validator is disabled.").publish()
                    return
    ...
    def wrapper(*args, **kwargs):  # noqa: PLR0914, PLR0915
        ...
        try:
            ...
            start_msg = f"Started disruption {method_name} ({args[0].base_disruption_name} nemesis) on the target node '{str(args[0].target_node)}'"
            args[0].log.debug("{start_symbol} {msg} {start_symbol}".format(start_symbol=">" * 12, msg=start_msg))
            args[0].cluster.log_message("{start_symbol} {msg} {start_symbol}".format(start_symbol="=" * 12, msg=start_msg))
            ...
            with DisruptionEvent(nemesis_name=args[0].base_disruption_name, node=args[0].target_node, publish_event=True) as nemesis_event, args[0].actions_log.action_scope(f"Disruption {method_name} on {args[0].target_node.name}"):
                nemesis_info = argus_create_nemesis_info(nemesis=args[0], class_name=class_name, method_name=method_name, start_time=start_time)
                try:
                    ...
                    if nemesis_info:
                        argus_finalize_nemesis_info(nemesis=args[0], method_name=method_name, start_time=int(start_time), nemesis_event=nemesis_event)

                    end_msg = f"Finished disruption {method_name} ({args[0].base_disruption_name} nemesis) with status '{get_nemesis_status(nemesis_event)}'"
                    args[0].log.debug("{end_symbol} {msg} {end_symbol}".format(end_symbol="<" * 12, msg=end_msg))
                    args[0].cluster.log_message("{end_symbol} {msg} {end_symbol}".format(end_symbol="=" * 12, msg=end_msg))
            ...
            if num_data_nodes_before != num_data_nodes_after:
                args[0].log.error("num data nodes before %s and data nodes after %s does not match" % (num_data_nodes_before, num_data_nodes_after))
            if args[0].cluster.params.get("use_zero_nodes") and num_zero_nodes_before != num_zero_nodes_after:
                args[0].log.error("num zero nodes before %s and zero nodes after %s does not match" % (num_zero_nodes_before, num_zero_nodes_after))
            # TODO: Temporary print. Will be removed later
            data_validation_prints(args=args)
        finally:
            if is_exclusive:
...
class CategoricalMonkey(Nemesis):
    ...
    def get_disruption_distribution(dist: dict, default_weight: float) -> Tuple[List[Callable], List[float]]:
        ...
        for _name, _weight in dist.items():
            ...
            if not is_nonnegative_number(_weight):
                raise ValueError("Each disruption weight must be a non-negative number. '{weight}' is not a valid weight.")
    ...
    def get_disrupt_methods() -> Dict[str, Callable]:
        return {attr[0]: attr[1] for attr in inspect.getmembers(CategoricalMonkey) if attr[0].startswith("disrupt_") and callable(attr[1])}
...
class MdcChaosMonkey(Nemesis):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.disruptions_list = self.build_disruptions_by_name(["disrupt_destroy_data_then_repair", "disrupt_no_corrupt_repair", "disrupt_nodetool_decommission"])
        self.disruptions_list = self.shuffle_list_of_disruptions(self.disruptions_list)
