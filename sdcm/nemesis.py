...
def use_data_nodes_as_target(func):
    @wraps(func)
...
def use_zero_nodes_as_target(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
...
def use_any_nodes_as_target(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def _replace_cluster_node(self, old_node_ip: str | None = None, host_id: str | None = None,
                              timeout: int | float = MAX_TIME_WAIT_FOR_NEW_NODE_UP, rack=0, is_token_node: bool = False) -> BaseNode:
        ...
        if is_token_node:
            new_node = skip_on_capacity_issues(self.cluster.add_nodes)(
                count=1, dc_idx=self.target_node.dc_idx, enable_auto_bootstrap=True, rack=rack, is_token_node=is_token_node)[0]
        else:
            new_node = skip_on_capacity_issues(self.cluster.add_nodes)(
    ...
    def replace_node(self, old_node_ip: str, host_id: str, rack: int = 0, is_token_node: bool = False) -> BaseNode:
    ...
    def disrupt_terminate_and_replace_node(self):  # pylint: disable=invalid-name
        ...
        new_node = self.replace_node(old_node_ip, host_id, rack=self.target_node.rack,
                                     is_token_node=self.target_node._is_zero_token_node)
        try:
    ...
    def disrupt_grow_shrink_zero_nodes(self):
        ...
        if self.cluster.params.get('cluster_backend') != 'aws':
            raise UnsupportedNemesis("The zero tokens  only supports 'AWS'")
