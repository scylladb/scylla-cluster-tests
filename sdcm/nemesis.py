...
class Nemesis(NemesisFlags):
    ...
    def disrupt_restart_with_resharding(self):
        ...
        self.log.info(
            f"Restart node with resharding. New murmur3_partitioner_ignore_msb_bits value: {murmur3_partitioner_ignore_msb_bits}"
        )
    ...
    def replace_full_file_name_to_prefix(self, one_file, ks_cf_for_destroy):
        ...
        try:
            ...
            self.log.debug(
                'File name "{file_name}" is not as expected for Scylla data files. Search files for "{ks_cf_for_destroy}" table'.format(
    ...
    def _nemesis_stress_failure_handler(self, stress_pool, errors):
        ...
        if len(errors) == len(stress_pool.get_results()):
            ...
            raise NemesisStressFailure(
                f"Aborting '{self.__class__.__name__}' nemesis as stress command failed with the following errors:\n{errors_str}"
            )
    ...
    def modify_table_twcs_window_size(self):
        ...
        if not reshape_twcs_records:
            self.log.warning(
                "Log message with sstables for reshape was not found. Autocompaction alreadycompact sstables by timewindows"
            )
    ...
    def disrupt_snapshot_operations(self):
        ...
        def _few_tables():
            ...
            if not tables:
                ...
                # If not - raise exception to investigate the issue
                raise Exception(
                    f'Something wrong happened, tables for "{selected_keyspace}" keyspace were notfound in the list:\n{ks_cf}'
                )
    ...
    def disrupt_remove_node_then_add_node(self):
        ...
        if self.cluster.params.get("db_type") == "cloud_scylla":
            raise UnsupportedNemesis(
                "Skipping this nemesis due the replace node option that supported by Cloud is tested by CloudReplaceNonResponsiveNode nemesis"
            )
        if self._is_it_on_kubernetes():
    ...
    def disrupt_network_reject_inter_node_communication(self):
        ...
        InfoEvent(
            f"{name} {textual_matching_rule} that belongs to inter node communication connections (port=7000 and 7001) will be {textual_pkt_action} for {wait_time}s"
        ).publish()
    ...
    def disrupt_network_reject_node_exporter(self):
        ...
        InfoEvent(
            f"{name} {textual_matching_rule} that belongs to node-exporter(port=9100) connections will be {textual_pkt_action} for {wait_time}s"
        ).publish()
    ...
    def disrupt_network_reject_thrift(self):
        ...
        InfoEvent(
            f"{name} {textual_matching_rule} that belongs to Thrift(port=9160) connections will be {textual_pkt_action} for {wait_time}s"
        ).publish()
    ...
    def start_and_interrupt_decommission_streaming(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            with (
                ...
                ignore_raft_topology_cmd_failing(),
                self.run_nemesis(
                    node_list=self.cluster.data_nodes, nemesis_label="DecommissionStreamingErr"
    ...
    def _add_new_node_in_new_dc(self, is_zero_node=False) -> BaseNode:
        if is_zero_node:
            new_node = skip_on_capacity_issues(self.cluster.add_nodes)(
                1, dc_idx=0, enable_auto_bootstrap=True, is_zero_node=is_zero_node
            )[0]  # add node
        else:
    ...
    def _switch_to_network_replication_strategy(self, keyspaces: List[str]) -> None:
        ...
        for keyspace in keyspaces:
            ...
            if keyspace == "system_auth" and replication_strategy.replication_factors[0] != len(
                ...
                self.log.warning(
                    f"system_auth keyspace is not replicated on all nodes ({replication_strategy.replication_factors[0]}/{len(nodes_by_region[region])})."
                )
            with self.action_log_scope(
                "Switching replication strategy to Network",
    ...
    def disrupt_add_remove_dc(self) -> None:
        ...
        self.tester.create_keyspace(
            "keyspace_new_dc", replication_factor={datacenters[0]: min(3, len(self.cluster.data_nodes))}
        )
        node_added = False
            ...
            with temporary_replication_strategy_setter(node) as replication_strategy_setter:
                ...
                with (
                    ...
                    self.action_log_scope(
                        "Run rebuild on the new datacenter", target=new_node.name, metadata={"nodetool_cmd": cmd}
                    ),
                ):
                ...
                for cluster_node in self.cluster.data_nodes:
                    with self.action_log_scope("Run repair", target=cluster_node.name, metadata={"nodetool_cmd": cmd}):
                        cluster_node.run_nodetool(sub_cmd=cmd, publish_event=True)
                datacenters = list(self.tester.db_cluster.get_nodetool_status().keys())
                with self.action_log_scope(
                    "Run write and then read data to multiDC keyspace", target=self.target_node.name
                ):
    ...
    def disrupt_create_index(self):
        ...
        if is_tablets_feature_enabled(self.target_node):
            if SkipPerIssues(
                issues="https://github.com/scylladb/scylla-enterprise/issues/5461", params=self.tester.params
            ):
    ...
    def disrupt_bootstrap_streaming_error(self):
        ...
        if new_node.db_up() and self.target_node.raft.is_cluster_topology_consistent():
            ...
            with (
                Nemesis.run_nemesis(node_list=un_nodes, nemesis_label="BootstrapStreaminError") as verification_node,
                FailedDecommissionOperationMonitoring(
    ...
    def disrupt_serial_restart_elected_topology_coordinator(self):
        ...
        for num_of_restart in range(num_of_restarts):
            with self.run_nemesis(
                node_list=self.cluster.nodes, nemesis_label="search coordinator"
            ) as verification_node:
    ...
    def _refuse_connection_from_banned_node(self, use_iptables=False):
        ...
        with (
            self.run_nemesis(
                node_list=self.cluster.nodes, nemesis_label=f"Running {simulate_node_unavailability.__name__}"
            ) as working_node,
            ExitStack() as stack,
        ):
            stack.enter_context(
...
def disrupt_method_wrapper(method, is_exclusive=False):  # noqa: PLR0915
    ...
    def wrapper(*args, **kwargs):  # noqa: PLR0914, PLR0915
        ...
        try:
            ...
            start_msg = f"Started disruption {method_name} ({current_disruption} nemesis) on the target node '{str(args[0].target_node)}'"
            ...
            with (
                ...
                args[0].actions_log.action_scope(
                    "Disrupt method",
                    target=args[0].target_node.name,
                ...
                try:
                    ...
                    end_msg = f"Finished disruption {method_name} ({current_disruption} nemesis) with status '{get_nemesis_status(nemesis_event)}'"
