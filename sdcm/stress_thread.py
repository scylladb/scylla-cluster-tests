...
class CassandraStressThread(DockerBasedStressThread):
    ...
    def __init__(
        ...
        keyspace_num=1,
        keyspace_name="",
        compaction_strategy="",  # noqa: PLR0913
        profile=None,
        node_list=None,
        round_robin=False,
        ...
        super().__init__(loader_set=loader_set, stress_cmd=stress_cmd, timeout=timeout, stress_num=stress_num, node_list=node_list, round_robin=round_robin, stop_test_on_failure=stop_test_on_failure, params=params)
    ...
    def set_stress_operation(self, stress_cmd):
        if " mixed " in stress_cmd:
            ...
            raise ValueError("Cannot detect supported stress operation type from the stress command: %s" % stress_cmd)
    ...
    def set_hdr_tags(self, stress_cmd):
        ...
        if "user profile=" in stress_cmd:
            ...
            if "insert=" in stress_cmd:
                ...
                raise ValueError("Cannot detect supported stress operation type from the stress command with user profile: %s", stress_cmd)
        ...
        if self.node_list and "-node" not in stress_cmd:
            ...
            if self.loader_set.test_config.MULTI_REGION:
                ...
                if loader_dc := datacenter_name_per_region.get(loader.region):
                    ...
                    LOGGER.error("Not found datacenter for loader region '%s'. Datacenter per loader dict: %s", loader.region, datacenter_name_per_region)
            ...
            if self.params.get("rack_aware_loader"):
                ...
                if any(len(racks) > 1 for racks in by_region_rack_names.values()) and "rack" in self._get_available_suboptions(cmd_runner, "-node"):
                    if loader_rack := rack_names.get((str(loader.region), str(loader.rack))):
                        ...
                        node_list = self.loader_set.get_nodes_per_datacenter_and_rack_idx(db_nodes=self.node_list).get((str(loader.region), str(loader.rack)))
    ...
    def _add_errors_option(stress_cmd: str, to_add: list) -> str:
        ...
        new_error_suboptions = list({suboption.split("=", 1)[0]: suboption for suboption in to_add + current_error_suboptions}.values())
    ...
    def _get_available_suboptions(self, loader, option, _cache={}):  # noqa: B006
        if cached_value := _cache.get(option):
            return cached_value
        try:
            result = loader.run(cmd=f'cassandra-stress help {option} | grep "^Usage:"', timeout=self.timeout, ignore_status=True).stdout
            ...
            if match := re.search(r"\s-log ([^-]*)-?", stress_cmd):
                ...
                if "hdrfile" not in cs_log_option:
                    ...
                    if replacing_hdr_file := re.search(r"hdrfile=(.*?)\s", cs_log_option):
                        stress_cmd = stress_cmd.replace(f"hdrfile={replacing_hdr_file.group(1)}", f"hdrfile={hdr_log_name}")
        else:
    ...
    def _run_cs_stress(self, loader, loader_idx, cpu_idx, keyspace_idx):  # noqa: PLR0914
        ...
        if "k8s" in self.params.get("cluster_backend"):
            ...
            remote_hdr_file_name_full_path = loader.remoter.run(f"realpath $HOME/{remote_hdr_file_name}", ignore_status=False, verbose=False).stdout.strip()
            cmd_runner_name = loader.ip_address
        ...
        if self.params.get("cs_debug"):
            cmd_runner.send_files(get_data_dir_path("logback-tools-debug.xml"), "/etc/scylla/cassandra/logback-tools.xml", delete_dst=True)
        if self.profile:
        ...
        try:
            ...
            reporter = CassandraStressVersionReporter(cmd_runner, prefix, loader.parent_cluster.test_config.argus_client())
            reporter.report()
        except Exception:  # noqa: BLE001
            LOGGER.info("Failed to collect cassandra-stress version information", exc_info=True)
        with (
            cleanup_context,
            CassandraStressExporter(instance_name=cmd_runner_name, metrics=nemesis_metrics_obj(), stress_operation=stress_cmd_opt, stress_log_filename=log_file_name, loader_idx=loader_idx, cpu_idx=cpu_idx),
            CassandraStressEventsPublisher(node=loader, cs_log_filename=log_file_name, stop_test_on_failure=self.stop_test_on_failure) as publisher,
            CassandraStressEvent(node=loader, stress_cmd=self.stress_cmd, log_file_name=log_file_name) as cs_stress_event,
            CassandraStressHDRExporter(instance_name=cmd_runner_name, hdr_tags=self.hdr_tags, metrics=nemesis_metrics_obj(), stress_operation=stress_cmd_opt, stress_log_filename=local_hdr_file_name, loader_idx=loader_idx, cpu_idx=cpu_idx),
            hdrh_logger_context,
        ):
        ...
        for loader in self.loaders:
            ...
            for cpu_idx in range(self.stress_num):
                for ks_idx in range(1, self.keyspace_num + 1):
                    self.results_futures += [self.executor.submit(self._run_cs_stress, *(loader, loader_idx, cpu_idx, ks_idx))]
