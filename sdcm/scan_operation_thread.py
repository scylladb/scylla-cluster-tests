...
class FullscanOperationBase:
    def __init__(
        ...
        scan_event: Type[FullScanEvent]
        | Type[FullPartitionScanEvent]
        | Type[FullPartitionScanReversedOrderEvent]
        | Type[FullScanAggregateEvent],
    ):
        """
    ...
    def execute_query(
        ...
        session,
        cmd: str,
        event: Type[FullScanEvent | FullPartitionScanEvent | FullPartitionScanReversedOrderEvent],
    ) -> ResultSet:
        # pylint: disable=unused-argument
        self.log.debug("Will run command %s", cmd)
        return session.execute(
            SimpleStatement(cmd, fetch_size=self.fullscan_params.page_size, consistency_level=ConsistencyLevel.ONE)
        )

    def run_scan_event(
        self,
        cmd: str,
        scan_event: Type[FullScanEvent | FullPartitionScanEvent | FullPartitionScanReversedOrderEvent] = None,
    ) -> OneOperationStat:
        scan_event = scan_event or self.scan_event
        cmd = cmd or self.randomly_form_cql_statement()
        with scan_event(
            node=self.db_node.name,
            ks_cf=self.fullscan_params.ks_cf,
            message=f"Will run command {cmd}",
            user=self.fullscan_params.user,
            password=self.fullscan_params.user_password,
        ) as scan_op_event:
            self.current_operation_stat = OneOperationStat(
                op_type=scan_op_event.__class__.__name__, nemesis_at_start=self.db_node.running_nemesis, cmd=cmd
            )
            ...
            with self.fullscan_params.db_cluster.cql_connection_patient(
                ...
                try:
                    ...
                    msg = (
                        f"{msg} while running Nemesis: {self.db_node.running_nemesis}"
...
class FullPartitionScanOperation(FullscanOperationBase):
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.ck_filter = ""
        self.limit = ""
        self.reversed_query_output = tempfile.NamedTemporaryFile(  # pylint: disable=consider-using-with
            mode="w+",
            delete=False,
            encoding="utf-8",
        )
        self.normal_query_output = tempfile.NamedTemporaryFile(  # pylint: disable=consider-using-with
            mode="w+",
            delete=False,
            encoding="utf-8",
        )
    ...
    def get_table_clustering_order(self) -> str:
        ...
        try:
            with self.fullscan_params.db_cluster.cql_connection_patient(node=node, connect_timeout=300) as session:
                ...
                return get_table_clustering_order(
                    ks_cf=self.fullscan_params.ks_cf, ck_name=self.fullscan_params.ck_name, session=session
                )
        except Exception as error:  # pylint: disable=broad-except
            self.log.error(traceback.format_exc())
            self.log.error(
                "Failed getting table %s clustering order through node %s : %s",
    ...
    def randomly_form_cql_statement(self) -> Optional[tuple[str, str]]:  # pylint: disable=too-many-branches
        ...
        with self.fullscan_params.db_cluster.cql_connection_patient(node=db_node, connect_timeout=300) as session:
            ...
            if pks := get_partition_keys(
                ks_cf=self.fullscan_params.ks_cf, session=session, pk_name=self.fullscan_params.pk_name
            ):
                ...
                reversed_query = (
                    f"select {','.join(selected_columns)} from {self.fullscan_params.ks_cf}"
                ...
                self.log.debug(
                    "[scan: %s, type: %s] Randomly formed reversed query is: %s",
                ...
                self.log.debug(
                    "No partition keys found for table: %s! A reversed query cannot be executed!",
                    self.fullscan_params.ks_cf,
                )
                return None
    ...
    def _compare_output_files(self) -> bool:
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=True, encoding="utf-8") as reorder_normal_query_output:
            ...
            LOCAL_CMD_RUNNER.run(
                cmd=f"cp {reorder_normal_query_output.name} {self.normal_query_output.name}", ignore_status=True
            )
            self.normal_query_output.flush()
        ...
        if not result.stderr:
            ...
            self.log.warning(
                "Comparison of output of normal and reversed queries failed with stderr:\n%s", result.stderr
            )
    ...
    def run_scan_operation(self, cmd: str = None):  # pylint: disable=too-many-locals
        ...
        full_partition_op_stat = OneOperationStat(
            op_type=self.__class__.__name__, nemesis_at_start=self.db_node.running_nemesis, cmd=str(queries)
        )
...
class FullScanAggregatesOperation(FullscanOperationBase):
    ...
    def execute_query(
        ...
        try:
            ...
            self.current_operation_stat.exceptions.append(repr(exc))
            event.message = (
                f"{type(self).__name__} operation failed: {repr(exc)},"
            ...
            if "Operation timed out" in repr(exc):
                event.message = (
                    f"{type(self).__name__} operation failed due to operation timed out: {repr(exc)},"
    ...
    def _validate_fullscan_result(self, cmd_result: ResultSet, validate_forward_service_requests_start_time):
        ...
        try:
            ...
            self.log.debug("Fullscan aggregation result: %s", output)
            return (
                "Fullscan failed - 'forward_service_requests_dispatched_to_other_nodes' was not triggered",
                Severity.ERROR,
            )
    ...
    def validate_forward_service_requests_dispatched_to_other_nodes(self, start_time):
        ...
        prometheus_forward_service_requests = prometheus.query(
            "scylla_forward_service_requests_dispatched_to_other_nodes", start_time, time.time()
        )
