...
from sdcm.sct_events.database import FullScanEvent, FullPartitionScanReversedOrderEvent, FullPartitionScanEvent, FullScanAggregateEvent
...
class ScanOperationThread(OperationThread):
    ...
    def __init__(self, thread_params: ThreadParams, thread_name: str = ""):
        ...
        self.operation_instance_map = {
            "table_and_aggregate": lambda: self.generator.choice([full_scan_operation, full_scan_aggregates_operation]),
            "random": lambda: self.generator.choice([full_scan_operation, full_scan_aggregates_operation, full_partition_scan_operation]),
...
class FullscanOperationBase:
    def __init__(
        ...
        thread_stats: OperationThreadStats,
        scan_event: Type[FullScanEvent] | Type[FullPartitionScanEvent] | Type[FullPartitionScanReversedOrderEvent] | Type[FullScanAggregateEvent],
    ):
        """
        ...
        self._exp_backoff_retry_policy_params = {"max_num_retries": 15.0, "min_interval": 1.0, "max_interval": 1800.0}
    ...
    def execute_query(self, session, cmd: str, event: Type[FullScanEvent | FullPartitionScanEvent | FullPartitionScanReversedOrderEvent]) -> ResultSet:
        self.log.debug("Will run command %s", cmd)
        return session.execute(SimpleStatement(cmd, fetch_size=self.fullscan_params.page_size, consistency_level=ConsistencyLevel.ONE))

    def run_scan_event(self, cmd: str, scan_event: Type[FullScanEvent | FullPartitionScanEvent | FullPartitionScanReversedOrderEvent] = None) -> OneOperationStat:
        scan_event = scan_event or self.scan_event
        cmd = cmd or self.randomly_form_cql_statement()
        with scan_event(node=self.db_node.name, ks_cf=self.fullscan_params.ks_cf, message=f"Will run command {cmd}", user=self.fullscan_params.user, password=self.fullscan_params.user_password) as scan_op_event:
            self.current_operation_stat = OneOperationStat(op_type=scan_op_event.__class__.__name__, nemesis_at_start=self.db_node.running_nemesis, cmd=cmd)
            ...
            with self.cql_connection(connect_timeout=300) as session:
                try:
                    ...
                    msg = f"{msg} while running Nemesis: {self.db_node.running_nemesis}" if self.db_node.running_nemesis else msg
    ...
    def cql_connection(self, **kwargs):
        ...
        with self.fullscan_params.db_cluster.cql_connection_patient(node=node, user=self.fullscan_params.user, password=self.fullscan_params.user_password, **kwargs) as session:
...
class FullScanOperation(FullscanOperationBase):
    ...
    def randomly_form_cql_statement(self) -> str:
        ...
        cmd = base_query.substitute(ks_cf=self.fullscan_params.ks_cf, timeout=f" USING TIMEOUT {self.fullscan_params.full_scan_operation_limit}s", bypass_cache=BYPASS_CACHE_VALUES[0])
        return cmd
...
class FullPartitionScanOperation(FullscanOperationBase):
    ...
    reversed_query_filter_ck_by = {"lt": " and {} < {}", "gt": " and {} > {}", "lt_and_gt": " and {} < {} and {} > {}", "no_filter": ""}
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.reversed_query_filter_ck_stats = {
            "lt": {"count": 0, "total_scan_duration": 0},
    ...
    def get_table_clustering_order(self) -> str:
        ...
        try:
            with self.cql_connection(node=node, connect_timeout=300) as session:
                ...
                return get_table_clustering_order(ks_cf=self.fullscan_params.ks_cf, ck_name=self.fullscan_params.ck_name, session=session)
        except Exception as error:  # noqa: BLE001
            self.log.error(traceback.format_exc())
            self.log.error("Failed getting table %s clustering order through node %s : %s", self.fullscan_params.ks_cf, node.name, error)
    ...
    def randomly_form_cql_statement(self) -> Optional[tuple[str, str]]:
        ...
        with self.cql_connection(node=db_node, connect_timeout=300) as session:
            ...
            if pks := get_partition_keys(ks_cf=self.fullscan_params.ks_cf, session=session, pk_name=self.fullscan_params.pk_name):
                ...
                reversed_query = f"select {','.join(selected_columns)} from {self.fullscan_params.ks_cf}" + f" where {self.fullscan_params.pk_name} = {partition_key}"
                ...
                match ck_filter:
                    case "lt_and_gt":
                        ...
                        reversed_query += self.reversed_query_filter_ck_by[ck_filter].format(self.fullscan_params.ck_name, ck_random_max_value, self.fullscan_params.ck_name, ck_random_min_value)
                    ...
                    case "gt":
                        ...
                        # order by ck desc limit 5
                        # normal query should be: select * from scylla_bench.test where pk = 1 and ck > 15 limit 5
                        reversed_query += self.reversed_query_filter_ck_by[ck_filter].format(self.fullscan_params.ck_name, ck_random_min_value)
                ...
                self.log.debug("[scan: %s, type: %s] Randomly formed reversed query is: %s", self.fullscan_stats.scans_counter, ck_filter, reversed_query)
            else:
                self.log.debug("No partition keys found for table: %s! A reversed query cannot be executed!", self.fullscan_params.ks_cf)
                return None
    ...
    def execute_query(self, session, cmd: str, event: Type[FullScanEvent | FullPartitionScanEvent | FullPartitionScanReversedOrderEvent]) -> ResponseFuture:
        self.log.debug('Will run command "%s"', cmd)
    ...
    def _compare_output_files(self) -> bool:
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=True, encoding="utf-8") as reorder_normal_query_output:
            ...
            LOCAL_CMD_RUNNER.run(cmd=f"cp {reorder_normal_query_output.name} {self.normal_query_output.name}", ignore_status=True)
            self.normal_query_output.flush()
            ...
            log_file = Path(TestConfig().logdir()) / "fullscans" / f"partition_range_scan_diff_{datetime.datetime.now(tz=utc).strftime('%Y_%m_%d-%I_%M_%S')}.log"
    ...
    def run_scan_operation(self, cmd: str = None):
        ...
        full_partition_op_stat = OneOperationStat(op_type=self.__class__.__name__, nemesis_at_start=self.db_node.running_nemesis, cmd=str(queries))
...
class FullScanAggregatesOperation(FullscanOperationBase):
    ...
    def randomly_form_cql_statement(self) -> str:
        cmd = FullScanAggregateCommands.AGG_COUNT_ALL.base_query.substitute(ks_cf=self.fullscan_params.ks_cf, timeout=f" USING TIMEOUT {self.fullscan_params.full_scan_aggregates_operation_limit}s", bypass_cache=BYPASS_CACHE_VALUES[0])
        return cmd

    def execute_query(self, session, cmd: str, event: Type[FullScanEvent | FullPartitionScanEvent | FullPartitionScanReversedOrderEvent]) -> None:
        self.log.debug("Will run command %s", cmd)
        validate_mapreduce_service_requests_start_time = time.time()
        cmd_result = session.execute(query=cmd, trace=False, timeout=self._session_execution_timeout)
    ...
    def _validate_fullscan_result(self, cmd_result: ResultSet, validate_mapreduce_service_requests_start_time):
        ...
        try:
            self.validate_mapreduce_service_requests_dispatched_to_other_nodes(validate_mapreduce_service_requests_start_time)
        except Retry as retry_exception:
            self.log.debug("prometheus_mapreduce_service_requests metrics:\n %s", str(retry_exception))
            self.log.debug("Fullscan aggregation result: %s", output)
    ...
    def validate_mapreduce_service_requests_dispatched_to_other_nodes(self, start_time):
        prometheus = PrometheusDBStats(TestConfig().tester_obj().monitors.nodes[0].external_address)
        prometheus_mapreduce_service_requests = prometheus.query("scylla_mapreduce_service_requests_dispatched_to_other_nodes", start_time, time.time())
...
class PagedResultHandler:
    ...
    def handle_page(self, rows):
        ...
        if self.scan_operation.scan_event == FullPartitionScanEvent:
            for row in rows:
                self.scan_operation.normal_query_output.write(self._row_to_string(row=row, include_data_column=include_data_column))
            ...
            if self.scan_operation.fullscan_params.validate_data:
                for row in rows:
                    self.scan_operation.reversed_query_output.write(self._row_to_string(row=row, include_data_column=include_data_column))
