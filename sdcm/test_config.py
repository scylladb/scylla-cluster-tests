...
class TestConfig(metaclass=Singleton):
    ...
    def common_tags(cls) -> Dict[str, str]:
        ...
        tags = dict(RunByUser=get_username(), TestName=str(cls.test_name()), TestId=str(cls.test_id()), version=job_name.split("/", 1)[0] if job_name else "unknown", CreatedBy="SCT")
    ...
    def _link_running_syslog_logdir(cls, syslog_logdir):
        ...
        if syslog_logdir == current_logdir:
            ...
            return
        LOGGER.debug("Syslog docker is running on the another directory. Linking it's directory %s to %s", syslog_logdir, current_logdir)
