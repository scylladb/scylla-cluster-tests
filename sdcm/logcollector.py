...
from sdcm.paths import SCYLLA_YAML_PATH, SCYLLA_PROPERTIES_PATH, SCYLLA_MANAGER_AGENT_YAML_PATH, SCYLLA_MANAGER_YAML_PATH
...
class CollectingNode:
    ...
    def install_package(self, package_name: str, package_version: str = None, ignore_status: bool = False) -> None:
        if self.distro.is_rhel_like:
            ...
            pkg_cmd = 'DEBIAN_FRONTEND=noninteractive apt-get -o DPkg::Lock::Timeout=120 -o Dpkg::Options::="--force-confold" -o Dpkg::Options::="--force-confdef"'
            ...
            if version_prefix:
                ...
                result = self.remoter.run(f'apt-cache depends --recurse {package_name}{version_prefix} | grep {package_name} | grep Depends | cut -d ":" -f 2', ignore_status=ignore_status)
                packages_to_install = [f"{pkg}{version_prefix}" for pkg in [package_name] + list(set(result.stdout.splitlines()))]
...
class BaseMonitoringEntity(BaseLogEntity):
    ...
    def get_monitoring_version(self, node):
        try:
            ...
            result = node.remoter.run(rf"ls {basedir} | grep 'scylla-monitoring-src\|scylla-grafana-monitoring-scylla-monitoring'", ignore_status=True, verbose=False)
...
class CommandLog(BaseLogEntity):
    ...
    def collect(self, node, local_dst, remote_dst=None, local_search_path=None) -> Optional[str]:
        ...
        remote_logfile, is_file_remote = LogCollector.collect_log_remotely(node=node, cmd=self.cmd, log_filename=os.path.join(remote_dst, self.name))
        if not remote_logfile:
            LOGGER.warning("Nothing to collect. Command '%s' did not prepare log file on remote host '%s'", self.cmd, node.name)
            return None
        local_path = Path(local_dst) / Path(remote_logfile).name
        if is_file_remote:
            LogCollector.receive_log(node=node, remote_log_path=remote_logfile, local_dir=local_dst, timeout=self.collect_timeout)
        else:
            # copy locally
...
class DirLog(FileLog):
    ...
    def collect_from_builder(self, builder, local_dst, search_in_dir) -> None:
        ...
        LOGGER.warning("'DirLog' class doesn't support 'collect_from_builder' method. It is to be implemented. Ignoring gathering following logs: '%s'", self.name)
...
class PrometheusSnapshots(BaseMonitoringEntity):
    ...
    @retrying(n=3, sleep_time=10, allowed_exceptions=(PrometheusSnapshotErrorException, Exception), message="Create prometheus snapshot")
    ...
    def collect(self, node, local_dst, remote_dst=None, local_search_path=None) -> Optional[str]:
        ...
        if remote_snapshot_archive := self.get_prometheus_snapshot_remote(node):
            LogCollector.receive_log(node, remote_log_path=remote_snapshot_archive, local_dir=local_dst, timeout=self.collect_timeout)
...
class MonitoringStack(BaseMonitoringEntity):
    ...
    def get_monitoring_data_stack(self, node, local_dist) -> str:
        ...
        node.remoter.send_files(src=annotations_file.name, dst=os.path.join(monitor_base_dir, monitor_install_dir_name, "sct_monitoring_addons"))

        archive_name = os.path.join(monitor_base_dir, f"monitoring_data_stack_{monitor_branch}_{monitor_version}.tar.gz")
        if not node.remoter.run(f"tar czvf '{archive_name}' --exclude='gpx_scylladb_*' -C '{monitor_base_dir}' '{monitor_install_dir_name}'", ignore_status=True).ok:
            return ""
    ...
    def get_grafana_screenshot(self, node, local_dst):
        ...
        try:
            for dashboard in self.grafana_dashboards:
                try:
                    dashboard_metadata = MonitoringStack.get_dashboard_by_title(grafana_ip=normalize_ipv6_url(node.grafana_address), port=self.grafana_port, title=dashboard.title)
                    if not dashboard_metadata:
                        LOGGER.error("Dashboard with title '%s' was not found", dashboard.title)
                        continue

                    grafana_url = self.grafana_entity_url_tmpl.format(node_ip=normalize_ipv6_url(node.grafana_address), grafana_port=self.grafana_port, path=dashboard_metadata["url"], st=self.start_time)
                    screenshot_path = os.path.join(local_dst, "%s-%s-%s-%s.png" % (self.name, dashboard.name, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"), node.name))
                    ...
                    with requests.get(grafana_url, stream=True, params=f"width={dashboard.resolution[0]}&height=-1&kiosk") as response:
...
class LogCollector:
    ...
    def create_local_storage_dir(self, base_local_dir):
        local_dir = os.path.join(base_local_dir, self.current_run, "{}-{}".format(self.cluster_log_type, self.test_id[:8]))
        try:
    ...
    def collect_log_remotely(node, cmd: str, log_filename: str) -> Tuple[Optional[str], bool]:
        ...
        if not ssh_connected and (ssm_runner := node.aws_ssm_runner):
            ...
            try:
                ...
                ssm_result = ssm_runner.run_command_and_save_output(command=collect_log_command, local_output_file=log_filename, comment=f"Collect log {log_filename}", ignore_status=True)
                ok = ssm_result.ok
                if not ssm_result.ok:
                    LOGGER.error("SSM command failed for instance %s: %s ", node._instance.get("InstanceId"), collect_log_command)
    ...
    def collect_logs(self, local_search_path: Optional[str] = None) -> list[str]:
        def collect_logs_per_node(node):
            ...
            for log_entity in self.log_entities:
                try:
                    log_entity.collect(node=node, local_dst=local_parent_dir if log_entity.collect_from_parent else local_node_dir, remote_dst=remote_node_dir, local_search_path=local_search_path)
                except Exception as details:  # noqa: BLE001
        ...
        if workers_number := len(self.nodes):
            ...
            try:
                ParallelObject(self.nodes, num_workers=workers_number, timeout=self.collect_timeout).run(collect_logs_per_node, ignore_exceptions=True)
            except Exception as details:  # noqa: BLE001
        ...
        for root, _, _ in os.walk(local_search_path):
            if self.cluster_dir_prefix in root:
                for entity in self.log_entities:
                    ...
                    node_dirs = {dir_name for dir_name in os.listdir(root) if os.path.isdir(os.path.join(root, dir_name))}
                    if len(node_names) != len(node_dirs):
                        inactive_nodes = node_dirs.difference(node_names)
                        for dir_name in inactive_nodes:
                            entity.collect(CollectingNode(name=dir_name), os.path.join(self.local_dir, dir_name), local_search_path=os.path.join(root, dir_name))
    ...
    def _compress_file(self, src_path: str, src_name: str) -> str:
        ...
        LocalCmdRunner().run(cmd=f"tar --zstd --warning=no-file-changed -cf '{archive_name}' -C '{archive_dir}' --transform 's/{log_filename}/{src_name}/' '{log_filename}'")
...
def save_kallsyms_map(node):
    ...
    if remote_node_dir := create_remote_storage_dir(node):
        uptime = datetime.datetime.strptime(node.remoter.run("uptime -s", ignore_status=True).stdout.strip(), "%Y-%m-%d %H:%M:%S").strftime("%Y%m%d_%H%M%S")
        ...
        if os.path.exists(kallsyms_file_path):
            LOGGER.debug("The kallsyms file '%s' already exists and not changed. Not collecting it's map", kallsyms_file_path)
            return
    ...
    if remote_node_dir := create_remote_storage_dir(node):
        for log_entity in log_entities:
            if os.path.exists(os.path.join(node.logdir, log_entity.name)):
                LOGGER.debug("Diagnostic file '%s' already exists and not changed. Skipping collection.", log_entity.name)
                continue
            try:
...
class LoaderLogCollector(LogCollector):
    ...
    log_entities = [
        ...
        FileLog(name="test.crt", command="test -f /etc/scylla/ssl_conf/{0} && cat /etc/scylla/ssl_conf/{0}".format("test.crt")),
    ]
...
class MonitorLogCollector(LogCollector):
    ...
    log_entities = [
        ...
        FileLog(name="scylla_manager.log", command="sudo journalctl -u scylla-manager.service --no-tail", search_locally=True),
        FileLog(name="manager_scylla_backend.log", command="sudo journalctl -u scylla-server.service -u scylla-jmx.service --no-tail", search_locally=True),
        ...
        if not os.listdir(self.local_dir):
            ...
            for obj in builders:
                builder = CollectingNode(
                    name=obj["builder"]["name"], ssh_login_info={"hostname": obj["builder"]["public_ip"], "user": obj["builder"]["user"], "key_file": obj["builder"]["key_file"]}, instance=None, global_ip=obj["builder"]["public_ip"]
                )
                for ent in self.log_entities:
...
class PythonSCTLogCollector(BaseSCTLogCollector):
    ...
    def _compress_file(self, src_path: str, src_name: str) -> list[str]:
        ...
        if os.path.getsize(src_path) < self.too_big_log_size:
            ...
            runner.run(f"bash {os.path.join(os.path.dirname(__file__), 'log_archive.sh')} {src_path} {self.too_big_log_size} {src_name}")
...
class KubernetesAPIServerLogCollector(BaseSCTLogCollector):
    ...
    def generate_apiserver_call_stats_file(self, apiserver_logdir: str) -> None:
        ...
        if not os.path.exists(apiserver_logdir):
            LOGGER.warning("Kube API server logs dir '%s' doesn't exist. Cannot calculate Kube API call stats", apiserver_logdir)
            return
    ...
    def collect_logs(self, local_search_path: Optional[str] = None) -> list[str]:
        try:
            ...
            s3_link = upload_remote_files_directly_to_s3(
                node.ssh_login_info, [snapshot_path], s3_bucket=S3Storage.bucket_name, s3_key=f"{self.test_id}/{self.current_run}/corrupted-sstables-{keyspace}-{table_name}.tar.gz", max_size_gb=20, public_read_acl=True
            )
            if not s3_link:
                # upload malformed sstable along with several others and schema file
                malformed_files = node.remoter.run(f"ls {snapshot_path}/{sstable_name.rsplit('-', 1)[0]}*", ignore_status=True).stdout.split()
...
class Collector:
    ...
    def get_aws_instances_by_testid(self):
        ...
        for instance in filtered_instances["db_nodes"]:
            ...
            self.db_cluster.append(
                CollectingNode(
                    name=name[0],
                    ssh_login_info={"hostname": self.get_aws_ip_address(instance), "user": self.params.get("ami_db_scylla_user"), "key_file": self.params.get("user_credentials_path")},
                    instance=instance,
        ...
        for instance in filtered_instances["monitor_nodes"]:
            ...
            self.monitor_set.append(
                CollectingNode(
                    name=name[0],
                    ssh_login_info={"hostname": self.get_aws_ip_address(instance), "user": self.params.get("ami_monitor_user"), "key_file": self.params.get("user_credentials_path")},
                    instance=instance,
                    global_ip=self.get_aws_ip_address(instance),
                    tags={
                        **self.tags,
        ...
        for instance in filtered_instances["loader_nodes"]:
            ...
            self.loader_set.append(
                CollectingNode(
                    name=name[0],
                    ssh_login_info={"hostname": self.get_aws_ip_address(instance), "user": self.params.get("ami_loader_user"), "key_file": self.params.get("user_credentials_path")},
                    instance=instance,
                    global_ip=self.get_aws_ip_address(instance),
                    tags={
                        **self.tags,
    ...
    def get_gce_instances_by_testid(self):
        ...
        for instance in filtered_instances["db_nodes"]:
            self.db_cluster.append(
                CollectingNode(
                    name=instance.name,
                    ssh_login_info={"hostname": self.get_gce_ip_address(instance), "user": self.params.get("gce_image_username"), "key_file": self.params.get("user_credentials_path")},
                    instance=instance,
    ...
    def create_collecting_nodes(self):
        try:
            ...
            collecting_nodes = [
                CollectingNode(
                    name=instance.name,
                    ssh_login_info={"hostname": instance.public_ip_address or instance.private_ip_address, "user": instance.user_name, "key_file": f"~/.ssh/{instance.ssh_key_name}"},
                    instance=instance,
    ...
    def get_docker_instances_by_testid(self):
        ...
        for instance in filtered_instances["db_nodes"]:
            self.db_cluster.append(
                CollectingNode(
                    name=instance.name,
                    ssh_login_info={"hostname": self.get_gce_ip_address(instance), "user": "scylla-test", "key_file": self.params.get("user_credentials_path")},
                    instance=instance,
                    global_ip=self.get_gce_ip_address(instance),
                    tags={
                        **self.tags,
    ...
    def get_xcloud_instances_by_testid(self):
        ...
        api_client = ScyllaCloudAPIClient(api_url=self.params.cloud_env_credentials["base_url"], auth_token=self.params.cloud_env_credentials["api_token"])
        ...
        for node_data in cluster_nodes:
            ...
            node_mock = XCloudNodeMock(_cluster_id=cluster_id, _node_id=node_data.get("id"), parent_cluster=parent_cluster_mock)
    ...
    def run(self):
        ...
        for cluster_log_collector, nodes in self.cluster_log_collectors.items():
            log_collector = cluster_log_collector(nodes, test_id=self.test_id, storage_dir=self.storage_dir, params=self.params)
            ...
            try:
                ...
                LOGGER.warning("%s is not able to collect logs. Moving to the next log collector", log_collector.__class__.__name__, exc_info=True)
...
def check_archive(remoter, path: str) -> bool:
    ...
    if not archive_is_ok:
        LOGGER.error("Archive `%s' is corrupted: `%s' returns %d\n-- STDOUT: --\n%s\n\n-- STDERR: --\n%s", path, cmd, result.exit_status, result.stdout, result.stderr)
