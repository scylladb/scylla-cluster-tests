...
class DirLog(FileLog):
    ...
    def collect_from_builder(self, builder, local_dst, search_in_dir) -> None:
        ...
        LOGGER.warning(
            "'DirLog' class doesn't support 'collect_from_builder' method. It is to be implemented. Ignoring gathering following logs: '%s'",
            self.name,
        )
...
class PythonSCTLogCollector(BaseSCTLogCollector):
    ...
    def _compress_file(self, src_path: str, src_name: str) -> list[str]:
        ...
        if os.path.getsize(src_path) < self.too_big_log_size:
            ...
            runner.run(
                f"bash {os.path.join(os.path.dirname(__file__), 'log_archive.sh')} {src_path} {self.too_big_log_size} {src_name}"
            )
