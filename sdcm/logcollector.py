    ...
    log_entities = [
        ...
        CommandLog(
            ...
            command=(
                ...
                '" 2>/dev/null); do echo "=== $dev ==="; sudo nvme smart-log $dev 2>/dev/null; done '
                "|| true"
            ),
        ),
        CommandLog(
            ...
            command=(
                ...
                "for item in devs "
                "for ns in (item.get('Namespaces',[item]) if isinstance(item,dict) and 'Namespaces' in item else [item])]"
                '" 2>/dev/null); do echo "=== $dev ==="; sudo nvme error-log $dev 2>/dev/null; done '
                "|| true"
            ),
        ),
        CommandLog(
            name="nvme_self_test_log",
            command=(
                "command -v nvme > /dev/null 2>&1 && "
                "for dev in $(sudo nvme list -o json 2>/dev/null "
                '| python3 -c "import sys,json; '
                ...
                "for item in devs "
                "for ns in (item.get('Namespaces',[item]) if isinstance(item,dict) and 'Namespaces' in item else [item])]"
                '" 2>/dev/null); do echo "=== $dev ==="; sudo nvme self-test-log $dev 2>/dev/null; done '
                "|| true"
            ),
        ),
    ]
