...
@lru_cache
def report_package_to_argus(client: ArgusSCTClient, tool_name: str, package_version: str, additional_data: str = None, date: str = None, revision_id: str = None) -> None:
    package = Package(name=f"{tool_name}", version=package_version, date=date, revision_id=revision_id, build_id=additional_data)
...
class ToolReporterBase:
    ...
    def _report_to_argus(self) -> None:
        if not self.argus_client:
            ...
            return
        try:
            report_package_to_argus(client=self.argus_client, tool_name=self.TOOL_NAME, package_version=self.version, additional_data=self.additional_data, date=self.date, revision_id=self.revision_id)
...
class CassandraStressVersionReporter(ToolReporterBase):
    ...
    def _collect_version_info(self) -> None:
        ...
        if driver_version := result.get("scylla-java-driver"):
            ...
            CassandraStressJavaDriverVersionReporter(driver_version=driver_version, command_prefix=None, runner=None, argus_client=self.argus_client).report()
...
class CqlStressCassandraStressVersionReporter(ToolReporterBase):
    ...
    def _collect_version_info(self) -> None:
        ...
        if driver_details := result.get("scylla-driver", {}):
            ...
            CqlStressRustDriverVersionReporter(driver_version=driver_version, date=driver_date, revision_id=driver_revision_id, command_prefix=None, runner=None, argus_client=self.argus_client).report()
...
class CqlStressRustDriverVersionReporter(ToolReporterBase):
    ...
    def __init__(self, driver_version: str, date: str, revision_id: str, runner: CommandRunner, command_prefix: str = None, argus_client: ArgusSCTClient = None) -> None:
        super().__init__(runner, command_prefix, argus_client)
        self.version = driver_version
        self.date = date
...
class ScyllaBenchVersionReporter(ToolReporterBase):
    ...
    def _collect_version_info(self) -> None:
        ...
        if driver_details := version_info.get("scylla-driver", {}):
            ScyllaBenchGoCqlDriverVersionReporter(driver_version=driver_details.get("version"), date=driver_details.get("commit_date"), revision_id=driver_details.get("commit_sha"), argus_client=self.argus_client).report()
...
class GeminiVersionReporter(ToolReporterBase):
    ...
    def _collect_version_info(self) -> None:
        ...
        if driver_details := version_info.get("scylla-driver", {}):
            GeminiGoCqlDriverVersionReporter(driver_version=driver_details.get("version"), date=driver_details.get("commit_date"), revision_id=driver_details.get("commit_sha"), argus_client=self.argus_client).report()
