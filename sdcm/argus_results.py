...
class PeriodicDiskUsageToArgus:
    ...
    The rack balance status is determined based on the maximum difference in disk usage among nodes in the same rack.
    If the difference exceeds the specified threshold, the rack is marked as unbalanced.
    If `_enforce_balance` attribute is set on the cluster, unbalanced racks are marked with ERROR status, otherwise with WARNING status.
    """

    def __init__(
        self, cluster: "BaseScyllaCluster", argus_client: ArgusClient, interval: int = 600, threshold: int = 5
    ):
    ...
    def _unbalanced_status(self):
        """
        Return ERROR if balance enforcement is enabled, WARNING otherwise.
        """
        if hasattr(self.cluster, "_enforce_balance") and self.cluster._enforce_balance:
            return Status.ERROR
        ...
        while not self.stop_event.is_set():
            ...
            for rack, rack_nodes in rack_groups.items():
                ...
                if rack_usages[rack]:
                    ...
                    # if nodes differ by more than threshold% in disk usage, mark the rack as unbalanced
                    status = Status.PASS if rack_delta <= self.threshold else self._unbalanced_status()
