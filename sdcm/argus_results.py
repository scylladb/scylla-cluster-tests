...
from argus.client.generic_result import GenericResultTable, ColumnMetadata, ResultType, Status, ValidationRule, StaticGenericResultTable
...
LATENCY_ERROR_THRESHOLDS = {"replace_node": {"percentile_90": 5, "percentile_99": 10}, "default": {"percentile_90": 5, "percentile_99": 10}}
...
class ReactorStallStatsResult(StaticGenericResultTable):
    class Meta:
        name = ""
        description = ""
        Columns = [ColumnMetadata(name="total", unit="count", type=ResultType.INTEGER), *[ColumnMetadata(name=f"{interval}ms", unit="count", type=ResultType.INTEGER) for interval in STALL_INTERVALS]]
...
class IOPropertiesDeviationResultsTable(StaticGenericResultTable):
    ...
    class Meta:
        ...
        Columns = [
            ...
            ColumnMetadata(name="read_bandwidth_pct_deviation", unit="%", type=ResultType.INTEGER, higher_is_better=False),
            ColumnMetadata(name="write_iops_pct_deviation", unit="%", type=ResultType.INTEGER, higher_is_better=False),
            ColumnMetadata(name="write_bandwidth_pct_deviation", unit="%", type=ResultType.INTEGER, higher_is_better=False),
        ]
...
def submit_results_to_argus(argus_client: ArgusClient, result_table: GenericResultTable):
    try:
        ...
        if exc.args[1] == "DataValidationError":
            FailedResultEvent(f"Argus validation failed for the result in {result_table.name}. Please check the 'Results' tab for more details.").publish()
        else:
            raise


def send_result_to_argus(
    argus_client: ArgusClient,
    workload: str,
    name: str,
    description: str,  # noqa: PLR0914
    cycle: int | str,
    result: dict,
    start_time: float = 0,
    ...
    if hdr_summary_len > 2:
        ...
        result_table_summary.add_result(column="duration", row=summary_row_name, value=result["duration_in_sec"], status=Status.UNSET)
        result_table_summary.add_result(column="start time", row=summary_row_name, value=start_time, status=Status.UNSET)
        if overview_screenshot:
            result_table_summary.add_result(column="Overview", row=summary_row_name, value=overview_screenshot[0], status=Status.UNSET)
        if qa_screenshot:
            result_table_summary.add_result(column="QA dashboard", row=summary_row_name, value=qa_screenshot[0], status=Status.UNSET)
...
def send_manager_benchmark_results_to_argus(argus_client: ArgusClient, result: dict, result_table: StaticGenericResultTable, row_name: str = None) -> None:
    if not row_name:
        row_name = "#1"
...
def send_iotune_results_to_argus(argus_client: ArgusClient, results: dict, node, params):
    ...
    table = IOPropertiesResultsTable(cluster_backend=params.get("cluster_backend"), instance_type=node.db_node_instance_type)
    ...
    table = IOPropertiesDeviationResultsTable(cluster_backend=params.get("cluster_backend"), instance_type=node.db_node_instance_type)
    for key, value in results["deviation_pct"].items():
        table.add_result(column=f"{key}_pct_deviation", row="deviation_percent", value=value, status=Status.PASS if value < 15 else Status.WARNING)
