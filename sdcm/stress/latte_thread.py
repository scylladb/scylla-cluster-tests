...
class LatteStressThread(DockerBasedStressThread):
    ...
    def build_stress_cmd(self, cmd_runner, loader, hosts):
        ...
        if LatteStressThread.SCHEMA_CMD_CALL_COUNTER[script_name] < 1 or not self.params.get("enable_argus"):
            ...
            try:
                LOGGER.debug("Output for 'latte schema' (tag: %s) cmd\nstdout: %s\nstderr: %s", first_tag_or_op, result.stdout, result.stderr)
    ...
    def parse_final_output(result):
        ...
        latency_mean_regex = re.compile(r"\s*(?:Mean resp\. time|Request latency)\s*(?:\[(ms|s)\])?\s*(?P<latency_mean>\d+\.\d+)")
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        ...
        first_tag_or_op = "-" + (find_latte_tags(self.stress_cmd) or [self.stress_operation])[0]
        log_file_name = os.path.join(loader.logdir, "latte%s-l%s-c%s-%s.log" % (first_tag_or_op, loader_idx, cpu_idx, uuid.uuid4()))
        ...
        cmd_runner = cleanup_context = RemoteDocker(
            ...
            extra_docker_opts=(f"--network=host --security-opt seccomp=unconfined --entrypoint /bin/bash {cpu_options} --label shell_marker={self.shell_marker} -v {remote_hdr_file_name_full_path}:/{remote_hdr_file_name}"),
        )
        ...
        with (
            cleanup_context,
            hdrh_logger_context,
            LatteExporter(keyspace=keyspace_holder, instance_name=loader.ip_address, metrics=nemesis_metrics_obj(), stress_operation=self.stress_operation, stress_log_filename=log_file_name, loader_idx=loader_idx, cpu_idx=cpu_idx),
            LatteHDRExporter(
