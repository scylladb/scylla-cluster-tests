...
class DockerBasedStressThread:
    ...
    def __init__(self, loader_set, stress_cmd, timeout, stress_num=1, node_list=None, round_robin=False, params=None, stop_test_on_failure=True):
    ...
    def get_results(self):
        ...
        LOGGER.debug("Wait for %d stress tasks results on worker pool of size %d", len(self.results_futures), self.max_workers)
    ...
    def kill(self):
        if self.loaders and self.loaders[0].is_kubernetes():
            ...
            for loader in self.loaders:
                loader.remoter.run(cmd=f"docker rm -f `docker ps -a -q --filter label=shell_marker={self.shell_marker}`", timeout=60, ignore_status=True)
