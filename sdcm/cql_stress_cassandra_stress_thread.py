...
class CqlStressCassandraStressThread(CassandraStressThread):
    ...
    def create_stress_cmd(self, cmd_runner, keyspace_idx, loader):
        ...
        if "-pop" in stress_cmd and "seq=" in stress_cmd:
            ...
            LOGGER.warning("-pop seq=x..y syntax is not YET supported by cql-stress-cassandra-stress. Replacing seq=x..y with dist=SEQ(x..y).")
    ...
    def _run_cs_stress(self, loader, loader_idx, cpu_idx, keyspace_idx):  # noqa: PLR0914
        ...
        try:
            ...
            reporter = CqlStressCassandraStressVersionReporter(cmd_runner, prefix, loader.parent_cluster.test_config.argus_client())
            reporter.report()
        ...
        with (
            cleanup_context,
            CqlStressCassandraStressExporter(instance_name=cmd_runner_name, metrics=nemesis_metrics_obj(), stress_operation=stress_cmd_opt, stress_log_filename=log_file_name, loader_idx=loader_idx, cpu_idx=cpu_idx),
            CqlStressCassandraStressEventsPublisher(node=loader, log_filename=log_file_name) as publisher,
            CqlStressCassandraStressEvent(node=loader, stress_cmd=self.stress_cmd, log_file_name=log_file_name) as cs_stress_event,
            CqlStressHDRExporter(instance_name=cmd_runner_name, hdr_tags=self.hdr_tags, metrics=nemesis_metrics_obj(), stress_operation=stress_cmd_opt, stress_log_filename=local_hdr_file_name, loader_idx=loader_idx, cpu_idx=cpu_idx),
            hdrh_logger_context,
        ):
            ...
            try:
                ...
                with SoftTimeoutContext(timeout=self.timeout, operation="cql-stress-cassandra-stress"):
                    result = cmd_runner.run(cmd=node_cmd, timeout=hard_timeout, log_file=log_file_name, retry=0, timestamp_logs=True)
