...
class EksNodePool(CloudK8sNodePool):
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.ssh_key_pair_name = (
            os.path.basename(self.k8s_cluster.credentials[0].key_pair_name)
            if ssh_key_pair_name is None
            else ssh_key_pair_name
        )
    ...
    def _launch_template_cfg(self) -> dict:
        ...
        root_disk_def = LaunchTemplateBlockDeviceMappingRequestTypeDef(
            DeviceName="/dev/xvda", Ebs=LaunchTemplateEbsBlockDeviceRequestTypeDef(**root_ebs_block_device_params)
        )
    ...
    def undeploy(self):
        try:
            self.k8s_cluster.eks_client.delete_nodegroup(
                clusterName=self.k8s_cluster.short_cluster_name, nodegroupName=self.name
            )
        except Exception as exc:  # noqa: BLE001
            self.k8s_cluster.log.debug(
                "Failed to delete nodegroup %s/%s, due to the following error:\n%s",
                self.k8s_cluster.short_cluster_name,
                self.name,
                exc,
            )
...
class EksCluster(KubernetesCluster, EksClusterCleanupMixin):
    ...
    def create_kubectl_config(self) -> None:
        LOCALRUNNER.run(
            f"aws eks --region {self.region_name} update-kubeconfig --name {self.short_cluster_name} --kubeconfig={self.kube_config_path}"
        )
    ...
    def create_scylla_manager_agent_config(self, s3_provider_endpoint: str = None, namespace: str = SCYLLA_NAMESPACE):
        if s3_provider_endpoint:
            self.log.warning(
                "Ignoring the value ('%s') for the 's3_provider_endpoint' parameter,which is needed only using S3-compatible services like MinIO.",
