...
import boto3
import tenacity
from mypy_boto3_ec2.type_defs import LaunchTemplateBlockDeviceMappingRequestTypeDef, LaunchTemplateEbsBlockDeviceRequestTypeDef, RequestLaunchTemplateDataTypeDef, LaunchTemplateTagSpecificationRequestTypeDef
...
def init_k8s_eks_cluster(region_name: str, availability_zone: str, params: dict, credentials: List[cluster.UserRemoteCredentials], cluster_uuid: str = None):
    ...
    ec2_security_group_ids, ec2_subnet_ids = get_ec2_network_configuration(regions=[region_name], availability_zones=availability_zones, params=params)
    return EksCluster(
...
def deploy_k8s_eks_cluster(k8s_cluster) -> None:
    ...
    k8s_cluster.set_nodeselector_for_deployments(pool_name=k8s_cluster.AUXILIARY_POOL_NAME, namespace="kube-system")
    ...
    scylla_pool = EksNodePool(
        name=k8s_cluster.SCYLLA_POOL_NAME, num_nodes=params.get("n_db_nodes"), instance_type=params.get("instance_type_db"), role_arn=params.get("eks_nodegroup_role_arn"), disk_size=params.get("root_disk_size_db"), k8s_cluster=k8s_cluster
    )
...
class EksNodePool(CloudK8sNodePool):
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.ssh_key_pair_name = os.path.basename(self.k8s_cluster.credentials[0].key_pair_name) if ssh_key_pair_name is None else ssh_key_pair_name
        self.security_group_ids = self.k8s_cluster.ec2_security_group_ids[0] if security_group_ids is None else security_group_ids
    ...
    def _launch_template_cfg(self) -> dict:
        ...
        root_disk_def = LaunchTemplateBlockDeviceMappingRequestTypeDef(DeviceName="/dev/xvda", Ebs=LaunchTemplateEbsBlockDeviceRequestTypeDef(**root_ebs_block_device_params))
        ...
        if self.tags:
            launch_template["TagSpecifications"] = [LaunchTemplateTagSpecificationRequestTypeDef(ResourceType="instance", Tags=tags_as_ec2_tags(self.tags))]
    ...
    def _node_group_cfg(self) -> dict:
        ...
        node_pool_config = {
            ...
            "scalingConfig": {"minSize": self.num_nodes, "maxSize": self.num_nodes, "desiredSize": self.num_nodes},
    ...
    def resize(self, num_nodes):
        ...
        self.k8s_cluster.eks_client.update_nodegroup_config(
            clusterName=self.k8s_cluster.short_cluster_name,
            nodegroupName=self.name,
            scalingConfig={"minSize": num_nodes, "maxSize": num_nodes, "desiredSize": num_nodes},
        )
        self.num_nodes = num_nodes

    def undeploy(self):
        try:
            self.k8s_cluster.eks_client.delete_nodegroup(clusterName=self.k8s_cluster.short_cluster_name, nodegroupName=self.name)
        except Exception as exc:  # noqa: BLE001
            self.k8s_cluster.log.debug("Failed to delete nodegroup %s/%s, due to the following error:\n%s", self.k8s_cluster.short_cluster_name, self.name, exc)
...
class EksCluster(KubernetesCluster, EksClusterCleanupMixin):
    ...
    def create_eks_cluster(self, wait_till_functional=True):
        ...
        if wait_till_functional:
            wait_for(lambda: self.cluster_status == "ACTIVE", step=60, throw_exc=True, timeout=1200, text=f"Waiting till EKS cluster {self.short_cluster_name} become operational")

        self.eks_client.create_addon(clusterName=self.short_cluster_name, addonName="vpc-cni", addonVersion=self.vpc_cni_version)
    ...
    def add_k8s_admin_principal(self):
        ...
        try:
            for principal in admin_principals:
                ...
                eks.associate_access_policy(clusterName=self.short_cluster_name, principalArn=principal, policyArn="arn:aws:eks::aws:cluster-access-policy/AmazonEKSClusterAdminPolicy", accessScope={"type": "cluster"})
    ...
    def create_token_update_thread(self):
        return EksTokenUpdateThread(aws_cmd=f"aws eks --region {self.region_name} get-token --cluster-name {self.short_cluster_name}", kubectl_token_path=self.kubectl_token_path)

    def create_kubectl_config(self) -> None:
        LOCALRUNNER.run(f"aws eks --region {self.region_name} update-kubeconfig --name {self.short_cluster_name} --kubeconfig={self.kube_config_path}")
    ...
    def create_ebs_storge_class(self, storage_class_backend="gp3"):
        if storage_class_backend in SUPPORTED_EBS_STORAGE_CLASSES:
            tags_specification = "\n".join([f'  tagSpecification_{i}: "{k}={v}"' for i, (k, v) in enumerate(self.tags.items(), start=1)])
    ...
    def create_ebs_csi_driver_serviceaccount(self):
        ...
        LOCALRUNNER.run(f"eksctl utils associate-iam-oidc-provider --region={self.region_name} --cluster={self.short_cluster_name} --approve")
        LOCALRUNNER.run(
            f"eksctl create iamserviceaccount --name ebs-csi-controller-sa --namespace kube-system "
    ...
    def deploy_ebs_csi_driver(self):
        wait_for(lambda: self.ebs_csi_driver_status == "ACTIVE", step=60, throw_exc=True, timeout=600, text="Waiting till aws-ebs-csi-driver become operational")
    ...
    def create_scylla_manager_agent_config(self, s3_provider_endpoint: str = None, namespace: str = SCYLLA_NAMESPACE):
        if s3_provider_endpoint:
            self.log.warning("Ignoring the value ('%s') for the 's3_provider_endpoint' parameter,which is needed only using S3-compatible services like MinIO.", s3_provider_endpoint)
        data = {}
    ...
    def upgrade_kubernetes_platform(self, pod_objects: list[cluster.BaseNode], use_additional_scylla_nodepool: bool) -> (str, CloudK8sNodePool):
        ...
        for node_pool, need_upgrade in ((self.AUXILIARY_POOL_NAME, True), (self.SCYLLA_POOL_NAME, not use_additional_scylla_nodepool)):
            if not need_upgrade:
                continue
        ...
        if use_additional_scylla_nodepool:
            ...
            self.prepare_k8s_scylla_nodes(node_pools=[self.pools[self.SCYLLA_POOL_NAME], new_scylla_pool])

            # Move Scylla pods to the new nodes
            self.move_pods_to_new_node_pool(pod_objects=pod_objects, node_pool_name=new_scylla_pool_name, pod_readiness_timeout_minutes=120)
...
class EksScyllaPodContainer(BaseScyllaPodContainer):
    ...
    def _instance_wait_safe(self, instance_method: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        try:
            ...
            raise cluster.NodeError(f"Timeout while running '{instance_method.__name__}' method on AWS instance '{self.ec2_host.id}'") from None
...
class EksScyllaPodCluster(ScyllaPodCluster):
    ...
    def add_nodes(self, count: int, ec2_user_data: str = "", dc_idx: int = None, rack: int = 0, enable_auto_bootstrap: bool = False, instance_type=None) -> List[EksScyllaPodContainer]:
        new_nodes = super().add_nodes(count=count, ec2_user_data=ec2_user_data, dc_idx=dc_idx, rack=rack, enable_auto_bootstrap=enable_auto_bootstrap, instance_type=instance_type)
...
class MonitorSetEKS(MonitorSetAWS):
    ...
    def _create_instances(self, count, ec2_user_data="", dc_idx=0, az_idx=0, instance_type=None, is_zero_node=False):
        instances = super()._create_instances(count=count, ec2_user_data=ec2_user_data, dc_idx=dc_idx, az_idx=az_idx, instance_type=instance_type, is_zero_node=is_zero_node)
