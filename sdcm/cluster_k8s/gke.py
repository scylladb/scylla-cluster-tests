...
def init_k8s_gke_cluster(gce_datacenter: str, availability_zone: str, params: dict, cluster_uuid: str = None):
...
def deploy_k8s_gke_cluster(k8s_cluster) -> None:
    ...
    if params.get("n_loaders"):
        loader_pool = GkeNodePool(name=k8s_cluster.LOADER_POOL_NAME, instance_type=params.get("gce_instance_type_loader"), num_nodes=params.get("n_loaders"), k8s_cluster=k8s_cluster)
...
class GkeNodePool(CloudK8sNodePool):
    ...
    def _deploy_cmd(self) -> str:
        ...
        cmd = [
            f"container --project {self.gce_project} node-pools create {self.name}",
    ...
    def resize(self, num_nodes: int):
        self.k8s_cluster.gcloud.run(f"container clusters resize {self.k8s_cluster.short_cluster_name} --project {self.gce_project} --region {self.gce_region} --node-pool {self.name} --num-nodes {num_nodes} --quiet")
        self.num_nodes = int(num_nodes)
        self.wait_for_nodes_readiness()

    def undeploy(self):
        self.k8s_cluster.gcloud.run(f"container --project {self.gce_project} node-pools delete {self.name} --cluster {self.k8s_cluster.short_cluster_name} --region {self.gce_region} --quiet")

    @property
    def instance_group_name(self) -> str:
        try:
            group_link = yaml.safe_load(self.k8s_cluster.gcloud.run(f"container node-pools describe {self.name} --region {self.gce_region} --project {self.gce_project} --cluster {self.k8s_cluster.short_cluster_name}")).get(
                "instanceGroupUrls"
            )[0]
    ...
    def remove_instance(self, instance_name: str):
        self.k8s_cluster.gcloud.run(f"compute instance-groups managed delete-instances {self.instance_group_name} --region={self.gce_region} --node-locations={self.gce_zone} --instances={instance_name}")
...
class GkeCluster(KubernetesCluster):
    ...
    def _authenticate_in_gcloud():
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=True, encoding="utf-8") as tmp_gcloud_creds_file:
            ...
            auth_cmd = f"gcloud auth activate-service-account {credentials['client_email']} --key-file {tmp_gcloud_creds_file.name} --project {credentials['project_id']}"
    ...
    def deploy(self):
        self.log.info("Create GKE cluster `%s' with %d node(s) in %s", self.short_cluster_name, self.n_nodes, self.AUXILIARY_POOL_NAME)
        ...
        with self.gcloud as gcloud:
            ...
            self.deploy_node_pool(GkeNodePool(name=self.AUXILIARY_POOL_NAME, num_nodes=self.n_nodes, disk_size=self.gce_disk_size, disk_type=self.gce_disk_type, k8s_cluster=self, instance_type=self.gce_instance_type, is_deployed=True))

        self.log.info("Setup RBAC for GKE cluster `%s'", self.name)
        self.kubectl(f"create clusterrolebinding cluster-admin-binding --clusterrole cluster-admin --user {self.gce_user}")
    ...
    def create_kubectl_config(self):
        self.gcloud.run(f"container clusters get-credentials {self.short_cluster_name} --region {self.gce_region}")
    ...
    def upgrade_kubernetes_platform(self, pod_objects: list[cluster.BaseNode], use_additional_scylla_nodepool: bool) -> (str, CloudK8sNodePool):
        ...
        with self.gcloud as gcloud:
            ...
            gcloud.run(f"container clusters upgrade {self.short_cluster_name} --master --quiet --project {self.gce_project} --region {self.gce_region} --cluster-version {upgrade_version}")
            ...
            for node_pool, need_upgrade in ((self.AUXILIARY_POOL_NAME, True), (self.SCYLLA_POOL_NAME, not use_additional_scylla_nodepool)):
                ...
                gcloud.run(f"container clusters upgrade {self.short_cluster_name} --quiet --project {self.gce_project} --region {self.gce_region} --node-pool={node_pool}")
...
class GkeScyllaPodContainer(BaseScyllaPodContainer):
    ...
    def k8s_node(self):
        ...
        return k8s_cluster.gce_service.get(project=k8s_cluster.gce_project, zone=k8s_cluster.gce_zone, instance=self.node_name)
    ...
    def _destroy(self):
        if self.k8s_node:
            ...
            operation = instances_client.delete(instance=self.k8s_node.name, project=self.k8s_cluster.gce_project, zone=self.k8s_cluster.gce_zone)
    ...
    def _instance_wait_safe(self, instance_method: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        try:
            ...
            raise cluster.NodeError(f"Timeout while running '{instance_method.__name__}' method on GCE instance '{self.k8s_node.id}'") from None
...
class GkeScyllaPodCluster(ScyllaPodCluster):
    ...
    def add_nodes(self, count: int, ec2_user_data: str = "", dc_idx: int = None, rack: int = 0, enable_auto_bootstrap: bool = False, instance_type=None) -> List[GkeScyllaPodContainer]:
