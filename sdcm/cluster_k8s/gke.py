...
class GkeNodePool(CloudK8sNodePool):
    ...
    def resize(self, num_nodes: int):
        self.k8s_cluster.gcloud.run(
            f"container clusters resize {self.k8s_cluster.short_cluster_name} --project {self.gce_project} --region {self.gce_region} --node-pool {self.name} --num-nodes {num_nodes} --quiet"
        )
    ...
    def undeploy(self):
        self.k8s_cluster.gcloud.run(
            f"container --project {self.gce_project} node-pools delete {self.name} --cluster {self.k8s_cluster.short_cluster_name} --region {self.gce_region} --quiet"
        )
    ...
    def instance_group_name(self) -> str:
        try:
            group_link = yaml.safe_load(
                self.k8s_cluster.gcloud.run(
                    f"container node-pools describe {self.name} --region {self.gce_region} --project {self.gce_project} --cluster {self.k8s_cluster.short_cluster_name}"
                )
    ...
    def remove_instance(self, instance_name: str):
        self.k8s_cluster.gcloud.run(
            f"compute instance-groups managed delete-instances {self.instance_group_name} --region={self.gce_region} --node-locations={self.gce_zone} --instances={instance_name}"
        )
...
class GkeCluster(KubernetesCluster):
    ...
    def upgrade_kubernetes_platform(
        ...
        with self.gcloud as gcloud:
            ...
            gcloud.run(
                f"container clusters upgrade {self.short_cluster_name} --master --quiet --project {self.gce_project} --region {self.gce_region} --cluster-version {upgrade_version}"
            )
            ...
            for node_pool, need_upgrade in (
                ...
                gcloud.run(
                    f"container clusters upgrade {self.short_cluster_name} --quiet --project {self.gce_project} --region {self.gce_region} --node-pool={node_pool}"
                )
        ...
        if use_additional_scylla_nodepool:
            ...
            self.move_pods_to_new_node_pool(
                pod_objects=pod_objects, node_pool_name=new_scylla_pool_name, pod_readiness_timeout_minutes=120
            )
...
class GkeScyllaPodContainer(BaseScyllaPodContainer):
    ...
    def k8s_node(self):
        ...
        return k8s_cluster.gce_service.get(
            project=k8s_cluster.gce_project, zone=k8s_cluster.gce_zone, instance=self.node_name
        )
    ...
    def _destroy(self):
        if self.k8s_node:
            ...
            operation = instances_client.delete(
                instance=self.k8s_node.name, project=self.k8s_cluster.gce_project, zone=self.k8s_cluster.gce_zone
            )
