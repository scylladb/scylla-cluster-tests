...
class CloudK8sNodePool(metaclass=abc.ABCMeta):
    ...
    def wait_for_nodes_readiness(self):
        ...
        @timeout_wrapper(
            message=(
                f"{self.k8s_cluster.region_name}: Wait for {self.num_nodes} node(s) in the '{self.name}' pool to be ready..."
            ),
            sleep_time=30,
        ...
        def wait_nodes_are_ready():
            ...
            result = self.k8s_cluster.kubectl_no_wait(
                f"wait --timeout={self.readiness_timeout // 5}m -l {self.pool_label_name}={self.name} --for=condition=Ready node",
...
class KubernetesCluster(metaclass=abc.ABCMeta):
    ...
    def get_operator_image(self, repo: str = "", chart_version: str = "") -> str:
        ...
        raise ValueError(
            f"Cannot get operator image version from the '{chart_name}' chart located at '{repo}' having '{chart_version}' version"
        )
        ...
        if ComparableScyllaOperatorVersion(new_chart_version.split("-")[0]) > "1.5.0":
            ...
            try:
                with TemporaryDirectory() as tmpdir:
                    self.helm(
                        f"pull {local_repo_name}/scylla-operator --devel --untar --version {new_chart_version} --destination {tmpdir}"
                    )
    ...
    def prepare_k8s_scylla_nodes(self, node_pools: list[CloudK8sNodePool] | CloudK8sNodePool) -> None:
        ...
        if self.is_performance_tuning_enabled:
            ...
            if scylla_utils_docker_image := self.params.get("k8s_scylla_utils_docker_image"):
                self.kubectl(
                    'patch scyllaoperatorconfigs.scylla.scylladb.com cluster --type merge -p \'{"spec":{"scyllaUtilsImage":"%s"}}\''
                    % scylla_utils_docker_image,
                    ignore_status=False,
                )
    ...
    def _affinity_modifiers_for_monitoring_resources(self):
        ...
        if not node_pool:
            self.log.warning(
                "'Monitoring' node pool was not found. Will schedule 'monitoring' resources creation in the 'Auxiliary' one."
            )
        ...
        if not affinity_modifiers:
            self.log.warning(
                "'Auxiliary' node pool was not found. Will schedule 'monitoring' resources creation without affinity rules."
            )
...
class BaseScyllaPodContainer(BasePodContainer):
    ...
    def fstrim_scylla_disks(self):
        ...
        podnames = (
            self.k8s_cluster.kubectl(
                f"get pod -l {pods_selector} --field-selector spec.nodeName={self.node_name} -o jsonpath='{{range .items[*]}}{{.metadata.name}}'",
                namespace=namespace,
            )
            .stdout.strip()
            .split("\n")
        ...
        assert podnames, (
            f"Failed to find pods using '{pods_selector}' selector on '{self.node_name}' node in '{namespace}' namespace. Didn't run the 'fstrim' command"
        )
    ...
    def alternator_ca_bundle_path(self):
        ca_bundle_cmd_output = self.k8s_cluster.kubectl(
            f"get configmap/{self.parent_cluster.scylla_cluster_name}-alternator-local-serving-ca --template='{{{{ index .data \"ca-bundle.crt\" }}}}'",
            namespace=self.parent_cluster.namespace,
            ignore_status=True,
        )
...
class PodCluster(cluster.BaseCluster):
    ...
    def add_nodes(
        ...
        if len(nodes) != count:
            raise RuntimeError(
                f"Requested '{count}' number of nodes to add in the '{dc_idx}' DC, while only '{len(nodes)}' new nodes where found"
            )
        return nodes
...
class ScyllaPodCluster(cluster.BaseScyllaCluster, PodCluster):
    ...
    def connection_bundle_file(self) -> Path | None:
        ...
        bundle_cmd_output = k8s_cluster.kubectl(
            f"get secret/{self.scylla_cluster_name}-local-cql-connection-configs-admin --template='{{{{ index .data \"{self.scylla_cluster_name}.sct.scylladb.com\" }}}}'",
            namespace=self.namespace,
            ignore_status=True,
        )
        ...
        lb_external_hostname = k8s_cluster.kubectl(
            "get service/haproxy-kubernetes-ingress -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'",
        ...
        for current_dc_idx in dc_idx:
            ...
            if current_dc_idx > 0:
                ...
                if not dc_podip_mapping.get(current_dc_idx, []):
                    assert dc_podip_mapping[0], (
                        f"Couldn't not find IP addresses of the nodes from the first DC (dc_idx=0) to be used as 'external seeds' for the pods of another DC (dc_idx={current_dc_idx})"
                    )
            ...
            if self.params.get("use_mgmt") and current_dc_idx == 0 and (self.k8s_scylla_manager_auth_token is None):
                self.k8s_scylla_manager_auth_token = (
                    base64.b64decode(
                        kubectl(
                            f"get secrets/{self.scylla_cluster_name}-auth-token --template='{{{{ index .data \"auth-token.yaml\" }}}}'",
                            namespace=self.namespace,
                        ).stdout.strip()
                    )
                    .decode("utf-8")
    ...
    def restart_scylla(self, nodes=None, random_order=False):
        ...
        for i, k8s_cluster in enumerate(self.k8s_clusters):
            ...
            for statefulset in statefulsets:
                k8s_cluster.kubectl(
                    f"rollout status statefulset/{statefulset.metadata.name} --watch=true --timeout={readiness_timeout}m",
                    namespace=self.namespace,
    ...
    def prefill_cluster(self, dataset_name: str):
        ...
        if not test_data:
            raise ValueError(
                f"Dataset unexpected value: '{dataset_name}'. Dataset with this name is not defined in the sdcm.utils.sstable.load_inventory.Expected values: BIG_SSTABLE_MULTI_COLUMNS_DATA, MULTI_COLUMNS_DATA"
            )
