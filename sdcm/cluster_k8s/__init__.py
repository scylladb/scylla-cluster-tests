...
class KubernetesCluster(metaclass=abc.ABCMeta):  # pylint: disable=too-many-public-methods,too-many-instance-attributes
    ...
    def deploy_scylla_manager(self, pool_name: str = None) -> None:
        ...
        if not self.params.get("reuse_cluster"):
            ...
            scylla_operator_repo_base = self.params.get("k8s_scylla_operator_docker_image").split("/")[0]
            if scylla_operator_repo_base:
                values.set("controllerImage.repository", scylla_operator_repo_base)

            scylla_operator_image_tag = self.params.get("k8s_scylla_operator_docker_image").split(":")[-1]
    ...
    def deploy_scylla_operator(self, pool_name: str = None) -> None:
        if not self.params.get("reuse_cluster"):
            ...
            if (
                self.params.get("k8s_enable_tls")
                and ComparableScyllaOperatorVersion(self.scylla_operator_chart_version.split("-")[0]) >= "1.9.0"
            ):
        ...
        return HelmValues(
            {
                ...
                "alternator": {"enabled": False, "port": 8000, "writeIsolation": "always"},
                "developerMode": False,
                "cpuset": True,
    ...
    def deploy_scylla_cluster(
        ...
        if self.params.get("append_scylla_args"):
            ...
            self.kubectl(
                f"patch scyllaclusters {cluster_name} --type merge -p '{json.dumps(data)}'",
                namespace=namespace,
            )
...
class ScyllaPodCluster(cluster.BaseScyllaCluster, PodCluster):  # pylint: disable=too-many-public-methods
    ...
    def add_nodes(  # pylint: disable=too-many-locals,too-many-branches
        self,
        count: int,
        ec2_user_data: str = "",
...
class LoaderPodCluster(cluster.BaseLoaderSet, PodCluster):
    ...
    def node_setup(
        self, node: BasePodContainer, verbose: bool = False, db_node_address: Optional[str] = None, **kwargs
    ) -> None:
