...
from sdcm.utils.remote_logger import get_system_logging_thread, CertManagerLogger, ScyllaOperatorLogger, KubectlClusterEventsLogger, ScyllaManagerLogger, KubernetesWrongSchedulingLogger, HaproxyIngressLogger
...
class CloudK8sNodePool(metaclass=abc.ABCMeta):  # pylint: disable=too-many-instance-attributes
    def __init__(self, k8s_cluster: "KubernetesCluster", name: str, num_nodes: int, instance_type: str, image_type: str, disk_size: int = None, disk_type: str = None, labels: dict = None, taints: list = None, is_deployed: bool = False):
    ...
    def __str__(self):
        data = [f'name="{self.name}"', *[f'{param}="{value}"' for param, value in self.__dict__.items() if param not in ["name", "k8s_cluster"]]]
    ...
    def cpu_and_memory_capacity(self) -> Tuple[float, float]:
        for item in self.k8s_cluster.k8s_core_v1_api.list_node().items:
            if item.metadata.labels.get(self.pool_label_name, "") == self.name:
                ...
                return convert_cpu_value_from_k8s_to_units(capacity["cpu"]), convert_memory_value_from_k8s_to_units(capacity["memory"])
    ...
    def wait_for_nodes_readiness(self):
        ...
        @timeout_wrapper(message=(f"{self.k8s_cluster.region_name}: Wait for {self.num_nodes} node(s) in the '{self.name}' pool to be ready..."), sleep_time=30, timeout=readiness_timeout * 60)
        def wait_nodes_are_ready():
            # To make it more informative in worst case scenario made it repeat 5 times, by readiness_timeout // 5
            result = self.k8s_cluster.kubectl_no_wait(f"wait --timeout={self.readiness_timeout // 5}m -l {self.pool_label_name}={self.name} --for=condition=Ready node", timeout=readiness_timeout // 5 * 60 + 10, verbose=False)
...
class KubernetesCluster(metaclass=abc.ABCMeta):  # pylint: disable=too-many-public-methods,too-many-instance-attributes
    ...
    def kubectl_no_wait(self, *command, namespace=None, timeout=KUBECTL_TIMEOUT, remoter=None, ignore_status=False, verbose=True):
        return KubernetesOps.kubectl(self, *command, namespace=namespace, timeout=timeout, remoter=remoter, ignore_status=ignore_status, verbose=verbose)

    def kubectl(self, *command, namespace=None, timeout=KUBECTL_TIMEOUT, remoter=None, ignore_status=False, verbose=True):
    ...
    def kubectl_wait(self, *command, namespace=None, timeout=KUBECTL_TIMEOUT, remoter=None, verbose=True):
        ...
        def wait_body():
            ...
            result = self.kubectl("wait --timeout=1m", *command, namespace=namespace, timeout=timeout, remoter=remoter, verbose=verbose)
    ...
    def kubectl_multi_cmd(self, *command, namespace=None, timeout=KUBECTL_TIMEOUT, remoter=None, ignore_status=False, verbose=True):
        if self.api_call_rate_limiter:
            self.api_call_rate_limiter.wait()
        return KubernetesOps.kubectl_multi_cmd(self, *command, namespace=namespace, timeout=timeout, remoter=remoter, ignore_status=ignore_status, verbose=verbose)
    ...
    def create_docker_hub_auth_secret(self, namespace: str) -> None:
        ...
        self.kubectl(
            f"create secret docker-registry docker-auth --docker-server={docker_hub_url} --docker-username={docker_hub_creds['username']} --docker-password={docker_hub_creds['password']} --docker-email={docker_hub_creds['email']}",
            namespace=namespace,
        )
    ...
    def set_nodeselector_for_deployments(self, pool_name: str, namespace: str = "kube-system", selector: str = "") -> None:
        ...
        deployment_names = self.kubectl(f"get deployments -l '{selector}' --no-headers -o custom-columns=:.metadata.name", namespace=namespace).stdout.split()
    ...
    def deploy_cert_manager(self, pool_name: str = None) -> None:
        ...
        if not self.params.get("reuse_cluster"):
            ...
            self.log.debug(self.helm(f"install cert-manager jetstack/cert-manager --version v{self.params.get('k8s_cert_manager_version')}", namespace=cert_manager_namespace, values=helm_values))

        self.kubectl_wait("--all --for=condition=Ready pod", namespace="cert-manager", timeout=600)
        wait_for(self.check_if_cert_manager_fully_functional, text="Waiting for cert-manager to become fully operational", timeout=10 * 60, step=10, throw_exc=True)
    ...
    def get_operator_image(self, repo: str = "", chart_version: str = "") -> str:
        ...
        raise ValueError(f"Cannot get operator image version from the '{chart_name}' chart located at '{repo}' having '{chart_version}' version")
    ...
    def start_scylla_cluster_events_thread(self, namespace: str = SCYLLA_NAMESPACE) -> None:
        self._scylla_cluster_events_threads[namespace] = KubectlClusterEventsLogger(self, self.scylla_cluster_event_log(namespace=namespace), namespace=namespace)
    ...
    def deploy_scylla_operator(self, pool_name: str = None) -> None:
        if not self.params.get("reuse_cluster"):
            ...
            self.log.debug(
                self.helm_install(target_chart_name="scylla-operator", source_chart_name="scylla-operator/scylla-operator", version=self.scylla_operator_chart_version, use_devel=True, namespace=SCYLLA_OPERATOR_NAMESPACE, values=values)
            )
            ...
            if ComparableScyllaOperatorVersion(scylla_operator_version) >= "1.8.0":
                patch_cmd = f'patch deployment scylla-operator --type=json -p=\'[{{"op": "add", "path": "/spec/template/spec/containers/0/args/-", "value": "--feature-gates=AutomaticTLSCertificates={enable_tls}" }}]\' '
            ...
            if enable_tls == "true" and ComparableScyllaOperatorVersion(scylla_operator_version) >= "1.9.0":
                ...
                for flag in (f"--crypto-key-buffer-size-min={crypto_key_buffer_size}", f"--crypto-key-buffer-size-max={crypto_key_buffer_size}"):
            ...
            KubernetesOps.wait_for_pods_readiness(kluster=self, total_pods=lambda pods: pods > 0, readiness_timeout=5, namespace=SCYLLA_OPERATOR_NAMESPACE)
    ...
    def upgrade_scylla_operator(self, new_helm_repo: str, new_chart_version: str, new_docker_image: str = "") -> None:
        self.log.info("Upgrade Scylla Operator using '%s' helm chart and '%s' docker image\nHelm repo: %s", new_chart_version, new_docker_image, new_helm_repo)
        ...
        if new_chart_version in ("", "latest"):
            ...
            self.log.info("Using automatically found following latest scylla-operator upgrade chart version: %s", new_chart_version)
        else:
        ...
        if ComparableScyllaOperatorVersion(new_chart_version.split("-")[0]) > "1.5.0":
            ...
            try:
                with TemporaryDirectory() as tmpdir:
                    self.helm(f"pull {local_repo_name}/scylla-operator --devel --untar --version {new_chart_version} --destination {tmpdir}")
                    ...
                    for current_file in os.listdir(crd_basedir):
                        if not current_file.endswith((".yaml", ".yml")):
                            continue
                        self.apply_file(os.path.join(crd_basedir, current_file), modifiers=[], envsubst=False, server_side=True, force_conflicts=True)
        ...
        values = HelmValues(json.loads(self.helm("get values scylla-operator -o json", namespace=SCYLLA_OPERATOR_NAMESPACE)))
        ...
        if self.params.get("k8s_enable_tls") and ComparableScyllaOperatorVersion(self.scylla_operator_chart_version.split("-")[0]) >= "1.8.0":
            patch_cmd = 'patch deployment scylla-operator --type=json -p=\'[{"op": "add","path": "/spec/template/spec/containers/0/args/-", "value": "--feature-gates=AutomaticTLSCertificates=true" }]\' '
            self.kubectl(patch_cmd, namespace=SCYLLA_OPERATOR_NAMESPACE)
        time.sleep(5)
    ...
    def check_scylla_cluster_sa_annotations(self, namespace: str = SCYLLA_NAMESPACE):
        ...
        raw_sa_data = self.kubectl(f"get sa {self.params.get('k8s_scylla_cluster_name')}-member -o yaml", namespace=namespace).stdout
        sa_data = yaml.safe_load(raw_sa_data)
        error_msg = f"ServiceAccount annotations don't have expected values.\nExpected: {SCYLLA_CLUSTER_SA_ANNOTATIONS}\nActual: {sa_data['metadata']['annotations']}"
    ...
    def get_scylla_cluster_helm_values(self, cpu_limit, memory_limit, pool_name: str = None, cluster_name=None) -> HelmValues:
        ...
        if self.params.get("k8s_enable_sni") and ComparableScyllaOperatorVersion(self.scylla_operator_chart_version.split("-")[0]) >= "1.8.0":
        ...
        return HelmValues(
            {
                ...
                "scyllaImage": {"repository": self.params.get("docker_image"), "tag": self.params.get("scylla_version")},
                "agentImage": {"repository": "scylladb/scylla-manager-agent", "tag": self.params.get("scylla_mgmt_agent_version")},
                "serviceAccount": {"create": True, "annotations": SCYLLA_CLUSTER_SA_ANNOTATIONS, "name": f"{cluster_name}-member"},
                "alternator": {
                    ...
                    **({"writeIsolation": self.params.get("alternator_write_isolation")} if self.params.get("alternator_write_isolation") else {}),
                },
                "developerMode": False,
                "cpuset": True,
                ...
                "racks": [
                    {
                        ...
                        "members": 0,
                        "storage": {"storageClassName": self.params.get("k8s_scylla_disk_class"), "capacity": f"{self.params.get('k8s_scylla_disk_gi')}Gi"},
                        "resources": {
    ...
    def install_static_local_volume_provisioner(self, node_pools: list[CloudK8sNodePool] | CloudK8sNodePool) -> None:
        ...
        self.apply_file(LOCAL_PROVISIONER_FILE, modifiers=[affinity_modifier for current_pool in node_pools for affinity_modifier in current_pool.affinity_modifiers], envsubst=False)
    ...
    def install_dynamic_local_volume_provisioner(self, node_pools: list[CloudK8sNodePool] | CloudK8sNodePool) -> None:
        ...
        config_modifiers = [affinity_modifier for current_pool in node_pools for affinity_modifier in current_pool.affinity_modifiers]
        ...
        def image_modifier(obj):
            ...
            for container_data in obj["spec"]["template"]["spec"]["containers"]:
                if "scylladb/k8s-local-volume-provisioner" in container_data["image"]:
                    container_data["image"] = f"{container_data['image'].split(':')[0]}:{K8S_LOCAL_VOLUME_PROVISIONER_VERSION}"
        ...
        def example_disk_modifier(obj):
            ...
            for container_data in obj["spec"]["template"]["spec"]["containers"]:
                ...
                for i, cmd in enumerate(container_data["command"]):
                    ...
                    container_data["command"][i] = re.sub(r"seek=\d+", f"seek={disk_size_kb}", container_data["command"][i])

        with TemporaryDirectory() as tmp_dir_name:
            repo_dst_dir = os.path.join(tmp_dir_name, "dynamic-local-volume-provisioner")

            download_from_github(repo="scylladb/k8s-local-volume-provisioner", tag=f"tags/v{K8S_LOCAL_VOLUME_PROVISIONER_VERSION}", dst_dir=repo_dst_dir)
            ...
            if "k8s-local" in self.params.get("cluster_backend"):
                ...
                self.apply_file(path_to_disk_setup_config, modifiers=config_modifiers + [example_disk_modifier] + [image_modifier], envsubst=False)
    ...
    def prepare_k8s_scylla_nodes(self, node_pools: list[CloudK8sNodePool] | CloudK8sNodePool) -> None:
        ...
        modifiers = [affinity_modifier for current_pool in node_pools for affinity_modifier in current_pool.affinity_modifiers]
        ...
        if self.is_performance_tuning_enabled:
            ...
            if scylla_utils_docker_image := self.params.get("k8s_scylla_utils_docker_image"):
                self.kubectl('patch scyllaoperatorconfigs.scylla.scylladb.com cluster --type merge -p \'{"spec":{"scyllaUtilsImage":"%s"}}\'' % scylla_utils_docker_image, ignore_status=False)
    ...
    def deploy_ingress_controller(self, pool_name: str = None):
        ...
        if not self.params.get("reuse_cluster"):
            ...
            self.apply_file(INGRESS_CONTROLLER_CONFIG_PATH, modifiers=get_pool_affinity_modifiers(self.POOL_LABEL_NAME, pool_name), environ=environ)
    ...
    @log_run_info
    def deploy_scylla_cluster(self, node_pool_name: str, namespace: str = SCYLLA_NAMESPACE, cluster_name: str = None, s3_provider_endpoint: str = None) -> None:
        ...
        if not self.params.get("k8s_scylla_cpu_limit"):
            ...
            cpu_limit = int(cpu_limit - OPERATOR_CONTAINERS_RESOURCES["cpu"] * self.tenants_number - COMMON_CONTAINERS_RESOURCES["cpu"] - SCYLLA_MANAGER_AGENT_RESOURCES["cpu"] * self.tenants_number)
        ...
        if not self.params.get("k8s_scylla_memory_limit"):
            memory_limit = memory_limit - OPERATOR_CONTAINERS_RESOURCES["memory"] * self.tenants_number - COMMON_CONTAINERS_RESOURCES["memory"] - SCYLLA_MANAGER_AGENT_RESOURCES["memory"] * self.tenants_number
        ...
        self.log.debug(
            self.helm_install(
                ...
                version=self.scylla_operator_chart_version,
                use_devel=True,
                values=self.get_scylla_cluster_helm_values(cpu_limit=self.scylla_cpu_limit, memory_limit=self.scylla_memory_limit, pool_name=node_pool_name, cluster_name=cluster_name),
                namespace=namespace,
            )
        )
        self.wait_till_cluster_is_operational()
    ...
    def _affinity_modifiers_for_monitoring_resources(self):
        ...
        if not node_pool:
            self.log.warning("'Monitoring' node pool was not found. Will schedule 'monitoring' resources creation in the 'Auxiliary' one.")
        ...
        if not affinity_modifiers:
            self.log.warning("'Auxiliary' node pool was not found. Will schedule 'monitoring' resources creation without affinity rules.")
    ...
    def deploy_prometheus_operator(self) -> None:
        ...
        if not self.params.get("reuse_cluster"):
            ...
            self.apply_file(PROMETHEUS_OPERATOR_CONFIG_PATH, namespace=PROMETHEUS_OPERATOR_NAMESPACE, modifiers=self._affinity_modifiers_for_monitoring_resources, envsubst=False, server_side=True)
            time.sleep(3)
        self.kubectl("rollout status deployment prometheus-operator", namespace=PROMETHEUS_OPERATOR_NAMESPACE)

    def deploy_scylla_cluster_monitoring(self, cluster_name: str, namespace: str, monitoring_type: str = "Platform") -> None:
        self.log.info("Deploy 'ScyllaDBMonitoring' (type: %s) for the '%s' Scylla cluster in the '%s' namespace", monitoring_type, cluster_name, namespace)
        self.apply_file(
        ...
        for condition in ("Progressing=False", "Degraded=False", "Available=True"):
            self.kubectl_wait(f"--for='condition={condition}' scylladbmonitoring {cluster_name}", namespace=namespace, timeout=600)
    ...
    def get_grafana_ip(self, cluster_name: str, namespace: str) -> str:
        if self.cluster_backend in ("k8s-eks", "k8s-gke"):
            cmd = f"get pod -l scylla-operator.scylladb.com/deployment-name={cluster_name}-grafana --no-headers -o custom-columns=:.status.podIP"
        else:
    ...
    def register_sct_grafana_dashboard(self, cluster_name: str, namespace: str) -> str:  # pylint: disable=too-many-locals
        ...
        with open(sct_dashboard_file, encoding="utf-8") as sct_dashboard_file_obj:
            ...
            dashboard_config["dashboard"]["title"] = dashboard_config["dashboard"]["title"].replace("$test_name", f"{get_test_name()}--{cluster_name}")
        ...
        grafana_user = base64.b64decode(self.kubectl(f"get secret/{cluster_name}-grafana-admin-credentials --template='{{{{ index .data \"username\" }}}}'", namespace=namespace).stdout.strip()).decode("utf-8")
        grafana_password = base64.b64decode(self.kubectl(f"get secret/{cluster_name}-grafana-admin-credentials --template='{{{{ index .data \"password\" }}}}'", namespace=namespace).stdout.strip()).decode("utf-8")
        grafana_cert = base64.b64decode(self.kubectl(f"get secret/{cluster_name}-grafana-serving-ca --template='{{{{ index .data \"tls.crt\" }}}}'", namespace=namespace).stdout.strip()).decode("utf-8")
        ...
        if upload_result != "200":
            self.log.warning("Error uploading SCT dashboard '%s' to the grafana in the '%s' namespace: %s", sct_dashboard_file, namespace, upload_result)
        else:
            self.log.info("SCT dashboard '%s' uploaded successfully to the grafana in the '%s' namespace", sct_dashboard_file, namespace)
    ...
    def scylla_manager_cluster(self) -> "ManagerPodCluser":
        return ManagerPodCluser(k8s_clusters=[self], namespace=SCYLLA_MANAGER_NAMESPACE, container="scylla-manager", cluster_prefix="mgr-", node_prefix="mgr-node-", params=init_and_verify_sct_config(), n_nodes=1)

    def create_secret_from_data(self, secret_name: str, namespace: str, data: dict, secret_type: str = "Opaque"):
        prepared_data = {key: base64.b64encode(json.dumps(value).encode("utf-8")).decode("utf-8") for key, value in data.items()}
    ...
    def update_secret_from_data(self, secret_name: str, namespace: str, data: dict, secret_type: str = "Opaque"):
        ...
        for secret in self.k8s_core_v1_api.list_namespaced_secret(namespace).items:
            if secret.metadata.name == secret_name:
                ...
                break
        if not existing:
            self.create_secret_from_data(secret_name=secret_name, namespace=namespace, data=data, secret_type=secret_type)
            return
    ...
    def create_secret_from_directory(self, secret_name: str, path: str, namespace: str, secret_type: str = "generic", only_files: List[str] = None):
        files = [fname for fname in os.listdir(path) if os.path.isfile(os.path.join(path, fname)) and (not only_files or fname in only_files)]
        cmd = f"create secret {secret_type} {secret_name} " + " ".join([f"--from-file={fname}={os.path.join(path, fname)}" for fname in files])
    ...
    def start_token_update_thread(self):
        ...
        wait_for(os.path.exists, timeout=30, step=5, text="Wait for gcloud token", throw_exc=True, path=self.kubectl_token_path)
    ...
    def upgrade_kubernetes_platform(self, pod_objects: list[cluster.BaseNode], use_additional_scylla_nodepool: bool) -> (str, CloudK8sNodePool):
        raise NotImplementedError("Kubernetes upgrade is not implemented on this backend")

    def move_pods_to_new_node_pool(self, pod_objects: list[cluster.BaseNode], node_pool_name: str, cluster_name: str = "", cluster_namespace: str = SCYLLA_NAMESPACE, pod_readiness_timeout_minutes: int = 20):
        ...
        scylla_cluster_info = yaml.safe_load(self.kubectl(f"get scyllaclusters.scylla.scylladb.com {cluster_name} -o yaml", namespace=cluster_namespace).stdout)
        racks_info = scylla_cluster_info["spec"]["datacenter"]["racks"]
        total_pods = 0
        for i, rack in enumerate(racks_info):
            rack["placement"]["nodeAffinity"]["requiredDuringSchedulingIgnoredDuringExecution"]["nodeSelectorTerms"][0]["matchExpressions"][0]["values"] = [node_pool_name]
            racks_info[i] = rack
    ...
    def scylla_config_map(self, namespace: str = SCYLLA_NAMESPACE) -> dict:
        with self.scylla_config_lock:
            try:
                config_map = self.k8s_core_v1_api.read_namespaced_config_map(name=SCYLLA_CONFIG_NAME, namespace=namespace).data or {}
                exists = True
            ...
            if original_config_map == config_map:
                ...
                return
            if exists:
                self.k8s_core_v1_api.patch_namespaced_config_map(name=SCYLLA_CONFIG_NAME, namespace=namespace, body=[{"op": "replace", "path": "/data", "value": config_map}])
            else:
                self.k8s_core_v1_api.create_namespaced_config_map(namespace=namespace, body=V1ConfigMap(data=config_map, metadata={"name": SCYLLA_CONFIG_NAME}))
...
class BasePodContainer(cluster.BaseNode):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, name: str, parent_cluster: PodCluster, node_prefix: str = "node", node_index: int = 1, base_logdir: Optional[str] = None, dc_idx: int = 0, rack=0):
        self.node_index = node_index
        cluster.BaseNode.__init__(self, name=name, parent_cluster=parent_cluster, base_logdir=base_logdir, node_prefix=node_prefix, dc_idx=dc_idx, rack=rack)
    ...
    def _init_remoter(self, ssh_login_info):
        self.remoter = KubernetesCmdRunner(kluster=self.k8s_cluster, pod_image=self.image, pod_name=self.name, container=self.parent_cluster.container, namespace=self.parent_cluster.namespace)
    ...
    def start_journal_thread(self):
        self._journal_thread = get_system_logging_thread(logs_transport="k8s_client", node=self, target_log_file=self.system_log)
        if self._journal_thread:
            self.k8s_cluster.log.info("Use %s as logging daemon", type(self._journal_thread).__name__)
            self._journal_thread.start()
        else:
            TestFrameworkEvent(source=self.__class__.__name__, source_method="start_journal_thread", message="Got no logging daemon by unknown reason").publish()
    ...
    @property
    def _pod(self):
        pods = KubernetesOps.list_pods(self.k8s_cluster, namespace=self.parent_cluster.namespace, field_selector=f"metadata.name={self.name}")
    ...
    def _cluster_ip_service(self):
        services = KubernetesOps.list_services(self.k8s_cluster, namespace=self.parent_cluster.namespace, field_selector=f"metadata.name={self.name}")
    ...
    def private_ip_address(self) -> Optional[str]:
        if ip := self.k8s_cluster.scylla_pods_ip_mapping.get(self.parent_cluster.namespace, {}).get(self.name, {}).get("current_ip"):
            return ip
    ...
    def wait_till_k8s_pod_get_uid(self, timeout: int = None, ignore_uid=None, throw_exc=False) -> str:
        ...
        wait_for(lambda: self.k8s_pod_uid and self.k8s_pod_uid != ignore_uid, timeout=timeout, text=f"Wait till host {self} get uid", throw_exc=throw_exc)
        return self.k8s_pod_uid

    def wait_for_k8s_node_readiness(self):
        wait_for(self._wait_for_k8s_node_readiness, text=f"Wait for k8s host {self.node_name} to be ready...", timeout=self.pod_readiness_timeout * 60, step=5, throw_exc=True)

    def _wait_for_k8s_node_readiness(self):
        if self.node_name is None:
            raise RuntimeError(f"Can't find node for pod {self.name}")
        result = self.k8s_cluster.kubectl(f"wait node --timeout={self.pod_readiness_timeout // 3}m --for=condition=Ready {self.node_name}", namespace=self.parent_cluster.namespace, timeout=self.pod_readiness_timeout // 3 * 60 + 10)
        if result.stdout.count("condition met") != 1:
            raise RuntimeError("Node is not reported as ready")
        return True

    def wait_for_pod_to_appear(self):
        wait_for(self._wait_for_pod_to_appear, text="Wait for pod to appear...", timeout=self.pod_readiness_timeout * 60, throw_exc=True)
    ...
    def wait_for_pod_readiness(self, pod_readiness_timeout_minutes: int = None):
        ...
        KubernetesOps.wait_for_pod_readiness(kluster=self.k8s_cluster, pod_name=self.name, namespace=self.parent_cluster.namespace, pod_readiness_timeout_minutes=timeout)
    ...
    def hard_reboot(self):
        self.k8s_cluster.kubectl(f"delete pod {self.name} --now", namespace=self.parent_cluster.namespace, timeout=self.pod_terminate_timeout * 60 + 10)

        self.wait_for_pod_readiness()

    def soft_reboot(self):
        # Kubernetes brings pods back to live right after it is deleted
        self.k8s_cluster.kubectl(f"delete pod {self.name} --grace-period={self.pod_terminate_timeout * 60}", namespace=self.parent_cluster.namespace, timeout=self.pod_terminate_timeout * 60 + 10)
    ...
    def start_coredump_thread(self):
        self._coredump_thread = CoredumpExportFileThread(self, self._maximum_number_of_cores_to_publish, ["/var/lib/scylla/coredumps"])
...
class BaseScyllaPodContainer(BasePodContainer):  # pylint: disable=abstract-method,too-many-public-methods
    ...
    def __str__(self):
        ...
        #  to here
        return "Node %s [%s | %s%s]%s" % (self.name, self.public_ip_address, self.private_ip_address, " | %s" % self.ipv6_ip_address if self.test_config.IP_SSH_CONNECTIONS == "ipv6" else "", self._dc_info_str())
    ...
    @cluster.log_run_info
    @retrying(n=3, sleep_time=5, allowed_exceptions=NETWORK_EXCEPTIONS + (CommandTimedOut,), message="Failed to stop scylla.server, retrying...")
    def stop_scylla_server(self, verify_up=False, verify_down=True, timeout=300, ignore_status=False):
        if verify_up:
            self.wait_db_up(timeout=timeout)
        self.remoter.run('sh -c "supervisorctl stop scylla || supervisorctl stop scylla-server"', timeout=timeout, ignore_status=ignore_status)
        if verify_down:
            self.wait_db_down(timeout=timeout)
    ...
    def init(self) -> None:
        ...
        self.scylla_network_configuration = ScyllaNetworkConfiguration(network_interfaces=self.network_interfaces, scylla_network_config=[])
    ...
    def wait_for_svc(self):
        wait_for(self._wait_for_svc, text=f"Wait for k8s service {self.name} to be ready...", timeout=self.pod_readiness_timeout * 60, throw_exc=True)
    ...
    @retrying(n=60, sleep_time=10, allowed_exceptions=(k8s_exceptions.ApiException, invoke.exceptions.UnexpectedExit), message="Failed to run fstrim command...")
    def fstrim_scylla_disks(self):
        ...
        podnames = self.k8s_cluster.kubectl(f"get pod -l {pods_selector} --field-selector spec.nodeName={self.node_name} -o jsonpath='{{range .items[*]}}{{.metadata.name}}'", namespace=namespace).stdout.strip().split("\n")
        assert podnames, f"Failed to find pods using '{pods_selector}' selector on '{self.node_name}' node in '{namespace}' namespace. Didn't run the 'fstrim' command"
    ...
    def alternator_ca_bundle_path(self):
        ca_bundle_cmd_output = self.k8s_cluster.kubectl(
            f"get configmap/{self.parent_cluster.scylla_cluster_name}-alternator-local-serving-ca --template='{{{{ index .data \"ca-bundle.crt\" }}}}'", namespace=self.parent_cluster.namespace, ignore_status=True
        )
...
class PodCluster(cluster.BaseCluster):
    ...
    def __init__(
        ...
        super().__init__(cluster_uuid=cluster_uuid, cluster_prefix=cluster_prefix, node_prefix=node_prefix, n_nodes=n_nodes, params=params, region_names=[], node_type=node_type, add_nodes=add_nodes)
    ...
    def _create_node(self, node_index: int, pod_name: str, dc_idx: int, rack: int) -> BasePodContainer:
        node = self.PodContainerClass(parent_cluster=self, name=pod_name, base_logdir=self.logdir, node_prefix=self.node_prefix, node_index=node_index, dc_idx=dc_idx, rack=rack)
    ...
    def add_nodes(
        ...
        if len(nodes) != count:
            raise RuntimeError(f"Requested '{count}' number of nodes to add in the '{dc_idx}' DC, while only '{len(nodes)}' new nodes where found")
        return nodes
    ...
    def wait_sts_rollout_restart(self, pods_to_wait: int, dc_idx: int = 0):
        ...
        for statefulset in KubernetesOps.list_statefulsets(k8s_cluster, namespace=self.namespace):
            k8s_cluster.kubectl(f"rollout status statefulset/{statefulset.metadata.name} --watch=true --timeout={timeout}m", namespace=self.namespace, timeout=timeout * 60 + 10)
    ...
    def wait_for_pods_readiness(self, pods_to_wait: int, total_pods: int, readiness_timeout: int = None, dc_idx: int = 0):
        KubernetesOps.wait_for_pods_readiness(
            kluster=self.k8s_clusters[dc_idx], total_pods=total_pods, readiness_timeout=readiness_timeout or self.get_nodes_reboot_timeout(pods_to_wait), selector=self.pod_selector, namespace=self.namespace
        )

    def wait_for_pods_running(self, pods_to_wait: int, total_pods: int | callable, dc_idx: int = 0):
        KubernetesOps.wait_for_pods_running(kluster=self.k8s_clusters[dc_idx], total_pods=total_pods, timeout=self.get_nodes_reboot_timeout(pods_to_wait), selector=self.pod_selector, namespace=self.namespace)
    ...
    def generate_namespace(self, namespace_template: str) -> str:
        ...
        with NAMESPACE_CREATION_LOCK:
            namespaces = self.k8s_clusters[0].kubectl("get namespaces --no-headers -o=custom-columns=:.metadata.name").stdout.split()
...
class ScyllaPodCluster(cluster.BaseScyllaCluster, PodCluster):  # pylint: disable=too-many-public-methods
    def __init__(
        ...
        for k8s_cluster in k8s_clusters:
            ...
            k8s_cluster.deploy_scylla_cluster(node_pool_name=node_pool_name, namespace=self.namespace, cluster_name=self.scylla_cluster_name, **kwargs)
        super().__init__(
        ...
        for k8s_cluster in k8s_clusters:
            if k8s_cluster.scylla_pods_ip_change_tracker_thread:
                k8s_cluster.scylla_pods_ip_change_tracker_thread.register_callbacks(callbacks=self.refresh_scylla_pod_ip_address, namespace=self.namespace, pod_name="__each__", add_pod_name_as_kwarg=True)
    ...
    def wait_for_nodes_up_and_normal(self, nodes=None, verification_node=None, iterations=None, sleep_time=None, timeout=None):  # pylint: disable=too-many-arguments
        ...
        for dc_idx, dc_nodes in dc_node_mapping.items():
            self.wait_for_pods_readiness(pods_to_wait=len(dc_nodes), total_pods=len([n for n in self.nodes if n.dc_idx == dc_idx]), readiness_timeout=timeout, dc_idx=dc_idx)
            self.check_nodes_up_and_normal(nodes=nodes, verification_node=verification_node)

    @timeout_wrapper(timeout=300, sleep_time=3, allowed_exceptions=NETWORK_EXCEPTIONS + (ClusterNodesNotReady,), message="Waiting for nodes to join the cluster")
    ...
    def _k8s_scylla_cluster_api(self, dc_idx: int = 0) -> Resource:
        return KubernetesOps.dynamic_api(self.k8s_clusters[dc_idx].dynamic_client, api_version=SCYLLA_API_VERSION, kind=SCYLLA_CLUSTER_RESOURCE_KIND)

    @retrying(n=20, sleep_time=3, allowed_exceptions=(k8s_exceptions.ApiException,), message="Failed to update ScyllaCluster's spec...")
    def replace_scylla_cluster_value(self, path: str, value: Any, dc_idx: int = 0) -> Optional[ANY_KUBERNETES_RESOURCE]:
        self.k8s_clusters[dc_idx].log.debug("Replace `%s' with `%s' in %s's spec", path, value, self.scylla_cluster_name)
        ...
        return self._k8s_scylla_cluster_api(dc_idx=dc_idx).patch(body=request_body, name=self.scylla_cluster_name, namespace=self.namespace, content_type=JSON_PATCH_TYPE)
    ...
    def get_scylla_cluster_value(self, path: str, dc_idx: int = 0) -> Optional[ANY_KUBERNETES_RESOURCE]:
        ...
        """
        cluster_data = self._k8s_scylla_cluster_api(dc_idx=dc_idx).get(namespace=self.namespace, name=self.scylla_cluster_name)
    ...
    def get_scylla_cluster_plain_value(self, path: str, dc_idx: int = 0) -> Union[Dict, List, str, None]:
        ...
        """
        cluster_data = self._k8s_scylla_cluster_api(dc_idx=dc_idx).get(namespace=self.namespace, name=self.scylla_cluster_name).to_dict()
        return walk_thru_data(cluster_data, path)
    ...
    def add_scylla_cluster_value(self, path: str, element: Any, dc_idx: int = 0):
        ...
        if init:
            ...
            path = path + "/-"
            value = element
        self._k8s_scylla_cluster_api(dc_idx=dc_idx).patch(body=[{"op": operation, "path": path, "value": value}], name=self.scylla_cluster_name, namespace=self.namespace, content_type=JSON_PATCH_TYPE)
    ...
    def remove_scylla_cluster_value(self, path: str, element_name: str, dc_idx: int = 0):
        ...
        self._k8s_scylla_cluster_api(dc_idx=dc_idx).patch(body=[{"op": "remove", "path": f"{path}/{found_index}"}], name=self.scylla_cluster_name, namespace=self.namespace, content_type=JSON_PATCH_TYPE)
    ...
    def get_cluster_manager(self, create_if_not_exists: bool = False, force_add: bool = False, **add_cluster_extra_params) -> AnyManagerCluster:
        return super().get_cluster_manager(create_if_not_exists=create_if_not_exists, force_add=force_add, **add_cluster_extra_params)
    ...
    def node_config_setup(self, client_encrypt=None):  # pylint: disable=too-many-arguments,invalid-name
        ...
        if append_scylla_yaml:
            ...
            if any(option in append_scylla_yaml for option in unsupported_options):
                raise NotImplementedError(f"{unsupported_options} are not supported in append_scylla_yaml by k8s-* backends yet")
    ...
    def _get_rack_nodes(self, rack: int, dc_idx: int) -> list:
        return sorted([node for node in self.nodes if node.rack == rack and node.dc_idx == dc_idx], key=lambda n: n.name)

    def add_nodes(
        self,  # pylint: disable=too-many-locals,too-many-branches  noqa: PLR0913
        count: int,
        ec2_user_data: str = "",
        ...
        new_nodes = []
        self.log.debug("'%s' configuration was taken for the 'dc_idx': %s", "Single-DC" if len(dc_idx) < 2 else "Multi-DC", dc_idx)
        for current_dc_idx in dc_idx:
            node_count_in_dc = count[current_dc_idx] if current_dc_idx < len(count) else count[0]
            self.log.debug("Going to provision '%s' nodes (node_count_in_dc) in DC with '%s' (dc_idx)", node_count_in_dc, current_dc_idx)
            ...
            if current_dc_idx > 0:
                ...
                if not dc_podip_mapping.get(current_dc_idx, []):
                    assert dc_podip_mapping[0], f"Couldn't not find IP addresses of the nodes from the first DC (dc_idx=0) to be used as 'external seeds' for the pods of another DC (dc_idx={current_dc_idx})"
            ...
            self.replace_scylla_cluster_value(f"/spec/datacenter/racks/{rack}/members", total_dc_members, dc_idx=current_dc_idx)
            new_nodes.extend(super().add_nodes(count=node_count_in_dc, ec2_user_data=ec2_user_data, dc_idx=current_dc_idx, rack=rack, enable_auto_bootstrap=enable_auto_bootstrap))
            ...
            if self.params.get("use_mgmt") and current_dc_idx == 0 and (self.k8s_scylla_manager_auth_token is None):
                self.k8s_scylla_manager_auth_token = (
                    base64.b64decode(kubectl(f"get secrets/{self.scylla_cluster_name}-auth-token --template='{{{{ index .data \"auth-token.yaml\" }}}}'", namespace=self.namespace).stdout.strip()).decode("utf-8").strip()
                )
                ...
                if existing_k8s_scylla_manager_auth_token != self.k8s_scylla_manager_auth_token:
                    auth_token_base64 = base64.b64encode(self.k8s_scylla_manager_auth_token.encode("utf-8")).decode("utf-8")
                    patch_cmd = f'patch secret/{self.scylla_cluster_name}-auth-token --type=json -p=\'[ {{"op": "replace", "path": "/data/auth-token.yaml", "value": "{auth_token_base64}" }}]\''
            ...
            if current_dc_idx > 0 and is_external_seeds_set:
                ...
                #       our small sleep.
                time.sleep(10)
                self.wait_for_pods_running(pods_to_wait=node_count_in_dc, total_pods=total_dc_members, dc_idx=current_dc_idx)
                self.wait_for_pods_readiness(pods_to_wait=node_count_in_dc, total_pods=total_dc_members, dc_idx=current_dc_idx)
        return new_nodes
    ...
    def decommission(self, node: BaseScyllaPodContainer, timeout: int | float = None):
        ...
        with adaptive_timeout(operation=Operations.DECOMMISSION, node=node):
            self.replace_scylla_cluster_value(f"/spec/datacenter/racks/{rack}/members", current_members - 1, dc_idx=dc_idx)
            self.k8s_clusters[node.dc_idx].kubectl(f"wait --timeout={timeout}s --for=delete pod {node.name}", namespace=self.namespace, timeout=timeout + 10)
        self.terminate_node(node, scylla_shards=scylla_shards)
        shutil.move(node.system_log, os.path.join(node.logdir, f"system_{datetime.now().strftime('%y_%m_%d_%H_%M_%S')}.log"))
    ...
    def upgrade_scylla_cluster(self, new_version: str) -> None:
        ...
        @timeout_wrapper(timeout=self.nodes[0].pod_replace_timeout * 2 * 60, sleep_time=self.PodContainerClass.pod_readiness_delay)
        def wait_till_any_node_get_new_image(nodes_with_old_image: list):
            ...
            raise RuntimeError(f"No node was upgraded. Expected image version '{new_version}', but found {actual_versions}")
    ...
    def check_kubernetes_monitoring_health(self) -> bool:
        ...
        with ClusterHealthValidatorEvent() as kmh_event:
            for k8s_cluster in self.k8s_clusters:
                try:
                    prometheus_ip = k8s_cluster.get_prometheus_ip(cluster_name=self.scylla_cluster_name, namespace=self.namespace)
                    ...
                    ClusterHealthValidatorEvent.MonitoringStatus(
                        error=f"Failed to connect to K8S prometheus server (namespace={self.namespace}) at {prometheus_ip}:{k8s_cluster.prometheus_port}, due to the: \n".join(traceback.format_exception(type(exc), exc, exc.__traceback__))
                    ).publish()
    ...
    def restart_scylla(self, nodes=None, random_order=False):
        ...
        for i, k8s_cluster in enumerate(self.k8s_clusters):
            ...
            k8s_cluster.kubectl(f"patch scyllacluster {self.scylla_cluster_name} --type merge -p '{json.dumps(patch_data)}'", namespace=self.namespace)
            ...
            for statefulset in statefulsets:
                k8s_cluster.kubectl(f"rollout status statefulset/{statefulset.metadata.name} --watch=true --timeout={readiness_timeout}m", namespace=self.namespace, timeout=readiness_timeout * 60 + 10)
    ...
    def prefill_cluster(self, dataset_name: str):
        ...
        if not test_data:
            raise ValueError(f"Dataset unexpected value: '{dataset_name}'. Dataset with this name is not defined in the sdcm.utils.sstable.load_inventory.Expected values: BIG_SSTABLE_MULTI_COLUMNS_DATA, MULTI_COLUMNS_DATA")
        ...
        for node in self.nodes:
            with self.cql_connection_exclusive(node) as session:
                ...
                SstableLoadUtils.upload_sstables(node=node, test_data=test_data[0], create_schema=create_schema, **kwarg)
...
class LoaderPodCluster(cluster.BaseLoaderSet, PodCluster):
    ...
    def add_nodes(
        ...
        new_nodes = []
        for current_dc_idx in dc_idx:
            self.k8s_clusters[current_dc_idx].deploy_loaders_cluster(node_pool_name=self.node_pool_name, namespace=self.namespace)
            node_count_in_dc = count[current_dc_idx] if current_dc_idx < len(count) else count[0]
            ...
            self.k8s_clusters[current_dc_idx].log.debug("Check the '%s' loaders cluster in the '%s' namespace", self.loader_cluster_name, self.namespace)
