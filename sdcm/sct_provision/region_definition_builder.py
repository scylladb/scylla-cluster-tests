...
class DefinitionBuilder(abc.ABC):
    ...
    def build_instance_definition(self, region: str, node_type: NodeTypeType, index: int, instance_type: str = None) -> InstanceDefinition:
        ...
        tags = common_tags | {"NodeType": node_type, "keep_action": "terminate" if action == "destroy" else "", "NodeIndex": str(index)}
    ...
    def build_region_definition(self, region: str, availability_zone: str, n_db_nodes: int, n_loader_nodes: int, n_monitor_nodes: int) -> RegionDefinition:
        ...
        return RegionDefinition(backend=self.BACKEND, test_id=self.test_id, region=region, availability_zone=availability_zone, definitions=definitions)
    ...
    def build_all_region_definitions(self) -> List[RegionDefinition]:
        ...
        for region, db_nodes, loader_nodes, monitor_nodes in zip(self.regions, n_db_nodes, n_loader_nodes, n_monitor_nodes):
            region_definitions.append(self.build_region_definition(region=region, availability_zone=availability_zone, n_db_nodes=db_nodes, n_loader_nodes=loader_nodes, n_monitor_nodes=monitor_nodes))
    ...
    def _get_user_data_objects(self, instance_name: str, node_type: NodeTypeType) -> List[SctUserDataObject]:
        ...
        user_data_objects = [klass(test_config=self.test_config, params=self.params, instance_name=instance_name, node_type=node_type) for klass in user_data_object_classes]
