...
class AWSInstanceParamsBuilder(AWSInstanceParamsBuilderBase, metaclass=abc.ABCMeta):
    ...
    def Placement(self) -> Optional[AWSPlacementInfo]:
        return AWSPlacementInfo(AvailabilityZone=self._region_name + self._availability_zones[self.availability_zone], GroupName=self.placement_group)
    ...
    def _ec2_network_configuration(self) -> Tuple[List[str], List[List[str]]]:
        ec2_network_configuration = EC2NetworkConfiguration(regions=self.params.region_names, availability_zones=self._availability_zones, params=self.params)
    ...
    def _network_interface_params(self, interface_index: int = 0):
        LOGGER.debug("ec2_subnet_ids: %s; availability_zone: %s, interface_index: %s", self._ec2_subnet_ids, self.availability_zone, interface_index)
        return {
