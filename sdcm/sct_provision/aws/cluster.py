...
from sdcm.sct_provision.aws.instance_parameters_builder import ScyllaInstanceParamsBuilder, LoaderInstanceParamsBuilder, MonitorInstanceParamsBuilder, OracleScyllaInstanceParamsBuilder, ScyllaZeroTokenParamsBuilder
...
class ClusterBase(BaseModel):
    ...
    def _instance_parameters(self, region_id: int, availability_zone: int = 0) -> AWSInstanceParams:
        params_builder = self._INSTANCE_PARAMS_BUILDER(params=self.params, region_id=region_id, user_data_raw=self._user_data, availability_zone=availability_zone, placement_group=self.placement_group_name)
    ...
    def provision(self):
        ...
        for region_id in range(len(self._regions_with_nodes)):
            ...
            for az_id, _ in enumerate(self._azs):
                ...
                instances = self.provision_plan(region_id, self._azs[az_id]).provision_instances(instance_parameters=instance_parameters, node_tags=node_tags, node_names=node_names, node_count=node_count)
                if not instances:
...
class DBCluster(ClusterBase):
    ...
    def _zero_token_instance_parameters(self, region_id: int, availability_zone: int = 0) -> AWSInstanceParams:
        params_builder = self._ZERO_TOKEN_INSTANCE_PARAMS_BUILDER(params=self.params, region_id=region_id, user_data_raw=self._user_data, availability_zone=availability_zone, placement_group=self.placement_group_name)
        return AWSInstanceParams(**params_builder.model_dump(exclude_none=True, exclude_unset=True, exclude_defaults=True))
    ...
    def provision(self):
        ...
        for region_id in range(len(self._regions_with_nodes)):
            ...
            for az_id, _ in enumerate(self._azs):
                ...
                if zero_node_count:
                    instance_parameters = self._zero_token_instance_parameters(region_id=region_id, availability_zone=az_id)
                    node_tags = self._node_tags(region_id=region_id, az_id=az_id)[node_count:]
                    for node_tag in node_tags:
                        node_tag.update({"ZeroTokenNode": "True"})
                    node_names = self._node_names(region_id=region_id, az_id=az_id)[node_count:]
                    instances = self.provision_plan(region_id, self._azs[az_id]).provision_instances(instance_parameters=instance_parameters, node_tags=node_tags, node_names=node_names, node_count=zero_node_count)
                    if not instances:
                        raise RuntimeError("End of provision plan reached, but no instances provisioned")
                    total_instances_provisioned.extend(instances)
        return total_instances_provisioned
...
class PlacementGroup(ClusterBase):
    ...
    def provision(self):
        if self.placement_group_name:
            create_cluster_placement_groups_aws(name=self.placement_group_name, tags=self.common_tags, region=self._region(0))
