...
class GceBuilder:
    ...
    def _add_cloud_configuration_to_jenkins(self):
        ...
        res = requests.post(
            ...
            params=dict(script=JENKINS_CONFIG_FORMAT.format(name=self.name, project_id=self.region.project, region_name=self.region.region_name, max_instances=20, jenkins_labels=self.jenkins_labels)),
        )
    ...
    @staticmethod
    def _wait_for_extended_operation(operation: ExtendedOperation, verbose_name: str = "operation", timeout: int = 300) -> Any:
        """
    ...
    def _create_launch_template(self) -> compute_v1.InstanceTemplate:
        ...
        try:
            ...
            click.echo(f"template: {template.name} already exists. if change affecting were made, delete it and run this again")
