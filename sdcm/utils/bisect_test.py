...
def get_repo_urls(start_date: datetime, end_date: datetime, is_enterprise: bool = False):
    ...
    dates = [obj["Prefix"].rsplit("/", 2)[-2] for obj in response.get("CommonPrefixes", []) if obj["Prefix"].endswith("Z/")]
    repos = [f"https://{bucket}/{prefix}{date}/{suffix}" for date in dates if start_date <= datetime.strptime(date, "%Y-%m-%dT%H:%M:%SZ") <= end_date]
...
def bisect_test(test_func):
    ...
    def wrapper(*args, **kwargs):  # noqa: PLR0914
        ...
        while low <= high:
            ...
            TestFrameworkEvent(source="bisection", message=f"Updated Scylla binaries to: {version}", severity=Severity.WARNING).publish_or_dump()
            test_func(*args, **kwargs)
