...
class SSHLoggerBase(LoggerBase):
    ...
    def _retrieve(self, since: str) -> None:
        self._log.debug(self.RETRIEVE_LOG_MESSAGE_TEMPLATE.format(log_file=self._target_log_file, since=since or "the beginning"))
        try:
            ...
            remote_cmd = f"cat <<'EOF' > /tmp/logger_cmd_{os.getpid()}.sh\n{cmd}\nEOF\nsetsid bash /tmp/logger_cmd_{os.getpid()}.sh & echo $! > {remote_pid_file}; wait"
            self._remoter.run(
                cmd=remote_cmd,
            ...
            self._log.error("Error retrieving remote node DB service log for file %s: %s", self._target_log_file, details)
...
class HDRHistogramFileLogger(SSHLoggerBase):
    ...
    def validate_and_collect_hdr_file(self):
        ...
        LOGGER.debug("'%s' file is not found on the runner. Try to find it on the loader %s", self._target_log_file, self._node.name)
        HDRFileMissed(message=f"'{self._remote_log_file}' HDR file was not copied to the runner from loader", severity=Severity.WARNING).publish()
        result = self._node.remoter.run(f"test -f {self._remote_log_file}", ignore_status=True)
        if not result.ok:
            HDRFileMissed(message=f"'{self._remote_log_file}' HDR file was not created on the loader {self._node.name}", severity=Severity.ERROR).publish()
        try:
            ...
            HDRFileMissed(message=f"'{self._remote_log_file}' HDR file couldn't copied from loader {self._node.name}", severity=Severity.ERROR).publish()
    ...
    def _journal_thread(self) -> None:
        ...
        while not te_is_set:
            ...
            te_is_set = self._termination_event.is_set()
            LOGGER.debug("_termination_event is set?: %s. %s -> %s", te_is_set, self._remote_log_file, self._target_log_file)
...
class KubectlGeneralLogger(CommandNodeLoggerBase):
    ...
    def _logger_cmd(self) -> str:
        ...
        cmd = self._node.k8s_cluster.kubectl_cmd(f"logs --previous=false -f --since={int(self.time_delta)}s", self._node.name, "-c", parent_cluster.container, namespace=parent_cluster.namespace)
        ...
        while True:
            ...
            if isoparse(log_timestamp) >= isoparse(self._last_log_timestamp):
                TestFrameworkEvent(
                    ...
                    message=f"Failed to find last log timestamp in the log stream for {self._pod_name} after reconnection. Possibly some log lines are missed or duplicated. Last reread timestamp: {log_timestamp}",
    ...
    def _log_loop(self):
        ...
        while not self._termination_event.is_set():
            try:
                ...
                self._log.debug("Stream from pod %s logs has been closed, waiting for %s seconds and trying to reconnect", self._pod_name, self.RECONNECT_DELAY)
                self._stream = None
                if self._termination_event.is_set():
                    return
...
class KubectlClusterEventsLogger(CommandClusterLoggerBase):
    ...
    @property
    def _logger_cmd(self) -> str:
        cmd = "get events -w -o custom-columns=FirstSeen:.firstTimestamp,LastSeen:.lastTimestamp,Count:.count,From:.source.component,Type:.type,Reason:.reason,Message:.message"
...
class CertManagerLogger(CommandClusterLoggerBase):
    ...
    @property
    def _logger_cmd(self) -> str:
        cmd = self._cluster.kubectl_cmd(f"logs --previous=false -f --since={int(self.time_delta)}s --all-containers=true -l app.kubernetes.io/instance=cert-manager", namespace="cert-manager")
        return f"{cmd} >> {self._target_log_file} 2>&1"
...
class ScyllaManagerLogger(CommandClusterLoggerBase):
    ...
    @property
    def _logger_cmd(self) -> str:
        cmd = self._cluster.kubectl_cmd(f"logs --previous=false -f --since={int(self.time_delta)}s --all-containers=true -l app.kubernetes.io/instance=scylla-manager", namespace=self._cluster._scylla_manager_namespace)
        return f"{cmd} >> {self._target_log_file} 2>&1"
...
class ScyllaOperatorLogger(CommandClusterLoggerBase):
    ...
    @property
    def _logger_cmd(self) -> str:
        cmd = self._cluster.kubectl_cmd(f"logs --previous=false -f --since={int(self.time_delta)}s --all-containers=true -l app.kubernetes.io/instance=scylla-operator", namespace=self._cluster._scylla_operator_namespace)
        return f"{cmd} >> {self._target_log_file} 2>&1"
...
class HaproxyIngressLogger(CommandClusterLoggerBase):
    ...
    @property
    def _logger_cmd(self) -> str:
        cmd = self._cluster.kubectl_cmd(f"logs --previous=false -f --since={int(self.time_delta)}s --all-containers=true -l app.kubernetes.io/name=haproxy-ingress", namespace="haproxy-controller")
        return f"{cmd} >> {self._target_log_file} 2>&1"
...
class KubernetesWrongSchedulingLogger(CommandClusterLoggerBase):
    ...
    def _logger_cmd(self) -> str:
        ...
        if self._cluster.SCYLLA_POOL_NAME in self._cluster.pools:
            ...
            self._log.warning("'%s' pool is not registered. Can not get node names to check pods scheduling", self._cluster.SCYLLA_POOL_NAME)
            return ""
        try:
