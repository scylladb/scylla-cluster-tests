...
class GceRegion:
    ...
    def configure_firewall(self):
        firewall_configurations = [
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-ssh", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["22"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-all-internal", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="all")], source_ranges=["10.0.0.0/8"], network=self.network.self_link),
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-icmp", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="icmp")], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-grafana", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["3000"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-node-gossip", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["7000", "7001"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-node-api", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["10000"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(
                name=f"{self.SCT_NETWORK_NAME}-allow-node-cql", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["9042", "9142", "19042", "19142"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link
            ),
            Firewall(
                name=f"{self.SCT_NETWORK_NAME}-allow-prometheus", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["9090", "9100", "9103", "9180"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link
            ),
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-http", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["80"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-alternator", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["8080"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(
            ...
            Firewall(name=f"{self.SCT_NETWORK_NAME}-allow-vector", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="tcp", ports=["15000"])], source_ranges=["0.0.0.0/0"], network=self.network.self_link),
            Firewall(
                name=f"{self.SCT_NETWORK_NAME}-deny-all", direction="INGRESS", denied=[compute_v1.Denied(I_p_protocol="all")], source_ranges=["0.0.0.0/0"], network=self.network.self_link, target_tags=["sct-network-only"], priority=200
            ),
            Firewall(
            ...
            Firewall(
                name=f"{self.SCT_NETWORK_NAME}-allow-public", direction="INGRESS", allowed=[compute_v1.Allowed(I_p_protocol="all")], source_ranges=["0.0.0.0/0"], network=self.network.self_link, target_tags=["sct-allow-public"], priority=100
            ),
        ]
        for firewall_rule in firewall_configurations:
            try:
                ...
                if firewall.allowed != firewall_rule.allowed or firewall.source_ranges != firewall_rule.source_ranges:
                    ...
                    self.firewall_client.patch(project=self.project, firewall=firewall_rule.name, firewall_resource=firewall_rule)
    ...
    def create_backup_service_account(self):
        ...
        try:
            backup_service_account = (
                self.iam.projects()
                .serviceAccounts()
                .create(name="projects/" + self.project, body={"accountId": self.SCT_BACKUP_SERVICE_ACCOUNT, "serviceAccount": {"displayName": "Account for having access to the gcs bucket for SCT backup tests"}})
                .execute()
            )
            ...
            if not exc.status_code == 409:
                raise
            service_accounts = self.iam.projects().serviceAccounts().list(name=f"projects/{self.project}", pageSize=100).execute()
    ...
    def add_network_peering(self, peering_name: str, peer_project: str, peer_net: str, wait_for_active: bool = True) -> None:
        peer_network_url = f"projects/{peer_project}/global/networks/{peer_net}"
        peering_request = compute_v1.AddPeeringNetworkRequest(
            network=self.SCT_NETWORK_NAME, project=self.project, networks_add_peering_request_resource=compute_v1.NetworksAddPeeringRequest(name=peering_name, peer_network=peer_network_url, auto_create_routes=True)
        )
    ...
    def cleanup_vpc_peering_connection(self, peering_name: str) -> bool:
        ...
        remove_request = compute_v1.RemovePeeringNetworkRequest(network=self.SCT_NETWORK_NAME, project=self.project, networks_remove_peering_request_resource=compute_v1.NetworksRemovePeeringRequest(name=peering_name))
        ...
        if final_status:
            LOGGER.warning("GCP network peering %s still exists after deletion attempt (status: %s)", peering_name, final_status)
            return False
        return True
