    ...
    def build_histograms_summary_with_interval(self, path: str,
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                for e, start_interval in enumerate(start_intervals):
                    ...
                    LOGGER.debug(f"Submitted future for tags {self.hdr_tags} and interval {interval_num} out of {len(start_intervals)}")
                    interval_num += 1
                results = {}
                for e, future in enumerate(futures):
                    res = future.result()
                    LOGGER.debug(f"Got result for {e} future for tag {self.hdr_tags[e % len(self.hdr_tags)]} and interval {e // len(self.hdr_tags)}")
                    if res:
