...
class _HdrRangeHistogramBuilder:
    ...
    def build_histogram_summary_by_tag(self, path: str, hdr_tag: str) -> dict[str, dict[str, int]] | None:
        ...
        if os.path.exists(path) and os.path.isfile(path):
            ...
            if not histogram:
                LOGGER.debug(f'Exiting with None for tag {hdr_tag} in path {path}')
                return None
        elif os.path.exists(path) and os.path.isdir(path):
            histogram = self._build_histogram_from_dir(base_path=path, hdr_tag=hdr_tag)
        else:
            LOGGER.debug(f'Exiting with None for tag {hdr_tag} in path {path} - not a file or directory')
            return None

        val = self._get_summary_for_operation_by_hdr_tag(histogram)
        LOGGER.debug(f'Exiting with value {val} for tag {hdr_tag} in path {path}')
        return val
