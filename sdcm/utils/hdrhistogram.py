...
def make_hdrhistogram_summary(hdr_tags: list[str], stress_operation: str, start_time: int | float, end_time: int | float, pattern: str = "", base_path="", absolute_time: bool = True) -> list[dict[str, dict[str, int]]]:
    """
...
def make_hdrhistogram_summary_by_interval(
    hdr_tags: list[str], stress_operation: str, path: str, start_time: int | float, end_time: int | float, interval: int | float = TIME_INTERVAL, absolute_time: bool = True
...
def make_hdrhistogram_summary_from_log_line(hdr_tags: list[str], stress_operation: str, log_line: str, hst_log_start_time: float) -> dict[str, dict[str, int]]:
    """
...
_HistorgramSummary = make_dataclass("HistorgramSummary", [(_generate_percentile_name(perc), float) for perc in PERCENTILES] + [("throughput", int)], bases=(_HistorgramSummaryBase,))
...
class _HdrHistogram(HdrHistogram):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(lowest_trackable_value=_HdrHistogram.LOWEST, highest_trackable_value=_HdrHistogram.HIGHEST, significant_figures=_HdrHistogram.SIGNIFICANT, *args, **kwargs)
...
class _HdrRangeHistogramBuilder:
    def __init__(self, hdr_tags: list[str], stress_operation: str, start_time: int | float, end_time: int | float, hdr_file_pattern: str = "*/hdrh-*.hdr"):
    ...
    def build_histograms_summary_with_interval(self, path: str, interval=TIME_INTERVAL) -> list[dict[str, dict[str, int]]]:
        ...
        try:
            ...
            LOGGER.debug(f"Building histogram summary for {path} with tags {self.hdr_tags} and intervals {len(start_intervals)}")
            ...
            for start_interval in start_intervals:
                ...
                for hdr_tag in self.hdr_tags:
                    summary_with_interval_by_tag = self._build_histograms_summary_with_interval_by_tag(path, hdr_tag, start_interval, end_interval, interval_num)
                    LOGGER.debug(f"Got result for tag {hdr_tag} and interval #{interval_num}: {summary_with_interval_by_tag}")
                    if summary_with_interval_by_tag:
                        ...
                        results[summary_with_interval_by_tag["interval_num"]].update(summary_with_interval_by_tag["result"])
                        LOGGER.debug("Updated results for interval_num=%s", summary_with_interval_by_tag.get("interval_num"))
                interval_num += 1

                LOGGER.debug(f"Processing interval {interval_num} out of {len(start_intervals)} for tags {self.hdr_tags}")
    ...
    def _build_histogram_from_file(self, hdr_file: str, hdr_tag: str) -> _HdrRangeHistogram | None:
        def analyze_hdr_file():
            ...
            if not (next_hist := hdr_reader.get_next_interval_histogram(range_start_time_sec=self.start_time, range_end_time_sec=self.end_time, absolute=self.absolute_time)):
            ...
            while next_hist:
                ...
                while next_hist:
                    line_index += 1
                    try:
                        next_hist = hdr_reader.get_next_interval_histogram(range_start_time_sec=self.start_time, range_end_time_sec=self.end_time, absolute=self.absolute_time)
                    except IndexError:
        ...
        if not file_with_correct_time_interval:
            ...
            LOGGER.debug("The file '%s' does not include the time interval from `%s` to `%s`", hdr_file, self.start_time, self.end_time)
            return None

        LOGGER.debug(f"Collected histogram from file {hdr_file} with tag {hdr_tag}: {histogram}")
        # Keep this message for future debug
        LOGGER.debug("Collect data from the file '%s' (time interval from `%s` to `%s`)", hdr_file, self.start_time, self.end_time)
        if histogram.get_start_time_stamp() == 0:
            return None

        return _HdrRangeHistogram(start_time=self.start_time, end_time=self.end_time, histogram=histogram, hdr_tag=histogram.get_tag())
    ...
    def _get_summary_for_operation_by_hdr_tag(self, histogram: _HdrRangeHistogram) -> dict[str, dict[str, int]] | None:
        if histogram.histogram and (parsed_summary := self._convert_raw_histogram(histogram.histogram, histogram.start_time, histogram.end_time)):
    ...
    @staticmethod
    def _convert_raw_histogram(histogram: _HdrHistogram, base_start_ts: float = 0.0, base_end_ts: float = 0.0) -> "_HistorgramSummary":
        percentiles_data = {}
        if percentiles := histogram.get_percentile_to_value_dict(PERCENTILES):
            ...
            percentiles_data["throughput"] = round(histogram.get_total_count() / ((histogram.get_end_time_stamp() - histogram.get_start_time_stamp()) / 1000))
            return _HistorgramSummary(start_time=histogram.get_start_time_stamp() or base_start_ts, end_time=histogram.get_end_time_stamp() or base_end_ts, stddev=histogram.get_stddev(), **percentiles_data)
        return None
    ...
    def _build_histograms_summary_with_interval_by_tag(self, path: str, hdr_tag: str, start_interval: int, end_interval: int, interval_num: int) -> dict[str, Any] | None:
