...
def get_number_of_irq_bits(node) -> int:
    ...
    num_cores_total = int(node.remoter.run(f"hwloc-calc --restrict {cpu_mask_all} --number-of core machine:0").stdout.strip())
    num_process_units_total = int(node.remoter.run(f"hwloc-calc --restrict {cpu_mask_all} --number-of PU machine:0").stdout.strip())
...
class PerftuneExecutor:
    ...
    def get_cpu_mask(self) -> str:
        result = self.node.remoter.run(f"{self.node.add_install_prefix(PERFTUNE_LOCATION)} --tune net --nic {self.nic_name} --get-cpu-mask")
        return result.stdout.strip()

    def get_irq_cpu_mask(self) -> str:
        result = self.node.remoter.run(f"{self.node.add_install_prefix(PERFTUNE_LOCATION)} --tune net --nic {self.nic_name} --get-irq-cpu-mask")
        return result.stdout.strip()
...
class PerftuneOutputChecker:
    def __init__(self, node):
        ...
        self.expected_result = PerftuneExpectedResult(number_of_cpu_cores=get_number_of_cpu_cores(node), nic_name=nic_name, expected_irq_bits=get_number_of_irq_bits(node))
    ...
    def compare_option_file_with_overridden_irq_cpu_mask_param(self, option_file_dict) -> None:
        ...
        altered_option_file_contents = self.executor.get_options_file_contents(override_irq_cpu_mask=random_irq_cpu_mask_string)
        if altered_option_file_contents["irq_cpu_mask"] != random_irq_cpu_mask_string:
            PerftuneResultEvent(message=f"Despite overriding the irq_cpu_mask param, its value is was not altered properly in the output of the 'dump-options-file' command on node {self.node}", severity=Severity.ERROR).publish()
    ...
    def compare_perftune_results(self) -> None:
        ...
        try:
            ...
            PerftuneResultEvent(message=f"Unexpected error when verifying the output of Perftune: {error}", severity=Severity.ERROR, trace=traceback.format_exc()).publish()
