...
class AwsVpcPeering:
    ...
    def peering_connection(origin_region, target_region):
        ...
        peering_connection = origin_region.client.describe_vpc_peering_connections(Filters=[{"Name": "tag:Name", "Values": [peering_name]}])
        if origin_vpx := peering_connection.get("VpcPeeringConnections"):
            ...
            origin_vpx = origin_region.resource.VpcPeeringConnection(res.get("VpcPeeringConnection").get("VpcPeeringConnectionId"))
    ...
    def configure_route_tables(origin_region, target_region, vpc_peering_id):
        for index in range(AwsRegion.SCT_SUBNET_PER_AZ):
            ...
            try:
                route_table.create_route(DestinationCidrBlock=str(target_region.vpc_ipv4_cidr), VpcPeeringConnectionId=vpc_peering_id)
            except ClientError as ex:
                if "already exists" not in str(ex):
                    raise
            try:
                route_table.create_route(DestinationIpv6CidrBlock=str(target_region.vpc_ipv6_cidr), VpcPeeringConnectionId=vpc_peering_id)
            except ClientError as ex:
                if "already exists" not in str(ex):
                    raise
        ...
        try:
            origin_region.sct_security_group.authorize_ingress(
                IpPermissions=[
                    {
                        "IpProtocol": "-1",
                        "IpRanges": [{"CidrIp": str(target_region.vpc_ipv4_cidr), "Description": f"VPC-peering for {target_region.region_name}"}],
                        "Ipv6Ranges": [{"CidrIpv6": str(target_region.vpc_ipv6_cidr), "Description": f"VPC-peering for {target_region.region_name}"}],
                    },
                ]
            )
        except ClientError as ex:
        ...
        try:
            origin_region.sct_security_group.authorize_ingress(
                IpPermissions=[
                    {
                        "IpProtocol": "-1",
                        "IpRanges": [{"CidrIp": str(origin_region.vpc_ipv4_cidr), "Description": f"Local for {origin_region.region_name}"}],
                        "Ipv6Ranges": [{"CidrIpv6": str(origin_region.vpc_ipv6_cidr), "Description": f"Loca; for {origin_region.region_name}"}],
                    },
                ]
            )
        except ClientError as ex:
    ...
    def configure(self):
        ...
        for origin_region, target_region in regions_pair:
            LOG.info("Configure peering for: %s %s, %s %s", origin_region.region_name, origin_region.vpc_ipv4_cidr, target_region.region_name, target_region.vpc_ipv4_cidr)
