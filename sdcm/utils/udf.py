...
class UDF(BaseModel):
    ...
    def get_create_query(self, ks: str, create_or_replace: bool = True) -> str:
        ...
        return f"{create_part} {ks}.{self.name}{self.args} RETURNS {self.called_on_null_input_returns} ON NULL INPUT RETURNS {self.return_type} LANGUAGE {self.language} AS '{self.script}'"
