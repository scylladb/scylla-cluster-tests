...
class RemoteDocker(BaseNode):
    def __init__(self, node, image_name, ports=None, command_line="tail -f /dev/null", extra_docker_opts="", docker_network=None):
        ...
        res = self.node.remoter.run(f"{self.sudo_needed} docker run {extra_docker_opts} -d {ports} {image_name} {command_line}", verbose=True, retry=3)
    ...
    def internal_ip_address(self):
        if not self._internal_ip_address:
            if self.docker_network:
                self._internal_ip_address = self.node.remoter.run(f"docker inspect --format='{{{{ (index .NetworkSettings.Networks \"{self.docker_network}\").IPAddress }}}}' {self.docker_id}").stdout.strip()
            else:
                self._internal_ip_address = self.node.remoter.run(f"docker inspect --format='{{{{ .NetworkSettings.Networks.bridge.IPAddress }}}}' {self.docker_id}").stdout.strip()
    ...
    def create_network(self, docker_network):
        try:
            ret = self.node.remoter.run(f"docker network create {docker_network} --label 'com.docker.compose.network=default'").stdout.strip()
            LOGGER.debug(ret)
    ...
    def send_files(self, src, dst, **kwargs):
        ...
        result &= self.node.remoter.run(f"{self.sudo_needed} docker cp {remote_tempfile} {self.docker_id}:{dst}", verbose=kwargs.get("verbose"), ignore_status=True).ok
        return result

    def receive_files(self, src, dst, **kwargs):
        remote_tempfile = self.node.remoter.run("mktemp").stdout.strip()

        result = self.node.remoter.run(f"{self.sudo_needed} docker cp {self.docker_id}:{src} {remote_tempfile}", verbose=kwargs.get("verbose"), ignore_status=True).ok
