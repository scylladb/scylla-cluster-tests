...
class RemoteDocker(BaseNode):
        ...
        if not self._internal_ip_address:
            if self.docker_network:
                self._internal_ip_address = self.node.remoter.run(
                    f"docker inspect --format='{{{{ (index .NetworkSettings.Networks \"{self.docker_network}\").IPAddress }}}}' {self.docker_id}"
                ).stdout.strip()
            else:
                self._internal_ip_address = self.node.remoter.run(
                    f"docker inspect --format='{{{{ .NetworkSettings.IPAddress }}}}' {self.docker_id}"
                ).stdout.strip()
    ...
    def pull_image(node, image):
        ...
        remote_cmd = (
            node.remoter.sudo
            if RemoteDocker.running_in_docker(node) and not RemoteDocker.running_in_podman(node)
            else node.remoter.run
        )
