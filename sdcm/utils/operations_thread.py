...
class OperationThreadStats:
    ...
    def get_stats_pretty_table(self) -> PrettyTable | None:
        ...
        for stat in self.stats:
            pretty_table.add_row([stat.op_type, stat.duration, "\n".join(stat.exceptions), stat.nemesis_at_start, stat.nemesis_at_end, stat.success, stat.cmd])
        return pretty_table
...
class OperationThread:
    ...
    def __init__(self, thread_params: ThreadParams, thread_name: str = ""):
        ...
        self.operation_params = {"generator": self.generator, "thread_params": self.thread_params, "thread_stats": self.thread_stats}
    ...
    def _wait_until_user_table_exists(self, timeout_min: int = 40):
        ...
        if self.thread_params.ks_cf.lower() == "random":
            wait.wait_for(func=lambda: len(self.thread_params.db_cluster.get_non_system_ks_cf_list(db_node)) > 0, step=60, text=text, timeout=60 * timeout_min, throw_exc=True)
            self.thread_params.ks_cf = self.thread_params.db_cluster.get_non_system_ks_cf_list(db_node)[0]
        else:
            wait.wait_for(func=lambda: self.thread_params.ks_cf in (self.thread_params.db_cluster.get_non_system_ks_cf_list(db_node)), step=60, text=text, timeout=60 * timeout_min, throw_exc=True)
