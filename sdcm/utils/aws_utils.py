...
class EksClusterCleanupMixin:
    ...
    def attached_security_group_ids(self) -> List[str]:
        return [group_desc["GroupId"] for group_desc in self.ec2_client.describe_security_groups(Filters=self.cluster_owned_objects_filter)["SecurityGroups"]]
    ...
    def _get_attached_nodegroup_names(self, status: str = None) -> List[str]:
        ...
        for nodegroup_name in self.attached_nodegroup_names:
            if status == self.eks_client.describe_nodegroup(clusterName=self.short_cluster_name, nodegroupName=nodegroup_name)["nodegroup"]["status"]:
    ...
    def check_if_all_network_interfaces_detached(self, sg_id):
        for interface_description in self.ec2_client.describe_network_interfaces(Filters=[{"Name": "group-id", "Values": [sg_id]}])["NetworkInterfaces"]:
    ...
    def delete_network_interfaces_of_sg(self, sg_id: str):
        network_interfaces = self.ec2_client.describe_network_interfaces(Filters=[{"Name": "group-id", "Values": [sg_id]}])["NetworkInterfaces"]
        ...
        def _destroy_attached_nodegroups():
            for node_group_name in self._get_attached_nodegroup_names(status=status):
                try:
                    ...
                    LOGGER.debug("Failed to delete nodegroup %s/%s, due to the following error:\n%s", self.short_cluster_name, node_group_name, exc)
            time.sleep(10)
            return wait_for(lambda: not self._get_attached_nodegroup_names(status="DELETING"), text="Waiting till target nodegroups are deleted", step=10, timeout=300, throw_exc=False)
    ...
    def destroy_oidc_provider(self):
        try:
            ...
            for oidc_provider in oidc_providers["OpenIDConnectProviderList"]:
                oidc_provider_tags = self.iam_client.list_open_id_connect_provider_tags(OpenIDConnectProviderArn=oidc_provider["Arn"])["Tags"]
                ...
                LOGGER.warning("Couldn't find any OIDC provider associated with the '%s' EKS cluster", self.short_cluster_name)
        except Exception as exc:  # noqa: BLE001
            LOGGER.warning("Failed to delete OIDC provider for the '%s' cluster due to the following error:\n%s", self.short_cluster_name, exc)
...
def init_monitoring_info_from_params(monitor_info: dict, params: dict, regions: List):
    ...
    if monitor_info["device_mappings"] is None:
        if monitor_info["disk_size"]:
            monitor_info["device_mappings"] = [{"DeviceName": ec2_ami_get_root_device_name(image_id=params.get("ami_id_monitor").split()[0], region_name=regions[0]), "Ebs": {"VolumeSize": monitor_info["disk_size"], "VolumeType": "gp3"}}]
        else:
...
def init_db_info_from_params(db_info: dict, params: dict, regions: List, root_device: str = None):
    ...
    if db_info["device_mappings"] is None and (root_device or params.get("ami_id_db_scylla")):
        if db_info["disk_size"]:
            root_device = root_device if root_device else ec2_ami_get_root_device_name(image_id=params.get("ami_id_db_scylla").split()[0], region_name=regions[0])
            db_info["device_mappings"] = [{"DeviceName": root_device, "Ebs": {"VolumeSize": db_info["disk_size"], "VolumeType": "gp3"}}]
        else:
            db_info["device_mappings"] = []

        additional_ebs_volumes_num = params.get("data_volume_disk_num")
        if additional_ebs_volumes_num > 0:
            ebs_info = {"DeleteOnTermination": True, "VolumeType": params.get("data_volume_disk_type"), "VolumeSize": params.get("data_volume_disk_size")}
...
class EC2NetworkConfiguration:
    ...
    def subnets_per_region(self) -> dict:
        ...
        for region in self.regions:
            ...
            for availability_zone in self.availability_zones:
                ec2_subnet_ids[region][availability_zone] = self.subnets_per_availability_zone(region=region, aws_region=aws_region, availability_zone=availability_zone)
    ...
    def security_groups(self):
        ...
        for region in self.regions:
            ec2_security_group_ids.append(self.region_security_groups(region=region, aws_region=AwsRegion(region_name=region)))
...
def get_common_params(params: dict, regions: List, credentials: List, services: List, availability_zone: str = None) -> dict:
    ...
    ec2_network_configuration = EC2NetworkConfiguration(regions=regions, availability_zones=availability_zones, params=params)
    return dict(
...
@retrying(n=90, sleep_time=10, allowed_exceptions=(PublicIpNotReady, botocore.exceptions.ClientError), message="Waiting for instance to get public ip")
...
class AwsIAM:
    ...
    def add_resource_to_iam_policy(self, policy_arn: str, resource_to_add: str) -> None:
        ...
        policy_version = self.iam_client.get_policy_version(PolicyArn=policy_arn, VersionId=policy["Policy"]["DefaultVersionId"])
        ...
        self.iam_client.create_policy_version(PolicyArn=policy_arn, PolicyDocument=json.dumps(policy_document), SetAsDefault=True)
