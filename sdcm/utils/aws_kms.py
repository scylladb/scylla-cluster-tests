...
class AwsKms:
    ...
    def create_alias(self, kms_key_alias_name, tolerate_already_exists=True):
        ...
        for region_name in self.region_names:
            ...
            LOGGER.info("Creating '%s' alias for the '%s' KMS key in the '%s' region", kms_key_alias_name, kms_key_id, region_name)
            try:
    ...
    def rotate_kms_key(self, kms_key_alias_name):
        ...
        for region_name in self.region_names:
            ...
            LOGGER.info("Assigning the '%s' alias to the '%s' KMS key in the '%s' region", kms_key_alias_name, new_kms_key_id, region_name)
                ...
                for kms_key_id in kms_keys:
                    ...
                    for current_alias in current_aliases:
                        ...
                        if not current_alias_name.startswith("alias/testid-"):
                            LOGGER.info("KMS: %s[region '%s'][key '%s'] ignore the '%s' alias as not matching", dry_run_prefix, region_name, kms_key_id, current_alias_name)
                            continue
                        if current_alias_creation_date < alias_allowed_date:
                            tags_dict = {
                                "TestId": current_alias_name.split("alias/testid-")[-1],
                            }
                            aws_instances = list_instances_aws(tags_dict=tags_dict, region_name=region_name, group_as_region=False)
                            ...
                            if not alias_is_unused:
                                LOGGER.info(
                                    "KMS: %s[region '%s'][key '%s'] Found old alias -> '%s' (%s). Skip it because related DB nodes still exist.", dry_run_prefix, region_name, kms_key_id, current_alias_name, current_alias_creation_date
                                )
                                continue
                            LOGGER.info("KMS: %s[region '%s'][key '%s'] %s old alias -> '%s' (%s)", dry_run_prefix, region_name, kms_key_id, ("found" if dry_run else "deleting"), current_alias_name, current_alias_creation_date)
                            if not dry_run:
