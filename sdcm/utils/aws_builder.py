...
class AwsBuilder:
    ...
    def update_launch_template_if_needed(self, runner):
        ...
        if launch_template_data.get("LaunchTemplateData") != curr_launch_template_data:
            try:
                ...
                res = self.region.client.create_launch_template_version(LaunchTemplateName=self.launch_template_name, SourceVersion=str(default_version.get("VersionNumber")), **launch_template_data)
                version_number = res.get("LaunchTemplateVersion", {}).get("VersionNumber")
                self.region.client.modify_launch_template(LaunchTemplateName=self.launch_template_name, DefaultVersion=str(version_number))
    ...
    def create_launch_template(self):
        ...
        if not runner.image:
            runner.create_image()
        try:
            self.region.client.create_launch_template(LaunchTemplateName=self.launch_template_name, **self.get_launch_template_data(runner))
        except botocore.exceptions.ClientError as error:
            LOGGER.debug(error.response)
    ...
    def add_scaling_group_to_jenkins(self):
        ...
        res = requests.post(
            ...
            params=dict(script=ASG_CONFIG_FORMAT.format(name=self.name, num_executors=self.NUM_EXECUTORS, region_name=self.region.region_name, min_size=0, max_size=100, asg_id=self.name, jenkins_labels=self.jenkins_labels)),
        )
