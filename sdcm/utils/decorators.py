...
class retrying:
    ...
    def __init__(self, n=3, sleep_time=1, allowed_exceptions=(Exception,), message="", timeout=0, raise_on_exceeded=True):
        if n:
...
def latency_calculator_decorator(original_function: Optional[Callable] = None, *, legend: Optional[str] = None, cycle_name: Optional[str] = None, workload_type: Optional[str] = None, row_name: Optional[str] = None):
    """
    ...
    def wrapper(func):
        ...
        def wrapped(*args, **kwargs):  # noqa: PLR0914
            ...
            if isinstance(_self, ClusterTester):
                ...
                raise ValueError(f"Not expected instance type '{type(_self)}'. Supported types: 'ClusterTester', 'Nemesis'")
            ...
            with EventCounterContextManager(name=func.__name__, event_type=(DatabaseLogEvent.REACTOR_STALLED,)) as counter:
                res = func(*args, **kwargs)
            ...
            try:
                ...
                hdr_tags = []
            try:
                result["hdr"] = tester.get_hdrhistogram_by_interval(hdr_tags=hdr_tags, stress_operation=workload, start_time=start, end_time=end)
            ...
            try:
                result["hdr_summary"] = tester.get_hdrhistogram(hdr_tags=hdr_tags, stress_operation=workload, start_time=start, end_time=end)
            ...
            try:
                ...
                if "InsufficientInstanceCapacity" in str(ex):
                    if not check_cluster_layout(cluster):
                        TestFrameworkEvent(source=inner_func.__name__, message=f"Test failed due to capacity issues: {ex} cluster is unbalanced, continuing with test would yield unknown results", severity=Severity.CRITICAL).publish()
                    else:
                ...
                if not check_cluster_layout(cluster):
                    TestFrameworkEvent(
                        source=inner_func.__name__, message=f"Test failed due to service availability issues: {ex} cluster is unbalanced, continuing with test would yield unknown results", severity=Severity.CRITICAL
                    ).publish()
                else:
                    raise UnsupportedNemesis("Capacity Issue") from ex
...
def critical_on_capacity_issues(func: callable) -> callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if "InsufficientInstanceCapacity" in str(ex):
                TestFrameworkEvent(source=func.__name__, message=f"Test failed due to capacity issues: {ex} cluster is probably unbalanced, continuing with test would yield unknown results", severity=Severity.CRITICAL).publish()
            raise
        except ServiceUnavailable as ex:
            TestFrameworkEvent(source=func.__name__, message=f"Test failed due to service availability issues: {ex} cluster is probably unbalanced, continuing with test would yield unknown results", severity=Severity.CRITICAL).publish()
