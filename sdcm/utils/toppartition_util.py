...
class TopPartitionCmd(ABC):
    ...
    def verify_output(self, output: str):
        ...
        for _sampler in self._built_args["samplers"].split(","):
            ...
            assert toppartition_result[sampler]["toppartitions"] == self._built_args["toppartition"], "Wrong expected and actual top partitions number for {} sampler".format(sampler)
            assert toppartition_result[sampler]["capacity"] == self._built_args["capacity"], "Wrong expected and actual capacity number for {} sampler".format(sampler)
            assert len(toppartition_result[sampler]["partitions"].keys()) <= int(self._built_args["toppartition"]), "Wrong number of requested and expected toppartitions for {} sampler".format(sampler)
...
class NewApiTopPartitionCmd(TopPartitionCmd):
    ...
    def get_cmd_args(self) -> str:
        return "--ks-filters {ks_filters} --cf-filters {cf_filters} -s {capacity} -k {toppartition} -a {samplers} -d {duration}".format(**self._built_args)
