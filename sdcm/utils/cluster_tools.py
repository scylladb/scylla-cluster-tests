    ...
    if capacity_errors_check_mode == "per-initial_config":
        for dc_idx, nodes_in_dc in nodes_by_dc_idx.items():
            ...
            if current_dc_config_count != len(nodes_in_dc):
                LOGGER.debug(f"Datacenter {dc_idx=} rack distribution: {dict(racks)}, config count: {current_dc_config_count}, {len(nodes_in_dc)=}")
                return False
        return True
        ...
        raise ValueError(f"Unknown capacity_errors_check_mode: {capacity_errors_check_mode}. Supported modes are: 'per-initial_config', 'disabled'.")
