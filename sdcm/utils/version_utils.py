...
FILE_REGEX_DICT = {
    ScyllaFileType.DEBIAN: [
        (re.compile(r"deb\s+\[arch=(?P<arch>[^\s]*).*?\].*\s(?P<url>http.*?)\s(?P<version_code_name>.*?)\s(?P<component>.*?)$"), "{url}/dists/{version_code_name}/{component}/binary-{arch}/Packages"),
    ],
    ScyllaFileType.YUM: [
        (re.compile(r"baseurl=(?P<url>http.*scylladb.com.*)"), "{url}" + REPOMD_XML_PATH),
        (re.compile(r"deb\s+(?P<url>http.*?)\s(?P<version_code_name>.*?)\s(?P<component>.*?)$"), "{url}/dists/{version_code_name}/{component}/binary-amd64/Packages.gz"),
    ],
}
...
class ComparableScyllaVersion:
    ...
    def parse(version_string: str):
        ...
        if match := SEMVER_REGEX.match(_scylla_version):
            return match.groups()
        raise ValueError(f"Cannot parse provided '{version_string}' scylla_version for the comparison. Transformed scylla_version: {_scylla_version}")
...
class ComparableScyllaOperatorVersion(ComparableScyllaVersion):
    ...
    def parse(version_string: str):
        ...
        if match := SEMVER_REGEX.match(_scylla_operator_version):
            return match.groups()
        raise ValueError(f"Cannot parse provided '{version_string}' scylla_operator_version for the comparison. Transformed scylla_operator_version: {_scylla_operator_version}")
...
def get_scylla_urls_from_repository(repo_details):
    ...
    ParallelObject(objects=urls, timeout=SCYLLA_URL_RESPONSE_TIMEOUT).run(func=lambda _url: get_url_content(url=_url, return_url_data=False))
    return urls
...
def get_branch_version_from_centos_repository(urls, full_version: bool = False):
    def get_version(url):
        ...
        if full_version:
            major_versions = [f"{package['version'][1]['ver']}-{package['version'][1]['rel']}" for package in parser.getList() if package["name"][0] in SUPPORTED_PACKAGES]
        else:
            major_versions = [package["version"][1]["ver"] for package in parser.getList() if package["name"][0] in SUPPORTED_PACKAGES]
        return set(major_versions)
...
def assume_version(params: dict[str], scylla_version: Optional[str] = None) -> str:
    ...
    if match := re.match(r"[\D\d]*-(\d+\.\d+)", scylla_version_source) or re.match(r"\D*(\d+\.\d+)", scylla_version_source):
...
def resolve_latest_repo_symlink(url: str) -> str:
    ...
    for timestamp, build in build_list:
        ...
        ScyllaRepoEvent(url=url, error=f"There is no a sibling directory which contains same repo file (ETag={latest_etag})").publish_or_dump(default_logger=LOGGER)
        return url

    if (timestamp, build) != build_list[0]:
        ScyllaRepoEvent(url=url, error=f"{url} doesn't point to the latest repo ({base}{build_list[0][1]}{rest})").publish_or_dump(default_logger=LOGGER)
...
class scylla_versions:
    ...
    def __call__(self, func):
        ...
        def inner(*args, **kwargs):
            ...
            raise MethodVersionNotFound("Method '{}' with version '{}' is not supported in '{}'!".format(func.__name__, scylla_version, cls_self.__class__.__name__))
    ...
    if dist_type in ("centos", "rocky", "rhel"):
        ...
        for repo_file in response["Contents"]:
            ...
            if filename.startswith("scylla-") and filename.endswith(".repo"):
                ...
                _S3_SCYLLA_REPOS_CACHE[(dist_type, dist_version)][version_prefix] = "https://s3.amazonaws.com/{bucket}/{path}".format(bucket=bucket, path=repo_file["Key"])
        ...
        for repo_file in response["Contents"]:
            ...
            if filename.startswith("scylla-") and (filename.endswith("-{}.list".format(dist_version)) or repo_regex.search(filename)):
                version_prefix = filename.replace("-{}.list".format(dist_version), "").replace(".list", "").split("-")[-1]
                _S3_SCYLLA_REPOS_CACHE[(dist_type, dist_version)][version_prefix] = "https://s3.amazonaws.com/{bucket}/{path}".format(bucket=bucket, path=repo_file["Key"])
...
def get_branched_repo(scylla_version: str, dist_type: Literal["centos", "ubuntu", "debian", "rocky"] = "centos", bucket: str = "downloads.scylladb.com") -> Optional[str]:
    """
    Get a repo/list of scylla, based on scylla version match
