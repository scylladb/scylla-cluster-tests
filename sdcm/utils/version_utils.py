...
class ComparableScyllaVersion:
    ...
    def parse(version_string: str):
        ...
        if match := SEMVER_REGEX.match(_scylla_version):
            return match.groups()
        raise ValueError(
            f"Cannot parse provided '{version_string}' scylla_version for the comparison. Transformed scylla_version: {_scylla_version}"
        )
...
class ComparableScyllaOperatorVersion(ComparableScyllaVersion):
    ...
    def parse(version_string: str):
        ...
        if match := SEMVER_REGEX.match(_scylla_operator_version):
            return match.groups()
        raise ValueError(
            f"Cannot parse provided '{version_string}' scylla_operator_version for the comparison. Transformed scylla_operator_version: {_scylla_operator_version}"
        )
