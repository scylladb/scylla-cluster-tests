    ...
    def _run_all_stress_cmds(self, stress_queue, params):
        ...
        for stress_cmd in stress_cmds:
            ...
            if stress_cmd.startswith("scylla-bench"):
                stress_queue.append(self.run_stress_thread(stress_cmd=stress_cmd, stats_aggregate_cmds=False, round_robin=self.params.get("round_robin")))
            else:
        ...
        for i, cs_profile in enumerate(cs_profiles):
            ...
            with open(cs_profile, encoding="utf-8") as file:
                ...
                for cmd in [line.lstrip("#").strip() for line in content if line.find("cassandra-stress") > 0]:
                    stress_cmd = (cmd.format(profile_dst)) if duration_per_cs_profile is None else (cmd.format(profile_dst, duration_per_cs_profile[i].strip()))
    ...
    def run_prepare_write_cmd(self):
        ...
        if not skip_optional_stage("prepare_write"):
            ...
            if prepare_verify_cmd:
                self._run_all_stress_cmds(verify_queue, params={"stress_cmd": prepare_verify_cmd, "keyspace_num": keyspace_num})
        ...
        def _set_credentials_to_cmd(cmd):
            if roles and "<sla credentials " in cmd:
                if "user=" in cmd:
                    ...
                    raise EnvironmentError(f"Stress command is defined wrong. Credentials already applied. Remove unnecessary and re-run the test. Command: {cmd}")

                index = re.search(r"<sla credentials (\d+)>", cmd)
                role_index = int(index.groups(0)[0]) if index else None
                if role_index is None:
                    # if stress command is not defined as expected, stop the tests and fix it. Then re-run
                    raise EnvironmentError(f"Stress command is defined wrong. Expected pattern '<credentials \\d>' was not found. Fix the command and re-run the test. Command: {cmd}")
