...
GitStatus = TypedDict("GitStatus", {"branch.oid": str, "branch.head": str, "branch.upstream": str | None, "upstream.url": str | None})
...
def get_git_status_info() -> GitStatus:
    try:
        ...
        #    # branch.ab +0 -0
        status_proc = subprocess.run(args=["git", "status", "-b", "--porcelain=v2", ".nothing"], check=True, capture_output=True)
        ...
        origin_proc = subprocess.run(args=["git", "config", "--get", f"remote.{remote}.url"], check=True, capture_output=True)
