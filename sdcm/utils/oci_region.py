...
# TODO: add pagination for the list API calls
...
class OciRegion:
    ...
    def _find_route_table(self, display_name):
        rts = self.network.list_route_tables(
            compartment_id=self.compartment_id, vcn_id=self.vcn.id, display_name=display_name
        ).data
        for rt in rts:
    ...
    def _find_security_list(self):
        lists = self.network.list_security_lists(
    ...
    def _find_internet_gateway(self):
        ...
        igws = self.network.list_internet_gateways(compartment_id=self.compartment_id, vcn_id=self.vcn.id).data
        for igw in igws:
    ...
    def _find_nat_gateway(self):
        """Find existing nat gateway for this VCN."""
        natgws = self.network.list_nat_gateways(compartment_id=self.compartment_id, vcn_id=self.vcn.id).data
        for ngw in natgws:
            if ngw.lifecycle_state == "AVAILABLE":
                return ngw
        return None
    ...
    def subnet(
        ...
        with KEY_BASED_LOCKS.get_lock(f"oci-subnet--i-{subnet_index}--public-{public}"):
            ...
            LOGGER.info("DEBUG: looking for subnet with name: %s", name)
            subnets = self.network.list_subnets(
                compartment_id=self.compartment_id, vcn_id=self.vcn.id, display_name=name
            ).data
            for subnet in subnets:
                LOGGER.info("DEBUG: current subnet name: %s", subnet.display_name)
                if subnet.lifecycle_state == "AVAILABLE":
                    LOGGER.info("DEBUG: choosen subnet name: %s", subnet.display_name)
                    _cache[(subnet_index, public)] = subnet
                    return subnet
        return None

    def create_subnet(self, ipv4_cidr, ipv6_cidr, subnet_index: Optional[int] = None, public: bool = False):
        name = self.subnet_name(subnet_index=subnet_index, public=public)
    ...
    def _get_home_region(self) -> str:
        ...
        try:
            ...
            region_subscriptions = self.identity.list_region_subscriptions(self._config["tenancy"]).data
