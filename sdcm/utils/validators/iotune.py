...
class IOTuneValidator:
    ...
    def _run_io_tune(self) -> IOProperties:
        ...
        if not preset_disk:
            ...
            TestFrameworkEvent(source="send_iotune_results_to_argus", message="Unable to continue - node should have io_properties.yaml, but it doesn't.", severity=Severity.ERROR).publish()
    ...
    def _prepare_results_table(self):
        ...
        if tested_mountpoint != preset_disk["mountpoint"]:
            ...
            TestFrameworkEvent(source=self.__class__.__name__, message=f"Disks differ - probably a mistake: {tested_mountpoint} vs {preset_disk['mountpoint']}", severity=Severity.ERROR).publish()
            raise IOTuneException(f"Disks differ - probably a mistake: {tested_mountpoint} vs {preset_disk['mountpoint']}")
