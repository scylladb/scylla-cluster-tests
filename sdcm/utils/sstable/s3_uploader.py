...
def upload_sstables_to_s3(node: CollectingNode | BaseNode, keyspace: str, test_id: str, tables: list | None = None, public: bool = True):
    ...
    try:
        ...
        s3_link = upload_remote_files_directly_to_s3(
            node.ssh_login_info, snapshot_paths, s3_bucket=S3Storage.bucket_name, s3_key=f"{test_id}/{snapshot_date}/sstables-{snapshot_date}-{node.name}-{keyspace}.tar.gz", max_size_gb=400, public_read_acl=public
        )
        if s3_link:
