...
from sdcm.utils.sstable.load_inventory import TestDataInventory, BIG_SSTABLE_COLUMN_1_DATA, COLUMN_1_DATA, MULTI_NODE_DATA, BIG_SSTABLE_MULTI_COLUMNS_DATA, MULTI_COLUMNS_DATA
...
class SstableLoadUtils:
    ...
    @staticmethod
    def calculate_columns_count_in_table(target_node, keyspace_name: str = "keyspace1", table_name: str = "standard1") -> int:
    ...
    @staticmethod
    def upload_sstables(node, test_data: TestDataInventory, keyspace_name: str = "keyspace1", table_name=None, create_schema: bool = False, is_cloud_cluster=False, **kwargs):
        key_store = KeyStore()
        ...
        if is_cloud_cluster:
            ...
            try:
                remote_get_file(LOCAL_CMD_RUNNER, test_data.sstable_url, local_temp_path, hash_expected=test_data.sstable_md5, retries=2, user_agent=creds["user_agent"])
            ...
            remote_get_file(node.remoter, test_data.sstable_url, test_data.sstable_file, hash_expected=test_data.sstable_md5, retries=2, user_agent=creds["user_agent"])
        ...
        if create_schema:
            with RemoteTemporaryFolder(node=node) as tmp_folder:
                ...
                SstableLoadUtils.create_table_for_load(node=node, schema_file_and_path=f"{tmp_folder.folder_name}/schema.cql", session=kwargs["session"])
    ...
    @classmethod
    def run_load_and_stream(cls, node, keyspace_name: str = "keyspace1", table_name: str = "standard1", start_timeout=60, end_timeout=600):
        """runs load and stream using API request and waits for it to finish"""
        with wait_for_log_lines(node, start_line_patterns=[cls.LOAD_AND_STREAM_RUN_EXPR], end_line_patterns=[cls.LOAD_AND_STREAM_DONE_EXPR.format(keyspace_name, table_name)], start_timeout=start_timeout, end_timeout=end_timeout):
    ...
    @staticmethod
    @timeout_decor(timeout=600, allowed_exceptions=(AssertionError,), message="Waiting for resharding completion message to appear in logs")
    def validate_resharding_after_refresh(node, system_log_follower):
        ...
        for line in resharding_logs:
            ...
            for one_file in re.findall(r"(/var/.*?),", line, re.IGNORECASE):
                ...
                assert "/upload/" in one_file, f"Loaded file was resharded not in 'upload' folder on the node {node.name}"

    @classmethod
    def get_load_test_data_inventory(cls, column_number: int, big_sstable: bool, load_and_stream: bool) -> List[TestDataInventory]:
        if column_number == 1:
    ...
    @classmethod
    def create_keyspace(cls, node, keyspace_name: str = "keyspace1", strategy: str = "NetworkTopologyStrategy", replication_factor: int = 1):
        node.run_cqlsh("CREATE KEYSPACE %s WITH replication = {'class': '%s', 'replication_factor': %s}" % (keyspace_name, strategy, replication_factor))
