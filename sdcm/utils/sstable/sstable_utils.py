...
class SstableUtils:
    ...
    def count_ttl_expired_tombstones(self):
        sstables = self.get_sstables()
        tombstones_num = 0
        for sstable in sstables:
            tombstones_num += self._count_ttl_expired_tombstones(sstable=sstable)
    ...
    def _count_ttl_expired_tombstones(self, sstable: str) -> int:
        """
        Counts the number of ttl-expired tombstones in a given SSTable.

        :param sstable: The SSTable file path.
        :return: The number of ttl-expired tombstones in the SSTable, or 0 if SSTable doesn't exist.
        """
        ...
        if result.ok:
            ...
            self.log.debug("Found %d TTL-expired tombstones in SSTable %s", num_tombstones, sstable)
            return num_tombstones
        else:
            self.log.debug("Failed to count TTL-expired tombstones in %s dump: %s", sstable, result.stderr)
            return 0
    ...
    def _are_tombstones_in_sstabledump(self, sstable: str, remote_json_path: str = REMOTE_SSTABLEDUMP_PATH) -> bool:
        ...
        check_tombstones_cmd = f'sudo grep -q deletion_time {remote_json_path}'
