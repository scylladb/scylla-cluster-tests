...
class SstableUtils:
    ...
    def set_db_node_if_unset_or_down(self):
        if not self.db_node:
            self.db_node = random.choice([node for node in self.db_cluster.data_nodes if node.db_up()])
        elif not self.db_node.db_up():
            self.db_node = random.choice([node for node in self.db_node.parent_cluster.data_nodes if node.db_up()])
    ...
    def get_table_repair_date(self) -> str | None:
        ...
        """
        self.set_db_node_if_unset_or_down()
