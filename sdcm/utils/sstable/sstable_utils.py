...
class SstableUtils:
    ...
    def check_that_sstables_are_encrypted(self, sstables=None, expected_bool_value: bool = True) -> list:
        ...
        for sstable in sstables:
            ...
            if sstables_res.stdout:
                ...
                if dump_cmd == "sstabledump":
                    ...
                    sstables_encrypted_mapping[sstable] = all("scylla_encryption_options" in metadata.get("extension_attributes", {}) for table, metadata in scylla_metadata.items())
            ...
            elif " file not found)" in sstables_res.stderr or "Cannot find file" in sstables_res.stderr or "No such file or directory" in sstables_res.stderr:
            ...
            #       > last={key: pk{00080000000000000008}, token: -6917704163689751025})
                ...
                self.log.warning("'%s' sstable cannot be loaded with 'first and last keys of summary are misordered' error. Not representative. Skipping it.", sstable)
        ...
        assert encryption_results.count(expected_bool_value) / len(encryption_results) >= encryption_success_part, (
            f"Sstables encryption check failed. Success part: '{encryption_success_part}'. Expected bool value: '{expected_bool_value}'. Encryption results: {encryption_results}"
        )
    ...
    def count_sstable_tombstones(self, sstable: str) -> int:
        ...
        result = self.db_node.remoter.run(f"sudo cat {self.REMOTE_SSTABLEDUMP_PATH}", verbose=False, ignore_status=False)

        if not result.ok:
            self.log.debug("Failed to retrieve SSTable dump data for %s: (%s, %s)", sstable, result.stdout, result.stderr)
            return 0

        try:
            dump_data = json.loads(result.stdout)
            num_tombstones = sum(1 for partition in dump_data.get("sstables", {}).get("anonymous", []) if "tombstone" in partition or partition.get("expired") is True)
    ...
    def get_table_repair_date(self) -> str | None:
        ...
        with self.db_cluster.cql_connection_patient(node=self.db_node, connect_timeout=300, user=self.user, password=self.password) as session:
            try:
                query = f"SELECT repair_time from system.repair_history WHERE keyspace_name = '{self.keyspace}' AND table_name = '{self.table}' ALLOW FILTERING;"
    ...
    def get_table_repair_date_and_delta_minutes(self) -> (datetime.datetime, int):
        ...
        self.log.debug("Found table-repair-date: %s, Ended %s minutes ago", table_repair_date, delta_repair_date_minutes)
    ...
    def _run_sstabledump(self, sstable: str, remote_json_path: str = REMOTE_SSTABLEDUMP_PATH) -> bool:
        ...
        dump_result = self.db_node.remoter.run(f'sudo bash -c "{dump_cmd} {sstable} 1>{remote_json_path}"', verbose=False, ignore_status=True)
    ...
    def get_compacted_tombstone_deletion_info(self, sstable: str) -> list:
        ...
        if not result.ok:
            self.log.warning("Failed to retrieve SSTable dump data for %s: (%s, %s)", sstable, result.stdout, result.stderr)
    ...
    def verify_post_repair_sstable_tombstones(self, table_repair_date: datetime.datetime, sstable: str):
        ...
        for entry in tombstones_deletion_info:
            ...
            self.log.debug("Checking tombstone delete date %s < table repair date: %s", tombstone_date, table_repair_date)

            if tombstone_date < table_repair_date:
                non_deleted_tombstones.append(f"(key: {key_info}, date: {tombstone_date.strftime('%Y-%m-%d %H:%M:%S')})")

        if non_deleted_tombstones:
            raise NonDeletedTombstonesFound(f"Found pre-repair time ({table_repair_date}) tombstones in a post-repair sstable ({sstable}): {non_deleted_tombstones}")
...
def _generate_sstable_dump_command(node, command: str, keyspace: str, table: str) -> str:
    ...
    return f"SCYLLA_CONF={scylla_conf_dir} {node.add_install_prefix('/usr/bin/scylla')} sstable {command} --keyspace {keyspace} --table {table} --sstables"
