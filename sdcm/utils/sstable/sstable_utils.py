...
class SstableUtils:
    ...
    def check_that_sstables_are_encrypted(
        self,
        sstables=None,  # pylint: disable=too-many-branches
    ...
    def get_compacted_tombstone_deletion_info(self, sstable: str) -> list:
        ...
        result = self.db_node.remoter.run(
            "sudo grep marked_deleted /tmp/sstabledump.json", verbose=False, ignore_status=True
        )
        if result.ok:
            tombstones_deletion_info = result.stdout.splitlines()
        else:
            self.log.warning(
                "Failed to find compacted tombstones in %s: (%s, %s)", sstable, result.stdout, result.stderr
            )
