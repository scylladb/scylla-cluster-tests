...
# Delimiter of the heredoc that writes the files below onto a node. Quoting it is what stops the
# writing shell from expanding the '$' of the script, so it cannot be made unique per call:
# << "SCTEOF_$(date +%s)" is a literal delimiter, and unquoting it to get the expansion would
# expand the body instead. The constraint is therefore on the content - no line of what is written
# may be the delimiter on its own - and test_azure_secondary_nics_script.py enforces it.
HEREDOC_DELIMITER = "SCTEOF"
...
for idx, interface in enumerate(interfaces):
    ...
    # field, so it is derived the same way as the IPv4 one: Azure reserves the first usable address
    # of every subnet prefix as the gateway.
