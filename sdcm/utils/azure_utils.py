...
class AzureService(metaclass=Singleton):
    ...
    def blob(self) -> BlobServiceClient:
        return BlobServiceClient(account_url=self.blob_account_url, credential=AzureNamedKeyCredential(name=self.blob_credentials["account"], key=self.blob_credentials["key"]))
            ...
            while True:
                try:
                    ...
                    if "RateLimiting" in str(e) and retry_count < max_retries:
                        ...
                        LOGGER.warning("Azure Resource Graph rate limiting encountered. Retrying in %d seconds (attempt %d/%d). Query: %s", delay, retry_count, max_retries, query)
                        time.sleep(delay)
                        continue
                    else:
...
def list_instances_azure(tags_dict: Optional[dict[str, str]] = None, running: bool = False, verbose: bool = False) -> list[VirtualMachine]:
    query_bits = [
        "Resources",
