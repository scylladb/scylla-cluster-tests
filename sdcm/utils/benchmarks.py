...
class ScyllaClusterBenchmarkManager(metaclass=Singleton):
    ...
    def _collect_benchmark_output(self):
        ...
        for runner in self._benchmark_runners:
            if runner.benchmark_results:
                results = {"test_id": self._test_id, "node_instance_type": runner.node_instance_type, "node_name": runner.node_name, **runner.benchmark_results}
    ...
    def _get_benchmark_results(self, instance_type: str = "") -> list[dict]:
        ...
        def search_results():
            results = self._es.search(index=ES_INDEX, q=query, filter_path=filter_path, size=1000)
    ...
    def _compare_results(self):
        ...
        for runner in self._benchmark_runners:
            ...
            try:
                ...
                self._comparison.update(self._check_results(node_name=runner.node_name, averages=averages, result=result, margins=Margins(sysbench_eps=0.03, cassandra_fio_read_bw=0.01, cassandra_fio_write_bw=0.01)))
            except Exception as exc:  # noqa: BLE001
                LOGGER.warning("Failed to generate comparable result for the following item:\n%s\nException:%s", runner.benchmark_results, exc)
                continue
    ...
    def _check_results(node_name: str, averages: Averages, result: ComparableResult, margins: Margins) -> dict:
        ...
        for item in result._fields:
            ...
            results[node_name][item] = {"value": result[item], "average": averages[item], "average_ratio": avg_ratio, "is_within_margin": avg_ratio > (1 - margins[item])}
        return results
    ...
    def _get_average_results(es_docs: list):
        ...
        for item in es_docs:
            try:
                results.append(
                    ComparableResult(sysbench_eps=item["sysbench_events_per_second"], cassandra_fio_read_bw=item["cassandra_fio_lcs_64k_read"]["read"]["bw"], cassandra_fio_write_bw=item["cassandra_fio_lcs_64k_write"]["write"]["bw"])
                )
            except Exception as exc:  # noqa: BLE001
        ...
        return Averages(sysbench_eps=sum(eps) / len(eps), cassandra_fio_read_bw=sum(read_bw) / len(read_bw), cassandra_fio_write_bw=sum(write_bw) / len(write_bw))
...
class ScyllaNodeBenchmarkRunner:
    ...
    def _install_ubuntu_prerequisites(self):
        ...
        try:
            ...
            LOGGER.warning("Failed to install Ubuntu prerequisites for the node benchmarking tools. Exception:\n%s", exc)
