...
class AWSPricing:
    ...
    def get_on_demand_instance_price(self, region_name: str, instance_type: str):
        ...
        assert response["PriceList"], "failed to get price for {instance_type} in {region_name}".format(region_name=region_name, instance_type=instance_type)
    ...
    def get_spot_instance_price(region_name, instance_type):
        ...
        result = client.describe_spot_price_history(InstanceTypes=[instance_type], ProductDescriptions=["Linux/UNIX (Amazon VPC)", "Linux/UNIX"], StartTime=datetime.now() - timedelta(hours=3), EndTime=datetime.now())
...
class AzurePricing:
    def get_instance_price(self, region, instance_type, state, lifecycle):
        if state == "running":
            ...
            try:
                if lifecycle == InstanceLifecycle.ON_DEMAND:
                    return [price["retailPrice"] for price in prices if "Spot" not in price["meterName"] and "Low" not in price["meterName"]][0]
                else:
    ...
    def _get_sku_prices(instance_type: str, region):
        resp = requests.get(f"https://prices.azure.com/api/retail/prices?$filter=serviceName eq 'Virtual Machines' and armSkuName eq '{instance_type}' and armRegionName eq '{region}' and priceType eq 'consumption'")
        if not resp.ok:
