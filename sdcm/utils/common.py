...
def wait_for_tasks(node: "BaseNode", module: str, timeout: int = 300, **kwargs):
    """
    ...
    :param kwargs: Key-value pairs to match against task attributes.

    Example:
        tasks = wait_for_tasks(node, "node_ops", state="done", type="bootstrap")
    """
    ...
    def _get_tasks():
        ...
        return [t for t in tasks if all(t.get(k) == v for k, v in kwargs.items())]
