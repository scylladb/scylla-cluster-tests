...
def get_scylla_gce_images_versions(
    ...
    if version and version != "all":
        ...
        if "rc" not in version and len(version.split(".")) < 3:
            ...
            filters += "')"
    if arch:
        filters += f"(architecture eq {arch.to_gcp_format()})"
...
def get_ami_images(branch: str, region: str, arch: VmArch) -> list:
    ...
    try:
        amis = get_branched_ami(scylla_version=branch, region_name=region, arch=arch.to_aws_format())
...
def get_ami_images_versioned(region_name: str, arch: VmArch, version: str) -> list[list[str]]:
    return [
        ...
        for ami in get_scylla_ami_versions(region_name=region_name, arch=arch.to_aws_format(), version=version)
    ]
