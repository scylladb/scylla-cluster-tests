...
# ``--api-key-rotation`` don't match. The value is either a double-quoted
# string (captured in full, so spaces inside quotes don't split the secret)
# or a bare non-whitespace token.
...
# Same quoting rule: capture a double-quoted value as a unit.
