...
def get_db_tables(session, keyspace_name, node, with_compact_storage=True):
    ...
    output = []
    for row in list(
        session.execute(f"select table_name from system_schema.tables where keyspace_name='{keyspace_name}'")
    ):
        try:
            ...
            LOGGER.error(
                "Failed to describe '%s.%s' table. Maybe the table has been deleted. Error: %s",
                keyspace_name,
                row.table_name,
                err.result.stderr,
            )
