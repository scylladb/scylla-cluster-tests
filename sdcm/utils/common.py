...
CREATE_TABLE_REGEX = re.compile(r"CREATE\s+TABLE\s+(?P<keyspace>[^\s.]+)\.(?P<table>[^\s(]+)\s*\([^)]+\)(?P<options>[^;]*)")
...
def remote_get_file(remoter, src, dst, hash_expected=None, retries=1, user_agent=None):
    ...
    while retries > 0 and _remote_get_hash(remoter, dst) != hash_expected:
        _remote_get_file(remoter, src, dst, user_agent)
        retries -= 1
    assert _remote_get_hash(remoter, dst) == hash_expected, f"Hash mismatch: src={src}, dst={dst}, expected_hash={hash_expected}"


def get_first_view_with_name_like(view_name_substr: str, session) -> tuple:
    query = f"select keyspace_name, view_name, base_table_name from system_schema.views where view_name like '%_{view_name_substr}' ALLOW FILTERING"
...
def get_views_of_base_table(keyspace_name: str, base_table_name: str, session) -> list:
    query = f"select view_name from system_schema.views where keyspace_name = '{keyspace_name}' and base_table_name = '{base_table_name}' ALLOW FILTERING"
    LOGGER.debug("Run query: %s", query)
    result = session.execute(query)
    views = []
    for row in result:
...
def get_entity_columns(keyspace_name: str, entity_name: str, session) -> list:
    query = f"select column_name, kind, type from system_schema.columns where keyspace_name = '{keyspace_name}' and table_name='{entity_name}'"
    LOGGER.debug("Run query: %s", query)
    result = session.execute(query)
    view_details = []
...
def generate_random_string(length):
    return random.choice(string.ascii_uppercase) + "".join(random.choice(string.ascii_uppercase + string.digits) for x in range(length - 1))
...
def verify_scylla_repo_file(content, is_rhel_like=True):
    ...
    if is_rhel_like:
        body_prefix = ["#", "[scylla", "name=", "baseurl=", "enabled=", "gpgcheck=", "type=", "skip_if_unavailable=", "gpgkey=", "repo_gpgcheck=", "enabled_metadata="]
    else:
        body_prefix = ["#", "deb"]
...
class S3Storage:
    ...
    def __init__(self, bucket=None):
        ...
        self.transfer_config = boto3.s3.transfer.TransferConfig(multipart_threshold=self.enable_multipart_threshold_size, multipart_chunksize=self.multipart_chunksize, num_download_attempts=self.num_download_attempts)
    ...
    def generate_url(self, file_path, dest_dir=""):
        ...
        return "https://{bucket_name}.s3.amazonaws.com/{dest_dir}/{file_name}".format(dest_dir=dest_dir, file_name=file_name, bucket_name=bucket_name)
    ...
    def set_public_access(self, key):
        ...
        grantees = {"Grantee": {"Type": "Group", "URI": "http://acs.amazonaws.com/groups/global/AllUsers"}, "Permission": "READ"}
    ...
    def download_file(self, link, dst_dir):
        ...
        try:
            ...
            self._bucket.download_file(Key=key_name, Filename=os.path.join(dst_dir, file_name), Config=self.transfer_config)
    ...
    for log_file in log_files:
        for log_type in log_types:
            if log_type in log_file:
                results.append({"file_path": log_file, "type": log_type, "link": create_proxy_argus_s3_url(log_file).format(test_id, log_file.split("/")[-1]), "date": convert_to_date(log_file.split("/")[1])})
                break
    ...
    def get_builder_docker_client(builder: Dict[str, str]) -> None:
        ...
        try:
            with warnings.catch_warnings():
                ...
                client = docker.DockerClient(base_url=f"ssh://{builder['user']}@{normalize_ipv6_url(builder['public_ip'])}:22")
            client.ping()
...
def list_resources_docker(tags_dict: Optional[dict] = None, builder_name: Optional[str] = None, running: bool = False, group_as_builder: bool = False, verbose: bool = False) -> Dict[str, Union[list, dict]]:
    log = LOGGER if verbose else Mock()
    filters = {}
...
def list_instances_aws(tags_dict=None, region_name=None, running=False, group_as_region=False, verbose=False, availability_zone=None):
    ...
    def get_instances(region):
        ...
        custom_filter = []
        if tags_dict:
            custom_filter = [{"Name": "tag:{}".format(key), "Values": value if isinstance(value, list) else [value]} for key, value in tags_dict.items()]
        response = client.describe_instances(Filters=custom_filter)
        instances[region] = [instance for reservation in response["Reservations"] for instance in reservation["Instances"]]
    ...
    for curr_region_name, per_region_instances in instances.items():
        if running:
            ...
            if pending_instances:
                ...
                try:
                    if verbose:
                        LOGGER.info(f"Waiting for {len(instance_ids)} pending instances in {curr_region_name} to become running")
                    waiter.wait(InstanceIds=instance_ids, WaiterConfig={"Delay": 15, "MaxAttempts": 40})
                    # Refresh instance data after waiting
                    response = client.describe_instances(InstanceIds=instance_ids)
                    updated_instances = [instance for reservation in response["Reservations"] for instance in reservation["Instances"]]
    ...
    if availability_zone is not None:
        ...
        for curr_region_name, per_region_instances in instances.items():
            instances[curr_region_name] = [i for i in per_region_instances if i["Placement"]["AvailabilityZone"] == curr_region_name + availability_zone]
...
def list_test_security_groups(tags_dict=None, region_name=None, group_as_region=False, verbose=False):
    ...
    def get_security_groups_ips(region):
        ...
        time.sleep(random.random())
        client: EC2Client = boto3.client("ec2", region_name=region)
        custom_filter = [{"Name": "tag:{}".format(key), "Values": value if isinstance(value, list) else [value]} for key, value in tags_dict.items() if key != "NodeType"]
    ...
    ParallelObject(aws_regions, timeout=100, num_workers=len(aws_regions)).run(get_security_groups_ips, ignore_exceptions=True)
...
def list_load_balancers_aws(tags_dict=None, regions=None, group_as_region=False, verbose=False):
    ...
    def get_load_balancers(region):
        ...
        tag_mappings = itertools.chain.from_iterable(page["ResourceTagMappingList"] for page in paginator.paginate(TagFilters=tag_filter, ResourceTypeFilters=["elasticloadbalancing:loadbalancer"]))
        load_balancers[region] = list(tag_mappings)
        if verbose:
            LOGGER.info("%s: done [%s/%s]", region, len(list(load_balancers.keys())), len(aws_regions))

    ParallelObject(aws_regions, timeout=100, num_workers=len(aws_regions)).run(get_load_balancers, ignore_exceptions=False)
...
def list_cloudformation_stacks_aws(tags_dict=None, regions=None, group_as_region=False, verbose=False):
    ...
    def get_stacks(region):
        ...
        tag_mappings = itertools.chain.from_iterable(page["ResourceTagMappingList"] for page in paginator.paginate(TagFilters=tag_filter, ResourceTypeFilters=["cloudformation:stack"]))
    ...
    if not group_as_region:
        cloudformation_stacks = list(itertools.chain(*list(cloudformation_stacks.values())))  # flatten the list of lists
...
def list_launch_templates_aws(tags_dict=None, regions=None, verbose=False):
    ...
    ParallelObject(aws_regions, timeout=120, num_workers=len(aws_regions)).run(get_launch_templates, ignore_exceptions=False)
...
def list_instances_gce(tags_dict: Optional[dict] = None, running: bool = False, verbose: bool = True) -> list[GceInstance]:
...
def list_clusters_eks(tags_dict: Optional[dict] = None, regions: list = None, verbose: bool = False) -> List[EksClusterForCleaner]:
    class EksCleaner:
...
def filter_k8s_clusters_by_tags(
    tags_dict: dict, clusters: list[Union["EksClusterForCleaner", "GkeClusterForCleaner"]]
) -> list[  # noqa: F821
...
def _get_ami_versions(region_name: str, arch: AwsArchType, version: str, version_tag_name: str, extra_filters: list | None = None, version_processor_fn: Callable | None = None) -> list[EC2Image]:
    ...
    images = []
    for client, owner in zip((ec2_resource, get_scylla_images_ec2_resource(region_name=region_name)), SCYLLA_AMI_OWNER_ID_LIST):
        filters = [
    ...
    images = [image for image in images if image.tags and "debug" not in {i["Key"]: i["Value"] for i in image.tags}.get("Name", "")]
    return images
...
def get_scylla_ami_versions(region_name: str, arch: AwsArchType = "x86_64", version: str = None) -> list[EC2Image]:
    ...
    return _get_ami_versions(region_name=region_name, arch=arch, version=version, version_tag_name="scylla_version", extra_filters=extra_filters, version_processor_fn=_process_enterprise_scylla_version)
...
def get_vector_store_ami_versions(region_name: str, arch: AwsArchType = "x86_64", version: str = None) -> list[EC2Image]:
    ...
    return _get_ami_versions(region_name=region_name, arch=arch, version=version, version_tag_name="vector-store-version")
...
def get_ami_images(branch: str, region: str, arch: AwsArchType) -> list:
    ...
    for ami in amis:
        ...
        rows.append(["AWS", ami.name, ami.image_id, ami.creation_date, tags.get("Name"), tags.get("build-id", tags.get("build_id", r"N\A"))[:6], tags.get("arch"), tags.get("ScyllaVersion"), ami.owner_id])
...
def convert_name_to_ami_if_needed(
    ...
    if len(param_values) == 1 and not param_values[0].startswith("ami-"):
        ...
        for region_name in region_names:
            ...
            for client, _ in zip((ec2_resource, get_scylla_images_ec2_resource(region_name=region_name)), SCYLLA_AMI_OWNER_ID_LIST):
                for tag_name in ("tag:Name", "name"):
                    if images := sorted(client.images.filter(Filters=[{"Name": tag_name, "Values": [param_values[0]]}]), key=lambda x: x.creation_date, reverse=True):
...
def get_ami_images_versioned(region_name: str, arch: AwsArchType, version: str) -> list[list[str]]:
    return [["AWS", ami.name, ami.image_id, ami.creation_date, get_ec2_image_name_tag(ami)] for ami in get_scylla_ami_versions(region_name=region_name, arch=arch, version=version)]


def find_equivalent_ami(ami_id: str, source_region: str, target_regions: list[str] | None = None, target_arch: AwsArchType | None = None) -> list[dict[str, str]]:
    """
    ...
    for ec2_resource in (boto3.resource("ec2", region_name=source_region), get_scylla_images_ec2_resource(region_name=source_region)):
        try:
    ...
    for region in search_regions:
        ...
        for client, owner in zip((target_ec2_resource, get_scylla_images_ec2_resource(region_name=region)), SCYLLA_AMI_OWNER_ID_LIST):
            filters = [
...
def get_gce_images_versioned(version: str = None) -> list[list[str]]:
    return [["GCE", image.name, image.self_link, image.creation_timestamp] for image in get_scylla_gce_images_versions(version=version)]
...
def get_gce_images(branch: str, arch: AwsArchType) -> list:
    ...
    for image in gce_images:
        rows.append(["GCE", image.name, image.self_link, image.creation_timestamp, image.labels.get("build-id") or image.name.rsplit("-build-", maxsplit=1)[-1], image.labels.get("arch"), image.labels.get("scylla_version")])
...
def get_branched_gce_images(scylla_version: str, project: str = SCYLLA_GCE_IMAGES_PROJECT, arch: AwsArchType = None) -> list[GceImage]:
    branch, build_id = scylla_version.split(":", 1)
...
def get_aws_builders(tags=None, running=True):
    ...
    for aws_builder in aws_builders:
        ...
        builders.append({"builder": {"public_ip": aws_builder.get("PublicIpAddress"), "name": builder_name, "user": "jenkins", "key_file": os.path.expanduser(ssh_key_path)}})
...
def get_gce_builders(tags=None, running=True):
    ...
    for gce_builder in gce_builders:
        builders.append({"builder": {"public_ip": gce_public_addresses(gce_builder)[0], "name": gce_builder.name, "user": "scylla-test", "key_file": os.path.expanduser(ssh_key_path)}})
...
def get_builder_by_test_id(test_id):
    ...
    def search_test_id_on_builder(builder):
        remoter = RemoteCmdRunnerBase.create_remoter(builder["public_ip"], user=builder["user"], key_file=builder["key_file"])

        LOGGER.info("Search on %s", builder["name"])
        result = remoter.run("find {where} -name test_id | xargs grep -rl {test_id}".format(where=base_path_on_builder, test_id=test_id), ignore_status=True, verbose=False)
...
class PageFetcher:
    ...
    def wait(self, seconds=10):
        ...
        def error_message(msg):
            return "{}. Requested: {}; retrieved: {}; empty retrieved {}".format(msg, self.requested_pages, self.retrieved_pages, self.retrieved_empty_pages)
...
def reach_enospc_on_node(target_node):
    ...
    wait.wait_for(func=approach_enospc, timeout=300, step=5, text="Wait for new ENOSPC error occurs in database", throw_exc=False)
...
def validate_if_scylla_load_high_enough(start_time, wait_cpu_utilization, prometheus_stats, event_severity=Severity.ERROR, instance=None):
    end_time = int(time.time())
    scylla_load = prometheus_stats.get_scylla_reactor_utilization(start_time=start_time, end_time=end_time, instance=instance)

    if scylla_load < wait_cpu_utilization:
        CpuNotHighEnoughEvent(message=f"Load {scylla_load} isn't high enough(expected at least {wait_cpu_utilization}). The test results may be not correct.", severity=event_severity).publish()
        return False
...
def describering_parsing(describering_output):
    ...
    for index, attribute in enumerate(heads):
        ...
        result["details"] = [_list2dic(attr_list, ["host", "datacenter", "rack"]) for attr_list in re.findall(r"EndpointDetails\(host:([\d\.,]+), datacenter:([^,]+), rack:([^\)]+)\),?", found_attributes[0][4])]
    return result
...
def list_placement_groups_aws(tags_dict=None, region_name=None, available=False, group_as_region=False, verbose=False):
    ...
    ParallelObject(aws_regions, timeout=100, num_workers=len(aws_regions)).run(get_placement_groups, ignore_exceptions=True)

    for curr_region_name, instances in placement_groups.items():
        if available:
            placement_groups[curr_region_name] = [i for i in placement_groups[curr_region_name] if i["State"] == "available"]
        else:
