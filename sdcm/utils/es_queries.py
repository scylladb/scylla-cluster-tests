...
class QueryFilter:
    ...
    def filter_test_details(self):
        test_details = 'test_details.job_name:"{}" '.format(self.test_doc["_source"]["test_details"]["job_name"].split("/")[0])
...
class PerformanceQueryFilter(QueryFilter):
    ...
    def build_filter_job_name(self):
        ...
        if {*self.extra_jobs_to_compare} != {full_job_name}:
            extra_jobs_filter = " ".join([f' OR test_details.job_name.keyword: "{job}"' for job in self.extra_jobs_to_compare])
...
class QueryFilterCS(QueryFilter):
    ...
    def test_details_params(self):
        return self._CMD + self._PRELOAD_CMD if self.test_doc["_source"]["test_details"].get(self._PRELOAD_CMD[0]) else self._CMD
        ...
        for cassandra_stress in self.test_details_params():
            for param in self.cs_params():
                if param == "rate threads":
                    test_details += r" AND test_details.{}.rate\ threads: {}".format(cassandra_stress, self.test_doc["_source"]["test_details"][cassandra_stress][param])
...
class QueryFilterScyllaBench(QueryFilter):
    _CMD = ("scylla-bench",)
    _PARAMS = ("mode", "workload", "partition-count", "clustering-row-count", "concurrency", "connection-count", "replication-factor", "duration")

    def test_cmd_details(self):
        test_details = ["AND test_details.{}.{}: {}".format(cmd, param, self.test_doc["_source"]["test_details"][cmd][param]) for param in self._PARAMS for cmd in self._CMD]
