        ...
        if self.validate_partitions and self.partitions_dict_before:
            ...
            if partitions_dict_after is not None:
                if not ignore_limit_rows_number and self.limit_rows_number:
                    missing_rows = {
                        key: val for key, val in partitions_dict_after.items() if val < self.limit_rows_number
                    }
                    if missing_rows:
                ...
                PartitionRowsValidationEvent(
                    message="Partition rows number is validated.", severity=Severity.NORMAL
                ).publish()
...
def fetch_all_rows(  # pylint: disable=too-many-arguments
    session,
    default_fetch_size,
    statement,
    retries: int = 4,
