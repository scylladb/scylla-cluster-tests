...
class PartitionsValidationAttributes:
    ...
    def __init__(
        self, tester, table_name: str, primary_key_column: str, limit_rows_number: int = 0, max_partitions_in_test_table: str | None = None, partition_range_with_data_validation: str | None = None, validate_partitions: bool = False
    ):
        """
    ...
    def _init_partition_range(self):
        if self.partition_range_with_data_validation:
            ...
            if self.max_partitions_in_test_table:
                self.non_validated_partitions = self.max_partitions_in_test_table - (self.partition_end_range - self.partition_start_range)
    ...
    def get_count_pk_rows_query(self, key: str, ignore_limit_rows_number: bool = False) -> str:
        ...
        count_pk_rows_cmd = f"select count(*) from {self.table_name} where {self.primary_key_column} = {key}{limit_query} using timeout 5m"
    ...
    def collect_partitions_info(self, ignore_limit_rows_number: bool = False) -> dict[int, int] | None:
        ...
        try:
            with self.db_cluster.cql_connection_patient(node=self.db_cluster.nodes[0], connect_timeout=600) as session:
                ...
                pk_list = sorted(get_partition_keys(ks_cf=self.table_name, session=session, pk_name=self.primary_key_column))
        except Exception as exc:  # noqa: BLE001
            TestFrameworkEvent(source=self.__class__.__name__, message=error_message.format(exc), severity=Severity.ERROR).publish()
            return None
        ...
        if self.partition_range_with_data_validation:
            ...
            pk_list = [partition for partition in pk_list if int(partition) in range(self.partition_start_range, self.partition_end_range)]
        save_into_file_name = self.PARTITIONS_ROWS_BEFORE if not self.partitions_rows_collected else self.PARTITIONS_ROWS_AFTER
        ...
        with open(partitions_stats_file, "a", encoding="utf-8") as stats_file:
            for i in pk_list:
                count_pk_rows_cmd = self.get_count_pk_rows_query(key=i, ignore_limit_rows_number=ignore_limit_rows_number)
                try:
                    with self.db_cluster.cql_connection_patient(node=self.db_cluster.nodes[0], connect_timeout=600) as session:
                        pk_rows_num_query_result = fetch_all_rows(session=session, default_fetch_size=3000, statement=count_pk_rows_cmd, retries=1, timeout=600, raise_on_exceeded=True, verbose=False)
    ...
    def validate_rows_per_partitions(self, ignore_limit_rows_number: bool = False):
        ...
        if not ignore_limit_rows_number and self.limit_rows_number:
            ...
            if missing_rows:
                PartitionRowsValidationEvent(message=f"Found missing rows for partitions: {missing_rows}", severity=Severity.CRITICAL).publish()
                return
            ...
            PartitionRowsValidationEvent(message=f"Row amount in partitions is not same before and after running of nemesis, difference(after-before): {diff}", severity=Severity.CRITICAL).publish()
            return
...
def get_table_clustering_order(ks_cf: str, ck_name: str, session) -> str:
    ...
    cmd = f"SELECT clustering_order from system_schema.columns WHERE keyspace_name = '{keyspace}' and table_name = '{table}' and column_name = '{ck_name}'"
...
def fetch_all_rows(session, default_fetch_size, statement, retries: int = 4, timeout: int = None, raise_on_exceeded: bool = False, verbose=True):
    """
    ******* Caution *******
