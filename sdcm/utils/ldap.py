...
class LdapContainerMixin:
    ...
    def ldap_container_run_args(self) -> dict:
        return dict(
            ...
            detach=True,
            ports={
                f"{LDAP_PORT}/tcp": None,
                f"{LDAP_SSL_PORT}/tcp": None,
            },
            environment=[f"LDAP_ORGANISATION={ORGANISATION}", f"LDAP_DOMAIN={LDAP_DOMAIN}", f"LDAP_ADMIN_PASSWORD={LDAP_PASSWORD}"],
        )
    ...
    def search_ldap_entry(search_base, search_filter):
        LdapContainerMixin.ldap_conn.search(search_base=search_base, search_filter=search_filter, attributes=ALL_ATTRIBUTES)
...
class LdapUtilsMixin:
    ...
    def ldap_server_ip(self) -> str:
        ...
        ldap_server_ip = "127.0.0.1" if self.test_config.IP_SSH_CONNECTIONS == "public" else self.test_config.LDAP_ADDRESS[0]
    ...
    def ldap_port(self) -> str:
        if self.params.get("ldap_server_type") == LdapServerType.MS_AD:
            return LDAP_PORT
        ldap_port = LDAP_SSH_TUNNEL_LOCAL_PORT if self.test_config.IP_SSH_CONNECTIONS == "public" else self.test_config.LDAP_ADDRESS[1]
        return ldap_port
    ...
    def _add_ldap_entry(self, ldap_entry: list):
        ...
        self.localhost.add_ldap_entry(ip=self.ldap_server_ip, ldap_port=self.ldap_port, user=username, password=LDAP_PASSWORD, ldap_entry=ldap_entry)
    ...
    def create_role_in_ldap(self, ldap_role_name: str, unique_members: list):
        ...
        role_entry = [f"cn={ldap_role_name},{LDAP_BASE_OBJECT}", ["groupOfUniqueNames", "simpleSecurityObject", "top"], {"uniqueMember": unique_members_list, "userPassword": LDAP_PASSWORD}]
    ...
    def wait_verify_user_no_permissions(self, username: str):
        ...
        try:
            with self.db_cluster.cql_connection_patient(node=self.db_cluster.nodes[0], user=username, password=LDAP_PASSWORD) as session:
                session.execute(
    ...
    def wait_verify_user_permissions(self, username: str, keyspace: str = "customer_ldap"):
        ...
        with self.db_cluster.cql_connection_patient(node=self.db_cluster.nodes[0], user=username, password=LDAP_PASSWORD) as session:
            ...
            session.execute(f""" CREATE TABLE IF NOT EXISTS {keyspace}.info (key varchar, c varchar, v varchar, PRIMARY KEY(key, c)) """)
    ...
    def add_user_in_ldap(self, username: str):
        user_entry = [f"uid={username},ou=Person,{LDAP_BASE_OBJECT}", ["uidObject", "organizationalPerson", "top"], {"userPassword": LDAP_PASSWORD, "sn": "PersonSn", "cn": "PersonCn"}]
        self._add_ldap_entry(ldap_entry=user_entry)

    def create_role_in_scylla(self, node, role_name: str, is_superuser: bool = True, is_login: bool = True):
