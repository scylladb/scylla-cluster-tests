...
class LongevityDataValidator:
    ...
    def __init__(self, longevity_self_object, user_profile_name, base_table_partition_keys, stress_cmds_part="prepare_write_cmd"):
        """
    ...
    def keyspace_name(self):
        if not self._keyspace_name:
            ...
            if not profiles:
                ...
                LOGGER.warning("Keyspace is not recognized. Data validation can't be performed")
    ...
    def view_names_for_updated_data(self):
        ...
        if not self._mvs_for_updated_data:
            ...
            self._mvs_for_updated_data = [view_name.replace(self.SUFFIX_FOR_VIEW_AFTER_UPDATE, "") for view_name in mvs_names]
    ...
    def view_names_after_updated_data(self):
        ...
        if not self._mvs_after_updated_data:
            self._mvs_after_updated_data = self.get_view_name_from_profile(self.SUFFIX_FOR_VIEW_AFTER_UPDATE, all_entries=True)
    ...
    def copy_immutable_expected_data(self):
        ...
        if self._validate_not_updated_data:
            ...
            LOGGER.debug("Copy expected data for immutable rows: %s -> %s", self.view_name_for_not_updated_data, self.expected_data_table_name)
            if not self.longevity_self_object.copy_view(
                node=self.longevity_self_object.db_cluster.nodes[0], src_keyspace=self.keyspace_name, src_view=self.view_name_for_not_updated_data, dest_keyspace=self.keyspace_name, dest_table=self.expected_data_table_name, copy_data=True
            ):
                self._validate_not_updated_data = False
                DataValidatorEvent.DataValidator(
                    severity=Severity.ERROR, error=f"Problem during copying expected data from {self.view_name_for_not_updated_data} to {self.expected_data_table_name}. Data validation of not updated rows won' be performed"
                ).publish()
    ...
    def copy_updated_expected_data(self):
        ...
        if self._validate_updated_data:
            if not self.view_names_for_updated_data:
                ...
                DataValidatorEvent.DataValidator(
                    severity=Severity.WARNING, message=f"Problem during copying expected data: view not found. View names for updated data: {self.view_names_for_updated_data}. Data validation of updated rows won' be performed"
                ).publish()
                return
            ...
            for src_view in self.view_names_for_updated_data:
                ...
                if not self.longevity_self_object.copy_view(
                    ...
                    DataValidatorEvent.DataValidator(severity=Severity.ERROR, error=f"Problem during copying expected data from {src_view} to {expected_data_table_name}. Data validation of updated rows won' be performed").publish()
                self._validate_updated_per_view.append(True)

    def save_count_rows_for_deletion(self):
        if not self.view_name_for_deletion_data:
            DataValidatorEvent.DataValidator(
                severity=Severity.WARNING, message=f"Problem during copying expected data: not found. View name for deletion data: {self.view_name_for_deletion_data}. Data validation of deleted rows won' be performed"
            ).publish()
            return
        ...
        with self.longevity_self_object.db_cluster.cql_connection_patient(self.longevity_self_object.db_cluster.nodes[0], keyspace=self.keyspace_name) as session:
            rows_before_deletion = fetch_all_rows(session=session, default_fetch_size=self.DEFAULT_FETCH_SIZE, statement=f"SELECT {pk_name} FROM {self.view_name_for_deletion_data}")
    ...
    def validate_range_not_expected_to_change(self, session, during_nemesis=False):
        ...
        if not (self._validate_not_updated_data and self.view_name_for_not_updated_data and self.expected_data_table_name):
            LOGGER.debug("Verify immutable rows can't be performed as expected data has not been saved. See error above in the sct.log")
            return

        if not during_nemesis:
            LOGGER.debug("Verify immutable rows")

        actual_result = fetch_all_rows(session=session, default_fetch_size=self.DEFAULT_FETCH_SIZE, statement=f"SELECT * FROM {self.view_name_for_not_updated_data}", verbose=not during_nemesis)
        if not actual_result:
            DataValidatorEvent.ImmutableRowsValidator(severity=Severity.WARNING, message=f"Can't validate immutable rows. Fetch all rows from {self.view_name_for_not_updated_data} failed. See error above in the sct.log").publish()
            return

        expected_result = fetch_all_rows(session=session, default_fetch_size=self.DEFAULT_FETCH_SIZE, statement=f"SELECT * FROM {self.expected_data_table_name}", verbose=not during_nemesis)
        if not expected_result:
            DataValidatorEvent.ImmutableRowsValidator(severity=Severity.WARNING, message=f"Can't validate immutable rows. Fetch all rows from {self.expected_data_table_name} failed. See error above in the sct.log").publish()
            return
        ...
        if len(actual_result) > len(expected_result):
            DataValidatorEvent.ImmutableRowsValidator(severity=Severity.WARNING, message=f"Actual dataset length more then expected ({len(actual_result)} > {len(expected_result)}). Issue #6181").publish()
        elif not during_nemesis:
            assert len(actual_result) == len(expected_result), "One or more rows are not as expected, suspected LWT wrong update. Actual dataset length: {}, Expected dataset length: {}".format(len(actual_result), len(expected_result))
            ...
            DataValidatorEvent.ImmutableRowsValidator(severity=Severity.NORMAL, message="Validation immutable rows finished successfully").publish()
        elif len(actual_result) < len(expected_result):
            DataValidatorEvent.ImmutableRowsValidator(
                severity=Severity.ERROR, error=f"Verify immutable rows. One or more rows not found as expected, suspected LWT wrong update. Actual dataset length: {len(actual_result)}, Expected dataset length: {len(expected_result)}"
            ).publish()
        else:
            LOGGER.debug("Verify immutable rows. Actual dataset length: %s, Expected dataset length: %s", len(actual_result), len(expected_result))
    ...
    def list_of_view_names_for_update_test(self):
        ...
        return list(
            zip(
                ...
                [self.set_expected_data_table_name(view) for view in self.view_names_for_updated_data],
                self._validate_updated_per_view,
            )
        )

    def fetch_data_for_validation_after_update(self, during_nemesis: bool, views_set: tuple, session) -> Optional[DataForValidation]:
        ...
        before_update_rows = fetch_all_rows(session=session, default_fetch_size=self.DEFAULT_FETCH_SIZE, statement=f"SELECT {partition_keys} FROM {views_set[0]}", verbose=not during_nemesis)
        if not before_update_rows:
            DataValidatorEvent.UpdatedRowsValidator(severity=Severity.WARNING, message=f"Can't validate updated rows. Fetch all rows from {views_set[0]} failed. See error above in the sct.log").publish()
            return None

        after_update_rows = fetch_all_rows(session=session, default_fetch_size=self.DEFAULT_FETCH_SIZE, statement=f"SELECT {partition_keys} FROM {views_set[1]}", verbose=not during_nemesis)
        if not after_update_rows:
            DataValidatorEvent.UpdatedRowsValidator(severity=Severity.WARNING, message=f"Can't validate updated rows. Fetch all rows from {views_set[1]} failed. See error above in the sct.log").publish()
            return None

        expected_rows = fetch_all_rows(session=session, default_fetch_size=self.DEFAULT_FETCH_SIZE, statement=f"SELECT {partition_keys} FROM {views_set[2]}", verbose=not during_nemesis)
        if not expected_rows:
            DataValidatorEvent.UpdatedRowsValidator(severity=Severity.WARNING, message=f"Can't validate updated row. Fetch all rows from {views_set[2]} failed. See error above in the sct.log").publish()
            return None

        return DataForValidation(views=views_set, actual_data=sorted(before_update_rows + after_update_rows), expected_data=sorted(expected_rows), before_update_rows=before_update_rows, after_update_rows=after_update_rows)
    ...
    def save_data_for_debugging(data_for_validation: DataForValidation) -> str:
        ...
        with open(os.path.join(logdir, f"{data_for_validation.views[0]}_{unique_index}_debug.json"), "w", encoding="utf8") as json_file:
            json.dump(sorted(data_for_validation.before_update_rows), json_file)
            LOGGER.info("before_update_rows json: %s", json_file.name)

        with open(os.path.join(logdir, f"{data_for_validation.views[1]}_{unique_index}_debug.json"), "w", encoding="utf8") as json_file:
            json.dump(sorted(data_for_validation.after_update_rows), json_file)
            LOGGER.info("after_update_rows json: %s", json_file.name)

        with open(os.path.join(logdir, f"{data_for_validation.views[2]}_{unique_index}_debug.json"), "w", encoding="utf8") as json_file:
            json.dump(sorted(data_for_validation.expected_data), json_file)
            LOGGER.info("expected_rows json: %s", json_file.name)

        with open(os.path.join(logdir, f"{data_for_validation.views[0]}_{unique_index}_actual_data_debug.json"), "w", encoding="utf8") as json_file:
    ...
    def analyze_updated_data_and_save_in_file(self, data_for_validation: DataForValidation, session, logdir: str):
        ...
        if not self.base_table_name:
            DataValidatorEvent.UpdatedRowsValidator(severity=Severity.ERROR, error="Analyzing of updated rows failed. Failed to get base table name from profile").publish()
            return
        ...
        for i, diff_row in enumerate(difference_set):
            ...
            columns_for_validation = ", ".join(self.get_entity_columns(entity_name=data_for_validation.views[0], session=session))
            query = f"select %s from %s where {' and '.join(missed_row_as_str)}"
            ...
            row_data.update({"source_all_columns": list(session.execute(query % (columns_for_validation, self.base_table_name)))})

            if row_data["before"] or row_data["after"]:
                message = (
                    f"Row {diff_row} is found in the "
                ...
                error = (
                    f"Row {diff_row} is expected to be find in whether before (the row was not updated) or "
        ...
        if result:
            with open(os.path.join(logdir, f"{data_for_validation.views[0]}_analyze_result_debug.json"), "w", encoding="utf8") as json_file:
                try:
            ...
            DataValidatorEvent.UpdatedRowsValidator(
                severity=Severity.CRITICAL, error=f"Validation updated rows failed. View {data_for_validation.views[0]}.Failed data has been analyzed and saved with json format. Find the result files in sct-runner archive"
            ).publish()
    ...
    def validate_range_expected_to_change(self, session, during_nemesis=False):
        ...
        if not (self._validate_updated_data and self.view_names_for_updated_data):
            LOGGER.debug("Verify updated rows can't be performed as expected data has not been saved. See error above in the sct.log")
            return
        ...
        for views_set in views_list:
            ...
            if not views_set[3]:
                DataValidatorEvent.UpdatedRowsValidator(severity=Severity.WARNING, message=f"Can't start validation for {views_set[0]}. Copying expected data failed. See error above in the sct.log").publish()
                continue

            data_for_validation = self.fetch_data_for_validation_after_update(during_nemesis=during_nemesis, views_set=views_set, session=session)
            ...
            if len(data_for_validation.actual_data) > len(data_for_validation.expected_data):
                DataValidatorEvent.UpdatedRowsValidator(
                    severity=Severity.WARNING, message=f"View {views_set[0]}. Actual dataset length {len(data_for_validation.actual_data)} more then expected dataset length: {len(data_for_validation.expected_data)}. Issue #6181"
                ).publish()
                continue

            if during_nemesis:
                LOGGER.debug("Validation updated rows.  View %s. Actual dataset length %s, Expected dataset length: %s.", data_for_validation.views[0], len(data_for_validation.actual_data), len(data_for_validation.expected_data))
                continue

            if (len(data_for_validation.actual_data) != len(data_for_validation.expected_data)) or data_for_validation.actual_data != data_for_validation.expected_data:
                LOGGER.debug(
                    "%s. Rows amount:\n  before update: %s\n  after update: %s\n  expected: %s\n actual: %s",
                    data_for_validation.views[0],
                    len(data_for_validation.before_update_rows),
                    len(data_for_validation.after_update_rows),
                ...
                self.analyze_updated_data_and_save_in_file(data_for_validation=data_for_validation, session=session, logdir=logdir)
            else:
                DataValidatorEvent.UpdatedRowsValidator(severity=Severity.NORMAL, message=f"Validation updated rows finished successfully. View {data_for_validation.views[0]}").publish()
    ...
    def validate_deleted_rows(self, session, during_nemesis=False):
        ...
        actual_result = fetch_all_rows(session=session, default_fetch_size=self.DEFAULT_FETCH_SIZE, statement=f"SELECT {pk_name} FROM {self.view_name_for_deletion_data}", verbose=not during_nemesis)
        if actual_result is None:
            DataValidatorEvent.DeletedRowsValidator(severity=Severity.ERROR, error=f"Can't validate deleted rows. Fetch all rows from {self.view_name_for_deletion_data} failed. See error above in the sct.log").publish()
            return

        if len(actual_result) < self.rows_before_deletion:
            if not during_nemesis:
                # raise info event in the end of test only
                DataValidatorEvent.DeletedRowsValidator(severity=Severity.NORMAL, message="Validation deleted rows finished successfully").publish()
            else:
                LOGGER.debug("Validation deleted rows finished successfully")
        elif len(actual_result) == self.rows_before_deletion:
            DataValidatorEvent.DeletedRowsValidator(severity=Severity.WARNING, message="Rows were not deleted. Maybe need to increase dataset for delete.").publish()
        else:
            LOGGER.warning("Deleted row were not found. May be issue #6181. Actual dataset length: {}, Expected dataset length: {}".format(len(actual_result), self.rows_before_deletion))
