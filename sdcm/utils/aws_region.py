...
class AwsRegion:
    ...
    def sct_internet_gateway(self) -> EC2ServiceResource.InternetGateway:
        ...
        assert len(existing_igws) == 1, (
            f"More than 1 Internet Gateway with {self.SCT_INTERNET_GATEWAY_NAME} found in {self.region_name}: {existing_igws}!"
        )
    ...
    def sct_security_group(self) -> EC2ServiceResource.SecurityGroup:
        ...
        assert len(existing_sgs) == 1, (
            f"More than 1 Security group with {self.SCT_SECURITY_GROUP_NAME} found in {self.region_name}: {existing_sgs}!"
        )
