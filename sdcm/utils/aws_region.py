...
class AwsRegion:
    ...
    def sct_vpc(self) -> EC2ServiceResource.Vpc:
        ...
        if len(existing_vpcs) == 0:
            return None
        assert len(existing_vpcs) == 1, f"More than 1 VPC with {self.SCT_VPC_NAME} found in {self.region_name}: {existing_vpcs}"
    ...
    def az_subnet_name(self, region_az, subnet_index):
        ...
        return self.SCT_SUBNET_NAME.format(availability_zone=region_az, subnet_index="" if not subnet_index else f"-{subnet_index}")
    ...
    def sct_subnet(self, region_az, subnet_index: int = None) -> EC2ServiceResource.Subnet:
        ...
        def wait_for_subnet_available():
            assert existing_subnets[0]["State"] == "available", f"State of {subnet_name} is not available. Fix it (delete or make it available) and re-run"
        ...
        if len(existing_subnets) == 0:
            return None
        assert len(existing_subnets) == 1, f"More than 1 Subnet with {subnet_name} found in {self.region_name}: {existing_subnets}!"
    ...
    def create_sct_subnet(self, region_az, ipv4_cidr, ipv6_cidr, subnet_index: int = None):
        ...
        result = self.client.create_subnet(CidrBlock=str(ipv4_cidr), Ipv6CidrBlock=str(ipv6_cidr), VpcId=self.sct_vpc.vpc_id, AvailabilityZone=region_az)
    ...
    def create_sct_subnets(self):
        ...
        for range_index in range(self.SCT_SUBNET_PER_AZ):
            for az_name in self.availability_zones:
                self.create_sct_subnet(region_az=az_name, ipv4_cidr=next(ipv4_cidrs_iter), ipv6_cidr=next(ipv6_cidrs_iter), subnet_index=range_index)
                self.create_sct_subnet_route_table(subnet_name=self.az_subnet_name(region_az=az_name, subnet_index=range_index), index=range_index)
    ...
    def associate_subnet_with_route_table(self, region_az: str, index: int = None):
        ...
        response = self.client.describe_route_tables(Filters=[{"Name": "association.subnet-id", "Values": [subnet.subnet_id]}])
        ...
        if not (associated_route_table := response["RouteTables"]):
            ...
            LOGGER.info("Subnet '%s' has been associated with '%s' route table.", subnet.subnet_id, sct_route_table.route_table_id)
            return
        ...
        if associated_route_table_id != sct_route_table.route_table_id:
            self.client.replace_route_table_association(AssociationId=associated_route_table_id, RouteTableId=sct_route_table.route_table_id)
            LOGGER.info("Subnet '%s' has been associated with '%s' route table.", subnet.subnet_id, sct_route_table.route_table_id)
        # If subnet is associated with appropriate route table
        else:
            LOGGER.info("Subnet '%s' is already associated with '%s' route table.", subnet.subnet_id, sct_route_table.route_table_id)

    @property
    def sct_internet_gateway(self) -> EC2ServiceResource.InternetGateway:
        igws = self.client.describe_internet_gateways(Filters=[{"Name": "tag:Name", "Values": [self.SCT_INTERNET_GATEWAY_NAME]}])
        LOGGER.debug("Found Internet gateways: %s", igws)
        existing_igws = igws.get("InternetGateways", [])
        if len(existing_igws) == 0:
            return None
        assert len(existing_igws) == 1, f"More than 1 Internet Gateway with {self.SCT_INTERNET_GATEWAY_NAME} found in {self.region_name}: {existing_igws}!"
    ...
    def create_sct_internet_gateway(self):
        ...
        if self.sct_internet_gateway:
            LOGGER.warning("Internet Gateway '%s' already exists! Id: '%s'.", self.SCT_INTERNET_GATEWAY_NAME, self.sct_internet_gateway.internet_gateway_id)
        else:
            ...
            LOGGER.info("'%s' with id '%s' created. Attaching to '%s'", self.SCT_INTERNET_GATEWAY_NAME, igw_id, self.sct_vpc.vpc_id)
    ...
    def sct_route_table(self, index: int = None) -> EC2ServiceResource.RouteTable:
        ...
        route_tables = self.client.describe_route_tables(Filters=[{"Name": "tag:Name", "Values": [sct_route_table_name]}])
        LOGGER.debug("Found Route Tables: %s", route_tables)
        existing_rts = route_tables.get("RouteTables", [])
        if len(existing_rts) == 0:
            return None
        assert len(existing_rts) == 1, f"More than 1 Route Table with {sct_route_table_name} found in {self.region_name}: {existing_rts}!"
    ...
    def sct_route_tables(self) -> list:
        ...
        route_tables.update(rt for index in range(self.SCT_SUBNET_PER_AZ) if (rt := self.sct_route_table(index=index)))
    ...
    def create_sct_subnet_route_table(self, subnet_name: str, index: int = None):
        ...
        if sct_route_table := self.sct_route_table(index=index):
            LOGGER.warning("Route Table '%s' already exists! Id: '%s'.", subnet_route_table, sct_route_table.route_table_id)
            return
        ...
        route_table.create_route(DestinationCidrBlock="0.0.0.0/0", GatewayId=self.sct_internet_gateway.internet_gateway_id)
        route_table.create_route(DestinationIpv6CidrBlock="::/0", GatewayId=self.sct_internet_gateway.internet_gateway_id)
    ...
    def configure_sct_route_table(self, index: int = None):
        ...
        if sct_route_table := self.sct_route_table(index=index):
            LOGGER.warning("Route Table '%s' already exists! Id: '%s'.", self.sct_route_table_name(index=index), sct_route_table.route_table_id)
        else:
            route_tables = list(self.sct_vpc.route_tables.all())
            assert len(route_tables) == 1, f"Only one main route table should exist for {self.SCT_VPC_NAME}. Found {len(route_tables)}!"
    ...
    def sct_security_group(self) -> EC2ServiceResource.SecurityGroup:
        security_groups = self.client.describe_security_groups(Filters=[{"Name": "tag:Name", "Values": [self.SCT_SECURITY_GROUP_NAME]}])
        LOGGER.debug("Found Security Groups: %s", security_groups)
        existing_sgs = security_groups.get("SecurityGroups", [])
        if len(existing_sgs) == 0:
            return None
        assert len(existing_sgs) == 1, f"More than 1 Security group with {self.SCT_SECURITY_GROUP_NAME} found in {self.region_name}: {existing_sgs}!"
    ...
    def create_sct_security_group(self):
        ...
        if self.sct_security_group:
            LOGGER.warning("Security Group '%s' already exists! Id: '%s'.", self.SCT_SECURITY_GROUP_NAME, self.sct_security_group.group_id)
        else:
            result = self.client.create_security_group(Description="Security group that is used by SCT", GroupName=self.SCT_SECURITY_GROUP_NAME, VpcId=self.sct_vpc.vpc_id)
            ...
            security_group.authorize_ingress(
                IpPermissions=[
                    {"IpProtocol": "-1", "UserIdGroupPairs": [{"Description": "Allow ALL traffic inside the Security group", "GroupId": sg_id, "UserId": security_group.owner_id}]},
                ]
            )
    ...
    def get_sct_test_security_group(self, test_id) -> EC2ServiceResource.SecurityGroup:
        ...
        security_groups = self.client.describe_security_groups(Filters=[{"Name": "tag:Name", "Values": [name]}, {"Name": "tag:TestId", "Values": [test_id]}])
        LOGGER.debug("Found Security Groups: %s", security_groups)
        existing_sgs = security_groups.get("SecurityGroups", [])
        if len(existing_sgs) == 0:
            return None
        assert len(existing_sgs) == 1, f"More than 1 Security group with {name} found in {self.region_name}: {existing_sgs}!"
        return self.resource.SecurityGroup(existing_sgs[0]["GroupId"])
    ...
    def provide_sct_test_security_group(self, test_id: str):
        ...
        if security_group := self.get_sct_test_security_group(test_id=test_id):
            ...
            result = self.client.create_security_group(Description=f"Security group that is used by test {test_id}", GroupName=name, VpcId=self.sct_vpc.vpc_id)
            sg_id = result["GroupId"]
            security_group = self.resource.SecurityGroup(sg_id)
            security_group.create_tags(Tags=[{"Key": "Name", "Value": name}, {"Key": "RunByUser", "Value": get_username()}, {"Key": "CreatedBy", "Value": "SCT"}, {"Key": "TestId", "Value": test_id}])
            ...
            security_group.authorize_ingress(
                IpPermissions=[
                    ...
                    {"FromPort": 3000, "ToPort": 3000, "IpProtocol": "tcp", "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Grafana for ALL"}], "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow Grafana for ALL"}]},
                    {"FromPort": 9042, "ToPort": 9042, "IpProtocol": "tcp", "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow CQL for ALL"}], "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow CQL for ALL"}]},
                    {
                        "FromPort": 19042,
                        "ToPort": 19042,
                        "IpProtocol": "tcp",
                    ...
                    {"FromPort": 9142, "ToPort": 9142, "IpProtocol": "tcp", "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow SSL CQL for ALL"}], "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow SSL CQL for ALL"}]},
                    {
                        "FromPort": 19142,
                        "ToPort": 19142,
                        "IpProtocol": "tcp",
                    ...
                    {"FromPort": 8080, "ToPort": 8080, "IpProtocol": "tcp", "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Alternator for ALL"}], "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow Alternator for ALL"}]},
                    {"FromPort": 9090, "ToPort": 9090, "IpProtocol": "tcp", "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Prometheus for ALL"}], "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow  Prometheus for ALL"}]},
                    {
                        "FromPort": 9093,
                        "ToPort": 9093,
                        "IpProtocol": "tcp",
                    ...
                    },
                    {
                        ...
                        "ToPort": 7000,
                        "IpProtocol": "tcp",
                        "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Inter-node communication (RPC) for ALL"}],
                        "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow Inter-node communication (RPC) for ALL"}],
                    },
                    {
                        "FromPort": 7001,
                        "ToPort": 7001,
                        "IpProtocol": "tcp",
                        "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow SSL inter-node communication (RPC) for ALL"}],
                        "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow SSL inter-node communication (RPC) for ALL"}],
                    },
                    {
                        "FromPort": 7199,
                        "ToPort": 7199,
                    ...
                    },
                    {
                        ...
                        "ToPort": 10001,
                        "IpProtocol": "tcp",
                        "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Scylla Manager Agent REST API  for ALL"}],
                        "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow Scylla Manager Agent REST API for ALL"}],
                    },
                    {
                        "FromPort": 56090,
                        "ToPort": 56090,
                        "IpProtocol": "tcp",
                        "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Scylla Manager Agent version 2.1 Prometheus API for ALL"}],
                        "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow Scylla Manager Agent version 2.1 Prometheus API for ALL"}],
                    },
                    {
                        "FromPort": 15000,
                        "ToPort": 15000,
                    ...
                    {
                        "IpProtocol": "-1",
                        "IpRanges": [{"CidrIp": "172.0.0.0/11", "Description": "Allow traffic from Scylla Cloud lab while VPC peering for ALL"}],
                    },
                    {
                        "FromPort": 5080,
                        "ToPort": 5080,
                    ...
                    },
                    {
                        ...
                        "ToPort": 5090,
                        "IpProtocol": "tcp",
                        "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Scylla Manager Agent Prometheus API for ALL"}],
                        "Ipv6Ranges": [{"CidrIpv6": "::/0", "Description": "Allow Scylla Manager Agent Prometheus API for ALL"}],
                    },
                    {
                        "FromPort": 5112,
                        "ToPort": 5112,
                        "IpProtocol": "tcp",
                        "IpRanges": [{"CidrIp": "0.0.0.0/0", "Description": "Allow Scylla Manager pprof Debug For ALL"}],
    ...
    def sct_ssh_security_group(self) -> EC2ServiceResource.SecurityGroup:
        security_groups = self.client.describe_security_groups(Filters=[{"Name": "tag:Name", "Values": [self.SCT_SSH_GROUP_NAME]}])
        existing_sgs = security_groups.get("SecurityGroups", [])
        if len(existing_sgs) == 0:
            return None
        assert len(existing_sgs) == 1, f"More than 1 Security group with {self.SCT_SSH_GROUP_NAME} found in {self.region_name}: {existing_sgs}!"
        return self.resource.SecurityGroup(existing_sgs[0]["GroupId"])
    ...
    def create_sct_ssh_security_group(self):
        ...
        if self.sct_ssh_security_group:
            LOGGER.warning("Security Group '%s' already exists! Id: '%s'.", self.SCT_SSH_GROUP_NAME, self.sct_ssh_security_group.group_id)
        else:
            result = self.client.create_security_group(Description="Security group for builders that is used by SCT", GroupName=self.SCT_SSH_GROUP_NAME, VpcId=self.sct_vpc.vpc_id)
            sg_id = result["GroupId"]
            security_group = self.resource.SecurityGroup(sg_id)
    ...
    def sct_keypair(self):
        ...
        assert len(existing_key_pairs) == 1, f"More than 1 Key Pair with {self.SCT_KEY_PAIR_NAME} found in {self.region_name}: {existing_key_pairs}!"
