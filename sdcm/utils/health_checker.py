...
def check_nulls_in_peers(gossip_info, peers_details, current_node) -> HealthEventsGenerator:
    ...
    for node, node_info in peers_details.items():
        ...
        is_target = current_node.print_node_running_nemesis(node.ip_address)
        message = f"Current node {current_node}. Found nulls in system.peers for node {node}{is_target} with status {gossip_info.get(node, {}).get('status', 'n/a')} : {node_info}"
...
def check_node_status_in_gossip_and_nodetool_status(gossip_info, nodes_status, current_node) -> HealthEventsGenerator:
    if not nodes_status:
        LOGGER.warning("Node status info is not available. Search for the warning above. Verify node status can't be performed")
        return
        ...
        if node not in nodes_status:
            if node_info["status"] not in current_node.GOSSIP_STATUSES_FILTER_OUT:
                ...
                yield ClusterHealthValidatorEvent.NodeStatus(
                    severity=Severity.ERROR,
                    node=current_node.name,
                    error=f"Current node {current_node}. The node {node}{is_target} exists in the gossip but doesn't exist in the nodetool.status",
                )
            continue

        if (node_info["status"] == "NORMAL" and nodes_status[node]["status"] != "UN") or (node_info["status"] != "NORMAL" and nodes_status[node]["status"] == "UN"):
            LOGGER.debug("Gossip info: %s\nnodetool.status info: %s", gossip_info, nodes_status)
            yield ClusterHealthValidatorEvent.NodeStatus(
                severity=Severity.ERROR,
                node=current_node.name,
                error=f"Current node {current_node}. Wrong node status. Node {node}{is_target} status in nodetool.status is {nodes_status[node]['status']}, but status in gossip {node_info['status']}",
            )
    ...
    for node in not_in_gossip:
        if nodes_status[node]["status"] == "UN":
            ...
            yield ClusterHealthValidatorEvent.NodeSchemaVersion(
                severity=Severity.ERROR,
                node=current_node.name,
                error=f"Current node {current_node}. Node {node}{is_target} exists in the nodetool.status but missed in gossip.",
            )
...
def check_schema_version(gossip_info, peers_details, nodes_status, current_node) -> HealthEventsGenerator:
    ...
    if not peers_details:
        LOGGER.warning("SYSTEM.PEERS info is not availble. Search for the warning above. Verify schema version can't be performed")
        return

    if not gossip_info:
        LOGGER.warning("Gossip info is not availble. Search for the warning above. Verify schema version can't be performed")
        return
    ...
    for node, node_info in gossip_info.items():
        ...
        if node not in peers_details.keys():
            ...
            yield ClusterHealthValidatorEvent.NodeSchemaVersion(
                severity=Severity.ERROR,
                node=current_node.name,
                error=f"Current node {current_node}. Node {node}{is_target} exists in the gossip but missed in SYSTEM.PEERS.",
            )
            continue
        ...
        if node_info["schema"] != str(peers_details[node]["schema_version"]):
            ...
            yield ClusterHealthValidatorEvent.NodeSchemaVersion(
                severity=Severity.ERROR,
                node=current_node.name,
                error=f"Current node {current_node}. Wrong Schema version. Node {node}{is_target} schema version in SYSTEM.PEERS is {peers_details[node]['schema_version']}, but schema version in gossip {node_info['schema']}",
            )
    ...
    if not_in_gossip:
        ...
        yield ClusterHealthValidatorEvent.NodeSchemaVersion(
            severity=Severity.ERROR,
            node=current_node.name,
            error=f"Current node {current_node}. Nodes {','.join(node.ip_address for node in not_in_gossip)} exists in the SYSTEM.PEERS but missed in gossip.",
        )

    # Validate that same schema on all nodes in the gossip
    schema_version_on_all_nodes = [values["schema"] for values in gossip_info.values() if values["status"] not in current_node.GOSSIP_STATUSES_FILTER_OUT]

    if len(set(schema_version_on_all_nodes)) > 1:
        LOGGER.debug(debug_message)
        gossip_info_str = "\n".join(f"{node}: {schema_version['schema']}" for node, schema_version in gossip_info.items())
        yield ClusterHealthValidatorEvent.NodeSchemaVersion(
            severity=Severity.WARNING,
            node=current_node.name,
            message=f"Current node {current_node}. Schema version is not same on all nodes in gossip info: {gossip_info_str}",
        )

    # Validate that same schema on all nodes in the SYSTEM.PEERS
    schema_version_on_all_nodes = [str(values["schema_version"]) for node, values in peers_details.items() if node in gossip_info and gossip_info[node]["status"] not in current_node.GOSSIP_STATUSES_FILTER_OUT]

    if len(set(schema_version_on_all_nodes)) > 1:
        LOGGER.debug(debug_message)
        peers_info_str = "\n".join(f"{node}: {schema_version['schema_version']}" for node, schema_version in peers_details.items())
        yield ClusterHealthValidatorEvent.NodeSchemaVersion(
            severity=Severity.WARNING,
            node=current_node.name,
            message=f"Current node {current_node}. Schema version is not same on all nodes in SYSTEM.PEERS info: {peers_info_str}",
        )
...
def check_group0_tokenring_consistency(group0_members: list[Group0Member], tokenring_members: list[TokenRingMember], current_node) -> HealthEventsGenerator:
