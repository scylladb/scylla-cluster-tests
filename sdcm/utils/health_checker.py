...
def check_nulls_in_peers(gossip_info, peers_details, current_node) -> HealthEventsGenerator:
    ...
    for node, node_info in peers_details.items():
        ...
        is_target = current_node.print_node_running_nemesis(node.ip_address)
        message = f"Current node {current_node}. Found nulls in system.peers for node {node}{is_target} with status {gossip_info.get(node, {}).get('status', 'n/a')} : {peers_details[node]}"
...
def check_group0_tokenring_consistency(
    group0_members: list[dict[str, str]], tokenring_members: list[dict[str, str]], current_node
