...
class KubernetesOps:
        ...
        for current_config_path in sorted(config_paths):
            ...
            with NamedTemporaryFile(mode="tw") as temp_file:
                ...
                def run_kubectl(file_name):
                    try:
                        cls.kubectl(
                            kluster, "apply", server_side_arg, "-f", file_name, namespace=namespace, timeout=timeout
                        )
    ...
    def expose_pod_ports(
        cls, kluster, pod_name, ports, labels=None, selector=None, namespace=None, timeout=KUBECTL_TIMEOUT
    ):
        command = [
            "expose pod",
            pod_name,
    ...
    @staticmethod
    def wait_for_pods_with_condition(
        kluster,
        condition: str,
    ...
    def wait_for_pods_readiness(
        ...
        KubernetesOps.wait_for_pods_with_condition(
            kluster,
    ...
    def patch_kube_config(kluster, static_token_path, kube_config_path: str = None) -> None:
        ...
        LOGGER.debug(
            "Patch %s to use file token %s", kube_config_path, static_token_path, extra={"prefix": kluster.region_name}
        )
        ...
        LOGGER.debug(
            "Patched kubectl config at %s with static kubectl token from %s",
            kube_config_path,
            static_token_path,
    ...
    @classmethod
    def watch_events(
        cls, k8s_core_v1_api: k8s.client.CoreV1Api, name: str = None, namespace: str = None, timeout: int = None
    ):
        field_selector = f"involvedObject.name={name}" if name is not None else None
        if namespace is None:
            return k8s.watch.Watch().stream(
                k8s_core_v1_api.list_event_for_all_namespaces, field_selector=field_selector, timeout_seconds=timeout
            )
        return k8s.watch.Watch().stream(
            k8s_core_v1_api.list_namespaced_event,
            namespace=namespace,
    ...
    def gather_k8s_logs_by_operator(cls, kluster, logdir_path=None):
        ...
        LOCALRUNNER.run(
            f"timeout -v 15m docker cp $(docker create --name mustgather {operator_image}):/usr/bin/scylla-operator {operator_bin_path} && docker rm mustgather"
        )
        ...
        LOGGER.info(
            "K8S-LOGS: START running 'must-gather' scylla-operator command", extra={"prefix": kluster.region_name}
        )
        try:
            ...
            LOGGER.warning(
                "Failed to run scylla-operator's 'must gather' command: %s", exc, extra={"prefix": kluster.region_name}
            )
        else:
            LOGGER.info(
                "K8S-LOGS: END running 'must-gather' scylla-operator command", extra={"prefix": kluster.region_name}
            )
        try:
    ...
    def gather_k8s_logs(cls, logdir_path, kubectl=None, namespaces=None) -> None:
        ...
        if namespaces:
            ...
            cluster_wide_resource_types = kubectl(
                "api-resources --namespaced=false --verbs=list -o name"
            ).stdout.split()
        ...
        for resource_type in kubectl("api-resources --namespaced=true --verbs=get,list -o name").stdout.split():
            ...
            for namespace in namespaces:
                ...
                for _res in resources_wide.split("\n"):
                    ...
                    res_stdout = kubectl(
                        f"get {resource_type}/{res} -o yaml 2>&1 | tee {logfile}", namespace=namespace
                    ).stdout
                    ...
                    for container_name in container_names:
                        ...
                        kubectl(
                            f"logs pod/{res} -c={container_name} > {logfile}.log",
                            namespace=namespace,
                            ignore_status=True,
                        )
                        kubectl(
                            f"logs pod/{res} -c={container_name} --previous=true > {logfile}-previous.log",
                            namespace=namespace,
                            ignore_status=True,
                        )
...
class HelmContainerMixin:
    ...
    def helm(
        self, kluster, *command: str, namespace: Optional[str] = None, values: "HelmValues" = None, prepend_command=None
    ) -> str:
...
class ScyllaPodsIPChangeTrackerThread(threading.Thread):
    ...
    @retrying(
        n=3600,
        sleep_time=1,
        allowed_exceptions=(ProtocolError, IncompleteRead, ReadTimeoutError, TimeoutError, ValueError),
    )
        ...
        try:
            ...
            for subset in data.get("object", {}).get("subsets", []):
                for address in subset.get("addresses", []):
                    ...
                    self.log.info(
                        "'%s/%s' node has changed it's pod IP address from '%s' to '%s'. All old IPs: %s",
                        namespace,
                        name,
                        old_ip_candidate,
                        current_ip_address,
    ...
    def _call_callbacks(self, namespace: str, pod_name: str) -> None:
        ...
        for dict_key in (pod_name, "__each__"):
            ...
            for callback in callbacks:
                process_callback(
                    callback=callback[:-1], namespace=namespace, pod_name=pod_name, add_pod_name_as_kwarg=callback[-1]
                )
    ...
    def register_callbacks(
        ...
        namespace: str,
        pod_name: str = "__each__",
        add_pod_name_as_kwarg: bool = False,
    ) -> None:
        ...
        if not callbacks:
            self.log.warning(
                "No callbacks are provided. Nothing to register. namespace='%s', pod_name='%s'", namespace, pod_name
            )
            return
        ...
        for callback in callbacks:
            ...
            if (
                isinstance(callback, (tuple, list))
                and len(callback) == 3
...
def add_pool_node_affinity(value, pool_label_name, pool_name):
    ...
    node_affinity["requiredDuringSchedulingIgnoredDuringExecution"] = required_during = node_affinity.get(
    ...
    def add_pod_owner_pool_affinity(obj):
        if obj["kind"] in ("StatefulSet", "DaemonSet", "Deployment", "Job"):
            obj["spec"]["template"]["spec"]["affinity"] = add_pool_node_affinity(
                obj["spec"]["template"]["spec"].get("affinity", {}), pool_label_name, pool_name
            )

    def add_pod_pool_affinity(obj):
        if obj["kind"] == "Pod":
            obj["spec"]["affinity"] = add_pool_node_affinity(
                obj["spec"].get("affinity", {}), pool_label_name, pool_name
            )
    ...
    def add_node_config_pool_affinity(obj):
        if obj["kind"] == "NodeConfig":
            obj["spec"]["placement"]["affinity"] = add_pool_node_affinity(
                obj["spec"]["placement"].get("affinity", {}), pool_label_name, pool_name
            )
