...
class ChaosMesh:
    ...
    def initialize(self) -> None:
        ...
        aux_node_pool_affinity = get_helm_pool_affinity_values(self._k8s_cluster.POOL_LABEL_NAME, self._k8s_cluster.AUXILIARY_POOL_NAME)
        scylla_node_pool_affinity = get_helm_pool_affinity_values(self._k8s_cluster.POOL_LABEL_NAME, self._k8s_cluster.SCYLLA_POOL_NAME)
        runtime = self._k8s_cluster.kubectl("get nodes -o jsonpath='{.items[0].status.nodeInfo.containerRuntimeVersion}'").stdout
        runtime_settings = {"runtime": "containerd", "socketPath": "/run/containerd/containerd.sock"} if runtime.startswith("containerd") else {}
        tolerations = {"tolerations": [{"key": "role", "operator": "Equal", "value": "scylla-clusters", "effect": "NoSchedule"}]}
        ...
        self._k8s_cluster.helm_install(
            ...
            values=HelmValues(self.HELM_SETTINGS | {"chaosDaemon": chaos_daemon_settings, "controllerManager": aux_node_pool_affinity, "dnsServer": aux_node_pool_affinity}),
            atomic=True,
            timeout="30m",
        )
...
class ChaosMeshExperiment:
    ...
    def get_status(self) -> ExperimentStatus:  # noqa: PLR0911
        ...
        result = self._k8s_cluster.kubectl(f"get {self.CHAOS_KIND} {self._name} -n {self._namespace} -o jsonpath='{{.status.conditions}}'", verbose=False)
        try:
...
class PodFailureExperiment(ChaosMeshExperiment):
    ...
    def __init__(self, pod: "sdcm.cluster_k8s.BasePodContainer", duration: str):  # noqa: F821
        ...
        super().__init__(pod=pod, name=f"pod-failure-{pod.name}-{datetime.now().strftime('%d-%H.%M.%S')}", timeout=timeout)
        deep_merge(
            self._experiment,
            {
                "spec": {
...
class MemoryStressExperiment(ChaosMeshExperiment):
    ...
    def __init__(
        ...
        super().__init__(pod=pod, name=f"memory-stress-{pod.name}-{datetime.now().strftime('%d-%H.%M.%S')}", timeout=timeout)
        deep_merge(
            self._experiment,
            {
                "spec": {
...
class IOFaultChaosExperiment(ChaosMeshExperiment):
    ...
    def __init__(
        ...
        deep_merge(self._experiment, {"spec": {"action": "fault", "containerNames": ["scylla"], "duration": duration, "volumePath": volume_path, "errno": error.value, "percent": error_probability}})
        if path:
...
class NetworkPacketLossExperiment(ChaosMeshExperiment):
    ...
    def __init__(
        ...
        # timeout based on duration + 10 seconds margin
        timeout = time_period_str_to_seconds(duration) + 10
        super().__init__(pod=pod, name=f"network-packet-loss-{pod.name}-{datetime.now().strftime('%d-%H.%M.%S')}", timeout=timeout)
        deep_merge(self._experiment, {"spec": {"action": "loss", "duration": duration, "loss": {"loss": str(probability), "correlation": str(correlation)}}})
...
class NetworkCorruptExperiment(ChaosMeshExperiment):
    ...
    def __init__(
        ...
        # timeout based on duration + 10 seconds margin
        timeout = time_period_str_to_seconds(duration) + 10
        super().__init__(pod=pod, name=f"network-corrupt-{pod.name}-{datetime.now().strftime('%d-%H.%M.%S')}", timeout=timeout)
        deep_merge(self._experiment, {"spec": {"action": "corrupt", "duration": duration, "corrupt": {"corrupt": str(probability), "correlation": str(correlation)}}})
...
class NetworkDelayExperiment(ChaosMeshExperiment):
    ...
    def __init__(
        ...
        # timeout based on duration + 10 seconds margin
        timeout = time_period_str_to_seconds(duration) + 10
        super().__init__(pod=pod, name=f"network-delay-{pod.name}-{datetime.now().strftime('%d-%H.%M.%S')}", timeout=timeout)
        deep_merge(
            self._experiment,
            {
                "spec": {
...
class NetworkBandwidthLimitExperiment(ChaosMeshExperiment):
    ...
    def __init__(
        ...
        # timeout based on duration + 10 seconds margin
        timeout = time_period_str_to_seconds(duration) + 10
        super().__init__(pod=pod, name=f"network-limit-{pod.name}-{datetime.now().strftime('%d-%H.%M.%S')}", timeout=timeout)
        deep_merge(
            self._experiment,
            {
                "spec": {
