...
class ContainerManager:
    ...
    @classmethod
    def ssh_copy_id(
        cls, instance: INodeWithContainerManager, name: str, user: str, key_file: str, comment: str = "test@local"
    ) -> None:
        container = cls.get_container(instance, name)
    ...
    def destroy_unregistered_containers(
        ...
        for container in containers:
            if container not in instance._containers.values():
                container.remove(v=True, force=True)
                LOGGER.debug(
                    "Container '%s' labeled with '%s' node is not registered for this node. It is likely a leftover container from previous test run and was destroyed.",
                    container.name,
                    instance.name,
                )
    ...
    def build_container_image(cls, instance: INodeWithContainerManager, name: str, **extra_build_args) -> Image:
        ...
        image = cls.build_with_progress_prints(
            docker_client=docker_client, image_tag=image_tag, **dockerfile_args, **build_args
        )
        return image
