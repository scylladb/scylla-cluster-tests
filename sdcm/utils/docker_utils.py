...
class ContainerManager:
    ...
    @staticmethod
    def _get_attr_for_name(instance: INodeWithContainerManager, name: _Name, attr: str, default: Optional[Any] = None, name_only_lookup: bool = False) -> Optional[Any]:
        attr_candidate_list = []
    ...
    def get_container(cls, instance: INodeWithContainerManager, name, raise_not_found_exc: bool = True) -> Optional[Container]:
        ...
        if container:
            try:
                ...
                docker_client.ping()
                LOGGER.info("Docker version: %s\nActive containers: %s", docker_client.version(), [c.name for c in docker_client.containers.list()])
    ...
    @classmethod
    def ssh_copy_id(cls, instance: INodeWithContainerManager, name: str, user: str, key_file: str, comment: str = "test@local") -> None:
        container = cls.get_container(instance, name)
    ...
    def get_ip_address(cls, instance: INodeWithContainerManager, name: str) -> str:
        if docker_network := instance.parent_cluster.params.get("docker_network"):
            ip_address = cls.get_container(instance, name).attrs["NetworkSettings"]["Networks"][docker_network]["IPAddress"]
        else:
    ...
    @classmethod
    def destroy_unregistered_containers(cls, instance: INodeWithContainerManager, docker_client: DockerClient = None) -> None:
        ...
        for container in containers:
            if container not in instance._containers.values():
                container.remove(v=True, force=True)
                LOGGER.debug("Container '%s' labeled with '%s' node is not registered for this node. It is likely a leftover container from previous test run and was destroyed.", container.name, instance.name)
    ...
    def build_container_image(cls, instance: INodeWithContainerManager, name: str, **extra_build_args) -> Image:
        ...
        assert dockerfile_args is not None, f"Please, define Dockerfile opts property in your class for `{name}' container."
        ...
        build_args.update(getattr(instance, f"{name.family}_container_image_build_args", cls._build_args)(**extra_build_args))

        LOGGER.debug("Build arguments for Docker image `%s':\n%s,", image_tag, pformat(build_args, indent=8))

        image = cls.build_with_progress_prints(docker_client=docker_client, image_tag=image_tag, **dockerfile_args, **build_args)
        return image
