...
def get_view_name_from_user_profile(params, stress_cmds_part: str, search_for_user_profile: str, view_name_substr: str):
    ...
    """
    stress_cmd = get_stress_command_for_profile(params=params, stress_cmds_part=stress_cmds_part, search_for_user_profile=search_for_user_profile)
    if not stress_cmd:
        return ""
...
def get_profile_content(stress_cmd):
    ...
    """
    cs_profile = re.search(r"profile=(?P<path>.*)/(?P<file>.*\.yaml)", stress_cmd) or re.search(r"(?P<path>.*)/(?P<file>.*\.yaml)", stress_cmd)
