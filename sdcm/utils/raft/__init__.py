...
from sdcm.utils.features import is_consistent_topology_changes_feature_enabled, is_consistent_cluster_management_feature_enabled, is_group0_limited_voters_enabled
...
class RaftFeatureOperations(ABC):
    ...
    def check_group0_tokenring_consistency(self, group0_members: list[Group0Member], tokenring_members: list[TokenRingMember]) -> [HealthEventsGenerator | None]: ...
...
class RaftFeature(RaftFeatureOperations):
    ...
    def clean_group0_garbage(self, raise_exception: bool = False) -> None:
        ...
        while host_ids:
            ...
            wait_for(func=is_node_down, step=5, timeout=60, throw_exc=False, text=f"Waiting node with {removing_host_id} marked down", removing_host_id=removing_host_id)
            wait_for(func=lambda: not is_node_in_bootstrapping_status(removing_host_id), step=5, timeout=120, throw_exc=False, text=f"Waiting node with {removing_host_id} doesn't have status BOOTSTRAPPING")

            ignore_dead_nodes_opt = f"--ignore-dead-nodes {','.join(host_ids)}" if host_ids else ""

            result = self._node.run_nodetool(f"removenode {removing_host_id} {ignore_dead_nodes_opt}", ignore_status=True, verbose=True, retry=3)
            if not result.ok:
                LOGGER.error("Removenode with host_id %s failed with %s", removing_host_id, result.stdout + result.stderr)
    ...
    def get_severity_change_filters_scylla_start_failed(timeout: int | None = None) -> tuple:
        return (
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*storage_service - decommission.*Operation failed", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*node_ops - decommission.*seastar::sleep_aborted", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*This node was decommissioned and will not rejoin the ring", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.RUNTIME_ERROR, regex=r".*This node was decommissioned and will not rejoin the ring", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(
                new_severity=Severity.WARNING, event_class=DatabaseLogEvent.RUNTIME_ERROR, regex=r".*Startup failed: std::runtime_error \(the topology coordinator rejected request to join the cluster", extra_time_to_expiration=timeout
            ),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*gossip - is_safe_for_restart.*status=LEFT", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent, regex=".*init - Startup failed.*"),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*node_ops - bootstrap.*Operation failed.*seastar::abort_requested_exception", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*raft - .* failed with: raft::transport_error .* connection is closed", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*raft - .*Transferring snapshot to.*Request is aborted by a caller", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*raft.*applier fiber stopped because of the error.*abort requested", extra_time_to_expiration=timeout),
            EventsSeverityChangerFilter(
                new_severity=Severity.WARNING, event_class=DatabaseLogEvent.RUNTIME_ERROR, regex=r".*raft_topology - tablets draining failed.*connection is closed\)\). Aborting the topology operation", extra_time_to_expiration=timeout
            ),
        )
    ...
    def check_group0_tokenring_consistency(self, group0_members: list[Group0Member], tokenring_members: list[TokenRingMember]) -> HealthEventsGenerator:
        LOGGER.debug("Check group0 and token ring consistency on node %s (host_id=%s)...", self._node.name, self._node.host_id)
        ...
        LOGGER.debug("Group0 and token-ring are consistent on node %s (host_id=%s)...", self._node.name, self._node.host_id)
...
class NoRaft(RaftFeatureOperations):
    ...
    def check_group0_tokenring_consistency(self, group0_members: list[Group0Member], tokenring_members: list[TokenRingMember]) -> Generator[None, None, None]:
...
__all__ = ["get_raft_mode", "get_node_status_from_system_by", "Group0MembersNotConsistentWithTokenRingMembersException", "TopologyOperations", "NodeState", "NodeStatus", "Group0Member"]
