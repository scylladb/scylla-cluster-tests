...
class HousekeepingDB:
    ...
    def from_params(cls, params: dict) -> "HousekeepingDB":
        return cls(host=params["housekeeping_db_host"], username=params["housekeeping_db_user"], password=params["housekeeping_db_password"])
    ...
    def connect(self, db_name: str = DB_NAME) -> None:
        self._connection = mysql.connector.connect(host=self.host, user=self.username, password=self.password, database=db_name)
