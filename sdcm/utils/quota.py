...
def verify_quota_is_configured_for_scylla_user(node):
    ...
    if quota_value == "0":
        ...
        raise QuotaConfigurationFailure("Quota was not configured for scylla user. The value for hard quota is: {}".format(quota_value))
...
def write_data_to_reach_end_of_quota(node, quota_size):
    ...
    quota_exceeded_appearances = node.follow_system_log(patterns=[r"Shutting down communications due to I\/O errors until operator intervention: Disk error:.*Disk quota exceeded"])
    ...
    try:
        wait_for(func=approach_end_of_quota, timeout=300, step=10, text="Waiting for 'Disk error: std::system_error (error system:122, Disk quota exceeded)' string to appear in the node log", throw_exc=False)
