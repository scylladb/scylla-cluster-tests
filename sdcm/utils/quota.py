...
def enable_quota_on_node(node):
    if not is_quota_enabled_on_node(node):
        ...
        change_default_grub_cmd = 'sed -i s\'/GRUB_CMDLINE_LINUX_DEFAULT="quiet splash"/GRUB_CMDLINE_LINUX_DEFAULT="quiet splash rootflags=uquota,pquota"/\' /etc/default/grub'
        mk_grub_cmd = "grub-mkconfig -o /boot/grub/grub.cfg"
        enable_uquota_on_mount_cmd = "sed -i s'/Options=noatime,discard/Options=noatime,discard,uquota/' /etc/systemd/system/var-lib-scylla.mount"
...
def write_data_to_reach_end_of_quota(node, quota_size):
    ...
    def approach_end_of_quota():
        ...
        try:
            ...
            LOGGER.warning(
                "We should have reached the expected I/O error and quota has exceeded\nMessage: {}".format(str(exc))
            )
    ...
    try:
        wait_for(
            ...
            timeout=300,
            step=10,
            text="Waiting for 'Disk error: std::system_error (error system:122, Disk quota exceeded)' string to appear in the node log",
            throw_exc=False,
        )
