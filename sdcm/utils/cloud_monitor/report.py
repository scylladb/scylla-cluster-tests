...
class BaseReport:
    ...
    def _jinja_render_template(self, **kwargs):
        ...
        env = jinja2.Environment(loader=loader, autoescape=True, extensions=["jinja2.ext.loopcontrols"], finalize=lambda x: x if x != 0 else "")
    ...
    def to_file(self):
        with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8", prefix="cloud-report_", delete=False, suffix=".html") as report_file:
        ...
        for cloud_provider in CLOUD_PROVIDERS:
            for instance in self.cloud_instances[cloud_provider]:
                ...
                if instance.owner not in results[user_type]:
                    stats = dict(num_running_instances_spot=0, num_running_instances_on_demand=0, num_stopped_instances=0)
...
class InstancesTimeDistributionReport(BaseReport, metaclass=abc.ABCMeta):
    def __init__(self, cloud_instances: CloudInstances, user=None):
        ...
        self.user = user
        self.report = {"unknown": defaultdict(list), "7": defaultdict(list), "5": defaultdict(list), "3": defaultdict(list)}
        self.qa_users = KeyStore().get_qa_users()
