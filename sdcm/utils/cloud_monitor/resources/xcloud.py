...
class XCloudResources:
    ...
    def process_cluster(self, api_client, account_id, cluster, environment):
        try:
            ...
            db_node_count = len(api_client.get_cluster_nodes(
                account_id=account_id, cluster_id=cluster_id, enriched=True))
    ...
    def get_vs_node_count(api_client, account_id, cluster_id, cluster_name):
        ...
        try:
            ...
            for dc in dcs:
                dc_id = dc.get('id')
                availability_zones = (api_client.get_vector_search_nodes(
                    account_id=account_id, cluster_id=cluster_id, dc_id=dc_id).get('availabilityZones') or [])
