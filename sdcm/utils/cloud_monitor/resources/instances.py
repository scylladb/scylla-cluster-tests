...
class AWSInstance(CloudInstance):
    ...
    def get_owner_from_cloud_trail(self):
        try:
            ...
            result = client.lookup_events(LookupAttributes=[{"AttributeKey": "ResourceName", "AttributeValue": self._instance["InstanceId"]}])
            for event in result["Events"]:
                if event["EventName"] == "RunInstances":
                    return event["Username"]
        except Exception as exc:  # noqa: BLE001
            LOGGER.warning("Error occurred when trying to find an owner for '%s' in CloudTrail: %s", self._instance["InstanceId"], exc)
        return None
...
class CloudInstances(CloudResources):
    ...
    def get_azure_instances(self):
        try:
            query_bits = ["Resources", "where type =~ 'Microsoft.Compute/virtualMachines'", "project id, resourceGroup, name"]
            res = AzureService().resource_graph_query(query=" | ".join(query_bits))
            get_virtual_machine = AzureService().compute.virtual_machines.get
            instances = [(get_virtual_machine(resource_group_name=vm["resourceGroup"], vm_name=vm["name"], expand="instanceView"), vm["resourceGroup"]) for vm in res]
