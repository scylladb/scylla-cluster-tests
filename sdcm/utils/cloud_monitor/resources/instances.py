...
class AWSInstance(CloudInstance):
    ...
    def __init__(self, instance):
        ...
        super().__init__(
            ...
            billing_project=self._tags.get("project", NA) if self._tags else NA,
        )
...
class GCEInstance(CloudInstance):
    ...
    def __init__(self, instance: GceInstance):
        ...
        super().__init__(
            ...
            billing_project=tags.get("project", NA) if tags else NA,
        )
    ...
    def get_azure_instances(self):
        try:
            ...
            instances = [
                (
                    get_virtual_machine(
                        resource_group_name=vm["resourceGroup"], vm_name=vm["name"], expand="instanceView"
                    ),
                    vm["resourceGroup"],
                )
                for vm in res
