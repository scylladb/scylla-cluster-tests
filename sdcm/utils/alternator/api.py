...
class Alternator:
    ...
    def verify_table_features(self, node, table_name=consts.TABLE_NAME, schema=enums.YCSBSchemaTypes.HASH_AND_RANGE,
        ...
        if wait_for_item_count >= 0:
            def repeat_scan_until_count(count, index=None, attempts=30):
                for _ in range(attempts):
                    response = dynamodb_api.client.scan(TableName=table_name, IndexName=index, Select='COUNT') \
                        if index else dynamodb_api.client.scan(TableName=table_name, Select='COUNT')
            ...
            if gsi is not None:
                assert repeat_scan_until_count(count=wait_for_item_count, index=gsi, attempts=1), \
                    f"GSI {gsi} did not reach {wait_for_item_count} items within the expected time"
            if lsi is not None:
                assert repeat_scan_until_count(count=wait_for_item_count, index=lsi, attempts=1), \
