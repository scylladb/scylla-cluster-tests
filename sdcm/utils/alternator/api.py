...
class Alternator:
    ...
    def get_dynamodb_api(self, node) -> AlternatorApi:
        ...
        if endpoint_url not in self.alternator_apis:
            ...
            if self.params.get("alternator_enforce_authorization"):
                ...
                aws_params.update(aws_access_key_id=aws_access_key_id, aws_secret_access_key=self.get_salted_hash(node, username=aws_access_key_id))
    ...
    def set_credentials(self, node):
        if self.params.get("alternator_enforce_authorization"):
            with node.parent_cluster.cql_connection_patient(node) as session:
                session.execute("CREATE ROLE %s WITH PASSWORD = %s AND login = true AND superuser = true", (self.params.get("alternator_access_key_id"), self.params.get("alternator_secret_access_key")))
    ...
    def create_table(
        ...
        **kwargs,
        ...
        if lsi:
            schema["LocalSecondaryIndexes"] = [{"IndexName": lsi, "KeySchema": schema["KeySchema"], "Projection": {"ProjectionType": "ALL"}}]
        if gsi:
            schema["GlobalSecondaryIndexes"] = [{"IndexName": gsi, "KeySchema": schema["KeySchema"], "Projection": {"ProjectionType": "ALL"}}]
        tags_list = []
        if tags:
        ...
        table = dynamodb_api.resource.create_table(TableName=table_name, BillingMode="PAY_PER_REQUEST", **schema, **kwargs)
    ...
    def verify_table_features(self, node, table_name=consts.TABLE_NAME, schema=enums.YCSBSchemaTypes.HASH_AND_RANGE, lsi: str = None, gsi: str = None, tags: dict[str, str] = None, wait_for_item_count: int = -1):
        dynamodb_api = self.get_dynamodb_api(node=node)
            ...
            def repeat_scan_until_count(count, index=None, attempts=40):
                for attempt in range(attempts):
                    ...
                    while start_key is not None:
                        response = dynamodb_api.client.scan(TableName=table_name, IndexName=index, Select="COUNT", **start_key) if index else dynamodb_api.client.scan(TableName=table_name, Select="COUNT", **start_key)
                        start_key = {"ExclusiveStartKey": response.get("LastEvaluatedKey")} if "LastEvaluatedKey" in response else None
                        c += response["Count"]
                    if c == count:
                        return True
                return False

            assert repeat_scan_until_count(count=wait_for_item_count, attempts=1), f"Table {table_name} did not reach {wait_for_item_count} items within the expected time"
            if gsi is not None:
                assert repeat_scan_until_count(count=wait_for_item_count, index=gsi), f"GSI {gsi} did not reach {wait_for_item_count} items within the expected time"
            if lsi is not None:
                assert repeat_scan_until_count(count=wait_for_item_count, index=lsi), f"LSI {lsi} did not reach {wait_for_item_count} items within the expected time"
    ...
    def batch_write_actions(self, node, table_name=consts.TABLE_NAME, new_items=None, delete_items=None, schema=schemas.HASH_SCHEMA):
        dynamodb_api = self.get_dynamodb_api(node=node)
        ...
        if delete_items:
            LOGGER.debug("Deleting %s items from table '%s'.\nDeleted: %s..", len(delete_items), table_name, pformat(delete_items))
    ...
    def is_table_exists(self, node, table_name: consts.TABLE_NAME):
        ...
        LOGGER.info("The table '{}'{} exists in endpoint {}..".format(table_name, "" if is_table_exists else "not", node.name))
