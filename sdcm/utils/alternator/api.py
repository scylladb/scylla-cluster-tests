...
class Alternator:
    ...
    def verify_table_features(self, node, table_name=consts.TABLE_NAME,
        ...
        if lsi is not None:
            lsi_names = [idx['IndexName'] for idx in table.get('LocalSecondaryIndexes', [])]
            assert lsi in lsi_names, f"Expected LSI {lsi} to be present"

        if gsi is not None:
            gsi_names = [idx['IndexName'] for idx in table.get('GlobalSecondaryIndexes', [])]
            assert gsi in gsi_names, f"Expected GSI {gsi} to be present"
