    ...
    def check_ssm_prerequisites(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            ssm_info = self.ssm_client.describe_instance_information(
                InstanceInformationFilterList=[{"key": "InstanceIds", "valueSet": [self.instance_id]}]
            )
            ...
            if not info_list:
                msg = (
                    "SSM Agent is NOT reporting in. Check IAM Instance Profile includes "
    ...
    def run(
        ...
        if not prerequisites_ok:
            ...
            return self._create_result(
                command=cmd, stdout="", stderr=prerequisites_msg, exited=255, ignore_status=ignore_status
            )
        ...
        try:
            ...
            for attempt in range(max_wait_attempts):
                ...
                if verbose:
                    LOGGER.debug(
                        "SSM command %s status (attempt %d/%d): %s", command_id, attempt + 1, max_wait_attempts, status
                    )
            ...
            return self._create_result(
                command=cmd, stdout=stdout, stderr=stderr, exited=exit_code, ignore_status=ignore_status
            )
            ...
            return self._create_result(
                command=cmd, stdout="", stderr=error_msg, exited=255, ignore_status=ignore_status
            )
        except (KeyError, IndexError) as e:
