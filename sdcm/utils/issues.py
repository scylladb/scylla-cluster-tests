...
class CachedGitHubIssues:
    ...
    def get_repo_data(self, owner, repo):
        ...
        issues = {
            issue["id"]: issue
            for issue in csv.DictReader(scsv.decode().splitlines(), fieldnames=("id", "state", "labels"))
        }
        issues = {
            issue["id"]: issue
            | dict(labels=[dict(name=label) for label in issue["labels"].strip().rstrip("|").split("|")])
...
class SkipPerIssues:
    ...
    def get_issue_details(self, issue):
        ...
        if not issue_parsed:
            ...
            ).publish()  # pylint: disable=protected-access
            return None
        try:
            if issue_details := self.cache.get_issue(
                owner=issue_parsed.user_id, repo_id=issue_parsed.repo_id, issue_id=issue_parsed.issue_id
            ):
                return github.Issue.Issue(
                    requester=None,
                    headers={},
                    attributes=dict(state=issue_details["state"].lower(), labels=issue_details["labels"]),
                    completed=True,
                )
        ...
        try:
            return self.github.get_repo(f"{issue_parsed.user_id}/{issue_parsed.repo_id}", lazy=True).get_issue(
                issue_parsed.issue_id
            )
    ...
    def issues_labeled(self) -> bool:
        if self.params.scylla_version:
            ...
            return any(f"sct-{branch_version}-skip" in label.name for label in issues_labels) or any(
                f"dtest/{branch_version}-skip" in label.name for label in issues_labels
            )
