...
SUPPORTED_PROJECTS = {"gcp-sct-project-1", "gcp-local-ssd-latency"} | {os.environ.get("SCT_GCE_PROJECT", "gcp-sct-project-1")}
...
GCE_IMAGE_URL_REGEX = re.compile(r"https://www.googleapis.com/compute/v1/projects/(?P<project>.*)/global/images/(?P<image>.*)")
...
class GcloudContainerMixin:
    ...
    def _get_gcloud_container(self) -> Container:
        ...
        res = container.exec_run(["gcloud", "auth", "activate-service-account", credentials["client_email"], "--key-file", "/tmp/gcloud_svc_account.json", "--project", credentials["project_id"]])
        if res.exit_code:
            raise DockerException(f"{container}[]: {res.output.decode('utf-8')}")
        return container
...
class GkeCleaner(GcloudContainerMixin):
    ...
    def list_orphaned_gke_disks(self) -> dict:
        disks_per_zone = {}
        try:
            disks = json.loads(self.gcloud.run('compute disks list --format="json(name,zone)" --filter="(name~^gke-.*-pvc-.* OR name~^pvc-.*) AND -users:*"'))
        except Exception as exc:  # noqa: BLE001
...
def gce_set_labels(instances_client: compute_v1.InstancesClient, instance: compute_v1.Instance, new_labels: dict, project: str, zone: str):
    """
    ...
    operation = instances_client.set_labels(project=project, zone=zone, instance=instance.name, instances_set_labels_request_resource=request)

    return wait_for_extended_operation(operation, f"setting labels on {instance.name}")


def gce_set_tags(instances_client: compute_v1.InstancesClient, instance: compute_v1.Instance, new_tags: list, project: str, zone: str):
    """
...
def gce_check_if_machine_type_supported(machine_types_client: compute_v1.MachineTypesClient, machine_type: str, project: str, zone: str):
    """
