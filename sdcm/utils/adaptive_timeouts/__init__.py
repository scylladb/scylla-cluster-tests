...
from sdcm.utils.adaptive_timeouts.load_info_store import NodeLoadInfoService, AdaptiveTimeoutStore, ArgusAdaptiveTimeoutStore, NodeLoadInfoServices
...
def _get_decommission_timeout(node_info_service: NodeLoadInfoService, tablets_enabled: bool = False) -> tuple[tuple[int, int | None], dict[str, Any]]:
...
def _get_query_timeout(node_info_service: NodeLoadInfoService, timeout: int | float = None, query: str = None) -> tuple[int | float, dict[str, Any]]:
    timeout, stats = _get_soft_timeout(node_info_service=node_info_service, timeout=timeout)
    stats["query"] = query
    return timeout, stats


def _get_service_level_propagation_timeout(node_info_service: NodeLoadInfoService, timeout: int | float = None, service_level_for_test_step: str = None) -> tuple[int | float, dict[str, Any]]:
...
class Operations(Enum):
    ...
    SERVICE_LEVEL_PROPAGATION = ("service_level_propagation", _get_service_level_propagation_timeout, ("timeout", "service_level_for_test_step"))
    ...
    def _monitor_timeouts(self):
        while self.running:
            ...
            if self.hard_timeout and duration > self.hard_timeout:
                HardTimeoutEvent(operation=self.operation, duration=duration, hard_timeout=self.hard_timeout).publish_or_dump()
                self.running = False
                return
            if not self.soft_timeout_triggered and self.soft_timeout and duration > self.soft_timeout:
                SoftTimeoutEvent(operation=self.operation, duration=duration, soft_timeout=self.soft_timeout, still_running=True).publish_or_dump()
...
def adaptive_timeout(
    ...
    **kwargs,
    ...
    monitor_ctx = TimeoutMonitor(operation.name, soft_timeout, hard_timeout, start_time=start_time) if tablet_sensitive_op and tablets_enabled and hard_timeout else nullcontext()
    ...
    try:
        ...
        try:
            if load_metrics and store_metrics:
                stats_storage.store(metrics=load_metrics, operation=operation.name, duration=duration, timeout=soft_timeout, timeout_occurred=timeout_occurred)
        except Exception as exc:  # noqa: BLE001
