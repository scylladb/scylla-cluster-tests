...
class AdaptiveTimeoutStore(metaclass=Singleton):
    ...
    def store(self, metrics: dict[str, Any], operation: str, duration: int | float, timeout: int, timeout_occurred: bool) -> None:
        pass
...
class ArgusAdaptiveTimeoutStore(AdaptiveTimeoutStore):
    ...
    def send_adaptive_timeout_results_to_argus(self, result: ArgusAdaptiveTimeoutResult):
        ...
        table.add_result(column="duration", row=f"#{cycle}", value=result.duration, status=Status.PASS if not result.timeout_occurred else Status.ERROR)
        ...
        table.add_result(column="cpu_load_5", row=f"#{cycle}", value=result.metrics.get("cpu_load_5"), status=Status.UNSET)
        table.add_result(column="shards_count", row=f"#{cycle}", value=result.metrics.get("shards_count"), status=Status.UNSET)
        table.add_result(column="read_bandwidth_mb", row=f"#{cycle}", value=result.metrics.get("read_bandwidth_mb"), status=Status.UNSET)
        table.add_result(column="write_bandwidth_mb", row=f"#{cycle}", value=result.metrics.get("write_bandwidth_mb"), status=Status.UNSET)
        table.add_result(column="read_iops", row=f"#{cycle}", value=result.metrics.get("read_iops"), status=Status.UNSET)
        table.add_result(column="write_iops", row=f"#{cycle}", value=result.metrics.get("write_iops"), status=Status.UNSET)
        table.add_result(column="node_data_size_mb", row=f"#{cycle}", value=result.metrics.get("node_data_size_mb"), status=Status.UNSET)
    ...
    def store(self, metrics: dict[str, Any], operation: str, duration: float, timeout: float, timeout_occurred: bool):
        result = ArgusAdaptiveTimeoutResult(operation=operation, duration=int(duration), timeout=int(timeout), timeout_occurred=timeout_occurred, end_time="N/A", metrics=metrics.copy())
        try:
...
class NodeLoadInfoServices(metaclass=Singleton):
    ...
    def get(self, node: BaseNode) -> NodeLoadInfoService:  # noqa: F821
        if node not in self._services:
            self._services[node.name] = NodeLoadInfoService(node.remoter, node.name, node.scylla_version_detailed, node_idx=str(getattr(node, "node_index", "")))
