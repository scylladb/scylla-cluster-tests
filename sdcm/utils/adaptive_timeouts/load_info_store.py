...
class ArgusAdaptiveTimeoutStore(AdaptiveTimeoutStore):
    ...
    def send_adaptive_timeout_results_to_argus(self, results: dict):
        ...
        cycle = next(self.cycle_counters[results['operation']]) + 1
        table = AdaptiveTimeoutResultsTable()
        table.add_result(column="duration", row=f"#{cycle}", value=results['duration'], status=Status.UNSET)
        table.add_result(column="timeout", row=f"#{cycle}", value=results["timeout"], status=Status.ERROR if results.get(
            "timeout_occurred") else Status.UNSET)
        table.add_result(column="end_time", row=f"#{cycle}", value=results["end_time"], status=Status.UNSET)
    ...
    def store(self, metrics: dict[str, Any], operation: str, duration: float, timeout: float,
        ...
        results = metrics
        try:
            results["end_time"] = datetime.fromtimestamp(time.time(), tz=timezone.utc).strftime("%H:%M:%S")
        except ValueError:
            results["end_time"] = "N/A"
