...
class NemesisNodeAllocator(metaclass=Singleton):
    ...
    def select_target_node(self,
        ...
        with self.lock:
            ...
            if not available_for_selection:
                reserved_nodes_map = {n.name: nem for n, nem in self.active_nemeses_on_nodes.items()}
    ...
    def set_running_nemesis(self, node: 'BaseNode', nemesis_name: str) -> bool:
        ...
        with self.lock:
            if node in self.active_nemeses_on_nodes:
                LOGGER.error(
                    "%s: Tried to set running nemesis for node %s, but it was already reserved by '%s' nemesis.",
        ...
        with self.lock:
            if node in self.active_nemeses_on_nodes:
                if self.active_nemeses_on_nodes[node] == nemesis_name:
                    ...
                    LOGGER.error("%s: tried to release node %s, but it was running another '%s' nemesis.",
                                 nemesis_name, node.name, self.active_nemeses_on_nodes[node])
            else:
                LOGGER.error("%s: tried to release node %s, but it was not running any nemesis.",
    ...
    def unset_running_nemesis_from_all_nodes(self, nemesis_name: str):
        ...
        """
        with self.lock:
            nodes_to_release = [node for node, owner in self.active_nemeses_on_nodes.items() if owner == nemesis_name]
            if nodes_to_release:
    ...
    def switch_target_node(self, old_node: 'BaseNode', new_node: 'BaseNode', nemesis_name: str) -> bool:
        ...
        with self.lock:
            if new_node in self.active_nemeses_on_nodes:
                reason = f"Requested node is already running nemesis '{self.active_nemeses_on_nodes[new_node]}'"
            ...
            if old_node and old_node in self.active_nemeses_on_nodes:
                if self.active_nemeses_on_nodes[old_node] == nemesis_name:
                    ...
                    LOGGER.error(
                        "%s: During switch, old node %s was running another '%s' nemesis.",
    ...
    @contextmanager
    def nodes_running_nemesis(self, nodes: Union[list['BaseNode'], 'BaseNode'], nemesis_name: str):
        """Temporarily marks nodes as running the specified nemesis."""
        from sdcm.cluster import BaseNode  # importing here to avoid circular import
            ...
            for node in nodes:
                if not self.set_running_nemesis(node, nemesis_name):
                    ...
                    raise NemesisNodeAllocationError(
                        f"{nemesis_name}: failed to set running nemesis for all requested nodes; {node.name} was already reserved.")
