...
class NemesisNodeAllocator(metaclass=Singleton):
    ...
    @staticmethod
    def _filter_nodes(nodes_to_filter: list["BaseNode"], is_seed: bool | DefaultValue | None = DefaultValue, dc_idx: int | None = None, rack: int | None = None, filter_seed: bool = False) -> list["BaseNode"]:
        """
    ...
    @contextmanager
    def run_nemesis(self, nemesis_label: str, node_list: list["BaseNode"] | None = None, pool_type: NEMESIS_TARGET_POOLS = NEMESIS_TARGET_POOLS.data_nodes, filter_seed: bool = False):
        """
        ...
        try:
            reserved_node = self.select_target_node(nemesis_name=unique_nemesis_label, pool_type=pool_type, filter_seed=filter_seed, node_list=node_list)
            yield reserved_node
        finally:
            if reserved_node:
    ...
    def select_target_node(
        ...
        with self.lock:
            ...
            candidate_nodes = self._filter_nodes(available_for_selection, is_seed=is_seed, dc_idx=dc_idx, rack=rack, filter_seed=filter_seed)
            ...
            if not candidate_nodes:
                ...
                reason = f"The following '{pool_type.value}' nodes are available for selection, but none of them match the specified criteria ({filter_str}).\n{[n.name for n in available_for_selection]}."
        ...
        with self.lock:
            if node in self.active_nemesis_on_nodes:
                TestFrameworkEvent(
                    ...
                    message=f"{nemesis_name}: setting running nemesis failed.\nTried to set running nemesis for node {node.name}, but it was already reserved by '{self.active_nemesis_on_nodes[node]}' nemesis.",
        ...
        with self.lock:
            if node in self.active_nemesis_on_nodes:
                if self.active_nemesis_on_nodes[node] == nemesis_name:
                    ...
                    TestFrameworkEvent(
                        source=self.__class__.__name__,
                        message=f"{nemesis_name}: unsetting running nemesis failed.\nTried to unset '{nemesis_name}' nemesis from node {node.name}, but it was running another '{self.active_nemesis_on_nodes[node]}' nemesis.",
                        severity=Severity.ERROR,
                    ).publish()
            else:
                TestFrameworkEvent(
                    source=self.__class__.__name__, message=f"{nemesis_name}: unsetting running nemesis failed.\nTried to unset '{nemesis_name}' nemesis from node {node.name}, but it was not running any nemesis.", severity=Severity.ERROR
                ).publish()
    ...
    def unset_running_nemesis_from_all_nodes(self, nemesis_name: str):
        ...
        with self.lock:
            ...
            if nodes_to_release:
                LOGGER.info("%s: unset running nemesis from nodes: %s", nemesis_name, [n.name for n in nodes_to_release])
        ...
        with self.lock:
            ...
            if old_node and old_node in self.active_nemesis_on_nodes:
                if self.active_nemesis_on_nodes[old_node] == nemesis_name:
                    ...
                    TestFrameworkEvent(
                        source=self.__class__.__name__,
                        message=f"{nemesis_name}: switching target node failed.\nDuring switch, old node {old_node.name} was running another '{self.active_nemesis_on_nodes[old_node]}' nemesis.",
                        severity=Severity.ERROR,
                    ).publish()
        ...
        try:
            for node in nodes:
                if not self.set_running_nemesis(node, nemesis_name):
                    ...
                    marked_nodes.clear()
                    raise NemesisNodeAllocationError(f"{nemesis_name}: failed to set running nemesis for all requested nodes; {node.name} was already reserved.")
