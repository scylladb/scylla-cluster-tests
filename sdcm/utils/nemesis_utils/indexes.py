...
def is_cf_a_view(node: BaseNode, ks, cf) -> bool:
    ...
    with node.parent_cluster.cql_connection_patient(node) as session:
        try:
            result = session.execute(f"SELECT view_name FROM system_schema.views WHERE keyspace_name = '{ks}' AND view_name = '{cf}'")
...
def get_column_names(session, ks, cf, is_primary_key: bool = False, filter_out_collections: bool = False, filter_out_static_columns: bool = False, filter_out_column_types: list[str] | None = None) -> list:
    ...
    res = session.execute(f"SELECT column_name, type FROM system_schema.columns WHERE keyspace_name = '{ks}' AND table_name = '{cf}' AND {filter_kind} ALLOW FILTERING")
    res_list = list(res)
...
def get_random_column_name(session, ks, cf, filter_out_collections: bool = False, filter_out_static_columns: bool = False, filter_out_column_types: list[str] | None = None) -> str | None:
    if column_names := get_column_names(session=session, ks=ks, cf=cf, filter_out_collections=filter_out_collections, filter_out_static_columns=filter_out_static_columns, filter_out_column_types=filter_out_column_types):
...
def wait_for_view_to_be_built(node: BaseNode, ks, view_name, timeout=300) -> None:
    ...
    while time.time() - start_time < timeout:
        result = node.run_nodetool(f"viewbuildstatus {ks}.{view_name}", ignore_status=True, verbose=True, publish_event=True)
...
def verify_query_by_index_works(session, ks, cf, column) -> None:
    ...
    if value is None:
        ...
        InfoEvent(message=f"No value for column {column} in {ks}.{cf}, skipping querying created index.", severity=Severity.NORMAL).publish()
        return
    try:
        ...
        InfoEvent(message=f"Index {ks}.{cf}({column}) does not work in query: {query}. Reason: {exc}", severity=Severity.ERROR).publish()
    if len(list(result)) == 0:
        InfoEvent(message=f"Index {ks}.{cf}({column}) does not work. No rows returned for query {query}", severity=Severity.ERROR).publish()
...
def drop_materialized_view(session, ks, view_name, timeout=300) -> None:
    ...
    with EventsFilter(event_class=DatabaseLogEvent.DATABASE_ERROR, regex=".*Error applying view update.*", extra_time_to_expiration=180):
...
def create_materialized_view(
    ...
    mv_name,
    mv_partition_key,
    mv_clustering_key,  # noqa: PLR0913
    mv_columns="*",
    ...
    if mv_columns != "*":
        mv_columns = [cql_quote_if_needed(cql_unquote_if_needed(col)) for col in (mv_columns if isinstance(mv_columns, list) else list(mv_columns))]
    mv_partition_key = [cql_quote_if_needed(cql_unquote_if_needed(pk)) for pk in (mv_partition_key if isinstance(mv_partition_key, list) else list(mv_partition_key))]
    mv_clustering_key = [cql_quote_if_needed(cql_unquote_if_needed(cl)) for cl in (mv_clustering_key if isinstance(mv_clustering_key, list) else list(mv_clustering_key))]
...
def create_materialized_view_for_random_column(session, keyspace_name, base_table_name, view_name):
    ...
    primary_key_columns = get_column_names(session=session, ks=keyspace_name, cf=base_table_name, is_primary_key=True, filter_out_column_types=unsupported_primary_key_columns)
    # selecting a supported column for creating a materialized-view (not a collection type).
    column = get_random_column_name(session=session, ks=keyspace_name, cf=base_table_name, filter_out_collections=True, filter_out_static_columns=True, filter_out_column_types=unsupported_primary_key_columns)
    if not column:
    ...
    with EventsFilter(event_class=DatabaseLogEvent, regex=".*Error applying view update.*", extra_time_to_expiration=180):
        create_materialized_view(session, keyspace_name, base_table_name, view_name, [column], primary_key_columns, mv_columns=[column] + primary_key_columns)
...
def wait_materialized_view_building_tasks_started(session, ks_name, view_name, timeout=600):
    ...
    while time.time() - start_time < timeout:
        try:
            row = session.execute(f"select id from system_schema.views where keyspace_name = '{ks_name}' and view_name = '{view_name}'").one()
            assert row.id, f"View {ks_name}.{view_name} was not found"
            result = list(session.execute(f"select * from system.view_building_tasks where key = 'view_building' and view_id = {row.id} ALLOW FILTERING;"))
            if len(result) > 0:
                return
            ...
            continue
        time.sleep(15)
    raise TimeoutError(f"Timeout error ({timeout} seconds) while waiting for view {ks_name}.{view_name} to start building")
