...
def get_column_names(
    ...
    res = session.execute(
        f"SELECT column_name, type FROM system_schema.columns WHERE keyspace_name = '{ks}' AND table_name = '{cf}' AND {filter_kind} ALLOW FILTERING"
    )
    res_list = list(res)
...
def wait_for_view_to_be_built(node: BaseNode, ks, view_name, timeout=300) -> None:
    ...
    while time.time() - start_time < timeout:
        result = node.run_nodetool(
            f"viewbuildstatus {ks}.{view_name}", ignore_status=True, verbose=False, publish_event=False
        )
