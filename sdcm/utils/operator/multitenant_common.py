...
def get_tenants(test_class_instance):
    ...
    for param_dict in test_class_instance.params.config_options:
        ...
        for i, _ in enumerate(tenants):
            ...
            if param_name.startswith("stress_") and isinstance(current_param_value, list) and len(current_param_value) == 1:
            ...
            LOGGER.debug("Setting '%s' option of the '%s' tenant with the following value: %s", param_name, str(tenants[i]), current_param_value)
