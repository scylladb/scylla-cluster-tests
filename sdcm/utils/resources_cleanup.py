    ...
    for region_name, lt_list in lts_per_region.items():
        ...
        for current_lt in lt_list:
            ...
            try:
                ...
                LOGGER.info("Successfully deleted '%s' LaunchTemplate. Response: %s\n", (current_lt_name or current_lt_id), response)
            except Exception as ex:  # noqa: BLE001
...
def clean_instances_gce(tags_dict: dict, dry_run=False):
    ...
    ParallelObject(map(lambda i: (i, tags_dict), gce_instances_to_clean), timeout=60).run(delete_instance, ignore_exceptions=False)
...
def clean_instances_azure(tags_dict: dict, regions=None, dry_run=False):
    ...
    for provisioner in provisioners:
        ...
        if len(all_instances) == len(instances_to_clean):
            ...
            LOGGER.info("test id %s from %s - instances to clean: %s", provisioner.test_id, provisioner.region, [inst.name for inst in instances_to_clean])
            if not dry_run:
                for instance in instances_to_clean:
...
def clean_clusters_gke(tags_dict: dict, dry_run: bool = False) -> None:
    ...
    def delete_cluster(cluster):
        if not dry_run:
            LOGGER.info("Going to delete %s GKE cluster from the %s project", cluster.name, os.environ.get("SCT_GCE_PROJECT"))
            try:
                res = cluster.destroy()
...
def clean_orphaned_gke_disks(tags_dict: dict, dry_run: bool = False) -> None:
    ...
    try:
        ...
        LOGGER.info("Found following orphaned GKE disks in the %s project: %s", os.environ.get("SCT_GCE_PROJECT"), orphaned_disks)
        if not dry_run:
            for zone, disk_names in orphaned_disks.items():
                gke_cleaner.clean_disks(disk_names=disk_names, zone=zone)
                LOGGER.info("Deleted following orphaned GKE disks in the '%s' zone (%s project): %s", zone, os.environ.get("SCT_GCE_PROJECT"), disk_names)
    except Exception as exc:  # noqa: BLE001
        LOGGER.error(exc)
...
def clean_clusters_eks(tags_dict: dict, regions: list = None, dry_run: bool = False) -> None:
    ...
    def delete_cluster(cluster):
        if not dry_run:
            ...
            try:
                res = cluster.destroy()
                LOGGER.info("'%s' EKS cluster in the '%s' region has been deleted. Response=%s", cluster.name, cluster.region_name, res)
            except Exception as exc:  # noqa: BLE001
                LOGGER.error(exc)
...
def clean_resources_according_post_behavior(params, config, logdir, dry_run=False):
    ...
    for cluster_nodes_type, action_type in actions_per_type.items():
        if action_type["action"] == "keep":
            ...
            LOGGER.info("Post behavior %s for %s. No critical events found. Schedule cleanup.", action_type["action"], cluster_nodes_type)
            for node_type in action_type["node_types"]:
                node_types_to_cleanup.append(node_type)
            continue
        else:
            LOGGER.info("Post behavior %s for %s. Test run Failed. Keep resources running", action_type["action"], cluster_nodes_type)
            continue
...
def clean_placement_groups_aws(tags_dict: dict, regions=None, dry_run=False):
    ...
    if regions:
        ...
        for region in regions:
            aws_placement_groups |= list_placement_groups_aws(tags_dict=tags_dict, region_name=region, group_as_region=True)
    else:
...
def cleanup_cluster_vpc_peering(api_client: ScyllaCloudAPIClient, account_id: str, cluster_id: int, cluster_name: str, config: dict) -> None:
    try:
    ...
    for peering in peerings:
        ...
        peering_details = api_client.get_vpc_peer_details(account_id=account_id, cluster_id=cluster_id, peer_id=peering_id)
        cloud_vpc_cidr = api_client.get_cluster_details(account_id=account_id, cluster_id=cluster_id, enriched=True)["dc"]["cidrBlock"]
        ...
        try:
            ...
            LOGGER.error("Failed to cleanup %s side configuration for Scylla Cloud peering %s: %s", xcloud_provider.upper(), peering_id, e)
...
def _cleanup_aws_vpc_peering_configuration(region_name: str, cloud_vpc_cidr: str, peering_id: str) -> None:
    ...
    for route_table in aws_region.sct_route_tables:
        for route in route_table.routes_attribute:
            if route.get("DestinationCidrBlock") == cloud_vpc_cidr and route.get("VpcPeeringConnectionId") == peering_id:
                try:
                    LOGGER.info("Removing route: %s -> %s from route table %s", cloud_vpc_cidr, peering_id, route_table.id)
...
def _cleanup_gcp_vpc_peering_configuration(region_name: str, peering_id: str, peer_project: str) -> None:
    ...
    peering_cleanup_success = gce_region.cleanup_vpc_peering_connection(f"sct-to-scylla-cloud-{peer_project}-{peering_id}")
        ...
        for index in [None, 0, 1]:
            ...
            for route in route_table.routes_attribute:
                ...
                if route_network.subnet_of(cidr_pool):
                    LOGGER.info("Removing blackhole route: %s from route table %s in region %s", dest_cidr, route_table.id, region_name)
                    if not dry_run:
...
def clean_inactive_peerings_gce(config: dict, regions=None, dry_run=False) -> None:
    ...
    for region_name in regions:
        ...
        for peering in network.peerings:
            ...
            LOGGER.info("Removing inactive peering %s to %s Scylla Cloud project, in region %s", peering.name, peer_project, region_name)
            if not dry_run:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if "peering operation in progress" in str(exc).lower() and attempt < max_retries - 1:
                            LOGGER.warning("Peering operation is in progress, waiting %s seconds before retry (attempt %s/%s)", retry_delay, attempt + 1, max_retries)
                            time.sleep(retry_delay)
                            retry_delay *= 2
                        else:
                            LOGGER.error("Failed to remove peering %s after %s attempts: %s", peering.name, attempt + 1, exc)
                            raise
...
def clean_clusters_scylla_cloud(tags_dict: dict, config: dict, dry_run: bool = False) -> None:
    ...
    api_client = ScyllaCloudAPIClient(api_url=config.cloud_env_credentials["base_url"], auth_token=config.cloud_env_credentials["api_token"])
