    ...
    def _get_keyspaces_to_decrease_rf(self, session) -> list:
        ...
        for row in cql_result.current_rows:
            ...
            if "NetworkTopologyStrategy" in replication["class"]:
                ...
                if rf is None:
                    LOGGER.warning(f"Datacenter {self.datacenter} not found in replication strategy for keyspace {keyspace_name}.")
                    continue
    ...
    def revert_to_original_keyspaces_rf(self, node_to_wait_for_balance: "BaseNode" = None):
        if self.decreased_rf_keyspaces:
            ...
            for keyspace in self.decreased_rf_keyspaces:
                self._alter_keyspace_rf(keyspace=keyspace, replication_factor=self.original_nodes_number, node=self.cluster.data_nodes[0])
    ...
    def decrease_keyspaces_rf(self):
        ...
        if self.original_nodes_number > 1:
            ...
            if decreased_rf_keyspaces:
                LOGGER.debug(f"Found the following keyspaces with replication factor to decrease: {decreased_rf_keyspaces}")
                try:
                    for keyspace in decreased_rf_keyspaces:
                        self._alter_keyspace_rf(keyspace=keyspace, replication_factor=self.original_nodes_number - 1, node=node)
            ...
            LOGGER.error(f"DC {self.datacenter} has {self.original_nodes_number} nodes. Cannot alter replication factor")
