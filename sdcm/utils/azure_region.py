...
class AzureRegion:
    ...
    def create_sct_resource_group(self, tags: Optional[dict] = None) -> None:
        ...
        if self.azure_service.resource.resource_groups.check_existence(
            ...
            LOGGER.info("Resource group `%s' already exists in region `%s'", self.sct_resource_group_name, self.region_name)
            return
    ...
    def create_sct_network_security_group(self, tags: Optional[dict] = None) -> None:
        ...
        with suppress(ResourceNotFoundError):
            LOGGER.info("Network security group `%s' already exists in resource group `%s'", self.sct_network_security_group.name, self.sct_resource_group_name)
            return
    ...
    def create_sct_virtual_network(self, tags: Optional[dict] = None) -> None:
        ...
        with suppress(ResourceNotFoundError):
            LOGGER.info("Virtual network `%s' already exists in resource group `%s'", self.sct_virtual_network.name, self.sct_resource_group_name)
            return
    ...
    def create_sct_subnet(self) -> None:
        ...
        with suppress(ResourceNotFoundError):
            LOGGER.info("Subnet `%s' already exists in network `%s'", self.sct_subnet.name, self.sct_virtual_network_name)
            return
    ...
    def create_network_interface(self, network_interface_name: str, tags: Optional[dict] = None, create_public_ip_address: bool = True) -> NetworkInterface:
        parameters = self.common_parameters(tags=tags) | {
            "ip_configurations": [
                {
    ...
    def create_gallery_image(self, gallery_image_name: str, os_state: AzureOsState, tags: Optional[dict] = None) -> None:
        with suppress(ResourceNotFoundError):
            ...
            LOGGER.info("Gallery image `%s' already exists in the gallery `%s'", gallery_image.name, self.sct_gallery_name)
            return
    ...
    def get_gallery_image_version(self, gallery_image_name: str, gallery_image_version_name: str) -> GalleryImageVersion:
    ...
    def create_gallery_image_version(self, gallery_image_name: str, gallery_image_version_name: str, source_id: str, tags: Optional[dict] = None) -> None:
    ...
    def append_target_region_to_image_version(self, gallery_image_name: str, gallery_image_version_name: str, region_name: str) -> None:
