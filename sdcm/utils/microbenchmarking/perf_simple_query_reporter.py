...
class PerfSimpleQueryAnalyzer(BaseResultsAnalyzer):
    ...
    def check_regression(
        self,
        test_id,
        mad_deviation_limit=0.02,
        regression_limit=0.05,
        is_gce=False,
        extra_jobs_to_compare=None,  # noqa: PLR0914
    ) -> dict:
        ...
        for tag_row in tests_filtered["hits"]["hits"]:
            if (
                keys_exists(tag_row, "_source", "versions", "scylla-server", "date")
                and keys_exists(tag_row, "_source", "results", "perf_simple_query_result")
                and keys_exists(
                    tag_row,
                    "_source",
                    "setup_details",
                ...
                if (
                    tag_row["_source"]["setup_details"]["instance_type_db"] == doc["_source"]["setup_details"]["instance_type_db"]
                    and tag_row["_id"] != doc["_id"]
                    and tag_row["_source"]["test_details"]["job_name"] in (doc["_source"]["test_details"]["job_name"], *extra_jobs_to_compare)
                ):
                    sorted_results[int(str(tag_row["_source"]["versions"]["scylla-server"]["date"]) + str(tag_row["_source"]["test_details"]["start_time"]))] = tag_row["_source"]
        ...
        for row in sorted_results:
            ...
            if len(filtered_by_scylla_date.keys()) < self.collect_last_scylla_date_count:
                if date not in filtered_by_scylla_date.keys() or median_tps > filtered_by_scylla_date[date]["results"]["perf_simple_query_result"]["stats"]["median tps"]:
        ...
        def make_table_line_for_render(data):
            ...
            table_line["test_version"] = data["results"]["perf_simple_query_result"]["versions"]["scylla-server"]
        ...
        subject = f"{self._get_email_tags(doc, is_gce)} perf_simple_query Microbenchmark - Regression - {test_stats['versions']['scylla-server']['run_date_time']}"
        for_render = {
