...
class PerfSimpleQueryAnalyzer(BaseResultsAnalyzer):
    ...
    def check_regression(  # noqa: PLR0914
        self,
        test_id,
        mad_deviation_limit=0.02,
        regression_limit=0.05,
        is_gce=False,
        extra_jobs_to_compare=None,
    ) -> dict:
        ...
        for tag_row in tests_filtered["hits"]["hits"]:
            if (
                ...
                if (
                    tag_row["_source"]["setup_details"]["instance_type_db"]
                    == doc["_source"]["setup_details"]["instance_type_db"]
                    and tag_row["_id"] != doc["_id"]
                    and tag_row["_source"]["test_details"]["job_name"]
                    in (doc["_source"]["test_details"]["job_name"], *extra_jobs_to_compare)
                ):
                    sorted_results[
                        int(
                            str(tag_row["_source"]["versions"]["scylla-server"]["date"])
                            + str(tag_row["_source"]["test_details"]["start_time"])
                        )
                    ] = tag_row["_source"]
        ...
        for row in sorted_results:
            ...
            if len(filtered_by_scylla_date.keys()) < self.collect_last_scylla_date_count:
                if (
                    date not in filtered_by_scylla_date.keys()
                    or median_tps
                    > filtered_by_scylla_date[date]["results"]["perf_simple_query_result"]["stats"]["median tps"]
                ):
