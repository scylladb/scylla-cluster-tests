...
def wait_for(func, step=1, text=None, timeout=None, throw_exc=True, stop_event=None, **kwargs):
    ...
    try:
        retry = tenacity.Retrying(reraise=throw_exc, stop=tenacity.stop_any(*stops), wait=tenacity.wait_fixed(step), before_sleep=retry_logger, retry=(tenacity.retry_if_result(lambda value: not value) | tenacity.retry_if_exception_type()))
...
def exponential_retry(func: Callable[[], R], exceptions: tuple[type[BaseException]] | type[BaseException] = Exception, threshold: float = 300, retries: int = 10, logger: logging.Logger | None = LOGGER) -> R:
    """
    ...
    def retry_logger(retry_state: tenacity.RetryCallState) -> None:
        logger.error("Call to method %s (retries: %s) failed: %s", retry_state.fn, retry_state.attempt_number, retry_state.outcome.exception())
...
@contextmanager
def wait_for_log_lines(node, start_line_patterns, end_line_patterns, start_timeout=60, end_timeout=120, error_msg_ctx=""):
