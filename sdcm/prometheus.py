...
class NemesisMetrics:
    ...
    def __init__(self, metric_name_suffix=""):
        ...
        self._disrupt_counter = self.create_counter(f"{name}_counter", "Counter for nemesis disruption methods", ["method", "event"])
...
class PrometheusAlertManagerListener(threading.Thread):
    ...
    def wait_till_alert_manager_up(self):
        ...
        if self._stop_flag.is_set():
            ...
            raise TimeoutError(f"Prometheus Alert Manager({self._alert_manager_url}) did not get up for {self._timeout}s")
    ...
    def _publish_end_of_alerts(self, alerts: dict):
        ...
        for alert in alerts.values():
            ...
            continuous_hash = PrometheusAlertManagerEvent.get_continuous_hash_from_dict({"node": node, "starts_at": alert.get("startsAt"), "alert_name": alert_name})
    ...
    def silence(self, alert_name: str, duration: Optional[int] = None, start: Optional[datetime.datetime] = None, end: Optional[datetime.datetime] = None) -> str:
        """
...
class AlertSilencer:
    def __init__(self, alert_manager: PrometheusAlertManagerListener, alert_name: str, duration: Optional[int] = None, start: Optional[datetime.datetime] = None, end: Optional[datetime.datetime] = None):
