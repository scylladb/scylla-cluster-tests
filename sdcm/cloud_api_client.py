...
class CloudProviderType(Enum):
    ...
    def from_sct_backend(cls, backend_name: str) -> "CloudProviderType":
        ...
        if backend_upper == "GCE":  # map SCT 'gce' backend to Scylla Cloud 'GCP' provider
            ...
            raise ValueError(f"Unsupported cloud provider backend: {backend_name}. Supported backends: gce, aws")
...
class ScyllaCloudAPIClient:
    ...
    def __init__(self, api_url: str, auth_token: str, raise_for_status: bool = False):
        ...
        self.session.headers.update({"Authorization": f"Bearer {self.auth_token}", "Content-Type": "application/json", "Accept": "application/json"})
    ...
    def _create_session(retries: int = 5) -> requests.Session:
        ...
        retry_strategy = Retry(total=retries, backoff_factor=1, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=["HEAD", "GET", "PUT", "DELETE", "OPTIONS", "TRACE"])
    ...
    def request(self, method: str, endpoint: str, /, params=None, **body) -> dict | None:
        ...
        LOGGER.debug("Making %s request to %s with query parameters %s and payload %s", method, url, pformat(params), pformat(body))
    ...
    def get_scylla_versions(self, defaults: bool = False) -> dict[str, Any]:
        ...
        return self.request("GET", "deployment/scylla-versions", params=params)
    ...
    def get_regions(self, *, cloud_provider_id: int, defaults: bool = False) -> dict[str, Any]:
        ...
        if defaults:
            params["defaults"] = "true"
        return self.request("GET", f"/deployment/cloud-provider/{cloud_provider_id}/regions", params=params)
    ...
    def get_instance_id_by_name(self, *, cloud_provider_id: int, region_id: int, instance_type_name: str) -> int:
        ...
        try:
            ...
            raise ScyllaCloudAPIError(f"Instance type '{instance_type_name}' not found in region_id: {region_id} for cloud_provider_id: {cloud_provider_id}, available instance types: {', '.join(t['externalId'] for t in instance_types)}")
    ...
    def client_ip(self) -> str:
        ...
        Attempts to obtain the IP from an external service (similar to how Siren frontend operates). Falls back
        to API method if the external service fails.
        The API method is not the primary source because it returns IPv6 address for cloud instances, and Siren does
        not yet support IPv6 addresses.
        """
        try:
    ...
    def get_clusters(self, *, account_id: int, metrics: str = "", enriched: bool = False) -> list[dict[str, Any]]:
        ...
        return self.request("GET", f"/account/{account_id}/clusters", params=params)["clusters"]
    ...
    def get_cluster_details(self, *, account_id: int, cluster_id: int, enriched: bool = False) -> dict[str, Any]:
        ...
        if enriched:
            params["enriched"] = "true"
        return self.request("GET", f"/account/{account_id}/cluster/{cluster_id}", params=params)["cluster"]
    ...
    def get_cluster_nodes(self, *, account_id: int, cluster_id: int, enriched: bool = False) -> list[dict[str, Any]]:
        ...
        if enriched:
            params["enriched"] = "true"
        return self.request("GET", f"/account/{account_id}/cluster/{cluster_id}/nodes", params=params)["nodes"]
    ...
    def install_vector_search(self, *, account_id: int, cluster_id: int, dc_id: int, number_of_nodes: int, instance_type_id: int) -> dict[str, Any]:
        """Install Vector Search nodes into an existing cluster"""
        return self.request("POST", f"/account/{account_id}/cluster/{cluster_id}/dc/{dc_id}/vector-search", defaultNodes=number_of_nodes, defaultInstanceTypeId=instance_type_id)
    ...
    def create_fw_rule(self, *, account_id: int, cluster_id: int, ip_address: str) -> dict[str, Any]:
        ...
        return self.request("POST", f"/account/{account_id}/cluster/{cluster_id}/network/firewall/allowed", ipAddress=ip_address)
    ...
    def create_vpc_peer(self, *, account_id: int, cluster_id: int, vpc_id: str, cidr_block: str, owner_id: str, region_id: int, dc_id: int, allow_cql: bool, asynchronous: bool = False) -> dict[str, Any]:
        """
        ...
        return self.request("POST", url, vpcId=vpc_id, cidrBlock=cidr_block, ownerId=owner_id, regionId=region_id, dcId=dc_id, allowCql=allow_cql, params=params)
    ...
    def update_vpc_peer(self, *, account_id: int, cluster_id: int, peer_id: str, status: str, cidr_block: str) -> dict[str, Any]:
        """
        ...
        """
        return self.request("PUT", f"/account/{account_id}/cluster/{cluster_id}/network/vpc/peer/{peer_id}", status=status, cidrBlock=cidr_block)
    ...
    def get_network_connections(self, *, account_id: int, cluster_id: int, type: str = "", dc: int | None = None) -> list[dict[str, Any]]:
        ...
        if dc:
            params["dc"] = dc
        return self.request("GET", f"/account/{account_id}/cluster/{cluster_id}/network/vpc/connection", params=params)

    def create_network_connection(self, *, account_id: int, cluster_id: int, cidr_list: list[str], dc_id: int, data: dict[str, Any], name: str, connection_type: Literal["AWS_TGW_ATTACHMENT"]) -> dict[str, Any]:
        """
        ...
        """
        return self.request("POST", f"/account/{account_id}/cluster/{cluster_id}/network/vpc/connection", cidrList=cidr_list, clusterDCID=dc_id, data=data, name=name, type=connection_type)
    ...
    def update_network_connection(
        ...
        *,
        ...
        """
        return self.request("PATCH", f"/account/{account_id}/cluster/{cluster_id}/network/vpc/connection/{connection_id}", cidrList=cidr_list, name=name, status=status)

    def recreate_network_connection(self, *, account_id: int, cluster_id: int, connection_id: str, cidr_list: list[str], data: dict[str, Any], name: str, status: Literal["ACTIVE", "INACTIVE"]) -> dict[str, Any]:
        ...
        :param status: new status of the connection ("ACTIVE" or "INACTIVE")
        """
        return self.request("PUT", f"/account/{account_id}/cluster/{cluster_id}/network/vpc/connection/{connection_id}", cidrList=cidr_list, data=data, name=name, status=status)
    ...
    def delete_network_connection(self, *, account_id: int, cluster_id: int, connection_id: str) -> dict[str, Any]:
        ...
        return self.request("DELETE", f"/account/{account_id}/cluster/{cluster_id}/network/vpc/connection/{connection_id}")
