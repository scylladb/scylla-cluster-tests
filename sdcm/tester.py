...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):
    ...
    def create_table(
        ...
        if additional_columns == "":
            query = (
                "CREATE COLUMNFAMILY IF NOT EXISTS %s (key %s, c varchar, v varchar, PRIMARY KEY(key, c)) WITH comment='test cf'"
                % (name, key_type)
            )
        else:
    ...
    def create_materialized_view(
        ...
        query = f"CREATE MATERIALIZED VIEW {ks_name}.{mv_name} AS SELECT {select_clause} FROM {ks_name}.{base_table_name} WHERE {where_clause} PRIMARY KEY ({pk_clause}, {cl_clause}) WITH comment='test MV'"
        if compression is not None:
    ...
    def _wait_for_view(self, scylla_cluster, session, key_space, view):
        ...
        def _view_build_finished(live_nodes_amount):
            result = self.rows_to_list(
                session.execute(
                    "SELECT status FROM system_distributed.view_build_status WHERE keyspace_name='{0}' AND view_name='{1}'".format(
                        key_space, view
                    )
                )
            )
    ...
    def _wait_for_view_build_start(self, session, key_space, view, seconds_to_wait=20):
        def _check_build_started():
            result = self.rows_to_list(
                session.execute(
                    "SELECT last_token FROM system.views_builds_in_progress WHERE keyspace_name='{0}' AND view_name='{1}'".format(
                        key_space, view
                    )
                )
            )
            ...
            if result:
                ...
                for column in columns_list or result.column_names:
                    column_kind = session.execute(
                        "select kind from system_schema.columns where keyspace_name='{ks}' and table_name='{name}' and column_name='{column}'".format(
        ...
        with self.db_cluster.cql_connection_patient(node, verbose=False) as session:
            ...
            try:
                ...
                if succeeded_rows != len(source_table_rows):
                    self.log.warning(
                        "Problem during copying data. Not all rows were inserted. Rows expected to be inserted: %s; Actually inserted rows: %s.",
    ...
    def populate_data_parallel(self, size_in_gb: int, replication_factor: int = 3, blocking=True, read=False):
        ...
        stress_fixed_params = f" -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor={replication_factor}) compaction(strategy=LeveledCompactionStrategy)' -mode cql3 native -rate threads=200 -col 'size=FIXED(1024) n=FIXED(1)' "
        stress_keys = "n="
        population = " -pop seq="
    ...
    def wait_data_dir_reaching(self, size, node):
        query = '(sum(node_filesystem_size_bytes{{mountpoint="{0.scylla_dir}", instance=~"{1.private_ip_address}"}})-sum(node_filesystem_avail_bytes{{mountpoint="{0.scylla_dir}", instance=~"{1.private_ip_address}"}}))'.format(
            self, node
        )
