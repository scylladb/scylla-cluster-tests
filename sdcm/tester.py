...
from sdcm.cluster_cloud import ScyllaCloudCluster, LoaderSetCloud
...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):
    ...
    def set_system_auth_rf(self, db_cluster=None):
        ...
        if self.params.get('db_type') == 'cloud_scylla' or self.params.get("cluster_backend") in ("baremetal", "cloud"):
    ...
    def get_cluster_cloud(self, loader_info, db_info):
        cloud_api_client = ScyllaCloudAPIClient(
            api_url=self.params.get('cloud_api_url'),
            auth_token=self.params.get('cloud_api_token'),
        ...
        self.db_cluster = ScyllaCloudCluster(
            ...
            params=self.params,
            region_names=self.params.get('cloud_region'),
            add_nodes=True)

        if loader_info['n_nodes'][0] > 0:
            self.log.info("Creating Scylla Cloud loader set")
            self.loaders = LoaderSetCloud(
                cloud_api_client=cloud_api_client,
                user_prefix=user_prefix,
                n_nodes=loader_info['n_nodes'][0],
                params=self.params,
                region_names=self.params.get('cloud_region'))
        else:
            self.loaders = None

        # monitors are not needed as Scylla Cloud has built-in monitoring
        self.monitors = NoMonitorSet()
