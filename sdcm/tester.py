...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):
    ...
    def get_nemesis_class(self):
        ...
        # Normalize nemesis_class_name to a list (legacy-dict config may store raw strings).
        if isinstance(list_class_name, str):
            list_class_name = [list_class_name]
...
||||||| parent of 4b1bc03a8 (feature(tester): run gemini through run_stress_thread in the provision test)
=======
        stop_test_on_failure = False if not self.params.get("stop_test_on_stress_failure") else stop_test_on_failure
>>>>>>> 4b1bc03a8 (feature(tester): run gemini through run_stress_thread in the provision test)
