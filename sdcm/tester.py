...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):
    ...
    def pre_create_alternator_backuped_tables(self, **kwargs):
        ...
        if self.params.get('alternator_port'):
            ...
            schema = self.params.get("dynamodb_primarykey_type")
            table_name = self.params.get('alternator_table_name') or alternator.consts.TABLE_NAME
            self.alternator.create_table(node=node, schema=schema, table_name=table_name,
                                         tablets_enabled=tablets_enabled and tablets_support_lwt,
                                         **kwargs)
    ...
    def verify_alternator_tables_features(self, table_names: list = None, **kwargs):
        node = self.db_cluster.nodes[0]
        if table_names:
            for table_name in table_names:
                self.alternator.verify_table_features(node, table_name, **kwargs)
