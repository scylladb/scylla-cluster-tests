...
from sdcm.cluster import BaseCluster, NoMonitorSet, SCYLLA_DIR, TestConfig, UserRemoteCredentials, BaseLoaderSet, BaseMonitorSet, BaseScyllaCluster, BaseNode, MINUTE_IN_SEC
...
from sdcm.provision.helpers.certificate import create_ca, update_certificate, cleanup_ssl_config, CLIENT_FACING_CERTFILE, CLIENT_FACING_KEYFILE, CA_CERT_FILE, SCYLLA_SSL_CONF_DIR
...
from sdcm.utils.aws_utils import init_monitoring_info_from_params, get_ec2_services, get_common_params, init_db_info_from_params, ec2_ami_get_root_device_name
...
from sdcm.utils.ldap import LDAP_USERS, LDAP_PASSWORD, LDAP_ROLE, LDAP_BASE_OBJECT, LdapConfigurationError, LdapServerType
...
from sdcm.results_analyze import PerformanceResultsAnalyzer, SpecifiedStatsPerformanceAnalyzer, LatencyDuringOperationsPerformanceAnalyzer
...
from sdcm.utils.threads_and_processes_alive import gather_live_processes_and_dump_to_file, gather_live_threads_and_dump_to_file
...
def teardown_on_exception(method):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            TestFrameworkEvent(source=args[0].__class__.__name__, source_method="SetUp", exception=exc).publish_or_dump()
...
def update_cqlshrc(cqlshrc_file: str) -> None:
    ...
    config["ssl"] = {"validate": "true", "certfile": f"{SCYLLA_SSL_CONF_DIR / CA_CERT_FILE.name}", "userkey": f"{SCYLLA_SSL_CONF_DIR / CLIENT_FACING_KEYFILE.name}", "usercert": f"{SCYLLA_SSL_CONF_DIR / CLIENT_FACING_CERTFILE.name}"}
...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):
    ...
    def _init_test_duration(self):
        ...
        if self._stress_duration:
            self._duration = int(self._prepare_stress_duration) + int(self._stress_duration) + self.test_config.TEST_WARMUP_TEARDOWN

            self.log.info("SCT Test duration: %s; Stress duration: %s; Prepare duration: %s", self._duration, self._stress_duration, self._prepare_stress_duration)
        else:
    ...
    def init_argus_run(self):
        try:
            ...
            self.test_config.argus_client().set_sct_runner(public_ip=get_sct_runner_ip(), private_ip=get_my_ip(), region="N/A", backend=self.params.get("cluster_backend"))
    ...
    def start_argus_heartbeat_thread(self) -> threading.Event:
        ...
        thread = threading.Thread(name="argus-heartbeat", target=send_argus_heartbeat, kwargs={"client": self.test_config.argus_client(), "stop_signal": thread_stop_signal})
        thread.daemon = True
        thread.start()
    ...
    def generate_scylla_server_package(self) -> Package:
        ...
        expr = re.compile(r"(?P<version>(?P<main>[\w.~]+)-(0.)?(?P<date>[0-9]{8,8}).(?P<commit>\w+).) with build-id (?P<build_id>[\dabcdef]+)")
    ...
    def argus_collect_manager_version(self):
        ...
        if self.monitors.nodes and self.params.get("use_mgmt") and not self.k8s_clusters:
            ...
            client_version, server_version, *_ = (version[0].split(":")[1].strip() for version in manager_tool.sctool.version)
            manager_packages = [Package(name="scylla-manager-client", date="", version=client_version, revision_id="", build_id=""), Package(name="scylla-manager-server", date="", version=server_version, revision_id="", build_id="")]
    ...
    def generate_operator_packages(self) -> list[Package]:
        ...
        if self.k8s_clusters:
            ...
            operator_packages.append(Package(name="operator-chart", date="", version=operator_helm_chart_version, revision_id="", build_id=""))
            operator_image_version = self.k8s_clusters[0].get_operator_image()
            operator_packages.append(Package(name="operator-image", date="", version=operator_image_version, revision_id="", build_id=""))

            operator_helm_repo = self.params.get("k8s_scylla_operator_helm_repo")
            operator_packages.append(Package(name="operator-helm-repo", date="", version=operator_helm_repo, revision_id="", build_id=""))
    ...
    def _is_test_error(self, events: dict[str, list[str]]) -> bool:
        ...
        def check_error(event: str):
            errors = [
                ...
                re.compile(r"source=[\w]+.SetUp\(\).+exception=403 FORBIDDEN QUOTA_EXCEEDED", re.IGNORECASE | re.DOTALL),
    ...
    def argus_finalize_test_run(self):
        try:
            ...
            last_events = get_events_grouped_by_category(limit=last_events_limit, _registry=self.events_processes_registry)
            events_sorted = []
    ...
    def configure_ldap(self, node, use_ssl=False):
        ...
        role_entry = [f"cn={ldap_role},{LDAP_BASE_OBJECT}", ["groupOfUniqueNames", "simpleSecurityObject", "top"], {"uniqueMember": unique_members_list, "userPassword": user_password}]
        self.localhost.add_ldap_entry(ip=ldap_address[0], ldap_port=ldap_address[1], user=ldap_username, password=LDAP_PASSWORD, ldap_entry=role_entry)

        organizational_unit_entry = [f"ou=Person,{LDAP_BASE_OBJECT}", ["organizationalUnit", "top"], {"ou": "Person"}]
        self.localhost.add_ldap_entry(ip=ldap_address[0], ldap_port=ldap_address[1], user=ldap_username, password=LDAP_PASSWORD, ldap_entry=organizational_unit_entry)
        ...
        for user in [self.params.get("authenticator_user")] + LDAP_USERS:
            ...
            user_entry = [f"uid={user},ou=Person,{LDAP_BASE_OBJECT}", ["uidObject", "organizationalPerson", "top"], {"userPassword": password, "sn": "PersonSn", "cn": "PersonCn"}]
            self.localhost.add_ldap_entry(ip=ldap_address[0], ldap_port=ldap_address[1], user=ldap_username, password=LDAP_PASSWORD, ldap_entry=user_entry)
    ...
    def _init_test_timeout_thread(self) -> threading.Timer:
        ...
        self.log.info("Test start time %s, duration is %s and timeout set to %s", start_time, self.test_duration, end_time)
        ...
        thread = threading.Timer(60 * int(self.test_duration), kill_the_test, kwargs={"start": self.start_time, "duration": self.test_duration})
        thread.daemon = True
        thread.start()
        return thread
    ...
    def _ensure_scylla_qa_internal(self):
        ...
        if not scylla_qa_internal_path.exists():
            ...
            try:
                clone_repo(remoter=LOCALRUNNER, repo_url="git@github.com:scylladb/scylla-qa-internal.git", destination_dir_name=str(scylla_qa_internal_path), clone_as_root=False, branch="master")
    ...
    def reliable_replication_factor(self) -> int:
        ...
        rf_candidate = max([min_nodes_dc - 1, 1]) if is_tablets_feature_enabled(self.db_cluster.nodes[0]) else min_nodes_dc
    ...
    def is_encrypt_keys_needed(self):
        ...
        return append_scylla_yaml and ("system_key_directory" in append_scylla_yaml or "system_info_encryption" in append_scylla_yaml or "kmip_hosts" in append_scylla_yaml)
    ...
    def prepare_azure_kms(self) -> None:  # noqa: PLR0911
        ...
        try:
            ...
            InfoEvent(message=f"When checking if should enable Azure KMS, version check raised an error: {e}. Trying to enable it anyway.", severity=Severity.ERROR).publish()
    ...
    def prepare_gcp_kms(self) -> None:  # noqa: PLR0911
        ...
        append_scylla_yaml["gcp_hosts"][gcp_host] = {"gcp_project_id": key_info["project_id"], "gcp_location": key_info["location"], "master_key": key_info["key_uri"]}
    ...
    def setUp(self):  # noqa: PLR0912, PLR0915
        ...
        if cluster_backend in ("aws", "k8s-eks"):
            self.test_config.set_multi_region((self.params.get("simulated_regions") or 0) > 1 or len(self.params.region_names) > 1)
        elif cluster_backend in ("gce", "k8s-gke"):
            self.test_config.set_multi_region((self.params.get("simulated_regions") or 0) > 1 or len(self.params.gce_datacenters) > 1)
        ...
        if not self.params.get("cluster_backend").startswith("k8s") and any([self.params.get("client_encrypt"), self.params.get("server_encrypt")]):
                ...
                if self.params.get("use_ldap"):
                    with temp_authenticator(db_cluster.nodes[0], "org.apache.cassandra.auth.PasswordAuthenticator"):
                        ...
                        if self.params.get("ldap_server_type") == LdapServerType.MS_AD:
                            change_default_password(node=db_cluster.nodes[0], user=self.params.get("authenticator_user"), password=self.params.get("authenticator_password"))
        ...
        def _create_monitors():
            ...
            if self.monitors_multitenant:
                monitors_init_in_parallel = ParallelObject(timeout=3600, objects=[[monitor] for monitor in self.monitors_multitenant], num_workers=len(self.monitors_multitenant))
                monitors_init_in_parallel.run(func=(lambda m: m.wait_for_init()), unpack_objects=True, ignore_exceptions=False)
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(_create_db_clusters), executor.submit(_create_loaders), executor.submit(_create_monitors), executor.submit(_create_gemini_cluster)]
    ...
    def pre_create_alternator_tables(self):
        ...
        if self.params.get("alternator_port"):
            ...
            self.alternator.create_table(node=node, schema=schema, tablets_enabled=tablets_enabled and tablets_support_lwt and tablets_supported_alternator_workload)
            self.alternator.create_table(node=node, schema=schema, isolation=alternator.enums.WriteIsolation.FORBID_RMW, table_name=NO_LWT_TABLE_NAME, tablets_enabled=tablets_enabled and tablets_supported_alternator_workload)
    ...
    def get_nemesis_class(self):
        ...
        for i, nemesis_name in enumerate(nemesis_class_names):
            ...
            nemesis_threads.append({"nemesis": getattr(nemesis, nemesis_name), "nemesis_selector": nemesis_selector, "nemesis_seed": int(nemesis_seeds[i % len(nemesis_seeds)]) if nemesis_seeds else None})
    ...
    def get_cluster_gce(self, loader_info, db_info, monitor_info):  # noqa: PLR0912, PLR0914
        ...
        cluster_additional_disks = {"pd-ssd": self.params.get("gce_pd_ssd_disk_size_db"), "pd-standard": self.params.get("gce_pd_standard_disk_size_db")}
    ...
    def get_cluster_azure(self, loader_info, db_info, monitor_info):
        ...
        for region in regions:
            provisioners.append(provisioner_factory.create_provisioner(backend="azure", test_id=test_id, region=region, availability_zone=self.params.get("availability_zone")))
        if db_info["n_nodes"] is None:
            n_db_nodes = self.params.get("n_db_nodes")
            if isinstance(n_db_nodes, int):  # legacy type
                db_info["n_nodes"] = [n_db_nodes]
        ...
        self.db_cluster = ScyllaAzureCluster(
            image_id=azure_image, root_disk_size=db_info["disk_size"], instance_type=db_info["type"], provisioners=provisioners, n_nodes=db_info["n_nodes"], user_name=self.params.get("azure_image_username"), **common_params
        )
    ...
    def get_cluster_aws(self, loader_info, db_info, monitor_info):
        ...
        if loader_info["device_mappings"] is None:
            if loader_info["disk_size"]:
                loader_info["device_mappings"] = [
                    {"DeviceName": ec2_ami_get_root_device_name(image_id=self.params.get("ami_id_loader").split()[0], region_name=regions[0]), "Ebs": {"VolumeSize": loader_info["disk_size"], "VolumeType": "gp3"}}
                ]
            else:
                loader_info["device_mappings"] = []
        ...
        def _get_all_zones_common_params() -> list[dict]:
            ...
            availability_zones = [zone[-1] for zone in aws_region.get_availability_zones_for_instance_type(self.params.get("instance_type_db"))]
            for zone in availability_zones:
                all_zones_common_params.append(get_common_params(params=self.params, regions=regions, credentials=self.credentials, services=services, availability_zone=zone))
            return all_zones_common_params

        common_params = get_common_params(params=self.params, regions=regions, credentials=self.credentials, services=services)
        ...
        def _create_auto_zone_scylla_aws_cluster():
            ...
            for cl_zone_params in _get_all_zones_common_params():
                ...
                try:
                    return ScyllaAWSCluster(ec2_ami_id=self.params.get("ami_id_db_scylla").split(), ec2_ami_username=self.params.get("ami_db_scylla_user"), **cl_zone_params)
        ...
        def _get_instance_params() -> dict:
            return dict(ec2_instance_type=db_info["type"], ec2_block_device_mappings=db_info["device_mappings"], n_nodes=db_info["n_nodes"])
        ...
        def create_cluster(db_type="scylla"):
            ...
            if db_type == "scylla":
                ...
                return ScyllaAWSCluster(ec2_ami_id=self.params.get("ami_id_db_scylla").split(), ec2_ami_username=self.params.get("ami_db_scylla_user"), **cl_params)
            elif db_type == "cassandra":
                return CassandraAWSCluster(ec2_ami_id=self.params.get("ami_id_db_cassandra").split(), ec2_ami_username=self.params.get("ami_db_cassandra_user"), **cl_params)
    ...
    def get_cluster_docker(self):
        ...
        container_node_params = dict(docker_image=self.params.get("docker_image"), docker_image_tag=self.params.get("scylla_version"), node_key_file=self.credentials[0].key_file)
        ...
        self.loaders = cluster_docker.LoaderSetDocker(n_nodes=self.params.get("n_loaders"), **container_node_params, **common_params)
        self.monitors = cluster_docker.MonitorSetDocker(n_nodes=self.params.get("n_monitor_nodes"), targets=dict(db_cluster=self.db_cluster, loaders=self.loaders), **common_params)

    def get_cluster_baremetal(self):
        baremetal_info: cluster_baremetal.BareMetalCredentials = KeyStore().get_baremetal_config(self.params.get("s3_baremetal_config"))
        user_credentials = self.params.get("user_credentials_path")
        self.credentials.append(UserRemoteCredentials(key_file=user_credentials))
        params = dict(
    ...
    def get_cluster_k8s_local_kind_cluster(self):
        ...
        auxiliary_pool = mini_k8s.MinimalK8SNodePool(k8s_cluster=k8s_cluster, name=k8s_cluster.AUXILIARY_POOL_NAME, num_nodes=self.params.get("k8s_n_auxiliary_nodes") or 2, image_type="fake-image-type", instance_type="fake-instance-type")
        ...
        if self.params.get("n_loaders"):
            loader_pool = mini_k8s.MinimalK8SNodePool(k8s_cluster=k8s_cluster, name=k8s_cluster.LOADER_POOL_NAME, num_nodes=self.params.get("n_loaders"), image_type="fake-image-type", instance_type="fake-instance-type")
            k8s_cluster.deploy_node_pool(loader_pool, wait_till_ready=False)

        scylla_pool = mini_k8s.MinimalK8SNodePool(k8s_cluster=k8s_cluster, name=k8s_cluster.SCYLLA_POOL_NAME, num_nodes=self.params.get("n_db_nodes"), image_type="fake-image-type", instance_type="fake-instance-type")
        ...
        if n_monitor_nodes:
            monitor_pool = mini_k8s.MinimalK8SNodePool(k8s_cluster=k8s_cluster, name=k8s_cluster.MONITORING_POOL_NAME, num_nodes=n_monitor_nodes, image_type="fake-image-type", instance_type="fake-instance-type")
    ...
    def _add_and_wait_for_cluster_nodes_in_parallel(clusters):
        ...
        add_nodes_in_parallel = ParallelObject(timeout=3600, objects=[[cluster] for cluster in clusters], num_workers=len(clusters))
    ...
    def get_cluster_k8s_gke(self, n_k8s_clusters: int):
        ...
        for i in range(n_k8s_clusters):
            self.k8s_clusters.append(
                gke.init_k8s_gke_cluster(
                    ...
                    cluster_uuid=(f"{self.test_config.test_id()[:8]}" if n_k8s_clusters < 2 else f"{self.test_config.test_id()[:6]}-{i + 1}"),
                    params=self.params,
                )
            )
        ParallelObject(timeout=7200, num_workers=n_k8s_clusters, objects=self.k8s_clusters).run(func=gke.deploy_k8s_gke_cluster, unpack_objects=True, ignore_exceptions=False)
        ...
        if self.params.get("n_monitor_nodes") > 0:
            for i in range(self.k8s_clusters[0].tenants_number):
                ...
                for k8s_cluster in self.k8s_clusters:
                    k8s_cluster.scylla_pods_ip_change_tracker_thread.register_callbacks(callbacks=self.monitors_multitenant[i].reconfigure_scylla_monitoring, namespace=self.db_clusters_multitenant[i].namespace)
    ...
    def get_cluster_k8s_eks(self, n_k8s_clusters: int):
        ...
        ParallelObject(timeout=7200, num_workers=n_k8s_clusters, objects=self.k8s_clusters).run(func=eks.deploy_k8s_eks_cluster, unpack_objects=True, ignore_exceptions=False)
        ...
        for i in range(self.k8s_clusters[0].tenants_number):
            ...
            if self.params.get("use_mgmt"):
                for k8s_cluster in self.k8s_clusters:
                    k8s_cluster.create_iamserviceaccount_for_s3_access(db_cluster_name=self.db_clusters_multitenant[i].scylla_cluster_name, namespace=self.db_clusters_multitenant[i].namespace)
        self.db_cluster = self.db_clusters_multitenant[0]
        ...
        # Deploy main VM-based monitoring
        monitor_info = {"n_nodes": None, "type": None, "disk_size": None, "disk_type": None, "n_local_ssd": None, "device_mappings": None}
        init_monitoring_info_from_params(monitor_info, params=self.params, regions=region_names)
        if monitor_info["n_nodes"]:
            common_params = get_common_params(params=self.params, regions=region_names, credentials=self.credentials, services=get_ec2_services(region_names))
    ...
    def get_cluster_cloud(self, loader_info, db_info, monitor_info):  # noqa: PLR0912
        cloud_api_client = ScyllaCloudAPIClient(api_url=self.params.cloud_env_credentials["base_url"], auth_token=self.params.cloud_env_credentials["api_token"], raise_for_status=True)
        ...
        if cloud_provider == "aws":
            ...
            if loader_info["device_mappings"] is None:
                if loader_info["disk_size"]:
                    loader_info["device_mappings"] = [
                        {
                            "DeviceName": ec2_ami_get_root_device_name(image_id=self.params.get("ami_id_loader").split()[0], region_name=regions[0]),
                            "Ebs": {
                                "VolumeType": "gp3",
        ...
        loader_ips = [node.public_ip_address for node in self.loaders.nodes if node.public_ip_address] if self.loaders else []

        self.log.info("Creating Scylla Cloud cluster with allowed IPs: client + %d loader nodes", len(loader_ips))
        self.db_cluster = ScyllaCloudCluster(cloud_api_client=cloud_api_client, user_prefix=user_prefix, n_nodes=db_info["n_nodes"][0], params=self.params, add_nodes=True, allowed_ips=loader_ips)
    ...
    def init_resources(self, loader_info=None, db_info=None, monitor_info=None):
        if loader_info is None:
            loader_info = {"n_nodes": None, "type": None, "disk_size": None, "disk_type": None, "n_local_ssd": None, "device_mappings": None}
        if db_info is None:
            db_info = {"n_nodes": None, "type": None, "disk_size": None, "disk_type": None, "n_local_ssd": None, "device_mappings": None}
    ...
    def run_stress_cassandra_thread(  # noqa: PLR0913
        self, stress_cmd, duration=None, stress_num=1, keyspace_num=1, profile=None, prefix="", round_robin=False, stats_aggregate_cmds=True, keyspace_name=None, compaction_strategy="", stop_test_on_failure=True, params=None, **_
    ):
        # stress_cmd = self._cs_add_node_flag(stress_cmd)
        if duration:
    ...
    def run_stress_thread_bench(self, stress_cmd, duration=None, round_robin=False, stats_aggregate_cmds=True, stop_test_on_failure=True, **_):
        if duration:
            timeout = self.get_duration(duration)
        elif self._stress_duration and "-duration=" in stress_cmd:
            timeout = self.get_duration(self._stress_duration)
    ...
    def run_stress_thread_harry(self, stress_cmd, duration=None, round_robin=False, stats_aggregate_cmds=True, stop_test_on_failure=True, **_):
        timeout = self.get_duration(duration)
        ...
        harry_thread = CassandraHarryThread(
            loader_set=self.loaders, stress_cmd=stress_cmd, timeout=timeout, node_list=self.db_cluster.nodes, round_robin=round_robin, stop_test_on_failure=stop_test_on_failure, params=self.params, credentials=self.db_cluster.get_db_auth()
        )
        harry_thread.run()
        return harry_thread

    def run_ycsb_thread(self, stress_cmd, duration=None, stress_num=1, prefix="", round_robin=False, stats_aggregate_cmds=True, **_):
        timeout = self.get_duration(duration)

        if self.create_stats:
            self.update_stress_cmd_details(stress_cmd, prefix, stresser="ycsb", aggregate=stats_aggregate_cmds)

        return YcsbStressThread(loader_set=self.loaders, stress_cmd=stress_cmd, timeout=timeout, stress_num=stress_num, node_list=self.db_cluster.nodes, round_robin=round_robin, params=self.params, cluster_tester=self).run()

    def run_latte_thread(self, stress_cmd, duration=None, stress_num=1, prefix="", round_robin=False, stats_aggregate_cmds=True, stop_test_on_failure=True, **_):
        if duration:
            timeout = self.get_duration(duration)
        elif self._stress_duration and (" --duration" in stress_cmd or " -d" in stress_cmd):
            timeout = self.get_duration(self._stress_duration)
            stress_cmd = re.sub(r"\s(?:--duration|-d)[ =]\d+[smhd]\s", f" --duration {self._stress_duration}m ", stress_cmd)
        else:
            timeout = get_timeout_from_stress_cmd(stress_cmd) or self.get_duration(duration)
        ...
        return LatteStressThread(
            loader_set=self.loaders, stress_cmd=stress_cmd, timeout=timeout, stress_num=stress_num, node_list=self.db_cluster.nodes, round_robin=round_robin, stop_test_on_failure=stop_test_on_failure, params=self.params
        ).run()

    def run_hydra_kcl_thread(self, stress_cmd, duration=None, stress_num=1, prefix="", round_robin=False, stats_aggregate_cmds=True, **_):
        timeout = self.get_duration(duration)

        if self.create_stats:
            self.update_stress_cmd_details(stress_cmd, prefix, stresser="hydra-kcl", aggregate=stats_aggregate_cmds)

        return KclStressThread(loader_set=self.loaders, stress_cmd=stress_cmd, timeout=timeout, stress_num=stress_num, node_list=self.db_cluster.nodes, round_robin=round_robin, params=self.params).run()

    def run_nosqlbench_thread(self, stress_cmd, duration=None, stress_num=1, prefix="", round_robin=False, stats_aggregate_cmds=True, stop_test_on_failure=True, **_):
        timeout = self.get_duration(duration)
    ...
    def run_table_compare_thread(self, stress_cmd, duration=None, stress_num=1, round_robin=False, **_):
        ...
        return CompareTablesSizesThread(loader_set=self.loaders, stress_cmd=stress_cmd, timeout=timeout, stress_num=stress_num, node_list=self.db_cluster.nodes, round_robin=round_robin, params=self.params).run()

    def run_ndbench_thread(self, stress_cmd, duration=None, stress_num=1, prefix="", round_robin=False, stats_aggregate_cmds=True, **_):
        timeout = self.get_duration(duration)

        if self.create_stats:
            self.update_stress_cmd_details(stress_cmd, prefix, stresser="ndbench", aggregate=stats_aggregate_cmds)

        return NdBenchStressThread(loader_set=self.loaders, stress_cmd=stress_cmd, timeout=timeout, stress_num=stress_num, node_list=self.db_cluster.nodes, round_robin=round_robin, params=self.params).run()

    def run_cdclog_reader_thread(self, stress_cmd, duration=None, stress_num=1, prefix="", round_robin=False, stats_aggregate_cmds=True, enable_batching=True, keyspace_name=None, base_table_name=None):
        timeout = self.get_duration(duration)
    ...
    def run_gemini(self, cmd, duration=None):
        ...
        return GeminiStressThread(test_cluster=self.db_cluster, oracle_cluster=self.cs_db_cluster, loaders=self.loaders, stress_cmd=cmd, timeout=timeout, params=self.params).run()
    ...
    def verify_gemini_results(self, queue):
        ...
        if self.create_stats:
            ...
            self.update({"status": stats["status"], "test_details": {"status": stats["status"]}, "errors": stats["errors"]})
        return stats
    ...
    def run_tombstone_gc_verification_thread(self, duration=None, interval=600, propagation_delay_in_seconds=3600, **kwargs):
        ...
        TombstoneGcVerificationThread(
            db_cluster=self.db_cluster, duration=self.get_duration(duration), interval=interval, termination_event=self.db_cluster.nemesis_termination_event, propagation_delay_in_seconds=propagation_delay_in_seconds, **kwargs
        ).start()
    ...
    def wait_validate_keyspace_existence(self, session, keyspace_name, timeout=180, step=5):
        ...
        does_keyspace_exist = wait.wait_for(func=self.is_keyspace_in_cluster, step=step, text=text, timeout=timeout, session=session, keyspace_name=keyspace_name, throw_exc=False)
    ...
    def create_table(
        ...
        if additional_columns == "":
            query = "CREATE COLUMNFAMILY IF NOT EXISTS %s (key %s, c varchar, v varchar, PRIMARY KEY(key, c)) WITH comment='test cf'" % (name, key_type)
        else:
            query = "CREATE COLUMNFAMILY IF NOT EXISTS %s (key %s PRIMARY KEY%s) WITH comment='test cf'" % (name, key_type, additional_columns)
    ...
    def _wait_for_view(self, scylla_cluster, session, key_space, view):
        ...
        def _view_build_finished(live_nodes_amount):
            result = self.rows_to_list(session.execute("SELECT status FROM system_distributed.view_build_status WHERE keyspace_name='{0}' AND view_name='{1}'".format(key_space, view)))
    ...
    def _wait_for_view_build_start(self, session, key_space, view, seconds_to_wait=20):
        def _check_build_started():
            result = self.rows_to_list(session.execute("SELECT last_token FROM system.views_builds_in_progress WHERE keyspace_name='{0}' AND view_name='{1}'".format(key_space, view)))
    ...
    def copy_table(self, node, src_keyspace, src_table, dest_keyspace, dest_table, columns_list=None, copy_data=False):
        ...
        """
        result = True
        create_statement = "SELECT * FROM system_schema.table where table_name = '%s' and keyspace_name = '%s'" % (src_table, src_keyspace)
        if not self.create_table_as(node, src_keyspace, src_table, dest_keyspace, dest_table, create_statement, columns_list):
            return False

        if copy_data:
            try:
                result = self.copy_data_between_tables(node, src_keyspace, src_table, dest_keyspace, dest_table, columns_list)
    ...
    def copy_view(self, node, src_keyspace, src_view, dest_keyspace, dest_table, columns_list=None, copy_data=False):
        ...
        """
        result = True
        create_statement = "SELECT * FROM system_schema.views where view_name = '%s' and keyspace_name = '%s'" % (src_view, src_keyspace)
        self.log.debug("Start create table with statement: {}".format(create_statement))
        if not self.create_table_as(node, src_keyspace, src_view, dest_keyspace, dest_table, create_statement, columns_list):
            return False
        self.log.debug("Finish create table")
        if copy_data:
            try:
                result = self.copy_data_between_tables(node, src_keyspace, src_view, dest_keyspace, dest_table, columns_list)
            except Exception as error:  # noqa: BLE001
    ...
    def create_table_as(self, node, src_keyspace, src_table, dest_keyspace, dest_table, create_statement, columns_list=None):
        ...
        with self.db_cluster.cql_connection_patient(node) as session:
            ...
            if result:
                result = session.execute("SELECT * FROM {keyspace}.{table} LIMIT 1".format(keyspace=src_keyspace, table=src_table))
                ...
                if not columns:
                    self.log.error("Wrong supplied columns list: %s. The columns do not exist in the %s.%s table", columns, src_keyspace, src_table)
                    return False

                for column in columns_list or result.column_names:
                    column_kind = session.execute("select kind from system_schema.columns where keyspace_name='{ks}' and table_name='{name}' and column_name='{column}'".format(ks=src_keyspace, name=src_table, column=column))
                ...
                create_cql = "create table {keyspace}.{name} ({columns}, PRIMARY KEY ({pk}))".format(keyspace=dest_keyspace, name=dest_table, columns=", ".join(["%s %s" % (c[0], c[1]) for c in columns]), pk=", ".join(primary_keys))
    ...
    def copy_data_between_tables(self, node, src_keyspace, src_table, dest_keyspace, dest_table, columns_list=None):
        ...
        with self.db_cluster.cql_connection_patient(node, verbose=False) as session:
            ...
            statement = "SELECT {columns} FROM {keyspace}.{table}".format(keyspace=src_keyspace, table=src_table, columns=",".join(columns_list) if columns_list else "*")
            ...
            insert_statement = session.prepare("insert into {keyspace}.{name} ({columns}) values ({values})".format(keyspace=dest_keyspace, name=dest_table, columns=", ".join(columns), values=", ".join(["?" for _ in columns])))
            ...
            results = execute_concurrent_with_args(session=session, statement=insert_statement, parameters=source_table_rows, concurrency=max_workers, results_generator=True)

            try:
                succeeded_rows = sum(1 for (success, result) in results if success)
                if succeeded_rows != len(source_table_rows):
                    self.log.warning("Problem during copying data. Not all rows were inserted. Rows expected to be inserted: %s; Actually inserted rows: %s.", len(source_table_rows), succeeded_rows)
                    return False
            except Exception as exc:  # noqa: BLE001
            ...
            if result:
                if result.current_rows[0].count != len(source_table_rows):
                    self.log.warning("Problem during copying data. Rows in source table: %s; Rows in destination table: %s.", len(source_table_rows), len(result.current_rows))
                    return False
    ...
    def get_describecluster_info(self) -> Optional[ClusterInformation]:
        ...
        if describecluster_output.ok:
            ...
            return ClusterInformation(name=name, snitch=snitch, partitioner=partitioner, schema_versions=schema_versions)
    ...
    def save_schema(self):
        ...
        for node in self.db_cluster.nodes:
            ...
            self.save_cqlsh_output_in_file(node=node, cmd="select JSON * from system_schema.tables", log_file="system_schema_tables.log")
            self.save_cqlsh_output_in_file(node=node, cmd="select JSON * from system.truncated", log_file="system_truncated.log")
            self.save_cqlsh_output_in_file(node=node, cmd="desc schema with internals", log_file="schema_with_internals.log")
            break
    ...
    def fixture_event_system(self, setup_logging):
        ...
        start_events_device(log_dir=self.logdir, _registry=getattr(self, "_registry", None) or self.events_processes_registry)
        yield
    ...
    def _check_if_db_log_time_consistency_looks_good(self):
        ...
        if looks_good:
            ...
            logging.error("DB logs time consistency is NOT perfect, details:\n%s", yaml.safe_dump(result, sort_keys=False))
    ...
    def collect_sct_logs(self):
        ...
        for log_collector in [BaseSCTLogCollector, PythonSCTLogCollector]:
            s3_link.extend(log_collector([], self.test_config.test_id(), os.path.join(self.logdir, "collected_logs"), params=self.params).collect_logs(self.logdir))
        if s3_link:
    ...
    def populate_data_parallel(self, size_in_gb: int, replication_factor: int = 3, blocking=True, read=False):
        ...
        stress_fixed_params = (
            f" -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor={replication_factor}) compaction(strategy=LeveledCompactionStrategy)' -mode cql3 native -rate threads=200 -col 'size=FIXED(1024) n=FIXED(1)' "
        )
        stress_keys = "n="
        population = " -pop seq="
        ...
        for i in range(1, n_loaders + 1):
            stress_cmd = base_cmd + stress_keys + str(keys_per_node) + population + str(start) + ".." + str(keys_per_node * i) + stress_fixed_params
    ...
    def alter_table_encryption(self, table, scylla_encryption_options=None, upgradesstables=True):
        ...
        if not scylla_encryption_options:
            ...
            with self.db_cluster.cql_connection_patient(self.db_cluster.nodes[0]) as session:
                query = "ALTER TABLE {table} WITH scylla_encryption_options = {scylla_encryption_options};".format(table=table, scylla_encryption_options=scylla_encryption_options)
    ...
    def alter_test_tables_encryption(self, upgradesstables=True, stress_command=""):
        ...
        if (scylla_encryption_options := self.params.get("scylla_encryption_options")) and "write" in stress_command and "user_info_encryption" not in append_scylla_yaml:
    ...
    def get_num_of_hint_files(self, node):
        result = node.remoter.run("sudo find {0.scylla_hints_dir} -name *.log -type f| wc -l".format(self), verbose=True)
    ...
    def wait_for_hints_to_be_sent(self, node, num_dest_nodes):
        ...
        assert self.get_num_of_hint_files(node) <= hints_after_send_completed, "Waiting until the number of hint files will be %s." % hints_after_send_completed
    ...
    def wait_data_dir_reaching(self, size, node):
        query = '(sum(node_filesystem_size_bytes{{mountpoint="{0.scylla_dir}", instance=~"{1.private_ip_address}"}})-sum(node_filesystem_avail_bytes{{mountpoint="{0.scylla_dir}", instance=~"{1.private_ip_address}"}}))'.format(self, node)
    ...
    def check_latency_during_ops(self, hdr_tags: list[str]):
        ...
        results_analyzer = analyzer(es_index=self._test_index, email_recipients=self.params.get("email_recipients"), events=get_events_grouped_by_category(_registry=self.events_processes_registry))
        with open(self.latency_results_file, encoding="utf-8") as file:
            latency_results = json.load(file)
        self.log.debug("latency_results were loaded from file %s and its result is %s", self.latency_results_file, latency_results)
        ...
        if latency_results and self.create_stats:
            ...
            histogram_total_data = self.get_hdrhistogram(hdr_tags=hdr_tags, stress_operation=workload, start_time=start_time, end_time=end_time)
            histogram_data_by_interval = self.get_hdrhistogram_by_interval(hdr_tags=hdr_tags, stress_operation=workload, start_time=start_time, end_time=end_time)
            ...
            try:
                results_analyzer.check_regression(test_id=self._test_id, data=latency_results, node_benchmarks=benchmarks_results, email_subject_postfix=self.params.get("email_subject_postfix"))
            except Exception as exc:  # noqa: BLE001
                TestFrameworkEvent(message="Failed to check regression", trace=sys._getframe().f_back, source=self.__class__.__name__, source_method="check_regression", exception=exc).publish_or_dump()

    def check_regression(self):
        results_analyzer = PerformanceResultsAnalyzer(
            es_index=self._test_index, email_recipients=self.params.get("email_recipients"), events=get_events_grouped_by_category(_registry=self.events_processes_registry, limit=self.params.get("events_limit_in_email"))
        )
        is_gce = bool(self.params.get("cluster_backend") == "gce")
        benchmarks_results = self.db_cluster.get_node_benchmarks_results() if self.db_cluster else {}
        try:
            results_analyzer.check_regression(
                self._test_id, is_gce, email_subject_postfix=self.params.get("email_subject_postfix"), use_wide_query=True, node_benchmarks=benchmarks_results, extra_jobs_to_compare=self.params.get("perf_extra_jobs_to_compare")
            )
        except Exception as exc:  # noqa: BLE001
            TestFrameworkEvent(message="Failed to check regression", trace=sys._getframe().f_back, source=self.__class__.__name__, source_method="check_regression", exception=exc).publish_or_dump()
    ...
    def check_regression_with_baseline(self, subtest_baseline):
        ...
        try:
            results_analyzer.check_regression_with_subtest_baseline(
                self._test_id, base_test_id=self.test_config.test_id(), subtest_baseline=subtest_baseline, is_gce=is_gce, extra_jobs_to_compare=self.params.get("perf_extra_jobs_to_compare")
            )
        except Exception as exc:  # noqa: BLE001
            TestFrameworkEvent(message="Failed to check regression", trace=sys._getframe().f_back, source=self.__class__.__name__, source_method="check_regression", exception=exc).publish_or_dump()
    ...
    def check_regression_multi_baseline(self, subtests_info=None, metrics=None, email_subject=None):
        ...
        try:
            return results_analyzer.check_regression_multi_baseline(self._create_test_id(doc_id_with_timestamp=False), metrics=metrics, subtests_info=subtests_info, subject=email_subject)
        except Exception as exc:  # noqa: BLE001
            TestFrameworkEvent(message="Failed to check regression", trace=sys._getframe().f_back, source=self.__class__.__name__, source_method="check_regression", exception=exc).publish_or_dump()

    def check_specified_stats_regression(self, stats):
        perf_analyzer = SpecifiedStatsPerformanceAnalyzer(es_index=self._test_index, email_recipients=self.params.get("email_recipients"), events=get_events_grouped_by_category(_registry=self.events_processes_registry))
        try:
    ...
    def wait_for_no_tablets_splits(self, n=3, sleep_time=180):
        ...
        def _is_no_tablets_splits():
            ...
            assert results_set == {"none"} or not results_set, "Tablet splits or merges still in progress: %s" % results_set
    ...
    def collect_logs(self) -> None:
        ...
        self.log.info("Logs collected. Run command `hydra investigate show-logs %s' to get links", self.test_config.test_id())
    ...
    def get_used_capacity(self, node) -> float:
        ...
        fs_size_res = self.prometheus_db.query(query=filesystem_capacity_query, start=int(time.time()) - 5, end=int(time.time()))
        ...
        used_cap_res = self.prometheus_db.query(query=used_capacity_query, start=int(time.time()) - 5, end=int(time.time()))
    ...
    def save_email_data(self):
        ...
        grafana_screenshots = []
        try:
            grafana_screenshots = self.monitors.get_grafana_screenshots_from_all_monitors(self.start_time) if self.monitors else {}
    ...
    def _argus_add_relocatable_pkg(self, email_data):
        ...
        if relocatable_pkg := email_data.get("relocatable_pkg"):
            self.test_config.argus_client().submit_packages([Package(name="relocatable_pkg", date="", version=relocatable_pkg, revision_id="", build_id="")])
    ...
    def all_nodes_scylla_shards(self):
        ...
        for node in self.db_cluster.nodes:
            ...
            all_nodes_shards["live_nodes"].append({"name": node.name, "ip": f"{node.public_ip_address} | {node.private_ip_address}{f' | {ipv6}' if ipv6 else ''}", "shards": node.scylla_shards})
    ...
    def _get_common_email_data(self) -> Dict[str, Any]:
        ...
        config_file_name = ";".join(os.path.splitext(os.path.basename(cfg))[0] for cfg in self.params.get("config_files"))
        test_status = self.get_test_status()
        ...
        restore_monitor_job_base_link = "https://jenkins.scylladb.com/view/QA/job/QA-tools/job/hydra-show-monitor/parambuild/?"
        if build_id:
    ...
    def get_hdrhistogram(self, hdr_tags: list[str], stress_operation: str, start_time: float, end_time: float) -> dict[str, Any]:
        if not self.params["use_hdrhistogram"]:
            return {}
        self.log.info("Build HDR histogram (tags: %s) with start time: %s, end time: %s; for operation: %s", hdr_tags, start_time, end_time, stress_operation)
        histogram_data = make_hdrhistogram_summary(hdr_tags=hdr_tags, stress_operation=stress_operation, start_time=start_time, end_time=end_time, base_path=self.loaders.logdir)
        self.log.info("HDR histogram summary result: %s", histogram_data)
        return histogram_data[0] if histogram_data else {}

    def get_hdrhistogram_by_interval(self, hdr_tags: list[str], stress_operation: str, start_time: float, end_time: float, time_interval: int = 600) -> list[dict[str, Any]]:
        if not self.params["use_hdrhistogram"]:
            return []
        self.log.info("Build HDR histogram (tags: %s) with start time: %s, end time: %s, time interval: %s for operation: %s", hdr_tags, start_time, end_time, time_interval, stress_operation)
        histogram_data = make_hdrhistogram_summary_by_interval(hdr_tags=hdr_tags, stress_operation=stress_operation, path=self.loaders.logdir, start_time=start_time, end_time=end_time, interval=time_interval)
        self.log.info("HDR histogram summary result: %s", histogram_data)
