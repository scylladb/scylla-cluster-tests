...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(self, *args, **kwargs):  # pylint: disable=too-many-statements,too-many-locals,too-many-branches
        ...
        self.log.debug("IP used for SSH connections is '%s'", ip_ssh_connections)
    ...
    def _is_test_error(self, events: dict[str, list[str]]) -> bool:  # pylint: disable=no-self-use
        """
    ...
    def prepare_kms_host(self) -> None:
        version_supports_kms = (
            self.params.is_enterprise and ComparableScyllaVersion(self.params.scylla_version) >= "2023.1.3"
        )
        ...
        should_enable_kms = (
            version_supports_kms
            and backend_support_kms
            and not kms_configured_in_sct
            and not test_uses_oracle
            and not self.params.get("enterprise_disable_kms")
        )

        if should_enable_kms:
            self.params["scylla_encryption_options"] = (
                "{ 'cipher_algorithm' : 'AES/ECB/PKCS5Padding', 'secret_key_strength' : 128, 'key_provider': 'KmsKeyProviderFactory', 'kms_host': 'auto'}"  # pylint: disable=line-too-long
            )
    ...
    def get_nemesis_class(self):
        ...
        for i, nemesis_name in enumerate(nemesis_class_names):
            ...
            nemesis_threads.append({"nemesis": getattr(nemesis, nemesis_name), "nemesis_selector": nemesis_selector})
    ...
    def run_stress_thread(
        ...
        keyspace_num=1,
        profile=None,
        prefix="",  # pylint: disable=too-many-arguments
        round_robin=False,
    ...
    def create_keyspace(self, keyspace_name, replication_factor):
        ...
        with self.db_cluster.cql_connection_patient(execution_node) as session:
            ...
            execution_result = session.execute(
                "CREATE KEYSPACE IF NOT EXISTS %s WITH replication=%s" % (keyspace_name, str(replication_strategy))
            )
        if execution_result:
    ...
    def create_table(  # pylint: disable=too-many-arguments,too-many-branches
        self,
        name,
        key_type="varchar",
    ...
    def create_materialized_view(
        ...
        query = (
            "CREATE MATERIALIZED VIEW {ks}.{mv_name} AS SELECT {mv_columns} FROM {ks}.{table_name} "
            "WHERE {where_clause} PRIMARY KEY ({pk}, {cl}) WITH comment='test MV'".format(
                ks=ks_name,
                mv_name=mv_name,
                mv_columns=mv_columns_str,
                table_name=base_table_name,
                where_clause=" and ".join(wc for wc in where_clause),
                pk=pk_clause,
                cl=cl_clause,
            )
        )
    ...
    def copy_table(
        ...
        src_keyspace,
        src_table,
        dest_keyspace,  # pylint: disable=too-many-arguments
        dest_table,
        columns_list=None,
        copy_data=False,
    ):
    ...
    def get_describecluster_info(self) -> Optional[ClusterInformation]:  # pylint: disable=too-many-locals
        ...
        if describecluster_output.ok:
            ...
            partitioner_pattern = re.compile("((?<=Partitioner: )[\w.]*)")  # pylint: disable=anomalous-backslash-in-string
            schema_versions_pattern = re.compile("([a-z0-9-]{36}: \[[\d., ]*\])")  # pylint: disable=anomalous-backslash-in-string
    ...
    def check_latency_during_ops(self):
        ...
        if latency_results and self.create_stats:
            ...
            histogram_total_data = self.get_cs_range_histogram(
                stress_operation=workload, start_time=start_time, end_time=end_time
            )
            histogram_data_by_interval = self.get_cs_range_histogram_by_interval(
                stress_operation=workload, start_time=start_time, end_time=end_time
            )
    ...
    def check_regression_with_baseline(self, subtest_baseline):
        ...
        try:
            results_analyzer.check_regression_with_subtest_baseline(
                self._test_id, base_test_id=self.test_config.test_id(), subtest_baseline=subtest_baseline, is_gce=is_gce
            )
        except Exception as ex:  # pylint: disable=broad-except
            self.log.exception("Failed to check regression: %s", ex)
    ...
    def check_regression_multi_baseline(
        self,
        subtests_info=None,  # pylint: disable=inconsistent-return-statements
        metrics=None,
        email_subject=None,
    ):
        results_analyzer = PerformanceResultsAnalyzer(
    ...
    def get_cs_range_histogram(
        ...
        if not self.params["use_hdr_cs_histogram"]:
            return {}
        self.log.info(
            "Build HDR histogram with start time: %s, end time: %s; for operation: %s",
            start_time,
            end_time,
            stress_operation,
        )
    ...
    def get_cs_range_histogram_by_interval(
        ...
        if not self.params["use_hdr_cs_histogram"]:
            return []
        self.log.info(
            "Build HDR histogram with start time: %s, end time: %s, time interval: %s for operation: %s",
            start_time,
            end_time,
            time_interval,
            stress_operation,
