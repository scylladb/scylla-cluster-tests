...
class LoaderCluster(SctClusterBase):
    _es_data_mapping = {"nodes": "n_loaders", "type": "instance_type_loader", "gce_type": "gce_instance_type_db", "ami_id": "ami_id_db_scylla"}


class DbCluster(SctClusterBase):
    _es_data_mapping = {"nodes": "n_db_nodes", "type": "instance_type_db", "gce_type": "gce_instance_type_db", "ami_id": "ami_id_db_scylla"}


class MonitorCluster(SctClusterBase):
    _es_data_mapping = {"nodes": "n_monitor_nodes", "type": "instance_type_monitor", "gce_type": "gce_instance_type_monitor", "ami_id": "ami_id_db_scylla"}
...
class TestResultClass(ClassBase):
    ...
    def _get_es_query_from_instance_data(cls, instance_data: dict):
        ...
        for data_path, data_value in instance_data.items():
            ...
            if es_data_path is None:
                raise ValueError(f"Unknown data path {data_path} only following are known:\n" + "\n".join(mappings.keys()))
    ...
    def get_same_tests_query(self):
        list_of_attributes = ["loader_cluster.type", "loader_cluster.gce_type", "loader_cluster.nodes", "db_cluster.type", "db_cluster.gce_type", "db_cluster.nodes"]
