...
class LatencyDuringOperationsPerformanceAnalyzer(BaseResultsAnalyzer):
    ...
    def _get_previous_results(self, test_doc, is_gce=False):
        ...
        test_results = self._es.search(  # pylint: disable=unexpected-keyword-arg; pylint doesn't understand Elasticsearch code
            index=self._es_index, doc_type=self._es_doc_type, q=query, filter_path=filter_path, size=self._limit
        )
        if not test_results:
                ...
                for version in results_per_nemesis_by_version[nemesis]:
                    try:
                        best_results[nemesis][version] = sorted(
                            ...
                            key=lambda obj: obj.get("average_time_operation_in_sec"),
                        )[0]
                    except IndexError:
                        best_results[nemesis][version] = {}

                best_results[nemesis] = {
                    per_version: best_results[nemesis][per_version]
                    for per_version in sorted(best_results[nemesis].keys(), key=lambda version: version, reverse=True)
                }
            return best_results
...
class PerformanceResultsAnalyzer(BaseResultsAnalyzer):
    ...
    def check_regression(
        ...
        tests_filtered = self._es.search(  # pylint: disable=unexpected-keyword-arg
            index=self._es_index,
            q=query,
            filter_path=filter_path,
            size=self._limit,
...
class ThroughputLatencyGradualGrowPayloadPerformanceAnalyzer(BaseResultsAnalyzer):
    ...
    def __init__(self, es_index, es_doc_type, email_recipients=(), logger=None, events=None):  # pylint: disable=too-many-arguments
        super().__init__(
            ...
            es_doc_type=es_doc_type,
            email_recipients=email_recipients,
            email_template_fp="results_incremental_throughput_increase.html",
            logger=logger,
            events=events,
        )
...
class SearchBestThroughputConfigPerformanceAnalyzer(BaseResultsAnalyzer):
    ...
    def check_regression(self, test_name, setup_details, test_results):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        results = {"test_name": test_name, "setup_details": setup_details, "test_results": test_results}
        email_data = {"email_body": results, "template": self._email_template_fp}
        self.save_email_data_file(subject, email_data, file_path="email_data.json")
