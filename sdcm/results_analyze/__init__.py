...
class PerformanceResultsAnalyzer(BaseResultsAnalyzer):
    ...
    def _test_stats(self, test_doc):
        ...
        if (
            ...
            self.log.error(
                "Cannot find one of the fields: results, results.stats_average, results.stats_total for test id: {}!".format(
                    test_doc["_id"]
                )
            )
            return None
    ...
    def check_regression_multi_baseline(  # noqa: PLR0912, PLR0914, PLR0915
        ...
        if subject is None:
            subject = "Performance Regression Compare Results - {test.test_name} - {test.software.scylla_server_any.version.as_string}".format(
                test=rp_main_test
            )
        else:
