...
from sdcm.utils.es_queries import QueryFilter, PerformanceFilterYCSB, PerformanceFilterScyllaBench, PerformanceFilterCS, CDCQueryFilterCS, LatencyWithNemesisQueryFilter
...
class BaseResultsAnalyzer:
    ...
    def _get_setup_details(test_doc, is_gce):
        ...
        for setup_param in QueryFilter(test_doc, is_gce).setup_instance_parameters():
            setup_details.update([(setup_param.replace("gce_", ""), test_doc["_source"]["setup_details"].get(setup_param))])
    ...
    def _test_version(self, test_doc):
        if test_doc["_source"].get("base_target_versions"):
            ...
            return {"base_version": base_version, "base_build_id": base_build_id, "target_version": target_version, "target_build_id": target_build_id}
    ...
    def render_to_html(self, results, html_file_path="", template=None):
        ...
        env = jinja2.Environment(loader=loader, autoescape=True, extensions=["jinja2.ext.loopcontrols", "jinja2.ext.do"])
    ...
    def cmp(self, src, dst, version_dst, best_test_id):
        ...
        for param in self.PARAMS:
            ...
            try:
                ...
                self.log.exception("Failed to compare {} results: {} vs {}, version {}".format(param, src[param], dst[param], version_dst))
        return cmp_res
...
class LatencyDuringOperationsPerformanceAnalyzer(BaseResultsAnalyzer):
    ...
    def __init__(self, es_index, email_recipients=(), logger=None, events=None):
        super().__init__(es_index=es_index, email_recipients=email_recipients, email_template_fp="results_latency_during_ops_short.html", logger=logger, events=events)
    ...
    def _get_previous_results(self, test_doc, is_gce=False):
        filter_path = ["hits.hits._id", "hits.hits._source.results", "hits.hits._source.versions", "hits.hits._source.test_details", "hits.hits._source.latency_during_ops"]
    ...
    def _calculate_cycles_average(self, stat):
        ...
        stat.update({"hdr_summary_average": nemesis_average_by_workloads, "average_time_operation": f"{timedelta(seconds=int(operation_time_average))}", "average_time_operation_in_sec": int(operation_time_average)})
        ...
        try:
            ...
            for nemesis_key, nemesis_results in results_per_nemesis_by_version.items():
                for version_key, version_result in nemesis_results.items():
                    try:
                        best_results[nemesis_key][version_key] = sorted(version_result, key=lambda obj: obj.get("average_time_operation_in_sec"))[0]
                    except IndexError:
                        best_results[nemesis_key][version_key] = {}

                best_results[nemesis_key] = {per_version: best_results[nemesis_key][per_version] for per_version in sorted(best_results[nemesis_key].keys(), key=lambda version: version, reverse=True)}
            return best_results
            ...
            for nemesis in best_result:
                ...
                for _, best in best_result[nemesis].items():
                    for workload in best["hdr_summary_average"]:
                        ...
                        for perc in self.percentiles:
                            diff[workload][perc] = _calculate_relative_change_magnitude(current_result[nemesis]["hdr_summary_average"][workload][perc], best["hdr_summary_average"][workload][perc])
                    best["average_time_operation_in_sec_diff"] = _calculate_relative_change_magnitude(current_result[nemesis]["average_time_operation_in_sec"], best["average_time_operation_in_sec"])
        except Exception as exc:  # noqa: BLE001
    ...
    def check_regression(self, test_id, data, is_gce=False, node_benchmarks=None, email_subject_postfix=None):  # noqa: PLR0914
        ...
        last_events = {
            Severity.ERROR.name: last_error_events[Severity.ERROR.name][:100] if Severity.ERROR.name in last_error_events else [],
        ...
        subject = f"{self._get_email_tags(doc, is_gce)} Performance Regression Compare Results ({email_subject_postfix} {dataset_size}) - {test_name} - {test_version} - {str(test_start_time)}"
    ...
    def prepare_attachment_files_for_email(self, results):
        ...
        return [
            self.save_html_to_file(results, file_name="reactor_stall_events_list.html", template_file="results_reactor_stall_events_list.html"),
            self.save_html_to_file(results, file_name="full_email_report.html", template_file="results_latency_during_ops.html"),
            self.save_html_to_file(results, file_name="hdr_details_report.html", template_file="results_latency_during_ops_details_hdr_report.html"),
        ]
...
class SpecifiedStatsPerformanceAnalyzer(BaseResultsAnalyzer):
    ...
    def __init__(self, es_index, email_recipients=(), logger=None, events=None):
        super().__init__(es_index=es_index, email_recipients=email_recipients, email_template_fp="", logger=logger, events=events)
    ...
    def check_regression(self, test_id, stats):  # noqa: PLR0914
        ...
        filter_path = [".".join([es_base_path, "_id"]), ".".join([es_source_path, "results", "throughput"]), ".".join([es_source_path, "versions"])]
...
class PerformanceResultsAnalyzer(BaseResultsAnalyzer):
    ...
    def __init__(self, es_index, email_recipients=(), logger=None, events=None):
        super().__init__(es_index=es_index, email_recipients=email_recipients, email_template_fp="results_performance.html", logger=logger, events=events)
    ...
    def _test_stats(self, test_doc):
        # check if stats exists
        if "results" not in test_doc["_source"] or "stats_average" not in test_doc["_source"]["results"] or "stats_total" not in test_doc["_source"]["results"]:
            self.log.error("Cannot find one of the fields: results, results.stats_average, results.stats_total for test id: {}!".format(test_doc["_id"]))
            return None
    ...
    def _query_filter(test_doc, is_gce, use_wide_query=False, lastyear=False, extra_jobs_to_compare=None):
        if test_doc["_source"]["test_details"].get("scylla-bench"):
            return PerformanceFilterScyllaBench(test_doc, is_gce, use_wide_query, lastyear, extra_jobs_to_compare=extra_jobs_to_compare)()
        elif test_doc["_source"]["test_details"].get("ycsb"):
            return PerformanceFilterYCSB(test_doc, is_gce, use_wide_query, lastyear, extra_jobs_to_compare=extra_jobs_to_compare)()
        elif "cdc" in test_doc["_source"]["test_details"].get("sub_type", ""):
            return CDCQueryFilterCS(test_doc, is_gce, use_wide_query, lastyear, extra_jobs_to_compare=extra_jobs_to_compare)()
        else:
            return PerformanceFilterCS(test_doc, is_gce, use_wide_query, lastyear, extra_jobs_to_compare=extra_jobs_to_compare)()
    ...
    def check_regression(
        ...
        test_id,
        is_gce=False,
        email_subject_postfix=None,  # noqa: PLR0914
        ...
        filter_path = ["hits.hits._id", "hits.hits._source.results.stats_average", "hits.hits._source.results.stats_total", "hits.hits._source.results.throughput", "hits.hits._source.versions"]
        tests_filtered = self._es.search(index=self._es_index, q=query, filter_path=filter_path, size=self._limit, request_timeout=30)
        ...
        for row in tests_filtered["hits"]["hits"]:
            ...
            group_by_version[version]["tests"][version_info["date"]] = {"test_stats": curr_test_stats, "version": {k: version_info_data for k in self.PARAMS}}
            old_best = group_by_version[version]["stats_best"]
            group_by_version[version]["stats_best"] = {k: self._get_best_value(k, curr_test_stats[k], old_best[k]) for k in self.PARAMS if k in curr_test_stats and k in old_best}
            # replace best test id if best value changed
            for k in self.PARAMS:
                if k in curr_test_stats and k in old_best and group_by_version[version]["stats_best"][k] == curr_test_stats[k]:
        ...
        for version, group in group_by_version.items():
            ...
            latest_res = self.cmp(test_stats, latest_version_test["test_stats"], version, latest_version_test["version"])
        ...
        last_events, events_summary = self.get_events(event_severity=[Severity.CRITICAL.name, Severity.ERROR.name, Severity.DEBUG.name])
        ...
        if ycsb:
            if ycsb_engine := ycsb.get("raw_cmd", "").split():
                if len(ycsb_engine) > 3:
                    index = (ycsb_engine.index("run") if "run" in ycsb_engine else None) or (ycsb_engine.index("load") if "load" in ycsb_engine else None)
    ...
    def check_regression_with_subtest_baseline(self, test_id, base_test_id, subtest_baseline, is_gce=False, extra_jobs_to_compare=None) -> None:  # noqa: PLR0914
        ...
        for row in tests_filtered["hits"]["hits"]:
            ...
            if base_test_id in row["_id"] and sub_type not in current_tests:
                ...
                current_tests[sub_type]["best_test_id"] = {k: f"#{version_info['commit_id']}, {version_info['date']}" for k in self.PARAMS}
            ...
            if sub_type not in group_by_version_sub_type[version]:
                ...
                group_by_version_sub_type[version][sub_type]["best_test_id"] = {k: f"#{version_info['commit_id']}, {version_info['date']}" for k in self.PARAMS}
            ...
            group_by_version_sub_type[version][sub_type]["stats_best"] = {k: self._get_best_value(k, curr_test_stats[k], old_best[k]) for k in self.PARAMS if k in curr_test_stats and k in old_best}
            # replace best test id if best value changed
            for k in self.PARAMS:
                if k in curr_test_stats and k in old_best and group_by_version_sub_type[version][sub_type]["stats_best"][k] == curr_test_stats[k]:
                    group_by_version_sub_type[version][sub_type]["best_test_id"][k] = f"#{version_info['commit_id']}, {version_info['date']}"
        ...
        for sub_type, tests in current_tests.items():
            current_prometheus_stats[sub_type] = {stat: tests["results"].get(stat, {}) for stat in TestStatsMixin.PROMETHEUS_STATS}
        ...
        for version, group in group_by_version_sub_type.items():
            ...
            for sub_type, tests in group.items():
                ...
                if sub_type not in current_tests:
                    continue
                cmp_res[sub_type] = self.cmp(tests["stats_best"], current_tests[sub_type]["stats"], version, tests["best_test_id"])
        ...
        test_name = full_test_name.split(".", 1)[1]  # Example: longevity_test.LongevityTest.test_custom_time
        if ycsb:
            subject = f"{self._get_email_tags(doc, is_gce)} (Alternator) Performance Regression - {test_name} - {test_version} - {str(test_start_time)}"
        else:
            subject = f"{self._get_email_tags(doc, is_gce)} Performance Regression Compare Results - {test_name} - {test_version} - {str(test_start_time)}"
    ...
    def _mark_best_tests(self, prior_subtests, metrics, tests_info, main_test_id):
        ...
        for _, _prior_tests in prior_subtests.items():
            prior_tests = MagicList([prior_test for prior_test in _prior_tests if prior_test.main_test_id != main_test_id])
            if not prior_tests:
                continue
            for metric_path in metrics:
                ...
                self._add_best_for_info(main_tests_by_id[best_subtest.main_test_id][0], best_subtest, metric_path, tests_info)

    @staticmethod
    def _clean_up_not_marked_runs(prior_subtests, tests_info):
        main_tests_by_id = MagicList(tests_info.keys()).group_by("test_id")
        for _, prior_tests in prior_subtests.items():
            for prior_test in [prior_test for prior_test in prior_tests if not main_tests_by_id.get(prior_test.main_test_id, False)]:
    ...
    def _find_versions_to_compare_with(self, test, prior_main_tests, tests_info):
        ...
        grouped_by_version = prior_main_tests.group_by(data_path="software.scylla_server_any.version.major_as_int", sort_keys=-1, group_values={"data_path": "software.scylla_server_any.version.as_int", "sort_keys": -1})
    ...
    def _get_prior_tests_for_subtests(subtests: list):
        ...
        for subtest in subtests:
            prior_tests = MagicList([prior_sub_test for prior_sub_test in subtest.get_prior_tests() if prior_sub_test.metrics and prior_sub_test.metrics.is_valid()])
    ...
    def check_regression_multi_baseline(  # noqa: PLR0912, PLR0914, PLR0915
        ...
        if subject is None:
            subject = "Performance Regression Compare Results - {test.test_name} - {test.software.scylla_server_any.version.as_string}".format(test=rp_main_test)
        else:
        ...
        prior_main_tests = MagicList([prior_test for prior_test in rp_main_test.get_prior_tests() if prior_test.is_valid() and prior_test.start_time and prior_test.start_time.is_valid() and prior_test.test_id != rp_main_test.test_id])
        ...
        rp_subtests_of_current_test = MagicList([subtest for subtest in rp_main_test.get_subtests() if subtest.is_valid() and subtest.complete_time and subtest.complete_time.is_valid()]).sort_by("subtest_name")
        ...
        rp_subtests_of_current_test = MagicList([test for test in rp_subtests_of_current_test if test.subtest_name in rp_subtests_info])
        ...
        prior_tests_by_main_test_id = {subtest.subtest_name: prior_tests.group_by("main_test_id") for subtest, prior_tests in prior_subtests.items()}
        ...
        for group_name, group_substest_infos in subtests_groups.items():
            ...
            for baseline_subtest_name, subtest_infos in subtest_info_grouped_by_baseline.items():
                ...
                for subtest_info in MagicList(subtest_infos).sort_by("name"):
                    if subtest_info["subtest_name"] in tmp_subtest_by_name and tmp_subtest_by_name[subtest_info["subtest_name"]]:
            ...
            for subtest in grouped_subtests:
                ...
                rp_metrics_table_l1[subtest.subtest_name] = rp_metrics_table_l2 = {"total_metrics": 0, "data": collections.OrderedDict()}
                for metric_path in metrics:
                    rp_metrics_table_l2["data"][metric_path] = rp_metrics_table_l3 = {"relative": collections.OrderedDict(), "absolute": collections.OrderedDict()}
                    ...
                    if not baseline_prior_tests:
                        ...
                        for main_test, _ in rp_main_tests_to_compare.items():
                            ...
                            if baseline_metric is None or metric_instance is None:
                                metric_bucket_baseline[main_test] = None
                                continue
                            metric_bucket_baseline[main_test] = baseline_metric.rate(metric_instance, name=f"{metric_instance.name}<br>baseline diff")
...
class PredefinedStepsTestPerformanceAnalyzer(LatencyDuringOperationsPerformanceAnalyzer):
    ...
    def prepare_attachment_files_for_email(self, results):
        return [
            self.save_html_to_file(results, file_name="reactor_stall_events_list.html", template_file="results_reactor_stall_events_list.html"),
            self.save_html_to_file(results, file_name="full_email_report.html", template_file="results_performance_predefined_steps.html"),
        ]
...
class SearchBestThroughputConfigPerformanceAnalyzer(BaseResultsAnalyzer):
    ...
    def __init__(self, es_index, email_recipients=(), logger=None, events=None):
        super().__init__(es_index=es_index, email_recipients=email_recipients, email_template_fp="results_search_best_throughput_config.html", logger=logger, events=events)
    ...
    def check_regression(self, test_name, setup_details, test_results) -> None:
        ...
        results = {"test_id": setup_details.get("test_id", ""), "test_name": test_name, "setup_details": setup_details, "test_results": test_results}
