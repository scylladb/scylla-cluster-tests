...
class MicroBenchmarkingResultsAnalyzer(BaseResultsAnalyzer):
    ...
    def __init__(self, email_recipients, db_version=None, es_index=None):
        ...
        super().__init__(es_index=es_index, email_recipients=email_recipients, email_template_fp="results_microbenchmark.html", query_limit=10000, logger=LOGGER)
    ...
    def check_regression(self, current_results):
        ...
        tests_filtered = self._get_prior_tests(filter_path, additional_filter="((-_exists_:excluded) OR (excluded:false))")
        ...
        for res in results:
            test_type = "%s_%s" % (res["_source"]["test_group_properties"]["name"], res["_source"]["test_args"])
        ...
        def set_results_for(current_result, metrica):
            list_of_results_from_db.sort(key=lambda x: datetime.datetime.strptime(x["_source"]["test_run_date"], self._run_date_pattern))
            ...
            def get_commit_date(val):
                return datetime.datetime.strptime(val["_source"]["versions"]["scylla-server"]["date"], "%Y%m%d").date()
        ...
        with chdir(os.path.join(results_path, "perf_fast_forward_output")):
            ...
            for fullpath, subdirs, files in os.walk(os.getcwd()):
                ...
                if os.path.dirname(fullpath).endswith("perf_fast_forward_output") and len(subdirs) > 1:
                    raise LargeNumberOfDatasetsException("Test set {} has more than one datasets: {}".format(os.path.basename(fullpath), subdirs))
                ...
                if not subdirs:
                    ...
                    for filename in files:
                        ...
                        datastore.update({"hostname": self.hostname, "test_args": test_args, "test_run_date": self.test_run_date, "dataset_name": dataset_name, "excluded": False})
                        if update_db:
                            self._es.create_doc(index=self._es_index, doc_id="%s_%s" % (self.test_run_date, test_type), body=datastore)
    ...
    def exclude_test_run(self, testrun_id=""):
        ...
        for res in testrun_results["hits"]["hits"]:
            ...
            self._es.update_doc(index=self._es_index, doc_id=res["_id"], body={"excluded": True})
    ...
    def exclude_before_date(self, date=""):
        ...
        filter_path = ("hits.hits._id", "hits.hits._source.hostname", "hits.hits._source.versions.scylla-server.run_date_time")
        self.log.info("Exclude tests before date {}".format(date))
        results = self._es.search(index=self._es_index, filter_path=filter_path, size=self._limit, q="hostname:'%s' AND versions.scylla-server.version:%s*" % (self.hostname, self.db_version[:3]))
        if not results:
            self.log.info("Nothing to exclude")
            return
    ...
    def exclude_testrun_by_commit_id(self, commit_id=None):
        ...
        filter_path = ("hits.hits._id", "hits.hits._source.hostname", "hits.hits._source.versions.scylla-server.commit_id", "hits.hits._source.test_run_date")
        ...
        for doc in results["hits"]["hits"]:
            self.log.info("Exclude test: %s\nCommit: #%s\nRun Date time: %s\n", doc["_id"], doc["_source"]["versions"]["scylla-server"]["commit_id"], doc["_source"]["test_run_date"])
            self._es.update_doc(index=self._es_index, doc_id=doc["_id"], body={"excluded": True})


def main(args):
    if args.mode == "exclude":
        mbra = MicroBenchmarkingResultsAnalyzer(email_recipients=None, db_version=args.db_version, es_index=args.es_index)
        if args.testrun_id:
    ...
    if args.mode == "check":
        mbra = MicroBenchmarkingResultsAnalyzer(email_recipients=args.email_recipients.split(","), es_index=args.es_index)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Microbencmarking stats utility \
        for upload and analyze or exclude test result from future analyze"
    )

    subparser = parser.add_subparsers(
        dest="mode", title="Microbencmarking utility modes", description="Microbencmarking could be run in two modes definded by subcommand", metavar="Modes", help="To see subcommand help use  microbenchmarking.py subcommand -h"
    )
    ...
    exclude_group.add_argument("--testrun-id", action="store", default="", help="Exclude test results for testrun id")
    exclude_group.add_argument("--test-id", action="store", default="", help="Exclude test result by id")
    ...
    exclude_group.add_argument("--commit-id", action="store", default="", help="Exclude test run for specific commit id")
    exclude.add_argument("--db-version", action="store", default="", help="Exclude test results for scylla version", required=True)
    exclude.add_argument("--es-index", action="store", default="", help="ES Index for excluding results")

    check = subparser.add_parser("check", help="Upload and analyze test result")
    check.add_argument("--update-db", action="store_true", default=False, help="Upload current microbenchmarking stats to ElasticSearch")
    check.add_argument("--results-path", action="store", default=".", help="Path where to search for test results")
    check.add_argument("--email-recipients", action="store", default="bentsi@scylladb.com", help="Comma separated email addresses list that will get the report")
    check.add_argument("--report-path", action="store", default="", help="Save HTML generated results report to the file path before sending by email")
    check.add_argument("--hostname", action="store", default="", help="Run check regression for host with hostname")
    check.add_argument("--es-index", action="store", default="", help="ES index name where previous results are stored")
