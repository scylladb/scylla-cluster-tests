...
class MicroBenchmarkingResultsAnalyzer(BaseResultsAnalyzer):  # pylint: disable=too-many-instance-attributes
    ...
    def check_regression(self, current_results):  # pylint: disable=arguments-differ
        ...
        tests_filtered = self._get_prior_tests(
            filter_path, additional_filter="((-_exists_:excluded) OR (excluded:false))"
        )
        ...
        def set_results_for(current_result, metrica):
            list_of_results_from_db.sort(
                key=lambda x: datetime.datetime.strptime(x["_source"]["test_run_date"], self._run_date_pattern)
            )
            ...
            if (
                len(list_of_results_from_db) > 1
                and get_commit_id(list_of_results_from_db[-1]) == self.cur_version_info["commit_id"]
            ):
                last_idx = -2
                    ...
                    for filename in files:
                        ...
                        if update_db:
                            self._es.create_doc(
                                ...
                                doc_id="%s_%s" % (self.test_run_date, test_type),
                                body=datastore,
                            )
    ...
    def exclude_test_run(self, testrun_id=""):
        ...
        for res in testrun_results["hits"]["hits"]:
            ...
            self._es.update_doc(
                index=self._es_index, doc_type=self._es_doc_type, doc_id=res["_id"], body={"excluded": True}
            )
    ...
    def exclude_by_test_id(self, test_id=""):
        ...
        if doc:
            self._es.update_doc(
                index=self._es_index, doc_type=self._es_doc_type, doc_id=doc["_id"], body={"excluded": True}
            )
        else:
            self.log.info("Nothing to exclude")
            return
    ...
    def exclude_before_date(self, date=""):
        ...
        try:
            ...
            try:
                ...
                self.log.error(
                    'Wrong format of parameter --before-date. Should be "YYYY-MM-DD" or "YYYY-MM-DD hh:mm:ss"'
                )
                return
        ...
        results = self._es.search(  # pylint: disable=unexpected-keyword-arg
            ...
            filter_path=filter_path,
            size=self._limit,
            q="hostname:'%s' AND versions.scylla-server.version:%s*" % (self.hostname, self.db_version[:3]),
        )
        if not results:
            self.log.info("Nothing to exclude")
            return
...
def parse_args():
    ...
    exclude_group.add_argument(
        "--commit-id", action="store", default="", help="Exclude test run for specific commit id"
    )
    exclude.add_argument(
        "--db-version", action="store", default="", help="Exclude test results for scylla version", required=True
    )
