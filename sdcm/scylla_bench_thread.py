...
class ScyllaBenchThread(DockerBasedStressThread):
    ...
    def __init__(self, stress_cmd, loader_set, timeout, node_list=None, round_robin=False, stop_test_on_failure=False, stress_num=1, credentials=None, params=None):
        super().__init__(loader_set=loader_set, stress_cmd=stress_cmd, timeout=timeout, stress_num=stress_num, node_list=node_list, round_robin=round_robin, params=params, stop_test_on_failure=stop_test_on_failure)
        ...
        self.sb_workload: ScyllaBenchWorkloads = ScyllaBenchWorkloads(re.search(r"-workload=(.+?) ", stress_cmd).group(1))
        ...
        if self.params.get("rack_aware_loader"):
            ...
            if any(len(racks) > 1 for racks in by_region_rack_names.values()):
                if not (loader_rack := rack_names.get((str(loader.region), str(loader.rack)))):
                    ...
                    raise ValueError(f"Rack name not found for loader {loader} in region {loader.region} and rack {loader.rack}")
    ...
    def create_stress_cmd(self, stress_cmd, loader, cmd_runner):
        ...
        if self.params.get("client_encrypt"):
            ...
            if self.params.get("client_encrypt_mtls"):
                stress_cmd = f"{stress_cmd.strip()} -tls-client-key-file {SCYLLA_SSL_CONF_DIR}/{TLSAssets.CLIENT_KEY} -tls-client-cert-file {SCYLLA_SSL_CONF_DIR}/{TLSAssets.CLIENT_CERT}"
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        ...
        if self.sb_mode == ScyllaBenchModes.WRITE and self.sb_workload == ScyllaBenchWorkloads.TIMESERIES:
            ...
            write_timestamp = wait_for(lambda: loader.parent_cluster.sb_write_timeseries_ts, step=5, timeout=30, text='Waiting for "scylla-bench -workload=timeseries -mode=write" been started, to pick up timestamp')
        ...
        with (
            ScyllaBenchStressExporter(instance_name=cmd_runner_name, metrics=nemesis_metrics_obj(), stress_operation=str(self.sb_mode.value), stress_log_filename=log_file_name, loader_idx=loader_idx),
            cleanup_context,
            ScyllaBenchStressEventsPublisher(node=loader, sb_log_filename=log_file_name, stop_test_on_failure=self.stop_test_on_failure) as publisher,
