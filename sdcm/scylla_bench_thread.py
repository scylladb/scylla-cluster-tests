...
class ScyllaBenchThread(DockerBasedStressThread):
    ...
    def adjust_cmd_node_option(self, stress_cmd, loader, cmd_runner):
        ...
        dc_options_cmd = ""
        node_list_for_current_cmd = self.node_list
        if self.params.get("rack_aware_loader"):
            ...
            if any(len(racks) > 1 for racks in by_region_rack_names.values()):
                ...
                if not (loader_dc := self.get_datacenter_name_for_loader(loader)):
                    ...
                    raise ValueError(
                        f"Datacenter name not found for loader {loader} in region {loader.region} and rack {loader.rack}")

                dc_options_cmd += loader_dc
                LOGGER.debug("Rack value for stress command:  %s", dc_options_cmd)
                if not (node_list_for_current_cmd := self.loader_set.get_nodes_per_datacenter_and_rack_idx(
                        db_nodes=self.node_list).get((str(loader.region),
                                                      str(loader.rack)))):
                    raise ValueError(
                        f"No nodes found for loader {loader} in region {loader.region} and rack {loader.rack}")

        if self.loader_set.test_config.MULTI_REGION and "-datacenter=" not in dc_options_cmd:
            ...
            dc_options_cmd = self.get_datacenter_name_for_loader(loader)

        node_ip_list = [n.cql_address for n in node_list_for_current_cmd]
