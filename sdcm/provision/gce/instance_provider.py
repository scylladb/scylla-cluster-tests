...
class VirtualMachineProvider:
    ...
    def get_or_create(
        ...
        """
        Get existing instances or create new ones in parallel.

        Instances are created in parallel (similar to Azure provisioning):
        1. First loop: Initiate all instance creation operations (non-blocking)
        2. Second loop: Wait for all operations to complete
        ...
        instances = []
        pending_instance_creations = []  # List of (definition, operation, normalized_name, user_data, startup_script) tuples
        error_to_raise = None

        # First loop: Check cache and initiate creation for new instances (non-blocking)
    ...
    def _wait_for_instance_creation(
        ...
        """Wait for instance creation operation to complete and finalize setup.

        Handles waiting for the GCE operation, retrieves the instance,
        sets labels, and caches the result. If the operation fails,
    ...
    def _create_instance_with_retry(
        self,
        definition: InstanceDefinition,
        pricing_model: PricingModel,
        user_data: str = "",
        ...
        """Create a single GCE instance with retry logic (used for retry after failure)."""
        normalized_name = normalize_instance_name(definition.name)
        try:
            operation = self._build_and_insert_instance(
                definition, pricing_model, user_data, startup_script, normalized_name
