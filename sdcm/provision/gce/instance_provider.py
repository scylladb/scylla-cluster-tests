...
class VirtualMachineProvider:
            ...
            for instance in instances:
                if instance.metadata and instance.metadata.items:
                    for item in instance.metadata.items:
                        if item.get("key") == "TestId" and item.get("value") == self.test_id:
    ...
    def get_or_create(
        ...
        try:
            instance = self._create_instance(
                definition, pricing_model, user_data, startup_script
            )
            self._cache[normalized_name] = instance
            return instance
        except Exception as exc:
            raise ProvisionError(
                f"Failed to create instance {normalized_name}: {exc}"
            ) from exc
    ...
    def _create_instance(
        ...
        if definition.ssh_key:
            public_key, key_type = pub_key_from_private_key_file(definition.ssh_key.key_file)
            metadata["ssh-keys"] = f"{definition.user_name}:{key_type} {public_key} {definition.user_name}"
        ...
        # Add network tags
        network_tags = self.network_provider.get_network_tags(
            allow_public_access=definition.use_public_ip
        )
        if network_tags:
