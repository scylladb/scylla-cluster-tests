...
from sdcm.provision.gce.constants import DISK_TYPE_PD_STANDARD
...
class VirtualMachineProvider:
    ...
    def _create_instance(  # noqa: PLR0914
        ...
        # Determine root disk type based on machine type
        # z3-highmem machines cannot use pd-standard, they require hyperdisk-balanced
        if "z3-highmem" in definition.type:
            root_disk_type = "hyperdisk-balanced"
            # z3-highmem has built-in local SSDs, don't add extra local SSDs
            local_ssd_count = 0
        else:
            root_disk_type = DISK_TYPE_PD_STANDARD
        ...
        if "z3-highmem" in definition.type:
            ...
            # Remove any local SSD data disks since z3-highmem has built-in local SSDs
