...
def rank_zone_letters(
    ...
    """Reorder `zone_letters` in `region` best-first by obtainability.

    Unscored letters keep their relative order and go last - a zone the API declined to score is not proven
    bad. Letters below `min_obtainability` are dropped; `0.0` (the default) never drops anything, matching
    GCP's "likelihood, not guarantee" framing.

    Returns `None` when advice is unavailable, which callers must treat as "keep the existing order". That is
    deliberately distinct from an empty list, which means advice WAS available and no zone met the minimum -
    conflating the two would let a configured minimum be silently ignored precisely when it rejects
    everything.
    """
    if not zone_letters:
        return []
    zones = [f"{region}-{letter}" for letter in zone_letters]
    advice = get_zone_advice(machine_types=machine_types, size=size, region=region, zones=zones)
    if not advice:
        return None

    by_letter = {item.zone_letter: item.obtainability for item in advice}
    scored = [letter for letter in zone_letters if letter in by_letter and by_letter[letter] >= min_obtainability]
    dropped = [letter for letter in zone_letters if letter in by_letter and by_letter[letter] < min_obtainability]
    unscored = [letter for letter in zone_letters if letter not in by_letter]
...
def rank_regions(
    ...
    LOGGER.info("GCE capacity advice by region: %s", ", ".join(f"{region}={best[region]:.2f}" for region in scored))
    return scored + unscored
