...
class DiskProvider:
    ...
    def create_disks_config(
        ...
        Args:
            ...
            local_ssd_count: Number of local SSDs to attach
            data_disks: List of additional data disks
        ...
        # Add root disk
        disks.append(
            self.create_root_disk_config(
                instance_name, image_url, root_disk_type, root_disk_size_gb
            )
        )
        ...
        for i in range(local_ssd_count):
            disks.append(
                self.create_local_ssd_config(instance_name, i, DISK_INTERFACE_NVME)
            )

        # Add additional data disks
        if data_disks:
            for data_disk in data_disks:
                if int(data_disk.size) > 0:
                    disks.append(
                        self.create_persistent_disk_config(
                            instance_name, data_disk.type, int(data_disk.size)
                        )
                    )
