    ...
    def __init__(
        ...
        **config
        ...
        Args:
            ...
            availability_zone: GCE zone letter (e.g., 'b')
        ...
        for gce_instance in self._vm_provider.list():
            try:
                ...
                LOGGER.warning(
                    "Failed to cache instance %s: %s", gce_instance.name, exc
                )

        LOGGER.debug(
            "Initialized GceProvisioner for test_id=%s, region=%s, zone=%s",
            test_id, region, zone
        )
                ...
                for instance in instances.instances:
                    ...
                    if instance.metadata and instance.metadata.items:
                        for item in instance.metadata.items:
                            if item.get("key") == "TestId" and item.get("value") == test_id:
    ...
    def get_or_create_instances(
        ...
        for definition in definitions:
            ...
            if definition.user_data:
                user_data_parts = [ud.script for ud in definition.user_data]
    ...
    def _gce_instance_to_vm_instance(self, gce_instance: compute_v1.Instance) -> VmInstance:
        ...
        if gce_instance.creation_timestamp:
            creation_time = datetime.fromisoformat(
                gce_instance.creation_timestamp.replace("Z", "+00:00")
            )
