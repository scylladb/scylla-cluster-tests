...
from sdcm.provision.aws.spot_placement_score import get_scores, rank_az_letters_for_roles, rank_regions
...
class AZResolver:
    ...
    def resolve(self) -> None:
        ...
        if new_value != original_value:
            ...
            if dropped:
                ...
                LOGGER.info(
                    "AZResolver: availability_zone '%s' outranked on spot placement score in regions %s; using '%s'",
                    original_value,
                    region_names,
                    new_value,
                )
            ...
            LOGGER.debug(
                "AZResolver: availability_zone '%s' already valid for regions %s", original_value, region_names
            )
    ...
    def scored_role_requests(self) -> list[tuple[str, list[str], int]]:
        """One scoring request per node role, so an AZ is judged on everything it has to hold.

        Ranking on DB types alone leaves the loaders and the monitor unscored in the AZ they are about to be
        placed in, and they are not always the plentiful ones: measured in us-west-2, the DB type scored 3 in
        every AZ while `c` scored 1 for both the loader and the monitor, so an alphabetical tiebreak among
        equal DB scores would have walked straight into it. Kept as separate requests on purpose -
        `InstanceTypes` inside one request means "any of these will do".
        """
        requests = [("db", self.scored_instance_types(), self.spot_target_capacity())]
        for role, type_key, count_key in (
            ("loaders", "instance_type_loader", "n_loaders"),
            ("monitor", "instance_type_monitor", "n_monitor_nodes"),
        ):
            instance_type = self._params.get(type_key)
            count = _node_count_total(self._params.get(count_key))
            if instance_type and count > 0:
                requests.append((role, [instance_type], count))
        return requests
    ...
    def _score_may_choose_az(self) -> bool:
        ...
        actually matters - an `availability_zone` nobody chose: `defaults/aws_config.yaml` pins 'a' for every run,
        ...
        behaviour.
        """
        if not is_spot_placement_scoring_enabled(self._params):
            return False
    ...
    def _rank_letters_by_spot_score(self, region_names: list[str], letters: list[str]) -> list[str]:
        ...
        ranked = rank_az_letters_for_roles(
            role_requests=self.scored_role_requests(),
        ...
        if ranked is None:
            return letters
        # `spot_placement_score_min` can drop AZs, and the callers fill a fixed number of AZ slots from what
        # comes back. Returning a short list would quietly provision a cluster across fewer AZs than the test
        # asked for - the kind of silent narrowing the minimum is supposed to prevent, not cause. So fail
        # loudly whenever scoring leaves fewer AZs than we would otherwise have filled.
        #
        # The bar is what was actually achievable before scoring: a config asking for more AZs than the region
        # supports for its instance types is already truncated upstream, and that pre-existing behaviour is not
        # ours to turn into an error. Unreachable with the default min_score=0, which never drops anything.
        required = min(len(self._configured_az_letters()), len(letters))
        if len(ranked) < required:
            raise NoValidAvailabilityZoneError(
                f"Only {len(ranked)} availability zone(s) in {region_names[0]} reached "
                f"spot_placement_score_min={min_score} for {self.scored_instance_types()}, but {required} "
                f"are needed (candidates were {letters}, qualifying {ranked}); lower the threshold, "
