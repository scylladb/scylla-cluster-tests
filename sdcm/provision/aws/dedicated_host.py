...
class SCTDedicatedHosts:
    ...
    def reserve(cls, params) -> None:
        ...
        if host_ids := params.get("aws_dedicated_host_ids"):
            ...
            response = ec2.describe_hosts(Filters=[{"Name": "tag:TestId", "Values": [test_id]}, {"Name": "state", "Values": ["available"]}])
        ...
        if response["Hosts"]:
            ...
            for availability_zone in cls._get_supported_availability_zones(
                ...
                host_id = cls.allocate(region_name=region, availability_zone=availability_zone, instance_type=params.get("instance_type_db"), quantity=1, tags=tags)
    ...
    def release_by_tags(cls, tags_dict: dict, regions=None, dry_run=False) -> None:
        ...
        if regions:
            ...
            for region in regions:
                aws_hosts |= cls.list_hosts(tags_dict=local_tags_dict, region_name=region, group_as_region=True)
        else:
