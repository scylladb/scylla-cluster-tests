...
from sdcm.provision.aws.constants import SPOT_REQUEST_TIMEOUT, SPOT_REQUEST_WAITING_TIME, STATUS_FULFILLED, SPOT_STATUS_UNEXPECTED_ERROR, SPOT_PRICE_TOO_LOW, FLEET_LIMIT_EXCEEDED_ERROR, SPOT_CAPACITY_NOT_AVAILABLE_ERROR
...
def convert_tags_to_filters(tags: TagsType) -> List[Dict[str, str]]:
    return [{"Name": "tag:{}".format(name), "Values": value if isinstance(value, list) else [value]} for name, value in tags.items()]
...
def wait_for_provision_request_done(region_name: str, request_ids: List[str], is_fleet: bool, timeout: float = SPOT_REQUEST_TIMEOUT, wait_interval: float = SPOT_REQUEST_WAITING_TIME):
    waiting_time = 0
    provisioned_instance_ids = []
    while not provisioned_instance_ids and waiting_time < timeout:
        time.sleep(wait_interval)
        if is_fleet:
            provisioned_instance_ids = get_provisioned_fleet_instance_ids(region_name=region_name, request_ids=request_ids)
        else:
...
def get_provisioned_fleet_instance_ids(region_name: str, request_ids: List[str]) -> Optional[List[str]]:
    try:
        ...
        LOGGER.error("Failed to describe spot fleet requests in region %s for request IDs %s: %s", region_name, request_ids, exc)
        return []
    for req in resp["SpotFleetRequestConfigs"]:
        ...
        if activity_status == SPOT_STATUS_UNEXPECTED_ERROR:
            ...
            try:
                ...
                error_messages = [f"{i['EventType']}: {i['EventInformation'].get('EventDescription', 'No description')}" for i in history_resp["HistoryRecords"]]

                for error in [FLEET_LIMIT_EXCEEDED_ERROR, SPOT_CAPACITY_NOT_AVAILABLE_ERROR]:
                    if error in errors:
                        LOGGER.error(
                            "Critical spot fleet provisioning failure in region %s for fleet request %s: State='%s', ActivityStatus='%s', Error='%s'. Error history: %s. This request cannot be fulfilled and provisioning will not retry.",
                            region_name,
                            request_id,
                            fleet_state,
                            activity_status,
                ...
                LOGGER.error("Failed to retrieve spot fleet request history for %s in region %s: %s", request_id, region_name, exc)
        LOGGER.warning("Spot fleet request not yet fulfilled in region %s for request %s: State='%s', ActivityStatus='%s'", region_name, request_id, fleet_state, activity_status)
        return []
    ...
    for request_id in request_ids:
        try:
            ...
            LOGGER.error("Failed to describe spot fleet instances for fleet request %s in region %s: %s", request_id, region_name, exc)
            return None
...
def get_provisioned_spot_instance_ids(region_name: str, request_ids: List[str]) -> Optional[List[str]]:
    ...
    try:
        ...
        LOGGER.error("Failed to describe spot instance requests in region %s for request IDs %s: %s", region_name, request_ids, exc)
        return []
    provisioned = []
    for req in resp["SpotInstanceRequests"]:
        ...
        if status_code != STATUS_FULFILLED or state != "active":
            if status_code in [SPOT_PRICE_TOO_LOW, SPOT_CAPACITY_NOT_AVAILABLE_ERROR]:
                ...
                LOGGER.error(
                    "Critical spot provisioning failure in region %s for request %s: Status='%s', State='%s', Message='%s'. This request cannot be fulfilled and provisioning will not retry.",
                    region_name,
                    request_id,
                    status_code,
                    state,
                ...
                return None
            LOGGER.warning("Spot instance request not yet fulfilled in region %s for request %s: Status='%s', State='%s', Message='%s'", region_name, request_id, status_code, state, status_message)
            return []
        provisioned.append(req["InstanceId"])
    return provisioned


def create_spot_fleet_instance_request(region_name: str, count: int, price: float, fleet_role: str, instance_parameters: SpotFleetLaunchSpecificationTypeDef, valid_until: datetime.datetime = None) -> str:
...
def create_spot_instance_request(
    region_name: str, count: int, instance_parameters: RequestSpotLaunchSpecificationTypeDef, full_availability_zone: str, valid_until: datetime.datetime = None, tag_specifications: Sequence[TagSpecificationTypeDef] = None
) -> List[str]:
    params = {
        "DryRun": False,
        "InstanceCount": count,
        ...
        TagSpecifications=[
            {
                ...
                "Tags": [{"Key": key, "Value": value} for key, value in tags.items()] + [{"Key": "Name", "Value": name}],
            }
        ],
    )
    return result
