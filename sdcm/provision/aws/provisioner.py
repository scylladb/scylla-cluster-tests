...
from sdcm.provision.aws.utils import ec2_services, ec2_clients, find_instance_by_id, set_tags_on_instances, wait_for_provision_request_done, create_spot_fleet_instance_request, create_spot_instance_request
from sdcm.provision.aws.constants import SPOT_CNT_LIMIT, SPOT_FLEET_LIMIT, SPOT_REQUEST_TIMEOUT, STATUS_FULFILLED, SPOT_STATUS_UNEXPECTED_ERROR, FLEET_LIMIT_EXCEEDED_ERROR, SPOT_CAPACITY_NOT_AVAILABLE_ERROR
...
class AWSInstanceProvisioner(InstanceProvisionerBase):
    ...
    def provision(self, provision_parameters: ProvisionParameters, instance_parameters: AWSInstanceParams, count: int, tags: Union[List[TagsType], TagsType] = None, names: List[str] = None) -> List[Instance]:
        if tags is None:
            tags = {}
    ...
    @staticmethod
    def _provision_on_demand_instances(provision_parameters: ProvisionParameters, instance_parameters: AWSInstanceParams, count: int, tags: List[TagsType]) -> List[Instance]:
        instance_parameters_dict = instance_parameters.model_dump(exclude_none=True, exclude_defaults=True, exclude_unset=True, encode_user_data=False)
        ...
        instance_parameters_dict["TagSpecifications"] = [{"ResourceType": "instance", "Tags": tags_as_ec2_tags(tags[0])}]
        ...
        if cr_id := SCTCapacityReservation.reservations.get(provision_parameters.availability_zone, {}).get(instance_parameters.InstanceType):
            instance_parameters_dict["CapacityReservationSpecification"] = {"CapacityReservationTarget": {"CapacityReservationId": cr_id}}
        if host_id := SCTDedicatedHosts.get_host(provision_parameters.region_name + provision_parameters.availability_zone, instance_parameters.InstanceType):
        ...
        instances = ec2_services[provision_parameters.region_name].create_instances(**instance_parameters_dict, MinCount=count, MaxCount=count)
    ...
    def _provision_spot_instances(self, provision_parameters: ProvisionParameters, instance_parameters: AWSInstanceParams, count: int, tags: Union[List[TagsType], TagsType]) -> List[Instance]:
        ...
        while rest_to_provision:
            ...
            if self._is_provision_type_fleet(count) and instances_to_provision > 1:
                new_instances = self._execute_spot_fleet_instance_request(provision_parameters=provision_parameters, instance_parameters=instance_parameters, count=instances_to_provision, tags=tags)
            else:
                new_instances = self._execute_spot_instance_request(provision_parameters=provision_parameters, instance_parameters=instance_parameters, count=instances_to_provision, tags=tags)
            provisioned_instances.extend(new_instances)
            rest_to_provision -= instances_to_provision
        return provisioned_instances

    def _execute_spot_fleet_instance_request(self, provision_parameters: ProvisionParameters, instance_parameters: AWSInstanceParams, count: int, tags: List[TagsType]) -> List[Instance]:
        ...
        # picks the tags of the first instance to apply to all instances upfront
        # later those would be updated with individual tags (Name, etc.)
        instance_parameters_dict["TagSpecifications"] = [{"ResourceType": "spot-fleet-request", "Tags": tags_as_ec2_tags(tags[0])}]

        request_id = create_spot_fleet_instance_request(region_name=provision_parameters.region_name, count=count, fleet_role=self._iam_fleet_role, instance_parameters=instance_parameters_dict)
        ...
        if not instance_ids:
            with contextlib.suppress(Exception):
                self._ec2_client(provision_parameters).cancel_spot_fleet_requests(SpotFleetRequestIds=[request_id], TerminateInstances=True)
            return []
        ...
        self._ec2_client(provision_parameters).cancel_spot_fleet_requests(SpotFleetRequestIds=[request_id], TerminateInstances=False)
        return [find_instance_by_id(region_name=provision_parameters.region_name, instance_id=instance_id) for instance_id in instance_ids]

    def _get_provisioned_fleet_instance_ids(self, provision_parameters: ProvisionParameters, request_ids: List[str]) -> Optional[List[str]]:
        try:
        ...
        for req in resp["SpotFleetRequestConfigs"]:
            if req["SpotFleetRequestState"] == "active" and "ActivityStatus" not in req and req["ActivityStatus"] == STATUS_FULFILLED:
                continue
    ...
    def _execute_spot_instance_request(self, provision_parameters: ProvisionParameters, instance_parameters: AWSInstanceParams, count: int, tags: List[TagsType]) -> List[Instance]:
        # picks the tags of the first instance to apply to all instances upfront
        # later those would be updated with individual tags (Name, etc.)
        tag_specifications = [
