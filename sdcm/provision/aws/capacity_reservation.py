...
class SCTCapacityReservation:
    ...
    def get_cr_from_aws(cls, params, force_fetch=False) -> None:
        ...
        reservations = ec2.describe_capacity_reservations(Filters=[{"Name": "tag:test_id", "Values": [test_id]}, {"Name": "state", "Values": ["active"]}])
        result = {}
    ...
    def _get_supported_availability_zones(ec2, instance_types: List[str], initial_az: str) -> List[str]:
        ...
        azs = set.intersection(*[{offering["Location"] for offering in offerings if offering["InstanceType"] == instance_type} for instance_type in instance_types])
        azs = list(azs)
    ...
    def is_capacity_reservation_enabled(params: dict) -> bool:
        ...
        return params.get("cluster_backend") == "aws" and params.get("test_id") and not params.get("reuse_cluster") and params.get("use_capacity_reservation") is True and params.get("instance_provision") == "on_demand" and is_single_dc
    ...
    def reserve(cls, params) -> None:
        ...
        if params.get("use_placement_group"):
            ...
            if response["PlacementGroups"]:
                placement_group_arn = [group["GroupArn"] for group in response["PlacementGroups"] if group["State"] == "available"][0]
        ...
        reservations = {}
        for availability_zone in cls._get_supported_availability_zones(ec2=ec2, instance_types=list(request.keys()), initial_az=region + params.get("availability_zone")):
            ...
            for instance_type, instance_count in request.items():
                cr_id = cls._create(ec2=ec2, test_id=test_id, availability_zone=availability_zone, instance_type=instance_type, instance_count=instance_count, duration=duration, placement_group_arn=placement_group_arn)
                if cr_id:
            ...
            if reservations:
                ...
                LOGGER.info("Capacity reservations created in '%s' az: %s for duration: %s", availability_zone, reservations, duration)
                return
    ...
    def _create(ec2, test_id, availability_zone, instance_type, instance_count, duration, placement_group_arn=None) -> str | None:
        ...
        try:
            response = ec2.create_capacity_reservation(
                ...
                TagSpecifications=[
                    {"ResourceType": "capacity-reservation", "Tags": [{"Key": "test_id", "Value": test_id}, {"Key": "RunByUser", "Value": get_username()}, {"Key": "JenkinsJobTag", "Value": os.environ.get("BUILD_TAG")}]},
                ],
                **additional_params,
            )
            ...
            try:
                ...
                for reservation in reservations["CapacityReservations"]:
                    try:
                        ...
                        LOGGER.info("Capacity reservation %s in region %s cancelled successfully.", reservation["CapacityReservationId"], region)
                    except ClientError as exp:
                        LOGGER.error("Failed to cancel capacity reservation %s in region %s. Error: %s", reservation["CapacityReservationId"], region, exp)
