...
class _ScoresUnavailableError(Exception):
    """The API could not answer (permissions, throttling, transport, a non-XML body).

    Distinct from "the API answered with nothing": this one must never be cached, or a single blip blinds
    every provisioning attempt for a whole TTL.
    """
...
def _query_scores(instance_types: list[str], target_capacity: int, regions: list[str], single_az: bool) -> list[dict]:
    """Call the API across region chunks, following `NextToken`.

    Raises `_ScoresUnavailableError` on any error, rather than returning [], so `get_scores` can tell a failed
    query apart from a query that legitimately scored nothing and keep the former out of the cache.
    """
    ...
    for region_chunk in _chunk_regions(regions):
        ...
        while True:
            ...
            try:
                ...
                raise _ScoresUnavailableError(str(exc)) from exc
...
_scores_cache = TTLCache(maxsize=64, ttl=SPOT_PLACEMENT_SCORE_CACHE_TTL)
...
def get_scores(
    ...
    Only a non-empty answer is cached. Caching the [] we return on failure would turn one throttled or
    briefly-unauthorized call into a whole TTL of unscored provisioning, which is exactly when the scores are
    worth having; an empty-but-successful answer is cheap enough to ask again.
    """
    key = hashkey(tuple(sorted(instance_types)), target_capacity, tuple(sorted(regions)), single_az)
    if (cached_scores := _scores_cache.get(key)) is not None:
    ...
    except _ScoresUnavailableError:
        return []  # already logged where it was raised, with the code that caused it
    except Exception as exc:  # noqa: BLE001
        LOGGER.warning("Spot placement scores unavailable (unexpected %s): %s", type(exc).__name__, exc)
        return []
    if scores:
        _scores_cache[key] = scores
    return scores


get_scores.cache_clear = _scores_cache.clear
...
def rank_az_letters_for_roles(
    role_requests: list[tuple[str, list[str], int]], region: str, az_letters: list[str], min_score: int = 0
) -> list[str] | None:
    """Reorder `az_letters` by the WORST score across every role the cluster needs in that AZ.

    `role_requests` is (role name, instance types, count) per role - DB, loaders, monitor - each scored in
    its own call. They must never be merged into one: `InstanceTypes` means "place N of any of these", so a
    plentiful t3 monitor would answer for the whole request and flatten the ranking.

    The minimum is the right summary because the cluster needs *all* of these types in the chosen AZ, so the
    AZ is worth exactly what its scarcest role is worth. Scoring DB types alone hides the case that motivated
    this: measured in us-west-2, the DB type scored 3 in every AZ while `c` scored 1 for both the loader and
    the monitor, so an alphabetical tiebreak among equal DB scores would walk straight into it.
    """
    if not az_letters:
        return []
    worst: dict[str, int] = {}
    for role, instance_types, count in role_requests:
        if not instance_types or count < 1:
            continue
        scores = get_scores(instance_types=instance_types, target_capacity=count, regions=[region])
        if not scores:
        ...
        LOGGER.info(
            "Spot placement %s: %s x%d in %s -> %s",
            role,
            ",".join(instance_types),
            count,
            region,
            " ".join(f"{letter}={by_letter[letter]}" for letter in sorted(by_letter)),
        )
    ...
    scored = [letter for letter in az_letters if letter in worst and worst[letter] >= min_score]
    dropped = [letter for letter in az_letters if letter in worst and worst[letter] < min_score]
    unscored = [letter for letter in az_letters if letter not in worst]
