...
def create_certificate(
    ...
    key_file: Path,
    cname: str,
    ca_cert_file: Path = None,
    ca_key_file: Path = None,
    server_csr_file: Path = None,
...
def _import_ca_to_jks_truststore(
    localhost, cert_file: Path = CA_CERT_FILE, jks_file: Path = JKS_TRUSTSTORE_FILE, jks_password: str = "cassandra"
) -> None:
    ...
    exit_code, output = java.exec_run(
        f"keytool -importcert -noprompt -file {tmp_cert_path} -keystore {tmp_truststore_path} -storepass {jks_password}"
    )
    if exit_code != 0:
...
def export_pem_cert_to_pkcs12_keystore(
    cert_file: Path,
    cert_key_file: Path,
    dst_pkcs12_file: Path,
    cert_key_password: Any = None,
    p12_password: str = "cassandra",
) -> None:
