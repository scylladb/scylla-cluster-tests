...
def _create_ca(cname: str = "scylladb.com", valid_days: int = 365) -> None:
    ...
    if CA_CERT_FILE.exists() and CA_KEY_FILE.exists():
        LOGGER.info("CA %s certificate and %s key already exist. Skipping creation.", CA_CERT_FILE, CA_KEY_FILE)
        return
    ...
    with open(file=CA_KEY_FILE, mode="wb") as file:
        file.write(private_key.private_bytes(encoding=serialization.Encoding.PEM, format=serialization.PrivateFormat.PKCS8, encryption_algorithm=serialization.BestAvailableEncryption(CA_DEFAULT_PASSWORD.encode())))


def create_certificate(cert_file: Path, key_file: Path, cname: str, ca_cert_file: Path = None, ca_key_file: Path = None, server_csr_file: Path = None, ip_addresses: list = None, dns_names: list = None, valid_days: int = 365) -> None:
    """
    ...
    if server_csr_file:
        ...
        server_csr = x509.CertificateSigningRequestBuilder().subject_name(subject).add_extension(x509.SubjectAlternativeName(alt_names), critical=False).sign(private_key, hashes.SHA256())
...
def _import_ca_to_jks_truststore(localhost, cert_file: Path = CA_CERT_FILE, jks_file: Path = JKS_TRUSTSTORE_FILE, jks_password: str = "cassandra") -> None:
    ...
    exit_code, output = java.exec_run(f"keytool -importcert -noprompt -file {tmp_cert_path} -keystore {tmp_truststore_path} -storepass {jks_password}")
    if exit_code != 0:
...
def export_pem_cert_to_pkcs12_keystore(cert_file: Path, cert_key_file: Path, dst_pkcs12_file: Path, cert_key_password: Any = None, p12_password: str = "cassandra") -> None:
    ...
    pkcs12 = serialize_key_and_certificates(name=b"scylladb", key=key, cert=cert, cas=None, encryption_algorithm=serialization.BestAvailableEncryption(p12_password.encode()))
