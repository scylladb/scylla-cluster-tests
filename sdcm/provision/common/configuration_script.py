...
class ConfigurationScriptBuilder(AttrBuilder, metaclass=abc.ABCMeta):
    ...
    def _script_body(self) -> str:
        ...
        if self.logs_transport == "syslog-ng":
            script += configure_syslogng_destination_conf(host=self.syslog_host_port[0], port=self.syslog_host_port[1], throttle_per_second=SYSLOGNG_LOG_THROTTLE_PER_SECOND)
