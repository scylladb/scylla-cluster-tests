    ...
    def build_user_data_yaml(self) -> str:
        ...
        for idx, user_data_object in enumerate(self.user_data_objects):
            ...
            if user_data_object.script_to_run:
                ...
                runcmds.append(f"cd {CLOUD_INIT_SCRIPTS_PATH}; bash -eux {script_path}; test  $? = 0 || touch {script_path}.failed")
        ...
        user_data_yaml = yaml.dump(data={"packages": list(packages), "write_files": scripts, "runcmd": runcmds} | self.yum_repos | self.apt_configuration)
        return "#cloud-config\n" + user_data_yaml
