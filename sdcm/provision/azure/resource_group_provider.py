...
class ResourceGroupProvider:
    ...
    def __post_init__(self):
        ...
        try:
            ...
            assert resource_group.location == self._region, f"resource group {resource_group.name} does not belong to {self._region} region (location)"
    ...
    def get_or_create(self) -> ResourceGroup:
        ...
        resource_group = self._azure_service.resource.resource_groups.create_or_update(
            ...
            parameters={"location": self._region, "tags": {"creation_time": datetime.utcnow().isoformat(sep=" ", timespec="seconds"), "_az": self._az}},
        )
