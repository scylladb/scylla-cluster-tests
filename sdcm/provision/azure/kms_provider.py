...
class AzureKmsProvider:
    ...
    def managed_identity_config(self):
        return {"resource_group": self._kms_config["resource_group"], "identity_name": self._kms_config["identity_name"], "principal_id": self._kms_config["managed_identity_principal_id"]}
    ...
    def _get_managed_identity_id(self) -> str:
        return (
            f"/subscriptions/{self._azure_service.subscription_id}/resourcegroups/{self.managed_identity_config['resource_group']}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{self.managed_identity_config['identity_name']}"
        )
                        ...
                        "access_policies": [
                            {
                                ...
                                "permissions": {"keys": ["get", "encrypt", "decrypt", "wrapKey", "unwrapKey"], "secrets": ["get"], "certificates": ["get"]},
                            },
                            {
                                # SCT service principal
                                "tenant_id": self._azure_service.azure_credentials["tenant_id"],
                                "object_id": self.sct_service_principal_id,
                                "permissions": {"keys": ["create", "get", "list", "update", "import", "delete", "rotate"], "secrets": ["get"], "certificates": ["get"]},
                            },
                        ],
                    },
                },
