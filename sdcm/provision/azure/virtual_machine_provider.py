...
class VirtualMachineProvider:
    ...
    def get_or_create(self, definitions: List[InstanceDefinition], nics_ids: List[str], pricing_model: PricingModel) -> List[VirtualMachine]:
        v_ms = []
        pollers = []
        error_to_raise = None
        for definition, nic_id in zip(definitions, nics_ids):
            ...
            tags = definition.tags | {"ssh_user": definition.user_name, "ssh_key": definition.ssh_key.name, "creation_time": datetime.utcnow().isoformat(sep=" ", timespec="seconds")}
            ...
            if definition.user_data is None:
                ...
                os_profile = self._get_os_profile(
                    computer_name=definition.name, admin_username=definition.user_name, admin_password=binascii.hexlify(os.urandom(20)).decode(), ssh_public_key=definition.ssh_key.public_key.decode(), custom_data=custom_data
                )
                params.update({"user_data": base64.b64encode(builder.get_scylla_machine_image_json().encode("utf-8")).decode("latin-1")})
            storage_profile = self._get_scylla_storage_profile(image_id=definition.image_id, name=definition.name, disk_size=definition.root_disk_size)
            params.update(os_profile)
            params.update(storage_profile)
            params.update(self._get_pricing_params(pricing_model))
            try:
                poller = self._azure_service.compute.virtual_machines.begin_create_or_update(resource_group_name=self._resource_group_name, vm_name=definition.name, parameters=params)
        ...
        for definition, poller in pollers:
            try:
                poller.wait()
                v_m = self._azure_service.compute.virtual_machines.get(self._resource_group_name, definition.name, expand="instanceView")
                if v_m.instance_view and v_m.instance_view.statuses:
                    statuses = ", ".join(f"{status.code}: {status.display_status}" for status in v_m.instance_view.statuses)
    ...
    def delete(self, name: str, wait: bool = True):
        ...
        self._azure_service.compute.virtual_machines.begin_update(self._resource_group_name, vm_name=name, parameters={"storageProfile": {"osDisk": {"createOption": "FromImage", "deleteOption": "Delete"}}})
    ...
    def reboot(self, name: str, wait: bool = True, hard: bool = False) -> None:
        ...
        if wait:
            ...
            while time.time() - start_time < 600:  # 10 minutes
                time.sleep(10)
                instance_view = self._azure_service.compute.virtual_machines.instance_view(self._resource_group_name, vm_name=name)
    ...
    def add_tags(self, name: str, tags: Dict[str, str]) -> VirtualMachine:
        ...
        self._azure_service.compute.virtual_machines.begin_update(self._resource_group_name, name, parameters={"tags": current_tags}).wait()
        v_m = self._azure_service.compute.virtual_machines.get(self._resource_group_name, name)
        self._cache[v_m.name] = v_m
        return v_m

    @staticmethod
    def _get_os_profile(computer_name: str, admin_username: str, admin_password: str, ssh_public_key: str, custom_data: str) -> Dict[str, Any]:
        os_profile = {
            "os_profile": {
    ...
    def _get_scylla_storage_profile(image_id: str, name: str, disk_size: Optional[int] = None) -> Dict[str, Any]:
        ...
        if image_id.startswith("/subscriptions/"):
            ...
            storage_profile["storage_profile"].update({"image_reference": {"community_gallery_image_id": image_id}, "deleteOption": "Delete"})
        else:
    ...
    def run_command(self, name: str, command: str) -> Result:
        ...
        result = self._azure_service.compute.virtual_machines.begin_run_command(self._resource_group_name, name, command_object).result()
    ...
    def _replace_null_value_from_tags_with_empty_string(tags: Dict[str, str]) -> Dict[str, str]:
        ...
        for key, value in tags.items():
            if value == "null":
                LOGGER.warning("Tag '%s' has value 'null' which is not allowed by Azure API. Replacing with empty string.", key)
                tags[key] = ""
        return tags
