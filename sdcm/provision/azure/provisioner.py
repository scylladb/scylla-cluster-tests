...
class AzureProvisioner(Provisioner):
    ...
    def __init__(self, test_id: str, region: str, availability_zone: str, azure_service: AzureService = AzureService(), **config):
        ...
        self._rg_provider = ResourceGroupProvider(self._resource_group_name, self._region, self._az, self._azure_service)
        self._network_sec_group_provider = NetworkSecurityGroupProvider(self._resource_group_name, self._region, self._azure_service)
        self._vnet_provider = VirtualNetworkProvider(self._resource_group_name, self._region, self._az, self._azure_service)
        ...
        self._vm_provider = VirtualMachineProvider(self._resource_group_name, self._region, self._az, self._enable_azure_kms, self._azure_service)
    ...
    @classmethod
    def discover_regions(cls, test_id: str = "", regions: list = None, azure_service: AzureService = AzureService(), **kwargs) -> List["AzureProvisioner"]:
        ...
        all_resource_groups: List[ResourceGroup] = [rg for rg in azure_service.resource.resource_groups.list() if rg.name.startswith("SCT-") and (rg.location in regions if regions else True)]
        if test_id:
            provisioner_params = [(test_id, rg.location, cls._get_az_from_name(rg), azure_service) for rg in all_resource_groups if test_id in rg.name]
        else:
            # extract test_id from rg names where rg.name format is: SCT-<test_id>-<region>-<az>
            provisioner_params = [(test_id, rg.location, cls._get_az_from_name(rg), azure_service) for rg in all_resource_groups if (test_id := rg.name.split("SCT-")[-1][:36]) and len(test_id) == 36]
    ...
    def get_or_create_instances(self, definitions: List[InstanceDefinition], pricing_model: PricingModel = PricingModel.SPOT) -> List[VmInstance]:
        ...
        nics = self._nic_provider.get_or_create(subnet_id, ip_addresses_ids=[address.id for address in ip_addresses], names=[definition.name for definition in definitions_to_provision])
        try:
            v_ms = self._vm_provider.get_or_create(definitions=definitions_to_provision, nics_ids=[nic.id for nic in nics], pricing_model=pricing_model)
    ...
    def _vm_to_instance(self, v_m: VirtualMachine) -> VmInstance:
        ...
        creation_time = datetime.fromisoformat(tags.pop("creation_time")).replace(tzinfo=timezone.utc) if "creation_time" in tags else None
