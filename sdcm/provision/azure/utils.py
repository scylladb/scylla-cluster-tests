...
def get_scylla_images(scylla_version: str, region_name: str, arch: VmArch = VmArch.X86, azure_service: AzureService = AzureService()) -> list[GalleryImageVersion]:
    ...
    if len(version_bucket) == 1:
        if "." in scylla_version:
            ...
            tags_to_search["scylla_version"] = lambda ver: ver and ver.replace("~", "-").startswith(scylla_version) and "-dev" not in ver
        else:
            # commit id
...
def get_released_scylla_images(scylla_version: str, region_name: str, arch: VmArch = VmArch.X86, azure_service: AzureService = AzureService()) -> list[CommunityGalleryImageVersion]:
...
IMAGE_URL_REGEX = re.compile(r".*/resourceGroups/(?P<resource_group_name>.*)/providers/Microsoft.Compute/images/(?P<image_name>.*)")


COMMUNITY_IMAGE_URL_REGEX = re.compile(r".*/CommunityGalleries/(?P<public_gallery_name>.*)/Images/(?P<gallery_image_name>.*)/Versions/(?P<version>.*)")
