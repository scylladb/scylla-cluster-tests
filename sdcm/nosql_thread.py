...
class NoSQLBenchStressThread(DockerBasedStressThread):
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        ...
        result = {}
        with NoSQLBenchStressEvent(node=loader, stress_cmd=stress_cmd, log_file_name=log_file_name) as stress_event, NoSQLBenchEventsPublisher(node=loader, log_filename=log_file_name):
            try:
                # copy graphite-exporter config file to loader
                loader.remoter.send_files(src=self.GRAPHITE_EXPORTER_CONFIG_SRC_PATH, dst=self.GRAPHITE_EXPORTER_CONFIG_DST_PATH, verbose=False)
                ...
                loader.remoter.run(cmd=graphite_run_cmd, timeout=self.timeout + self.shutdown_timeout, log_file=log_file_name, ignore_status=True, timestamp_logs=True)

                result = loader.remoter.run(
                    cmd=f"docker run --name=nb --network=nosql {self.docker_image_name} {stress_cmd} --report-graphite-to graphite-exporter:9109", timeout=self.timeout + self.shutdown_timeout, log_file=log_file_name, timestamp_logs=True
                )
