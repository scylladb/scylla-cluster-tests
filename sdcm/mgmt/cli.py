...
class BackupTask(ManagerTask):
    ...
    def delete_backup_snapshot(self):
        if self.status == TaskStatus.DONE:
            ...
            LOGGER.warning(
                "Did not delete the snapshot of task {}, since the status of said task is {}, and the manager can only delete snapshots of finished tasks".format(
                    self.id, str(self.status)
                )
            )
...
class RestoreTask(ManagerTask):
    ...
    def download_bw(self) -> float | None:
        ...
        try:
            ...
            LOGGER.warning(
                "Failed to extract Download bandwidth from the sctool restore progress output.Check Manager version, bandwidth metrics are supported starting from 3.4."
            )
            return None
    ...
    def load_and_stream_bw(self) -> float | None:
        ...
        try:
            ...
            LOGGER.warning(
                "Failed to extract Load&Stream bandwidth from the sctool restore progress output.Check Manager version, bandwidth metrics are supported starting from 3.4."
            )
            return None
...
class ScyllaManagerTool(ScyllaManagerBase):
    ...
    def add_cluster(
        ...
        if client_encrypt:
            if not db_cluster:
                LOGGER.warning(
                    "db_cluster is not given. Scylla-Manager connection to cluster may fail since not using client-encryption parameters."
                )
...
class SCTool:
    ...
    def get_table_value(self, parsed_table, identifier, column_name=None, is_search_substring=False):
        ...
        column_name_index = (
            column_titles.index(column_name.upper()) if column_name else 1
