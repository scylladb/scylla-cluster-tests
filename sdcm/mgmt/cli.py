...
from sdcm.mgmt.common import TaskStatus, ScyllaManagerError, HostStatus, HostSsl, HostRestStatus, duration_to_timedelta, DEFAULT_TASK_TIMEOUT
...
class ManagerTask:
    ...
    def wait_for_status(self, list_status, check_task_progress=True, timeout=3600, step=120):
        ...
        try:
            return wait.wait_for(func=self.is_status_in_list, step=step, throw_exc=True, text=text, list_status=list_status, check_task_progress=check_task_progress, timeout=timeout)
        except WaitForTimeoutError as ex:
            raise WaitForTimeoutError("Failed on waiting until task: {} reaches status of {}: current task status {}: {}".format(self.id, list_status, self.status, str(ex))) from ex
    ...
    def wait_for_percentage(self, minimum_percentage, timeout=3600, step=10):
        ...
        is_percentage_reached = wait.wait_for(func=self.has_progress_reached_percentage, minimum_percentage=minimum_percentage, step=step, text=text, timeout=timeout, throw_exc=True)
    ...
    def wait_and_get_final_status(self, timeout=DEFAULT_TASK_TIMEOUT, step=60, only_final=False):
        ...
        if only_final:
            ...
            list_final_status = [TaskStatus.ERROR, TaskStatus.ERROR_FINAL, TaskStatus.STOPPED, TaskStatus.DONE, TaskStatus.ABORTED]
...
class BackupTask(ManagerTask):
    ...
    def is_task_in_uploading_stage(self):
        full_progress_string = self.progress_string(parse_table_res=False, is_verify_errorless_result=True).stdout.lower()
    ...
    def wait_for_uploading_stage(self, timeout=1440, step=10):
        ...
        is_status_reached = wait.wait_for(func=self.is_task_in_uploading_stage, step=step, text=text, timeout=timeout, throw_exc=True)
    ...
    def delete_backup_snapshot(self):
        if self.status == TaskStatus.DONE:
            ...
            LOGGER.warning("Did not delete the snapshot of task {}, since the status of said task is {}, and the manager can only delete snapshots of finished tasks".format(self.id, str(self.status)))
...
class RestoreTask(ManagerTask):
    ...
    def download_bw(self) -> float | None:
        ...
        try:
            ...
            LOGGER.warning("Failed to extract Download bandwidth from the sctool restore progress output.Check Manager version, bandwidth metrics are supported starting from 3.4.")
            return None
    ...
    def load_and_stream_bw(self) -> float | None:
        ...
        try:
            ...
            LOGGER.warning("Failed to extract Load&Stream bandwidth from the sctool restore progress output.Check Manager version, bandwidth metrics are supported starting from 3.4.")
            return None
...
class ManagerCluster(ScyllaManagerBase):
    ...
    def get_cluster_id_by_name(self, cluster_name: str):
        try:
            ...
            return self.sctool.get_table_value(parsed_table=cluster_list, column_name=column_to_search, identifier=cluster_name)
    ...
    def create_restore_task(self, restore_schema=False, restore_data=False, location_list=None, snapshot_tag=None, dc_mapping=None, object_storage_method=None, extra_params=None):
    ...
    def create_backup_task(
        ...
        transfers=None,
        object_storage_upload_mode=None,
        legacy_args=None,
    ):
    ...
    def create_repair_task(self, dc_list=None, keyspace=None, interval=None, num_retries=None, fail_fast=None, intensity=None, parallel=None, cron=None, start_date=None, ignore_down_hosts=False):
        # the interval string:
        ...
        return RepairTask(task_id=task_id, cluster_id=self.id, manager_node=self.manager_node)  # return the manager's object with new repair-task-id
    ...
    def get_healthcheck_task(self):
        healthcheck_id = self.sctool.get_table_value(parsed_table=self._get_task_list(), column_name="task", identifier="healthcheck/", is_search_substring=True)
        return HealthcheckTask(task_id=healthcheck_id, cluster_id=self.id, manager_node=self.manager_node)  # return the manager's health-check-task object with the found id
        ...
        for dc_name, hosts_table in dict_status_tables.items():
            if len(hosts_table) < 2:
                ...
                for line in hosts_table[1:]:
                    ...
                    rest_http_status_code = self._extract_value_with_regex(string=rest_value, regex_pattern=r"\(([0-9]*?)\)")
                    ...
                    dict_hosts_health[host] = self._HostHealth(
                        status=HostStatus.from_str(status), rtt=rtt, rest_status=HostRestStatus.from_str(rest_status), rest_rtt=rest_rtt, ssl=HostSsl.from_str(ssl), rest_http_status_code=rest_http_status_code
                    )
...
class ScyllaManagerTool(ScyllaManagerBase):
    ...
    def add_cluster(self, name, host=None, db_cluster=None, client_encrypt=None, disable_automatic_repair=True, auth_token=None, credentials=None, force_non_ssl_session_port=False, alternator_credentials=None):
        """
        ...
        manager_cluster = self.clusterClass(manager_node=self.manager_node, cluster_id=cluster_id, client_encrypt=client_encrypt)
...
class SCTool:
    ...
    def run(self, cmd, is_verify_errorless_result=False, parse_table_res=True, is_multiple_tables=False, replace_broken_unicode_values=True):
    ...
    def run_repair(self, cluster_id, timeout=0):
        ...
        for unit, task in tasks.items():
            ...
            if task_status["status"] != STATUS_DONE or task_status["error"]:
                LOGGER.error("Repair unit %s failed, status: %s, error count: %s", unit, task_status["status"], task_status["error"])
                status = False
