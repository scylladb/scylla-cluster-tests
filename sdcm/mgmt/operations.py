...
class BucketOperations(ClusterTester):
    ...
    def install_azcopy_dependencies(self, node):
        ...
        self.backup_azure_blob_service = f"https://{self.test_config.backup_azure_blob_credentials['account']}.blob.core.windows.net/"
...
class SnapshotOperations(ClusterTester):
    ...
    def get_all_snapshot_files(self, cluster_id):
        ...
        if self.params.get("backup_bucket_backend") == "s3":
            return self._get_all_snapshot_files_s3(cluster_id=cluster_id, bucket_name=bucket_name, region_name=region_name)
...
class DatabaseOperations(ClusterTester):
    ...
    def create_ks_and_tables(self, num_ks, num_table):
        ...
        with self.db_cluster.cql_connection_patient(self.db_cluster.nodes[0]) as session:
            for keyspace in range(num_ks):
                session.execute(f"CREATE KEYSPACE IF NOT EXISTS ks00{keyspace} WITH replication={{'class':'NetworkTopologyStrategy', 'replication_factor':1}}")
                for table in range(num_table):
                    session.execute(f"CREATE COLUMNFAMILY IF NOT EXISTS ks00{keyspace}.table00{table} (key varchar, c varchar, v varchar, PRIMARY KEY(key, c))")
    ...
    def delete_keyspace_directory(self, db_node, keyspace_name):
        ...
        try:
            ...
            if result.stderr:
                raise FilesNotCorrupted("Files were not corrupted. CorruptThenRepair nemesis can't be run. Error: {}".format(result))
            if directory_size_result.stdout:
                directory_size = directory_size_result.stdout[: directory_size_result.stdout.find("\t")]
                self.log.debug("Removed the directory of keyspace {} from node {}\nThe size of the directory is {}".format(keyspace_name, db_node, directory_size))
    ...
    def insert_data_while_excluding_each_node(self, total_num_of_rows, keyspace_name="keyspace2"):
        ...
        for node in self.db_cluster.nodes[1:]:
            ...
            stress_thread = self.run_stress_thread(stress_cmd=stress_command_template.format(num_of_rows_per_insertion, keyspace_name, start_of_range, end_of_range))
            time.sleep(15)
    ...
    def create_missing_rows_in_cluster(self, create_missing_rows_in_multiple_nodes, total_num_of_rows, keyspace_to_be_repaired=None):
        if create_missing_rows_in_multiple_nodes:
            self.insert_data_while_excluding_each_node(total_num_of_rows=total_num_of_rows, keyspace_name=keyspace_to_be_repaired)
...
class StressLoadOperations(ClusterTester, LoaderUtilsMixin):
    ...
    def run_and_verify_stress_in_threads(self, cs_cmds: list[str], stop_on_failure: bool = False) -> None:
        ...
        for stress_cmd in cs_cmds:
            _thread = self.run_stress_thread(stress_cmd=stress_cmd, round_robin=True, stop_test_on_failure=stop_on_failure)
...
class ManagerTestFunctionsMixIn(
        ...
        email_data.update(
            {
                ...
                "manager_agent_repo": (self.params.get("scylla_mgmt_agent_address") or self.params.get("scylla_mgmt_address")),
    ...
    def locations(self) -> list[str]:
        ...
        buckets = [bucket.format(region=region) for bucket in bucket_locations] if isinstance(bucket_locations, list) else bucket_locations.format(region=region).split()
    ...
    def verify_backup_success(self, mgr_cluster, backup_task, ks_names: list = None, tables_names: list = None, truncate=True, restore_data_with_task=False, timeout=None):
        if ks_names is None:
            ks_names = ["keyspace1"]
        ...
        if restore_data_with_task:
            self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=backup_task.get_snapshot_tag(), timeout=timeout, restore_data=True)
        else:
            snapshot_tag = backup_task.get_snapshot_tag()
            self.restore_backup_without_manager(mgr_cluster=mgr_cluster, snapshot_tag=snapshot_tag, ks_tables_list=ks_tables_map)
    ...
    def verify_alternator_backup_success(self, mgr_cluster, backup_task, delete_tables: list = None, timeout=None):
        ...
        self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=backup_task.get_snapshot_tag(), timeout=timeout, restore_schema=True)
        self.restore_backup_with_task(mgr_cluster=mgr_cluster, snapshot_tag=backup_task.get_snapshot_tag(), timeout=timeout, restore_data=True)

    def restore_backup_without_manager(self, mgr_cluster, snapshot_tag, ks_tables_list, location=None, precreated_backup=False):
    ...
    def restore_backup_with_task(self, mgr_cluster, snapshot_tag, timeout, restore_schema=False, restore_data=False, location_list=None, extra_params=None, object_storage_method=None):
        ...
        restore_task = mgr_cluster.create_restore_task(
            restore_schema=restore_schema, restore_data=restore_data, location_list=location_list, snapshot_tag=snapshot_tag, dc_mapping=dc_mapping, extra_params=extra_params, object_storage_method=object_storage_method
        )
        ...
        InfoEvent(message=f"The restore task has ended successfully. Restore run time: {restore_task.duration}.").publish()
    ...
    def create_repair_and_alter_it_with_repair_control(self):
        ...
        mgr_cluster = manager_tool.add_cluster(name=self.CLUSTER_NAME + "_repair_control", db_cluster=self.db_cluster, auth_token=self.monitors.mgmt_auth_token)
        ...
        self.create_missing_rows_in_cluster(create_missing_rows_in_multiple_nodes=True, keyspace_to_be_repaired=keyspace_to_be_repaired, total_num_of_rows=292968720)
...
def run_manager_backup(mgr_cluster, locations, object_storage_upload_mode=None, timeout=7200):
    ...
    task = mgr_cluster.create_backup_task(location_list=locations, rate_limit_list=["0"], object_storage_upload_mode=object_storage_upload_mode)
