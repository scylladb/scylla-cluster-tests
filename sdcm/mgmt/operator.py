...
class BaseClass:
    ...
    def _map_dict(data: dict, mapping: dict) -> dict:
        return {mapping[key]: value for key, value in data.items() if value is not None and mapping.get(key, None) is not None}
...
class OperatorManagerCluster(ManagerCluster):
    ...
    def __init__(self, manager_node, cluster_id=None, client_encrypt=False, cluster_name: str = None, scylla_cluster=None):
    ...
    def id(self):
        if self._id is None:
            self._id = wait_for(self.get_cluster_id_by_name, cluster_name=self.cluster_name, timeout=120, text="Waiting manager cluster to appear", throw_exc=True)
        return self._id
    ...
    def _create_operator_backup_task(
        ...
        if name is None:
            name = self._pick_original_name("default-backup-task-name", [so_task.name for so_task in self.operator_backup_tasks])
    ...
    def _create_scylla_operator_repair_task(self, dc_list=None, keyspace=None, interval=None, num_retries=None, fail_fast=None, intensity=None, parallel=None, name=None) -> ScyllaOperatorRepairTask:
        ...
        if name is None:
            name = self._pick_original_name("default-repair-task-name", [so_task.name for so_task in self.operator_repair_tasks])
    ...
    def create_repair_task(self, dc_list=None, keyspace=None, interval=None, num_retries=None, fail_fast=None, intensity=None, parallel=None, name=None) -> RepairTask:
        ...
        so_task = self._create_scylla_operator_repair_task(dc_list=dc_list, keyspace=keyspace, interval=interval, num_retries=num_retries, fail_fast=fail_fast, intensity=intensity, parallel=parallel, name=name)
    ...
    def get_mgr_repair_task(self, so_repair_task: ScyllaOperatorRepairTask) -> Optional[RepairTask]:
        so_repair_task_status = wait_for(func=self.get_operator_repair_task_status, text=f"Waiting until operator repair task: {so_repair_task.name} get it's status", step=2, timeout=300, task_name=so_repair_task.name, throw_exc=True)
    ...
    def get_mgr_backup_task(self, so_backup_task: ScyllaOperatorBackupTask) -> Optional[BackupTask]:
        so_backup_task_status = wait_for(func=self.get_operator_backup_task_status, text=f"Waiting until operator backup task '{so_backup_task.name}' get it's status", step=2, timeout=600, task_name=so_backup_task.name, throw_exc=True)
...
class ScyllaManagerToolOperator(ScyllaManagerTool):
    ...
    def get_cluster(self, cluster_name):
        ...
        if cluster is None:
            cluster = self.clusterClass(manager_node=self.manager_node, cluster_name=cluster_name, scylla_cluster=self.scylla_cluster)
        else:
    ...
    def add_cluster(self, name, host=None, db_cluster=None, client_encrypt=None, disable_automatic_repair=True, auth_token=None, credentials=None, force_non_ssl_session_port=False):
        raise NotImplementedError()
