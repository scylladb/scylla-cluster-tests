...
def run_commands_wait_and_cleanup(
    ...
    for cmd_num, cmd in enumerate(cleanup_commands):
        try:
            ...
            log.debug(
                f"{name}: failed to execute cleanup command {cmd} on node {node} due to the following error: {exc!s}"
            )
...
def get_rate_limit_for_network_disruption(monitoring_set, target_node, rnd) -> Optional[str]:
    ...
    return f"{rnd.randrange(min_limit, max_limit)}{rate_limit_suffix}"
...
class RandomInterruptionNetworkMonkey(NemesisBaseClass):
    ...
    def _disrupt_k8s(self, list_of_timeout_options):
        """Apply a random Chaos Mesh network experiment (delay, loss, corrupt, or rate limit) for a random duration."""
        ...
        with (
            ...
            self.runner.action_log_scope(f"Network interruption on {self.runner.target_node.name} for {duration}"),
        ):
            experiment.start()
    ...
    def _disrupt_k8s(self, list_of_timeout_options):
        ...
        with suppress_expected_unavailability_errors():
            with DbNodeLogger(
                ...
                "block network traffic",
                target_node=self.runner.target_node,
                additional_info=f"for {duration}",
            ):
                experiment.start()
            experiment.wait_until_finished()
            time.sleep(15)
...
class RejectInterNodeNetworkMonkey(NemesisBaseClass):
    ...
    def disrupt(self):
        ...
        InfoEvent(
            f"{type(self).__name__} {textual_matching_rule} that belongs to "
...
class RejectNodeExporterNetworkMonkey(NemesisBaseClass):
    ...
    def disrupt(self):
        """Drop or reject node-exporter traffic (port 9100) with a random iptables rule for a random duration."""
        install_iptables(self.runner.target_node)
...
class RejectThriftNetworkMonkey(NemesisBaseClass):
    """Generates random firewall rule to drop/reject packets for thrift connections, port 9160."""

    disruptive = True
    networking = True

    def disrupt(self):
        """Drop or reject Thrift traffic (port 9160) with a random iptables rule for a random duration."""
        install_iptables(self.runner.target_node)
