...
class CDCLogReaderThread(DockerBasedStressThread):
    ...
    def run(self):
        ...
        for loader_idx in range(len(self.loaders)):
            for cpu_idx in range(self.stress_num):
                self.results_futures += [self.executor.submit(self._run_stress, *(self.loaders[loader_idx], loader_idx, cpu_idx))]
        return self
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        ...
        docker = cleanup_context = RemoteDocker(loader, self.docker_image_name, extra_docker_opts=f"--network=host --label shell_marker={self.shell_marker}")
        ...
        try:
            with cleanup_context:
                result = docker.run(cmd=node_cmd, timeout=self.timeout + self.shutdown_timeout, ignore_status=True, log_file=log_file_name, verbose=True, retry=0)
                if not result.ok and not self.termination_event.is_set():
                    CDCReaderStressEvent.error(node=loader, stress_cmd=self.stress_cmd, errors=result.stderr.split("\n")).publish()
                return result
