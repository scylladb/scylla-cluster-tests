...
class TombstoneGcVerificationThread:
    def __init__(self, db_cluster: [BaseScyllaCluster, BaseCluster], duration: int, interval: int, termination_event: threading.Event, **kwargs):
    ...
    def _wait_until_user_table_exists(self, db_node, table_name: str = "random", timeout_min: int = 40):
        ...
        if table_name.lower() == "random":
            wait.wait_for(func=lambda: len(self.db_cluster.get_non_system_ks_cf_list(db_node)) > 0, step=60, text=text, timeout=60 * timeout_min, throw_exc=True)
        else:
            wait.wait_for(func=lambda: table_name in (self.db_cluster.get_non_system_ks_cf_list(db_node)), step=60, text=text, timeout=60 * timeout_min, throw_exc=True)
    ...
    def tombstone_gc_verification(self, max_sstable_num: int = 50):
        ...
        delta_repair_date_minutes = (int((datetime.datetime.now() - table_repair_date).total_seconds()) - self._sstable_utils.propagation_delay_in_seconds) // 60
        self.log.debug(
        ...
        if delta_repair_date_minutes <= 0:
            self.log.debug("Table %s repair date is smaller than propagation delay, aborting.", self._sstable_utils.ks_cf)
            return
        sstables = self._sstable_utils.get_sstables(from_minutes_ago=delta_repair_date_minutes)
        if not sstables:
            self.log.warning("No sstable with a creation time of last %s minutes found, aborting.", delta_repair_date_minutes)
            return
        ...
        for sstable in sstables:
            self._sstable_utils.verify_post_repair_sstable_tombstones(table_repair_date=table_repair_date, sstable=sstable)
