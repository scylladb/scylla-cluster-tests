...
class StressExporter(FileFollowerThread, metaclass=ABCMeta):
    ...
    def __init__(self, instance_name: str, metrics: NemesisMetrics, stress_operation: str, stress_log_filename: str, loader_idx: int, cpu_idx: int = 1, keyspace: str = ""):
        super().__init__()
...
class CassandraStressExporter(StressExporter):
    ...
    def create_metrix_gauge(self):
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(gauge_name, "Gauge for cassandra stress metrics", [f"cassandra_stress_{self.stress_operation}", "instance", "loader_idx", "cpu_idx", "type", "keyspace"])
        return gauge_name

    def metrics_position_in_log(self) -> MetricsPosition:
        return MetricsPosition(ops=2, lat_mean=5, lat_med=6, lat_perc_95=7, lat_perc_99=8, lat_perc_999=9, lat_max=10, errors=13)
...
class LatteExporter(StressExporter):
    ...
    def create_metrix_gauge(self):
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(gauge_name, "Gauge for latte metrics", [f"latte_{self.stress_operation}", "instance", "loader_idx", "cpu_idx", "type", "keyspace"])
        return gauge_name
    ...
    def split_line(line: str) -> list:
        ...
        if len(ret) != 12:
            LOGGER.error("'%s' line got splitted in the following unexpected list: %s", line, ret)
        return ret
...
class CassandraStressHDRExporter(StressExporter):
    ...
    def __init__(self, hdr_tags: list[str], instance_name: str, metrics: NemesisMetrics, stress_operation: str, stress_log_filename: str, loader_idx: int, cpu_idx: int = 1, keyspace: str = ""):
        super().__init__(instance_name, metrics, stress_operation, stress_log_filename, loader_idx, cpu_idx, keyspace)
    ...
    def create_metrix_gauge(self):
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(gauge_name, "Gauge for cassandra stress hdr percentiles", [f"cassandra_stress_hdr_{self.stress_operation}", "instance", "loader_idx", "cpu_idx", "type", "keyspace"])
        return gauge_name
    ...
    def set_metric(self, name: str, value: float) -> None:
        self.stress_metric.labels(self.current_line_hdr_tag, self.instance_name, self.loader_idx, self.cpu_idx, name, self.keyspace).set(value)

    def split_line(self, line: str) -> list:
        summary_data = make_hdrhistogram_summary_from_log_line(hdr_tags=self.hdr_tags, stress_operation=self.stress_operation, log_line=line, hst_log_start_time=self.log_start_time)
...
class LatteHDRExporter(CassandraStressHDRExporter):
    def create_metrix_gauge(self):
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(gauge_name, "Gauge for latte hdr percentiles", [f"latte_hdr_{self.stress_operation}", "instance", "loader_idx", "cpu_idx", "type", "keyspace"])
        return gauge_name
...
class CqlStressHDRExporter(CassandraStressHDRExporter):
    def create_metrix_gauge(self):
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(gauge_name, "Gauge for cql-stress hdr percentiles", [f"cql_stress_hdr_{self.stress_operation}", "instance", "loader_idx", "cpu_idx", "type", "keyspace"])
        return gauge_name
...
class CqlStressCassandraStressExporter(StressExporter):
    ...
    def __init__(self, instance_name: str, metrics: NemesisMetrics, stress_operation: str, stress_log_filename: str, loader_idx: int, cpu_idx: int = 1):
        self.keyspace_regex = re.compile(r".*Keyspace:\s(.*?)$")
        self.do_skip = True
    ...
    def create_metrix_gauge(self):
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(
                gauge_name, "Gauge for cql-stress cassandra stress metrics", [f"cql_stress_cassandra_stress_{self.stress_operation}", "instance", "loader_idx", "cpu_idx", "type", "keyspace"]
            )
        return gauge_name
    ...
    def metrics_position_in_log(self) -> MetricsPosition:
        ...
        """
        return MetricsPosition(ops=1, lat_mean=2, lat_med=3, lat_perc_95=4, lat_perc_99=5, lat_perc_999=6, lat_max=7, errors=9)
...
class ScyllaBenchStressExporter(StressExporter):
    def create_metrix_gauge(self) -> str:
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(gauge_name, "Gauge for scylla-bench stress metrics", [f"scylla_bench_stress_{self.stress_operation}", "instance", "loader_idx", "cpu_idx", "type", "keyspace"])
        return gauge_name
    ...
    def metrics_position_in_log(self) -> MetricsPosition:
        ...
        return MetricsPosition(ops=1, lat_mean=10, lat_med=9, lat_perc_95=7, lat_perc_99=6, lat_perc_999=5, lat_max=4, errors=3)
