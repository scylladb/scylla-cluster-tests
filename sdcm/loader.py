...
class CassandraStressHDRExporter(StressExporter):
    ...
    def set_metric(self, name: str, value: float) -> None:
        self.stress_metric.labels(
            self.current_line_hdr_tag, self.instance_name, self.loader_idx, self.cpu_idx, name, self.keyspace
        ).set(value)
...
class CqlStressCassandraStressExporter(StressExporter):
    ...
    def create_metrix_gauge(self):
        ...
        if gauge_name not in self.METRICS_GAUGES:
            self.METRICS_GAUGES[gauge_name] = self.metrics.create_gauge(
                ...
                [
                    f"cql_stress_cassandra_stress_{self.stress_operation}",
                    "instance",
                    "loader_idx",
                    "cpu_idx",
                    "type",
    ...
    def metrics_position_in_log(self) -> MetricsPosition:
        ...
        """
        return MetricsPosition(
            ops=1, lat_mean=2, lat_med=3, lat_perc_95=4, lat_perc_99=5, lat_perc_999=6, lat_max=7, errors=9
        )
