...
class EC2ClientWrapper:
    ...
    def _request_spot_fleet(
        self, instance_type, image_id, region_name, network_if, key_pair="", user_data="", count=3, block_device_mappings=None, aws_instance_profile=None, placement_group_name=None, tag_specifications=Sequence[TagSpecificationTypeDef]
    ):
    ...
    def _is_fleet_request_fulfilled(self, request_id):
        ...
        for req in resp["SpotFleetRequestConfigs"]:
            if req["SpotFleetRequestState"] != "active" or "ActivityStatus" not in req or req["ActivityStatus"] != STATUS_FULFILLED:
                if "ActivityStatus" in req and req["ActivityStatus"] == SPOT_STATUS_UNEXPECTED_ERROR:
                    ...
                    resp = self._client.describe_spot_fleet_request_history(SpotFleetRequestId=request_id, StartTime=search_start_time, MaxResults=10)
    ...
    def add_tags(self, instance_ids: list[str] | list[Instance] | str | Instance, tags: dict = None) -> None:
        ...
        instance_ids = [instance_or_id if isinstance(instance_or_id, str) else instance_or_id.instance_id for instance_or_id in instance_ids]
    ...
    def create_spot_fleet(
        ...
        if not instance_ids:
            err_code = resp if resp in [FLEET_LIMIT_EXCEEDED_ERROR, SPOT_CAPACITY_NOT_AVAILABLE_ERROR] else SPOT_STATUS_UNEXPECTED_ERROR
            raise CreateSpotFleetError(error_response={"Error": {"Code": err_code, "Message": resp}}, operation_name="create_spot_fleet")
    ...
    def add_capacity_reservation_param(boto3_params, availability_zone):
        if cr_id := SCTCapacityReservation.reservations.get(availability_zone).get(boto3_params["InstanceType"]):
            boto3_params["CapacityReservationSpecification"] = {"CapacityReservationTarget": {"CapacityReservationId": cr_id}}
