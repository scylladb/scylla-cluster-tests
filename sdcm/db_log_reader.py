...
class DbLogReader(Process):
    ...
    def _should_skip_decoding(self, event: LogEvent) -> bool:
        ...
        if self._disable_regex_compiled and self._disable_regex_compiled.match(event.type):
            LOGGER.debug("Skipping backtrace decoding for event type '%s' (matches backtrace_decoding_disable_regex)", event.type)
            return True
    ...
    def run(self):
        ...
        """
        LOGGER.debug("Logging for node %s is started with following configuration:\nsystem_log=%s\nlog_lines=%s\ndecoding_queue=%s", self._node_name, self._system_log, self._log_lines, self._decoding_queue is not None)
    ...
    def filter_backtraces(self, backtrace):
        ...
        try:
            if self._last_error and backtrace["event"].line_number <= self._last_error.line_number + 20 and not self._last_error.type == "BACKTRACE" and backtrace["event"].type == "BACKTRACE":
