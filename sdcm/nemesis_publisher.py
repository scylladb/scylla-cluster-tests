    ...
    def publish(self, disrupt_name, status=True, data=None):
        ...
        if status:
            new_nemesis_data.update(
                nemesis_name=disrupt_name, nemesis_duration=data["duration"], start_time=datetime.utcfromtimestamp(data["start"]), end_time=datetime.utcfromtimestamp(data["end"]), target_node=data["node"], outcome="passed"
            )
            ...
            if diff > 1.0:
                ...
                LOGGER.warning("Got too big error message running '%s' nemesis: %sMb.\nThe limit is '%sMb'. Trimming the error message...", disrupt_name, error_message_size_mb, self.error_message_size_limit_mb)
