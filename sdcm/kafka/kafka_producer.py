...
class KafkaThreadBase(Thread):
    ...
    def _check_thread_options(self, **kwargs) -> None:
        ...
        if missed_options:
            TestFrameworkEvent(source=self.__class__.__name__, message=f"Mandatory options are missed {missed_options} for python_thread {self.__class__.__name__}.", severity=Severity.CRITICAL).publish()
    ...
    def generate_field_value(self, field_type: str, rand_gen: random.Random) -> int | float | str | bool | bytes | None:
        ...
        try:
            ...
            TestFrameworkEvent(source=self.__class__.__name__, message=f"Field type '{field_type}' for Kafka record is not supported.", severity=Severity.ERROR).publish()
...
class KafkaProducerThread(KafkaThreadBase):
    ...
    def delivery_report(self, err: str, record: Message) -> None:
        if err:
            ...
            LOGGER.debug("Kafka record delivered to %s [%s] at offset %s", record.topic(), record.partition(), record.offset())
    ...
    def run(self) -> None:
        ...
        while not self.termination_event.is_set():
            ...
            if self.record_format == "avro":
                ...
                key_serialized = self.key_serializer(key, SerializationContext(self.topic, MessageField.KEY)) if self.key_serializer else None
                value_serialized = self.value_serializer(value, SerializationContext(self.topic, MessageField.VALUE)) if self.value_serializer else None
...
class KafkaValidatorThread(KafkaThreadBase):
        ...
        while not self.termination_event.is_set():
            ...
            while True:
                ...
                if len(rows) < batch_size:
                    ...
                    LOGGER.debug("Only %s out of %s records found in ScyllaDB for keys %s to %s. Waiting...", len(rows), batch_size, self.current_key, batch_end)
    ...
    def parse_results(self) -> (list[dict | None], list[str | None]):
        ...
        errors = []
        if self.mismatches:
            TestFrameworkEvent(source=self.__class__.__name__, message=f"Kafka records do not match ScyllaDB rows. Mismatches: {self.mismatches}", severity=Severity.ERROR).publish()
