    ...
    def install_connector_from_url(self, connector_url: str):
        if connector_url.startswith("http"):
            if connector_url.endswith(".jar"):
                self.remoter.run(f"curl --show-error --fail -L --create-dirs -O --output-dir {self.docker_compose_path / 'connectors'} {connector_url} ")
            if connector_url.endswith(".zip"):
                self.remoter.run(f"curl --show-error --fail -L -o /tmp/connector.zip {connector_url} && unzip -o /tmp/connector.zip -d {self.docker_compose_path / 'connectors'} && rm /tmp/connector.zip")
