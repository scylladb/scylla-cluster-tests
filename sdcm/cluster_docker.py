...
class DockerNode(cluster.BaseNode, NodeContainerMixin):
    ...
    def _init_remoter(self, ssh_login_info):
        ...
        # patch sudo for docker containers - run as root unless user is specified
        def docker_sudo(cmd, *args, **kwargs):
            if 'user' not in kwargs:
                kwargs['user'] = 'root'
            return self.remoter.run(cmd, *args, **kwargs)
        self.remoter.sudo = docker_sudo
