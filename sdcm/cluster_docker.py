...
class NodeContainerMixin:
    ...
    def node_container_run_args(self, seed_ip):
        ...
        return dict(
            name=self.name, image=self.node_container_image_tag, command=f'--seeds="{seed_ip}"' if seed_ip else None, volumes=volumes, network=self.parent_cluster.params.get("docker_network"), nano_cpus=smp * 10**9
        )  # Same as `docker run --cpus=N ...' CLI command.


class DockerNode(cluster.BaseNode, NodeContainerMixin):
    def __init__(self, parent_cluster: "DockerCluster", container: Optional[Container] = None, node_prefix: str = "node", base_logdir: Optional[str] = None, ssh_login_info: Optional[dict] = None, node_index: int = 1) -> None:
        super().__init__(name=f"{node_prefix}-{node_index}", parent_cluster=parent_cluster, ssh_login_info=ssh_login_info, base_logdir=base_logdir, node_prefix=node_prefix)
    ...
    def get_service_status(self, service_name: str, timeout: int = 500, ignore_status=False):
        ...
        return type("ServiceStatus", (), {"stdout": f"{service_name} {container_status}", "stderr": "", "ok": container_status in ["running", "active", "RUNNING"]})
...
class VectorStoreDockerNode(VectorStoreNodeMixin, DockerNode):
    ...
    def __init__(self, parent_cluster: "VectorStoreSetDocker", container: Optional[Container] = None, node_prefix: str = "vector", base_logdir: Optional[str] = None, ssh_login_info: Optional[dict] = None, node_index: int = 1) -> None:
        super().__init__(parent_cluster=parent_cluster, container=container, node_prefix=node_prefix, base_logdir=base_logdir, ssh_login_info=ssh_login_info, node_index=node_index)
    ...
    def vector_container_run_args(self, seed_ip=None):
        ...
        return dict(name=self.name, image=self.node_container_image_tag, environment=environment, ports=ports, network=self.parent_cluster.params.get("docker_network"))
...
class DockerCluster(cluster.BaseCluster):
    ...
    def node_container_context_path(self):
        ...
        return os.path.join(os.path.dirname(__file__), "../docker/scylla-sct", self.params.get("scylla_linux_distro").split("-")[0])

    def _create_node(self, node_index, container=None):
        node = DockerNode(
            parent_cluster=self, container=container, ssh_login_info=dict(hostname=None, user=self.node_container_user, key_file=self.node_container_key_file), base_logdir=self.logdir, node_prefix=self.node_prefix, node_index=node_index
        )
...
class ScyllaDockerCluster(cluster.BaseScyllaCluster, DockerCluster):
    def __init__(
        self, docker_image: str = DEFAULT_SCYLLA_DB_IMAGE, docker_image_tag: str = DEFAULT_SCYLLA_DB_IMAGE_TAG, node_key_file: Optional[str] = None, user_prefix: Optional[str] = None, n_nodes: Union[list, str] = 3, params: dict = None
    ) -> None:
        cluster_prefix = cluster.prepend_user_prefix(user_prefix, "db-cluster")
        node_prefix = cluster.prepend_user_prefix(user_prefix, "db-node")
        super().__init__(docker_image=docker_image, docker_image_tag=docker_image_tag, node_key_file=node_key_file, cluster_prefix=cluster_prefix, node_prefix=node_prefix, node_type="scylla-db", n_nodes=n_nodes, params=params)
    ...
    def node_startup(self, node, verbose=False, timeout=3600):
        ...
        for event in check_nodes_status(nodes_status=node.get_nodes_status(), current_node=node, removed_nodes_list=self.dead_nodes_ip_address_list):
            event.publish()
    ...
    def check_aio_max_nr(node: DockerNode, recommended_value: int = AIO_MAX_NR_RECOMMENDED_VALUE):
        ...
        if aio_max_nr < recommended_value:
            raise ScyllaDockerRequirementError(f"{node}: value of sysctl key `fs.aio-max-nr' ({aio_max_nr}) is less than recommended value ({recommended_value})")
    ...
    def get_scylla_args(self):
        append_scylla_args = self.params.get("append_scylla_args_oracle") if self.name.find("oracle") > 0 else self.params.get("append_scylla_args")
...
class VectorStoreSetDocker(VectorStoreClusterMixin, DockerCluster):
    ...
    def _create_node(self, node_index, container=None):
        node = VectorStoreDockerNode(parent_cluster=self, container=container, node_prefix=self.node_prefix, base_logdir=self.logdir, node_index=node_index)
...
class LoaderSetDocker(cluster.BaseLoaderSet, DockerCluster):
    def __init__(
        ...
        DockerCluster.__init__(self, docker_image=docker_image, docker_image_tag=docker_image_tag, node_key_file=node_key_file, cluster_prefix=cluster_prefix, node_prefix=node_prefix, node_type="loader", n_nodes=n_nodes, params=params)
    ...
    def node_setup(self, node: DockerNode, verbose=False, **kwargs):
        ...
        node.remoter.sudo("ln -sf /usr/lib/jvm/java-1.8.0-openjdk-amd64/jre/bin/java* /etc/alternatives/java", verbose=True, ignore_status=True)
    ...
    def _install_docker_cli(self, node, verbose=False):
        ...
        commands = (
            ...
            else ["curl -L https://download.docker.com/linux/centos/docker-ce.repo -o /etc/yum.repos.d/docker-ce.repo", "microdnf -y update", "microdnf -y install docker-ce-cli"]
        )
...
class DockerMonitoringNode(cluster.BaseNode):
    ...
    def __init__(self, parent_cluster: "MonitorSetDocker", node_prefix: str = "monitor-node", base_logdir: Optional[str] = None, node_index: int = 1, ssh_login_info: Optional[dict] = None) -> None:
        super().__init__(name=f"{node_prefix}-{node_index}", parent_cluster=parent_cluster, base_logdir=base_logdir, node_prefix=node_prefix, ssh_login_info=ssh_login_info)
        self.node_index = node_index
...
class MonitorSetDocker(cluster.BaseMonitorSet, DockerCluster):
    def __init__(self, targets: dict, user_prefix: Optional[str] = None, n_nodes: Union[list, int] = 3, params: dict = None) -> None:
        node_prefix = cluster.prepend_user_prefix(user_prefix, "monitor-node")
        cluster_prefix = cluster.prepend_user_prefix(user_prefix, "monitor-set")

        cluster.BaseMonitorSet.__init__(self, targets=targets, params=params)
        DockerCluster.__init__(self, cluster_prefix=cluster_prefix, node_prefix=node_prefix, node_type="monitor", n_nodes=n_nodes, params=params)

    def _create_node(self, node_index, container=None):
        node = DockerMonitoringNode(parent_cluster=self, base_logdir=self.logdir, node_prefix=self.node_prefix, node_index=node_index, ssh_login_info=dict(hostname=None, user=self.node_container_user, key_file=self.node_container_key_file))
        node.init()
        return node
