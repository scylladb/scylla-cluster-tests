...
class DockerNode(cluster.BaseNode, NodeContainerMixin):
    ...
    def start_scylla_server(self, verify_up=True, verify_down=False, timeout=300, verify_up_timeout=None):
        ...
        self._ensure_manager_agent_running()
    ...
    def _ensure_manager_agent_running(self):
        """Re-launch the manager agent if it was installed but its process died (e.g. container restart by nemesis).

        Idempotent: a no-op when the agent was never installed, or when it is still running.
        """
        container = ContainerManager.get_container(self, "node")
        exit_code, _ = container.exec_run("test -x /usr/bin/scylla-manager-agent")
        if exit_code != 0:
            return  # agent was never installed on this node
        exit_code, _ = container.exec_run("pgrep -f /usr/bin/scylla-manager-agent")
        if exit_code == 0:
            return  # agent is still running, don't spawn a second process
        self.remoter.run(
            "sudo bash -c 'nohup /usr/bin/scylla-manager-agent > /var/log/scylla-manager-agent.log 2>&1 &'",
        )
        self.wait_manager_agent_up(timeout=60)
...
class ScyllaDockerCluster(cluster.BaseScyllaCluster, DockerCluster):
    ...
    def install_scylla_manager(self, node):
        ...
        """
        mgmt_agent_image = f"scylladb/scylla-manager-agent:{self.params.get('scylla_mgmt_agent_version')}"
...
class MonitorSetDocker(cluster.BaseMonitorSet, DockerCluster):
    # The manager server's REST API port, same value the nodes use to reach a manager.
    MANAGER_HTTP_PORT = cluster.BaseNode.MANAGER_SERVER_PORT
    ...
    def install_scylla_manager(self, node):
        ...
        mgmt_image = self.params.get("mgmt_docker_image")
        ...
        # The image's ENTRYPOINT is the scylla-manager binary itself -- there is no shell
        # entrypoint expanding env vars into the config, and the binary reads only
        # /etc/scylla-manager/scylla-manager.yaml, whose baked database.hosts is the literal
        # name "scylla-manager-db". Nothing would resolve that on the default bridge, so
        # point the config at the DB container's IP by mounting our own copy over it.
        mgr_config_path = Path(self.logdir) / "scylla-manager.yaml"
        mgr_config_path.write_text(
            dedent(f"""\
                http: ':{self.MANAGER_HTTP_PORT}'
                https: ':5443'
        ...
        self._manager_container = docker_client.containers.run(
            ...
            name=mgr_name,
            network=docker_network,
            volumes={str(mgr_config_path): {"bind": "/etc/scylla-manager/scylla-manager.yaml", "mode": "ro"}},
            detach=True,
        )
        ...
        for _ in range(60):
            ...
            exit_code, output = self._manager_container.exec_run(
                ...
                f"http://127.0.0.1:{self.MANAGER_HTTP_PORT}/ping"
            )
