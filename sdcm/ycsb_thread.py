    ...
    def __init__(self, loader_node, loader_idx, ycsb_log_filename):
        ...
        for operation in self.collectible_ops:
            ...
            if gauge_name not in self.METRICS:
                ...
                self.METRICS[gauge_name] = metrics.create_gauge(gauge_name, "Gauge for ycsb metrics", ["instance", "loader_idx", "uuid", "type"])
...
class YcsbStressThread(DockerBasedStressThread):
    ...
    def parse_final_output(result):
        ...
        latency_99_regex = re.compile(r"\[(READ|INSERT|UPDATE)\],\s99thPercentileLatency\(us\),\s(?P<latency_99th_percentile>.*)")
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):  # noqa: PLR0914
        ...
        if "k8s" in self.params.get("cluster_backend"):
            ...
            if self.params.get("alternator_use_dns_routing"):
                LOGGER.info("Ignoring the 'alternator_use_dns_routing' option running on K8S, because it is always used in this case.")
        else:
            ...
            if self.params.get("alternator_use_dns_routing"):
                dns = RemoteDocker(loader, dns_image, command_line=dns_cmd, extra_docker_opts=f"--label shell_marker={self.shell_marker}", docker_network=self.params.get("docker_network"))
            ...
            cmd_runner = RemoteDocker(loader, self.docker_image_name, command_line="-c 'tail -f /dev/null'", extra_docker_opts=extra_docker_opts, docker_network=self.params.get("docker_network"))
        ...
        with YcsbStatsPublisher(loader, loader_idx, ycsb_log_filename=log_file_name):
            try:
                ...
                result = cmd_runner.run(
                    ...
                    watchers=[FailuresWatcher(r"\sERROR|=UNEXPECTED_STATE|=ERROR", callback=raise_event_callback, raise_exception=False)],
                    retry=0,
                    timestamp_logs=True,
                )
                ...
                ycsb_finish_event = YcsbStressEvent.finish(node=cmd_runner_name, stress_cmd=stress_cmd, log_file_name=log_file_name)
