...
class YcsbStressThread(DockerBasedStressThread):
    ...
    def build_stress_cmd(self, loader_idx, cpu_idx):
        ...
        if self.params.get("use_hdrhistogram"):
            stress_cmd += f" -p measurement.interval=intended -p measurementtype=hdrhistogram -p hdrhistogram.fileoutput=true -p status.interval=1"
    ...
    def _fix_hdr_files(self, loader_idx, cpu_idx):
        # This is a workaround for two issues with HDR files generated by YCSB:
        # - the generated files are not tagged, so we tag them manually
    ...
    def _terminate_hdr_loggers(self, contextes, loader_idx, cpu_idx):
        ...
        for hdrh_logger in contextes:
            hdrh_logger.stop()
        self._fix_hdr_files(loader_idx, cpu_idx)
