...
class GceSctRunner(SctRunner):
    ...
    # GCE nested virtualization rules out E2, H4D, every memory-optimized series (M1, M2, M3,
    # M4, X4) and everything on AMD or Arm silicon - which drops N2D, C2D, C3D (AMD EPYC) and
    # T2D/T2A along with the GPU series. That leaves the Intel general- and compute-optimized
    # families below.
    # See: https://docs.cloud.google.com/compute/docs/instances/nested-virtualization/overview
    NESTED_VIRTUALIZATION_INSTANCE_FAMILIES = ("n1", "n2", "c2", "c3")
...
class AzureSctRunner(SctRunner):
    ...
    def _network_interfaces(self) -> list[dict]:
        ...
        The runner always has one interface - 'scylla_network_config' describes the DB nodes -
