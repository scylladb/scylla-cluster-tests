...
class SctRunner(ABC):
    ...
    @abstractmethod
    def _create_instance(self, instance_type: str, base_image: Any, tags: dict[str, str], instance_name: str, root_disk_size_gb: int = 0, region_az: str = "", test_duration: int | None = None, address_pool: str | None = None) -> Any: ...
    ...
    def create_image(self) -> None:
        ...
        if not source_image:
            ...
            LOGGER.info("SCT Runner image exists in the source region `%s'! ID: %s", self.SOURCE_IMAGE_REGION, self._get_image_id(image=source_image))
    ...
    def create_instance(self, test_id: str, test_name: str, test_duration: int, instance_type: str = "", root_disk_size_gb: int = 0, restore_monitor: bool = False, restored_test_id: str = "", address_pool: str | None = None) -> Any:
        LOGGER.info("Creating SCT Runner instance...")
        image = self.image
        if not image:
            LOGGER.error("SCT Runner image was not found in %s! Use `hydra create-runner-image --cloud-provider %s --region %s'", self.region_name, self.CLOUD_PROVIDER, self.region_name)
            return None
...
class AwsSctRunner(SctRunner):
    ...
    def _image(self, image_type: ImageType = ImageType.SOURCE) -> Any:
        ...
        assert len(existing_amis) == 1, f"More than 1 SCT Runner AMI with {self.image_name}:{self.VERSION} found in {self.region_name}: {existing_amis}"

        return aws_region.resource.Image(existing_amis[0]["ImageId"])

    def _create_instance(
        self, instance_type: InstanceTypeType, base_image: Any, tags: dict[str, str], instance_name: str, root_disk_size_gb: int = 0, region_az: str = "", test_duration: int | None = None, address_pool: str | None = None
    ) -> Any:
        ...
        ec2_network_configuration_subnets = EC2NetworkConfiguration(regions=[aws_region.region_name], availability_zones=[self.availability_zone], params=self.params).subnets_per_region
        assert ec2_network_configuration_subnets, f"No SCT subnet found in the source region. Use `hydra prepare-regions --cloud-provider aws --region-name {aws_region.region_name}' to create cloud env!"
        ...
        base_image = convert_name_to_ami_if_needed(ami_id_param=base_image, region_names=tuple([aws_region.region_name]))
            ...
            TagSpecifications=[
                {
                    ...
                    "Tags": [{"Key": key, "Value": value} for key, value in tags.items()] + [{"Key": "Name", "Value": instance_name}],
                }
            ],
            BlockDeviceMappings=[
                {"DeviceName": ec2_ami_get_root_device_name(image_id=base_image, region_name=aws_region.region_name), "Ebs": {"VolumeSize": root_disk_size_gb or self.instance_root_disk_size(test_duration), "VolumeType": "gp3"}}
            ],
        )
        instance = result[0]
        ...
        if address_pool:
            ...
            unassigned_addresses = [addr for addr in aws_region.resource.vpc_addresses.filter(Filters=[{"Name": "tag:sct-runner-pool", "Values": [address_pool]}]) if not addr.instance_id]
        ...
        if len(instance.network_interfaces) > 1:
            for interface in instance.network_interfaces:
                if interface.attachment["DeviceIndex"] == 0 and interface.association_attribute is None:
                    ...
                    aws_region.resource.create_tags(Resources=[eip_allocation_id], Tags=tags_as_ec2_tags(TestConfig().common_tags()))
                    break
    ...
    def _setup_disks(remoter: Any, disks: list, mount_point: str) -> None:
        ...
        if len(disks) != 1:
            remoter.sudo(f"mdadm --create --verbose {raid_device} --level={raid_level} --raid-devices={len(disks)} " + " ".join(disks))
    ...
    def _create_image(self, instance: Any) -> Any:
        result = self.aws_region_source.client.create_image(Description=self.IMAGE_DESCRIPTION, InstanceId=instance.instance_id, Name=self.image_name, NoReboot=False)
    ...
    def _copy_source_image_to_region(self) -> None:
        result = self.aws_region.client.copy_image(Description=self.IMAGE_DESCRIPTION, Name=self.image_name, SourceImageId=self.source_image.image_id, SourceRegion=self.SOURCE_IMAGE_REGION)
...
class GceSctRunner(SctRunner):
    ...
    def set_tags(sct_runner_info: SctRunnerInfo, tags: dict):
        ...
        gce_set_labels(instances_client=instances_client, instance=sct_runner_info.instance, new_labels=tags_to_create, project=info["project_id"], zone=sct_runner_info.region_az)
    ...
    def _create_instance(self, instance_type: str, base_image: Any, tags: dict[str, str], instance_name: str, root_disk_size_gb: int = 0, region_az: str = "", test_duration: int | None = None, address_pool: str | None = None) -> Any:
        LOGGER.info("Creating instance...")
        disks = [
            disk_from_image(disk_type=f"projects/{self.project_name}/zones/{region_az}/diskTypes/pd-ssd", disk_size_gb=root_disk_size_gb or self.instance_root_disk_size(test_duration), boot=True, source_image=base_image, auto_delete=True)
        ]
    ...
    def terminate_sct_runner_instance(sct_runner_info: SctRunnerInfo) -> None:
        ...
        instances_client, info = get_gce_compute_instances_client()
        res = instances_client.delete(instance=instance.name, project=info["project_id"], zone=instance.zone.split("/")[-1])
        res.done()
...
class AzureSctRunner(SctRunner):
    ...
    def _create_instance(self, instance_type: str, base_image: Any, tags: dict[str, str], instance_name: str, root_disk_size_gb: int = 0, region_az: str = "", test_duration: int | None = None, address_pool: str | None = None) -> Any:
        ...
        if base_image is self.BASE_IMAGE:
            ...
            test_id = tags["TestId"]
            provisioner = provisioner_factory.create_provisioner(backend="azure", test_id=test_id, region=self.azure_region.location, availability_zone=self.availability_zone)
            ...
            self.instance = provisioner.get_or_create_instance(definition=vm_params, pricing_model=PricingModel.ON_DEMAND)
            return self.instance
...
def list_sct_runners(backend: str = None, test_runner_ip: str = None, user: str = None, test_id: str | tuple = None, verbose: bool = True) -> list[SctRunnerInfo]:
    ...
    if not filtered_runners:
        if test_runner_ip:
            ...
            if test_id:
                filter_desc.append(f"TestIds: {list(test_id) if isinstance(test_id, (tuple, list)) else f'TestId: {test_id}'}")
...
def update_sct_runner_tags(backend: str = None, test_runner_ip: str = None, test_id: str = None, tags: dict = None):
    ...
    if not test_runner_ip and not test_id:
        raise ValueError("update_sct_runner_tags requires either the test_runner_ip or test_id argument to find the runner")
    ...
    if not runner_to_update:
        LOGGER.warning("Could not find SCT runner with IP: %s, test_id: %s to update tags for.", test_runner_ip, test_id)
        return
...
def _manage_runner_keep_tag_value(utc_now: datetime, timeout_flag: bool, test_status: str, sct_runner_info: SctRunnerInfo, dry_run: bool = False) -> SctRunnerInfo:
    LOGGER.info("Managing runner's tags. Timeout flag: %s, logs_collected: %s, dry_run: %s", timeout_flag, sct_runner_info.logs_collected, dry_run)
...
def clean_sct_runners(test_status: str, test_runner_ip: str = None, backend: str = None, user: str = None, test_id: str | tuple = None, dry_run: bool = False, force: bool = False) -> None:
    ...
    for sct_runner_info in sct_runners_list:
        ...
        if sct_runner_info.public_ips and sct_runner_info.cloud_provider == "aws":
            ssh_run_cmd_result = ssh_run_cmd(command=cmd, test_id=sct_runner_info.test_id, node_name=sct_runner_name, force_use_public_ip=True)
        ...
        if not dry_run and test_runner_ip:
            _manage_runner_keep_tag_value(test_status=test_status, utc_now=utc_now, timeout_flag=timeout_flag, sct_runner_info=sct_runner_info, dry_run=dry_run)
