...
class AwsSctRunner(SctRunner):
    ...
    def _image(self, image_type: ImageType = ImageType.SOURCE) -> Any:
        ...
        assert len(existing_amis) == 1, (
            f"More than 1 SCT Runner AMI with {self.image_name}:{self.VERSION} found in {self.region_name}: {existing_amis}"
        )
    ...
    def _create_instance(
        ...
        assert ec2_network_configuration_subnets, (
            f"No SCT subnet found in the source region. Use `hydra prepare-regions --cloud-provider aws --region-name {aws_region.region_name}' to create cloud env!"
        )
    ...
    def _setup_disks(remoter: Any, disks: list, mount_point: str) -> None:
        ...
        if len(disks) != 1:
            remoter.sudo(
                f"mdadm --create --verbose {raid_device} --level={raid_level} --raid-devices={len(disks)} "
                + " ".join(disks)
            )
