...
class KclStressThread(DockerBasedStressThread):
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        docker = cleanup_context = RemoteDocker(loader, self.docker_image_name, extra_docker_opts=f"--label shell_marker={self.shell_marker}")
        stress_cmd = self.build_stress_cmd()

        if not os.path.exists(loader.logdir):
            os.makedirs(loader.logdir, exist_ok=True)
        log_file_name = os.path.join(loader.logdir, "kcl-l%s-c%s-%s.log" % (loader_idx, cpu_idx, uuid.uuid4()))
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        ...
        try:
            ...
            while not self._stop_event.is_set():
                with nemesis_node_allocator.run_nemesis(nemesis_label="Compare tables size by cf-stats", node_list=self.node_list) as node:
