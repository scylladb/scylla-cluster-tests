...
class SecretsManagerEntryMissing(RuntimeError):
    """Raised when a referenced Secrets Manager secret does not exist.

    Wraps the low-level ``ResourceNotFoundException`` from botocore with
    an actionable message pointing to the migration runbook so operators
    can tell at a glance what is missing and how to fix it.
    """


def _sm_not_found(exception):
    """Return True if *exception* is a Secrets Manager ResourceNotFoundException."""
    return (
...
def _raise_missing_secret(secret_name, prefix, original):  # pylint: disable=unused-argument
    """Re-raise an SM ResourceNotFoundException with a clearer, actionable message."""
    raise SecretsManagerEntryMissing(
        f"{secret_name!r} not in Secrets Manager ({KEYSTORE_REGION}); "
        f"run ./scripts/validate-keystore-sm.sh --migrate-missing"
    ) from original
...
class KeyStore:
    ...
    def _fetch_from_secrets_manager(self, file_name):
        ...
        try:
            ...
            if _sm_not_found(exc):
                _raise_missing_secret(secret_name, self._sm_prefix, exc)
            raise
