...
# this lock need to support current nemesis logic in "set_target_node "when nemesis cal release any lock rather than releasing only own locks
...
def unset_running_nemesis(nodes, nemesis=None, raise_on_nemesis_mismatching=True):
    # unlock node
...
def set_running_nemesis(nodes, nemesis, timeout=30):
    ...
    with NEMESIS_TARGET_SELECTION_LOCK:
        ...
        for node in nodes:
            ...
            elif not node.running_nemesis != nemesis:
...
@contextmanager
def lock_nodes(node_list: list[BaseNode], nemesis_label: str, timeout=30):
    try:
        set_running_nemesis(node_list, nemesis_label, timeout)
        try:
            yield
        finally:
            unset_running_nemesis(node_list, nemesis_label)
    except CantAcquireLockException:
        unset_running_nemesis(node_list, nemesis_label, False)
        raise
