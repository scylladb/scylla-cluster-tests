...
class AWSCluster(cluster.BaseCluster):
    ...
    def __init__(
        ...
        ec2_ami_id,
        ec2_subnet_id,
        ec2_security_group_ids,  # noqa: PLR0913
        services,
        credentials,
        cluster_uuid=None,
        ...
        self.params = params
        self.placement_group_name = PlacementGroup(params=params, common_tags=self.test_config.common_tags(), test_id=self.test_config.test_id()).placement_group_name
    ...
    def __str__(self):
        return "Cluster %s (AMI: %s)" % (self.name, self._ec2_ami_id)
    ...
    def _create_on_demand_instances(self, count, interfaces, ec2_user_data, dc_idx=0, instance_type=None, is_zero_node=False):
        ...
        params = dict(
            ...
            TagSpecifications=[{"ResourceType": "instance", "Tags": tags_as_ec2_tags(self.tags)}],
        )
        ...
        LOGGER.debug("Using EC2 service with DC-index: %s, (associated with region: %s)", dc_idx, self.region_names[dc_idx])
    ...
    def _create_instances(self, count, ec2_user_data="", dc_idx=0, az_idx=0, instance_type=None, is_zero_node=False):
        ...
        for i in range(interfaces_amount):
            LOGGER.debug("SubnetId: %s, dc_idx: %s, az index: %s, interface index: %s", self._ec2_subnet_id, dc_idx, az_idx, i)
            interfaces.append({"DeviceIndex": i, "SubnetId": self._ec2_subnet_id[dc_idx][az_idx][i], "Groups": self._ec2_security_group_ids[dc_idx]})
        ...
        if self.instance_provision == "mixed":
            instances = self._create_mixed_instances(count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, is_zero_node=is_zero_node)
        elif self.instance_provision == INSTANCE_PROVISION_ON_DEMAND:
            instances = self._create_on_demand_instances(count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, is_zero_node=is_zero_node)
        elif self.instance_provision == INSTANCE_PROVISION_SPOT_FLEET and count > 1:
            instances = self._create_spot_instances(count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, is_zero_node=is_zero_node)
        else:
            instances = self.fallback_provision_type(count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, is_zero_node=is_zero_node)
    ...
    def check_spot_error(self, cl_ex, instance_provision):
        if (
            ec2_client.MAX_SPOT_EXCEEDED_ERROR in cl_ex
            or ec2_client.FLEET_LIMIT_EXCEEDED_ERROR in cl_ex
            or ec2_client.SPOT_CAPACITY_NOT_AVAILABLE_ERROR in cl_ex
            or ec2_client.SPOT_PRICE_TOO_LOW in cl_ex
            or ec2_client.SPOT_STATUS_UNEXPECTED_ERROR in cl_ex
        ):
    ...
    def _create_mixed_instances(self, count, interfaces, ec2_user_data, dc_idx, instance_type=None, is_zero_node=False):
        ...
        if isinstance(self, (ScyllaAWSCluster, CassandraAWSCluster)):
            ...
            if count_spot > 0:
                ...
                instances.extend(self._create_spot_instances(count_spot, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, is_zero_node=is_zero_node))
            if count_on_demand > 0:
                self.instance_provision = INSTANCE_PROVISION_SPOT
                instances.extend(self._create_on_demand_instances(count_on_demand, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, is_zero_node=is_zero_node))
            self.instance_provision = "mixed"
        elif isinstance(self, LoaderSetAWS):
            self.instance_provision = INSTANCE_PROVISION_SPOT
            instances = self._create_spot_instances(count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type)
        elif isinstance(self, MonitorSetAWS):
            self.instance_provision = INSTANCE_PROVISION_ON_DEMAND
            instances.extend(self._create_on_demand_instances(count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type))
        else:
    ...
    def _get_instances(self, dc_idx, az_idx=None):
        ...
        ec2 = ec2_client.EC2ClientWrapper(region_name=self.region_names[dc_idx])
        results = list_instances_aws(tags_dict={"TestId": test_id, "NodeType": self.node_type}, running=True, region_name=self.region_names[dc_idx], group_as_region=True, availability_zone=availability_zone, verbose=True)
    ...
    def add_nodes(self, count, ec2_user_data="", dc_idx=0, rack=0, enable_auto_bootstrap=False, instance_type=None, is_zero_node=False):
        ...
        for rack_idx, rack_count in enumerate(rack_distribution):
            ...
            instances.extend(self._create_or_find_instances(count=rack_count, ec2_user_data=ec2_user_data, dc_idx=instance_dc, az_idx=rack_idx, instance_type=instance_type, is_zero_node=is_zero_node))
        for node_index, instance in enumerate(instances):
            ...
            node = self._create_node(instance, self._ec2_ami_username, self.node_prefix, self._node_index, self.logdir, dc_idx=dc_idx, rack=node_rack)
    ...
    def _create_node(self, instance, ami_username, node_prefix, node_index, base_logdir, dc_idx, rack):
        ...
        node = AWSNode(
            ec2_instance=instance, ec2_service=ec2_service, credentials=credentials, parent_cluster=self, ami_username=ami_username, node_prefix=node_prefix, node_index=node_index, base_logdir=base_logdir, dc_idx=dc_idx, rack=rack
        )
        node.init()
        return node
...
class AWSNode(cluster.BaseNode):
    ...
    def __init__(self, ec2_instance, ec2_service, credentials, parent_cluster, node_prefix="node", node_index=1, ami_username="root", base_logdir=None, dc_idx=0, rack=0):
        ...
        super().__init__(name=f"{node_prefix}-{self.node_index}", parent_cluster=parent_cluster, ssh_login_info=ssh_login_info, base_logdir=base_logdir, node_prefix=node_prefix, dc_idx=dc_idx, rack=rack)
    ...
    def network_interfaces(self):
        ...
        for interface in self._instance.network_interfaces:
            private_ip_addresses = [to_inet_ntop_format(private_address["PrivateIpAddress"]) for private_address in interface.private_ip_addresses]
            ipv6_addresses = [to_inet_ntop_format(ipv6_address["Ipv6Address"]) for ipv6_address in interface.ipv6_addresses]
            device_indexes.append(interface.attachment["DeviceIndex"])
            ipv4_public_address = to_inet_ntop_format(interface.association_attribute["PublicIp"]) if interface.association_attribute else None
            dns_public_name = interface.association_attribute["PublicDnsName"] if interface.association_attribute else None
            interfaces_tmp.append(
                NetworkInterface(
    ...
    def init(self):
        ...
        self.scylla_network_configuration = ScyllaNetworkConfiguration(network_interfaces=self.network_interfaces, scylla_network_config=self.parent_cluster.params["scylla_network_config"])
        # TODO: keep next two for debug purpose
        self.log.debug("Node %s scylla_network_config: %s", self.name, self.parent_cluster.params["scylla_network_config"])
        self.log.debug("Node %s network_interfaces: %s", self.name, self.scylla_network_configuration.network_interfaces)
        super().init()
    ...
    def _set_keep_duration(self, duration_in_hours: int) -> None:
        self._ec2_service.create_tags(Resources=[self._instance.id], Tags=[{"Key": "keep", "Value": str(duration_in_hours)}])
    ...
    def query_aws_metadata(self, path: str) -> str:
        return self.query_metadata(url=f"{self.METADATA_BASE_URL}{path}", token_url=self.TOKEN_URL, token_ttl_header="X-aws-ec2-metadata-token-ttl-seconds", token_header="X-aws-ec2-metadata-token")

    def check_spot_termination(self):
        try:
            self.wait_ssh_up(verbose=False)
            status = self.query_aws_metadata("spot/instance-action")
    ...
    def _refresh_instance_state(self):
        self._wait_public_ip()
        self.refresh_network_interfaces_info()
        public_ipv4_addresses = [interface.ipv4_public_address for interface in self.scylla_network_configuration.network_interfaces if interface.ipv4_public_address]
        ...
        private_ipv4_addresses = [interface.ipv4_private_addresses[0] for interface in self.scylla_network_configuration.network_interfaces]
    ...
    def _instance_wait_safe(self, instance_method: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        try:
            ...
            try:
                ...
                raise cluster.NodeError(f"Timeout while running '{instance_method.__name__}' method on AWS instance '{self._instance.id}'") from None
    ...
    def cql_address(self):
        address = self.scylla_network_configuration.test_communication if self.test_config.IP_SSH_CONNECTIONS == "public" else self.scylla_network_configuration.broadcast_rpc_address
    ...
    def config_ipv6_as_persistent(self):
        if self.distro.is_ubuntu:
            ...
            meta_url = "network/interfaces/macs/"
            mac = self.query_aws_metadata(meta_url)
            ipv6_cidr = self.query_aws_metadata(f"{meta_url}{mac}/subnet-ipv6-cidr-blocks")
            self.remoter.run(f"sudo sh -c  \"echo 'sudo ip route add {ipv6_cidr} dev eth0' >> /etc/sysconfig/network-scripts/init.ipv6-global\"")
    ...
    def restart(self):
        ...
        if self.is_seed:
            ...
            seed_provider = SeedProvider(class_name="org.apache.cassandra.locator.SimpleSeedProvider", parameters=[{"seeds": f"{random_node.ip_address}"}])
        ...
        with ExitStack() as stack:
            if self.is_data_device_lost_after_reboot:
                ...
                for db_filter in (
                    DbEventsFilter(db_event=DatabaseLogEvent.DATABASE_ERROR, node=self),
    ...
    def release_address(self):
        ...
        try:
            ...
            if exc.response["Error"]["Code"] == "InvalidAllocationID.NotFound":
                self.log.warning("Ignoring InvalidAllocationID.NotFound error when releasing address: %s. The allocation ID '%s' does not exist, likely already released.", exc, self.eip_allocation_id)
            else:
                raise
    ...
    def get_console_screenshot(self):
        result = self._ec2_service.meta.client.get_console_screenshot(InstanceId=self._instance.id)
...
class ScyllaAWSCluster(cluster.BaseScyllaCluster, AWSCluster):
    ...
    def add_nodes(self, count, ec2_user_data="", dc_idx=0, rack=0, enable_auto_bootstrap=False, instance_type=None, is_zero_node=False, **kwargs):
        if not ec2_user_data:
            if self._ec2_user_data and isinstance(self._ec2_user_data, str):
                ec2_user_data = re.sub(r"(--totalnodes\s)(\d*)(\s)", r"\g<1>{}\g<3>".format(len(self.nodes) + count), self._ec2_user_data)
        ...
        added_nodes = super().add_nodes(count=count, ec2_user_data=ec2_user_data, dc_idx=dc_idx, rack=rack, enable_auto_bootstrap=enable_auto_bootstrap, instance_type=instance_type, is_zero_node=is_zero_node, **kwargs)
        return added_nodes
...
class CassandraAWSCluster(ScyllaAWSCluster):
    def __init__(self, ec2_ami_id, ec2_subnet_id, ec2_security_group_ids, services, credentials, ec2_instance_type="c6i.xlarge", ec2_ami_username="ubuntu", ec2_block_device_mappings=None, user_prefix=None, n_nodes=3, params=None):
        if ec2_block_device_mappings is None:
            ec2_block_device_mappings = []
        # We have to pass the cluster name in advance in user_data
    ...
    def add_nodes(self, count, ec2_user_data="", dc_idx=0, rack=0, enable_auto_bootstrap=False, instance_type=None, **kwargs):
        if not ec2_user_data:
            if self.nodes:
                seeds = ",".join(self.get_seed_nodes())
                ec2_user_data = "--clustername %s --totalnodes %s --seeds %s --version community --release 2.1.15" % (self.name, count, seeds)
        ec2_user_data = self.update_bootstrap(ec2_user_data, enable_auto_bootstrap)
        added_nodes = super().add_nodes(count=count, ec2_user_data=ec2_user_data, dc_idx=dc_idx, rack=rack, instance_type=instance_type, **kwargs)
        return added_nodes
...
class LoaderSetAWS(cluster.BaseLoaderSet, AWSCluster):
    def __init__(self, ec2_ami_id, ec2_subnet_id, ec2_security_group_ids, services, credentials, ec2_instance_type="c6i.xlarge", ec2_block_device_mappings=None, ec2_ami_username="centos", user_prefix=None, n_nodes=10, params=None):
        node_prefix = cluster.prepend_user_prefix(user_prefix, "loader-node")
        node_type = "loader"
        cluster_prefix = cluster.prepend_user_prefix(user_prefix, "loader-set")
        user_data = "--clustername %s --totalnodes %s --bootstrap false --stop-services" % (cluster_prefix, n_nodes)
...
class VectorStoreAWSNode(VectorStoreNodeMixin, AWSNode):
    ...
    def __init__(self, ec2_instance, ec2_service, credentials, parent_cluster, node_prefix="vs-node", node_index=1, ami_username="ubuntu", base_logdir=None, dc_idx=0, rack=0):
        super().__init__(
    ...
    def configure_vector_store_service(self):
        ...
        env_content = f"VECTOR_STORE_URI=0.0.0.0:{self.parent_cluster.params.get('vector_store_port')}\nVECTOR_STORE_SCYLLADB_URI={self.scylla_uri}"
        ...
        self.remoter.run(f"sudo chown {self.parent_cluster.params.get('ami_vector_store_user')}: /home/ubuntu/vector-store/.env", verbose=True)
