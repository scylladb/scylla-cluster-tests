...
class AWSCluster(cluster.BaseCluster):  # pylint: disable=too-many-instance-attributes,abstract-method,
    ...
    def __str__(self):
        return "Cluster %s (AMI: %s Type: %s)" % (self.name, self._ec2_ami_id, self._ec2_instance_type)
    ...
    def add_nodes(self, count, ec2_user_data="", dc_idx=0, rack=0, enable_auto_bootstrap=False, instance_type=None):
        ...
        for node_index, instance in enumerate(instances):
            ...
            if (
                self.params.get("ip_ssh_connections") == "ipv6"
                and not node.distro.is_amazon2
                and not node.distro.is_ubuntu
            ):
    ...
    def _create_node(
        ...
        ami_username,
        node_prefix,
        node_index,  # pylint: disable=too-many-arguments
        base_logdir,
        dc_idx,
        rack,
    ):
        node = AWSNode(
            ...
            ec2_service=self._ec2_services[dc_idx],
...
class AWSNode(cluster.BaseNode):
    ...
    def query_aws_metadata(self, path):
        ...
        return self.remoter.run(
            f'curl -s -H "X-aws-ec2-metadata-token: {token}" http://169.254.169.254/{path}', verbose=False
        ).stdout.strip()
    ...
    def private_dns_name(self) -> str:
        return self.query_aws_metadata("latest/meta-data/local-hostname")
    ...
    def allocate_and_attach_elastic_ip(self, parent_cluster, dc_idx):
        primary_interface = [
            interface for interface in self._instance.network_interfaces if interface.attachment["DeviceIndex"] == 0
        ][0]
        ...
        self._eth1_private_ip_address = [
            interface for interface in self._instance.network_interfaces if interface.attachment["DeviceIndex"] == 1
