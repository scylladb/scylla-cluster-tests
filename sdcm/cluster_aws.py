...
class AWSCluster(cluster.BaseCluster):  # pylint: disable=too-many-instance-attributes,abstract-method,
    ...
    def _create_on_demand_instances(self, count, interfaces, ec2_user_data, dc_idx=0, instance_type=None, zero_token=False):  # pylint: disable=too-many-arguments
    ...
    def _create_spot_instances(self, count, interfaces, ec2_user_data='', dc_idx=0, instance_type=None, zero_token=False):  # pylint: disable=too-many-arguments
        # pylint: disable=too-many-locals
        ec2 = ec2_client.EC2ClientWrapper(region_name=self.region_names[dc_idx],
    ...
    def _create_instances(self, count, ec2_user_data='', dc_idx=0, az_idx=0, instance_type=None, zero_token=False):  # pylint: disable=too-many-arguments
        ...
        if self.instance_provision == 'mixed':
            instances = self._create_mixed_instances(count, interfaces, ec2_user_data, dc_idx,
                                                     instance_type=instance_type, zero_token=zero_token)
    ...
    def fallback_provision_type(self, count, interfaces, ec2_user_data, dc_idx, instance_type=None, zero_token=False):  # pylint: disable=too-many-arguments
        instances = None
            ...
            try:
                ...
                if instances_provision_type == INSTANCE_PROVISION_ON_DEMAND:
                    instances = self._create_on_demand_instances(
                        count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, zero_token=zero_token)
                else:
    ...
    def _create_mixed_instances(self, count, interfaces, ec2_user_data, dc_idx, instance_type=None, zero_token=False):  # pylint: disable=too-many-arguments
        instances = []
        max_num_on_demand = 2
        if isinstance(self, (ScyllaAWSCluster, CassandraAWSCluster)):
            ...
            if count_spot > 0:
                ...
                instances.extend(self._create_spot_instances(
                    count_spot, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, zero_token=zero_token))
            if count_on_demand > 0:
                ...
                instances.extend(self._create_on_demand_instances(
                    count_on_demand, interfaces, ec2_user_data, dc_idx, instance_type=instance_type, zero_token=zero_token))
    ...
    def _create_or_find_instances(self, count, ec2_user_data, dc_idx, az_idx=0, instance_type=None, zero_token=False):  # pylint: disable=too-many-arguments
        nodes = [node for node in self.nodes if node.dc_idx == dc_idx and node.rack == az_idx]
        if nodes:
            return self._create_instances(count, ec2_user_data, dc_idx, az_idx, instance_type=instance_type, zero_token=zero_token)
    ...
    def add_nodes(self, count, ec2_user_data='', dc_idx=0, rack=0, enable_auto_bootstrap=False, instance_type=None, zero_token=False):
        if not count:
            return []
        ...
        for rack_idx, rack_count in enumerate(rack_distribution):
            ...
            instances.extend(self._create_or_find_instances(
                count=rack_count, ec2_user_data=ec2_user_data, dc_idx=instance_dc, az_idx=rack_idx, instance_type=instance_type, zero_token=zero_token))
