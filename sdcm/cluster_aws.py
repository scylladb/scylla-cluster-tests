...
class AWSCluster(cluster.BaseCluster):
    ...
    def _create_instances(self, count, ec2_user_data='', dc_idx=0, az_idx=0, instance_type=None, is_zero_node=False):
        ...
        interfaces = []
        if check_ena_express_support(instance_type=instance_type or self._ec2_instance_type, region_name=self.region_names[dc_idx]):
            ena_conf = {
                "EnaSrdSpecification": {
                    "EnaSrdEnabled": True,
        ...
        for i in range(interfaces_amount):
            ...
            interfaces.append({'DeviceIndex': i,
                               ...
                               'Groups': self._ec2_security_group_ids[dc_idx], **ena_conf})
