...
class AWSCluster(cluster.BaseCluster):
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.placement_group_name = PlacementGroup(
            params=params, common_tags=self.test_config.common_tags(), test_id=self.test_config.test_id()
    ...
    def _create_on_demand_instances(
        ...
        LOGGER.debug(
            "Using EC2 service with DC-index: %s, (associated with region: %s)", dc_idx, self.region_names[dc_idx]
        )
    ...
    def fallback_provision_type(self, count, interfaces, ec2_user_data, dc_idx, instance_type=None, is_zero_node=False):
        ...
        if self.instance_provision.lower() == "spot" or (
            self.instance_provision == INSTANCE_PROVISION_SPOT_FLEET and count == 1
        ):
    ...
    def _create_mixed_instances(self, count, interfaces, ec2_user_data, dc_idx, instance_type=None, is_zero_node=False):
        ...
        if isinstance(self, (ScyllaAWSCluster, CassandraAWSCluster)):
            ...
            if count_on_demand > 0:
                self.instance_provision = INSTANCE_PROVISION_SPOT
                instances.extend(
                    self._create_on_demand_instances(
                        count_on_demand,
                        interfaces,
                        ec2_user_data,
                        dc_idx,
            ...
            instances.extend(
                self._create_on_demand_instances(count, interfaces, ec2_user_data, dc_idx, instance_type=instance_type)
            )
        else:
    ...
    def add_nodes(
        ...
        for rack_idx, rack_count in enumerate(rack_distribution):
            ...
            instances.extend(
                self._create_or_find_instances(
                    count=rack_count,
...
class AWSNode(cluster.BaseNode):
    ...
    def network_interfaces(self):
        ...
        for interface in self._instance.network_interfaces:
            ...
            ipv4_public_address = (
                to_inet_ntop_format(interface.association_attribute["PublicIp"])
    ...
    def init(self):
        ...
        self.kernel_panic_checker = AWSKernelPanicChecker(
            node=self, instance_id=self._instance.id, region=self._ec2_service.meta.client.meta.region_name
        )
...
class AWSKernelPanicChecker(threading.Thread):
    """Monitor AWS EC2 instance for kernel panics via console output and instance status."""
    ...
    def run(self):
        while not self._stop_event.is_set():
            try:
                statuses = self.ec2.describe_instance_status(InstanceIds=[self.instance_id], IncludeAllInstances=True)
                for inst in statuses.get("InstanceStatuses", []):
                    status = inst["InstanceStatus"]["Status"]
                    if status != "ok":
                ...
                if ("kernel panic" in output_lower or "not syncing" in output_lower) and not self._panic_detected:
                    ...
                    panic_text = (
                        " | ".join(panic_lines) if panic_lines else "Kernel panic detected (see full output below)"
                    )
                    message = f"Kernel panic detected in console log for instance {self.instance_id}: {panic_text}"

                    # Log the full console output to sct.log
