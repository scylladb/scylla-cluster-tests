    ...
    def network_interfaces(self):
        ...
        for interface in self._instance.network_interfaces:
            private_ip_addresses = [
                private_address["PrivateIpAddress"] for private_address in interface.private_ip_addresses
            ]
            # make sure we use ipv6 long format (some tools remove leading zeros)
            ipv6_addresses = [
                ipaddress.ip_address(ipv6_address["Ipv6Address"]).exploded for ipv6_address in interface.ipv6_addresses
            ]
