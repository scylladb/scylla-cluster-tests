...
class Steps(SlaUtils):
    ...
    def alter_sl_and_validate_io_queue_operations(
        ...
        with TestStepEvent(
            step=f"Alter shares from {service_level.shares} to {new_shares} Service Level {service_level.name} and validate io_queue_operations during load"
        ) as wp_event:
            try:
    ...
    def detach_service_level_and_run_load(sl_for_detach, role_with_sl_to_detach, sleep=600):
        with TestStepEvent(
            step=f"Detach service level {sl_for_detach.name} with {sl_for_detach.shares} shares from {role_with_sl_to_detach.name}."
        ) as wp_event:
            try:
    ...
    def attach_sl_and_validate_io_queue_operations(
        ...
        with TestStepEvent(
            step=f"Attach service level {new_service_level.name} with {new_service_level.shares} shares to {role_for_attach.name}. Validate io_queue_operations during load"
        ) as wp_event:
            try:
                ...
                for node in tester.db_cluster.nodes:
                    if node.db_up():
                        node.remoter.run(
                            "curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' http://127.0.0.1:10000/service_levels/switch_tenants"
                        )
                ...
                for node in tester.db_cluster.nodes:
                    if node.db_up():
                        result = node.remoter.run(
                            "curl -X GET --header 'Content-Type: application/json' --header 'Accept: application/json' http://127.0.0.1:10000/service_levels/count_connections"
                        )
