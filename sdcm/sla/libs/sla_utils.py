...
class SlaUtils:
    STRESS_READ_CMD = "cassandra-stress read cl=QUORUM duration={duration} -mode cql3 native user={user} password={password} -rate threads={threads} -pop {pop} -errors retries=50"
    ...
    def define_read_cassandra_stress_command(
        role: Role, load_type: str, c_s_workload_type: str, threads: int, stress_duration_min: int, max_rows_for_read: int = None, stress_command: str = STRESS_READ_CMD, throttle: int = 20000, num_of_partitions=50000000, **kwargs
    ):
        """
        ...
        params = {"n": num_of_partitions, "user": role.name, "password": role.password, "pop": pop, "duration": "%dm" % stress_duration_min, "threads": rate}
    ...
    def get_c_s_stats(self, read_queue, users, statistic_name, tester):
        ...
        for i, read in enumerate(read_queue):
            stat_rate, username = self.get_statistic_value_from_cassandra_stress_result(read_run=read, user_name=role_names[i], statistic_name=statistic_name, tester=tester)

            if stat_rate is None:
                return stat_rate

            assert username == role_names[i], f'Expected that stress was run with user "{role_names[i]}" but it was "{username}"'

            results[username] = float(stat_rate)

        return results

    def validate_io_queue_operations(self, start_time, end_time, read_users, prometheus_stats, db_cluster, expected_ratio=None, load_high_enough=None, publish_wp_error_event=False, possible_issue=None):
        ...
        for node in db_cluster.nodes:
            ...
            for step in ["30s", "45s", "60s", "120s"]:
                ...
                if io_queue_total_operations := prometheus_stats.get_scylla_io_queue_total_operations(start_time, end_time, node_ip, irate_sample_sec=step):
                    break
            ...
            if not io_queue_total_operations:
                ...
                WorkloadPrioritisationEvent.EmptyPrometheusData(message=f"Failed to get io_queue_total_operations data from Prometheus for node {node_ip}", severity=Severity.WARNING).publish()
                continue

            for role_sl_attribute in roles_full_info.values():
                if role_sl_attribute["sl_group"] in io_queue_total_operations[node_ip]:
                    role_sl_attribute["sl_group_ops"] = sum(io_queue_total_operations[node_ip][role_sl_attribute["sl_group"]]) / len(io_queue_total_operations[node_ip][role_sl_attribute["sl_group"]])
                else:
            ...
            if not expected_ratio:
                ...
                if load_high_enough is None:
                    node_cpu = prometheus_stats.get_scylla_reactor_utilization(start_time=start_time, end_time=end_time, instance=node_ip)
                result.append(
                    self.validate_io_queue_operations_relatively_to_share(
                        roles_full_info=roles_full_info, node_ip=node_ip, node_cpu=node_cpu, load_high_enough=load_high_enough, publish_wp_error_event=publish_wp_error_event, possible_issue=possible_issue
                    )
                )
                continue
    ...
    @staticmethod
    def validate_io_queue_operations_relatively_to_share(roles_full_info: dict, node_ip: str, load_high_enough: bool = None, node_cpu: float = None, publish_wp_error_event=False, possible_issue=None):
        # roles_full_info example:
        ...
        if not shares or len([s for s in shares if s]) < 2:
            WorkloadPrioritisationEvent.RatioValidationEvent(
                message=f"Not enough service level shares for validation. Expected two Service Levels, received {len([s for s in shares if s])}: {shares}. Runtime can not be validated", severity=Severity.NORMAL
            ).publish()
            return None
        ...
        if shares[0] == shares[1]:
            WorkloadPrioritisationEvent.RatioValidationEvent(message="Both roles have same shares. Runtime can not be validated", severity=Severity.NORMAL).publish()
            return None
        ...
        if 0.0 not in operations and shares_ratio == operations_ratio:
            WorkloadPrioritisationEvent.RatioValidationEvent(message=f"Role with higher shares got more resources on the node with IP {node_ip} as expected", severity=Severity.NORMAL).publish()
            return ""
        ...
        for service_level in roles_full_info.values():
            ops_per_sl_group.append(f"{service_level['sl_group']} (shares {service_level['service_level_shares']}): {round(service_level['sl_group_ops'], 2)}")
        ...
        if zero_operations_service_level:
            ...
            message = f"\n(Node {node_ip}) - Service level with higher shares got less resources unexpectedly.%s Runtime per service level group:\n  {ops_per_sl_group_str}{issue}"

        if load_high_enough is None and node_cpu is None:
            WorkloadPrioritisationEvent.RatioValidationEvent(message=message % " Maybe due to not enough high load.", severity=Severity.WARNING).publish()
        else:
            ...
            if publish_wp_error_event:
                WorkloadPrioritisationEvent.RatioValidationEvent(message=error_message, severity=Severity.ERROR).publish()
    ...
    def wait_for_service_level_propagated(self, cluster, service_level):
        ...
        for node in cluster.nodes:
            LOGGER.debug("Start wait for service level propagated for service_level %s on the node '%s'", service_level.name, node.name)
            if not (scheduler_shares := self.get_scheduler_shares_per_group(node=node, scheduler_group_name=service_level.scheduler_group_name)):
                raise SchedulerGroupNotFound(f"Scheduler groups for {service_level.name} service levels is not created on the node '{node.name}'. Maybe the issue scylla-cluster-tests#7082")

            if len(scheduler_shares) != node.scylla_shards:
                raise WrongServiceLevelShares(
                    f"Expected that scheduler group is created on every Scylla shard for {service_level.name} service level but actually it was created on {len(scheduler_shares)} shards: {scheduler_shares} on the node '{node.name}'"
                )

            if len(set(scheduler_shares)) > 1:
                raise WrongServiceLevelShares(f"Expected same shares on every shard for {service_level.name} service level but actually the shares on the every shard are: {scheduler_shares} on the node '{node.name}'")

            if scheduler_shares[0] != service_level.shares:
                raise WrongServiceLevelShares(f"Expected {service_level.name} service level with '{service_level.shares}' shares but actually it is {scheduler_shares} shares on the node '{node.name}'")
            LOGGER.debug("Finish wait for service level propagated for service_level %s on the node '%s'", service_level.name, node.name)
