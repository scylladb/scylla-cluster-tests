...
class SlaUtils:
    ...
    def validate_io_queue_operations_relatively_to_share(
        ...
        if not shares or len([s for s in shares if s]) < 2:
            WorkloadPrioritisationEvent.RatioValidationEvent(
                message=f"Not enough service level shares for validation. Expected two Service Levels, received {len([s for s in shares if s])}: {shares}. Runtime can not be validated",
        ...
        for service_level in roles_full_info.values():
            ops_per_sl_group.append(
                f"{service_level['sl_group']} (shares {service_level['service_level_shares']}): {round(service_level['sl_group_ops'], 2)}"
            )
    ...
    def validate_deviation(self, expected_ratio, actual_ratio, msg):
        ...
        if dev is None:
            WorkloadPrioritisationEvent.RatioValidationEvent(
                message=f"Can't compare expected and actual shares ratio. Expected: {expected_ratio}. Actual: {actual_ratio}",
        ...
        for node in cluster.nodes:
            ...
            if not (
                ...
                raise SchedulerGroupNotFound(
                    f"Scheduler groups for {service_level.name} service levels is not created on the node '{node.name}'. Maybe the issue scylla-cluster-tests#7082"
                )
            ...
            if len(set(scheduler_shares)) > 1:
                raise WrongServiceLevelShares(
                    f"Expected same shares on every shard for {service_level.name} service level but actually the shares on the every shard are: {scheduler_shares} on the node '{node.name}'"
                )

            if scheduler_shares[0] != service_level.shares:
                raise WrongServiceLevelShares(
                    f"Expected {service_level.name} service level with '{service_level.shares}' shares but actually it is {scheduler_shares} shares on the node '{node.name}'"
                )
            LOGGER.debug(
