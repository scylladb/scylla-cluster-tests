...
class BaseNode(AutoSshContainerMixin):
    ...
    def start_db_log_reader_thread(self):
        self._db_log_reader_thread = DbLogReader(
            ...
            log_lines=self.parent_cluster.params.get("logs_transport")
            in [
                "syslog-ng",
            ],
        )
    ...
    def remove_package(self, package_name: str, ignore_status: bool = True) -> None:
        ...
        if self.distro.is_rhel_like:
            ...
            remove_cmd = f"{pkg_mgr} remove -y {package_name}"
        elif self.distro.is_sles:
    ...
    def configure_remote_logging(self):
        if self.parent_cluster.params.get("logs_transport") not in [
            "syslog-ng",
        ]:
            return
            ...
            while True:
                ...
                try:
                    ...
                    with nemesis_class.run_nemesis(
                        node_list=ready_nodes, nemesis_label="KMS encryption check"
                    ) as target_node:
