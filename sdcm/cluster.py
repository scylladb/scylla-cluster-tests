...
class BaseNode(AutoSshContainerMixin, WebDriverContainerMixin):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    max_coredump_thread_exceptions: int = 10

    # Next 3 parameters are used and allowed to be changed by unit tests in unit_tests/test_decode_backtrace.py.
    # Want to change it to reduce the test time
    backtrace_lookup_period: int = 15
    get_core_pids_func_retrying: int = 10
    get_core_pids_func_sleep_timeout: int = 20
    ...
    def get_backtraces(self) -> Optional[Exception]:
        """
        ...
        try:
            ...
            self.log.error("Following network error occurred during getting back traces: %s", exc)
            return exc

    def _get_core_pids(self):
        @retrying(n=self.get_core_pids_func_retrying, sleep_time=self.get_core_pids_func_sleep_timeout, allowed_exceptions=NETWORK_EXCEPTIONS, message="Retrying on getting pid of cores")
        def _get_core_pids_inner():
    ...
    def backtrace_thread(self):
        ...
        while not self.termination_event.is_set():
            self.termination_event.wait(self.backtrace_lookup_period)
            try:
                last_exc = self.get_backtraces()
            ...
            self.log.error("Following error occurred during getting back traces: %s", last_exc)
            exceptions_count += 1
