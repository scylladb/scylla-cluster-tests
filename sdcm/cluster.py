...
class BaseNode(AutoSshContainerMixin):
    ...
    def __init__(
        ...
        self.last_health_check_stats: NodeHealthCheckStats | None = None
    ...
    def check_node_health(self, retries: int = CHECK_NODE_HEALTH_RETRIES) -> NodeHealthCheckStats | None:
        ...
        # published on the node so the cluster gate can still account for this node's time
        # if the check raises - otherwise the gate summary silently omits a whole node in
        # exactly the failure case worth measuring.
        self.last_health_check_stats = stats
        try:
                            ...
                            with ThreadPoolExecutor(max_workers=parallel_workers) as executor:
                                ...
                                for future in as_completed(futures):
                                    ...
                                    try:
                                        ...
                                        partial = getattr(node, "last_health_check_stats", None)
                                        if isinstance(partial, NodeHealthCheckStats):
                                            node_stats.append(partial)
