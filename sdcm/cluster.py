...
class BaseNode(AutoSshContainerMixin):
    ...
    def is_client_encrypt(self):
        result = self.remoter.run(
            f"grep ^client_encryption_options: {self.add_install_prefix(SCYLLA_YAML_PATH)} -A 3 | grep enabled | awk '{{print $2}}'",
            ignore_status=True,
        )
    ...
    def _dc_info_str(self):
        ...
        # We want to figure out all places where "params" attribute has type that not consistent
            ...
            TestFrameworkEvent(
                source=self.__class__.__name__,
                message=f"The 'params' attribute expected to by 'SCTConfiguration`, but actually it is a `{type(self.parent_cluster.params)}`",
                trace=sys._getframe().f_back,
                severity=Severity.ERROR,
            ).publish()
    ...
    def jmx_up(self):
        ...
        return (
            self.remoter.run(
                f"{self.systemctl} is-active scylla-jmx.service && {self.systemctl} status scylla-jmx.service | grep 'JMX is enabled to receive remote connections on port'",
                timeout=10,
                ignore_status=True,
            ).return_code
            == 0
    ...
    def process_scylla_args(self, append_scylla_args=""):
        ...
        if append_scylla_args:
            ...
            self.remoter.sudo(
                f"sed -i '/{append_scylla_args}/! s/SCYLLA_ARGS=\"/&{append_scylla_args} /' {self.scylla_server_sysconfig_path}"
            )
    ...
    def download_scylla_repo(self, scylla_repo):
        ...
        if self.distro.is_rhel_like:
            ...
            for apt_key in self.parent_cluster.params.get("scylla_apt_keys"):
                self.remoter.sudo(
                    f"gpg --homedir /tmp --no-default-keyring --keyring /etc/apt/keyrings/scylladb.gpg --keyserver hkp://keyserver.ubuntu.com:80 --keyserver-options timeout=10 --recv-keys {apt_key}",
                    retry=3,
                )
    ...
    def download_scylla_manager_repo(self, scylla_repo: str) -> None:
        ...
        if self.distro.is_debian_like:
            ...
            for apt_key in self.parent_cluster.params.get("scylla_apt_keys"):
                self.remoter.sudo(
                    f"gpg --homedir /tmp --no-default-keyring --keyring /etc/apt/keyrings/scylladb.gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys {apt_key}",
                    retry=3,
                )
    ...
    def upgrade_manager_agent(self, scylla_mgmt_address: str, start_agent_after_upgrade: bool = True) -> None:
        ...
        if self.distro.is_rhel_like:
            ...
            self.remoter.sudo("apt-get update", ignore_status=True)
            self.remoter.sudo(
                "DEBIAN_FRONTEND=noninteractive apt-get install -o Dpkg::Lock::Timeout=300 -o Dpkg::Options::='--force-confold' -o Dpkg::Options::='--force-confdef' -y scylla-manager-agent"
            )
    ...
    def upgrade_mgmt(self, scylla_mgmt_address, start_manager_after_upgrade=True):
        ...
        if self.distro.is_rhel_like:
            ...
            self.remoter.sudo("apt-get update", ignore_status=True)
            self.remoter.sudo(
                'DEBIAN_FRONTEND=noninteractive apt-get install -o Dpkg::Options::="--force-confold" -o Dpkg::Options::="--force-confdef" scylla-manager-server scylla-manager-client -y '
            )
        time.sleep(3)
...
class BaseScyllaCluster:
    ...
    def start_kms_key_rotation_thread(self) -> None:
        ...
        def _rotate_kms_key(kms_key_alias_name, kms_key_rotation_interval, db_cluster):
            while True:
                ...
                try:
                    nemesis_class = (
                        self.nemesis[0] if self.nemesis else getattr(import_module("sdcm.nemesis"), "Nemesis")
                    )
                    with nemesis_class.run_nemesis(
                        node_list=db_cluster.data_nodes, nemesis_label="KMS encryption check"
                    ) as target_node:
    ...
    def clean_replacement_node_options(node):
        ...
        if node.replacement_host_id:
            ...
            node.remoter.run(
                f'sudo sed -i -e "s/^replace_node_first_boot:/# replace_node_first_boot:/g" {node.add_install_prefix(SCYLLA_YAML_PATH)}'
            )
...
class BaseLoaderSet:
    ...
    def node_setup(self, node, verbose=False, **kwargs):
        ...
        if node.distro.is_rhel_like:
            ...
            node.remoter.sudo(
                "ln -sf /usr/lib/jvm/java-1.8.0-openjdk-1.8.0.*/jre/bin/java* /etc/alternatives/java",
                verbose=True,
                ignore_status=True,
            )
        else:
            ...
            node.remoter.sudo(
                "ln -sf /usr/lib/jvm/java-1.8.0-openjdk-*/bin/java* /etc/alternatives/java",
                verbose=True,
                ignore_status=True,
            )
...
class BaseMonitorSet:
    ...
    def _create_manager_prometheus_yaml(self, node):
        ...
        node.remoter.run(
            f"cp {self.monitor_install_path}/prometheus/scylla_manager_servers.yml {self.monitoring_conf_dir}/scylla_manager_servers.yml"
        )
