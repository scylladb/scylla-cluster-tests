...
from sdcm.provision.helpers.certificate import create_ca, install_client_certificate, install_encryption_at_rest_files, create_certificate, export_pem_cert_to_pkcs12_keystore, CA_CERT_FILE, CA_KEY_FILE, JKS_TRUSTSTORE_FILE, TLSAssets
...
from sdcm.logcollector import GrafanaScreenShot, PrometheusSnapshots, upload_archive_to_s3, save_kallsyms_map, collect_diagnostic_data, ScyllaLogCollector
from sdcm.utils.ldap import LDAP_SSH_TUNNEL_LOCAL_PORT, LDAP_BASE_OBJECT, LDAP_PASSWORD, LDAP_USERS, LDAP_PORT, DEFAULT_PWD_SUFFIX
...
class BaseNode(AutoSshContainerMixin):
    ...
    def node_rack(self) -> str:
        if not self._node_rack:
            ...
            if self.parent_cluster.node_type == "scylla-db":
                ...
                if node_rack := [rack_name for dc_rack_id, rack_name in rack_names.items() if dc_rack_id[0] == self.datacenter and dc_rack_id[1] == str(self.rack)]:
    ...
    def init(self) -> None:
        ...
        self.set_keep_alive()
        if self.node_type == "db" and not self.is_kubernetes() and not self.is_cloud() and self.parent_cluster.params.get("print_kernel_callstack"):
            if self.is_docker():
                ...
                try:
                    ...
                    LOGGER.error("Encountered an unhadled exception while changing 'perf_event_paranoid' value", exc_info=True)
    ...
    def update_rack_info_in_argus(self, dc_name: str, rack_name: str):
        try:
            client = self.test_config.argus_client()
            client.update_resource(name=self.name, update_data={"instance_info": {"rack_name": rack_name, "dc_name": dc_name}})
        except Exception:
        ...
        if self.test_config.IP_SSH_CONNECTIONS == "public":
            if self.test_config.SYSLOGNG_ADDRESS:
                try:
                    ...
                    pass
                ContainerManager.run_container(self, "auto_ssh:syslog_ng", local_port=self.test_config.SYSLOGNG_ADDRESS[1], remote_port=self.test_config.SYSLOGNG_SSH_TUNNEL_LOCAL_PORT)
            if self.test_config.VECTOR_ADDRESS:
                try:
                    ContainerManager.destroy_container(self, "auto_ssh:vector", ignore_keepalive=True)
                except NotFound:
                    pass
                ContainerManager.run_container(self, "auto_ssh:vector", local_port=self.test_config.VECTOR_ADDRESS[1], remote_port=self.test_config.VECTOR_SSH_TUNNEL_LOCAL_PORT)
            if self.test_config.LDAP_ADDRESS and self.parent_cluster.node_type == "scylla-db":
                try:
                    ...
                    pass
                ContainerManager.run_container(self, "auto_ssh:ldap", local_port=self.test_config.LDAP_ADDRESS[1], remote_port=LDAP_SSH_TUNNEL_LOCAL_PORT)
    ...
    def db_node_instance_type(self) -> Optional[str]:
        ...
        if backend in ("aws", "aws-siren"):
            if self._is_zero_token_node:
                return self.parent_cluster.params.get("zero_token_instance_type_db") or self.parent_cluster.params.get("instance_type_db")
    ...
    def proposed_scylla_yaml(self) -> ScyllaYaml:
        ...
        if append_scylla_yaml := copy.deepcopy(self.parent_cluster.params.get("append_scylla_yaml")) or {}:
            if any(key in append_scylla_yaml for key in ("system_key_directory", "system_info_encryption", "kmip_hosts")):
    ...
    def tags(self) -> Dict[str, str]:
        return {**self.parent_cluster.tags, "Name": str(self.name), "UserName": str(self.ssh_login_info.get("user")) if self.ssh_login_info else "", "Owner": "SCT", "RunByUser": "SCT"}
    ...
    def set_keep_alive(self):
        ...
        if self.test_config.should_keep_alive(node_type) and self._set_keep_alive():
            ...
            self._set_keep_duration(int(Decimal((self.test_config.TEST_DURATION + 125) / 60).quantize(Decimal("1"), ROUND_UP)))
    ...
    def is_client_encrypt(self):
        result = self.remoter.run(f"grep ^client_encryption_options: {self.add_install_prefix(SCYLLA_YAML_PATH)} -A 3 | grep enabled | awk '{{print $2}}'", ignore_status=True)
    ...
    def is_server_encrypt(self):
        result = self.remoter.run(f"grep '^server_encryption_options:' {self.add_install_prefix(SCYLLA_YAML_PATH)}", ignore_status=True)
    ...
    def extract_seeds_from_scylla_yaml(self):
        ...
        wait.wait_for(func=self.remoter.receive_files, step=10, text="Waiting for copying scylla.yaml", timeout=300, throw_exc=True, src=str(self.add_install_prefix(SCYLLA_YAML_PATH)), dst=yaml_dst_path)
    ...
    def start_network_interface(self, interface_name="eth1"):
        ...
        elif self.distro.uses_systemd and self.remoter.run("systemctl is-active --quiet systemd-networkd.service", ignore_status=True).ok:
    ...
    def start_spot_monitoring_thread(self):
        self._spot_monitoring_thread = threading.Thread(target=self.spot_monitoring_thread, name="SpotMonitoringThread", daemon=True)
    ...
    def start_journal_thread(self):
        ...
        if self._journal_thread:
            ...
            TestFrameworkEvent(source=self.__class__.__name__, source_method="start_journal_thread", message="Got no logging daemon by unknown reason").publish_or_dump()
    ...
    def _dc_info_str(self):
        ...
        if not hasattr(self.parent_cluster, "params"):
            TestFrameworkEvent(source=self.__class__.__name__, message="The parent cluster has not 'params' attribute", trace=sys._getframe().f_back, severity=Severity.ERROR).publish()
        # We want to figure out all places where "params" attribute has type that not consistent
            ...
            TestFrameworkEvent(
                source=self.__class__.__name__, message=f"The 'params' attribute expected to by 'SCTConfiguration`, but actually it is a `{type(self.parent_cluster.params)}`", trace=sys._getframe().f_back, severity=Severity.ERROR
            ).publish()
    ...
    def __str__(self):
        ...
        if self.scylla_network_configuration and self.scylla_network_configuration.broadcast_address_ip_type == "private":
            node_private_ip = self.scylla_network_configuration.broadcast_address
        else:
            node_private_ip = self.private_ip_address

        return "Node %s [%s | %s%s] (Type: %s)%s" % (self.name, self.public_ip_address, node_private_ip, " | %s" % self.ipv6_ip_address if self.test_config.IP_SSH_CONNECTIONS == "ipv6" else "", self._instance_type, self._dc_info_str())
    ...
    def wait_ssh_up(self, verbose=True, timeout=500):
        ...
        with adaptive_timeout(Operations.SSH_CONNECTIVITY, node=self, timeout=timeout, node_available=False):
            wait.wait_for(func=self.remoter.is_up, step=10 * ssh_timeout_multiplier, text=text, timeout=timeout * ssh_timeout_multiplier, throw_exc=True)
    ...
    def jmx_up(self):
        if not self.is_service_exists(service_name="scylla-jmx"):
            return True
        return self.remoter.run(f"{self.systemctl} is-active scylla-jmx.service && {self.systemctl} status scylla-jmx.service | grep 'JMX is enabled to receive remote connections on port'", timeout=10, ignore_status=True).return_code == 0
            ...
            if stat_line:
                try:
                    try:
                        ...
                        current_value = stat_dict[stat_line[0]] if "Space used" in stat_line[0] and stat_line[0] in stat_dict else 0
    ...
    def fstrim_scylla_disks(self):
        if not self._is_storage_virtualized():
            ...
            if result.failed:
                TestFrameworkEvent(
                    source=self.__class__.__name__, source_method="fstrim_scylla_disks", message=f"fstrim'ming of Scylla disks was failed: stdout: {result.stdout}, stderr: {result.stderr}", severity=Severity.ERROR
                ).publish_or_dump()
        else:
            TestFrameworkEvent(
    ...
    def get_cfstats(self, keyspace):
        ...
        wait.wait_for(keyspace_available, timeout=600, step=60, text="Waiting until keyspace {} is available".format(keyspace), throw_exc=False)
    ...
    def wait_db_up(self, verbose=True, timeout=3600):
        ...
        wait.wait_for(func=self.db_up, step=5, text=text, timeout=timeout, throw_exc=True, stop_event=self.stop_wait_db_up_event)
    ...
    def is_manager_agent_up(self, port=None):
        ...
        curl_output = self.remoter.run(cmd=f'''curl -k --write-out "%{{http_code}}\n" --silent --output /dev/null "{http_address}"''', verbose=True, ignore_status=True)
    ...
    def is_manager_server_up(self, port=None):
        ...
        curl_output = self.remoter.run(f'''curl --write-out "%{{http_code}}\n" --silent --output /dev/null "http://127.0.0.1:{port}/ping"''', verbose=True, ignore_status=True)
        http_status_code = int(curl_output.stdout.strip())
        return http_status_code == 204
    ...
    def wait_manager_server_up(self, verbose=True, timeout=300, port=None):
        ...
        try:
            wait.wait_for(func=self.is_manager_server_up, port=port, step=10, text=text, timeout=timeout, throw_exc=True)
        except RetryError:
            wait.wait_for(func=self.is_manager_server_up, port=self.OLD_MANAGER_PORT, step=10, text=text, timeout=timeout, throw_exc=True)
    ...
    def follow_system_log(self, patterns: Optional[List[Union[str, re.Pattern, LogEvent]]] = None, start_from_beginning: bool = False) -> Iterable[str]:
        ...
        with open(self.system_log, "r", encoding="utf-8") as log_file:
            ...
            while left <= right:
                ...
                if log_time < on_datetime:
                    ...
                    right = mid - 1
            self.log.debug("Asked to open log at %s, the closest log line is %s. log size: %s, line: <%s>...", on_datetime, log_time, log_size, line[100])
            yield log_file

    def start_decode_on_monitor_node_thread(self):
        self._decoding_backtraces_thread = threading.Thread(target=self.decode_backtrace, name="DecodeOnMonitorNodeThread", daemon=True)
    ...
    def _remote_yaml(self, path, sudo=True):
        ...
        return remote_file(remoter=self.remoter, remote_path=path, serializer=dict_to_yaml_file, deserializer=yaml_file_to_dict, sudo=sudo)

    def _remote_properties(self, path):
        self.log.debug("Update %s properties configuration file", path)
        return remote_file(remoter=self.remoter, remote_path=path, serializer=properties.serialize, deserializer=properties.deserialize, sudo=True)
    ...
    def remote_scylla_yaml(self) -> ContextManager[ScyllaYaml]:
        with self._remote_yaml(path=self.add_install_prefix(abs_path=SCYLLA_YAML_PATH)) as scylla_yaml:
            ...
            if not diff:
                ScyllaYamlUpdateEvent(node_name=self.name, message=f"ScyllaYaml has not been changed on node: {self.name}").publish()
                return
            scylla_yaml.clear()
            scylla_yaml.update(
            ...
            ScyllaYamlUpdateEvent(node_name=self.name, message=f"ScyllaYaml has been changed on node: {self.name}. Diff: {diff}").publish()
    ...
    def get_saslauthd_config(self):
        ...
        return {"ldap_servers": f"ldap://{ldap_server_ip}:{ldap_port}/", "ldap_search_base": f"ou=Person,{LDAP_BASE_OBJECT}", "ldap_bind_dn": f"cn=admin,{LDAP_BASE_OBJECT}", "ldap_bind_pw": LDAP_PASSWORD}
    ...
    def get_saslauthd_ms_ad_config():
        ...
        return {"ldap_servers": f"ldap://{ldap_server_ip}:{ldap_port}/", "ldap_search_base": ldap_search_base, "ldap_filter": "(cn=%u)", "ldap_bind_dn": ldap_bind_dn, "ldap_bind_pw": ldap_bind_pw}
    ...
    def process_scylla_args(self, append_scylla_args=""):
        if append_scylla_args:
            scylla_help = self.remoter.run(f"{self.add_install_prefix('/usr/bin/scylla')} --help", ignore_status=True, verbose=False).stdout
            scylla_help_seastar = self.remoter.run(f"{self.add_install_prefix('/usr/bin/scylla')} --help-seastar", ignore_status=True, verbose=False).stdout
            scylla_arg_parser = ScyllaArgParser.from_scylla_help(
                help_text=f"{scylla_help}\n{scylla_help_seastar}", duplicate_cb=lambda dups: ScyllaHelpErrorEvent.duplicate(message=f"Scylla help contains duplicate for the following arguments: {','.join(dups)}").publish()
            )
            append_scylla_args = scylla_arg_parser.filter_args(append_scylla_args, unknown_args_cb=lambda args: ScyllaHelpErrorEvent.filtered(message="Following arguments are filtered out: " + ",".join(args)).publish())
        ...
        if append_scylla_args:
            ...
            self.remoter.sudo(f"sed -i '/{append_scylla_args}/! s/SCYLLA_ARGS=\"/&{append_scylla_args} /' {self.scylla_server_sysconfig_path}")
    ...
    def create_node_certificate(self, cert_file, cert_key, csr_file=None):
        create_certificate(
            cert_file, cert_key, self.name, ca_cert_file=CA_CERT_FILE, ca_key_file=CA_KEY_FILE, server_csr_file=csr_file, ip_addresses=[self.ip_address, self.public_ip_address], dns_names=[self.public_dns_name, self.private_dns_name]
        )
    ...
    def download_scylla_manager_repo(self, scylla_repo: str) -> None:
        ...
        self.remoter.sudo(f"curl -o {repo_path} -L {scylla_repo} --connect-timeout 10 --retry 5 --retry-max-time 100", retry=3)
    ...
    def fetch_apt_keys(self):
        ...
        try:
            ...
            for apt_key in self.parent_cluster.params.get("scylla_apt_keys"):
                self.remoter.sudo(f"gpg --homedir /tmp --no-default-keyring --keyring {temp_keyring} --keyserver hkp://keyserver.ubuntu.com:80 --keyserver-options timeout=10 --recv-keys {apt_key}", retry=3)
            # Export all keys at once to the keyring file
            self.remoter.sudo(shell_script_cmd(f"gpg --homedir /tmp --no-default-keyring --keyring {temp_keyring} --export --armor | gpg --dearmor > /etc/apt/keyrings/scylladb.gpg"), retry=3)
        finally:
            # Ensure cleanup
    ...
    def install_package(self, package_name: str, package_version: str = None, ignore_status: bool = False) -> None:
        if self.distro.is_rhel_like:
            pkg_cmd = next(mgr for mgr in ["microdnf", "yum", "dnf"] if self.remoter.run(f"test -e /usr/bin/{mgr}", ignore_status=True).ok)
    ...
    def update_manager_agent_backup_config(self, region: Optional[str] = None, general_config: Optional[dict[str, str]] = None) -> None:
    ...
    def upgrade_manager_agent(self, scylla_mgmt_address: str, start_agent_after_upgrade: bool = True) -> None:
        ...
        if self.distro.is_rhel_like:
            ...
            self.remoter.sudo("apt-get update", ignore_status=True)
            self.remoter.sudo("DEBIAN_FRONTEND=noninteractive apt-get install -o Dpkg::Lock::Timeout=300 -o Dpkg::Options::='--force-confold' -o Dpkg::Options::='--force-confdef' -y scylla-manager-agent")
    ...
    def clean_scylla_data(self):
        ...
        """
        clean_commands_list = ["rm -rf /var/lib/scylla/data/*", "find /var/lib/scylla/commitlog -type f -delete", "find /var/lib/scylla/hints -type f -delete", "find /var/lib/scylla/view_hints -type f -delete"]
    ...
    def upgrade_system(self):
        if self.distro.is_rhel_like:
            ...
            self.remoter.run('sudo DEBIAN_FRONTEND=noninteractive apt-get -o Dpkg::Options::="--force-confold" -o Dpkg::Options::="--force-confdef" upgrade -y ', retry=3)
    ...
    def install_scylla(self, scylla_repo, scylla_version=None):
        ...
        if self.distro.is_rhel_like:
            ...
            if self.distro.is_centos9 or self.distro.is_rhel8 or self.distro.is_oel8 or self.distro.is_rocky8 or self.distro.is_rocky9:
                self.install_epel()
    ...
    def scylla_setup(self, disks, devname: str):
        ...
        self.remoter.run("sudo /usr/lib/scylla/scylla_setup --nic {} --disks {} --setup-nic-and-disks {}".format(devname, ",".join(disks), extra_setup_args))
    ...
    def upgrade_mgmt(self, scylla_mgmt_address, start_manager_after_upgrade=True):
        ...
        if self.distro.is_rhel_like:
            ...
            self.remoter.sudo("apt-get update", ignore_status=True)
            self.remoter.sudo('DEBIAN_FRONTEND=noninteractive apt-get install -o Dpkg::Options::="--force-confold" -o Dpkg::Options::="--force-confdef" scylla-manager-server scylla-manager-client -y ')
        time.sleep(3)
    ...
    def install_mgmt(self, package_url: Optional[str] = None) -> None:
        ...
        if self.is_docker():
            self.remoter.sudo("yum remove -y scylla scylla-jmx scylla-tools scylla-tools-core scylla-server scylla-conf")
        ...
        with self.remote_manager_yaml() as manager_yaml:
            ...
            version = self.remoter.run("sctool version | grep 'Client version:' | awk -F': ' '{print $2}'").stdout.strip()
    ...
    def start_scylla_manager_log_capture(self):
        self._scylla_manager_journal_thread = threading.Thread(target=self.scylla_manager_log_thread, name="ScyllaManagerJournalThread", daemon=True)
    ...
    def is_service_exists(self, service_name: str, timeout: int = 500) -> bool:
        return self.remoter.run(f"{self.systemctl} list-unit-files | grep {service_name}.service", timeout=timeout, ignore_status=True).return_code == 0
    ...
    @retrying(n=3, sleep_time=5, allowed_exceptions=NETWORK_EXCEPTIONS, message="Failed to stop scylla.server, retrying...")
    def stop_scylla_server(self, verify_up=False, verify_down=True, timeout=300, ignore_status=False):
        ...
        try:
            ...
            if details.__class__.__name__.endswith("CommandTimedOut"):
                LOGGER.error("timeout during stopping scylla-server, collect diagnostic data and wait for systemd to kill it")
                collect_diagnostic_data(self)
                wait.wait_for(
                    func=lambda: self._service_cmd(service_name="scylla-server", cmd="is-active", timeout=timeout, ignore_status=True).stdout.strip() == "inactive",
                    step=60,
    ...
    def restart_scylla_server(self, verify_up_before=False, verify_up_after=True, timeout=1800, verify_up_timeout=None):
        ...
        if verify_up_before:
            self.wait_db_up(timeout=verify_up_timeout)
        with adaptive_timeout(operation=Operations.RESTART_SCYLLA, node=self, timeout=timeout, stats_storage=AdaptiveTimeoutStore()):
    ...
    def restart_node_with_resharding(self, murmur3_partitioner_ignore_msb_bits: int = 12) -> None:
        ...
        with wait_for_log_lines(node=self, start_line_patterns=[DB_LOG_PATTERN_RESHARDING_START], end_line_patterns=[DB_LOG_PATTERN_RESHARDING_FINISH], start_timeout=600, end_timeout=900, error_msg_ctx=resharding_details):
    ...
    def run_nodetool(
        ...
        with NodetoolEvent(nodetool_command=sub_cmd, node=self.name, options=options, publish_event=publish_event) as nodetool_event:
            try:
                if long_running:
    ...
    def node_health_events(self) -> Iterator[ClusterHealthValidatorEvent]:
        ...
        return itertools.chain(
            check_nodes_status(nodes_status=nodes_status, current_node=self, removed_nodes_list=self.parent_cluster.dead_nodes_ip_address_list),
            check_node_status_in_gossip_and_nodetool_status(gossip_info=gossip_info, nodes_status=nodes_status, current_node=self),
            check_schema_version(gossip_info=gossip_info, peers_details=peers_details, nodes_status=nodes_status, current_node=self),
            check_nulls_in_peers(gossip_info=gossip_info, peers_details=peers_details, current_node=self),
            check_group0_tokenring_consistency(group0_members=group0_members, tokenring_members=tokenring_members, current_node=self),
        )
    ...
    def check_node_health(self, retries: int = CHECK_NODE_HEALTH_RETRIES) -> None:
        ...
        for retry_n in range(1, retries + 1):
            ...
            LOGGER.debug("Wait for %d secs before next try to validate the health of the node `%s'", CHECK_NODE_HEALTH_RETRY_DELAY, self.name)
            ...
            for dc, dc_status in statuses.items():
                for node_ip, node_properties in dc_status.items():
                    if node := node_ip_map.get(node_ip):
                        nodes_status[node] = {"status": node_properties["state"], "dc": dc, "rack": node_properties["rack"]}
                    else:  # noqa: PLR5501
                        if node_ip:
    ...
    def get_peers_info(self):
        ...
        if not (peers_details or err):
            LOGGER.error("No data, no errors. Check the output from the cql command for the correctness:\n%s", cql_results)
        return peers_details

    @retrying(n=10, sleep_time=5, raise_on_exceeded=False)
    def get_gossip_info(self) -> dict[BaseNode, dict]:
        gossip_info = self.run_nodetool("gossipinfo", verbose=False, warning_event_on_exception=(Exception,), publish_event=False)
        ...
        for line in gossip_info.stdout.split():
            ...
            if schema and ip and status and dc:
                if node := node_ip_map.get(ip):
                    ...
                    LOGGER.debug("Get gossip info. Node with IP %s is not found in the cluster. Node status is: %s", ip, status)
                else:
                    LOGGER.error("Get gossip info. Failed to find a node with status %s in the cluster by IP: %s", status, ip)
    ...
    def run_cqlsh(self, cmd, keyspace=None, timeout=120, verbose=True, split=False, connect_timeout=60, num_retry_on_failure=1, retry_interval=3):
    ...
    def get_scylla_config_param(self, config_param_name, verbose=True):
        ...
        """
        try:
            request_out = self.remoter.run(f"sudo curl --request GET http://localhost:10000/v2/config/{config_param_name}")
            if "No such config entry" in request_out.stdout:
                self.log.error(f"Failed to retreive value of {config_param_name} parameter. Error: {request_out.stdout}")
                return None
            if verbose:
    ...
    def install_epel(self):
        ...
        if self.distro.is_rhel8:
            self.remoter.run("sudo yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm")
    ...
    def run_scylla_sysconfig_setup(self) -> bool:
        ...
        syscofnig_result = self.remoter.sudo(f"scylla_sysconfig_setup --nic {nic_name} --homedir /var/lib/scylla --confdir /etc/scylla")
        ...
        cpuset_diff = self.remoter.run("diff -q /etc/scylla.d/cpuset.conf /etc/scylla.d/cpuset.conf.old", ignore_status=True)
    ...
    def log_message(self, message: str, level: str = "info") -> None:
        try:
            self.remoter.run(f"logger -p {level} -t scylla-cluster-tests {shlex.quote(message)}", ignore_status=True, verbose=False, retry=0, timeout=10, suppress_errors=True)
        except Exception:  # noqa: BLE001
            pass

    def query_metadata(self, url: str, headers: dict = None, token_url: str = None, token_header: str = None, token_ttl_header: str = None, token_ttl: int = 21600) -> str:
        """Query metadata service with optional token handling"""
        if token_url and self._metadata_token["expires"] < time.time():
            self._metadata_token["token"] = self.remoter.run(f'curl -X PUT "{token_url}" -H "{token_ttl_header}: {token_ttl}"').stdout.strip()
...
class FlakyRetryPolicy(RetryPolicy):
    ...
    def on_request_error(self, query, consistency, error, retry_num):
        ...
        """
        if retry_num < 5:
            LOGGER.debug("Server error on query=%r (consistency=%s) attempt #%d: %s", query, consistency, retry_num, error)
...
class BaseCluster:
    ...
    def __init__(self, cluster_uuid=None, cluster_prefix="cluster", node_prefix="node", n_nodes=3, params=None, region_names=None, node_type=None, extra_network_interface=False, add_nodes=True):
        ...
        self.racks_count = simulated_racks if (simulated_racks := self.params.get("simulated_racks")) else len(self.params.get("availability_zone").split(",")) if self.params.get("availability_zone") else 1
            ...
            if isinstance(n_nodes, list):
                for dc_idx, num in enumerate(n_nodes):
                    ...
                    for az_index in range(azs):
                        ...
                        self.add_nodes(nodes_per_az[az_index], dc_idx=dc_idx, rack=rack, enable_auto_bootstrap=self.auto_bootstrap)
    ...
    def terminate_node(self, node: BaseNode, scylla_shards: int = 0) -> None:
        ...
        try:
            ...
            log_links = ScyllaLogCollector([node], test_id=self.test_config.test_id(), storage_dir=os.path.join(self.logdir, "collected_logs"), params=self.params).collect_logs()
    ...
    def _create_session(
        ...
        if ssl_context is None and self.params.get("client_encrypt"):
            ...
            ssl_context = self.create_ssl_context(keyfile=node.ssl_conf_dir / key_name, certfile=node.ssl_conf_dir / cert_name, truststore=CA_CERT_FILE)
        ...
        cluster_driver = ClusterDriver(
            auth_provider=auth_provider, compression=compression, protocol_version=protocol_version, load_balancing_policy=load_balancing_policy, default_retry_policy=FlakyRetryPolicy(), connect_timeout=connect_timeout, **kwargs
        )
    ...
    def get_load_balancing_policy(self, whitelist_nodes: Optional[list[BaseNode]] = None) -> tuple[LoadBalancingPolicy, list[str | None]]:
        ...
        if self.params.get("rack_aware_loader") and self.racks_count > 1:
            ...
            LOGGER.debug("Using RackAwareRoundRobinPolicy. Loader rack: %s. Loader datacenter: %s. Node IPs: %s", loader_rack, loader_dc, node_ips)
        return wlrr, node_ips

    def cql_connection(self, node, keyspace=None, user=None, password=None, compression=True, protocol_version=None, port=None, ssl_context=None, connect_timeout=100, verbose=True, whitelist_nodes: list[BaseNode] | None = None):
        """
    ...
    def cql_connection_exclusive(self, node, keyspace=None, user=None, password=None, compression=True, protocol_version=None, port=None, ssl_context=None, connect_timeout=100, verbose=True):
    ...
    def cql_connection_patient(self, node, keyspace=None, user=None, password=None, compression=True, protocol_version=None, port=None, ssl_context=None, connect_timeout=100, verbose=True, whitelist_nodes: list[BaseNode] | None = None):
        """
    ...
    @retrying(n=8, sleep_time=15, allowed_exceptions=(NoHostAvailable,))
    def cql_connection_patient_exclusive(self, node, keyspace=None, user=None, password=None, compression=True, protocol_version=None, port=None, ssl_context=None, connect_timeout=100, verbose=True):
        """
    ...
    def get_non_system_ks_cf_with_tablets_list(self, db_node, filter_out_table_with_counter=False, filter_out_mv=False, filter_empty_tables=True, filter_func: Callable[..., bool] = None) -> List[str]:
        with self.cql_connection_patient(db_node, connect_timeout=600) as session:
            session.default_timeout = 60.0 * 5
            keyspaces = [row.keyspace_name for row in session.execute("select keyspace_name from system_schema.scylla_keyspaces")]
    ...
    def get_non_system_ks_cf_list(self, db_node, filter_out_table_with_counter=False, filter_out_mv=False, filter_empty_tables=True, filter_by_keyspace: list = None, filter_func: Callable[..., bool] = None) -> List[str]:
        return self.get_any_ks_cf_list(
            db_node,
            filter_out_table_with_counter=filter_out_table_with_counter,
            filter_out_mv=filter_out_mv,
    ...
    def get_any_ks_cf_list(
        self, db_node, filter_out_table_with_counter=False, filter_out_mv=False, filter_empty_tables=True, filter_out_system=False, filter_out_cdc_log_tables=False, filter_by_keyspace: list = None, filter_func: Callable[..., bool] = None
    ) -> List[str]:
    ...
    def get_all_tables_with_cdc(self, db_node: BaseNode) -> List[str]:
        ...
        ks_tables_with_cdc = [ks_cf_cdc[: -len(cdc.options.CDC_LOGTABLE_SUFFIX)] for ks_cf_cdc in all_ks_cf if ks_cf_cdc.endswith(cdc.options.CDC_LOGTABLE_SUFFIX)]
        ...
        with self.cql_connection_patient(db_node) as session:
            for ks_cf in all_ks_cf:
                ...
                if result and result[0].compaction["class"] == "TimeWindowCompactionStrategy":
                    twcs_tables_list.append({"name": ks_cf, "compaction": dict(result[0].compaction), "gc": result[0].gc, "dttl": result[0].dttl})
...
def wait_for_init_wrap(method):
    ...
    def wrapper(*args, **kwargs):
        ...
        default_critical_events = [{"event": type(event), "regex": r".*Startup failed.*"} for event in SYSTEM_ERROR_EVENTS]
        ...
        setup_kwargs = {k: v for k, v in kwargs.items() if k not in ["node_list", "check_node_health", "wait_for_db_logs"]}
        ...
        def verify_node_setup_or_startup(start_time, task_queue: queue.Queue, results: list):
            ...
            try:
                ...
                results.append(node)
                cl_inst.log.info("(%d/%d) nodes ready, node %s. Time elapsed: %d s", len(results), len(node_list), str(node), int(time_elapsed))
            except queue.Empty:
                pass
            if timeout and time_elapsed / 60 > timeout:
                msg = "TIMEOUT [%d min]: Waiting for node(-s) setup/startup(%d/%d) expired!" % (timeout, len(results), len(node_list))
...
class BaseScyllaCluster:
    ...
    def get_scylla_args(self):
        return self.params.get("append_scylla_args_oracle") if self.name.find("oracle") > 0 else self.params.get("append_scylla_args")
    ...
    def proposed_scylla_yaml(self) -> ScyllaYaml:
        ...
        """
        cluster_params_builder = ScyllaYamlClusterAttrBuilder(cluster_name=self.name, params=self.params, test_config=self.test_config, msldap_server_info=KeyStore().get_ldap_ms_ad_credentials())
    ...
    def set_seeds(self, wait_for_timeout=300, first_only=False):
        ...
        if first_only:
            ...
            node = self.nodes[0]
            node.wait_ssh_up()
            seed_nodes_addresses = wait.wait_for(self.read_seed_info_from_scylla_yaml, step=10, text="Waiting for seed read from scylla yaml", timeout=wait_for_timeout, throw_exc=True)
        else:
    ...
    def validate_seeds_on_all_nodes(self):
        for node in self.nodes:
            ...
            for ip in yaml_seeds_ips:
                assert ip in self.seed_nodes_addresses, "Wrong seed IP {act_ip} in the scylla.yaml on the {node_name} node. Expected {exp_ips}".format(node_name=node.name, exp_ips=self.seed_nodes_addresses, act_ip=ip)
    ...
    def enable_client_encrypt(self):
        ...
        for node in self.nodes:
            node.create_node_certificate(cert_file=node.ssl_conf_dir / TLSAssets.DB_CERT, cert_key=node.ssl_conf_dir / TLSAssets.DB_KEY, csr_file=node.ssl_conf_dir / TLSAssets.DB_CSR)
            # Create client facing node certificate, for client-to-node communication
            node.create_node_certificate(node.ssl_conf_dir / TLSAssets.DB_CLIENT_FACING_CERT, node.ssl_conf_dir / TLSAssets.DB_CLIENT_FACING_KEY)
    ...
    def _update_db_packages(self, new_scylla_bin, node_list, start_service=True):
        ...
        def check_package_suites_distro(node, extension):
            ...
            if not files_fit_extension.ok:
                TestFrameworkEvent(source=self.__class__.__name__, source_method="update_scylla_packages", message="Did not get the right packages for this distro", severity=Severity.CRITICAL).publish()
        ...
        def update_scylla_packages(node):
            ...
            if node.distro.is_rhel_like:
                ...
                def dpkg_force_install():
                    node.remoter.sudo(shell_script_cmd("yes Y | dpkg --force-depends -i /tmp/scylla/scylla*"), ignore_status=False, verbose=True)
        ...
        def is_scylla_active(node):
            return node.remoter.run("systemctl is-active scylla-server.service", ignore_status=True).stdout.strip() == "active"
    ...
    def check_nodes_running_nemesis_count(self):
        ...
        message = "; ".join(f"{node.ip_address} ({'seed' if node.is_seed else 'non-seed'}): {node.running_nemesis}" for node in nodes_running_nemesis)
    ...
    def check_nodes_up_and_normal(self, nodes: Optional[list[BaseNode]] = None, verification_node: Optional[BaseNode] = None):
        ...
        if down_nodes:
            raise ClusterNodesNotReady("Nodes %s didn't join to the cluster", ",".join([node.name for node in down_nodes]))
    ...
    def wait_for_nodes_up_and_normal(self, nodes=None, verification_node=None, iterations=60, sleep_time=3, timeout=0):
        @retrying(n=iterations, sleep_time=sleep_time, allowed_exceptions=NETWORK_EXCEPTIONS + (ClusterNodesNotReady,), message="Waiting for nodes to join the cluster", timeout=timeout)
    ...
    def wait_total_space_used_per_node(self, size=None, keyspace="keyspace1"):
        ...
        if size:
            ...
            wait.wait_for(func=self.cfstat_reached_threshold, step=10, timeout=600, text="Waiting until cfstat '%s' reaches value '%s'" % (key, size), key=key, threshold=size, keyspaces=keyspace, throw_exc=False)

    @optional_stage("nemesis")
    def add_nemesis(self, nemesis, tester_obj, hdr_tags: list[str] = None):
        for nem in nemesis:
            nemesis_obj = nem["nemesis"](tester_obj=tester_obj, termination_event=self.nemesis_termination_event, nemesis_selector=nem["nemesis_selector"], nemesis_seed=nem["nemesis_seed"])
            if hdr_tags:
    ...
    def start_nemesis(self, interval=None, cycles_count: int = -1):
        ...
        for nemesis in self.nemesis:
            nemesis_thread = threading.Thread(target=nemesis.run, name="NemesisThread", args=(interval, cycles_count), daemon=True)
        ...
        def _rotate_kms_key(kms_key_alias_name, kms_key_rotation_interval, db_cluster):
            while True:
                ...
                try:
                    ...
                    AwsKmsEvent(message=f"Failed to rotate AWS KMS key for the '{kms_key_alias_name}' alias", traceback=traceback.format_exc()).publish()
                try:
                    nemesis_node_allocator = self.test_config.tester_obj().nemesis_allocator
                    with nemesis_node_allocator.run_nemesis(nemesis_label="KMS encryption check", node_list=db_cluster.data_nodes) as target_node:
                        self.log.debug("Target node for 'rotate_kms_key' is %s", target_node.name)

                        ks_cf_list = db_cluster.get_non_system_ks_cf_list(db_node=target_node, filter_out_table_with_counter=True, filter_out_mv=True, filter_empty_tables=True)
                        chosen_ks_cf, chosen_ks_cf_sstables_num = "keyspace1.standard1", 0
                        for ks_cf in ks_cf_list:
                            res = target_node.run_nodetool(sub_cmd="cfstats", args=ks_cf, timeout=300, retry=3, publish_event=False, warning_event_on_exception=(Failure, UnexpectedExit, Libssh2_UnexpectedExit))
                            if res:
                    ...
                    AwsKmsEvent(message="Failed to get any table for the KMS key rotation thread", traceback=traceback.format_exc()).publish()
                except Exception:  # noqa: BLE001
                    AwsKmsEvent(message="Failed to check the fact of encryption (KMS) for sstables", traceback=traceback.format_exc()).publish()
    ...
    def scylla_configure_non_root_installation(self, node, devname):
        ...
        node.remoter.run(f"{node.offline_install_dir}/sbin/scylla_setup --nic {devname} --no-raid-setup", verbose=True, ignore_status=True)
        # simple config
        node.remoter.run(f"echo 'cluster_name: \"{self.name}\"' >> {node.offline_install_dir}/etc/scylla/scylla.yaml")
        node.remoter.run(f"sed -ie 's/- seeds: .*/- seeds: {node.ip_address}/g' {node.offline_install_dir}/etc/scylla/scylla.yaml")
        node.remoter.run(f"sed -ie 's/^listen_address: .*/listen_address: {node.ip_address}/g' {node.offline_install_dir}/etc/scylla/scylla.yaml")
        node.remoter.run(f"sed -ie 's/^rpc_address: .*/rpc_address: {node.ip_address}/g' {node.offline_install_dir}/etc/scylla/scylla.yaml")
    ...
    def node_setup(self, node: BaseNode, verbose: bool = False, timeout: int = 3600):  # noqa: PLR0912, PLR0914
        ...
        if node.distro.is_ubuntu and "fips" in (self.params.get("assert_linux_distro_features") or []):
            ...
            assert "ENTITLED" in pro_status.stdout and int(fibs_enabled.stdout) == 1, "Even though Ubuntu pro is enabled, FIPS is not enabled"
        ...
        if append_scylla_node_exporter_args := self.params.get("append_scylla_node_exporter_args"):
            ...
            with remote_file(remoter=node.remoter, remote_path=configuration_file, serializer=properties.serialize, deserializer=properties.deserialize, sudo=True) as properties_file:
        ...
        if self.params.get("gce_setup_hybrid_raid"):
            ...
            if not (gce_n_local_ssd_disk_db > 0 and gce_pd_ssd_disk_size_db > 0):
                msg = f"Hybrid RAID cannot be configured without NVMe ({gce_n_local_ssd_disk_db}) and PD-SSD ({gce_pd_ssd_disk_size_db})"
            ...
            node.remoter.send_files(src=f"{get_sct_root_path()}/scylla-qa-internal/custom_d1/{hybrid_raid_script}", dst=target_path)
    ...
    def node_startup(self, node: BaseNode, verbose: bool = False, timeout: int = 3600):
        if not self.test_config.REUSE_CLUSTER:
            node.log.debug("io.conf before reboot: %s", node.remoter.sudo(f"cat {node.add_install_prefix('/etc/scylla.d/io.conf')}").stdout)
            node.start_scylla_server(verify_up=False)
            if self.params.get("jmx_heap_memory"):
                node.restart_scylla_jmx()
            node.log.debug("io.conf right after reboot: %s", node.remoter.sudo(f"cat {node.add_install_prefix('/etc/scylla.d/io.conf')}").stdout)
            if self.params.get("use_mgmt") and self.node_type == "scylla-db":
                node.remoter.sudo(
                    shell_script_cmd("""\
    ...
    def _scylla_install(self, node):
        ...
        if self.params.get("unified_package"):
            node.offline_install_scylla(unified_package=self.params.get("unified_package"), nonroot=self.params.get("nonroot_offline_install"))
        else:
    ...
    def clean_replacement_node_options(node):
        ...
        if node.replacement_node_ip:
            ...
            node.remoter.run(f'sudo sed -i -e "s/^replace_address_first_boot:/# replace_address_first_boot:/g" {node.add_install_prefix(SCYLLA_YAML_PATH)}')
        if node.replacement_host_id:
            node.replacement_host_id = None
            node.remoter.run(f'sudo sed -i -e "s/^replace_node_first_boot:/# replace_node_first_boot:/g" {node.add_install_prefix(SCYLLA_YAML_PATH)}')
    ...
    def verify_decommission(self, node: BaseNode):
        ...
        with node_allocator.run_nemesis(nemesis_label="verify decommission", node_list=self.data_nodes) as verification_node:
            node_ip_list = get_node_ip_list(verification_node)
            missing_host_ids = verification_node.raft.search_inconsistent_host_ids()

            decommission_done = list(node.follow_system_log(patterns=["DECOMMISSIONING: done"], start_from_beginning=True))
            ...
            if target_node_ip in node_ip_list and not missing_host_ids and not decommission_done:
                ...
                error_msg = "Node that was decommissioned %s still in the cluster. Cluster status info: %s" % (node, cluster_status)
    ...
    def get_cluster_manager(self, create_if_not_exists: bool = True, force_add: bool = False, **add_cluster_extra_params) -> AnyManagerCluster:
        ...
        if mgr_cluster and force_add:
            ...
            mgr_cluster.delete()
            return self.create_cluster_manager(cluster_name=cluster_name, manager_tool=manager_tool, **add_cluster_extra_params)
    ...
    def create_cluster_manager(self, cluster_name: str, manager_tool=None, **add_cluster_extra_params) -> AnyManagerCluster:
...
class BaseLoaderSet:
    ...
    def gemini_version(self):
        if not self._gemini_version:
            try:
                result = self.nodes[0].remoter.run(f"docker run --rm {self.params.get('stress_image.gemini')} gemini --version", ignore_status=True)
                if result.ok:
    ...
    def node_setup(self, node, verbose=False, **kwargs):
        ...
        if self.params.get("client_encrypt"):
            node.create_node_certificate(node.ssl_conf_dir / TLSAssets.CLIENT_CERT, node.ssl_conf_dir / TLSAssets.CLIENT_KEY)
            for src in (CA_CERT_FILE, JKS_TRUSTSTORE_FILE):
                shutil.copy(src, node.ssl_conf_dir)
            export_pem_cert_to_pkcs12_keystore(node.ssl_conf_dir / TLSAssets.CLIENT_CERT, node.ssl_conf_dir / TLSAssets.CLIENT_KEY, node.ssl_conf_dir / TLSAssets.PKCS12_KEYSTORE)

            if self.params.get("use_prepared_loaders"):
                node.config_client_encrypt()

        result = node.remoter.run("test -e ~/PREPARED-LOADER", ignore_status=True)
        node.remoter.sudo("bash -cxe \"echo '*\t\thard\tcore\t\tunlimited\n*\t\tsoft\tcore\t\tunlimited' >> /etc/security/limits.d/20-coredump.conf\"")
        if result.exit_status == 0:
        ...
        for loader in self.nodes:
            try:
                for search_cmd in search_cmds:
                    wait.wait_for(kill_cs_process, text="Search and kill c-s processes", timeout=30, throw_exc=False, loader=loader, filter_cmd=search_cmd)
    ...
    def kill_docker_loaders(self):
        ...
        for loader in self.nodes:
            try:
                loader.remoter.run(cmd=f"docker ps -a -q --filter label=TestId={test_id} | xargs docker rm -f", verbose=True, ignore_status=True)
...
class BaseMonitorSet:
    ...
    def sct_dashboard_json_file(self):
        if not self._sct_dashboard_json_file:
            ...
            current_sct_dashboard_json_path = "/".join(original_path_parts[:-1] + [f"sct-monitor-{generate_random_string(5)}-{original_path_parts[-1]}"])
            ...
            self.sct_dashboard_json_file_content_update(update_params=self.json_file_params_for_replace, json_file=self._sct_dashboard_json_file)
    ...
    def install_scylla_monitoring_prereqs(self, node):
        ...
        if node.distro.is_rhel_like:
            ...
            node.install_package(package_name="software-properties-common python3 python3-dev python3-setuptools unzip wget python3-pip")
            ...
            node.install_package(package_name="apt-transport-https ca-certificates curl software-properties-common gnupg2")
            node.remoter.sudo('bash -ce "curl -fsSL https://download.docker.com/linux/debian/gpg | apt-key add -"', retry=3)
            node.remoter.sudo(cmd='add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/debian $(lsb_release -cs) stable"')
    ...
    def configure_overview_template(self, node: BaseNode):
        ...
        with remote_file(remoter=node.remoter, remote_path=str((self.monitor_install_path / "grafana" / "scylla-overview.template.json").absolute())) as file:
            ...
            row, index = next(((row, i) for i, row in enumerate(template["dashboard"]["rows"]) if find_overview_row(row)), (None, -1))
            if row:
            ...
            try:
                variable = next(var for var in template["dashboard"]["templating"]["list"] if var.get("class", "") == "by_template_var")
    ...
    def configure_scylla_monitoring(self, node, sct_metrics=True, alert_manager=True):  # noqa: PLR0914
        ...
        if sct_metrics and not self.params.get("reuse_cluster"):
            ...
            if self.params.get("gemini_cmd"):
                gemini_loader_targets_list = ["%s:2112" % getattr(node, self.DB_NODES_IP_ADDRESS) for node in self.targets["loaders"].nodes]
                scrape_configs.append(dict(job_name="gemini_metrics", honor_labels=True, static_configs=[dict(targets=gemini_loader_targets_list)]))
            ...
            if nosqlbench_cmds and "nosqlbench" in nosqlbench_cmds[0]:
                ...
                scrape_configs.append(dict(job_name="nosqlbench_metrics", honor_labels=True, static_configs=[dict(targets=graphite_exporter_target_list)]))
    ...
    @retrying(n=5, sleep_time=10, allowed_exceptions=(Failure, UnexpectedExit), message="Waiting for reconfiguring scylla monitoring")
    def reconfigure_scylla_monitoring(self):
        ...
        for db_node in getattr(self.targets.get("db_cluster"), "nodes", []):
            ...
            node_export_targets_per_dc[db_node.region]["targets"].append(f"{normalize_ipv6_url(getattr(db_node, self.DB_NODES_IP_ADDRESS))}:9577")
        ...
        for node in self.nodes:
            ...
            node_export_targets_per_dc["monitor"]["targets"].append(f"{normalize_ipv6_url(node.private_ip_address)}:9100")
        ...
        for node in self.nodes:
            ...
            if self.params.get("scylla_rsyslog_setup"):
                for db_node in self.targets["db_cluster"].nodes:
                    db_node.remoter.sudo(shell_script_cmd(f"[ -f /etc/rsyslog.d/scylla.conf ] || scylla_rsyslog_setup --remote-server {node.ip_address}"), verbose=True)
    ...
    def _create_manager_prometheus_yaml(self, node):
        ...
        node.remoter.run(f"echo '{str(full_yaml_string)}' > {self.monitor_install_path}/prometheus/scylla_manager_servers.yml")
        # Writing directly to the yaml in the conf dir results in permission denied
        node.remoter.run(f"cp {self.monitor_install_path}/prometheus/scylla_manager_servers.yml {self.monitoring_conf_dir}/scylla_manager_servers.yml")
    ...
    def start_scylla_monitoring(self, node):
        ...
        with node._remote_yaml(f"{self.monitor_install_path}/prometheus/rule_config.yml", sudo=False) as alert_manager_config:
            alert_manager_config.clear()
            alert_manager_config["global"] = dict(resolve_timeout="5m")
            alert_manager_config["route"] = dict(receiver="null", group_by=["job"], group_wait="30s", group_interval="5m", repeat_interval="12h")
    ...
    def add_sct_dashboards_to_grafana(self, node):
        def _register_grafana_json(json_filename):
            url = "'http://{0}:{1.grafana_port}/api/dashboards/db'".format(normalize_ipv6_url(node.external_address), self)
            result = LOCALRUNNER.run('curl -g -XPOST -i %s --data-binary @%s -H "Content-Type: application/json"' % (url, json_filename))
            return result.exited == 0

        wait.wait_for(_register_grafana_json, step=10, text="Waiting to register '%s'..." % self.sct_dashboard_json_file, json_filename=self.sct_dashboard_json_file, throw_exc=False)
    ...
    def update_default_time_range(self, start_timestamp: float, end_timestamp: float) -> None:
        ...
        cmd = f'find {os.path.join(self.monitor_install_path, "grafana/build/")} -name \'*.json\' -exec sed -i \'s/"from": "[^"]*"/"from": "{start_iso}"/; s/"to": "[^"]*"/"to": "{end_iso}"/\' {{}} +'
        for node in self.nodes:
            try:
    ...
    def get_grafana_annotations(self, node):
        ...
        try:
            res = requests.get(url=annotations_url.format(node_ip=normalize_ipv6_url(node.grafana_address), grafana_port=self.grafana_port))
            if res.ok:
                return res.content
        except Exception as ex:  # noqa: BLE001
    ...
    def set_grafana_annotations(self, node, annotations_data):
        ...
        res = requests.post(url=annotations_url.format(node_ip=normalize_ipv6_url(node.grafana_address), grafana_port=self.grafana_port), data=annotations_data, headers={"Content-Type": "application/json"})
    ...
    def get_grafana_screenshots(self, node: BaseNode, test_start_time: float) -> list[str]:
        ...
        screenshot_collector = GrafanaScreenShot(name="grafana-screenshot", test_start_time=test_start_time, extra_entities=grafana_extra_dashboards)
    ...
    def upload_annotations_to_s3(self):
        ...
        try:
            ...
            if annotations:
                ...
                response = requests.put(annotations_url, data=annotations, headers={"Content-type": "application/json; charset=utf-8"})
