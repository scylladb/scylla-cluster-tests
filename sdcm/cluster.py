...
class BaseCluster:
    ...
    def get_load_balancing_policy(self, node, whitelist_nodes=None):
        ...
        if self.params.get("rack_aware_loader"):
            ...
            if node.parent_cluster.racks_count > 1:
                ...
                LOGGER.info("Using RackAwareRoundRobinPolicy %s. Loader rack: %s. Loader datacenter: %s. Node IPs: %s",
                            node.name, loader_rack, loader_dc, node_ips)
        else:
            wlrr = WhiteListRoundRobinPolicy(node_ips)
            LOGGER.info("Using WhiteListRoundRobinPolicy %s", node.name)
