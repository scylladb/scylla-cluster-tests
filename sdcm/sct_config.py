...
def is_config_option_appendable(option_name: str) -> bool:
    ...
    return field.json_schema_extra.get("appendable", False)
...
def sct_field(*args, **kwargs):
    ...
    extra = {k: v for k, v in kwargs.items() if k in ("env", "appendable", "k8s_multitenancy_supported")}
...
class SCTConfiguration(BaseModel):
    ...
    perf_extra_jobs_to_compare: StringOrList = sct_field(
    ...
    manager_backup_restore_method: String = sct_field(
    ...
    backtrace_stall_decoding: Boolean = sct_field(
    ...
    backtrace_decoding_disable_regex: String = sct_field(
    ...
    dynamodb_primarykey_type: String = sct_field(
        description="Type of dynamodb table to create with range key or not, can be: "
    ...
    alternator_test_table: DictOrStr = sct_field(
    ...
    alternator_access_key_id: String = sct_field(
        description="the aws_access_key_id that would be used for alternator", env="SCT_ALTERNATOR_ACCESS_KEY_ID"
    )
    ...
    append_scylla_yaml: DictOrStrOrPydantic = sct_field(
    ...
    nemesis_class_name: StringOrList = sct_field(
        description="""
    ...
    nemesis_seed: IntOrList = sct_field(
    ...
    nemesis_grow_shrink_instance_type: String = sct_field(
    ...
    cluster_target_size: IntOrList = sct_field(
    ...
    run_gemini_in_rolling_upgrade: Boolean = sct_field(
    ...
    ami_id_vector_store: String = sct_field(
        description="AMS AMI id to use for vector store node",
        env="SCT_AMI_ID_VECTOR_STORE",
    )
    instance_type_vector_store: String = sct_field(
    ...
    ami_vector_store_user: String = sct_field(
        description="",
        env="SCT_AMI_VECTOR_STORE_USER",
    )
    ...
    vector_store_docker_image: String = sct_field(
    ...
    vector_store_version: String = sct_field(
    ...
    stress_cmd_cache_warmup: StringOrList = sct_field(
    ...
    stress_cmd_no_mv_profile: StringOrList = sct_field(
        description="",
    ...
    target_upgrade_version: String = sct_field(
        description="The target version to upgrade Scylla to.", env="SCT_TARGET_UPGRADE_VERSION"
    )
    ...
    upgrade_node_packages: String = sct_field(
        description="Specifies the packages to be upgraded on the node.", env="SCT_UPGRADE_NODE_PACKAGES"
    )
    ...
    enable_kms_key_rotation: Boolean = sct_field(
        description="Allows to disable KMS keys rotation. Applicable to GCP and Azure backends. "
    ...
    use_scylla_doctor_on_failure: Boolean = sct_field(
    ...
    post_behavior_vector_store_nodes: Literal["destroy", "keep", "keep-on-failure"] = sct_field(
        description="""
    ...
    enable_truncate_checks_on_node_upgrade: Boolean = sct_field(
    ...
    stress_before_upgrade: StringOrList = sct_field(
        description="Stress command to be run before upgrade (prepare stage)",
        env="SCT_STRESS_BEFORE_UPGRADE",
    )
    stress_during_entire_upgrade: StringOrList = sct_field(
    ...
    stress_after_cluster_upgrade: StringOrList = sct_field(
    ...
    jepsen_test_count: int = sct_field(
        description="Possible number of reruns of single Jepsen test command", env="SCT_JEPSEN_TEST_COUNT"
    )
    ...
    enable_argus: Boolean = sct_field(description="Control reporting to argus", env="SCT_ENABLE_ARGUS")
    ...
    def _check_unexpected_sct_variables(self):
        ...
        config_keys = {
            field.json_schema_extra.get("env", f"SCT_{field_name}".upper())
            for field_name, field in self.model_fields.items()
    ...
    def _validate_nemesis_can_run_on_non_seed(self) -> None:
        if self.get("nemesis_filter_seeds") is False or self.get("nemesis_class_name") == "NoOpMonkey":
            return
        seeds_num = self.get("seeds_num")
        num_of_db_nodes = sum([i for i in list(self.get("n_db_nodes"))]) + int(self.get("add_node_cnt"))
