...
class SCTConfiguration(dict):
    ...
    config_options = [
        ...
        dict(
            ...
            type=list,
            help="List if service level shares - how many server levels to create and test. Uses in SLA test.list of int, like: [100, 200]",
        ),
        dict(
        ...
        dict(
            ...
            type=boolean,
            help="if true, create 'cluster' placement group for test case for low-latency network performance achievement",
        ),
        ...
        dict(
            ...
            type=str,
            help="K8S release channel name to be used. Expected values are: 'rapid', 'regular', 'stable' and '' (static / No channel).",
        ),
        # k8s options
        dict(
        ...
        dict(
            ...
            type=boolean,
            help="Defines whether the K8S API server logging must be enabled and it's logs gathered. Be aware that it may be really huge set of data.",
        ),
        dict(
        ...
        dict(
            name="k8s_connection_bundle_file",
            env="SCT_K8S_CONNECTION_BUNDLE_FILE",
            type=_file,
            help="Serverless configuration bundle file",
            k8s_multitenancy_supported=True,
        ),
        ...
        dict(
            ...
            type=str_or_list_or_eval,
            help="jobs to compare performance results with, for example if running in staging, we still can compare with official jobs",
        ),
        dict(
        ...
        dict(
            ...
            type=dict_or_str,
            help="Threads amount of stress load for gradual performance test per sub-test. Example: {'read': 100, 'write': [200, 300], 'mixed': 300}",
        ),
        dict(
        ...
        dict(
            ...
            type=dict_or_str,
            help="Step duration of c-s load for gradual performance test per sub-test. Example: {'read': '30m', 'write': None, 'mixed': '30m'}",
        ),
        # RefreshTest
        ...
        dict(
            ...
            type=int,
            help="The time interval in minutes which gets waited before the KMS key rotation happens. Applied when the AWS KMS service is configured to be used.",
        ),
        dict(
            name="enterprise_disable_kms",
            env="SCT_ENTERPRISE_DISABLE_KMS",
            type=boolean,
            help="An escape hatch to disable KMS for enterprise run, when needed, we enable kms by default since if we use scylla 2023.1.3 and up",
        ),
        dict(
            name="logs_transport",
    ...
    def __init__(self):  # noqa: PLR0912, PLR0914, PLR0915
        ...
        if scylla_version := self.get("scylla_version"):
            ...
            if (
                self.get("n_loaders")
                and not self.get("bare_loaders")
                and not self.get("scylla_repo_loader")
                and self.get("cluster_backend") != "aws"
            ):
                ...
                self["scylla_repo_loader"] = find_scylla_repo(
                    scylla_version_for_loader, dist_type_loader, dist_version_loader
                )
        ...
        if self.get("authenticator") and self.get("authenticator") == "PasswordAuthenticator":
            ...
            if not (authenticator_password and authenticator_user):
                raise ValueError(
                    "For PasswordAuthenticator authenticator authenticator_user and authenticator_password have to be provided"
                )
        ...
        if run_fullscan_params := self.get("run_fullscan"):
            ...
            for param in run_fullscan_params:
                try:
                    ...
                    raise ValueError(
                        f"each item of run_fullscan list: {run_fullscan_params}, item {param}, must be JSON but got error: {repr(exp)}"
                    ) from exp
            ...
            for workload, params in performance_throughput_params.items():
                ...
                for thread_count in gradual_threads[workload]:
                    if not isinstance(thread_count, int):
                        raise ValueError(
                            f"Invalid thread count type for '{workload}': {thread_count} (type: {type(thread_count).__name__})"
                        )
                ...
                if len(gradual_threads[workload]) > 1 and len(gradual_threads[workload]) != len(params):
                    raise ValueError(
                        f"perf_gradual_threads for {workload} should be a single-element, integer or list, or a list with the same length as perf_gradual_throttle_steps for {workload}"
                    )
                    ...
                    for option in cmd.split():
                        if option.startswith("profile="):
                            ...
                            if not profile_path.startswith("/tmp"):
                                raise ValueError(
                                    f"Stress command parameter '{param_name}' contains wrong path '{profile_path}' to profile, it should be formed in following manner '/tmp/{{file_name_from_data_dir}}'"
                                )
                            ...
                            raise ValueError(
                                f"Stress command parameter '{param_name}' contains profile '{profile_path}' that does not exists under data_dir/"
                            )
    ...
    def _validate_placement_group_required_values(self):
        if self.get("use_placement_group"):
            ...
            assert az_count == 1 and regions_count == 1, (
                f"Number of Regions({regions_count}) and AZ({az_count}) should be 1 when param use_placement_group is used"
            )
    ...
    def _check_partition_range_with_data_validation_correctness(self):
        ...
        if partition_range_with_data_validation:
            error_message_template = (
                "Expected format of 'partition_range_with_data_validation' parameter is: <min PK value>-<max PK value>. {}Example: 0-250. Got value: %s"
