...
def _nested_env_subkey(env_key: str, field_env: str, sep: str) -> str | None:
    ...
    sdcm.utils.nested_env_key) so that a single underscore inside a field name
    (like SCT_INSTANCE_TYPE_DB) is never mistaken for a separator, and a field
    env name that is a prefix of another one never wrongly claims its sub-keys.
...
class SCTConfiguration(dict):
    ...
    config_options = [
        ...
        dict(
            name="cs_safepoint_logging",
            env="SCT_CS_SAFEPOINT_LOGGING",
            type=boolean,
            help="Enable JVM safepoint logging (-Xlog:safepoint) for the cassandra-stress loaders. "
        ...
        dict(
            ...
            help="""NVMe device self-test type to run: 1 (short, ~2 min) or 2 (extended, may take hours).
                 ...
                 Honored only on controllers that advertise Device Self-test support (Identify Controller OACS bit 4); unsupported controllers are skipped without issuing the command.
                 This has no effect on AWS: neither instance-store (Nitro SSD) nor EBS implements Device Self-test, so on AWS the diagnostics rely on SMART counters and the error log instead.""",
        ),
        dict(
