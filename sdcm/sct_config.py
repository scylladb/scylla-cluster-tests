...
class SCTConfiguration(dict):
    ...
    config_options = [
        dict(name="config_files", env="SCT_CONFIG_FILES", type=str_or_list_or_eval, help="a list of config files that would be used", appendable=False),
        dict(name="cluster_backend", env="SCT_CLUSTER_BACKEND", type=str, help="backend that will be used, aws/gce/azure/docker/xcloud", appendable=False),
        dict(name="test_method", env="SCT_TEST_METHOD", type=str, help="class.method used to run the test. Filled automatically with run-test sct command.", appendable=False),
        dict(
            name="test_duration",
        ...
        dict(name="n_test_oracle_db_nodes", env="SCT_N_TEST_ORACLE_DB_NODES", type=int_or_space_separated_ints, help="Number list of oracle test nodes in multiple data centers."),
        dict(name="n_loaders", env="SCT_N_LOADERS", type=int_or_space_separated_ints, help="Number list of loader nodes in multiple data centers"),
        dict(name="n_monitor_nodes", env="SCT_N_MONITORS_NODES", type=int_or_space_separated_ints, help="Number list of monitor nodes in multiple data centers"),
        dict(name="intra_node_comm_public", env="SCT_INTRA_NODE_COMM_PUBLIC", type=boolean, help="If True, all communication between nodes are via public addresses"),
        dict(
        ...
        dict(name="user_credentials_path", env="SCT_USER_CREDENTIALS_PATH", type=str, help="""Path to your user credentials. qa key are downloaded automatically from S3 bucket"""),
        dict(name="cloud_credentials_path", env="SCT_CLOUD_CREDENTIALS_PATH", type=str, help="""Path to your user credentials. qa key are downloaded automatically from S3 bucket"""),
        ...
        dict(name="cloud_prom_bearer_token", env="SCT_CLOUD_PROM_BEARER_TOKEN", type=str, help="""scylla cloud promproxy bearer_token to federate monitoring data into our monitoring instance"""),
        dict(name="cloud_prom_path", env="SCT_CLOUD_PROM_PATH", type=str, help="""scylla cloud promproxy path to federate monitoring data into our monitoring instance"""),
        dict(name="cloud_prom_host", env="SCT_CLOUD_PROM_HOST", type=str, help="""scylla cloud promproxy hostname to federate monitoring data into our monitoring instance"""),
        dict(
        ...
        dict(
            ...
            env="SCT_SCYLLA_REPO",
            type=str,
            help="Url to the repo of scylla version to install scylla. Can provide specific version after a colon e.g: `https://s3.amazonaws.com/downloads.scylladb.com/deb/ubuntu/scylla-2021.1.list:2021.1.18`",
        ),
        dict(name="scylla_apt_keys", env="SCT_SCYLLA_APT_KEYS", type=str_or_list, help="APT keys for ScyllaDB repos"),
        dict(name="unified_package", env="SCT_UNIFIED_PACKAGE", type=str, help="Url to the unified package of scylla version to install scylla"),
        dict(name="nonroot_offline_install", env="SCT_NONROOT_OFFLINE_INSTALL", type=boolean, help="Install Scylla without required root priviledge"),
        dict(name="install_mode", env="SCT_INSTALL_MODE", type=str, help="Scylla install mode, repo/offline/web", appendable=False),
        dict(
            name="scylla_version",
        ...
        dict(name="scylla_linux_distro", env="SCT_SCYLLA_LINUX_DISTRO", type=str, help="""The distro name and family name to use. Example: 'ubuntu-jammy' or 'debian-bookworm'.""", appendable=False),
        dict(name="scylla_linux_distro_loader", env="SCT_SCYLLA_LINUX_DISTRO_LOADER", type=str, help="""The distro name and family name to use. Example: 'ubuntu-jammy' or 'debian-bookworm'.""", appendable=False),
        dict(name="assert_linux_distro_features", env="SCT_ASSERT_LINUX_DISTRO_FEATURES", type=str_or_list_or_eval, help="""List of distro features relevant to SCT test. Example: 'fips'.""", appendable=True),
        dict(name="scylla_repo_m", env="SCT_SCYLLA_REPO_M", type=str, help="Url to the repo of scylla version to install scylla from for managment tests"),
        dict(name="scylla_mgmt_address", env="SCT_SCYLLA_MGMT_ADDRESS", type=str, help="Url to the repo of scylla manager version to install for management tests"),
        dict(name="scylla_mgmt_agent_address", env="SCT_SCYLLA_MGMT_AGENT_ADDRESS", type=str, help="Url to the repo of scylla manager agent version to install for management tests"),
        dict(name="manager_version", env="SCT_MANAGER_VERSION", type=str, help="Branch of scylla manager server and agent to install. Options in defaults/manager_versions.yaml", appendable=False),
        dict(name="target_manager_version", env="SCT_TARGET_MANAGER_VERSION", type=str, help="Branch of scylla manager server and agent to upgrade to. Options in defaults/manager_versions.yaml", appendable=False),
        dict(name="manager_scylla_backend_version", env="SCT_MANAGER_SCYLLA_BACKEND_VERSION", type=str, help="Branch of scylla db enterprise to install. Options in defaults/manager_versions.yaml", appendable=False),
        dict(name="scylla_mgmt_agent_version", env="SCT_SCYLLA_MGMT_AGENT_VERSION", type=str, help="", appendable=False),
        dict(name="scylla_mgmt_pkg", env="SCT_SCYLLA_MGMT_PKG", type=str, help="Url to the scylla manager packages to install for management tests"),
        dict(name="stress_cmd_lwt_i", env="SCT_STRESS_CMD_LWT_I", type=str, help="Stress command for LWT performance test for INSERT baseline"),
        dict(name="stress_cmd_lwt_d", env="SCT_STRESS_CMD_LWT_D", type=str, help="Stress command for LWT performance test for DELETE baseline"),
        dict(name="stress_cmd_lwt_u", env="SCT_STRESS_CMD_LWT_U", type=str, help="Stress command for LWT performance test for UPDATE baseline"),
        dict(name="stress_cmd_lwt_ine", env="SCT_STRESS_CMD_LWT_INE", type=str, help="Stress command for LWT performance test for INSERT with IF NOT EXISTS"),
        dict(name="stress_cmd_lwt_uc", env="SCT_STRESS_CMD_LWT_UC", type=str, help="Stress command for LWT performance test for UPDATE with IF <condition>"),
        dict(name="stress_cmd_lwt_ue", env="SCT_STRESS_CMD_LWT_UE", type=str, help="Stress command for LWT performance test for UPDATE with IF EXISTS"),
        dict(name="stress_cmd_lwt_de", env="SCT_STRESS_CMD_LWT_DE", type=str, help="Stress command for LWT performance test for DELETE with IF EXISTS"),
        dict(name="stress_cmd_lwt_dc", env="SCT_STRESS_CMD_LWT_DC", type=str, help="Stress command for LWT performance test for DELETE with IF condition>"),
        dict(name="stress_cmd_lwt_mixed", env="SCT_STRESS_CMD_LWT_MIXED", type=str, help="Stress command for LWT performance test for mixed lwt load"),
        dict(name="stress_cmd_lwt_mixed_baseline", env="SCT_STRESS_CMD_LWT_MIXED_BASELINE", type=str, help="Stress command for LWT performance test for mixed lwt load baseline"),
        dict(name="use_cloud_manager", env="SCT_USE_CLOUD_MANAGER", type=boolean, help="When define true, will install scylla cloud manager"),
        dict(name="use_ldap", env="SCT_USE_LDAP", type=boolean, help="When defined true, LDAP is going to be used."),
        dict(name="use_ldap_authorization", env="SCT_USE_LDAP_AUTHORIZATION", type=boolean, help="When defined true, will create a docker container with LDAP and configure scylla.yaml to use it"),
        dict(name="use_ldap_authentication", env="SCT_USE_LDAP_AUTHENTICATION", type=boolean, help="When defined true, will create a docker container with LDAP and configure scylla.yaml to use it"),
        dict(name="prepare_saslauthd", env="SCT_PREPARE_SASLAUTHD", type=boolean, help="When defined true, will install and start saslauthd service"),
        dict(name="ldap_server_type", env="SCT_LDAP_SERVER_TYPE", type=str, help="This option indicates which server is going to be used for LDAP operations. [openldap, ms_ad]"),
        dict(name="use_mgmt", env="SCT_USE_MGMT", type=boolean, help="When define true, will install scylla management"),
        dict(name="parallel_node_operations", env="SCT_PARALLEL_NODE_OPERATIONS", type=boolean, help="When defined true, will run node operations in parallel. Supported operations: startup"),
        dict(name="manager_prometheus_port", env="SCT_MANAGER_PROMETHEUS_PORT", type=int, help="Port to be used by the manager to contact Prometheus"),
        dict(name="target_scylla_mgmt_server_address", env="SCT_TARGET_SCYLLA_MGMT_SERVER_ADDRESS", type=str, help="Url to the repo of scylla manager version used to upgrade the manager server"),
        dict(name="target_scylla_mgmt_agent_address", env="SCT_TARGET_SCYLLA_MGMT_AGENT_ADDRESS", type=str, help="Url to the repo of scylla manager version used to upgrade the manager agents"),
        dict(
        ...
        dict(name="user_prefix", env="SCT_USER_PREFIX", type=str, help="the prefix of the name of the cloud instances, defaults to username"),
        dict(name="ami_id_db_scylla_desc", env="SCT_AMI_ID_DB_SCYLLA_DESC", type=str, help="version name to report stats to Elasticsearch and tagged on cloud instances"),
        dict(
            name="sct_public_ip",
        ...
        dict(name="backtrace_decoding", env="SCT_BACKTRACE_DECODING", type=boolean, help="""If True, all backtraces found in db nodes would be decoded automatically"""),
        dict(
        ...
        dict(name="instance_provision", env="SCT_INSTANCE_PROVISION", type=str, help="instance_provision: spot|on_demand|spot_fleet"),
        dict(
            name="instance_provision_fallback_on_demand",
            env="SCT_INSTANCE_PROVISION_FALLBACK_ON_DEMAND",
            type=boolean,
            help="instance_provision_fallback_on_demand: create instance on_demand provision type if instance with selected 'instance_provision' type creation failed. Expected values: true|false (default - false",
        ),
        dict(
            name="reuse_cluster",
        ...
        dict(name="test_id", env="SCT_TEST_ID", type=str, help="""Set the test_id of the run manually. Use only from the env before running Hydra"""),
        dict(
        ...
        dict(name="seeds_selector", env="SCT_SEEDS_SELECTOR", type=str, choices=["random", "first", "all"], help="""How to select the seeds. Expected values: random/first/all"""),
        dict(name="seeds_num", env="SCT_SEEDS_NUM", type=int, help="""Number of seeds to select"""),
        dict(name="email_recipients", env="SCT_EMAIL_RECIPIENTS", type=str_or_list, help="""list of email of send the performance regression test to"""),
        dict(name="email_subject_postfix", env="SCT_EMAIL_SUBJECT_POSTFIX", type=str, help="""Email subject postfix"""),
        dict(name="enable_test_profiling", env="SCT_ENABLE_TEST_PROFILING", type=boolean, help="""Turn on sct profiling"""),
        dict(name="ssh_transport", env="SSH_TRANSPORT", type=str, help="""Set type of ssh library to use. Could be 'fabric' (default) or 'libssh2'"""),
        # Scylla command line arguments options
        dict(name="experimental_features", env="SCT_EXPERIMENTAL_FEATURES", type=list, help="unlock specified experimental features"),
        dict(name="server_encrypt", env="SCT_SERVER_ENCRYPT", type=boolean, help="when enable scylla will use encryption on the server side"),
        dict(name="client_encrypt", env="SCT_CLIENT_ENCRYPT", type=boolean, help="when enable scylla will use encryption on the client side"),
        dict(name="peer_verification", env="SCT_PEER_VERIFICATION", type=boolean, help="enable peer verification for encrypted communication"),
        dict(name="client_encrypt_mtls", env="SCT_CLIENT_ENCRYPT_MTLS", type=boolean, help="when enabled scylla will enforce mutual authentication when client-to-node encryption is enabled"),
        dict(name="server_encrypt_mtls", env="SCT_SERVER_ENCRYPT_MTLS", type=boolean, help="when enabled scylla will enforce mutual authentication when node-to-node encryption is enabled"),
        dict(name="hinted_handoff", env="SCT_HINTED_HANDOFF", type=str, help="when enable or disable scylla hinted handoff (enabled/disabled)"),
        dict(
            name="authenticator",
        ...
        dict(name="authenticator_user", env="SCT_AUTHENTICATOR_USER", type=str, help="the username if PasswordAuthenticator is used"),
        dict(name="authenticator_password", env="SCT_AUTHENTICATOR_PASSWORD", type=str, help="the password if PasswordAuthenticator is used"),
        dict(name="authorizer", env="SCT_AUTHORIZER", type=str, help="which authorizer scylla will use AllowAllAuthorizer/CassandraAuthorizer"),
        ...
        dict(name="service_level_shares", env="SCT_SERVICE_LEVEL_SHARES", type=list, help="List if service level shares - how many server levels to create and test. Uses in SLA test.list of int, like: [100, 200]"),
        dict(name="alternator_port", env="SCT_ALTERNATOR_PORT", type=int, help="Port to configure for alternator in scylla.yaml"),
        dict(
            name="dynamodb_primarykey_type",
            env="SCT_DYNAMODB_PRIMARYKEY_TYPE",
            type=str,
            help=f"Type of dynamodb table to create with range key or not, can be:\n{','.join([schema.value for schema in alternator.enums.YCSBSchemaTypes])}",
        ...
        dict(
            ...
            type=str,
            help="Set the write isolation for the alternator table, see https://github.com/scylladb/scylla/blob/master/docs/alternator/alternator.md#write-isolation-policies for more details",
        ),
        dict(name="alternator_use_dns_routing", env="SCT_ALTERNATOR_USE_DNS_ROUTING", type=boolean, help="If true, spawn a docker with a dns server for the ycsb loader to point to"),
        dict(
        ...
        dict(name="alternator_enforce_authorization", env="SCT_ALTERNATOR_ENFORCE_AUTHORIZATION", type=boolean, help="If true, enable the authorization check in dynamodb api (alternator)"),
        dict(name="alternator_access_key_id", env="SCT_ALTERNATOR_ACCESS_KEY_ID", type=str, help="the aws_access_key_id that would be used for alternator"),
        dict(name="alternator_secret_access_key", env="SCT_ALTERNATOR_SECRET_ACCESS_KEY", type=str, help="the aws_secret_access_key that would be used for alternator"),
        dict(name="region_aware_loader", env="SCT_REGION_AWARE_LOADER", type=boolean, help="When in multi region mode, run stress on loader that is located in the same region as db node"),
        dict(name="append_scylla_args", env="SCT_APPEND_SCYLLA_ARGS", type=str, help="More arguments to append to scylla command line"),
        dict(name="append_scylla_args_oracle", env="SCT_APPEND_SCYLLA_ARGS_ORACLE", type=str, help="More arguments to append to oracle command line"),
        dict(name="append_scylla_yaml", env="SCT_APPEND_SCYLLA_YAML", type=dict_or_str, help="More configuration to append to /etc/scylla/scylla.yaml"),
        dict(name="append_scylla_node_exporter_args", env="SCT_SCYLLA_NODE_EXPORTER_ARGS", type=str, help="More arguments to append to scylla-node-exporter command line"),
        ...
        dict(name="nemesis_interval", env="SCT_NEMESIS_INTERVAL", type=int, k8s_multitenancy_supported=True, help="""Nemesis sleep interval to use if None provided specifically in the test"""),
        dict(
        ...
        dict(name="nemesis_during_prepare", env="SCT_NEMESIS_DURING_PREPARE", type=boolean, k8s_multitenancy_supported=True, help="""Run nemesis during prepare stage of the test"""),
        dict(
            name="nemesis_seed",
        ...
        dict(name="nemesis_add_node_cnt", env="SCT_NEMESIS_ADD_NODE_CNT", type=int, k8s_multitenancy_supported=True, help="""Add/remove nodes during GrowShrinkCluster nemesis"""),
        dict(name="nemesis_grow_shrink_instance_type", env="SCT_NEMESIS_GROW_SHRINK_INSTANCE_TYPE", type=_str, k8s_multitenancy_supported=True, help="""Instance type to use for adding/removing nodes during GrowShrinkCluster nemesis"""),
        dict(name="cluster_target_size", env="SCT_CLUSTER_TARGET_SIZE", type=int_or_space_separated_ints, help="""Used for scale test: max size of the cluster"""),
        dict(
        ...
        dict(name="nemesis_filter_seeds", env="SCT_NEMESIS_FILTER_SEEDS", type=boolean, k8s_multitenancy_supported=True, help="""If true runs the nemesis only on non seed nodes"""),
        # Stress Commands
        dict(
            name="stress_cmd",
            env="SCT_STRESS_CMD",
        ...
        dict(name="gemini_schema_url", env="SCT_GEMINI_SCHEMA_URL", type=str, help="""Url of the schema/configuration the gemini tool would use """),
        dict(name="gemini_cmd", env="SCT_GEMINI_CMD", type=str, help="""gemini command to run (for now used only in GeminiTest)"""),
        dict(name="gemini_seed", env="SCT_GEMINI_SEED", type=int, help="Seed number for gemini command"),
        dict(name="gemini_log_cql_statements", env="SCT_GEMINI_LOG_CQL_STATEMENTS", type=boolean, help="Log CQL statements to file"),
        dict(
        ...
        dict(name="run_gemini_in_rolling_upgrade", env="SCT_RUN_GEMINI_IN_ROLLING_UPGRADE", type=boolean, help="Enable running Gemini workload during rolling upgrade test. Default is false."),
        # AWS config options
        dict(name="instance_type_loader", env="SCT_INSTANCE_TYPE_LOADER", type=str, help="AWS image type of the loader node"),
        dict(name="instance_type_monitor", env="SCT_INSTANCE_TYPE_MONITOR", type=str, help="AWS image type of the monitor node"),
        dict(name="instance_type_db", env="SCT_INSTANCE_TYPE_DB", type=str, help="AWS image type of the db node"),
        dict(name="instance_type_db_oracle", env="SCT_INSTANCE_TYPE_DB_ORACLE", type=str, help="AWS image type of the oracle node"),
        dict(name="instance_type_runner", env="SCT_INSTANCE_TYPE_RUNNER", type=str, help="instance type of the sct-runner node"),
        dict(name="region_name", env="SCT_REGION_NAME", type=str_or_list_or_eval, help="AWS regions to use", appendable=False),
        dict(name="security_group_ids", env="SCT_SECURITY_GROUP_IDS", type=str_or_list, help="AWS security groups ids to use"),
        dict(name="use_placement_group", env="SCT_USE_PLACEMENT_GROUP", type=boolean, help="if true, create 'cluster' placement group for test case for low-latency network performance achievement"),
        dict(name="subnet_id", env="SCT_SUBNET_ID", type=str_or_list, help="AWS subnet ids to use"),
        dict(name="ami_id_db_scylla", env="SCT_AMI_ID_DB_SCYLLA", type=str, help="AMS AMI id to use for scylla db node"),
        ...
        dict(name="ami_id_db_cassandra", env="SCT_AMI_ID_DB_CASSANDRA", type=str, help="AMS AMI id to use for cassandra node"),
        ...
        dict(name="instance_type_vector_store", env="SCT_INSTANCE_TYPE_VECTOR_STORE", type=str, help="AWS/GCP cloud provider instance type for Vector Store nodes"),
        ...
        dict(name="root_disk_size_runner", env="SCT_ROOT_DISK_SIZE_RUNNER", type=int, help="root disk size in Gb for sct-runner"),
        ...
        dict(name="extra_network_interface", env="SCT_EXTRA_NETWORK_INTERFACE", type=boolean, help="if true, create extra network interface on each node"),
        dict(name="aws_instance_profile_name_db", env="SCT_AWS_INSTANCE_PROFILE_NAME_DB", type=str, help="This is the name of the instance profile to set on all db instances"),
        dict(name="aws_instance_profile_name_loader", env="SCT_AWS_INSTANCE_PROFILE_NAME_LOADER", type=str, help="This is the name of the instance profile to set on all loader instances"),
        dict(name="backup_bucket_backend", env="SCT_BACKUP_BUCKET_BACKEND", type=str, help="the backend to be used for backup (e.g., 's3', 'gcs' or 'azure')"),
        dict(name="backup_bucket_location", env="SCT_BACKUP_BUCKET_LOCATION", type=str_or_list, help="the bucket name to be used for backup (e.g., 'manager-backup-tests')"),
        dict(name="use_prepared_loaders", env="SCT_USE_PREPARED_LOADERS", type=boolean, help="If True, we use prepared VMs for loader (instead of using docker images)"),
        dict(name="scylla_d_overrides_files", env="SCT_SCYLLA_D_OVERRIDES_FILES", type=str_or_list_or_eval, help="list of files that should upload to /etc/scylla.d/ directory to override scylla config files"),
        # GCE config options
        ...
        dict(
            ...
            help="Supported: us-east1 - means that the zone will be selected automatically or you can mention the zone explicitly, for example: us-east1-b",
            appendable=False,
        ),
        ...
        dict(name="gce_setup_hybrid_raid", env="SCT_GCE_SETUP_HYBRID_RAID", type=boolean, help="If True, SCT configures a hybrid RAID of NVMEs and an SSD for scylla's data"),
        ...
        # azure options
        dict(name="azure_region_name", env="SCT_AZURE_REGION_NAME", type=str_or_list_or_eval, help="Supported: eastus ", appendable=False),
        ...
        dict(name="gke_k8s_release_channel", env="SCT_GKE_K8S_RELEASE_CHANNEL", type=str, help="K8S release channel name to be used. Expected values are: 'rapid', 'regular', 'stable' and '' (static / No channel)."),
        # k8s options
        dict(
            name="k8s_scylla_utils_docker_image",
            env="SCT_K8S_SCYLLA_UTILS_DOCKER_IMAGE",
            type=str,
            help=("Docker image to be used by Scylla operator to tune K8S nodes for performance. Used when k8s_enable_performance_tuning' is defined to 'True'. If not set then the default from operator will be used."),
        ),
        dict(name="k8s_enable_performance_tuning", env="SCT_K8S_ENABLE_PERFORMANCE_TUNING", type=boolean, help="Define whether performance tuning must run or not."),
        ...
        dict(name="k8s_scylla_operator_docker_image", env="SCT_K8S_SCYLLA_OPERATOR_DOCKER_IMAGE", type=str, help="Docker image to be used for installation of scylla operator."),
        dict(name="k8s_scylla_operator_upgrade_docker_image", env="SCT_K8S_SCYLLA_OPERATOR_UPGRADE_DOCKER_IMAGE", type=str, help="Docker image to be used for upgrade of scylla operator."),
        dict(name="k8s_scylla_operator_helm_repo", env="SCT_K8S_SCYLLA_OPERATOR_HELM_REPO", type=str, help="Link to the Helm repository where to get 'scylla-operator' charts from."),
        dict(name="k8s_scylla_operator_upgrade_helm_repo", env="SCT_K8S_SCYLLA_OPERATOR_UPGRADE_HELM_REPO", type=str, help="Link to the Helm repository where to get 'scylla-operator' charts for upgrade."),
        dict(name="k8s_scylla_operator_chart_version", env="SCT_K8S_SCYLLA_OPERATOR_CHART_VERSION", type=str, help=("Version of 'scylla-operator' Helm chart to use. If not set then latest one will be used.")),
        dict(name="k8s_scylla_operator_upgrade_chart_version", env="SCT_K8S_SCYLLA_OPERATOR_UPGRADE_CHART_VERSION", type=str, help="Version of 'scylla-operator' Helm chart to use for upgrade."),
        dict(
            name="k8s_functional_test_dataset",
            env="SCT_K8S_FUNCTIONAL_TEST_DATASET",
            type=str,
            help="Defines whether dataset uses for pre-fill cluster in functional test. Defined in sdcm.utils.sstable.load_inventory. Expected values: BIG_SSTABLE_MULTI_COLUMNS_DATA, MULTI_COLUMNS_DATA",
        ),
        dict(
            ...
            type=str,
            k8s_multitenancy_supported=True,
            help="The CPU limit that will be set for each Scylla cluster deployed in K8S. If not set, then will be autocalculated. Example: '500m' or '2'",
        ),
        dict(
            ...
            type=str,
            k8s_multitenancy_supported=True,
            help="The memory limit that will be set for each Scylla cluster deployed in K8S. If not set, then will be autocalculated. Example: '16384Mi'",
        ),
        dict(name="k8s_scylla_cluster_name", env="SCT_K8S_SCYLLA_CLUSTER_NAME", type=str, help=""),
        dict(name="k8s_n_scylla_pods_per_cluster", env="K8S_N_SCYLLA_PODS_PER_CLUSTER", type=int_or_space_separated_ints, help="Number of loader pods per loader cluster."),
        ...
        dict(name="k8s_n_loader_pods_per_cluster", env="SCT_K8S_N_LOADER_PODS_PER_CLUSTER", type=int_or_space_separated_ints, help="Number of loader pods per loader cluster."),
        dict(
        ...
        dict(name="k8s_instance_type_auxiliary", env="SCT_K8S_INSTANCE_TYPE_AUXILIARY", type=str, help="Instance type for the nodes of the K8S auxiliary/default node pool."),
        dict(name="k8s_instance_type_monitor", env="SCT_K8S_INSTANCE_TYPE_MONITOR", type=str, help="Instance type for the nodes of the K8S monitoring node pool."),
        ...
        dict(name="k8s_log_api_calls", env="SCT_K8S_LOG_API_CALLS", type=boolean, help="Defines whether the K8S API server logging must be enabled and it's logs gathered. Be aware that it may be really huge set of data."),
        dict(name="k8s_tenants_num", env="SCT_TENANTS_NUM", type=int, help="Number of Scylla clusters to create in the K8S cluster."),
        dict(name="k8s_enable_tls", env="SCT_K8S_ENABLE_TLS", type=boolean, help="Defines whether we enable the scylla operator TLS feature or not."),
        dict(name="k8s_enable_sni", env="SCT_K8S_ENABLE_SNI", type=boolean, help="Defines whether we install SNI and use it or not (serverless feature)."),
        dict(name="k8s_enable_alternator", env="SCT_K8S_ENABLE_ALTERNATOR", type=boolean, help="Defines whether we enable the alternator feature using scylla-operator or not."),
        ...
        dict(
            ...
            help="Defines the type of the K8S 'Service' objects type used for ScyllaDB pods. Empty value means 'do not set and allow scylla-operator to choose'.",
        ),
        dict(
            ...
            help="Defines the source of the IP address to be used for the 'broadcast_address' config option in the 'scylla.yaml' files. Empty value means 'do not set and allow scylla-operator to choose'.",
        ),
        dict(
            ...
            type=str,
            choices=("", "ServiceClusterIP", "PodIP", "ServiceLoadBalancerIngress"),
            help="Defines the source of the IP address to be used for the 'broadcast_rpc_address' config option in the 'scylla.yaml' files. Empty value means 'do not set and allow scylla-operator to choose'.",
        ),
        dict(name="k8s_use_chaos_mesh", env="SCT_K8S_USE_CHAOS_MESH", type=boolean, help="""enables chaos-mesh for k8s testing"""),
        dict(name="k8s_n_auxiliary_nodes", env="SCT_K8S_N_AUXILIARY_NODES", type=int, help="Number of nodes in auxiliary pool"),
        dict(name="k8s_n_monitor_nodes", env="SCT_K8S_N_MONITOR_NODES", type=int, help="Number of nodes in monitoring pool that will be used for scylla-operator's deployed monitoring pods."),
        # docker config options
        dict(name="mgmt_docker_image", env="SCT_MGMT_DOCKER_IMAGE", type=str, help="Scylla manager docker image, i.e. 'scylladb/scylla-manager:2.2.1' "),
        dict(name="docker_image", env="SCT_DOCKER_IMAGE", type=str, help="Scylla docker image repo, i.e. 'scylladb/scylla', if omitted is calculated from scylla_version"),
        dict(name="docker_network", env="SCT_DOCKER_NETWORK", type=str, help="local docker network to use, if there's need to have db cluster connect to other services running in docker"),
        dict(name="vector_store_docker_image", env="SCT_VECTOR_STORE_DOCKER_IMAGE", type=str, help="Vector Store docker image repo"),
        dict(name="vector_store_version", env="SCT_VECTOR_STORE_VERSION", type=str, help="Vector Store version / docker image tag"),
        ...
        # LongevityTest
        dict(name="stress_multiplier", env="SCT_STRESS_MULTIPLIER", type=int, help="Number of cassandra-stress processes"),
        dict(name="stress_multiplier_w", env="SCT_STRESS_MULTIPLIER_W", type=int, help="Number of cassandra-stress processes for write workload"),
        dict(name="stress_multiplier_r", env="SCT_STRESS_MULTIPLIER_R", type=int, help="Number of cassandra-stress processes for read workload"),
        dict(name="stress_multiplier_m", env="SCT_STRESS_MULTIPLIER_M", type=int, help="Number of cassandra-stress processes for mixed workload"),
        dict(name="run_fullscan", env="SCT_RUN_FULLSCAN", type=list, help=""),
        dict(name="run_full_partition_scan", env="SCT_run_full_partition_scan", type=str, help="Runs a background thread that issues reversed-queries on a table random partition by an interval"),
        dict(name="run_tombstone_gc_verification", env="SCT_RUN_TOMBSTONE_GC_VERIFICATION", type=str, help="Runs a background thread that verifies Tombstones GC on a table by an interval"),
        ...
        dict(name="pre_create_keyspace", env="SCT_PRE_CREATE_KEYSPACE", type=str_or_list, help="Command to create keysapce to be pre-create before running workload"),
        dict(name="post_prepare_cql_cmds", env="SCT_POST_PREPARE_CQL_CMDS", type=str_or_list, help="CQL Commands to run after prepare stage finished (relevant only to longevity_test.py)"),
        dict(
        ...
        dict(name="compaction_strategy", env="SCT_COMPACTION_STRATEGY", type=str, help="Choose a specific compaction strategy to pre-create schema with."),
        dict(name="sstable_size", env="SSTABLE_SIZE", type=int, help="Configure sstable size for the usage of pre-create-schema mode"),
        dict(name="cluster_health_check", env="SCT_CLUSTER_HEALTH_CHECK", type=boolean, help="When true, start cluster health checker for all nodes"),
        dict(
        ...
        dict(name="user_profile_table_count", env="SCT_USER_PROFILE_TABLE_COUNT", type=int, help="""number of tables to create for template user c-s"""),
        dict(name="add_cs_user_profiles_extra_tables", env="SCT_ADD_CS_USER_PROFILES_EXTRA_TABLES", type=boolean, help="""extra tables to create for template user c-s, in addition to pre-created tables"""),
        # MgmtCliTest
        dict(name="scylla_mgmt_upgrade_to_repo", env="SCT_SCYLLA_MGMT_UPGRADE_TO_REPO", type=str, help="Url to the repo of scylla manager version to upgrade to for management tests"),
        dict(
            name="mgmt_restore_extra_params",
            env="SCT_MGMT_RESTORE_EXTRA_PARAMS",
            type=str,
            help="Manager restore operation extra parameters: batch-size, parallel, etc.For example, `--batch-size 2 --parallel 1`. Provided string appends the restore cmd",
        ),
        dict(
            name="mgmt_agent_backup_config",
            env="SCT_MGMT_AGENT_BACKUP_CONFIG",
            type=dict_or_str_or_pydantic,
            help="Manager agent backup general configuration: checkers, transfers, low_level_retries. For example, {'checkers': 100, 'transfers': 2, 'low_level_retries': 20}",
        ),
        dict(
            name="mgmt_reuse_backup_snapshot_name",
            env="SCT_MGMT_REUSE_BACKUP_SNAPSHOT_NAME",
            type=str,
            help="Name of backup snapshot to use in Manager restore benchmark test, for example, 500gb_2t_ics. The name provides the info about dataset size (500gb), tables number (2) and compaction (ICS)",
        ),
        dict(name="mgmt_skip_post_restore_stress_read", env="SCT_MGMT_SKIP_POST_RESTORE_STRESS_READ", type=boolean, help="Skip post-restore c-s verification read in the Manager restore benchmark tests"),
        dict(name="mgmt_nodetool_refresh_flags", env="SCT_MGMT_NODETOOL_REFRESH_FLAGS", type=str, help="Nodetool refresh extra options like --load-and-stream or --primary-replica-only"),
        dict(name="mgmt_prepare_snapshot_size", env="SCT_MGMT_PREPARE_SNAPSHOT_SIZE", type=int, help="Size of backup snapshot in Gb to be prepared for backup"),
        dict(name="mgmt_snapshots_preparer_params", env="SCT_MGMT_SNAPSHOTS_PREPARER_PARAMS", type=dict_or_str, help="Custom parameters of c-s write operation used in snapshots preparer"),
        dict(name="one_one_restore_cluster_bootstrap_duration", env="SCT_ONE_ONE_RESTORE_CLUSTER_BOOTSTRAP_DURATION", type=int, help="Time in seconds it took Siren to bootstrap 1-1-restore cluster"),
        ...
        dict(name="perf_extra_jobs_to_compare", env="SCT_PERF_EXTRA_JOBS_TO_COMPARE", type=str_or_list_or_eval, help="jobs to compare performance results with, for example if running in staging, we still can compare with official jobs"),
        dict(name="perf_simple_query_extra_command", env="SCT_PERF_SIMPLE_QUERY_EXTRA_COMMAND", type=str, help="extra command line options to pass to perf_simple_query"),
        # PerformanceRegressionUserProfilesTest
        dict(name="cs_user_profiles", env="SCT_CS_USER_PROFILES", type=str_or_list, help="cassandra-stress user-profiles list. Executed in test step"),
        dict(name="prepare_cs_user_profiles", env="SCT_PREPARE_CS_USER_PROFILES", type=str_or_list, help="cassandra-stress user-profiles list. Executed in prepare step"),
        ...
        dict(name="perf_gradual_threads", env="SCT_PERF_GRADUAL_THREADS", type=dict_or_str, help="Threads amount of stress load for gradual performance test per sub-test. Example: {'read': 100, 'write': [200, 300], 'mixed': 300}"),
        dict(
        ...
        dict(name="perf_gradual_step_duration", env="SCT_PERF_GRADUAL_STEP_DURATION", type=dict_or_str, help="Step duration of c-s load for gradual performance test per sub-test. Example: {'read': '30m', 'write': None, 'mixed': '30m'}"),
        # RefreshTest
        ...
        dict(name="new_version", env="SCT_NEW_VERSION", type=str, help="Assign new upgrade version, use it to upgrade to specific minor release. eg: 3.0.1"),
        dict(
            name="target_upgrade_version",
            env="SCT_TARGET_UPGRADE_VERSION",
            type=str,
            help="Assign target upgrade version, use for decide if the truncate entries test should be run. This test should be performed in case the target upgrade version >= 3.1",
        ),
        dict(
            name="disable_raft",
            env="SCT_DISABLE_RAFT",
            type=boolean,
            help="As for now, raft will be enable by default in all [upgrade] tests, so this flag will allow usto still run [upgrade] test without raft enabled (or disabling raft), so we will have bettercoverage",
        ),
        dict(
            name="enable_tablets_on_upgrade",
            env="SCT_ENABLE_TABLETS_ON_UPGRADE",
            type=boolean,
            help="By default, the tablets feature is disabled. With this parameter, created for the upgrade test,the tablets feature will only be enabled after the upgrade",
        ),
        dict(
        ...
        dict(name="upgrade_node_system", env="SCT_UPGRADE_NODE_SYSTEM", type=boolean, help="Upgrade system packages on nodes before upgrading Scylla. Enabled by default"),
        dict(
            name="stress_cmd_1",
        ...
        dict(name="scylla_encryption_options", env="SCT_SCYLLA_ENCRYPTION_OPTIONS", type=str_or_list, help="options will be used for enable encryption at-rest for tables"),
        dict(
            name="kms_key_rotation_interval",
            env="SCT_KMS_KEY_ROTATION_INTERVAL",
            type=int,
            help="The time interval in minutes which gets waited before the KMS key rotation happens. Applied when AWS KMS or Azure KMS service is configured to be used. NOTE: Be aware that Azure Key rotations cost $1/rotation.",
        ),
        ...
        dict(
            ...
            type=boolean,
            help="Allows to disable KMS keys rotation. Applicable to GCP and Azure backends. In case of AWS backend its KMS keys will always be rotated as of now.",
        ),
        dict(name="enterprise_disable_kms", env="SCT_ENTERPRISE_DISABLE_KMS", type=boolean, help="An escape hatch to disable KMS for enterprise run, when needed, we enable kms by default since if we use scylla 2023.1.3 and up"),
        dict(name="logs_transport", env="SCT_LOGS_TRANSPORT", type=str, help="How to transport logs: syslog-ng, ssh or docker", choices=("ssh", "docker", "syslog-ng", "vector")),
        dict(name="collect_logs", env="SCT_COLLECT_LOGS", type=boolean, help="Collect logs from instances and sct runner"),
        dict(name="execute_post_behavior", env="SCT_EXECUTE_POST_BEHAVIOR", type=boolean, help="Run post behavior actions in sct teardown step"),
        dict(
        ...
        dict(name="internode_compression", env="SCT_INTERNODE_COMPRESSION", type=str, help="scylla option: internode_compression"),
        dict(name="internode_encryption", env="SCT_INTERNODE_ENCRYPTION", type=str, help="scylla sub option of server_encryption_options: internode_encryption"),
        dict(name="jmx_heap_memory", env="SCT_JMX_HEAP_MEMORY", type=int, help="The total size of the memory allocated to JMX. Values in MB, so for 1GB enter 1024(MB)"),
        dict(
        ...
        dict(name="append_scylla_setup_args", env="SCT_APPEND_SCYLLA_SETUP_ARGS", type=str, help="More arguments to append to scylla_setup command line"),
        dict(name="use_preinstalled_scylla", env="SCT_USE_PREINSTALLED_SCYLLA", type=boolean, help="Don't install/update ScyllaDB on DB nodes"),
        dict(name="force_run_iotune", env="SCT_FORCE_RUN_IOTUNE", type=boolean, help="Force running iotune on the DB nodes, regdless if image has predefined values"),
        dict(
        ...
        dict(name="store_cdclog_reader_stats_in_es", env="SCT_STORE_CDCLOG_READER_STATS_IN_ES", type=boolean, help="""Add cdclog reader stats to ES for future performance result calculating"""),
        dict(
        ...
        dict(name="stress_cdc_log_reader_batching_enable", env="SCT_STRESS_CDC_LOG_READER_BATCHING_ENABLE", type=boolean, help="""retrieving data from multiple streams in one poll"""),
        dict(name="use_legacy_cluster_init", env="SCT_USE_LEGACY_CLUSTER_INIT", type=boolean, help="""Use legacy cluster initialization with autobootsrap disabled and parallel node setup"""),
        dict(
        ...
        dict(name="num_nodes_to_rollback", env="SCT_NUM_NODES_TO_ROLLBACK", type=str, help="Number of nodes to upgrade and rollback in test_generic_cluster_upgrade"),
        dict(name="upgrade_sstables", env="SCT_UPGRADE_SSTABLES", type=boolean, help="Whether to upgrade sstables as part of upgrade_node or not"),
        dict(name="enable_truncate_checks_on_node_upgrade", env="SCT_ENABLE_TRUNCATE_CHECKS_ON_NODE_UPGRADE", type=boolean, help="Enables or disables truncate checks on each node upgrade and rollback"),
        dict(name="stress_before_upgrade", env="SCT_STRESS_BEFORE_UPGRADE", type=str, help="Stress command to be run before upgrade (preapre stage)"),
        dict(name="stress_during_entire_upgrade", env="SCT_STRESS_DURING_ENTIRE_UPGRADE", type=str, help="Stress command to be run during the upgrade - user should take care for suitable duration"),
        dict(name="stress_after_cluster_upgrade", env="SCT_STRESS_AFTER_CLUSTER_UPGRADE", type=str, help="Stress command to be run after full upgrade - usually used to read the dataset for verification"),
        # Jepsen test.
        dict(name="jepsen_scylla_repo", env="SCT_JEPSEN_SCYLLA_REPO", type=str, help="Link to the git repository with Jepsen Scylla tests"),
        dict(name="jepsen_test_cmd", env="SCT_JEPSEN_TEST_CMD", type=str_or_list, help="Jepsen test command (e.g., 'test-all')"),
        dict(name="jepsen_test_count", env="SCT_JEPSEN_TEST_COUNT", type=int, help="possible number of reruns of single Jepsen test command"),
        dict(
        ...
        dict(name="max_events_severities", env="SCT_MAX_EVENTS_SEVERITIES", type=str_or_list, help="Limit severity level for event types"),
        dict(name="scylla_rsyslog_setup", env="SCT_SCYLLA_RSYSLOG_SETUP", type=boolean, help="Configure rsyslog on Scylla nodes to send logs to monitoring nodes"),
        dict(name="events_limit_in_email", env="SCT_EVENTS_LIMIT_IN_EMAIL", type=int, help="Limit number events in email reports"),
        dict(
        ...
        dict(name="data_volume_disk_type", env="SCT_DATA_VOLUME_DISK_TYPE", type=str, help="Type of addtitional volumes: gp2|gp3|io2|io3"),
        dict(name="data_volume_disk_size", env="SCT_DATA_VOLUME_DISK_SIZE", type=int, help="Size of additional volume in GB"),
        dict(name="data_volume_disk_iops", env="SCT_DATA_VOLUME_DISK_IOPS", type=int, help="Number of iops for ebs type io2|io3|gp3"),
        dict(name="data_volume_disk_throughput", env="SCT_DATA_VOLUME_DISK_THROUGHPUT", type=int, help="Throughput in MiB/sec for ebs type gp3. Min is 125. Max is 1000."),
        dict(name="run_db_node_benchmarks", env="SCT_RUN_DB_NODE_BENCHMARKS", type=boolean, help="Flag for running db node benchmarks before the tests"),
        dict(
        ...
        dict(name="nemesis_multiply_factor", env="SCT_NEMESIS_MULTIPLY_FACTOR", type=int, k8s_multitenancy_supported=True, help="Multiply the list of nemesis to execute by the specified factor"),
        dict(
        ...
        dict(name="bare_loaders", env="SCT_BARE_LOADERS", type=boolean, help="Don't install anything but node_exporter to the loaders during cluster setup"),
        dict(name="stress_image", env="SCT_STRESS_IMAGE", type=dict_or_str, help="Dict of the images to use for the stress tools"),
        dict(
        ...
        dict(name="latte_schema_parameters", env="SCT_LATTE_SCHEMA_PARAMETERS", type=dict, help="""Optional. Allows to pass through custom rune script parameters to the 'latte schema' command."""),
        dict(name="num_loaders_step", env="SCT_NUM_LOADERS_STEP", type=int, help="Number of loaders which should be added per step"),
        dict(name="stress_threads_start_num", env="SCT_STRESS_THREADS_START_NUM", type=int, help="Number of threads for c-s command"),
        dict(name="num_threads_step", env="SCT_NUM_THREADS_STEP", type=int, help="Number of threads which should be added on per step"),
        dict(name="stress_step_duration", env="SCT_STRESS_STEP_DURATION", type=str, help="Duration of time for stress round"),
        dict(
            name="max_deviation",
        ...
        dict(name="n_stress_process", env="SCT_N_STRESS_PROCESS", type=int, help="""Number of stress processes per loader"""),
        dict(name="stress_process_step", env="SCT_STRESS_PROCESS_STEP", type=int, help="""add/remove num of process on each round"""),
        dict(name="use_hdrhistogram", env="SCT_USE_HDRHISTOGRAM", type=boolean, help="""Enable hdr histogram logging for cs"""),
        dict(
        ...
        dict(name="custom_es_index", env="SCT_CUSTOM_ES_INDEX", type=str, help="""Use custom ES index for storing test results"""),
        dict(
        ...
        dict(name="rack_aware_loader", env="SCT_RACK_AWARE_LOADER", type=boolean, help="When enabled, loaders will look for nodes on the same rack."),
        dict(
        ...
        dict(name="use_dns_names", env="SCT_USE_DNS_NAMES", type=boolean, help="""Use dns names instead of ip addresses for nodes in cluster"""),
        dict(name="validate_large_collections", env="SCT_VALIDATE_LARGE_COLLECTIONS", type=boolean, help="Enable validation for large cells in system table and logs"),
        dict(name="run_commit_log_check_thread", env="SCT_RUN_COMMIT_LOG_CHECK_THREAD", type=boolean, help="""Run commit log check thread if commitlog_use_hard_size_limit is True"""),
        dict(name="teardown_validators", env="SCT_TEARDOWN_VALIDATORS", type=dict_or_str, help="""Configuration for additional validations executed after the test"""),
        dict(
        ...
        dict(name="use_dedicated_host", env="SCT_USE_DEDICATED_HOST", type=boolean, help="""Allocates dedicated hosts for the instances for the entire duration of the test run (AWS only)"""),
        dict(name="aws_dedicated_host_ids", env="SCT_AWS_DEDICATED_HOST_IDS", type=str_or_list_or_eval, help="""list of host ids to use, relevant only if `use_dedicated_host: true` (AWS only)"""),
        dict(
        ...
        dict(name="bisect_end_date", env="SCT_BISECT_END_DATE", type=str, help="""Scylla build date until which bisecting should run. Format: YYYY-MM-DD"""),
        dict(name="kafka_backend", env="SCT_KAFKA_BACKEND", type=str, help="Enable validation for large cells in system table and logs", choices=(None, "localstack", "vm", "msk")),
        dict(name="kafka_connectors", env="SCT_KAFKA_CONNECTORS", type=str_or_list_or_eval, help="configuration for setup up kafka connectors"),
        dict(name="run_scylla_doctor", env="SCT_RUN_SCYLLA_DOCTOR", type=boolean, help="Run scylla-doctor in artifact tests"),
        dict(name="skip_test_stages", env="SCT_SKIP_TEST_STAGES", type=dict_or_str, help="""Skip selected stages of a test scenario"""),
        dict(name="use_zero_nodes", env="SCT_USE_ZERO_NODES", type=boolean, help="If True, enable support in sct of zero nodes(configuration, nemesis)"),
        dict(
        ...
        dict(name="zero_token_instance_type_db", env="SCT_ZERO_TOKEN_INSTANCE_TYPE_DB", type=str, help="""Instance type for zero token node"""),
        dict(name="sct_aws_account_id", env="SCT_AWS_ACCOUNT_ID", type=str, help="AWS account id on behalf of which the test is run"),
        dict(
            name="latency_decorator_error_thresholds",
            env="SCT_LATENCY_DECORATOR_ERROR_THRESHOLDS",
            type=dict_or_str,
            help="Error thresholds for latency decorator. Defined by dict: {<write, read, mixed>: {<default|nemesis_name>:{<metric_name>: {<rule>: <value>}}}",
        ),
        dict(
            name="workload_name",
            env="SCT_WORKLOAD_NAME",
            type=str,
            help="Workload name, can be: write|read|mixed|unset.Used for e.g. latency_calculator_decorator (use with 'use_hdrhistogram' set to true).If unset, workload is taken from test name.",
        ),
        dict(name="adaptive_timeout_store_metrics", env="SCT_ADAPTIVE_TIMEOUT_STORE_METRICS", type=boolean, help="Store adaptive timeout metrics in Argus. Disabled for performance tests only."),
        dict(name="xcloud_credentials_path", env="SCT_XCLOUD_CREDENTIALS_PATH", type=str, help="Path to Scylla Cloud credentials file, if stored locally"),
        dict(name="xcloud_env", env="SCT_XCLOUD_ENV", type=str, help="Scylla Cloud environment (e.g., lab)."),
        dict(name="xcloud_provider", env="SCT_XCLOUD_PROVIDER", type=str, help="Cloud provider for Scylla Cloud deployment (aws, gce)"),
        dict(name="xcloud_replication_factor", env="SCT_XCLOUD_REPLICATION_FACTOR", type=int, help="Replication factor for Scylla Cloud cluster"),
        dict(
        ...
        dict(name="n_vector_store_nodes", env="SCT_N_VECTOR_STORE_NODES", type=int, help="Number of vector store nodes (0 = VS is disabled)"),
        dict(name="vector_store_port", env="SCT_VECTOR_STORE_PORT", type=int, help="Vector Store API port"),
        dict(name="vector_store_scylla_port", env="SCT_VECTOR_STORE_SCYLLA_PORT", type=int, help="ScyllaDB connection port for Vector Store"),
        dict(name="vector_store_threads", env="SCT_VECTOR_STORE_THREADS", type=int, help="Vector Store indexing threads (if not set, defaults to number of CPU cores on VS node)"),
        dict(name="download_from_s3", env="SCT_DOWNLOAD_FROM_S3", type=list, help="Destination-source map of dirs/buckets to download from S3 before starting the test"),
    ]

    required_params = ["cluster_backend", "test_duration", "n_db_nodes", "n_loaders", "use_preinstalled_scylla", "user_credentials_path", "root_disk_size_db", "root_disk_size_monitor", "root_disk_size_loader"]
    ...
    backend_required_params = {
        ...
        "aws-siren": ["user_prefix", "instance_type_loader", "region_name", "cloud_credentials_path", "nemesis_filter_seeds"],
        "gce-siren": [
            "user_prefix",
            "gce_network",
            "gce_image_username",
        ...
        "k8s-local-kind": ["user_credentials_path", "scylla_version", "scylla_mgmt_agent_version", "k8s_scylla_operator_helm_repo", "k8s_scylla_cluster_name", "k8s_scylla_disk_gi", "mini_k8s_version", "mgmt_docker_image"],
        "k8s-local-kind-aws": ["user_credentials_path", "scylla_version", "scylla_mgmt_agent_version", "k8s_scylla_operator_helm_repo", "k8s_scylla_cluster_name", "k8s_scylla_disk_gi", "mini_k8s_version", "mgmt_docker_image"],
        "k8s-local-kind-gce": ["user_credentials_path", "scylla_version", "scylla_mgmt_agent_version", "k8s_scylla_operator_helm_repo", "k8s_scylla_cluster_name", "k8s_scylla_disk_gi", "mini_k8s_version", "mgmt_docker_image"],
        "k8s-gke": [
            "gke_cluster_version",
            "gce_instance_type_db",
            "gce_root_disk_type_db",
    ...
    defaults_config_files = {
        ...
        "k8s-local-kind-aws": [sct_abs_path("defaults/aws_config.yaml"), sct_abs_path("defaults/k8s_local_kind_aws_config.yaml"), sct_abs_path("defaults/k8s_local_kind_config.yaml")],
        "k8s-local-kind-gce": [sct_abs_path("defaults/k8s_local_kind_gce_config.yaml"), sct_abs_path("defaults/k8s_local_kind_config.yaml")],
    ...
    per_provider_multi_region_params = {"aws": ["region_name", "n_db_nodes", "ami_id_db_scylla", "ami_id_loader"], "gce": ["gce_datacenter", "n_db_nodes"]}
    ...
    ami_id_params = ["ami_id_db_scylla", "ami_id_loader", "ami_id_monitor", "ami_id_db_cassandra", "ami_id_db_oracle", "ami_id_vector_store"]
    aws_supported_regions = ["eu-west-1", "eu-west-2", "us-west-2", "us-east-1", "eu-north-1", "eu-central-1", "eu-west-3", "ca-central-1"]
    ...
    def __init__(self):  # noqa: PLR0912, PLR0914, PLR0915
        ...
        if scylla_version := self.get("scylla_version"):
            ...
            if self.get("cluster_backend") in ("docker", "k8s-eks", "k8s-gke", "k8s-local-kind", "k8s-local-kind-aws", "k8s-local-kind-gce"):
                ...
                for region in region_names:
                    ...
                    try:
                        ...
                        raise ValueError(f"AMIs for scylla_version='{scylla_version}' not found in {region} arch={aws_arch}") from ex
                    self.log.debug("Found AMI %s(%s) for scylla_version='%s' in %s", ami.name, ami.image_id, scylla_version, region)
                    ami_list.append(ami)
                ...
                for region in azure_region_names:
                    try:
                        if ":" in scylla_version:
                            azure_image = azure_utils.get_scylla_images(scylla_version=scylla_version, region_name=region)[0]
                        else:
                            azure_image = azure_utils.get_released_scylla_images(scylla_version=scylla_version, region_name=region)[0]
                    except Exception as ex:  # noqa: BLE001
                        raise ValueError(f"Azure Image for scylla_version='{scylla_version}' not found in {region}") from ex
                    self.log.debug("Found Azure Image %s for scylla_version='%s' in %s", azure_image.name, scylla_version, region)
                    scylla_azure_images.append(azure_image)
                self["azure_image_db"] = " ".join(getattr(image, "id", None) or getattr(image, "unique_id", None) for image in scylla_azure_images)
                ...
                raise ValueError("'scylla_version' can't used together with 'ami_id_db_scylla', 'gce_image_db' or with 'scylla_repo'")
            ...
            if not self.get("ami_id_db_oracle") and self.get("cluster_backend") == "aws":
                ...
                for region in region_names:
                    ...
                    try:
                        ...
                        raise ValueError(f"AMIs for oracle_scylla_version='{oracle_scylla_version}' not found in {region} arch={aws_arch}") from ex

                    self.log.debug("Found AMI %s for oracle_scylla_version='%s' in %s", ami.image_id, oracle_scylla_version, region)
                    ami_list.append(ami)
            ...
            if self.get("cluster_backend") == "aws":
                ...
                for region in region_names:
                    ...
                    try:
                        ...
                        raise ValueError(f"AMIs for vs_version='{vs_version}' not found in {region} arch={aws_arch}") from ex
                    self.log.debug("Found AMI %s(%s) for vs_version='%s' in %s", ami.name, ami.image_id, vs_version, region)
                    ami_list.append(ami)
        ...
        if self.get("authenticator") and self.get("authenticator") == "PasswordAuthenticator":
            ...
            if not (authenticator_password and authenticator_user):
                raise ValueError("For PasswordAuthenticator authenticator authenticator_user and authenticator_password have to be provided")

        if self.get("alternator_enforce_authorization"):
            if not self.get("authenticator") or not self.get("authorizer"):
                raise ValueError("When enabling `alternator_enforce_authorization` both `authenticator` and `authorizer` should be defined")
        ...
        if run_fullscan_params := self.get("run_fullscan"):
            ...
            for param in run_fullscan_params:
                try:
                    ...
                    raise ValueError(f"each item of run_fullscan list: {run_fullscan_params}, item {param}, must be JSON but got error: {repr(exp)}") from exp
        ...
        if (self.get("simulated_regions") or 0) > 1 or (self.get("simulated_racks") or 0) > 1 and num_of_db_nodes > 1 and cluster_backend != "docker":
            if snitch := self.get("endpoint_snitch"):
                assert snitch.endswith("GossipingPropertyFileSnitch"), f"Simulating racks requires endpoint_snitch to be GossipingPropertyFileSnitch while it set to {self['endpoint_snitch']}"
        ...
        if scylla_network_config := self.get("scylla_network_config"):
            check_list = {"listen_address": None, "rpc_address": None, "broadcast_rpc_address": None, "broadcast_address": None, "test_communication": None}
            ...
            for i, address_config in enumerate(scylla_network_config):
                for param in ["address", "ip_type", "public", "nic"]:
                    if address_config.get(param) is None:
                        raise ValueError(f"'{param}' parameter value for {number2word[i + 1]} address is not defined. It is must parameter")

                if address_config["ip_type"] == "ipv4" and address_config["nic"] == 1 and address_config["public"] is True:
                    raise ValueError("If ipv4 and public is True it has to be primary network interface, it means device index (nic) is 0")
        ...
        if self.get("use_zero_nodes"):
            ...
            if zero_nodes_num:
                zero_nodes_num = [zero_nodes_num] if isinstance(zero_nodes_num, int) else [int(i) for i in str(zero_nodes_num).split()]
                data_nodes_num = [data_nodes_num] if isinstance(data_nodes_num, int) else [int(i) for i in str(data_nodes_num).split()]
                assert len(zero_nodes_num) == len(data_nodes_num), "Config of zero token nodes is not equal config of data nodes for multi dc"
        ...
        if performance_throughput_params := self.get("perf_gradual_throttle_steps"):
            for workload, params in performance_throughput_params.items():
                ...
                if workload not in gradual_threads:
                    raise ValueError(f"Gradual threads for '{workload}' test is not defined in 'perf_gradual_threads' parameter")
                ...
                for thread_count in gradual_threads[workload]:
                    if not isinstance(thread_count, int):
                        raise ValueError(f"Invalid thread count type for '{workload}': {thread_count} (type: {type(thread_count).__name__})")
                ...
                if len(gradual_threads[workload]) > 1 and len(gradual_threads[workload]) != len(params):
                    raise ValueError(f"perf_gradual_threads for {workload} should be a single-element, integer or list, or a list with the same length as perf_gradual_throttle_steps for {workload}")
    ...
    def _validate_value(self, opt):
        ...
        try:
            ...
            if not (self.get("cluster_backend").startswith("k8s") and self.get("k8s_tenants_num") > 1 and opt.get("k8s_multitenancy_supported") and isinstance(self.get(opt["name"]), list)):
                if not (opt["name"] == "nemesis_selector" and isinstance(self.get("nemesis_class_name"), str) and len(self.get("nemesis_class_name").split(" ")) > 1):
        ...
        if choices:
            ...
            for cur_val_element in cur_val:
                assert cur_val_element in choices, "failed to validate '{}': {} not in {}".format(opt["name"], cur_val_element, choices)
                    ...
                    for option in cmd.split():
                        if option.startswith("profile="):
                            ...
                            if not profile_path.startswith("/tmp"):
                                raise ValueError(f"Stress command parameter '{param_name}' contains wrong path '{profile_path}' to profile, it should be formed in following manner '/tmp/{{file_name_from_data_dir}}'")
                            ...
                            raise ValueError(f"Stress command parameter '{param_name}' contains profile '{profile_path}' that does not exists under data_dir/")
    ...
    def verify_configuration(self):
        ...
        if (teardown_validators := self.get("teardown_validators.rackaware")) and teardown_validators.get("enabled", False):
    ...
    def _replace_docker_image_latest_tag(self):
        ...
        if scylla_version == "latest":
            ...
            if result == "latest":
                raise ValueError("scylla-operator expects semver-like tags for Scylla docker images. 'latest' should not be used.")
    ...
    def _resolve_xcloud_version_tag(self, version_tag: str) -> None:
        ...
        if tag_type == "release":
            if tag_value == "latest":
                cloud_api_client = ScyllaCloudAPIClient(api_url=self.cloud_env_credentials["base_url"], auth_token=self.cloud_env_credentials["api_token"], raise_for_status=True)
    ...
    def _validate_seeds_number(self):
        ...
        assert not num_of_db_nodes or seeds_num <= num_of_db_nodes, f"Seeds number ({seeds_num}) should be not more then nodes number ({num_of_db_nodes})"
    ...
    def _validate_nemesis_can_run_on_non_seed(self) -> None:
        ...
        assert num_of_db_nodes > seeds_num, "Nemesis cannot run when 'nemesis_filter_seeds' is true and seeds number is equal to nodes number"
    ...
    def _validate_number_of_db_nodes_divides_by_az_number(self):
        ...
        for nodes_num in [int(i) for i in str(self.get("n_db_nodes")).split(" ")]:
            assert nodes_num % az_count == 0, f"Number of db nodes ({nodes_num}) should be divisible by number of availability zones ({az_count})"

    def _validate_placement_group_required_values(self):
        if self.get("use_placement_group"):
            az_count = len(self.get("availability_zone").split(",")) if self.get("availability_zone") else 1
            regions_count = len(self.region_names)
            assert az_count == 1 and regions_count == 1, f"Number of Regions({regions_count}) and AZ({az_count}) should be 1 when param use_placement_group is used"
    ...
    def _check_per_backend_required_values(self, backend: str):
        if backend in self.available_backends:
            if backend in ("aws", "gce") and self.get("db_type") == "cloud_scylla":
                backend += "-siren"
            if backend == "xcloud":
                self.backend_required_params[backend] += self.xcloud_per_provider_required_params[self.get("xcloud_provider")]
            if backend == "aws" and self.get("n_vector_store_nodes") > 0:
                self.backend_required_params["aws"].extend(["ami_id_vector_store", "instance_type_vector_store", "ami_vector_store_user"])
            ...
            match backend:
                case "aws":
                    for region in self.region_names:
                        assert aws_check_instance_type_supported(instance_type, region), f"Instance type[{instance_type}] not supported in region [{region}]"
                case "gce":
                    ...
                    for datacenter in self.gce_datacenters:
                        for zone in GCE_SUPPORTED_REGIONS.get(datacenter):
                            ...
                            assert gce_check_if_machine_type_supported(machine_types_client, instance_type, project=info["project_id"], zone=_zone), f"Instance type[{instance_type}] not supported in zone [{_zone}]"
                case "azure":
                    if azure_region_names := self.get("azure_region_name"):
                        ...
                        for region in self.get("azure_region_name"):
                            assert azure_check_instance_type_available(instance_type, region), f"Instance type [{instance_type}] not supported in region [{region}]"
                case _:
    ...
    def _check_version_supplied(self, backend: str):
        ...
        assert all(self.get(o) for o in options_must_exist), (
            f"scylla version/repos wasn't configured correctly\nconfigure those options: {options_must_exist}\nand those environment variables: {['SCT_' + o.upper() for o in options_must_exist]}"
        )
    ...
    def _check_partition_range_with_data_validation_correctness(self):
        ...
        if partition_range_with_data_validation:
            error_message_template = "Expected format of 'partition_range_with_data_validation' parameter is: <min PK value>-<max PK value>. {}Example: 0-250. Got value: %s" % partition_range_with_data_validation
    ...
    def verify_configuration_urls_validity(self):
        ...
        if backend == "aws":
            ...
            for key_to_update, ami_list in [("user_data_format_version", ami_id_db_scylla), ("oracle_user_data_format_version", ami_id_db_oracle)]:
                if ami_list:
                    ...
                    for ami_id, region_name in zip(ami_list, region_names):
                        ...
                        assert "user_data_format_version" in tags.keys(), f"\n\t'user_data_format_version' tag missing from [{ami_id}] on {region_name}\n\texisting tags: {tags}"
                        user_data_format_versions.add(tags["user_data_format_version"])
                    assert len(user_data_format_versions) <= 1, f"shouldn't have mixed versions {user_data_format_versions}"
    ...
    def update_argus_with_version(self, scylla_version: str, package_name: str):
        try:
            ...
            if match := version_regex.match(scylla_version):
                ...
                package = Package(name=package_name, date=version_info.get("date", "#NO_DATE"), version=version_info["short"], revision_id=version_info.get("commit", "#NO_COMMIT"), build_id="#NO_BUILDID")
    ...
    def dict(self):
        ...
        if kafka_connectors := self.get("kafka_connectors"):
            out["kafka_connectors"] = [connector.dict(by_alias=True, exclude_none=True) for connector in kafka_connectors]
    ...
    def _verify_rackaware_configuration(self):
        ...
        racks_count = simulated_racks if (simulated_racks := self.get("simulated_racks")) else len(availability_zone.split(",")) if availability_zone else 1
        if racks_count == 1 and regions == 1:
            raise ValueError("Rack-aware validation can only be performed in multi-availability zone or multi-region environments.")

        loaders = sum(int(l) for l in n_loaders.split(" ")) if isinstance((n_loaders := self.get("n_loaders")), str) else n_loaders
    ...
    def _validate_cloud_backend_parameters(self):
        ...
        supported_instances = [i["externalId"] for i in cloud_api_client.get_instance_types(cloud_provider_id=provider_id, region_id=region_id)["instances"]]
        ...
        if db_instance_type not in supported_instances:
            raise ValueError(f"Database instance type '{db_instance_type}' is not supported in region '{region_name}' for cloud provider '{cloud_provider}'.\nSupported instance types: {', '.join(supported_instances)}")
        ...
        if int(self.get("n_vector_store_nodes")) > 0:
            ...
            if version.parse(scylla_version) < version.parse(MIN_SCYLLA_VERSION_FOR_VS):
                raise ValueError(f"Vector Search requires ScyllaDB {MIN_SCYLLA_VERSION_FOR_VS}+, but selected version is {scylla_version}")
            ...
            if vs_instance_type not in supported_types.keys():
                raise ValueError(f"Instance type '{vs_instance_type}' is not supported for Vector Search on {cloud_provider.upper()}.\nSupported types: {', '.join(supported_types.keys())}")
