...
from .exceptions import AuthenticationException, UnknownHostException, ConnectError, PKeyFileError, UnexpectedExit, CommandTimedOut, FailedToReadCommandOutput, ConnectTimeout, FailedToRunCommand, OpenChannelTimeout
...
class SSHReaderThread(Thread):
    ...
    def run(self):
        try:
            self._read_output(self._session, self._channel, self._timeout, self._timeout_read_data, self.stdout, self.stderr)
        except Exception as exc:  # noqa: BLE001
            self.raised = exc

    def _read_output(self, session: Session, channel: Channel, timeout: NullableTiming, timeout_read_data: NullableTiming, stdout_stream: Queue, stderr_stream: Queue):
...
class KeepAliveThread(Thread):
    ...
    def run(self):
        while self._session and self._keep_running.is_set():
            try:
                time_to_wait = self._session.eagain(self._session.keepalive_send, timeout=self._keepalive_timeout)
...
class Client:
    ...
    def __init__(
        ...
        port: int = None,
        pkey: str = None,
        allow_agent: bool = None,
        forward_ssh_agent: bool = None,
        proxy_host: str = None,
        keepalive_seconds: int = None,
    ...
    def __reduce__(self):
        return self.__class__, (self.host, self.user, self.password, self.port, self.pkey, self.allow_agent, self.forward_ssh_agent, self.proxy_host, self.keepalive_seconds, self.timings, self.flood_preventing)
    ...
    def _init_tune(self):
        ...
        if self.timings.keepalive_timeout:
            ...
            self.keepalive_thread = KeepAliveThread(self.session, keepalive_timeout=self.timings.keepalive_sending_timeout)
    ...
    def _pkey_auth(self):
        self.session.eagain(self.session.userauth_publickey_fromfile, args=(self.user, self._validate_pkey_path(self.pkey, self.host), self.password if self.password is not None else ""))
    ...
    def _validate_pkey_path(pkey: str, host: str = None) -> Optional[str]:
        ...
        if not exists(pkey):
            msg = "File %s does not exist. Please use either absolute or relative to user directory paths like '~/.ssh/my_key' for pkey parameter"
    ...
    def _password_auth(self):
        try:
            self.session.eagain(self.session.userauth_password, args=(self.user, self.password), timeout=self.timings.auth_timeout)
    ...
    @staticmethod
    def _process_output(watchers: List[StreamWatcher], encoding: str, stdout_stream: StringIO, stderr_stream: StringIO, reader: SSHReaderThread, timeout: NullableTiming, timeout_read_data_chunk: NullableTiming):
    ...
    @staticmethod
    def _process_output_no_watchers(session: Session, channel: Channel, encoding: str, stdout_stream: StringIO, stderr_stream: StringIO, timeout: NullableTiming, timeout_read_data_chunk: NullableTiming) -> bool:
    ...
    def run(self, command: str, warn: bool = False, encoding: str = "utf-8", hide=True, watchers=None, env=None, replace_env=False, in_stream=False, timeout=None) -> Result:
        ...
        result = Result(command=command, encoding=encoding, env=env, hide=("stderr", "stdout") if hide else (), pty=False, exited=None, shell=shell, stdout="", stderr="")
        ...
        try:
            ...
            return self._complete_run(channel, FailedToRunCommand(result, exc), timeout_reached, timeout, result, warn, stdout, stderr)
        try:
        ...
        if watchers:
            ...
            try:
                ...
                self._process_output(watchers, encoding, stdout, stderr, reader, timeout, self.timings.interactive_read_data_chunk_timeout)
            except Exception as exc:  # noqa: BLE001
            ...
            try:
                self.execute(command, channel=channel, use_pty=False)
                timeout_reached = not self._process_output_no_watchers(self.session, channel, encoding, stdout, stderr, timeout, self.timings.read_data_chunk_timeout)
            except Exception as exc:  # noqa: BLE001
                exception = FailedToReadCommandOutput(result, exc)
    ...
    def _complete_run(self, channel: Channel, exception: Exception, timeout_reached: NullableTiming, timeout: NullableTiming, result: Result, warn, stdout: StringIO, stderr: StringIO) -> Result:
    ...
    def open_channel(self) -> Channel:
        ...
        if self.forward_ssh_agent and not self.forward_requested:
            if not hasattr(chan, "request_auth_agent"):
                warn("Requested SSH Agent forwarding but libssh2 version used does not support it - ignoring")
                return chan
