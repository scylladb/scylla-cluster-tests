...
def run_long_running_cmd(remoter: RemoteCmdRunnerBase,  # pylint: disable=too-many-arguments,too-many-locals
                         cmd: str,
...
||||||| parent of b09f4db9a (fix(remoter): enforce run_long_running_cmd timeout as a real deadline)
        remoter.run(f"while [ -e /proc/{pid.strip()} ]; do sleep 0.1; done", timeout=timeout, retry=10, verbose=False)
=======
        _poll_process(remoter, pid, cmd, deadline=start_time + timeout if timeout else None, timeout=timeout)
>>>>>>> b09f4db9a (fix(remoter): enforce run_long_running_cmd timeout as a real deadline)
