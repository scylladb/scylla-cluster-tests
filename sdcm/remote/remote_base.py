...
class RemoteCmdRunnerBase(CommandRunner):
    ...
    def receive_files(
        ...
        if self.use_rsync():
            try:
                ...
                rsync = self._make_rsync_cmd([remote_source], local_dest, delete_dst, preserve_symlinks, timeout)
    ...
    def send_files(
        ...
        if self.use_rsync():
            try:
                ...
                rsync = self._make_rsync_cmd(local_sources, remote_dest, delete_dst, preserve_symlinks)
    ...
    def _make_scp_cmd(self, src: str, dst: str, connect_timeout: int = 300, alive_interval: int = 300) -> str:
        ...
        command = "scp -r -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=%d -o ServerAliveInterval=%d -o UserKnownHostsFile=%s -P %d %s %s %s %s"
        proxy_cmd = ""
        if self.proxy_host:
    ...
    def _make_rsync_cmd(
        self, src: list, dst: str, delete_dst: bool, preserve_symlinks: bool, timeout: int = 300
    ) -> str:
        """
        Given a list of source paths and a destination path, produces the
        ...
        return command % (symlink_flag, delete_flag, timeout, ssh_cmd, " ".join(src), dst)
