...
class RemoteCmdRunnerBase(CommandRunner):
    ...
    def receive_files(self, src: str, dst: str, delete_dst: bool = False, preserve_perm: bool = True, preserve_symlinks: bool = False, timeout: float = 300, sudo=False):
        """
        ...
        if self.use_rsync():
            try:
                ...
                rsync = self._make_rsync_cmd([remote_source], local_dest, delete_dst, preserve_symlinks, timeout, sudo=sudo)
        ...
        if try_scp:
            ...
            if remote_source:
                ...
                if sudo:
                    ...
                    res = self.run(f"mkdir -p {temp_remote} && sudo cp -r --parents {' '.join(src)} {temp_remote} && sudo chown -R {self.user}:{self.user} {temp_remote}", ignore_status=True)
    ...
    @retrying(n=3, sleep_time=5, allowed_exceptions=(RetryableNetworkException,))
    def send_files(self, src: str, dst: str, delete_dst: bool = False, preserve_symlinks: bool = False, verbose: bool = False, sudo=False) -> bool:
        """
    ...
    def _make_scp_cmd(self, src: str, dst: str, connect_timeout: int = 300, alive_interval: int = 300) -> str:
        ...
        command = "scp -q -r -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=%d -o ServerAliveInterval=%d -o UserKnownHostsFile=%s -P %d %s %s %s %s"
        proxy_cmd = ""
        if self.proxy_host:
            proxy_cmd = self._make_proxy_cmd()
        return command % (connect_timeout, alive_interval, self.known_hosts_file, self.port, key_option, proxy_cmd, " ".join(src), dst)
    ...
    def _make_rsync_cmd(self, src: list, dst: str, delete_dst: bool, preserve_symlinks: bool, timeout: int = 300, sudo=False) -> str:
        """
        Given a list of source paths and a destination path, produces the
        ...
        if self.proxy_host:
            proxy_cmd = self._make_proxy_cmd()
        ssh_cmd = self._make_ssh_command(user=self.user, port=self.port, hosts_file=self.known_hosts_file, key_file=self.key_file, extra_ssh_options=self.extra_ssh_options.replace("-tt", "-t"), proxy_cmd=proxy_cmd)
    ...
    def _make_proxy_cmd(self):
        ...
        proxy_command = f'-o ProxyCommand="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i {key} -W %h:%p {self.proxy_user}@{self.proxy_host}"'
    ...
    def _run_execute(self, cmd: str, timeout: Optional[float] = None, ignore_status: bool = False, verbose: bool = True, new_session: bool = False, watchers: Optional[List[StreamWatcher]] = None):
        ...
        command_kwargs = dict(command=cmd, warn=ignore_status, encoding="utf-8", hide=True, watchers=watchers, timeout=timeout, in_stream=False)
        if new_session:
    ...
    def _run_pre_run(
        self, cmd: str, timeout: Optional[float] = None, ignore_status: bool = False, verbose: bool = True, new_session: bool = False, log_file: Optional[str] = None, retry: int = 1, watchers: Optional[List[StreamWatcher]] = None
    ):
        pass
    ...
    def run(
        ...
        watchers = self._setup_watchers(verbose=verbose, log_file=log_file, additional_watchers=watchers, timestamp_logs=timestamp_logs)
