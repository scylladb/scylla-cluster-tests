...
def remote_file(remoter, remote_path: str | Path, serializer=StringIO.getvalue, deserializer=read_to_stringio, sudo=False, preserve_ownership=True, preserve_permissions=True, log_change=True):
    ...
    wait.wait_for(remoter.receive_files, step=10, text=f"Waiting for copying `{remote_path}' from {remoter.hostname}", timeout=300, throw_exc=True, src=str(remote_path), dst=local_tempfile, sudo=sudo)
    ...
    if original_content == content:
        ...
        remote_tempfile_move_cmd = shell_script_cmd(f"""\
            ...
            rm '{remote_tempfile}'
            """)
        wait.wait_for(remoter.send_files, step=10, text=f"Waiting for updating of `{remote_path}' on {remoter.hostname}", timeout=300, throw_exc=True, src=local_tempfile, dst=remote_tempfile, sudo=sudo)
        if sudo:
