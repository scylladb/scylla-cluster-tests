...
class CommandRunner(metaclass=ABCMeta):
    ...
    def _setup_watchers(self, verbose: bool, log_file: str, additional_watchers: list, timestamp_logs: bool = False) -> List[StreamWatcher]:
    ...
    def sudo(
        ...
        return self.run(cmd=cmd, timeout=timeout, ignore_status=ignore_status, verbose=verbose, new_session=new_session, log_file=log_file, retry=retry, watchers=watchers, timestamp_logs=timestamp_logs)
    ...
    def _print_command_results(self, result: Result, verbose: bool, ignore_status: bool):
        ...
        if verbose and result.failed and not ignore_status:
            self.log.error('<%s>: Error executing command: "%s"; Exit status: %s', hostname, result.command, result.exited)
            if result.stdout:
    ...
    def _make_ssh_command(
        user: str = "root", port: int = 22, opts: str = "", hosts_file: str = "/dev/null", key_file: str = None, connect_timeout: float = 300, alive_interval: float = 300, extra_ssh_options: str = "", proxy_cmd: str = None
    ) -> str:
...
def shell_script_cmd(cmd: str, shell_cmd: str = "bash -cxe", quote: str = '"', preprocessor: Callable[[str], str] = dedent) -> str:
