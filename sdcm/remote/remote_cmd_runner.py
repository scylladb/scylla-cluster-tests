...
class RemoteCmdRunner(RemoteCmdRunnerBase, ssh_transport="fabric", default=True):
    ...
    exception_retryable = (NoValidConnectionsError, SSHException, EOFError, AuthenticationException, ConnectionResetError, ConnectionAbortedError, ConnectionError, ConnectionRefusedError)
    ...
    def _run_pre_run(
        ...
        if not self.is_up(timeout=self.connect_timeout):
            raise SSHConnectTimeoutError('Unable to run "%s": failed connecting to "%s" during %ss' % (cmd, self.hostname, self.connect_timeout))
    ...
    def _create_connection(self) -> Connection:
        ...
        if self.proxy_host:
            gateway_connection = Connection(
                host=self.proxy_host, user=self.proxy_user, port=self.proxy_port, config=self.ssh_config, connect_timeout=self.connect_timeout, connect_kwargs={"key_filename": os.path.expanduser(self.proxy_key) if self.proxy_key else None}
            )
