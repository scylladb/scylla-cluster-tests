...
from .libssh2_client.exceptions import AuthenticationException, UnknownHostException, ConnectError, FailedToReadCommandOutput, CommandTimedOut, FailedToRunCommand, OpenChannelTimeout, SocketRecvError, UnexpectedExit, Failure
...
class RemoteLibSSH2CmdRunner(RemoteCmdRunnerBase, ssh_transport="libssh2"):
    ...
    def _create_connection(self) -> LibSSH2Client:
        return LibSSH2Client(host=self.hostname, user=self.user, port=self.port, pkey=os.path.expanduser(self.key_file), timings=Timings(keepalive_timeout=0, connect_timeout=self.connect_timeout))
