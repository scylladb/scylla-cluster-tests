...
class AgentCmdRunner(CommandRunner):
    """
    Command runner that executes commands via SCT agent REST API.

    Example of usage:
        runner = AgentCmdRunner(hostname="192.168.1.10", port=8443, api_key="secret")
        result = runner.run("echo hello")
        print(result.stdout)  # "hello\\n"
    """

    def __init__(self, hostname: str, port: int = 8443, api_key: str = "default-api-key",
    ...
    def _create_connection(self) -> AgentClient:
        return AgentClient(self.hostname, self.port, self.api_key, 30)
    ...
    def run(self,
        ...
        while attempt < retry:
            ...
            except (AgentConnectionError, AgentTimeoutError) as exc:
                last_exception = exc
                self.log.warning("Agent communication error (attempt %d/%d): %s", attempt, retry, exc)

                if attempt < retry:
                    self.log.info("Retrying command execution...")
                    continue
                raise RetryableNetworkException(
                    f"Failed to execute command after {retry} attempts: {exc}", original=exc) from exc
        ...
        raise RetryableNetworkException(
            f"Failed to execute command after {retry} attempts", original=last_exception
        ) from last_exception
