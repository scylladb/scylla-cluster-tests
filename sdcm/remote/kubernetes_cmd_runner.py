...
class KubernetesCmdRunner(RemoteCmdRunnerBase):
    ...
    def __init__(self, kluster, pod_image: str, pod_name: str, container: Optional[str] = None, namespace: str = "default") -> None:
    ...
    def _run_execute(self, cmd: str, timeout: Optional[float] = None, ignore_status: bool = False, verbose: bool = True, new_session: bool = False, watchers: Optional[List[StreamWatcher]] = None):
        ...
        if tmp[0] == "sudo":
            ...
            self.log.error("Cut off `sudo' from the cmd string: %s (%s:%s: %s)", cmd, frame.filename, frame.lineno, frame.code_context[0].rstrip())
            if tmp[1] == "-u":
                cmd = tmp[3]
            else:
                cmd = cmd[cmd.find("sudo") + 5 :]
        # Session should be created for each run
        return super()._run_execute(cmd, timeout=timeout, ignore_status=ignore_status, verbose=verbose, new_session=True, watchers=watchers)
    ...
    def receive_files(self, src, dst, delete_dst=False, preserve_perm=True, preserve_symlinks=False, timeout=300, sudo: bool = False):
        ...
        KubernetesOps.copy_file(self.kluster, f"{self.namespace}/{self.pod_name}:{src}", dst, container=self.container, timeout=timeout)
        return True
    ...
    def send_files(self, src, dst, delete_dst=False, preserve_symlinks=False, verbose=False, sudo: bool = False):
        ...
        with KEY_BASED_LOCKS.get_lock(f"k8s--{self.kluster.name}--{self.namespace}--{self.pod_name}"):
            KubernetesOps.copy_file(self.kluster, src, f"{self.namespace}/{self.pod_name}:{dst}", container=self.container, timeout=300)
        return True
...
class KubernetesPodWatcher(KubernetesRunner):
        ...
        with self._ws_lock:
            if not self.process or self.process.closed:
                ...
                if self.pod_counter_to_live < 1:
                    LOGGER.warning("'_read_from_stream': stopping '%s' pod because stream to it cannot be established having alive pod.", self.context.config.k8s_pod_name)
                    self._stop_pod()
                    time.sleep(10)
                    return ""
                else:
                    LOGGER.debug("'_read_from_stream' called for the %s pod having closed socket. Recreating it.", pod_name)
                    self._start()
            elif self.STREAM_CONNECTION_TTL < time.time() - self.stream_connection_start_time:
                LOGGER.debug("'_read_from_stream': Recreate '%s' pod log stream connection to avoid freezes. Reason: reached '%s' seconds of 'time to live'", pod_name, self.STREAM_CONNECTION_TTL)
                self.process.close()
                self._start()
            ...
            if isinstance(result, bytes):
                try:
                    ...
                    InfoEvent(message=f"Failure decoding 'utf-8' on the '{pod_name}' pod: {exc}", severity=Severity.WARNING).publish()
                    addon_result = self.process.read(5)
                    sum_result = result + addon_result
                    try:
                        return sum_result.decode("utf-8")
                    except UnicodeDecodeError as exc:
                        InfoEvent(message=f"[fallback error] Failure decoding 'utf-8' on the '{pod_name}' pod: {exc}", severity=Severity.ERROR).publish()
    ...
    def _start(self) -> None:
        ...
        with self._ws_lock:
            ...
            if self.current_read_bytes_num:
                LOGGER.debug("'runner._start()' method for the '%s' pod: Going to re-read '%s' num of bytes to avoid sending bytes to consumer which were already sent.", pod_name, self.current_read_bytes_num)
                reread_bytes = 0
                ...
                LOGGER.debug("'runner._start()' method for the '%s' pod: Successfully re-read '%s' num of bytes.", pod_name, self.current_read_bytes_num)

    def start(self, command: str, shell: str, env: dict) -> None:
        LOGGER.debug("Calling 'runner.start()' method for the '%s' pod.\ncommand: %s\n", self.context.config.k8s_pod_name, command)
        self._start()
    ...
    def _get_pod_status(self) -> dict:
        result_raw = self.context.config.k8s_kluster.kubectl(f"get pod {self.context.config.k8s_pod_name} -o jsonpath='{{.status}}'", namespace=self.context.config.k8s_namespace).stdout.strip()
    ...
    def _write_proc_stdin(self, data: str) -> None:
        LOGGER.warning("Unexpected operation on the 'K8S pod logs watcher' (%s) was attempted. Writing data to stdin cannot be done. Data: %s", self.context.config.k8s_pod_name, data)
    ...
    def _stop_pod(self) -> None:
        ...
        self.context.config.k8s_kluster.kubectl(f"exec {self.context.config.k8s_pod_name} -- /bin/bash -c 'rm /tmp/keep_running'", namespace=self.context.config.k8s_namespace, timeout=30, ignore_status=True)
        self.process.close()
    ...
    def stop(self) -> None:
        ...
        with self._ws_lock:
            if TestConfig().tester_obj().teardown_started:
                LOGGER.warning("'stop()' method is called for the '%s' pod as part of the 'tearDown'. 'log reader' socket is '%s'.", pod_name, "closed" if not self.process or self.process.closed else "open")
                self._stop_pod()
                return
            if not self.process or self.process.closed:
                LOGGER.warning(
                ...
                LOGGER.warning("'stop()' method is called for the '%s' pod, it is running and has open 'log reader' socket. So, looks like it is intentional close of the socket.", pod_name)
                self._stop_pod()

    def run(self, command, **kwargs):
        pod_name = self.context.config.k8s_pod_name

        LOGGER.debug("'remoter.run': '%s' pod will be called with following command: %s\nAnd kwargs: %s\n", pod_name, command, kwargs)
        ...
        wait_for(self._is_pod_ready_or_completed, step=2, text="'%s' pod is not ready/completed yet..." % pod_name, timeout=420, throw_exc=True)
...
class KubernetesPodRunner(KubernetesCmdRunner):
    def __init__(self, kluster, template_path: str, template_modifiers: list, pod_name_template: str, namespace: str, environ: dict) -> None:
        self.kluster = kluster
    ...
    def pod_name(self) -> str:
        LOGGER.debug("'remoter.pod_name': called for the '%s'. Current counter value is '%s'", self.pod_name_template, self._pod_counter)
    ...
    def receive_files(self, src, dst, delete_dst=False, preserve_perm=True, preserve_symlinks=False, timeout=300, sudo=False):
    ...
    def send_files(self, src, dst, delete_dst=False, preserve_symlinks=False, verbose=False, sudo=False):
        ...
        with self._mounted_files_lock:
            ...
            if existing_src := self._mounted_files_mapping.get(dst):
                ...
                raise ValueError("Cannot mount '%s' src file to the '%s' dst. It is already used by another src file -> '%s'." % (src, dst, existing_src))
