...
class KubernetesPodWatcher(KubernetesRunner):
        ...
        with self._ws_lock:
            if not self.process or self.process.closed:
                ...
                if self.pod_counter_to_live < 1:
                    LOGGER.warning(
                        "'_read_from_stream': stopping '%s' pod because stream to it cannot be established having alive pod.",
                ...
                LOGGER.debug(
                    "'_read_from_stream': Recreate '%s' pod log stream connection to avoid freezes. Reason: reached '%s' seconds of 'time to live'",
                    pod_name,
    ...
    def _start(self) -> None:
        ...
        with self._ws_lock:
            ...
            if self.current_read_bytes_num:
                LOGGER.debug(
                    "'runner._start()' method for the '%s' pod: Going to re-read '%s' num of bytes to avoid sending bytes to consumer which were already sent.",
                    pod_name,
    ...
    def _write_proc_stdin(self, data: str) -> None:
        LOGGER.warning(
            "Unexpected operation on the 'K8S pod logs watcher' (%s) was attempted. Writing data to stdin cannot be done. Data: %s",
            self.context.config.k8s_pod_name,
            data,
        )
    ...
    def stop(self) -> None:
        ...
        with self._ws_lock:
            if TestConfig().tester_obj().teardown_started:
                LOGGER.warning(
                    "'stop()' method is called for the '%s' pod as part of the 'tearDown'. 'log reader' socket is '%s'.",
                    pod_name,
            ...
            if not self.process or self.process.closed:
                ...
                LOGGER.warning(
                    "'stop()' method is called for the '%s' pod, it is running and has open 'log reader' socket. So, looks like it is intentional close of the socket.",
                    pod_name,
                )
                self._stop_pod()
    ...
    def send_files(self, src, dst, delete_dst=False, preserve_symlinks=False, verbose=False):
        ...
        with self._mounted_files_lock:
            ...
            if existing_src := self._mounted_files_mapping.get(dst):
                ...
                raise ValueError(
                    "Cannot mount '%s' src file to the '%s' dst. It is already used by another src file -> '%s'."
                    % (src, dst, existing_src)
                )
