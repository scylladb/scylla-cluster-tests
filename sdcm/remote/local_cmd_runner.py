...
class LocalCmdRunner(CommandRunner):
    ...
    def run(
        ...
        def _run():
            ...
            try:
                command_kwargs = dict(command=cmd, warn=ignore_status, encoding="utf-8", hide=True, watchers=watchers, timeout=timeout, env=os.environ, replace_env=True, in_stream=False)
                if new_session:
    ...
    def receive_files(self, src: str, dst: str, delete_dst: bool = False, preserve_perm: bool = True, preserve_symlinks: bool = False, timeout: float = 300, sudo: bool = False) -> bool:
        if src == dst:
            return True
    ...
    @retrying(n=3, sleep_time=5, allowed_exceptions=(RetryableNetworkException,))
    def send_files(self, src: str, dst: str, delete_dst: bool = False, preserve_symlinks: bool = False, verbose: bool = False, timeout: float = 300, sudo: bool = False) -> bool:
        if src == dst:
            return True
        sudo = "sudo " if sudo else ""
        return self.run(f"{sudo} cp {src} {dst}", timeout=timeout).ok
