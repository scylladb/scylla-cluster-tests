...
class DockerCmdRunner(CommandRunner):
    ...
    def is_up(self, timeout: float | None = None) -> bool:
        ...
        try:
            ...
            self.log.error("Error checking if Docker node %s is up: %s", self.node.name, str(e), exc_info=True)
            return False
    ...
    def run(
        ...
        watchers: list[StreamWatcher] | None = None, change_context: bool = False
    ) -> Result:
        ...
        def _run():
            return self._execute_command(cmd, timeout, ignore_status, verbose, watchers_list)
    ...
    def _execute_command(
        self, cmd: str, timeout: float | None, ignore_status: bool, verbose: bool, watchers: list[StreamWatcher]
    ) -> Result:
        ...
        try:
            exec_output = container.exec_run(["sh", "-c", cmd], tty=False, demux=True, stream=False)
