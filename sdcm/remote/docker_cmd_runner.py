...
class DockerCmdRunner(CommandRunner):
    ...
    def _execute_command(self, cmd: str, timeout: float | None, ignore_status: bool, verbose: bool, watchers: list[StreamWatcher], user: str | None = "") -> Result:
        ...
        try:
            ...
            result = Result(command=cmd, exited=exec_output.exit_code, stdout=stdout_bytes.decode(errors="replace") if stdout_bytes else "", stderr=stderr_bytes.decode(errors="replace") if stderr_bytes else "")
            result.duration = time.perf_counter() - start_time
            result.exit_status = exec_output.exit_code

            if verbose:
                self.log.debug("<%s>: Docker exec result (status=%d, duration=%.2fs)\nStdout:\n%s\nStderr:\n%s", self.node.name, result.exited, result.duration, result.stdout, result.stderr)
            ...
            self.log.error("Exception during docker exec on %s for command '%s': %s", self.node.name, cmd, str(exc), exc_info=True)
    ...
    def send_files(self, src: str, dst: str, delete_dst: bool = False, preserve_symlinks: bool = False, verbose: bool = False, timeout: float = 300, **kwargs) -> bool:
        ...
        try:
            ...
            self.log.error("Failed to send '%s' to container '%s:%s': %s", src, container.name, dst, str(e), exc_info=True)
            return False

    @retrying(n=3, sleep_time=5, allowed_exceptions=(Exception,))
    def receive_files(self, src: str, dst: str, delete_dst: bool = False, preserve_perm: bool = True, preserve_symlinks: bool = False, timeout: float = 300, **kwargs) -> bool:
        ...
        try:
            ...
            self.log.error("Failed to receive from container '%s:%s' to local '%s': %s", container.name, src, dst, str(e), exc_info=True)
            return False
    ...
    def sudo(
        ...
        return self.run(cmd=cmd, timeout=timeout, ignore_status=ignore_status, verbose=verbose, new_session=new_session, log_file=log_file, retry=retry, watchers=watchers, user=user)
