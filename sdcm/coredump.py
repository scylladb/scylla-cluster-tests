...
class CoredumpThreadBase(Thread):
    ...
    def _process_coredumps(self, in_progress: List[CoreDumpInfo], completed: List[CoreDumpInfo], uploaded: List[CoreDumpInfo]):
        """
    ...
    def extract_info_from_core_pids(self, new_cores: Optional[List[CoreDumpInfo]], exclude_cores: List[CoreDumpInfo]) -> List[CoreDumpInfo]:
        output = []
    ...
    def _upload_coredump(self, core_info: CoreDumpInfo):
        ...
        self.node.remoter.run("sudo curl --request PUT --fail --show-error --upload-file '%s' 'https://%s'" % (coredump, upload_url))
    ...
    def _get_executable_version(self, executable: str) -> Optional[str]:
        if self.node.distro.is_rhel_like:
            ...
            release_version = self.node.remoter.run(f"rpm -q --queryformat '%{{VERSION}}' {pkg}", ignore_status=True).stdout.strip()
        elif self.node.distro.is_ubuntu or self.node.distro.is_debian:
            pkg = self.node.remoter.sudo(f"dpkg -S {executable}", ignore_status=True).stdout.split(":")[0].strip()
            release_version = self.node.remoter.run(f"dpkg-query --showformat='${{Version}}' --show {pkg}", ignore_status=True).stdout.strip()
        else:
            raise RuntimeError("Distro is not supported")
    ...
    def update_new_coredump_with_exec_information(self, core_info: CoreDumpInfo) -> None:
        if core := self._get_core_by_pid(core_info.pid):
            core_info.update(executable=core.get("exe", r"N\A"), executable_version=self._get_executable_version(core.get("exe", r"N\A")))
...
class CoredumpExportSystemdThread(CoredumpThreadBase):
    ...
    def systemd_version(self):
        systemd_version = 0
        try:
            systemd_version = get_systemd_version(self.node.remoter.run("systemctl --version", ignore_status=True).stdout)
        except Exception:  # noqa: BLE001
    ...
    def get_list_of_cores_json(self) -> Optional[List[CoreDumpInfo]]:
        result = self.node.remoter.run("sudo coredumpctl -q --json=short", verbose=False, ignore_status=True)
        if not result.ok:
            return []
    ...
    def _wait_for_pending_coredumps(self):
        ...
        """
        result = self.node.remoter.run('systemctl list-units --type=service --state=running | grep -q "systemd-coredump@"', verbose=False, ignore_status=True)
        if result.ok:
            self.log.info("Pending coredumps found, waiting for them to complete")
            self.node.remoter.run('while systemctl list-units --type=service --state=running | grep -q "systemd-coredump@"; do sleep 1; done', verbose=False, ignore_status=True, timeout=1800)
    ...
    def get_list_of_cores(self) -> Optional[List[CoreDumpInfo]]:
        ...
        result = self.node.remoter.run("sudo coredumpctl --no-pager --no-legend 2>&1", verbose=False, ignore_status=True)
    ...
    def update_coredump_info_with_more_information(self, core_info: CoreDumpInfo):
        ...
        core_info.update(executable=executable, command_line=command_line, corefile=corefile, source_timestamp=event_timestamp, coredump_info="\n".join(coredump_info) + "\n")
    ...
    def _get_coredumpctl_info(self, core_info: CoreDumpInfo) -> List[str]:
        ...
        :return: list[str]
        """
        output = self.node.remoter.run(f"sudo coredumpctl info --no-pager --no-legend {core_info.pid}", verbose=False, ignore_status=False)
    ...
    def get_list_of_cores(self) -> Optional[List[CoreDumpInfo]]:
        ...
        for directory in self.coredumps_directories:
            for corefile in self.node.remoter.sudo(f"ls {directory}", verbose=False, ignore_status=True).stdout.split():
                ...
                output.append(CoreDumpInfo(pid=core_data["pid"], source_timestamp=float(core_data["source_timestamp"]), corefile=os.path.join(directory, os.path.basename(corefile)), node=self.node))
        if output:
