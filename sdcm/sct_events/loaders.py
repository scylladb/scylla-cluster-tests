...
class GeminiStressEvent(BaseStressEvent):
    def __init__(self, node: Any, cmd: str, log_file_name: Optional[str] = None, severity: Severity = Severity.NORMAL, publish_event: bool = True):
        self.node = str(node)
        self.cmd = cmd
...
NdBenchStressEvent.add_stress_subevents(start=Severity.NORMAL, finish=Severity.NORMAL, error=Severity.ERROR, failure=Severity.CRITICAL)
...
CassandraStressLogEvent.add_subevent_type("OperationOnKey", severity=Severity.CRITICAL, regex=r"Operation x10 on key\(s\) \[")
# TODO: change TooManyHintsInFlight severity to CRITICAL, when we have more stable hinted handoff
# TODO: backpressure mechanism.
CassandraStressLogEvent.add_subevent_type("TooManyHintsInFlight", severity=Severity.ERROR, regex=r"Too many hints in flight|Too many in flight hints")

CassandraStressLogEvent.add_subevent_type("IOException", severity=Severity.ERROR, regex=r"java\.io\.IOException")
CassandraStressLogEvent.add_subevent_type("ConsistencyError", severity=Severity.ERROR, regex="Cannot achieve consistency level")
...
CassandraStressLogEvent.add_subevent_type("RackAwarePolicy", severity=Severity.NORMAL, regex=r"Using provided rack name '.+' for RackAwareRoundRobinPolicy")
...
CS_ERROR_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex), event) for event in CS_ERROR_EVENTS]

CS_NORMAL_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex), event) for event in CS_NORMAL_EVENTS]
...
CqlStressCassandraStressLogEvent.add_subevent_type("ReadValidationError", severity=Severity.CRITICAL, regex=r"read validation error")


CQL_STRESS_CS_ERROR_EVENTS = (CqlStressCassandraStressLogEvent.ReadValidationError(),)
CQL_STRESS_CS_ERROR_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex), event) for event in CQL_STRESS_CS_ERROR_EVENTS]
...
ScyllaBenchLogEvent.add_subevent_type("RackAwarePolicy", severity=Severity.NORMAL, regex=r"Using provided rack name '.+' for RackAwareRoundRobinPolicy")
...
SCYLLA_BENCH_ERROR_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex), event) for event in SCYLLA_BENCH_ERROR_EVENTS]

SCYLLA_BENCH_NORMAL_EVENTS = (ScyllaBenchLogEvent.RackAwarePolicy(),)

SCYLLA_BENCH_NORMAL_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex), event) for event in SCYLLA_BENCH_NORMAL_EVENTS]

CASSANDRA_HARRY_ERROR_EVENTS = ()
CASSANDRA_HARRY_ERROR_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex), event) for event in CASSANDRA_HARRY_ERROR_EVENTS]
...
NoSQLBenchStressLogEvents.add_subevent_type("ProgressIndicatorStoppedEvent", severity=Severity.CRITICAL, regex=r"\d*\s+INFO\s+\[ProgressIndicator/logonly:\ds\]\s+PROGRESS\s+\w+:\s+\d{2}.\d{2}%/Stopped.*")

NoSQLBenchStressLogEvents.add_subevent_type("ProgressIndicatorRunningEvent", severity=Severity.DEBUG, regex=r"\d*\s+INFO\s+\[ProgressIndicator/logonly:\ds\]\s+PROGRESS\s+\w+:\s+\d{2}.\d{2}%/Running.*")

NoSQLBenchStressLogEvents.add_subevent_type("ProgressIndicatorFinishedEvent", severity=Severity.NORMAL, regex=r"\d*\s+INFO\s+\[ProgressIndicator/logonly:\ds\]\s+PROGRESS\s+\w+:\s+100.0%/Stopped\s+.*")


NOSQLBENCH_LOG_EVENTS = (NoSQLBenchStressLogEvents.ProgressIndicatorStoppedEvent(), NoSQLBenchStressLogEvents.ProgressIndicatorFinishedEvent(), NoSQLBenchStressLogEvents.ProgressIndicatorRunningEvent())
