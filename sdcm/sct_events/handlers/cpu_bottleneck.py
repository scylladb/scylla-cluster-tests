    ...
    def handle(self, event: CassandraStressLogEvent, tester_obj: "sdcm.tester.ClusterTester"):  # noqa: F821
        ...
        for node in list(tester_obj.db_cluster.nodes):
            ...
            try:
                ...
                result_record = node.sudo(cmd_record, timeout=40)
                LOGGER.info("perf record completed on node %s: %s", node.name, result_record)

                cmd_report = 'perf report -f --no-children --stdio > perf.report.txt'
                result_report = node.sudo(cmd_report, timeout=30)
                LOGGER.info("perf report completed on node %s: %s", node.name, result_report)
