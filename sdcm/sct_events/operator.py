...
class ScyllaOperatorLogEvent(LogEvent):
    ...
    def add_info(self: T_log_event, node, line: str, line_number: int) -> T_log_event:
        ...
        try:
            ...
            self.source_timestamp = datetime.datetime(year=year, month=month, day=day, hour=int(hour), minute=int(minute), second=int(second), microsecond=int(milliseconds), tzinfo=datetime.timezone.utc).timestamp()
...
ScyllaOperatorLogEvent.add_subevent_type("REAPPLY", severity=Severity.WARNING, regex="please apply your changes to the latest version and try again")
ScyllaOperatorLogEvent.add_subevent_type("TLS_HANDSHAKE_ERROR", severity=Severity.WARNING, regex="TLS handshake error from .*")
ScyllaOperatorLogEvent.add_subevent_type("OPERATOR_STARTED_INFO", severity=Severity.NORMAL, regex='"Starting controller" controller="ScyllaCluster"')
ScyllaOperatorLogEvent.add_subevent_type("WRONG_SCHEDULED_PODS", severity=Severity.WARNING, regex=KubernetesWrongSchedulingLogger.WRONG_SCHEDULED_PODS_MESSAGE)
...
SCYLLA_OPERATOR_EVENT_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex, re.IGNORECASE), event) for event in SCYLLA_OPERATOR_EVENTS]
