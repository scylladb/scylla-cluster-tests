...
class TestFrameworkEvent(InformationalEvent):
    ...
    def __init__(
        self, source: Any, source_method: Optional = None, args: Optional[Sequence] = None, kwargs: Optional[dict] = None, message: Optional = None, exception: Optional = None, trace: Optional = None, severity: Optional[Severity] = None
    ):
        if severity is None:
...
class CoreDumpEvent(InformationalEvent):
    def __init__(self, node: Any, corefile_url: str, backtrace: str, download_instructions: str, source_timestamp: Optional[float] = None, executable: Optional[str] = None, executable_version: Optional[str] = None):
        super().__init__(severity=Severity.ERROR)
...
InstanceStatusEvent.add_subevent_type("REBOOT", severity=Severity.WARNING, regex="Stopped target Host and Network Name Lookups")
...
INSTANCE_STATUS_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex, re.IGNORECASE), event) for event in INSTANCE_STATUS_EVENTS]
