...
class SpotProvisionOutcomeEvent(InformationalEvent):
    ...
    def __init__(
        ...
        # Every placement field is optional at the call site: `record_spot_provision_outcome` accepts None
        # for each, and the legacy paths do not always know the AZ. Normalise here rather than letting
        # `str.format` render a bare "None" into the message - and so that a reader adding, say,
        # `region.split("-")` later is not handed a None the annotation promised could not happen.
        self.region = region or "unknown"
        self.availability_zone = availability_zone or "unknown"
