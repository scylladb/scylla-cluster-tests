...
def start_events_device(log_dir: Optional[Union[str, Path]] = None, _registry: Optional[EventsProcessesRegistry] = None) -> None:
...
def enable_default_filters(sct_config: SCTConfiguration):
    ...
    if SkipPerIssues("scylladb/scylla-enterprise#2710", params=sct_config):
        EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex="ldap_connection - Seastar read failed: std::system_error (error system:104, read: Connection reset by peer)").publish()
    ...
    if SkipPerIssues(
        ...
        EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent, regex=r".*view - Error applying view update.*").publish()
    ...
    if SkipPerIssues("scylladb/scylla-enterprise#3148", params=sct_config):
        EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=CassandraStressLogEvent.ConsistencyError, regex=r".*Authentication error on host.*Cannot achieve consistency level for cl ONE").publish()
    ...
    if sct_config.get("use_cloud_manager") and SkipPerIssues("scylladb/siren#13281", params=sct_config):
        EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.DATABASE_ERROR, regex=r".*Startup failed.*").publish()
    ...
    EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent.ABORTING_ON_SHARD, regex=r".*Parent connection [\d]+ is aborting on shard").publish()
...
def enable_teardown_filters():
    ...
    EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=CassandraStressEvent, regex=r".*Error response from daemon: No such container.*", extra_time_to_expiration=60).publish()
