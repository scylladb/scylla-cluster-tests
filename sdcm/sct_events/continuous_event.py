...
class ContinuousEventsRegistry(metaclass=Singleton):
    ...
    def add_event(self, event: ContinuousEvent):
        ...
        if hash_bucket is None:
            ...
            LOGGER.error(
                "Continues event %s with hash %s (%s) is not suppose to have duplicates, while there is another event with same hash - %s",
                hash_bucket[-1],
    ...
    def find_running_disruption_events(self):
        running_nemesis_events = [event[0] for event in self.hashed_continuous_events.values() if event and event[0].base == "DisruptionEvent"]
...
class ContinuousEvent(SctEvent, abstract=True):
    ...
    def continuous_hash_tuple(self) -> tuple[Any | None]:
        return (self.__class__.__name__,) + tuple(getattr(self, attr_name, None) for attr_name in self.continuous_hash_fields)
