...
class DbEventsFilter(BaseFilter):
    def __init__(self, db_event: Union[LogEventProtocol, Type[LogEventProtocol]], line: Optional[Union[str, re.Pattern]] = None, node: Optional = None, extra_time_to_expiration: Optional[int] = 0):
        super().__init__()
...
class EventsFilter(BaseFilter):
    def __init__(self, event_class: Optional[Type[SctEventProtocol] | Type[SctEvent]] = None, regex: Optional[Union[str, re.Pattern]] = None, extra_time_to_expiration: Optional[int] = 0):
        assert event_class or regex, "Should call with event_class or regex, or both"
        assert not event_class or issubclass(event_class, SctEvent), "event_class should be a class inherits from SctEvent"
...
class EventsSeverityChangerFilter(EventsFilter):
    def __init__(self, new_severity: Severity, event_class: Optional[Type[SctEvent]] = None, regex: Optional[str] = None, extra_time_to_expiration: Optional[int] = None):
