...
class OutboundEventsProtocol(Protocol[T_outbound_events_protocol]):
    def outbound_events(self, stop_event: StopEvent, events_counter: multiprocessing.Value) -> Generator[T_outbound_events_protocol, None, None]: ...
...
class BaseEventsProcess(Generic[T_inbound_event, T_outbound_event], abc.ABC):
    ...
    def inbound_events(self) -> InboundEventsGenerator:
        yield from cast(OutboundEventsProtocol[T_inbound_event], get_events_process(name=self.inbound_events_process, _registry=self._registry)).outbound_events(stop_event=self.stop_event, events_counter=self._events_counter)
