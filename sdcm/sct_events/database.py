...
DatabaseLogEvent.add_subevent_type("OVERSIZED_ALLOCATION", severity=Severity.ERROR, regex="seastar_memory - oversized allocation:")
...
DatabaseLogEvent.add_subevent_type("UNKNOWN_VERB", severity=Severity.WARNING, regex="(unknown verb exception|unknown_verb_error)")
DatabaseLogEvent.add_subevent_type("CLIENT_DISCONNECT", severity=Severity.WARNING, regex=r"cql_server - exception while processing connection:")
...
DatabaseLogEvent.add_subevent_type("SYSTEM_PAXOS_TIMEOUT", severity=Severity.WARNING, regex="(mutation_write_|Operation timed out for system.paxos|Operation failed for system.paxos)")
DatabaseLogEvent.add_subevent_type("SERVICE_LEVEL_CONTROLLER", severity=Severity.WARNING, regex="Operation timed out for system_distributed.service_levels")
DatabaseLogEvent.add_subevent_type("GATE_CLOSED", severity=Severity.WARNING, regex='exception "gate closed" in no_wait handler ignored')
DatabaseLogEvent.add_subevent_type("RESTARTED_DUE_TO_TIME_OUT", severity=Severity.WARNING, regex="scylla-server.service.*State 'stop-sigterm' timed out")
DatabaseLogEvent.add_subevent_type(
...
DatabaseLogEvent.add_subevent_type("SCHEMA_FAILURE", severity=Severity.ERROR, regex=r"(.*ERROR|!ERR).*Failed to load schema version")
...
DatabaseLogEvent.add_subevent_type("DIRECTORY_NOT_EMPTY", severity=Severity.NORMAL, regex="remove failed: Directory not empty")
DatabaseLogEvent.add_subevent_type("FILESYSTEM_ERROR", severity=Severity.ERROR, regex="filesystem_error")
DatabaseLogEvent.add_subevent_type("DISK_ERROR", severity=Severity.ERROR, regex=r"storage_service - .*due to I\/O errors.*Disk error: std::system_error")
...
# REACTOR_STALLED must be above BACKTRACE as it has "Backtrace" in its message
DatabaseLogEvent.add_subevent_type("REACTOR_STALLED", mixin=ReactorStalledMixin, severity=Severity.DEBUG, regex="Reactor stalled")
...
DatabaseLogEvent.add_subevent_type("CORRUPTED_SSTABLE", severity=Severity.CRITICAL, regex="sstables::malformed_sstable_exception|invalid_mutation_fragment_stream")
...
DatabaseLogEvent.add_subevent_type("RPC_CONNECTION", severity=Severity.WARNING, regex=r"(^ERROR|!ERR).*rpc - client .*(connection dropped|fail to connect)")
...
SYSTEM_ERROR_EVENTS_PATTERNS: List[Tuple[re.Pattern, LogEventProtocol]] = [(re.compile(event.regex, re.IGNORECASE), event) for event in SYSTEM_ERROR_EVENTS]
...
class CompactionEvent(ScyllaDatabaseContinuousEvent):
    ...
    def msgfmt(self):
        ...
        compaction_process_id = " compaction_process_id={0.compaction_process_id}" if self.compaction_process_id is not None else ""
...
class ScyllaYamlUpdateEvent(InformationalEvent):
    def __init__(self, node_name: str, message: Optional[str] = None, diff: dict | None = None, severity=Severity.NORMAL, **__):
        super().__init__(severity=severity)
...
SCYLLA_DATABASE_CONTINUOUS_EVENTS = [ScyllaServerStatusEvent, BootstrapEvent, JMXServiceEvent, ScyllaSysconfigSetupEvent]
...
def get_pattern_to_event_to_func_mapping(node: str) -> List[ScyllaServerEventPatternFuncs]:
    ...
    def _end_event(event_type: Type[ScyllaDatabaseContinuousEvent], match: Match):
        ...
        if begin_event := event_registry.find_continuous_events_by_hash(continuous_hash):
            ...
            return
        TestFrameworkEvent(source=event_type.__name__, message=f"Did not find events of type {event_type} with hash {continuous_hash} ({kwargs}) with period type {EventPeriod.BEGIN.value}", severity=Severity.DEBUG).publish_or_dump()

    for event in SCYLLA_DATABASE_CONTINUOUS_EVENTS:
        mapping.append(ScyllaServerEventPatternFuncs(pattern=re.compile(event.begin_pattern), event_class=event, period_func=partial(_add_event, event_type=event)))
        mapping.append(ScyllaServerEventPatternFuncs(pattern=re.compile(event.end_pattern), event_class=event, period_func=partial(_end_event, event_type=event)))
