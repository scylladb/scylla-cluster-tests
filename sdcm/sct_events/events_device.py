...
from sdcm.sct_events.events_processes import EVENTS_MAIN_DEVICE_ID, StopEvent, EventsProcessesRegistry, start_events_process, get_events_process, verbose_suppress, suppress_interrupt
...
class EventsDevice(multiprocessing.Process):
    ...
    def outbound_events(self, stop_event: StopEvent, events_counter: multiprocessing.Value) -> Generator[Tuple[str, Any], None, None]:
            ...
            for events_counter.value, obj in enumerate(self.inbound_events(stop_event=stop_event), start=1):
                ...
                if isinstance(obj, BaseFilter):
                    if obj.clear_filter and not obj.expire_time:
                        ...
                        LOGGER.debug("%s: set expire_time to %s for filter with uuid=%s", self, obj.expire_time, obj.uuid)
                ...
                if obj.severity.value > Severity.WARNING.value:
                    ...
                    ACTION_LOGGER.error(f"{event_type} {obj.severity.name} Event (id={obj.event_id}) on node: {getattr(obj, 'node', None)}")
