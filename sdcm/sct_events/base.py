...
class SctEvent:
    ...
    def add_subevent_type(cls, name: str, /, *, abstract: bool = False, mixin: Optional[Type] = None, **kwargs) -> None:
        ...
        assert name.isidentifier() and not iskeyword(name), "name of an SCT event type should be a valid Python identifier and not a keyword"
    ...
    def __getstate__(self):
        ...
        if subcontext := getattr(self, "subcontext"):
            attrs["subcontext"] = [self.attribute_with_value_for_json(attributes_list=event.subcontext_fields, event=event) for event in subcontext]
        else:
            attrs["subcontext"] = []
        return attrs
    ...
    def __eq__(self, other):
        return (isinstance(other, type(self)) or isinstance(self, type(other))) and self.__getstate__() == other.__getstate__()
...
__all__ = ("SctEvent", "SctEventProtocol", "SystemEvent", "BaseFilter", "LogEvent", "LogEventProtocol", "T_log_event", "add_severity_limit_rules", "max_severity", "print_critical_events", "InformationalEvent", "EventPeriod")
