...
class AzureNode(cluster.BaseNode):
    ...
    def __init__(self, azure_instance: VmInstance, credentials, parent_cluster, node_prefix="node", node_index=1, base_logdir=None, dc_idx=0, rack=0):
        ...
        ssh_login_info = {"hostname": None, "user": azure_instance.user_name, "key_file": credentials.key_file, "extra_ssh_options": "-tt"}
        super().__init__(name=name, parent_cluster=parent_cluster, ssh_login_info=ssh_login_info, base_logdir=base_logdir, node_prefix=node_prefix, dc_idx=dc_idx, rack=rack)
    ...
    def check_spot_termination(self):
        ...
        try:
            ...
            result = self.remoter.run('curl http://169.254.169.254/metadata/scheduledevents?api-version=2020-07-01 -H "Metadata: true"', verbose=False)
...
class AzureCluster(cluster.BaseCluster):
    def __init__(
        ...
        super().__init__(cluster_uuid=cluster_uuid, cluster_prefix=cluster_prefix, node_prefix=node_prefix, n_nodes=n_nodes, params=params, region_names=region_names, node_type=node_type)
    ...
    def _create_node(self, instance, node_index, dc_idx, rack):
        try:
            node = AzureNode(azure_instance=instance, credentials=self._credentials[0], parent_cluster=self, node_prefix=self.node_prefix, node_index=node_index, base_logdir=self.logdir, dc_idx=dc_idx, rack=rack)
            node.init()
            return node
    ...
    def _create_instances(self, count, dc_idx=0, instance_type=None) -> List[VmInstance]:
        ...
        for node_index in range(self._node_index + 1, self._node_index + count + 1):
            definitions.append(self._definition_builder.build_instance_definition(region=region, node_type=self.node_type, index=node_index, instance_type=instance_type))
        return provision_instances_with_fallback(self.provisioners[dc_idx], definitions=definitions, pricing_model=pricing_model, fallback_on_demand=self.params.get("instance_provision_fallback_on_demand"))
...
class ScyllaAzureCluster(cluster.BaseScyllaCluster, AzureCluster):
    def __init__(self, image_id, root_disk_size, provisioners: List[AzureProvisioner], credentials, instance_type="Standard_L8s_v3", user_name="ubuntu", user_prefix=None, n_nodes=3, params=None, region_names=None):
...
class LoaderSetAzure(cluster.BaseLoaderSet, AzureCluster):
    def __init__(self, image_id, root_disk_size, provisioners, credentials, instance_type="Standard_D2_v4", user_name="centos", user_prefix=None, n_nodes=1, params=None, region_names=None):
...
class MonitorSetAzure(cluster.BaseMonitorSet, AzureCluster):
    def __init__(self, image_id, root_disk_size, provisioners, credentials, instance_type="Standard_D2_v4", user_name="centos", user_prefix=None, n_nodes=1, targets=None, params=None, region_names=None):
