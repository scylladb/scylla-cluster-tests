...
class AzureNode(cluster.BaseNode):
    ...
    def _configure_secondary_nics_os(self):
        ...
        self.remoter.sudo(
            f"bash -c 'cat > {service_path}' << '{HEREDOC_DELIMITER}'\n{service_unit}\n{HEREDOC_DELIMITER}"
        )
    ...
    def _public_ip_of(self, ip_configuration, device_index: int) -> str | None:
        ...
        `expand=IPConfigurations/PublicIPAddress`. The provisioner holds the full resource - it
        re-reads every Public IP it creates - so the address comes from there, with whatever the
        NIC happens to carry preferred when it is populated.
        """
        ...
        return self._instance._provisioner.public_ip_address(self._instance.name, version, device_index)
    ...
    def _get_ipv6_ip_address(self) -> str:
        """IPv6 address of the node, empty when the run did not ask for IPv6.

        The routable address where the run has one, otherwise the VNet-local (ULA) address, which
        is all a run that asked for no IPv6 Public IP ever has. Prefers what the API reports, and
        asks the OS only as a fallback - which needs SSH, so before the node has a remoter a
        missing address is an error rather than an empty string.
        """
        if self.scylla_network_configuration:
        ...
        return self._os_ipv6_address()

    def _os_ipv6_address(self) -> str:
        """VNet-local (ULA) IPv6 of the node's primary interface, as the OS sees it.

        Never the routable address: an Azure Public IP is reached by DNAT and never appears on an
        interface. Pinned to the primary interface rather than taking whichever the OS listed
        first - a secondary NIC carries a ULA of its own, and every other branch of
        `_get_ipv6_ip_address` reports the primary one.
        """
        if not (addresses := self._discover_ipv6_from_os()):
            return ""
        primary = self.network_interfaces[0].device_name if self.network_interfaces else ""
        device = primary if primary in addresses else sorted(addresses)[0]
        return next(iter(addresses[device]), "")
