...
class ScyllaCloudCluster(cluster.BaseScyllaCluster, cluster.BaseCluster):
    ...
    def __init__(self,
        ...
        self.vpc_peer_id = None
        self.vpc_peer_details = None
        self._aws_region = None

        if self.vpc_peering_enabled and self._cloud_provider == 'aws':
            if region_name := params.cloud_provider_params.get('region'):
    ...
    def destroy(self):
        ...
        if self.vpc_peering_enabled and self.vpc_peer_id:
    ...
    def setup_vpc_peering(self, dc_id: int) -> None:
        ...
        self.log.info("Setting up VPC peering for cluster %s", self._cluster_id)
        ...
        self.vpc_peer_id = self._api_client.create_vpc_peer(
            account_id=self._account_id,
            cluster_id=self._cluster_id,
        ...
        self.vpc_peer_details = self._api_client.get_vpc_peer_details(
            account_id=self._account_id, cluster_id=self._cluster_id, peer_id=self.vpc_peer_id)
        self.log.info("VPC peering created successfully. Peer ID: %s", self.vpc_peer_id)
    ...
    def _get_sct_vpc_info(self) -> dict[str, Any]:
        ...
        if self._cloud_provider == 'aws':
            ...
            # TODO: Implement GCP VPC info retrieval
            raise NotImplementedError("GCP VPC info retrieval not yet implemented")
        raise VpcPeeringError(f"Unsupported cloud provider for VPC peering: {self._cloud_provider}")
    ...
    def configure_vpc_networking(self) -> None:
        ...
        self.log.info("Configuring VPC peering %s", self.vpc_peer_id)
        if self._cloud_provider == 'aws':
            self.configure_aws_route_tables()
        elif self._cloud_provider == 'gce':
            self.configure_gcp_firewall_rules()
        else:
            raise VpcPeeringError(f"Unsupported provider: {self._cloud_provider}")
        self.log.info("VPC peering %s configuration completed", self.vpc_peer_id)
    ...
    def configure_aws_route_tables(self) -> None:
        ...
        peer_conn_id = self.vpc_peer_details.get('externalId')
        # SCT has SCT_SUBNET_PER_AZ=2 route tables indexed 0 and 1, plus the main route table (None)
        for index in [None, 0, 1]:
            route_table = self._aws_region.sct_route_table(index=index)
            if not route_table:
                continue

            try:
                self.log.debug("Adding route for %s via %s to route table %s (index %s)",
                               self.cloud_cidr, peer_conn_id, route_table.id, index)
                route_table.create_route(
                    DestinationCidrBlock=self.cloud_cidr, VpcPeeringConnectionId=peer_conn_id)
            except Exception as e:  # noqa: BLE001
                if 'RouteAlreadyExists' in str(e):
                    self.log.debug("Route for %s already exists in route table %s", self.cloud_cidr, route_table.id)
                else:
                    self.log.error(
                        "Failed to add route for %s to route table %s: %s", self.cloud_cidr, route_table.id, e)
                    raise

    def configure_gcp_firewall_rules(self) -> None:
        # TODO: Implement GCP firewall configuration
        raise NotImplementedError("GCP firewall configuration not yet implemented")

    def cleanup_vpc_peering(self) -> None:
        """Clean up VPC peering connection and local network configuration"""
        if not self.vpc_peer_id:
            return

        self.log.info("Cleaning up VPC peering %s", self.vpc_peer_id)
        try:
            if self._cloud_provider == 'aws':
                self.cleanup_aws_route_tables()
            elif self._cloud_provider == 'gce':
                self.cleanup_gcp_firewall_rules()
            self._api_client.delete_vpc_peer(
                account_id=self._account_id, cluster_id=self._cluster_id, peer_id=self.vpc_peer_id)
        except Exception as e:  # noqa: BLE001
            self.log.error("Error during VPC peering cleanup: %s", e)
        self.log.info("VPC peering %s cleanup completed", self.vpc_peer_id)
    ...
    def cleanup_gcp_firewall_rules(self) -> None:
        # TODO: Implement GCP firewall cleanup
        pass

    def accept_aws_vpc_peering_connection(self) -> None:
        """Accept the VPC peering connection on SCT/AWS side"""
        aws_peer_conn_id = self.vpc_peer_details['externalId']
        try:
            self._aws_region.client.accept_vpc_peering_connection(VpcPeeringConnectionId=aws_peer_conn_id)

            def check_peering_status():
                try:
                    connections = self._aws_region.client.describe_vpc_peering_connections(
                        VpcPeeringConnectionIds=[aws_peer_conn_id]
                    ).get('VpcPeeringConnections', [])
                    if connections:
                        status = connections[0]['Status']['Code']
                        self.log.debug("VPC peering connection %s status: %s", aws_peer_conn_id, status)
                        return status == 'active'
                    return False
                except Exception as e:  # noqa: BLE001
                    self.log.debug("Error checking VPC peering status: %s", e)
                    return False

            wait.wait_for(
                func=check_peering_status,
                step=10,
                text=f"Waiting for VPC peering connection {aws_peer_conn_id} to become active",
                timeout=180,
                throw_exc=True)
        except Exception as e:  # noqa: BLE001
            raise VpcPeeringError(f"Failed to accept VPC peering connection {aws_peer_conn_id} on SCT side: {e}") from e
        self.log.debug("VPC peering connection is accepted on SCT side")
