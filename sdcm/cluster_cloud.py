...
class CloudNode(cluster.BaseNode):
    ...
    def set_keep_alive(self):
        # only log for now for PoC
        self.log.info("Setting keep alive for node %s", self.name)
        return True

    def restart(self):
        # there is no dedicated Scylla Cloud API for node restart;
        # this should be implemented after approach to accessing cloud nodes is defined
        pass

    def terminate(self):
        # there is no dedicated Scylla Cloud API for node termination;
        # it should be handled at the cluster level through resizing
        self.log.warning("Individual node termination is not supported in Scylla Cloud. "
...
class CloudCluster(cluster.BaseCluster):
    """Base cluster class for Scylla Cloud"""

    def __init__(self, cloud_api_client: ScyllaCloudAPIClient, cluster_prefix='cluster',
                 node_prefix='node', n_nodes=3, params=None, region_names=None,
                 node_type=None, add_nodes=True, cluster_uuid=None):

        self._api_client = cloud_api_client
        self._account_id = params.get('cloud_account_id') or cloud_api_client.get_current_account_id()
        self._cluster_id = params.get('cloud_cluster_id') if params else None
    ...
    def _prepare_cluster_config(self, node_count, instance_type):
        cloud_provider = self.params.get('cloud_provider').upper() or 'AWS'
        ...
        instance_type_name = instance_type or self.params.get('cloud_instance_type_db')
        ...
        return {
            'account_id': self.params.get('cloud_account_id'),
            'cluster_name': self.name,
            'scylla_version': self.params.get('scylla_version'),
            'cidr_block': self.params.get('cloud_cluster_network') or '10.0.0.0/16',
            'broadcast_type': "PUBLIC",
            'allowed_ips': self.params.get('cloud_allowed_ips'),
            'cloud_provider_id': provider_id,
            'region_id': region_id,
            'instance_id': instance_id,
            'replication_factor': self.params.get('cloud_replication_factor'),
            'number_of_nodes': node_count,
            'account_credential_id': self.params.get('cloud_credential_id'),
            'free_trial': self.params.get('cloud_free_trial') or False,
            'user_api_interface': "CQL",
            'enable_dns_association': self.params.get('cloud_enable_dns') or True,
            'jump_start': False,
    ...
    def _resize_cluster(self, count, dc_idx, rack, instance_type):
        ...
        raise ScyllaCloudError("Not yet implemented in POC")
    ...
    def get_node_ips_param(self, public_ip=True):
        return 'cloud_nodes_public_ip' if public_ip else 'cloud_nodes_private_ip'


class ScyllaCloudCluster(cluster.BaseScyllaCluster, CloudCluster):
    """Scylla DB cluster running on Scylla Cloud"""

    def __init__(self, cloud_api_client: ScyllaCloudAPIClient, user_prefix=None,
                 n_nodes=3, params=None, region_names=None,
                 node_type='scylla-db', add_nodes=True):
...
class LoaderSetCloud(cluster.BaseLoaderSet, CloudCluster):
    """Loader nodes for Scylla Cloud clusters"""

    def __init__(self, cloud_api_client: ScyllaCloudAPIClient, user_prefix=None,
                 n_nodes=1, params=None, region_names=None):
        node_prefix = cluster.prepend_user_prefix(user_prefix, 'loader-node')
        cluster_prefix = cluster.prepend_user_prefix(user_prefix, 'loader-set')

        cluster.BaseLoaderSet.__init__(self, params=params)
        CloudCluster.__init__(
            self,
            cloud_api_client=cloud_api_client,
            cluster_prefix=cluster_prefix,
            node_prefix=node_prefix,
    ...
    def _create_instance(self, name, dc_idx, rack, instance_type):
        self.log.warning("Loader node provisioning not implemented for Scylla Cloud POC")

    def add_nodes(self, count, ec2_user_data='', dc_idx=0, rack=0, enable_auto_bootstrap=False, instance_type=None):
        self.log.warning("Not implemented in POC")
        return []
