...
def xcloud_super_if_supported(method):
    ...
    def wrapper(self, *args, **kwargs):
        if getattr(self, "xcloud_connect_supported", False):
            ...
            self.log.warning(f"Unable to find owner class '{owner_class_name}' in MRO for {method.__name__}. Using fallback super() call which may cause issues with deep inheritance.")
...
class CloudNode(cluster.BaseNode):
    ...
    def __init__(self, cloud_instance_data: dict[str, Any], parent_cluster: cluster.BaseScyllaCluster, node_prefix: str = "node", node_index: int = 1, base_logdir: str | None = None, dc_idx: int = 0, rack: int = 0):
        ...
        super().__init__(name=name, parent_cluster=parent_cluster, base_logdir=base_logdir, node_prefix=node_prefix, dc_idx=dc_idx, rack=rack)
    ...
    def tags(self) -> dict[str, str]:
        return {**super().tags, "NodeIndex": str(self.node_index), "CloudProvider": "scylla-cloud", "NodeId": str(self._node_id)}
    ...
    def scylla_version_detailed(self):
        return super().scylla_version_detailed if self.xcloud_connect_supported else self.parent_cluster.params.get("scylla_version")


class CloudVSNode(CloudNode):
    """A Vector Search node running on Scylla Cloud"""

    def __init__(self, cloud_instance_data: dict[str, Any], parent_cluster: cluster.BaseScyllaCluster, node_prefix: str = "vs-node", node_index: int = 1, base_logdir: str | None = None, dc_idx: int = 0, rack: int = 0):
        super().__init__(cloud_instance_data=cloud_instance_data, parent_cluster=parent_cluster, node_prefix=node_prefix, node_index=node_index, base_logdir=base_logdir, dc_idx=dc_idx, rack=rack)


class CloudManagerNode(CloudNode):
    """A Scylla Manager node running on Scylla Cloud"""

    def __init__(self, parent_cluster: cluster.BaseScyllaCluster, node_prefix: str = "manager", node_index: int = 1, base_logdir: str | None = None, dc_idx: int = 0, rack: int = 0):
        # minimal cloud_instance_data for manager node (most of the node details will be fetched via SDM)
        cloud_instance_data = {"id": None, "privateIp": None, "publicIp": None, "status": "ACTIVE", "instanceType": "CloudManaged"}
    ...
    def _init_remoter(self, ssh_login_info=None):
        ...
        if localhost.xcloud_connect_supported(self.parent_cluster.params):
            ...
            ssh_login_info = localhost.xcloud_connect_get_manager_ssh_address(cluster_id=self._cluster_id, sdm_environment=sdm_env)
            self.remoter = RemoteCmdRunner(**ssh_login_info)
        else:
...
class ScyllaCloudCluster(cluster.BaseScyllaCluster, cluster.BaseCluster):
    ...
    def __init__(self, cloud_api_client: ScyllaCloudAPIClient, user_prefix: str | None = None, n_nodes: int = 3, params: dict[str, Any] | None = None, node_type: str = "scylla-db", add_nodes: bool = True, allowed_ips: list | None = None):
        ...
        super().__init__(cluster_prefix=cluster_prefix, node_prefix=node_prefix, n_nodes=n_nodes, params=params, region_names=params.cloud_provider_params.get("region"), node_type=node_type, add_nodes=add_nodes)
    ...
    @cached_property
    def dc_id(self) -> int:
        return self._api_client.get_cluster_details(account_id=self._account_id, cluster_id=self._cluster_id, enriched=True)["dc"]["id"]

    def _resolve_cluster_id(self) -> int:
        """Resolve cluster ID from test_id for cluster reuse"""
        search_pattern = str(self.uuid)[:8]
        clusters = self._api_client.search_clusters_by_name_pattern(account_id=self._account_id, name_pattern=search_pattern)
        if not clusters:
            raise ScyllaCloudError(f"No Scylla Cloud cluster found with test_id pattern '{search_pattern}' in name.\nCheck that the cluster wasn't deleted and test_id is correct. Full test_id: {self.uuid}")

        cluster = clusters[0]
        cluster_id = cluster.get("id")
        self.log.info("Found cluster to reuse: '%s' (ID: %s, Status: %s)", cluster.get("clusterName"), cluster_id, cluster.get("status"))
        return cluster_id

    def _create_node(self, cloud_instance_data: dict[str, Any], node_index: int, dc_idx: int, rack: int, node_class: type[CloudNode] = CloudNode, node_prefix: str | None = None) -> CloudNode:
        try:
            node = node_class(cloud_instance_data=cloud_instance_data, parent_cluster=self, node_prefix=node_prefix or self.node_prefix, node_index=node_index, base_logdir=self.logdir, dc_idx=dc_idx, rack=rack)
            node.init()
            return node
        except Exception as e:
            raise ScyllaCloudError(f"Failed to create node: {e}") from e

    def add_nodes(self, count: int, ec2_user_data: str = "", dc_idx: int = 0, rack: int = 0, enable_auto_bootstrap: bool = False, instance_type: str | None = None) -> list[CloudNode]:
    ...
    def _create_cluster(self, count: int, dc_idx: int, rack: int, enable_auto_bootstrap: bool, instance_type: str | None) -> list[CloudNode]:
        ...
        self._cluster_id = self._api_client.get_cluster_request_details(account_id=self._account_id, request_id=self._cluster_request_id)["clusterId"]
        ...
        self.log.info("Successfully created cluster %s with %s DB nodes%s%s", self.name, len(nodes), " and {} VS nodes".format(len(vs_nodes)) if vs_nodes else "", " and Manager node" if self.manager_node else "")

        return nodes

    def _init_nodes_from_data(self, nodes_data: list[dict[str, Any]], node_class: type[CloudNode] = CloudNode, node_prefix: str | None = None, dc_idx: int = 0, rack: int = 0) -> list[CloudNode]:
        ...
        return [self._create_node(cloud_instance_data=node_data, node_index=i, dc_idx=dc_idx, rack=rack, node_class=node_class, node_prefix=node_prefix) for i, node_data in enumerate(nodes_data, start=1)]
    ...
    def _init_nodes_from_cluster(self, count: int, dc_idx: int, rack: int) -> list[CloudNode]:
        ...
        cluster_nodes = self._api_client.get_cluster_nodes(account_id=self._account_id, cluster_id=self._cluster_id, enriched=True)
        ...
        created_nodes = self._init_nodes_from_data(nodes_data=cluster_nodes[:count], node_class=CloudNode, dc_idx=dc_idx, rack=rack)
    ...
    def vs_nodes_data(self) -> list[dict]:
        ...
        vs_info = self._api_client.get_vector_search_nodes(account_id=self._account_id, cluster_id=self._cluster_id, dc_id=self.dc_id)
        return [node for az in vs_info.get("availabilityZones", []) for node in az.get("nodes", [])]
    ...
    def _init_vs_nodes_from_cluster(self) -> list[CloudVSNode]:
        ...
        created_nodes = self._init_nodes_from_data(nodes_data=self.vs_nodes_data, node_class=CloudVSNode, node_prefix="vs-node", dc_idx=0, rack=0)
    ...
    def _prepare_cluster_config(self, node_count: int, instance_type: str) -> dict[str, Any]:
        ...
        instance_id = self._api_client.get_instance_id_by_name(cloud_provider_id=self.provider_id, region_id=self.region_id, instance_type_name=instance_type_name)
        ...
        if self.vpc_peering_enabled:
            try:
                cidr_manager = CidrPoolManager(cidr_base=self.vpc_peering_params["cidr_pool_base"], subnet_size=self.vpc_peering_params["cidr_subnet_size"])
                cidr_block = cidr_manager.get_available_cidr(cloud_provider=self._cloud_provider, region=self.params.cloud_provider_params.get("region"))
        ...
        vs_config = {"defaultNodes": int(self.params.get("n_vector_store_nodes")), "defaultInstanceTypeId": XCLOUD_VS_INSTANCE_TYPES[self._cloud_provider][self.params.get("instance_type_vector_store")]} if self._deploy_vs_nodes else None

        expiration_hours = compute_cluster_exp_hours(self.test_config.TEST_DURATION, self.test_config.should_keep_alive(self.node_type))
    ...
    def _wait_for_cluster_ready(self, timeout: int = 600) -> None:
        ...
        def check_cluster_status():
            try:
                cluster_details = self._api_client.get_cluster_details(account_id=self._account_id, cluster_id=self._cluster_id, enriched=True)
        ...
        wait.wait_for(func=check_cluster_status, step=15, text="Waiting for cluster to be ready", timeout=timeout, throw_exc=True)
    ...
    def _wait_for_vs_nodes_ready(self, timeout: int = 600) -> None:
        ...
        wait.wait_for(func=check_vs_nodes_status, step=15, text="Waiting for Vector Search nodes to be ready", timeout=timeout, throw_exc=True)
    ...
    def _wait_for_manager_node_ready(self, timeout: int = 600) -> None:
        ...
        cluster_requests = self._api_client.get_cluster_requests(account_id=self._account_id, cluster_id=self._cluster_id)
        ...
        def is_manager_ready():
            details = self._api_client.get_cluster_request_details(account_id=self._account_id, request_id=mgr_request_id)
        ...
        wait.wait_for(func=is_manager_ready, step=15, text="Waiting for Manager node installation to complete", timeout=timeout, throw_exc=True)
    ...
    def destroy(self):
        ...
        if self._cluster_id:
            self._api_client.delete_cluster(account_id=self._account_id, cluster_id=self._cluster_id, cluster_name=self.name)
    ...
    def _reuse_existing_cluster(self):
        ...
        db_nodes = self._api_client.get_cluster_nodes(account_id=self._account_id, cluster_id=self._cluster_id, enriched=True)
        if len(db_nodes) != self.params.get("n_db_nodes"):
            raise ScyllaCloudError("DB node count mismatch. Update 'n_db_nodes' in test config or provision a new cluster.")
        if db_nodes[0].get("instance", {}).get("externalId") != self.params.cloud_provider_params.get("instance_type_db"):
            raise ScyllaCloudError("DB node instance type mismatch. Update DB instance type in test config or provision a new cluster.")
        ...
        if version != expected_version:
            self.log.warning("Scylla version mismatch: '%s' in cluster, '%s' in test config. Continuing with existing cluster version.", version, expected_version)

        # validate VS nodes config parameters
        vs_nodes = self._api_client.get_vector_search_nodes(account_id=self._account_id, cluster_id=self._cluster_id, dc_id=self.dc_id).get("availabilityZones") or []
        if vs_nodes:
            vs_nodes_per_az = [n for n in vs_nodes[0].get("nodes", []) if n.get("status") not in ("DELETED", "PENDING_DELETE")]
            if vs_nodes_per_az and len(vs_nodes_per_az) != int(self.params.get("n_vector_store_nodes")):
                raise ScyllaCloudError("Vector Search node count mismatch. Update 'n_vector_store_nodes' in test config or provision a new cluster.")
        ...
        self._cluster_created = True
        self.log.info("Successfully reused cluster '%s' with %s DB nodes%s%s", self.name, len(db_nodes), f" and {len(vs_nodes)} VS nodes" if vs_nodes else "", " and Manager node" if self.manager_node else "")
    ...
    def update_seed_provider(self):
        ...
        self.log.debug("Skip updating seed provider on Scylla Cloud, pending until approach to SSHing/accessing nodes is developed")
    ...
    def check_nodes_up_and_normal(self, nodes=None, verification_node=None):
        ...
        if down_nodes:
            raise cluster.ClusterNodesNotReady(f"Nodes {','.join([node.name for node in down_nodes])} are not in ACTIVE/NORMAL state")

    def _get_cluster_credentials(self):
        """Retrieve default credentials for password authentification"""
        connection_details = self._api_client.get_cluster_connection(account_id=self._account_id, cluster_id=self._cluster_id)
    ...
    def _retrieve_vpc_peering_info(self) -> None:
        ...
        self.vpc_peering_details = self._api_client.get_vpc_peer_details(account_id=self._account_id, cluster_id=self._cluster_id, peer_id=self.vpc_peering_id)
    ...
    def setup_vpc_peering(self, dc_id: int) -> None:
        ...
        if ipaddress.ip_network(sct_vpc_info["cidr"]).overlaps(ipaddress.ip_network(self.cloud_cidr)):
            raise VpcPeeringError(f"SCT VPC CIDR {sct_vpc_info['cidr']} overlaps with Scylla Cloud VPC CIDR {self.cloud_cidr}")

        self.vpc_peering_id = self._api_client.create_vpc_peer(
            account_id=self._account_id, cluster_id=self._cluster_id, vpc_id=sct_vpc_info["vpc_id"], cidr_block=sct_vpc_info["cidr"], owner_id=sct_vpc_info["owner_id"], region_id=self.region_id, dc_id=dc_id, allow_cql=True
        ).get("id")
        self.vpc_peering_details = self._api_client.get_vpc_peer_details(account_id=self._account_id, cluster_id=self._cluster_id, peer_id=self.vpc_peering_id)
    ...
    def _get_sct_vpc_info(self) -> dict[str, Any]:
        ...
        if self._cloud_provider == "aws":
            return {"vpc_id": self._aws_region.sct_vpc.vpc_id, "cidr": str(self._aws_region.vpc_ipv4_cidr), "owner_id": self._aws_region.sct_vpc.owner_id}
        if self._cloud_provider == "gce":
            return {"vpc_id": self._gce_region.network.name, "cidr": self._gce_region.region_subnet.ip_cidr_range, "owner_id": self._gce_region.project}
    ...
    def cloud_cidr(self) -> str:
        ...
        return self._api_client.get_cluster_details(account_id=self._account_id, cluster_id=self._cluster_id, enriched=True)["dc"]["cidrBlock"]
    ...
    def configure_vpc_networking(self) -> None:
        ...
        if self._cloud_provider == "aws":
            ...
            self._gce_region.add_network_peering(peering_name=self.gcp_peering_name, peer_project=self.vpc_peering_details.get("projectId"), peer_net=self.vpc_peering_details.get("networkName"), wait_for_active=True)
        ...
        self.log.info("%s side VPC peering %s configuration completed", self._cloud_provider.upper(), self.vpc_peering_id)
    ...
    def configure_aws_route_tables(self) -> None:
        ...
        for route_table in self._aws_region.sct_route_tables:
            try:
                self.log.debug("Adding route for %s via %s to route table %s", self.cloud_cidr, peering_id, route_table.id)
                ...
                if "RouteAlreadyExists" in str(e):
                    ...
                    raise VpcPeeringError(f"Failed to add route for {self.cloud_cidr} to route table {route_table.id}: {e}") from e
    ...
    def cleanup_vpc_peering(self) -> None:
        ...
        try:
            ...
            self._api_client.delete_vpc_peer(account_id=self._account_id, cluster_id=self._cluster_id, peer_id=self.vpc_peering_id)
        except Exception as e:  # noqa: BLE001
    ...
    def cleanup_aws_route_tables(self) -> None:
        ...
        for route_table in self._aws_region.sct_route_tables:
            ...
            for route in routes:
                if route.get("DestinationCidrBlock") == self.cloud_cidr and route.get("VpcPeeringConnectionId") == peering_id:
                    try:
                        self.log.debug("Removing route for %s from route table %s", self.cloud_cidr, route_table.id)
                        self._aws_region.client.delete_route(RouteTableId=route_table.id, DestinationCidrBlock=self.cloud_cidr)
                    except Exception as e:  # noqa: BLE001
                        if "InvalidRoute.NotFound" in str(e):
                            self.log.debug("Route for %s already removed from route table %s", self.cloud_cidr, route_table.id)
                        else:
                            self.log.warning("Failed to remove route for %s from route table %s: %s", self.cloud_cidr, route_table.id, e)
    ...
    def accept_aws_vpc_peering_connection(self) -> None:
        ...
        try:
            ...
            def check_peering_status():
                try:
                    connections = self._aws_region.client.describe_vpc_peering_connections(VpcPeeringConnectionIds=[peering_id]).get("VpcPeeringConnections", [])
                    if connections:
            ...
            wait.wait_for(func=check_peering_status, step=10, text=f"Waiting for AWS VPC peering connection {peering_id} to become active", timeout=180, throw_exc=True)
        except Exception as e:  # noqa: BLE001
