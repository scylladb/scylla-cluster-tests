...
def restore_monitoring_stack(test_id, date_time=None):  # pylint: disable=too-many-return-statements,too-many-locals
    ...
    if sorted(monitoring_data_arch) != sorted(monitoring_stack_arch):
        LOGGER.error(
            "Prometheus data is not fitting to to monitoring stack. Clusters are not same."
            "\n Prometheus data for clusters: %s"
            "\nMonitoring stack for clusters: %s",
            monitoring_data_arch,
            monitoring_stack_arch,
        )
        return False
    ...
    for monitoring_cluster, monitoring_arch in monitoring_stack_arch.items():
        ...
        monitoring_data_dir = create_monitoring_data_dir(
            monitoring_stack_base_dir, monitoring_data_arch[monitoring_cluster], tenant_dir=monitoring_cluster
        )
        ...
        dashboard_file = get_nemesis_dashboard_file_for_cluster(
            base_dir=monitoring_stack_base_dir,
        ...
        status = restore_grafana_dashboards_and_annotations(
            monitoring_stack_dir,
...
def get_monitoring_stack_scylla_version(monitoring_stack_dir):
    try:
        ...
        if not requests.head(
            f"https://github.com/scylladb/scylla-monitoring/tree/{monitoring_version}"
            f"/grafana/build/ver_{scylla_version}"
        ).ok:
