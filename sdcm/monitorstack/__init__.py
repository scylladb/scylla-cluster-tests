...
def restore_monitoring_stack(test_id, date_time=None):  # noqa: PLR0911
    ...
    for monitoring_cluster, monitoring_arch in monitoring_stack_arch.items():
        ...
        monitoring_data_dir = create_monitoring_data_dir(monitoring_stack_base_dir, monitoring_data_arch[monitoring_cluster], tenant_dir=monitoring_cluster)
        ...
        if not monitoring_stack_dir or not monitoring_data_dir:
            LOGGER.error("Creating monitoring stack directories failed:\ndata_dir: %s; stack_dir: %s", monitoring_data_dir, monitoring_stack_dir)
        _, scylla_version = get_monitoring_stack_scylla_version(monitoring_stack_dir)

        containers_ports = run_monitoring_stack_containers(monitoring_stack_dir, monitoring_data_dir, scylla_version, tenants_number=len(monitoring_stack_arch))
        if not containers_ports:
            return False

        # To support multi-tenant cluster
        dashboard_file = get_nemesis_dashboard_file_for_cluster(base_dir=monitoring_stack_base_dir, archive=monitoring_arch, file_name_for_search="scylla-dash-per-server-nemesis")
        LOGGER.info("Nemesis dashboard file for the cluster %s is '%s'", monitoring_cluster, dashboard_file)
        status = restore_grafana_dashboards_and_annotations(monitoring_stack_dir, grafana_docker_port=containers_ports["grafana_docker_port"], sct_dashboard_file=dashboard_file)
        if not status:
            return False
...
def get_monitoring_stack_archive(test_id, date_time):
    ...
    if date_time:
        found_archives = [archive for archive in monitoring_stack_archives if archive["date"] == datetime.datetime.strptime(date_time, "%Y%m%d_%H%M%S")]
...
def extract_monitoring_stack_archive(downloaded_monitor_set_archive, monitoring_stack_base_dir):
    return extract_file_from_archive("monitoring_data_stack", downloaded_monitor_set_archive, monitoring_stack_base_dir)


def extract_monitoring_data_archive(downloaded_monitor_set_archive, monitoring_stack_base_dir):
    return extract_file_from_archive("prometheus_data", downloaded_monitor_set_archive, monitoring_stack_base_dir)
...
def get_monitoring_data_dir(base_dir):
    monitoring_data_dirs = [d for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d))]
...
def get_monitoring_stack_scylla_version(monitoring_stack_dir):
    try:
        ...
        if not requests.head(f"https://github.com/scylladb/scylla-monitoring/tree/{monitoring_version}/grafana/build/ver_{scylla_version}").ok:
...
def restore_grafana_dashboards_and_annotations(monitoring_dockers_dir, grafana_docker_port, sct_dashboard_file):
    status = []
    try:
        status.append(restore_sct_dashboards(grafana_docker_port=grafana_docker_port, sct_dashboard_file=sct_dashboard_file))
...
def restore_sct_dashboards(grafana_docker_port, sct_dashboard_file):
    ...
    try:
        ...
        if res.status_code != 200:
            ...
            raise ErrorUploadSCTDashboard("Error uploading dashboard {}. Error message {}".format(sct_dashboard_file, res.text))
    ...
    try:
        ...
        for an in annotations:
            ...
            if res.status_code != 200:
                ...
                raise ErrorUploadAnnotations("Error during uploading annotation {}. Error message {}".format(an, res.text))
...
def start_dockers(monitoring_dockers_dir, monitoring_stack_data_dir, scylla_version, tenants_number):
    ...
    lr = LocalCmdRunner()
    lr.run("cd {monitoring_dockers_dir}; ./kill-all.sh -g {graf_port} -m {alert_port} -p {prom_port}".format(**locals()), ignore_status=True, verbose=False)
    ...
    alert_manager_config.write_text(
        dedent("""
        global:
          resolve_timeout: 5m
        route:
...
def verify_monitoring_stack(containers_ports):
    ...
    checked_statuses.append(verify_prometheus_is_available(prometheus_docker_port=containers_ports["prometheus_docker_port"]))
...
def verify_dockers_are_running(containers_ports):
    ...
    if result.ok and docker_names:
        if f"{GRAFANA_DOCKER_NAME}-{grafana_docker_port}" in docker_names and f"{PROMETHEUS_DOCKER_NAME}-{prometheus_docker_port}" in docker_names:
...
def verify_grafana_is_available(grafana_docker_port=GRAFANA_DOCKER_PORT):
    ...
    for dashboard in GrafanaEntity.base_grafana_dashboards:
        try:
            ...
            result = MonitoringStack.get_dashboard_by_title(grafana_ip="localhost", port=grafana_docker_port, title=dashboard.title)
...
def get_monitoring_stack_services(ports):
    return [
        {"service": "Grafana service", "name": GRAFANA_DOCKER_NAME, "port": ports["grafana_docker_port"]},
...
def kill_running_monitoring_stack_services(ports=None):
    dockers_ports = ports or {"grafana_docker_port": GRAFANA_DOCKER_PORT, "prometheus_docker_port": PROMETHEUS_DOCKER_PORT, "alert_docker_port": ALERT_DOCKER_PORT}
