...
class GCENode(cluster.BaseNode):
    ...
    def __init__(
        ...
        ssh_login_info = {"hostname": None, "user": gce_image_username, "key_file": credentials.key_file, "extra_ssh_options": "-tt"}
    ...
    def _set_keep_alive(self) -> bool:
        return gce_set_labels(instances_client=self._gce_service, instance=self._instance, new_labels={"keep": "alive"}, project=self.project, zone=self.zone) and super()._set_keep_alive()

    def _set_keep_duration(self, duration_in_hours: int) -> None:
        self._refresh_instance_state()
        gce_set_labels(instances_client=self._gce_service, instance=self._instance, new_labels={"keep": str(duration_in_hours)}, project=self.project, zone=self.zone)
    ...
    def _instance_wait_safe(self, instance_method: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        try:
            ...
            raise cluster.NodeError(f"Timeout while running '{instance_method.__name__}' method on GCE instance '{self._instance.id}'") from None

    def _refresh_instance_state(self):
        instance = self._instance_wait_safe(self._gce_service.get, project=self.project, zone=self.zone, instance=self._instance.name)
    ...
    def restart(self):
        ...
        self._instance_wait_safe(self._gce_service.reset, instance=self._instance.name, project=self.project, zone=self.zone)
    ...
    def _get_ipv6_ip_address(self):
        self.log.warning("On GCE, VPC networks only support IPv4 unicast traffic. They do not support IPv6 traffic within the network.")
        return ""
...
class GCECluster(cluster.BaseCluster):
    ...
    def __init__(
        ...
        self._gce_zone_names: list[str] = [f"{region}-{availability_zone or random_zone(region)}" for region in gce_region_names]
        ...
        super().__init__(cluster_uuid=cluster_uuid, cluster_prefix=cluster_prefix, node_prefix=node_prefix, n_nodes=n_nodes, params=params, region_names=gce_region_names, node_type=node_type, add_nodes=add_nodes)
    ...
    def _get_root_disk_struct(self, name, disk_type="pd-standard", dc_idx=0):
        ...
        return disk_from_image(
            disk_type=self._get_disk_url(disk_type, dc_idx=dc_idx),
    ...
    def _get_local_ssd_disk_struct(self, name, index, interface="NVME", dc_idx=0):
        ...
        return disk_from_image(
            disk_type=self._get_disk_url("local-ssd", dc_idx=dc_idx),
            boot=False,
            auto_delete=True,
            type_="SCRATCH",
            device_name=device_name,
    ...
    def _get_disks_struct(self, name, dc_idx):
        gce_disk_struct = [self._get_root_disk_struct(name=name, disk_type=self._gce_image_type, dc_idx=dc_idx)]
        ...
        if self._add_disks:
            for disk_type, disk_size in self._add_disks.items():
                ...
                if disk_size:
                    gce_disk_struct.append(self._get_persistent_disk_struct(name=name, disk_size=disk_size, disk_type=disk_type, dc_idx=dc_idx))
    ...
    def _create_instance(self, node_index, dc_idx, spot=False, enable_auto_bootstrap=False, instance_type=None):
        def set_tags_as_labels(_instance: compute_v1.Instance):
            ...
            gce_set_labels(instances_client=self._gce_service, instance=_instance, new_labels=normalized_tags, project=self.project, zone=self._gce_zone_names[dc_idx])
    ...
    @retrying(n=3, sleep_time=900, allowed_exceptions=(google.api_core.exceptions.GoogleAPIError, google.api_core.exceptions.NotFound), message="Retrying to create a GCE node...", raise_on_exceeded=True)
    def _create_node_with_retries(self, name: str, dc_idx: int, create_node_params: dict[str, Any]) -> compute_v1.Instance:
        try:
            ...
            if "Instance failed to start due to preemption" in str(gbe) or "Quota exceeded" in str(gbe):
                try:
                    ...
                    LOGGER.warning("Attempted to destroy node: {name: %s, idx:%s}, but could not find it. Possibly the node was destroyed already.", name, dc_idx)
    ...
    def _create_instances(self, count, dc_idx=0, enable_auto_bootstrap=False, instance_type=None):
        ...
        for node_index in range(self._node_index + 1, self._node_index + count + 1):
            instances.append(self._create_instance(node_index, dc_idx, spot, enable_auto_bootstrap, instance_type=instance_type))
        return instances

    def _destroy_instance(self, name: str, dc_idx: int):
        target_node = self._get_instances_by_name(dc_idx=dc_idx, name=name)
        operation = self._gce_service.delete(instance=target_node, project=self.project, zone=self._gce_zone_names[dc_idx])
        wait_for_extended_operation(operation, "Wait for instance deletion")
