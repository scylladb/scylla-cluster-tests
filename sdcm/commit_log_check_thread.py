...
class CommitLogCheckThread:
    ...
    def run(custer_tester, duration):
        ...
        if "Not found" in get_max_disk_size_metric(custer_tester.db_cluster):
            CommitLogCheckErrorEvent(
                severity=Severity.WARNING,
                message="CommitLogCheckThread will not start due to current scylla version has no commitlog/metrics/max_disk_size endpoint ",
            ).publish()
            return
        ...
        try:
            ...
            trace = traceback.format_exc()
            CommitLogCheckErrorEvent(
                message=f"CommitLogCheckThread.__init__ failed with unexpected exception: {exc.__repr__()} with traceback {trace}"
            ).publish()
        else:
            try:
                thread.start()
                ...
                trace = traceback.format_exc()
                CommitLogCheckErrorEvent(
                    message=f"CommitLogCheckThread.start failed with unexpected exception: {exc.__repr__()} with traceback {trace}"
                ).publish()
