...
class PhysicalMachineNode(cluster.BaseNode):
    ...
    def __init__(self, name, parent_cluster: "PhysicalMachineCluster", public_ip, private_ip, credentials, base_logdir=None, node_prefix=None):
        ssh_login_info = {"hostname": None, "user": getattr(parent_cluster, "ssh_username", credentials.name), "key_file": credentials.key_file}
        self._public_ip = public_ip
        self._private_ip = private_ip
        super().__init__(name=name, parent_cluster=parent_cluster, base_logdir=base_logdir, ssh_login_info=ssh_login_info, node_prefix=node_prefix)
...
class PhysicalMachineCluster(cluster.BaseCluster):
    ...
    def _create_node(self, name, public_ip, private_ip):
        node = PhysicalMachineNode(name, parent_cluster=self, public_ip=public_ip, private_ip=private_ip, credentials=self.credentials[0], base_logdir=self.logdir, node_prefix=self.node_prefix)
        node.init()
        return node
    ...
    def add_nodes(self, count, ec2_user_data="", dc_idx=0, rack=0, enable_auto_bootstrap=False, instance_type=None):
        ...
        for node_index in range(count):
            ...
            self.nodes.append(self._create_node(node_name, self._node_public_ips[node_index], self._node_private_ips[node_index]))
...
class ScyllaPhysicalCluster(cluster.BaseScyllaCluster, PhysicalMachineCluster):
    def __init__(self, **kwargs):
        ...
        kwargs.update(dict(cluster_prefix=cluster.prepend_user_prefix(user_prefix, "db-cluster"), node_prefix=cluster.prepend_user_prefix(user_prefix, "db-node"), node_type="scylla-db"))
        super().__init__(**kwargs)
...
class LoaderSetPhysical(cluster.BaseLoaderSet, PhysicalMachineCluster):
    def __init__(self, **kwargs):
        ...
        if username := kwargs.pop("ssh_username", None):
            self._ssh_username = username
        kwargs.update(dict(cluster_prefix=cluster.prepend_user_prefix(user_prefix, "loader-set"), node_prefix=cluster.prepend_user_prefix(user_prefix, "loader-node"), node_type="loader"))
...
class MonitorSetPhysical(cluster.BaseMonitorSet, PhysicalMachineCluster):
    def __init__(self, **kwargs):
        ...
        if username := kwargs.pop("ssh_username", None):
            self._ssh_username = username
        kwargs.update(dict(cluster_prefix=cluster.prepend_user_prefix(user_prefix, "monitor-set"), node_prefix=cluster.prepend_user_prefix(user_prefix, "monitor-node"), node_type="monitor"))
