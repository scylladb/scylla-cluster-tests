...
class EventGroup(Enum):
    NODES_RELATED_EVENTS = ["ScyllaServerStatusEvent", "RepairEvent", "JMXServiceEvent", "DatabaseLogEvent", "BootstrapEvent", "NodetoolEvent", "DisruptionEvent", "InstanceStatusEvent", "CompactionEvent"]
...
class ParallelTimelinesReportGenerator:
    ...
    def read_events_file(self) -> None:
        ...
        with self.events_file.open(encoding="utf-8") as file:
            ...
            LOGGER.info('File "%s" has been read successfully. %d rows have been processed.', self.events_file, len(self.events))
    ...
    def prepare_scylla_nodes_event_data(self) -> None:
        ...
        if scylla_nodes_events:
            ...
            scylla_nodes_events_sorted = sorted(scylla_nodes_events, key=lambda x: (int(x.node_name.split("-")[1]), x.base))
    ...
    def prepare_prometheus_event_data(self) -> None:
        ...
        if prometheus_events_data:
            prometheus_events_data_sorted = sorted(prometheus_events_data, key=lambda x: (x.original_node_name, x.alert_name))
    ...
    def prepare_sct_event_data(self) -> None:
        ...
        if sct_events_data:
            sct_events_data_sorted = sorted(sct_events_data, key=lambda x: (x.base, x.original_node_name, x.nemesis_name))
    ...
    def prepare_stress_event_data(self) -> None:
        ...
        if stress_events_data:
            stress_events_data_sorted = sorted(stress_events_data, key=lambda x: (x.base, x.original_node_name, x.stress_cmd))
    ...
    def _append_group_data(self, group_name: str, event_data: Event) -> None:
        ...
        for data in group_by_name_list[0]["data"]:
            if event_data.chart_label == data["label"]:
                data["data"].append({"timeRange": [event_data.begin_timestamp, event_data.end_timestamp], "val": event_data.chart_value})
    ...
    def _process_raw_data(self, events_to_process: list) -> List[Event]:
        ...
        for event in selected_events:
            ...
            if not event.begin_timestamp:
                ...
                LOGGER.warning("Empty end_timestamp when period_type=end for event name=%s, id=%s", event.base, event.event_id)
    ...
    def create_report_file(self) -> None:
        ...
        rendered_template = template.render(chart_data=self.chart_data, max_height=max_height, max_line_height=max_line_height, test_id=self.test_id, cluster_name=self.cluster_name)
