...
class BaseEmailReporter:
    ...
    def send_report(self, results):
        ...
        for _ in range(4):
            try:
                ...
                email = smtp.prepare_email(subject=report_data["subject"], recipients=self.email_recipients, content=report, files=attachments)
                break
    ...
    def _generate_report_attachments(self, attachments_data):
        if attachments_data is None:
            ...
            self.save_html_to_file({}, html_file_path=attachment_file, template_str="<html><body>Attachment was excluded due to the size limitation</body></html>")
    ...
    def build_report(self, report_data):
        ...
        report_data["last_events"] = self._get_last_events(report_data, self.last_events_body_limit_per_severity, self.last_events_body_limit_total, self.last_events_limit, self.last_events_severities)
        ...
        if last_events and isinstance(last_events, dict):
            for severity, events in last_events.items():
                ...
                for event in reversed(events):
                    if len(event) >= category_size_limit - severity_events_length or len(severity_events) >= events_in_category_limit or total_size + len(event) > total_size_limit:
    ...
    def _check_if_last_events_over_the_limit(report_data, category_size_limit, total_size_limit, events_in_category_limit):
        ...
        if last_events and isinstance(last_events, dict):
            for events in last_events.values():
                severity_events_length = 0
                for event in reversed(events):
                    if len(event) >= category_size_limit - severity_events_length or severity_events_length >= events_in_category_limit or total_size + len(event) > total_size_limit:
                        return True
                    severity_events_length += len(event)
                    total_size += len(event)
        return False
...
class LongevityEmailReporter(BaseEmailReporter):
    ...
    def cut_report_data(self, report_data, attachments_data, reason):
        if attachments_data is not None:
            if self._check_if_last_events_over_the_limit(attachments_data, self.last_events_body_limit_per_severity_in_attachment // 3, self.last_events_body_limit_total_in_attachment // 3, self.last_events_limit_in_attachment // 3):
                attachments_data["last_events"] = self._get_last_events(
                    attachments_data, self.last_events_body_limit_per_severity_in_attachment // 3, self.last_events_body_limit_total_in_attachment // 3, self.last_events_limit_in_attachment // 3, self.last_events_severities_in_attachment
                )
                return report_data, attachments_data
            return report_data, None
        if self._check_if_last_events_over_the_limit(report_data, self.last_events_body_limit_per_severity // 3, self.last_events_body_limit_total // 3, self.last_events_limit // 3):
            report_data["last_events"] = self._get_last_events(report_data, self.last_events_body_limit_per_severity // 3, self.last_events_body_limit_total // 3, self.last_events_limit // 3, self.last_events_severities)
            return report_data, None
        return None, None
    ...
    def build_report_attachments(self, attachments_data, template_str=None):
        attachments = (self.build_email_report(attachments_data, template_str), self.build_issue_template(attachments_data))
        return attachments

    def build_email_report(self, attachments_data, template_str=None):
        report_file = os.path.join(self.logdir, "email_report.html")
        attachments_data["last_events"] = self._get_last_events(
            attachments_data, self.last_events_body_limit_per_severity_in_attachment, self.last_events_body_limit_total_in_attachment, self.last_events_limit_in_attachment, self.last_events_severities_in_attachment
        )
    ...
    def get_config_file_link(attachments_data):
        ...
        for config_file in attachments_data["config_files"]:
            ...
            last_commit = subprocess.run(["git", "rev-list", "HEAD", "-1", config_file], capture_output=True, text=True, check=True)
            config_files.append({"file": f"[{config_file.split('/')[-1]}]", "link": f"https://github.com/scylladb/scylla-cluster-tests/blob/{last_commit.stdout.strip()}/{config_file}"})
...
def get_running_instances_for_email_report(test_id: str, ip_filter: str = None):
    ...
    for instance in instances:
        ...
        if ip_filter not in public_ips:
            nodes.append([instance.name, ", ".join(public_ips) if public_ips else "N/A", instance.status.lower(), "gce", instance.zone.split("/")[-1]])
    ...
    for builder_name, containers in resources.get("containers", {}).items():
        for container in containers:
            container.reload()
            nodes.append([container.name, get_ip_address_of_container(container), container.status, "docker container", builder_name])
