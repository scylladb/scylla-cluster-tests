...
class GeminiStressThread(DockerBasedStressThread):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, test_cluster, oracle_cluster, loaders, stress_cmd, timeout=None,
                 params=None):  # pylint: disable=too-many-arguments
        ...
        self.gemini_default_flags = {
            ...
            # TODO: something with token-aware policy is not working, using round-robin for now
            'oracle-host-selection-policy': 'round-robin',
            'test-host-selection-policy': 'round-robin',
            ...
            'token-range-slices': 2048,  # Number of partitions
            'partition-key-buffer-reuse-size': 100,  # Internal Channel Size per partition value generation
        }
    ...
    def _generate_gemini_command(self):
        ...
        cmd = (
            ...
            "--profiling-port=6060 "
            "--bind=0.0.0.0:2121 "
            f"--oracle-cluster=\"{oracle_nodes}\" "
            f"--test-cluster=\"{test_nodes}\" "
            ...
            f"--outfile=\"/{self.gemini_result_file}\" "
        )

        if log_statements:
            cmd += f"--test-statement-log-file=\"/{self.gemini_test_statements_file}\" --oracle-statement-log-file=\"/{self.gemini_oracle_statements_file}\" "

        if credentials and '--test-username' not in cmd:
            cmd += f"--test-username=\"{credentials[0]}\" --test-password=\"{credentials[1]}\" "

        if credentials and '--oracle-username' not in cmd:
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        for file_name in [self.gemini_result_file, self.gemini_test_statements_file,
