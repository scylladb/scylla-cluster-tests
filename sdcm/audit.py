...
AUDIT_LOG_REGEX = re.compile(r"node=\"(?P<node>.*?)\" category=\"(?P<category>.*?)\" cl=\"(?P<consistency>.*?)\" error=\"(?P<error>.*?)\" keyspace=\"(?P<keyspace_name>.*?)\" query=\"(?P<operation>.*?)\" client_ip=\"(?P<source>.*?)\" table=\"(?P<table_name>.*?)\" username=\"(?P<username>.*?)\"")
