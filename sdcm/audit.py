...
class AuditLogReader:
    ...
    def read(self, from_datetime: Optional[datetime] = None, category: Optional[AuditCategory] = None, operation: Optional[str] = None, limit_rows: int = 1000) -> List[AuditLogRow]:
...
def get_audit_log_rows(node, from_datetime: Optional[datetime] = None, category: Optional[AuditCategory] = None, operation: Optional[str] = None, limit_rows: int = 1000) -> List[AuditLogRow]:
            ...
            if "!NOTICE" in line[:120] and "scylla-audit" in line[:120]:
                ...
                if match := AUDIT_LOG_REGEX.search(line):
                    ...
                    if category and found_audit_log_fields.get("category", "") != category:
                        continue
                    found_audit_log_fields["operation"] = re.sub(r"\\(.)", r"\1", found_audit_log_fields.get("operation", ""))
                    ...
                    yield AuditLogRow(date=event_date, event_time=event_time, **found_audit_log_fields)
                    found_rows += 1
...
class SyslogAuditLogReader(AuditLogReader):
    def read(self, from_datetime: Optional[datetime] = None, category: Optional[AuditCategory] = None, operation: Optional[str] = None, limit_rows: int = 1000) -> List:
        """Return audit log rows from syslog based on the given filters."""
        rows = []
        for node in self._cluster.nodes:
            rows += list(get_audit_log_rows(node, from_datetime=from_datetime, category=category, operation=operation, limit_rows=limit_rows))
        LOGGER.debug("Found %s audit log rows", len(rows))
        return rows
...
class Audit:
        ...
        for node in self._cluster.nodes:
            with node.remote_scylla_yaml() as scylla_yaml:
                ...
                if prev_audit_config and prev_audit_config != audit_config:
                    InfoEvent(message=f"audit configuration is inconsistent between nodes: {prev_audit_config} != {audit_config}", severity=Severity.ERROR).publish()
        return audit_config
    ...
    def configure(self, audit_configuration: AuditConfiguration):
        ...
        for node in self._cluster.nodes:
            ...
            with node.remote_scylla_yaml() as scylla_yaml:
                scylla_yaml.update(
                    {"audit": audit_configuration.store, "audit_categories": ",".join(audit_configuration.categories), "audit_tables": ",".join(audit_configuration.tables), "audit_keyspaces": ",".join(audit_configuration.keyspaces)}
                )
        self._cluster.restart_scylla()
        LOGGER.debug("Audit configuration completed")
        self._configuration = audit_configuration

    def get_audit_log(self, from_datetime: Optional[datetime] = None, category: Optional[AuditCategory] = None, operation: Optional[str] = None, limit_rows: int = 1000) -> List:
        """Return audit log rows based on the given filters."""
