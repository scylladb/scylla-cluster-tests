    ...
    def __init__(self, loader_node, loader_idx, ndbench_log_filename):
        ...
        for operation in self.collectible_ops:
            ...
            if gauge_name not in self.METRICS:
                ...
                self.METRICS[gauge_name] = metrics.create_gauge(gauge_name, "Gauge for ndbench metrics", ["instance", "loader_idx", "type"])
...
class NdBenchStressThread(DockerBasedStressThread):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.stress_cmd = f"./gradlew {timeout} -Dndbench.config.cass.host={self.node_list[0].cql_address} {self.stress_cmd} run"
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):
        ...
        docker = cleanup_context = RemoteDocker(loader, self.docker_image_name, extra_docker_opts=f"--network=host --security-opt seccomp=unconfined --label shell_marker={self.shell_marker}")
        ...
        with NdBenchStatsPublisher(loader, loader_idx, ndbench_log_filename=log_file_name), NdBenchStressEventsPublisher(node=loader, ndbench_log_filename=log_file_name), cleanup_context:
            try:
                result = docker.run(cmd=node_cmd, timeout=self.timeout + self.shutdown_timeout, ignore_status=True, log_file=log_file_name, verbose=True, retry=0, timestamp_logs=True)
            except Exception as exc:  # noqa: BLE001
                ...
                ndbench_finish_event = NdBenchStressEvent.finish(node=loader, stress_cmd=self.stress_cmd, log_file_name=log_file_name)
