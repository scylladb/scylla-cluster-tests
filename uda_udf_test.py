...
class UDAUDFTest(ClusterTester):
    ...
    def setUp(self):
        super().setUp()
        self.email_reporter = LongevityEmailReporter(email_recipients=self.params.get("email_recipients"), logdir=self.logdir)
    ...
    def run_uda_udf_thread(self) -> CassandraStressThread:
        ...
        uda_udf_thread = self.run_stress_thread(stress_cmd=uda_udf_cmd, stats_aggregate_cmds=False, round_robin=False)
    ...
    def _verify_udf_functions(self):
        row_query = UDVerification(name="row_query", query=f"SELECT * FROM {self.KEYSPACE_NAME}.{self.CF_NAME} LIMIT 1", verifier_func=lambda c2, c3, c7: all([c2, c3, c7]))
        verifications = [
            UDVerification(
                name="lua_var_length_counter",
                query=f"SELECT {self.KEYSPACE_NAME}.lua_var_length_counter(c7_text) AS result FROM {self.KEYSPACE_NAME}.{self.CF_NAME} LIMIT 1",
                verifier_func=lambda c2, c3, c7, query_response: len(c7) == query_response.result,
            ),
            UDVerification(
                name="wasm_plus", query=f"SELECT {self.KEYSPACE_NAME}.wasm_plus(c2_int, c3_int) AS result FROM {self.KEYSPACE_NAME}.{self.CF_NAME} LIMIT 1", verifier_func=lambda c2, c3, c7, query_response: c2 + c3 == query_response.result
            ),
            UDVerification(
                name="wasm_div", query=f"SELECT {self.KEYSPACE_NAME}.wasm_div(c2_int, c3_int) AS result FROM {self.KEYSPACE_NAME}.{self.CF_NAME} LIMIT 1", verifier_func=lambda c2, c3, c7, query_response: c2 // c3 == query_response.result
            ),
        ]
        ...
        with self.db_cluster.cql_connection_patient(self.db_cluster.get_node(), verbose=False) as session:
            ...
            assert row_query.verifier_func(c2_value, c3_value, c7_value), "Expected row values to not be None, at least one them was. c2_value: %s, c3_value: %s, c7 value: %s" % (c2_value, c3_value, c7_value)
    ...
    def _verify_uda_aggregates(self):
        uda_verification = UDVerification(name="my_avg", query="SELECT ks.my_avg(c2_int) AS result FROM ks.uda_udf USING TIMEOUT 120s", verifier_func=lambda verification_query_result, avg_response: verification_query_result == avg_response)
        ...
        assert uda_verification.verifier_func(verification_query_result, avg_result), "UDA verifivation failed. UDA result: %s, Builtin AVG result: %s" % (verification_query_result, avg_result)
