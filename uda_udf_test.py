...
class UDAUDFTest(ClusterTester):
    ...
    def _verify_udf_functions(self):
        ...
        verifications = [
            ...
            UDVerification(
                name="wasm_plus",
                query=f"SELECT {self.KEYSPACE_NAME}.wasm_plus(c2_int, c3_int) AS result FROM {self.KEYSPACE_NAME}.{self.CF_NAME} LIMIT 1",
                verifier_func=lambda c2, c3, c7, query_response: c2 + c3 == query_response.result,
            ),
            UDVerification(
                name="wasm_div",
                query=f"SELECT {self.KEYSPACE_NAME}.wasm_div(c2_int, c3_int) AS result FROM {self.KEYSPACE_NAME}.{self.CF_NAME} LIMIT 1",
                verifier_func=lambda c2, c3, c7, query_response: c2 // c3 == query_response.result,
            ),
        ]
        ...
        with self.db_cluster.cql_connection_patient(self.db_cluster.get_node(), verbose=False) as session:
            ...
            assert row_query.verifier_func(c2_value, c3_value, c7_value), (
                "Expected row values to not be None, at least one them was. c2_value: %s, c3_value: %s, c7 value: %s"
                % (c2_value, c3_value, c7_value)
            )
