...
class BaseYCSBPerformanceRegressionTest(PerformanceRegressionTest):
    ycsb_workloads: list[YcsbWorkload] = [
        ...
        YcsbWorkload(name="workloadd", detailed_name="Read Latest (95/0/5 read/update/insert ratio)", sub_type="read_latest"),
        YcsbWorkload(name="workloade", detailed_name="Short Range (95/5 scan/insert ratio)", sub_type="short_range"),
        YcsbWorkload(name="workloadf", detailed_name="Read-Modify-Write (50/50 read/read-modify-write ratio)", sub_type="read_write_modify"),
    ]
    ...
    def _create_prepare_cmds(self, workload: YcsbWorkload):
        self.params["prepare_write_cmd"] = [
            f"bin/ycsb -jvm-args='-Dorg.slf4j.simpleLogger.defaultLogLevel=OFF' load scylla -s -P workloads/{workload.name} -p recordcount={self.records_size} -p insertcount={self.records_size} {cmd}"
            for cmd in self.params["prepare_write_cmd"]
        ]

    def _create_stress_cmd(self, workload: YcsbWorkload):
        cmd = f"bin/ycsb -jvm-args='-Dorg.slf4j.simpleLogger.defaultLogLevel=OFF' run scylla -s -P workloads/{workload.name} -p recordcount={self.records_size} -p operationcount={self.records_size} {self.params['stress_cmd']}"
        return cmd
