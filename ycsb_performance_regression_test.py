...
class BaseYCSBPerformanceRegressionTest(PerformanceRegressionTest):
    ...
    def _create_stress_cmd(self, workload: YcsbWorkload):
        ...
        if workload.name == 'workloadf':
            # this workload can run operations again, hence need to be exactly
            # the same size of records
            cmd = re.sub(r'(operationcount)=\d*', rf'\1={self.records_size}', cmd)
        return cmd
