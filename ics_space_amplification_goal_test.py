...
class IcsSpaceAmplificationTest(LongevityTest):
    def _get_filesystem_available_size_list(self, node, start_time):
        ...
        available_size_res = self.prometheus_db.query(
            query=available_capacity_query, start=int(start_time), end=int(time.time())
        )
    ...
    def _get_max_used_capacity_over_time_gb(self, node, start_time) -> float:
        ...
        min_available_capacity_gb = (
            min([int(val[1]) for val in self._get_filesystem_available_size_list(node=node, start_time=start_time)])
            / GB_SIZE
        )
        max_used_capacity_gb = fs_size_gb - min_available_capacity_gb
        self.log.debug(
            "The maximum used filesystem capacity of %s for the last %s minutes is: %s GB/ %s GB",
            node.private_ip_address,
            time_interval_minutes,
            max_used_capacity_gb,
            fs_size_gb,
        )
    ...
    def _get_nodes_space_ampl_over_time_gb(self, dict_nodes_initial_capacity, start_time, written_data_size_gb=0):
        ...
        for node in self.db_cluster.nodes:
            ...
            dict_nodes_space_amplification[node.private_ip_address] = round(
                node_max_used_capacity_gb - written_data_size_gb - node_initial_capacity, 2
            )
            self.log.info(
                "Node %s used capacity changed from %s to %s.",
                node.private_ip_address,
    ...
    def measure_nodes_space_amplification_after_write(
        self, dict_nodes_initial_capacity, written_data_size_gb, start_time
    ):
        ...
        self.log.info(
            "Space amplification results after a write of: %s are: %s",
            written_data_size_gb,
            dict_nodes_space_amplification,
        )
        InfoEvent(
            message=f"Space amplification results after a write of: {written_data_size_gb} are: {dict_nodes_space_amplification}"
        ).publish()
    ...
    def test_ics_space_amplification_goal(self):  # noqa: PLR0914
        ...
        for sag in sag_testing_values:
            ...
            InfoEvent(
                message=f"Nodes used capacity before start overwriting data: {dict_nodes_capacity_before_overwrite_data}"
            ).publish()
            for node, capacity in dict_nodes_capacity_before_overwrite_data.items():
                if capacity > nodes_original_capacity_with_delta[node]:
                    TestFrameworkEvent(
                        ...
                        message=capacity_exceed_msg.format(node, capacity, nodes_original_capacity_with_delta[node]),
            ...
            self.measure_nodes_space_amplification_after_write(
                ...
                written_data_size_gb=total_data_to_overwrite_gb,
                start_time=start_time,
            )

            dict_nodes_capacity_after_overwrite_data = self._get_nodes_used_capacity()
            InfoEvent(
                message=f"Nodes used capacity before start overwriting data: {dict_nodes_capacity_after_overwrite_data}"
            ).publish()
