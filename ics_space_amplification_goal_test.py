...
class IcsSpaceAmplificationTest(LongevityTest):
    def _get_filesystem_available_size_list(self, node, start_time):
        ...
        available_size_res = self.prometheus_db.query(query=available_capacity_query, start=int(start_time), end=int(time.time()))
    ...
    def _get_max_used_capacity_over_time_gb(self, node, start_time) -> float:
        ...
        self.log.debug("The maximum used filesystem capacity of %s for the last %s minutes is: %s GB/ %s GB", node.private_ip_address, time_interval_minutes, max_used_capacity_gb, fs_size_gb)
        return max_used_capacity_gb

    def _get_nodes_space_ampl_over_time_gb(self, dict_nodes_initial_capacity, start_time, written_data_size_gb=0):
        ...
        for node in self.db_cluster.nodes:
            ...
            node_max_used_capacity_gb = self._get_max_used_capacity_over_time_gb(node=node, start_time=start_time)
            dict_nodes_space_amplification[node.private_ip_address] = round(node_max_used_capacity_gb - written_data_size_gb - node_initial_capacity, 2)
            self.log.info("Node %s used capacity changed from %s to %s.", node.private_ip_address, node_initial_capacity, node_used_capacity)
    ...
    def measure_nodes_space_amplification_after_write(self, dict_nodes_initial_capacity, written_data_size_gb, start_time):
        dict_nodes_space_amplification = self._get_nodes_space_ampl_over_time_gb(dict_nodes_initial_capacity=dict_nodes_initial_capacity, written_data_size_gb=written_data_size_gb, start_time=start_time)
        self.log.info("Space amplification results after a write of: %s are: %s", written_data_size_gb, dict_nodes_space_amplification)
        InfoEvent(message=f"Space amplification results after a write of: {written_data_size_gb} are: {dict_nodes_space_amplification}").publish()
    ...
    def _alter_table_compaction(self, compaction_strategy=CompactionStrategy.INCREMENTAL, table_name=TABLE_NAME, keyspace_name=KEYSPACE_NAME, additional_compaction_params: dict = None):
        """
        ...
        for sag in sag_testing_values:
            ...
            triggers = [
                partial(
                    node.run_nodetool,
                    sub_cmd="compact",
                    args=f"{KEYSPACE_NAME} {TABLE_NAME}",
                )
                for node in self.db_cluster.nodes
            ]
            ...
            InfoEvent(message=f"Nodes used capacity before start overwriting data: {dict_nodes_capacity_before_overwrite_data}").publish()
            for node, capacity in dict_nodes_capacity_before_overwrite_data.items():
                if capacity > nodes_original_capacity_with_delta[node]:
                    TestFrameworkEvent(source=self.__class__.__name__, message=capacity_exceed_msg.format(node, capacity, nodes_original_capacity_with_delta[node]), severity=Severity.ERROR).publish()
            ...
            params = {"keyspace_num": 1, "stress_cmd": stress_cmd, "round_robin": self.params.get("round_robin")}
            ...
            self.measure_nodes_space_amplification_after_write(dict_nodes_initial_capacity=dict_nodes_capacity_before_overwrite_data, written_data_size_gb=total_data_to_overwrite_gb, start_time=start_time)

            dict_nodes_capacity_after_overwrite_data = self._get_nodes_used_capacity()
            InfoEvent(message=f"Nodes used capacity before start overwriting data: {dict_nodes_capacity_after_overwrite_data}").publish()
