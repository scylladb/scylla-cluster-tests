...
def get_proxy_command(instance: dict | compute_v1.Instance, force_use_public_ip: bool, strict_host_checking: bool = False) -> [str, str, str]:
    if isinstance(instance, compute_v1.Instance):
        return gce_get_proxy_command(instance, strict_host_checking=strict_host_checking)
    elif isinstance(instance, VirtualMachine):
        return azure_get_proxy_command(instance, force_use_public_ip=force_use_public_ip, strict_host_checking=strict_host_checking)
    else:
        return aws_get_proxy_command(instance=instance, force_use_public_ip=force_use_public_ip, strict_host_checking=strict_host_checking)
...
def azure_get_proxy_command(instance: VirtualMachine, force_use_public_ip: bool, strict_host_checking: bool):
    ...
    if not force_use_public_ip:
        ...
        bastion_username, bastion_ip = guess_username(bastion), AzureService().get_virtual_machine_ips(virtual_machine=bastion).public_ip
...
def select_instance(region: str = None, **tags) -> dict | None:
    ...
    question = questionary.select(
        "Select machine: ",
        choices=[Choice(f"aws - {get_tags(vm).get('Name')} - {vm.get('PublicIpAddress')} {vm['PrivateIpAddress']} - {get_region(vm)}", value=vm) for vm in aws_vms]
        + [Choice(f"gce - {vm.name} - {list(gce_public_addresses(vm))[0]} {list(gce_private_addresses(vm))[0]} - {vm.zone.split('/')[-1]}", value=vm) for vm in gce_vms]
        + [
            Choice(
...
def select_instance_group(region: str = None, backends: list | None = None, **tags) -> list:
    ...
    choices = (
        [Choice(f"aws - {get_tags(vm).get('Name')} - {vm.get('PublicIpAddress')} {vm['PrivateIpAddress']} - {get_region(vm)}", value=vm) for vm in aws_vms]
        + [Choice(f"gce - {vm.name} - {list(gce_public_addresses(vm))[0]} {list(gce_private_addresses(vm))[0]} - {vm.zone.split('/')[-1]}", value=vm) for vm in gce_vms]
        + [
            Choice(
                f"azure - {vm.name} - {AzureService().get_virtual_machine_ips(virtual_machine=vm).public_ip} {AzureService().get_virtual_machine_ips(virtual_machine=vm).private_ip} - {vm.location + '' if not vm.zones else vm.zones[0]}",
...
@click.option("-P", "--force-use-public-ip", is_flag=True, show_default=True, default=False, help="Force usage of public address")
...
def ssh(user, test_id, region, force_use_public_ip, node_name):
    ...
    if connect_vm:
        ...
        cmd = f'bash -c \'{tty_options}; ssh -tt {proxy_command} -i {target_key} -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o ServerAliveInterval=10 {target_username}@{target_ip} \''
        click.echo(cmd)
...
def ssh_run_cmd(node_name: str, command: str, user: str = None, test_id: str = None, region: str = None, force_use_public_ip: bool = None) -> subprocess.CompletedProcess | None:
    assert user or test_id or (node_name and command)
    connect_vm = select_instance(region=region, test_id=test_id, user=user, node_name=node_name)
    cmd_out = None

    if connect_vm:
        proxy_command, target_ip, target_username, target_key = get_proxy_command(connect_vm, force_use_public_ip, strict_host_checking=False)
        click.echo(click.style(f"run command {command} via ssh into: {get_name(connect_vm)}", fg="green", bold=True))

        cmd = f"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {proxy_command} -i {target_key}  -o ServerAliveInterval=10 {target_username}@{target_ip} {command}"
...
def tunnel(user, test_id, region, port, wait_for_port, node_name):
    ...
    if connect_vm:
        ...
        cmd = f'ssh -i {target_key} -N -L {local_port}:{target_ip}:{port} -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o ServerAliveInterval=10 {bastion_username}@{bastion_ip}'
        click.echo(cmd)
...
def copy_cmd(user, test_id, region, force_use_public_ip, src, dest):
    ...
    if connect_vm:
        ...
        pty.spawn(shlex.split(f'scp {proxy_command} -i {target_key} -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o ServerAliveInterval=10 -C {src} {dest}'))
...
def gcp_allow_public(user, test_id):
    ...
    for i in instances:
        ...
        gce_set_tags(instances_client=instances_client, instance=i, new_tags=["sct-allow-public"], project=info["project_id"], zone=i.zone.split("/")[-1])
...
@click.option("-p", "--packages-location", type=str, help="Location where new packages are placed. Can be local directory, s3:// or gs:// urls")
...
class ScyllaDBCluster(BaseScyllaCluster, BaseCluster):
    ...
    def init_nodes(self, instances):
        ...
        for instance in instances:
            login_info = {"hostname": get_target_ip(instance), "user": guess_username(instance), "key_file": self.key_name}
            ...
            node.remoter = RemoteCmdRunnerBase.create_remoter(**node.ssh_login_info, proxy_host=proxy_host, proxy_user=proxy_user, proxy_key=proxy_key)
