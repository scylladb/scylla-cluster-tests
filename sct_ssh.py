...
def get_proxy_command(
    ...
    if isinstance(instance, compute_v1.Instance):
        ...
        return azure_get_proxy_command(
            instance, force_use_public_ip=force_use_public_ip, strict_host_checking=strict_host_checking
        )
    else:
...
def azure_get_proxy_command(instance: VirtualMachine, force_use_public_ip: bool, strict_host_checking: bool):
    ...
    if not force_use_public_ip:
        ...
        bastion_username, bastion_ip = (
            guess_username(bastion),
            AzureService().get_virtual_machine_ips(virtual_machine=bastion).public_ip,
        )
...
def copy_cmd(user, test_id, region, force_use_public_ip, src, dest):
    ...
    if connect_vm:
        ...
        pty.spawn(
            shlex.split(
                f'scp {proxy_command} -i {target_key} -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o ServerAliveInterval=10 -C {src} {dest}'
            )
        )
