...
class UpgradeTest(FillDatabaseData, loader_utils.LoaderUtilsMixin):
    ...
    def _upgrade_node(self, node, upgrade_sstables=True, new_scylla_repo=None, new_version=None):  # noqa: PLR0915
        ...
        if upgrade_node_packages:
            ...
            if orig_is_enterprise and ComparableScyllaVersion(
                ...
                InfoEvent(
                    message=f"upgrade_node - orig is enterprise and version {self.orig_ver} < 2025.1.0~dev and scylla_version_upgrade_target is {self.params.scylla_version_upgrade_target}"
                ).publish()
                scylla_pkg = "scylla"
                scylla_pkg_ver = f"{scylla_pkg}{ver_suffix}"
            ...
            with self.actions_log.action_scope("updating packages", target=node.name):
                if node.distro.is_rhel_like:
                    ...
                    node.remoter.sudo(
                        f'DEBIAN_FRONTEND=noninteractive apt-get dist-upgrade {scylla_pkg_ver} -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold"'
                    )
    ...
    def test_upgrade_cql_queries(self):
        ...
        self.actions_log.info(
            "Waiting for cassandra-stress to start before upgrade", metadata={"wait_time_seconds": 300}
        )
        time.sleep(300)
    ...
    def search_for_idx_token_error_after_upgrade(self, node, step):
        ...
        if idx_token_error:
            IndexSpecialColumnErrorEvent(
                message=f'Node: {node.name}. Step: {step}. Found error: index special column "idx_token" is not recognized'
            ).publish()
    ...
    def test_rolling_upgrade(self):  # noqa: PLR0914, PLR0915
        ...
        self.metric_has_data(
            metric_query='sct_cassandra_stress_write_gauge{type="ops", keyspace="keyspace_entire_test"}or sct_cql_stress_cassandra_stress_write_gauge{type="ops", keyspace="keyspace_entire_test"}',
            n=10,
        )
        ...
        self.actions_log.info(
            "Waiting for cassandra-stress to start before upgrade", metadata={"wait_time_seconds": 60}
        )
        self.metric_has_data(
            metric_query='sct_cassandra_stress_write_gauge{type="ops", keyspace="keyspace1"}or sct_cql_stress_cassandra_stress_write_gauge{type="ops", keyspace="keyspace1"}',
            n=5,
        )
        ...
        self.metric_has_data(metric_query="gemini_cql_requests", n=10)
        ...
        with ignore_upgrade_schema_errors():
            ...
            self.actions_log.info(
                "Waiting for cassandra-stress to start before rollback", metadata={"wait_time_seconds": 60}
            )
            time.sleep(60)
        ...
        first_node.remoter.run(
            f"for i in `sudo find /var/lib/scylla/data/{keyspace}/ -type f |grep -v manifest.json |grep -v snapshots |head -n 1`; do echo $i; sudo {dump_cmd} $i 1>/tmp/sstabledump.output || exit 1; done",
            verbose=True,
        )
        ...
        assert load_error_num <= error_factor * 8 * len(self.db_cluster.nodes), (
            "Only allowing shards_num * %d schema load errors per host during the entire test, actual: %d"
    ...
    def _start_and_wait_for_node_rollback(self, node: BaseNode, step: int) -> None:
        self.actions_log.info(f"Step {step} - Rollback node", target=node.name, metadata={"dc": node.dc_idx})
    ...
    def test_cluster_upgrade_latency_regression(self):
        ...
        * self.run_raft_topology_upgrade_procedure()
        ...
        step = itertools_count(start=1)
        stress_before_upgrade_thread_pools = self._run_stress_workload("stress_before_upgrade", wait_for_finish=False)
    ...
    def test_kubernetes_operator_upgrade(self):
        ...
        progressing_last_transition_time_before_upgrade = self.k8s_cluster.kubectl(
            f"get --no-headers scyllacluster {self.params.get('k8s_scylla_cluster_name')} -o jsonpath='{{.status.conditions[?(@.type==\"Progressing\")].lastTransitionTime}}'",
    ...
    def test_kubernetes_platform_upgrade(self):
        ...
        data_plane_versions = (
            self.k8s_cluster.kubectl(
                f"get nodes --no-headers -l '{self.k8s_cluster.POOL_LABEL_NAME} in ({self.k8s_cluster.AUXILIARY_POOL_NAME},{scylla_pool.name})' -o custom-columns=:.status.nodeInfo.kubeletVersion"
            )
            .stdout.strip()
            .split()
        )
