    ...
    def inner(self, *args, **kwargs):
        ...
        with self.db_cluster.cql_connection_patient(node, keyspace='truncate_ks', connect_timeout=600) as session:
            ...
            try:
                ...
                self.actions_log.error("Failed truncate simple tables", metadata={
                                       "error": str(details), "traceback": traceback.format_exc()})
...
class UpgradeTest(FillDatabaseData, loader_utils.LoaderUtilsMixin):
    ...
    def read_data_from_truncated_tables(self, session):
        ...
        for table_name in tables_name:
            ...
            try:
                ...
                self.actions_log.error("Failed read data from tables", target=table_name,
                                       metadata={"error": str(details)})
    ...
    def _upgrade_node(self, node, upgrade_sstables=True, new_scylla_repo=None, new_version=None):  # noqa: PLR0915
        ...
        if upgrade_node_packages:
            ...
            self.actions_log.info("packages were sent", target=node.name)
            ...
            self.actions_log.info("draining node", target=node.name)
            node.run_nodetool("drain", timeout=15*60, coredump_on_timeout=True, long_running=True, retry=0)
            self.actions_log.info("drain node finished", target=node.name)
            self.actions_log.info("starting 'nodetool snapshot'", target=node.name)
            node.run_nodetool("snapshot")
            self.actions_log.info("snapshot finished", target=node.name)
            self.actions_log.info("stopping scylla server", target=node.name)
            node.stop_scylla_server()
            self.actions_log.info("stopping scylla server finished", target=node.name)
            self.actions_log.info("upgrading development packages", target=node.name)
            node.remoter.run('sudo rpm -UvhR --oldpackage /tmp/scylla/*development*', ignore_status=True)
            self.actions_log.info("upgrading development packages finished", target=node.name)
            self.actions_log.info("upgrading scylla packages", target=node.name)
            node.remoter.run('sudo rpm -URvh --replacefiles /tmp/scylla/*.rpm | true')
            node.remoter.run(r'rpm -qa scylla\*')
            self.actions_log.info("upgrading scylla packages finished", target=node.name)
        elif new_scylla_repo:
            self.actions_log.info("updating scylla repo", target=node.name)
            # backup the data
            ...
            self.actions_log.info("scylla repo updated")
            self.actions_log.info("backing up scylla config")
            backup_conf(node)
            self.actions_log.info("scylla config backed up")
            assert new_scylla_repo.startswith('http')
            self.actions_log.info("downloading new scylla repo", target=node.name)
            node.download_scylla_repo(new_scylla_repo)
            self.actions_log.info("new scylla repo downloaded", target=node.name)
            # flush all memtables to SSTables
            self.actions_log.info("draining node", target=node.name)
            node.run_nodetool("drain", timeout=15*60, coredump_on_timeout=True, long_running=True, retry=0)
            self.actions_log.info("drain node finished", target=node.name)
            self.actions_log.info("starting 'nodetool snapshot'", target=node.name)
            node.run_nodetool("snapshot")
            self.actions_log.info("nodetool snapshot finished", target=node.name)
            self.actions_log.info("stopping scylla server", target=node.name)
            ...
            self.actions_log.info('Target version for upgrade', target=node.name, metadata={
                                  "version": self.params.scylla_version_upgrade_target})
            if (orig_is_enterprise and
                ...
                self.actions_log.info('Enterprise version upgrade details', target=node.name, metadata={
                    "orig_version": self.orig_ver,
                    "target_version": self.params.scylla_version_upgrade_target
                })
                scylla_pkg = 'scylla'
                scylla_pkg_ver = f"{scylla_pkg}{ver_suffix}"
                self.actions_log.info('Rollback mode is set to reinstall', target=node.name)
            ...
            if node.distro.is_rhel_like:
                self.actions_log.info('Starting yum update', target=node.name)
                node.remoter.run(r'sudo yum update {}\* -y'.format(scylla_pkg_ver))
                self.actions_log.info('Finished yum update', target=node.name)
            else:
                self.actions_log.info('Starting apt-get update', target=node.name)
                ...
                self.actions_log.info('Finished apt-get update', target=node.name)

        self.actions_log.info('Fixing io.conf arguments compatibility', target=node.name)
        node.remoter.sudo(
            f"sed -i 's/--num-io-queues/--num-io-groups/' {node.add_install_prefix('/etc/scylla.d/io.conf')}")

        self.actions_log.info('Starting check_reload_systemd_config', target=node.name)
        check_reload_systemd_config(node)
        self.actions_log.info('Finished check_reload_systemd_config', target=node.name)

        self.actions_log.info('Starting run_scylla_sysconfig_setup', target=node.name)
        node.run_scylla_sysconfig_setup()
        self.actions_log.info('Finished run_scylla_sysconfig_setup', target=node.name)
        if scylla_yaml_updates:
            self.actions_log.info('Starting update_scylla_yaml', target=node.name)
            self._update_scylla_yaml_on_node(node_to_update=node, updates=scylla_yaml_updates)
            self.actions_log.info('Finished update_scylla_yaml', target=node.name)
        node.forget_scylla_version()
        node.drop_raft_property()
        self.actions_log.info('Starting Scylla server', target=node.name)
        node.start_scylla_server(verify_up_timeout=500)
        self.actions_log.info('Finished Scylla server', target=node.name)
        self.actions_log.info('Starting get_db_nodes_cpu_mode', target=node.name)
        self.db_cluster.get_db_nodes_cpu_mode()
        self.actions_log.info('Finished get_db_nodes_cpu_mode', target=node.name)
        result = node.remoter.run('scylla --version')
        new_ver = result.stdout.strip()
        assert self.orig_ver != new_ver, "scylla-server version didn't change during upgrade"
        self.new_ver = new_ver
        self.actions_log.info('Starting _update_argus_upgraded_version',
                              target=node.name, metadata={"version": new_ver})
        self._update_argus_upgraded_version(node, new_ver)
        self.actions_log.info('Finished _update_argus_upgraded_version', target=node.name)
        if upgrade_sstables:
            self.actions_log.info('Starting upgradesstables_if_command_available', target=node.name)
            self.upgradesstables_if_command_available(node)
            self.actions_log.info('Finished upgradesstables_if_command_available', target=node.name)
    ...
    def upgrade_os(self, nodes):
        def upgrade(node):
            self.actions_log.info('Starting upgrade_system', target=node.name)
            node.upgrade_system()
            self.actions_log.info('Finished upgrade_system', target=node.name)

        if self.params.get('upgrade_node_system'):
            self.actions_log.info('Starting upgrade OS on nodes')
            parallel_obj = ParallelObject(objects=nodes, timeout=self.system_upgrade_timeout)
            parallel_obj.run(upgrade)
            self.actions_log.info('Finished Upgrading OS on nodes')
    ...
    def _rollback_node(self, node, upgrade_sstables=True):
        ...
        self.actions_log.info('Scylla server version info', target=node.name, metadata={
                              "original_version": orig_ver, "latest_version": new_ver})
    ...
    def upgradesstables_if_command_available(self, node, queue=None):  # pylint: disable=invalid-name
        ...
        if "yes" in upgradesstables_supported.stdout:
            ...
            self.actions_log.info("Calling upgradesstables", target=node.name)
    ...
    def wait_for_sstable_upgrade(self, node, queue=None):
        ...
        try:
            self.actions_log.info("Start waiting for upgradesstables to finish", target=node.name)
    ...
    def test_cluster_upgrade_latency_regression(self):
        ...
        * self.run_raft_topology_upgrade_procedure()
        ...
        self.actions_log.info("Step1 - Populate DB data")
        # Prepare keyspace and tables for truncate test
        self.fill_db_data_for_truncate_test(insert_rows=NUMBER_OF_ROWS_FOR_TRUNCATE_TEST)
        self.run_prepare_write_cmd()

        self.actions_log.info("Step2 - Run 'read' command before upgrade")
        step = itertools_count(start=1)
        ...
        self.actions_log.info("Step3 - Upgrade cluster", metadata={"new_version": self.params.get('new_version')})

        self.actions_log.info("Upgrade part of nodes nodes before roll-back")
        nodes_to_upgrade = self.shuffle_nodes_and_alternate_dcs(list(self.db_cluster.nodes))
        upgraded_nodes = []
        for node_to_upgrade in nodes_to_upgrade[:self.params.get('num_nodes_to_rollback')]:
            self._start_and_wait_for_node_upgrade(node_to_upgrade, step=next(step))
        ...
        self.actions_log.info("Upgrade all nodes")
        for node_to_upgrade in nodes_to_upgrade:
            self._start_and_wait_for_node_upgrade(node_to_upgrade, step=next(step))
        self.actions_log.info("All nodes were upgraded successfully")

        self.actions_log.info("Step4 - Run raft topology upgrade procedure")
        self.run_raft_topology_upgrade_procedure()

        self.actions_log.info("Step5 - Wait for stress_during_entire_upgrade to finish")
        ...
        self.actions_log.info("Step6 - run 'stress_after_cluster_upgrade' stress command(s)")
        time.sleep(60)
    ...
    def test_kubernetes_scylla_upgrade(self):
        ...
        self.actions_log.info('Step1 - Populate DB with many types of tables and data')
        target_upgrade_version = self.params.get('new_version')
        self.prepare_keyspaces_and_tables()
        self.actions_log.info('Step2 - Populate some data before upgrading cluster')
        self.fill_and_verify_db_data('', pre_fill=True)
        self.actions_log.info('Step3 - Starting c-s write workload')
        self.verify_stress_thread(
            self.run_stress_thread(
                stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_w'))
            )
        )
        self.actions_log.info('Step4 - Starting c-s read workload')
        self.verify_stress_thread(
            self.run_stress_thread(
                stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_r'))
            )
        )
        self.actions_log.info('Step5 - Upgrade cluster', metadata={"new_version": target_upgrade_version})
        self.db_cluster.upgrade_scylla_cluster(target_upgrade_version)
        self.actions_log.info('Step6 - Wait till cluster got upgraded')
        self.wait_till_scylla_is_upgraded_on_all_nodes(target_upgrade_version)
        self.k8s_cluster.check_scylla_cluster_sa_annotations()

        self.actions_log.info('Step7 - Upgrade sstables')
        if self.params.get('upgrade_sstables'):
            ...
            # only check sstable format version if all nodes had 'nodetool upgradesstables' available
            if all(upgradesstables):
                self.actions_log.info("Waiting until jmx is up across the board")
            ...
            self.actions_log.info("Upgrade of sstables is disabled")

        self.actions_log.info('Step8 - Verify data after upgrade')
        self.fill_and_verify_db_data(note='after all nodes upgraded')
        self.actions_log.info('Step9 - Starting c-s read workload')
        self.verify_stress_thread(
            self.run_stress_thread(
                stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_r'))
            )
        )
        self.actions_log.info('Step10 - Starting c-s write workload')
        self.verify_stress_thread(
            self.run_stress_thread(
                stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_w'))
            )
        )
        self.actions_log.info('Step11 - Starting c-s read workload')
        self.verify_stress_thread(
            self.run_stress_thread(
                stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_r'))
            )
        )
        self.actions_log.info('Step12 - Search for errors in scylla log')
        for node in self.db_cluster.nodes:
            self.search_for_idx_token_error_after_upgrade(node=node, step=f'{str(node)} after upgrade')
        self.actions_log.info('Step13 - Checking how many failed_to_load_scheme errors happened during the test')
        error_factor = 3
        schema_load_error_num = self.count_log_errors(search_pattern='Failed to load schema version',
                                                      step='AFTER UPGRADE')
        # Warning example:
    ...
    def test_kubernetes_operator_upgrade(self):  # pylint: disable=too-many-locals,too-many-statements
        ...
        self.actions_log.info('Step1 - Populate DB with data')
        self.prepare_keyspaces_and_tables()
        self.fill_and_verify_db_data('', pre_fill=True)

        self.actions_log.info('Step2 - Run c-s write workload')
        self.verify_stress_thread(self.run_stress_thread(
            stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_w'))))

        self.actions_log.info('Step3 - Run c-s read workload')
        self.verify_stress_thread(self.run_stress_thread(
            stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_r'))))

        self.actions_log.info('Step4 - Upgrade scylla-operator')
        ...
        self.actions_log.info('Step5 - Validate scylla-operator version after upgrade')
        ...
        self.actions_log.info('Step6 - Wait for the update of Scylla cluster')
        ...
        self.actions_log.info('Step7 - Add new member to the Scylla cluster')
        ...
        self.actions_log.info('Step8 - Verify data in the Scylla cluster')
        self.fill_and_verify_db_data(note='after operator upgrade and scylla member addition')

        self.actions_log.info('Step9 - Run c-s read workload')
        self.verify_stress_thread(self.run_stress_thread(
            stress_cmd=self._cs_add_node_flag(self.params.get('stress_cmd_r'))))
    ...
    def test_kubernetes_platform_upgrade(self):
        ...
        self.actions_log.info("Step2 - Start 'stress_during_entire_upgrade'")
        stress_during_entire_upgrade_thread_pools = self._run_stress_workload(
            "stress_during_entire_upgrade", wait_for_finish=False)

        self.actions_log.info("Step3 - Upgrade kubernetes platform")
        ...
        self.actions_log.info("Step4 - Validate K8S versions after the upgrade")
        ...
        self.actions_log.info("Step5 - Check Scylla-operator pods")
        self.k8s_cluster.kubectl_wait(
            "--all --for=condition=Ready pod",
            namespace=self.k8s_cluster._scylla_operator_namespace,  # pylint: disable=protected-access
            timeout=600)

        self.actions_log.info("Step6 - Check Scylla pods")
        self.k8s_cluster.kubectl_wait(
            "--all --for=condition=Ready pod",
            namespace=self.db_cluster.namespace,
            timeout=600)
        ...
        self.actions_log.info("Step8 - Wait for the end of 'stress_during_entire_upgrade'")
        for stress_during_entire_upgrade_thread_pool in stress_during_entire_upgrade_thread_pools:
            self.verify_stress_thread(stress_during_entire_upgrade_thread_pool)

        self.actions_log.info("Step9 - Run 'fill_and_verify_db_data'")
        self.fill_and_verify_db_data(note='after operator upgrade and scylla member addition')

        self.actions_log.info("Step10 - Run 'stress_cmd_r' to check all the preloaded data")
...
class UpgradeCustomTest(UpgradeTest):
    ...
    def test_custom_profile_sequential_rolling_upgrade(self):
        ...
        new_scylla_repo = \
            ...
            "scylla.list"
        self.actions_log.info('Starting rolling upgrade', metadata={"new_version": new_version})
        self._custom_profile_rolling_upgrade(cs_user_profiles=cs_user_profiles,
                                             new_version=new_version, new_scylla_repo=new_scylla_repo)
        self.actions_log.info('Finished rolling upgrade', metadata={"new_version": new_version})
    ...
    def prepare_data_before_upgrade(self):
        self.actions_log.info('Running a prepare load for the initial custom data')
    ...
    def _custom_profile_rolling_upgrade(self, cs_user_profiles, new_scylla_repo=None, new_version=None):  # pylint: disable=too-many-locals,too-many-statements
        ...
        self.actions_log.info('Starting write workload during entire test')
        ...
        with ignore_upgrade_schema_errors():
            ...
            self.actions_log.info('after upgraded one node')
            self.search_for_idx_token_error_after_upgrade(node=self.db_cluster.node_to_upgrade,
                                                          step=step+' - after upgraded one node')
