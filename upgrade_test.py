        ...
        if self.do_truncates:
            ...
            with self.db_cluster.cql_connection_patient(node, keyspace="truncate_ks", connect_timeout=600) as session:
                ...
                try:
                    ...
                    InfoEvent(message=f"Failed truncate simple tables. Error: {str(details)}. Traceback: {traceback.format_exc()}", severity=Severity.ERROR).publish()
...
class UpgradeTest(FillDatabaseData, loader_utils.LoaderUtilsMixin):
    ...
    def read_data_from_truncated_tables(self, session):
        ...
        for table_name in tables_name:
            ...
            try:
                ...
                self.assertEqual(str(count[0][0]), "0", msg="Expected that there is no data in the table truncate_ks.{}, but found {} rows".format(table_name, count[0][0]))
                self.actions_log.info(f"Finish read data from {table_name} tables ")
            except Exception as details:  # noqa: BLE001
                InfoEvent(message=f"Failed read data from {table_name} tables. Error: {str(details)}. Traceback: {traceback.format_exc()}", severity=Severity.ERROR).publish()
    ...
    def validate_truncated_entries_for_table(self, session, system_truncated=False):
        ...
        for table_id in tables_id:
            if system_truncated:
                ...
                self.assertTrue(truncated_time, msg="Expected truncated entry in the system.truncated table, but it's not found")
            ...
            if system_truncated:
                self.assertEqual(truncated_time, [[None]], msg="Not expected truncated entry in the system.local table, but it's found")
            else:
                self.assertTrue(truncated_time, msg="Expected truncated entry in the system.local table, but it's not found")

    @truncate_entries
    def upgrade_node(self, node, upgrade_sstables=True):
        self._upgrade_node(node=node, upgrade_sstables=upgrade_sstables)

    @decorate_with_context([ignore_abort_requested_errors, ignore_ycsb_connection_refused, ignore_topology_change_coordinator_errors, ignore_raft_topology_cmd_failing])
    ...
    def _upgrade_node(self, node, upgrade_sstables=True, new_scylla_repo=None, new_version=None):  # noqa: PLR0915
        ...
        if upgrade_node_packages:
            ...
            if node.distro.is_rhel_like:
                ...
                new_is_enterprise = bool("scylla-enterprise.x86_64" in result.stdout or "No matches found" not in result.stdout)
            else:
            ...
            if orig_is_enterprise and ComparableScyllaVersion(self.orig_ver) < "2025.1.0~dev" <= ComparableScyllaVersion(self.params.scylla_version_upgrade_target):
                InfoEvent(message=f"upgrade_node - orig is enterprise and version {self.orig_ver} < 2025.1.0~dev and scylla_version_upgrade_target is {self.params.scylla_version_upgrade_target}").publish()
                scylla_pkg = "scylla"
                scylla_pkg_ver = f"{scylla_pkg}{ver_suffix}"
            ...
            with self.actions_log.action_scope("updating packages"):
                if node.distro.is_rhel_like:
                    ...
                    node.remoter.sudo(f'DEBIAN_FRONTEND=noninteractive apt-get dist-upgrade {scylla_pkg_ver} -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold"')

        node.remoter.sudo(f"sed -i 's/--num-io-queues/--num-io-groups/' {node.add_install_prefix('/etc/scylla.d/io.conf')}")
    ...
    def upgradesstables_if_command_available(self, node, queue=None):
        ...
        upgradesstables_supported = node.remoter.run('nodetool help | grep -q upgradesstables && echo "yes" || echo "no"')
    ...
    def wait_for_sstable_upgrade(self, node, queue=None):
        ...
        def wait_for_node_to_finish():
            try:
                result = node.remoter.sudo(r"find /var/lib/scylla/data/system -type f ! -path '*snapshots*' -printf %f\\n")
                all_sstable_files = result.stdout.splitlines()

                sstable_version_regex = re.compile(r"(\w+)-[^-]+-(.+)\.(db|txt|sha1|crc32)")

                sstable_versions = {sstable_version_regex.search(f).group(1) for f in all_sstable_files if sstable_version_regex.search(f)}

                assert len(sstable_versions) == 1, "expected all table format to be the same found {}".format(sstable_versions)
                assert list(sstable_versions)[0] == self.expected_sstable_format_version, "expected to format version to be '{}', found '{}'".format(self.expected_sstable_format_version, list(sstable_versions)[0])
        ...
        try:
            wait.wait_for(func=wait_for_node_to_finish, step=30, timeout=900, throw_exc=True, text="Waiting until upgardesstables is finished")
    ...
    def search_for_idx_token_error_after_upgrade(self, node, step):
        ...
        idx_token_error = list(node.follow_system_log(patterns=["Column idx_token doesn't exist"], start_from_beginning=True))
        if idx_token_error:
            IndexSpecialColumnErrorEvent(message=f'Node: {node.name}. Step: {step}. Found error: index special column "idx_token" is not recognized').publish()
    ...
    def test_rolling_upgrade(self):  # noqa: PLR0914, PLR0915
        ...
        self.metric_has_data(metric_query='sct_cassandra_stress_write_gauge{type="ops", keyspace="keyspace_entire_test"}or sct_cql_stress_cassandra_stress_write_gauge{type="ops", keyspace="keyspace_entire_test"}', n=10)
        ...
        self.actions_log.info("Waiting for cassandra-stress to start before upgrade")
        self.metric_has_data(metric_query='sct_cassandra_stress_write_gauge{type="ops", keyspace="keyspace1"}or sct_cql_stress_cassandra_stress_write_gauge{type="ops", keyspace="keyspace1"}', n=5)
        ...
        with ignore_upgrade_schema_errors():
            ...
            self.search_for_idx_token_error_after_upgrade(node=self.db_cluster.node_to_upgrade, step=step + " - after upgraded one node")
            ...
            self.search_for_idx_token_error_after_upgrade(node=self.db_cluster.node_to_upgrade, step=step + " - after upgraded two nodes")
        ...
        first_node.remoter.run(f"for i in `sudo find /var/lib/scylla/data/{keyspace}/ -type f |grep -v manifest.json |grep -v snapshots |head -n 1`; do echo $i; sudo {dump_cmd} $i 1>/tmp/sstabledump.output || exit 1; done", verbose=True)
        ...
        workload_prioritization_error_num = self.count_log_errors(search_pattern="workload prioritization.*read_failure_exception", step=step, search_for_idx_token_error=False)

        self.actions_log.info("schema_load_error_num: %s; workload_prioritization_error_num: %s" % (schema_load_error_num, workload_prioritization_error_num))
        ...
        assert load_error_num <= error_factor * 8 * len(self.db_cluster.nodes), "Only allowing shards_num * %d schema load errors per host during the entire test, actual: %d" % (error_factor, schema_load_error_num)
    ...
    def run_raft_topology_upgrade_procedure(self):
        ...
        for node in self.db_cluster.nodes:
            result = wait_for(func=self.db_cluster.is_features_enabled_on_node, timeout=60, step=1, text=f"Check feature enabled on node {node.name}", throw_exc=False, feature_list=[CONSISTENT_TOPOLOGY_CHANGES_FEATURE], node=node)
            feature_state_per_node.append(result)
        if not all(feature_state_per_node):
            self.actions_log.info("Step5.1 - Consistent topology changes is not supported. Cluster stay with gossip topology mode")
            return
    ...
    def _run_stress_workload(self, workload_name: str, wait_for_finish: bool = False, round_robin: bool = False) -> [CassandraStressThread]:
    ...
    def _add_sla_credentials_to_stress_commands(self, workloads_with_sla: list):
        ...
        with self.db_cluster.cql_connection_patient(node=self.db_cluster.nodes[0], user=loader_utils.DEFAULT_USER, password=loader_utils.DEFAULT_USER_PASSWORD) as session:
        ...
        self.add_sla_credentials_to_stress_cmds(workload_names=workloads_with_sla, roles=roles, params=self.params, parent_class_name=self.__class__.__name__)
    ...
    def test_generic_cluster_upgrade(self):
        ...
        self._add_sla_credentials_to_stress_commands(workloads_with_sla=["stress_during_entire_upgrade", "stress_after_cluster_upgrade"])
    ...
    def test_cluster_upgrade_latency_regression(self):
        ...
        * self.run_raft_topology_upgrade_procedure()
        ...
        step = itertools_count(start=1)
        stress_before_upgrade_thread_pools = self._run_stress_workload("stress_before_upgrade", wait_for_finish=False, round_robin=True)
        ...
        for i in range(len(stress_before_upgrade_results)):
            row = f"#{i + 1}"
            result_table.add_result(column="before_ops", row=row, status=argus_results.Status.UNSET, value=int(stress_before_upgrade_results[i][0]["op rate"]))
            result_table.add_result(column="before_mean", row=row, status=argus_results.Status.UNSET, value=float(stress_before_upgrade_results[i][0]["latency mean"]))
            result_table.add_result(column="before_p99", row=row, status=argus_results.Status.UNSET, value=float(stress_before_upgrade_results[i][0]["latency 99th percentile"]))
        argus_results.submit_results_to_argus(argus_client=self.test_config.argus_client(), result_table=result_table)

        stress_during_entire_upgrade_thread_pools = self._run_stress_workload("stress_during_entire_upgrade", wait_for_finish=False, round_robin=True)
        ...
        time.sleep(60)
        stress_after_upgrade_thread_pools = self._run_stress_workload("stress_after_cluster_upgrade", wait_for_finish=False, round_robin=True)
        ...
        for i in range(len(stress_after_upgrade_results)):
            row = f"#{i + 1}"
            result_table.add_result(column="after_p99", row=row, status=argus_results.Status.UNSET, value=float(stress_after_upgrade_results[i][0]["latency 99th percentile"]))
            result_table.add_result(column="after_mean", row=row, status=argus_results.Status.UNSET, value=float(stress_after_upgrade_results[i][0]["latency mean"]))
            result_table.add_result(column="after_ops", row=row, status=argus_results.Status.UNSET, value=int(stress_after_upgrade_results[i][0]["op rate"]))
        argus_results.submit_results_to_argus(argus_client=self.test_config.argus_client(), result_table=result_table)
    ...
    def test_kubernetes_scylla_upgrade(self):
        ...
        self.verify_stress_thread(self.run_stress_thread(stress_cmd=self._cs_add_node_flag(self.params.get("stress_cmd_w"))))
        InfoEvent(message="Step4 - Starting c-s read workload").publish()
        self.verify_stress_thread(self.run_stress_thread(stress_cmd=self._cs_add_node_flag(self.params.get("stress_cmd_r"))))
        ...
        error_factor = 3
        schema_load_error_num = self.count_log_errors(search_pattern="Failed to load schema version", step="AFTER UPGRADE")
        # Warning example:
        # workload prioritization - update_service_levels_from_distributed_data: an error occurred while retrieving
        # configuration (exceptions::read_failure_exception (Operation failed for system_distributed.service_levels
        # - received 0 responses and 1 failures from 1 CL=ONE.))
        workload_prioritization_error_num = self.count_log_errors(search_pattern="workload prioritization.*read_failure_exception", step="AFTER UPGRADE", search_for_idx_token_error=False)
        InfoEvent(message="schema_load_error_num: %s; workload_prioritization_error_num: %s" % (schema_load_error_num, workload_prioritization_error_num)).publish()
    ...
    def _get_current_operator_image_tag(self):
        return self.k8s_cluster.kubectl("get deployment scylla-operator -o custom-columns=:..image --no-headers", namespace=self.k8s_cluster._scylla_operator_namespace).stdout.strip().split(":")[-1]
    ...
    def test_kubernetes_operator_upgrade(self):
        ...
        progressing_last_transition_time_before_upgrade = self.k8s_cluster.kubectl(
            f"get --no-headers scyllacluster {self.params.get('k8s_scylla_cluster_name')} -o jsonpath='{{.status.conditions[?(@.type==\"Progressing\")].lastTransitionTime}}'", namespace=self.db_cluster.namespace
        ).stdout.strip()
        ...
        self.k8s_cluster.upgrade_scylla_operator(
            self.params.get("k8s_scylla_operator_upgrade_helm_repo") or self.params.get("k8s_scylla_operator_helm_repo"), self.params.get("k8s_scylla_operator_upgrade_chart_version") or "latest", upgrade_docker_image
        )
        ...
        for scylla_pod in self.db_cluster.nodes[::-1]:
            scylla_pod.wait_till_k8s_pod_get_uid(ignore_uid=old_scylla_pods_uids[scylla_pod.name], throw_exc=True, timeout=900)
        for scylla_pod in self.db_cluster.nodes[::-1]:
            scylla_pod.wait_for_pod_readiness()

        available = progressing = degraded = None
        for _ in range(20):
            scylla_cluster_conditions = json.loads(self.k8s_cluster.kubectl(f"get scyllacluster {self.params.get('k8s_scylla_cluster_name')} -o json", namespace=self.db_cluster.namespace).stdout.strip())["status"]["conditions"]
        ...
        pods = json.loads(self.k8s_cluster.kubectl("get pods -o json", namespace=self.db_cluster.namespace).stdout.strip())["items"]
        for pod in pods:
        ...
        assert not pods_with_wrong_image_tags, f"Found pods that have unexpected scylla-operator image tags.\nExpected is '{expected_docker_image_tag}'.\nPods: {json.dumps(pods_with_wrong_image_tags, indent=2)}"

        self.k8s_cluster.check_scylla_cluster_sa_annotations()

        InfoEvent(message="Step7 - Add new member to the Scylla cluster").publish()
        peer_db_node = self.db_cluster.nodes[0]
        new_nodes = self.db_cluster.add_nodes(count=1, dc_idx=peer_db_node.dc_idx, rack=peer_db_node.rack, enable_auto_bootstrap=True)
    ...
    def test_kubernetes_platform_upgrade(self):
        ...
        stress_during_entire_upgrade_thread_pools = self._run_stress_workload("stress_during_entire_upgrade", wait_for_finish=False)

        InfoEvent(message="Step3 - Upgrade kubernetes platform").publish()
        upgrade_version, scylla_pool = self.k8s_cluster.upgrade_kubernetes_platform(pod_objects=self.db_cluster.nodes, use_additional_scylla_nodepool=True)

        InfoEvent(message="Step4 - Validate K8S versions after the upgrade").publish()
        data_plane_versions = (
            self.k8s_cluster.kubectl(f"get nodes --no-headers -l '{self.k8s_cluster.POOL_LABEL_NAME} in ({self.k8s_cluster.AUXILIARY_POOL_NAME},{scylla_pool.name})' -o custom-columns=:.status.nodeInfo.kubeletVersion").stdout.strip().split()
        )
        # Output example:
        #   GKE                EKS
        #   v1.19.13-gke.700   v1.21.2-eks-c1718fb
        #   v1.19.13-gke.700   v1.21.2-eks-c1718fb
        assert all(ver.startswith(f"v{upgrade_version}.") for ver in data_plane_versions), f"Got unexpected K8S data plane version(s): {data_plane_versions}"
        ...
        # WARNING: foo
        assert any(ver.startswith(f"Server Version: v{upgrade_version}.") for ver in control_plane_versions), f"Got unexpected K8S control plane version: {control_plane_versions}"

        InfoEvent(message="Step5 - Check Scylla-operator pods").publish()
        self.k8s_cluster.kubectl_wait("--all --for=condition=Ready pod", namespace=self.k8s_cluster._scylla_operator_namespace, timeout=600)

        InfoEvent(message="Step6 - Check Scylla pods").publish()
        self.k8s_cluster.kubectl_wait("--all --for=condition=Ready pod", namespace=self.db_cluster.namespace, timeout=600)
        self.k8s_cluster.check_scylla_cluster_sa_annotations()
    ...
    def _update_argus_upgraded_version(self, node, new_version):
        try:
            ...
            if ver:
                package = Package(name="scylla-server-upgraded", date=ver.group("date"), version=ver.group("version"), revision_id=ver.group("commit_id"), build_id=node.get_scylla_build_id() or "#NO_BUILDID")
    ...
    def validate_limited_voters_feature_enabled(self):
        ...
        feature_state_per_node = []
        for node in self.db_cluster.nodes:
            result = wait_for(func=self.db_cluster.is_features_enabled_on_node, timeout=60, step=1, text=f"Check feature enabled on node {node.name}", throw_exc=False, feature_list=["GROUP0_LIMITED_VOTERS"], node=node)
            feature_state_per_node.append(result)
        if all(not enabled for enabled in feature_state_per_node):
            ...
            if num_of_nodes <= max_voter_number and num_of_non_voters > 0:
                InfoEvent(message=f"Number of voters: {num_of_nodes - num_of_non_voters} are less than expected: {max_voter_number}", severity=Severity.ERROR).publish()
            elif num_of_nodes > max_voter_number and (num_of_nodes - num_of_non_voters) > max_voter_number:
                InfoEvent(message=f"Number of voters: {num_of_nodes - num_of_non_voters} is higher than expected: {max_voter_number}", severity=Severity.ERROR).publish()
            else:
                InfoEvent(message="Feature limited voters is enabled correctly").publish()
            return
        else:
            nodes_without_feature = [node.name for _, node in filter(lambda state: not state[0], zip(feature_state_per_node, self.db_cluster.nodes))]
            raise Group0LimitedVotersFeatureNotEnableOnNodes(f"Limited voters feature not enabled on all nodes: {','.join(nodes_without_feature)}")
...
class UpgradeCustomTest(UpgradeTest):
    ...
    def test_custom_profile_sequential_rolling_upgrade(self):
        ...
        self._custom_profile_rolling_upgrade(cs_user_profiles=cs_user_profiles, new_version=new_version, new_scylla_repo=new_scylla_repo)
    ...
    def prepare_data_before_upgrade(self):
        ...
        stress_before_upgrade = self.run_cs_user_profiles(cs_profiles=user_profiles, duration_per_cs_profile=duration_per_cs_profile)
    ...
    def _custom_profile_rolling_upgrade(self, cs_user_profiles, new_scylla_repo=None, new_version=None):
        ...
        entire_write_thread_pool = self.run_cs_user_profiles(cs_profiles=user_profiles, duration_per_cs_profile=duration_per_cs_profile)
        ...
        for stress_cmd in user_profiles:
            ...
            try:
                self.metric_has_data(metric_query='sct_cassandra_stress_user_gauge{type="ops", keyspace="%s"}' % keyspace_name, n=10)
            except Exception as err:  # noqa: BLE001
                InfoEvent(f"Get metrix data for keyspace {keyspace_name} failed with error: {err}", severity=Severity.ERROR).publish()
        ...
        with ignore_upgrade_schema_errors():
            ...
            self._upgrade_node(self.db_cluster.node_to_upgrade, new_scylla_repo=new_scylla_repo, new_version=new_version)
            self.db_cluster.node_to_upgrade.check_node_health()
