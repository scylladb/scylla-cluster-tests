...
class UpgradeTest(FillDatabaseData):
    ...
    def fill_and_verify_alternator_db_data(self, node, note, pre_fill=False, rewrite_data=True):
        items, items2 = [], []
        schema = alternator.enums.YCSBSchemaTypes.HASH_AND_RANGE.value
        ...
        if not self.alternator.is_table_exists(node=node, table_name=table_name):
            self.alternator.create_table(node=node, table_name=table_name, schema=schema)
