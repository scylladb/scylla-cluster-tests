...
class CDCReplicationTest(ClusterTester):
    ...
    def collect_data_for_analysis(self, master_node: cluster.BaseNode, replica_node: cluster.BaseNode) -> None:
        with self.db_cluster.cql_connection_patient(node=master_node) as sess:
            ...
            res = sess.execute(SimpleStatement(f"select * from {self.KS_NAME}.{self.TABLE_NAME}", consistency_level=ConsistencyLevel.QUORUM, fetch_size=1000))
    ...
    def test_replication_longevity(self) -> None:
        ...
        self.db_cluster.nemesis.append(
            CategoricalMonkey(
                tester_obj=self,
    ...
    def test_replication(self, is_gemini_test: bool, mode: Mode) -> None:
        ...
        if mode == Mode.PREIMAGE:
            ...
            migrate_log_path = os.path.join(self.logdir, "scylla-migrate.log")
            (migrate_ok, consistency_ok) = self.check_consistency(migrate_log_path, compare_timestamps=(mode != Mode.POSTIMAGE))
            self.consistency_ok = consistency_ok
    ...
    def check_consistency(self, migrate_log_dst_path: str, compare_timestamps: bool = True) -> Tuple[bool, bool]:
        ...
        res = loader_node.remoter.run(
            cmd="./scylla-migrate check --master-address {} --replica-address {} --ignore-schema-difference {} {}.{} 2>&1 | tee scylla-migrate.log".format(
                self.db_cluster.nodes[0].external_address, self.cs_db_cluster.nodes[0].external_address, "" if compare_timestamps else "--no-writetime", self.KS_NAME, self.TABLE_NAME
            )
        )
    ...
    def copy_master_schema_to_replica(self) -> None:
        ...
        replica_node = self.cs_db_cluster.nodes[0]
        with self.cs_db_cluster.cql_connection_patient(node=replica_node) as sess:
            sess.execute(f"create keyspace if not exists {self.KS_NAME} with replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': 1}}")
    ...
    def start_replicator(self, mode: Mode) -> None:
        ...
        replicator_script = dedent(
            ...
            EOF
        """.format(self.KS_NAME, self.TABLE_NAME, self.db_cluster.nodes[0].external_address, self.cs_db_cluster.nodes[0].external_address, mode_str(mode))
        )
    ...
    def start_gemini(self, seed: Optional[int] = None) -> GeminiStressThread:
        ...
        return GeminiStressThread(test_cluster=self.db_cluster, oracle_cluster=None, loaders=self.loaders, stress_cmd=stress_cmd, timeout=timeout, params=params).run()
