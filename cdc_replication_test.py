...
class CDCReplicationTest(ClusterTester):
    ...
    def test_replication_longevity(self) -> None:
        ...
        self.db_cluster.nemesis.append(CategoricalMonkey(
            ...
            dist={
                ...
                'memory_stress': 1,
                'disrupt_toggle_cdc_feature_properties_on_table': 3
            }, default_weight=0))
    ...
    def check_consistency(self, master_node: BaseNode, replica_node: BaseNode, migrate_log_dst_path: str, compare_timestamps: bool = True) -> Tuple[bool, bool]:
        loader_node = self.loaders.nodes[0]
        ...
        result = runner.run(
            f"diff -sqd --speed-large-files {os.path.join(self.logdir, 'master-table')} {os.path.join(self.logdir, 'replica-table')}", ignore_status=True)
