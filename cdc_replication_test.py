...
REPLICATOR_URL = (
    "https://kbr-scylla.s3-eu-west-1.amazonaws.com/scylla-cdc-replicator-1.0.1-SNAPSHOT-jar-with-dependencies.jar"
)
...
class CDCReplicationTest(ClusterTester):
    ...
    def test_replication(self, is_gemini_test: bool, mode: Mode) -> None:
        ...
        if mode == Mode.PREIMAGE:
            ...
            (migrate_ok, consistency_ok) = self.check_consistency(
                migrate_log_path, compare_timestamps=(mode != Mode.POSTIMAGE)
            )
            self.consistency_ok = consistency_ok
    ...
    def copy_master_schema_to_replica(self) -> None:
        ...
        with self.cs_db_cluster.cql_connection_patient(node=replica_node) as sess:
            sess.execute(
                f"create keyspace if not exists {self.KS_NAME} with replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': 1}}"
            )
