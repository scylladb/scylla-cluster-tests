...
class TombstoneGcLongevityTest(TWCSLongevityTest):
    ...
    def setUp(self):
        ...
        self.propagation_delay = int(re.search(r"'propagation_delay_in_seconds':'(\d+)", post_prepare_cql_cmds).group(1))
    ...
    def _run_repair_and_major_compaction(self, wait_propagation_delay: bool = False):
        ...
        triggers = [
            partial(
                node.run_nodetool,
                sub_cmd="repair",
                args=f"-pr {self.keyspace} {self.table}",
            )
            for node in self.db_cluster.data_nodes
        ]
        ParallelObject(objects=triggers, timeout=1200).call_objects()
    ...
    def test_switch_tombstone_gc_modes(self):
        ...
        sstable_utils = SstableUtils(db_node=self.db_node, propagation_delay_in_seconds=self.propagation_delay, ks_cf=self.ks_cf)
        ...
        assert tombstone_num_post_repair >= tombstone_num_pre_repair, f"Found unexpected fewer tombstones: {tombstone_num_post_repair} / {tombstone_num_pre_repair}"
        ...
        with self.db_cluster.cql_connection_patient(node=self.db_node) as session:
            query = f"ALTER TABLE scylla_bench.test with gc_grace_seconds = 864000 and tombstone_gc = {{'mode': 'repair', 'propagation_delay_in_seconds':'{self.propagation_delay}'}};"
