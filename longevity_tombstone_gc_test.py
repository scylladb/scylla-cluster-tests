...
class TombstoneGcLongevityTest(TWCSLongevityTest):
    ...
    def test_switch_tombstone_gc_modes(self):
        ...
        sstable_utils = SstableUtils(
            db_node=self.db_node, propagation_delay_in_seconds=self.propagation_delay, ks_cf=self.ks_cf
        )
        ...
        for sstable in sstables:
            ...
            assert not tombstone_deletion_info, (
                f"Found unexpected existing tombstones: {tombstone_deletion_info} for sstable: {sstable}"
            )
