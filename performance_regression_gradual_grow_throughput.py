...
class PerformanceRegressionPredefinedStepsTest(PerformanceRegressionTest):
    ...
    def throttle_steps(self, workload_type):
        ...
        if workload_type not in throttle_steps:
            TestFrameworkEvent(source=self.__class__.__name__, message=f"Throttle steps for '{workload_type}' test is not defined in 'perf_gradual_throttle_steps' parameter", severity=Severity.CRITICAL).publish()
    ...
    def step_duration(self, workload_type):
        ...
        if workload_type not in step_duration:
            TestFrameworkEvent(source=self.__class__.__name__, message=f"Step duration for '{workload_type}' test is not defined in 'perf_gradual_step_duration' parameter", severity=Severity.CRITICAL).publish()
    ...
    def test_read_disk_only_gradual_increase_load(self):
        ...
        self._base_test_workflow(workload=workload, test_name="test_read_disk_only_gradual_increase_load (100% reads from disk)")
    ...
    def _base_test_workflow(self, workload: Workload, test_name):
        ...
        self.run_gradual_increase_load(workload=workload, stress_num=stress_num, num_loaders=num_loaders, test_name=test_name)
    ...
    def check_latency_during_steps(self, step):
        ...
        self.log.debug("Step %s: latency_results were loaded from file %s and its result is %s", step, self.latency_results_file, latency_results)
    ...
    def run_step(self, stress_cmds, current_throttle, num_threads, step_duration):
        ...
        for stress_cmd in stress_cmds:
            ...
            stress_cmd_to_run = stress_cmd.replace("$threads", f"{num_threads}").replace("$throttle", f"{current_throttle}")
    ...
    def run_gradual_increase_load(self, workload: Workload, stress_num, num_loaders, test_name):  # noqa: PLR0914
        ...
        for throttle_step, num_threads, current_throttle_step in zip(workload.throttle_steps, workload.num_threads, sequential_steps):
            self.log.info("Run cs command with rate: %s Kops; threads: %s; step name: %s", throttle_step, num_threads, current_throttle_step)
            ...
            run_step = (latency_calculator_decorator(legend=f"Gradual test step {current_throttle_step} op/s", cycle_name=current_throttle_step))(self.run_step)
            results, _ = run_step(stress_cmds=workload.cs_cmd_tmpl, current_throttle=current_throttle, num_threads=num_threads, step_duration=workload.step_duration)
    ...
    def run_performance_analyzer(self, total_summary):
        perf_analyzer = PredefinedStepsTestPerformanceAnalyzer(es_index=self._test_index, email_recipients=self.params.get("email_recipients"))
        ...
        try:
            perf_analyzer.check_regression(test_id=self._test_id, data=total_summary, is_gce=is_gce, email_subject_postfix=self.params.get("email_subject_postfix"))
        except Exception as exc:  # noqa: BLE001
            TestFrameworkEvent(message="Failed to check regression", source=self.__class__.__name__, source_method="check_regression", exception=exc).publish_or_dump()
