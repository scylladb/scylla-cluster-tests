...
class PerformanceRegressionPredefinedStepsTest(PerformanceRegressionTest):
    ...
    def throttle_steps(self, workload_type):
        ...
        if workload_type not in throttle_steps:
            TestFrameworkEvent(
                ...
                message=f"Throttle steps for '{workload_type}' test is not defined in 'perf_gradual_throttle_steps' parameter",
    ...
    def step_duration(self, workload_type):
        ...
        if workload_type not in step_duration:
            TestFrameworkEvent(
                source=self.__class__.__name__,
                message=f"Step duration for '{workload_type}' test is not defined in 'perf_gradual_step_duration' parameter",
                severity=Severity.CRITICAL,
            ).publish()
