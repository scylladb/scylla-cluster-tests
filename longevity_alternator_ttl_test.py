...
class AlternatorTtlLongevityTest(LongevityTest):
    ...
    def count_sstables_and_partitions_after_major_compaction(self, run_repair: bool = True):
        ...
        node.remoter.run(
            f"curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' http://127.0.0.1:10000/storage_service/keyspace_offstrategy_compaction/{self.keyspace}"
        )
