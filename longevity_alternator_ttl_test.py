...
class AlternatorTtlLongevityTest(LongevityTest):
    ...
    def _count_sstables_and_partitions(self, node: BaseNode) -> int:
        ...
        self.log.info("Table stats results are: %s sstables, %s estimated partitions", num_of_sstables, estimated_num_of_partitions)
    ...
    def count_sstables_and_partitions_after_major_compaction(self, run_repair: bool = True):
        ...
        self.log.info("force offstrategy on %s before running running a major compaction on node %s", self.keyspace, node.name)
        node.remoter.run(f"curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' http://127.0.0.1:10000/storage_service/keyspace_offstrategy_compaction/{self.keyspace}")
    ...
    def test_disable_enable_ttl_scan(self):
        ...
        stress_cmd = self.params.get("stress_cmd")
        if stress_cmd and not skip_optional_stage("main_load"):
            params = {"keyspace_num": keyspace_num, "stress_cmd": stress_cmd, "round_robin": self.params.get("round_robin")}
            self._run_all_stress_cmds(stress_queue, params)
