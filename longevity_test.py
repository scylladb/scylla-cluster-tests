...
from sdcm.sct_events.group_common_events import ignore_large_collection_warning, ignore_max_memory_for_unlimited_query_soft_limit
...
class LongevityTest(ClusterTester, loader_utils.LoaderUtilsMixin):
    ...
    def _get_scan_operation_params(self) -> list[ThreadParams]:
        ...
        for item in params:
            ...
            scan_operation_params_list.append(
                ThreadParams(db_cluster=self.db_cluster, termination_event=self.db_cluster.nemesis_termination_event, user=sla_role_name, user_password=sla_role_password, duration=self.get_duration(item_dict.get("duration")), **item_dict)
            )
    ...
    def run_pre_create_schema(self):
        ...
        if pre_create_schema:
            self._pre_create_schema(keyspace_num, scylla_encryption_options=self.params.get("scylla_encryption_options"))
    ...
    def _run_validate_large_collections_in_system(self, node, table="table_with_large_collection"):
        ...
        with self.db_cluster.cql_connection_exclusive(node=node) as session:
            query = f"SELECT * from system.large_cells WHERE keyspace_name='large_collection_test' AND table_name='{table}' ALLOW FILTERING"
    ...
    def test_custom_time(self):  # noqa: PLR0914
        ...
        if scan_operation_params := self._get_scan_operation_params():
            for scan_param in scan_operation_params:
                ...
                self.run_fullscan_thread(fullscan_params=scan_param, thread_name=str(scan_operation_params.index(scan_param)))
        ...
        if cluster_target_size := self.params.get("cluster_target_size"):
            ...
            cluster_target_size = list(map(int, cluster_target_size.split())) if isinstance(cluster_target_size, str) else [cluster_target_size]
            ...
            InfoEvent(message=f"Starting to grow cluster from {self.params.get('n_db_nodes')} to {cluster_target_size}").publish()
            ...
            while not is_target_reached(current_cluster_size, cluster_target_size):
                ...
                for dcx, target in enumerate(cluster_target_size):
                    if current_cluster_size[dcx] < target:
                        add_nodes_num = add_node_cnt if (target - current_cluster_size[dcx]) >= add_node_cnt else target - current_cluster_size[dcx]
                        InfoEvent(message=f"Adding next number of nodes {add_nodes_num} to dc_idx {dcx}").publish()
                        added_nodes.extend(self.db_cluster.add_nodes(count=add_nodes_num, enable_auto_bootstrap=True, dc_idx=dcx, rack=None))
    ...
    def test_batch_custom_time(self):
        ...
        if prepare_write_cmd and not skip_optional_stage("prepare_write"):
            self._run_stress_in_batches(total_stress=total_stress, batch_size=batch_size, stress_cmd=prepare_write_cmd)

        self.db_cluster.start_nemesis()

        stress_cmd = self.params.get("stress_cmd")
        if not skip_optional_stage("main_load"):
            self._run_stress_in_batches(total_stress=batch_size, batch_size=batch_size, stress_cmd=stress_cmd)
    ...
    def test_user_batch_custom_time(self):
        ...
        if not cs_user_profiles:
            TestFrameworkEvent(source="test_user_batch_custom_time", message="The Longevity test_user_batch_custom_time cannot run without 'cs_user_profiles' parameter. aborting.", severity=Severity.ERROR).publish()
            return
        self.db_cluster.add_nemesis(nemesis=self.get_nemesis_class(), tester_obj=self)
        ...
        for cs_profile in cs_user_profiles:
            ...
            for _ in range(user_profile_table_count):
                stress_params_list += self.create_templated_user_stress_params(next(templated_table_counter), cs_profile)
        if not skip_optional_stage("main_load"):
            self.log.debug("Starting stress in batches of: %d with %d stress commands", batch_size, len(stress_params_list))
            self._run_user_stress_in_batches(batch_size=batch_size, stress_params_list=stress_params_list, duration=duration)
    ...
    def _run_stress_in_batches(self, total_stress, batch_size, stress_cmd):
        ...
        if pre_create_schema:
            self._pre_create_schema(keyspace_num=total_stress, scylla_encryption_options=self.params.get("scylla_encryption_options"))
        ...
        for batch in range(num_of_batches):
            for i in range(1 + batch * batch_size, (batch + 1) * batch_size + 1):
                ...
                self._run_all_stress_cmds(stress_queue, params={"stress_cmd": stress_cmds_with_all_ips, "keyspace_name": keyspace_name, "round_robin": True})
            for stress in stress_queue:
                self.verify_stress_thread(stress)
    ...
    def _pre_create_schema(self, keyspace_num=1, scylla_encryption_options=None):
        ...
        for i in range(1, keyspace_num + 1):
            ...
            for col_idx in range(col_num):
                ...
                columns[cs_key] = "blob"
            self.create_table(name="standard1", keyspace_name=keyspace_name, key_type="blob", read_repair=0.0, columns=columns, scylla_encryption_options=scylla_encryption_options, compaction=compaction_strategy, sstable_size=sstable_size)
