...
class SctEventsTests(BaseEventsTest):
    ...
    def test_disruption_skipped_event(self):
        with self.assertRaises(UnsupportedNemesis), DisruptionEvent(nemesis_name="DeleteByRowsRange", node="target_node", publish_event=False) as nemesis_event:
            try:
    ...
    def test_disruption_raised_critical_event(self):
        with self.assertRaises(ZeroDivisionError), DisruptionEvent(nemesis_name="DeleteByRowsRange", node="target_node", publish_event=False) as nemesis_event:
            nemesis_event.event_id = "c2561d8b-97ca-44fb-b5b1-8bcc0d437318"
            self.assertEqual(str(nemesis_event), "(DisruptionEvent Severity.NORMAL) period_type=begin event_id=c2561d8b-97ca-44fb-b5b1-8bcc0d437318: nemesis_name=DeleteByRowsRange target_node=target_node")
    ...
    def test_disruption_error_event(self):
        ...
        self.assertIn(
            "(DisruptionEvent Severity.ERROR) period_type=end event_id=c2561d8b-97ca-44fb-b5b1-8bcc0d437318 duration=15s: nemesis_name=DeleteByRowsRange target_node=target_node errors=division by zero\n",
            str(nemesis_event),
        )
    ...
    def test_disruption_normal_event(self):
        ...
        self.assertEqual(str(nemesis_event), "(DisruptionEvent Severity.NORMAL) period_type=end event_id=c2561d8b-97ca-44fb-b5b1-8bcc0d437318 duration=15s: nemesis_name=DeleteByRowsRange target_node=target_node")

    def test_filter(self):
        enospc_line_1 = "[99.80.124.204] [stdout] Mar 31 09:08:10 warning|  [shard 8] commitlog - Exception in segment reservation: storage_io_error (Storage I/O error: 28: No space left on device)"
        enospc_line_2 = (
    ...
    def test_general_filter(self):
        with self.wait_for_n_events(self.get_events_logger(), count=4, timeout=3):
            with EventsFilter(event_class=CoreDumpEvent):
                CoreDumpEvent(corefile_url="http://", backtrace="asfasdfsdf", node="node xy", download_instructions="test_general_filter", source_timestamp=1578998425.0).publish()  # Tue 2020-01-14 10:40:25 UTC
    ...
    def test_severity_changer(self):
        ...
        with self.wait_for_n_events(self.get_events_logger(), count=5, timeout=3):
            with EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=TestFrameworkEvent, extra_time_to_expiration=extra_time_to_expiration):
                TestFrameworkEvent(source="critical that should be lowered #1", source_method="", severity=Severity.CRITICAL).publish()
            TestFrameworkEvent(source="critical that should be lowered #2", source_method="", severity=Severity.CRITICAL).publish()
            event = TestFrameworkEvent(source="critical that should not be lowered #3", source_method="", severity=Severity.CRITICAL)
    ...
    def test_severity_changer_db_log(self):
        ...
        with self.wait_for_n_events(self.get_events_logger(), count=5, timeout=3):
            with EventsSeverityChangerFilter(new_severity=Severity.WARNING, event_class=DatabaseLogEvent, extra_time_to_expiration=extra_time_to_expiration):
                DatabaseLogEvent.NO_SPACE_ERROR().add_info(node="A", line_number=22, line="error that should be lowered #1").publish()
            DatabaseLogEvent.NO_SPACE_ERROR().add_info(node="A", line_number=22, line="error that should be lowered #2").publish()
            event = DatabaseLogEvent.NO_SPACE_ERROR().add_info(node="A", line_number=22, line="error that should not be lowered #3")
            event.source_timestamp = time.time() + extra_time_to_expiration
            event.publish()
        ...
        with self.wait_for_n_events(self.get_events_logger(), count=2, timeout=3):
            for _ in range(2):
                time.sleep(1)
                DatabaseLogEvent.NO_SPACE_ERROR().add_info(node="A", line_number=22, line="error that shouldn't be lowered").publish()
    ...
    def test_filter_repair(self):
        ...
        with self.wait_for_n_events(self.get_events_logger(), count=9, timeout=3):
            with (
                DbEventsFilter(db_event=DatabaseLogEvent.DATABASE_ERROR, line="repair's stream failed: streaming::stream_exception"),
    ...
    def test_filter_expiration(self):
        with self.wait_for_n_events(self.get_events_logger(), count=4, timeout=10):
            ...
            with DbEventsFilter(db_event=DatabaseLogEvent.NO_SPACE_ERROR, node="A"):
                DatabaseLogEvent.NO_SPACE_ERROR().add_info(node="A", line_number=22, line=line_prefix + " this is filtered").publish()

            line_prefix = f"{datetime.utcfromtimestamp(time.time() + 1):%Y-%m-%dT%H:%M:%S+00:00}"
            DatabaseLogEvent.NO_SPACE_ERROR().add_info(node="A", line_number=22, line=line_prefix + " : this is not filtered").publish()
    ...
    def test_default_filters(self):
        ...
        with self.wait_for_n_events(self.get_events_logger(), count=5):
            DatabaseLogEvent.BACKTRACE().add_info(node="A", line_number=22, line="Jul 01 03:37:31 ip-10-0-127-151.eu-west-1.compute.internal scylla[6026]:Rate-limit: supressed 4294967292 backtraces on shard 5").publish()
            DatabaseLogEvent.BACKTRACE().add_info(node="A", line_number=22, line="other back trace that shouldn't be filtered").publish()
            ...
            CassandraStressLogEvent.ConsistencyError().add_info(
                node="A",
                line_number=22,
                line="ERROR 18:04:32,556 Authentication error on host rolling-upgrade--ubuntu-focal-db-node-24508405-0-3.c.sct-project-1.internal/10.142.1.155:9042: Cannot achieve consistency level for cl ONE. Requires 1, alive 0",
            ).publish()
    ...
    def test_failed_stall_during_filter(self):
        with self.wait_for_n_events(self.get_events_logger(), count=5, timeout=3):
            with DbEventsFilter(db_event=DatabaseLogEvent.NO_SPACE_ERROR), DbEventsFilter(db_event=DatabaseLogEvent.BACKTRACE, line="No space left on device"):
                event = DatabaseLogEvent.REACTOR_STALLED()
                event.add_info(node="A", line_number=22, line="[99.80.124.204] [stdout] Mar 31 09:08:10 warning|  reactor stall 20").publish()
    ...
    def test_duration_format(self, duration_input, duration_formatted):
        event = NodetoolEvent(nodetool_command="scrub --skip-corrupted drop_table_during_repair_ks_0", node="1.0.0.121", options="more options", publish_event=False)
    ...
    def test_count_reactor_stall(self):
        ...
        with self.wait_for_n_events(self.get_events_counter(), count=4, timeout=5):
            with EventCounterContextManager(DatabaseLogEvent.REACTOR_STALLED, name=count_condition_name, _registry=self.events_processes_registry) as counter_manager:
        ...
        reason_stat_dir = self.get_events_counter().events_stat_dir / Path(count_condition_name) / Path("DatabaseLogEvent.REACTOR_STALLED")
    ...
    def test_count_reactor_stall_with_obj_instance(self):
        ...
        counter = EventCounterContextManager(DatabaseLogEvent.REACTOR_STALLED, name=count_condition_name, _registry=self.events_processes_registry)
    ...
    def test_count_several_events(self):
        ...
        statistics = {}
        with self.wait_for_n_events(self.get_events_counter(), count=4, timeout=5):
            with EventCounterContextManager((DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.KERNEL_CALLSTACK), name=count_condition_name, _registry=self.events_processes_registry) as counter_manager:
                assert counter_manager._event_type == (DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.KERNEL_CALLSTACK)
    ...
    def test_skip_not_required_count_required_events(self):
        ...
        with self.wait_for_n_events(self.get_events_counter(), count=6, timeout=5):
            with EventCounterContextManager((DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.KERNEL_CALLSTACK), name=count_condition_name, _registry=self.events_processes_registry) as counter_manager:
                ...
                DatabaseLogEvent.REACTOR_STALLED().add_info("node1", "Reactor stalled for 33 ms", 111).publish()
                DatabaseLogEvent.BACKTRACE().add_info(node="node1", line_number=22, line="other back trace that shouldn't be filtered").publish()
                DatabaseLogEvent.REACTOR_STALLED().add_info("node2", "Reactor stalled for 2501 ms", 111).publish()
                DatabaseLogEvent.BACKTRACE().add_info(node="node2", line_number=22, line="other back trace that shouldn't be filtered").publish()
                DatabaseLogEvent.KERNEL_CALLSTACK().add_info("node2", "Kernel Stack: asdfasdf ms asdfjasdlfjas dfs", 112).publish()
                DatabaseLogEvent.BACKTRACE().add_info(node="node3", line_number=22, line="other back trace that shouldn't be filtered").publish()
                time.sleep(1)
                statistics = counter_manager.get_stats().copy()
    ...
    def test_count_with_several_count_managers(self):
        ...
        with self.wait_for_n_events(self.get_events_counter(), count=6, timeout=5):
            with (
                EventCounterContextManager((DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.KERNEL_CALLSTACK), name=count_condition_name1, _registry=self.events_processes_registry) as counter_manager1,
                EventCounterContextManager((DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.BACKTRACE), name=count_condition_name2, _registry=self.events_processes_registry) as counter_manager2,
            ):
                assert counter_manager1._event_type == (DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.KERNEL_CALLSTACK)
    ...
    def test_count_with_several_nested_count_managers(self):
        ...
        statistics2 = {}
        with self.wait_for_n_events(self.get_events_counter(), count=6, timeout=5):
            with EventCounterContextManager((DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.KERNEL_CALLSTACK), name=count_condition_name1, _registry=self.events_processes_registry) as counter_manager1:
                assert counter_manager1._event_type == (DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.KERNEL_CALLSTACK)
                DatabaseLogEvent.REACTOR_STALLED().add_info("node1", "Reactor stalled for 33 ms", 111).publish()
                DatabaseLogEvent.BACKTRACE().add_info(node="node1", line_number=22, line="other back trace that shouldn't be filtered").publish()
                time.sleep(1)
                with EventCounterContextManager((DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.BACKTRACE), name=count_condition_name2, _registry=self.events_processes_registry) as counter_manager2:
                    assert counter_manager2._event_type == (DatabaseLogEvent.REACTOR_STALLED, DatabaseLogEvent.BACKTRACE)
    ...
    def test_kill_nemesis_during_con_event(self):
        with self.assertRaises(KillNemesis), DisruptionEvent(nemesis_name="SomeNemesis", node="target_node", publish_event=False) as nemesis_event:
            nemesis_event.event_id = "c2561d8b-97ca-44fb-b5b1-8bcc0d437318"
            self.assertEqual(str(nemesis_event), "(DisruptionEvent Severity.NORMAL) period_type=begin event_id=c2561d8b-97ca-44fb-b5b1-8bcc0d437318: nemesis_name=SomeNemesis target_node=target_node")
