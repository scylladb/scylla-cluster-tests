...
class SctEventsTests(BaseEventsTest):
    ...
    def test_disruption_raised_critical_event(self):
        with (
            ...
            self.assertEqual(
                str(nemesis_event),
                "(DisruptionEvent Severity.NORMAL) period_type=begin event_id=c2561d8b-97ca-44fb-b5b1-8bcc0d437318: nemesis_name=DeleteByRowsRange target_node=target_node",
            )
    ...
    def test_disruption_normal_event(self):
        ...
        self.assertEqual(
            str(nemesis_event),
            "(DisruptionEvent Severity.NORMAL) period_type=end event_id=c2561d8b-97ca-44fb-b5b1-8bcc0d437318 duration=15s: nemesis_name=DeleteByRowsRange target_node=target_node",
        )
    ...
    def test_default_filters(self):
        ...
        with self.wait_for_n_events(self.get_events_logger(), count=5):
            DatabaseLogEvent.BACKTRACE().add_info(
                node="A",
                line_number=22,
                line="Jul 01 03:37:31 ip-10-0-127-151.eu-west-1.compute.internal scylla[6026]:Rate-limit: supressed 4294967292 backtraces on shard 5",
            ).publish()
            DatabaseLogEvent.BACKTRACE().add_info(
    ...
    def test_kill_nemesis_during_con_event(self):
        with (
            ...
            self.assertEqual(
                str(nemesis_event),
                "(DisruptionEvent Severity.NORMAL) period_type=begin event_id=c2561d8b-97ca-44fb-b5b1-8bcc0d437318: nemesis_name=SomeNemesis target_node=target_node",
            )
