...
class UtilsOperatorMultitenantCommonTests(unittest.TestCase):
    ...
    def _multitenant_class_with_shared_options(self, klass):
        self.monkeypatch.setenv("SCT_CONFIG_FILES", "unit_tests/test_data/test_config/multitenant/shared_option_values.yaml")
        ...
        for i, tenant in enumerate(tenants):
            ...
            self.assertEqual(tenant.params.get("prepare_write_cmd"), [f"fake__prepare_write_cmd__all_tenants__part{j}" for j in range(1, 3)])
            self.assertEqual(tenant.params.get("prepare_verify_cmd"), "fake__prepare_verify_cmd__all_tenants__single_str")
            for cmd_name in ("nemesis_selector", "stress_cmd_m", "stress_cmd", "stress_cmd_r"):
                self.assertEqual(tenant.params.get(cmd_name), [f"fake__{cmd_name}__all_tenants__part{j}" for j in range(1, 3)])
    ...
    def _multitenant_class_with_unique_options(self, klass):
        self.monkeypatch.setenv("SCT_CONFIG_FILES", ("unit_tests/test_data/test_config/multitenant/unique_option_values.yaml"))
        ...
        self.assertEqual(2, len(tenants))
        for i, tenant in enumerate(tenants):
            self.assertEqual(tenant.get_str_index(), f"k8s-{'perf' if 'Perf' in klass.__name__ else 'longevity'}-2-tenants")
            self.assertEqual(tenant.__class__.__name__, f"TenantFor{klass.__name__.replace('Multitenant', '')}-{i + 1}")
            if "Longevity" in klass.__name__:
                tenant_id = f"{tenant.test_config.test_id()}-{tenant._test_index}"
            else:
            ...
            self.assertEqual(tenant.params.get("prepare_verify_cmd"), [f"fake__prepare_verify_cmd__all_tenants__part{j}" for j in range(1, 3)])
            self.assertEqual(tenant.params.get("stress_cmd"), [f"fake__stress_cmd__all_tenants__part{j}" for j in range(1, 3)])
        ...
        for cmd_name in ("stress_cmd_r", "stress_cmd_m"):
            self.assertEqual(tenants[0].params.get(cmd_name), [f"fake__{cmd_name}__tenant1__part{j}" for j in range(1, 3)])
        self.assertEqual(tenants[0].params.get("nemesis_class_name"), "FakeNemesisClassNameForTenant1")
        self.assertEqual(tenants[0].params.get("nemesis_selector"), [f"fake__nemesis_selector__tenant1__part{j}" for j in range(1, 3)])
