...
from sdcm.remote import RemoteLibSSH2CmdRunner, RemoteCmdRunner, LocalCmdRunner, RetryableNetworkException, SSHConnectTimeoutError, shell_script_cmd
...
def generate_all_commands_with_all_options():
    ...
    for ip in ["::1", "127.0.0.1"]:
        for cmd in [
            ...
            "echo 0; false",
            "adasdasdasd",
            "/bin/bash -c \"printf 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\\n%.0s' {1..100};\"",
        ]:
                        ...
                        for retry_value in [1, 2]:
                            for timeout_value in [None, 5]:
                                for remoter_type in [RemoteCmdRunner, RemoteLibSSH2CmdRunner, KubernetesCmdRunner]:
                                    all_commands_with_all_options.append((remoter_type, ip, cmd, verbose_value, ignore_status_value, new_session_value, retry_value, timeout_value))
...
class TestRemoteCmdRunners(unittest.TestCase):
    ...
    def _create_and_run_twice_in_same_thread(remoter_type, key_file, stmt, kwargs, paramiko_thread_results):
        if issubclass(remoter_type, (RemoteCmdRunner, RemoteLibSSH2CmdRunner)):
            ...
            remoter = KubernetesCmdRunner(FakeKluster("http://127.0.0.1:8001"), pod_image="fake-pod-image", pod_name="sct-cluster-dc-1-kind-0", container="scylla", namespace="scylla")
        try:
    ...
    @staticmethod
    def _compare_results(expected, result, stmt, kwargs, fields_to_compare=("stdout", "stderr", "command", "exited", "ok")):
    ...
    def test_run_in_mainthread(self, remoter_type, host: str, stmt: str, verbose: bool, ignore_status: bool, new_session: bool, retry: int, timeout: Union[float, None]):
        kwargs = {"verbose": verbose, "ignore_status": ignore_status, "new_session": new_session, "retry": retry, "timeout": timeout}
        try:
    ...
    # @parameterized.expand(ALL_COMMANDS_WITH_ALL_OPTIONS)
    @unittest.skip("To be ran manually")
    def test_create_and_run_in_same_thread(self, remoter_type, host: str, stmt: str, verbose: bool, ignore_status: bool, new_session: bool, retry: int, timeout: Union[float, None]):
        kwargs = {"verbose": verbose, "ignore_status": ignore_status, "new_session": new_session, "retry": retry, "timeout": timeout}
        ...
        self._run_parallel(3, thread_body=self._create_and_run_in_same_thread, args=(remoter_type, host, self.key_file, stmt, kwargs, paramiko_thread_results), kwargs={})
    ...
    # @parameterized.expand(ALL_COMMANDS_WITH_ALL_OPTIONS)
    @unittest.skip("To be ran manually")
    def test_create_and_run_in_separate_thread(self, remoter_type, host: str, stmt: str, verbose: bool, ignore_status: bool, new_session: bool, retry: int, timeout: Union[float, None]):
        kwargs = {"verbose": verbose, "ignore_status": ignore_status, "new_session": new_session, "retry": retry, "timeout": timeout}
        self.log.info(repr({stmt: stmt, **kwargs}))
        try:
            expected = LocalCmdRunner().run(stmt, **kwargs)
        ...
        self._run_parallel(3, thread_body=self._create_and_run_in_separate_thread, args=(remoter, stmt, kwargs, libssh2_thread_results), kwargs={})
    ...
    def test_load_1000_threads(self, remoter_type, stmt: str):
        ...
        libssh2_thread_results = []
        self._run_parallel(1000, thread_body=self._create_and_run_in_same_thread, args=(remoter_type, self.key_file, stmt, kwargs, libssh2_thread_results), kwargs={})
...
class TestRemoteFile(unittest.TestCase):
    ...
    def test_remote_file(self):
        ...
        with remote_file(remoter=remoter, remote_path=some_file, preserve_ownership=False, preserve_permissions=False) as fobj:
    ...
    def test_remote_file_preserve_ownership(self):
        remoter = self.remoter_cls()
        some_file = "/some/path/some.file"
        with remote_file(remoter=remoter, remote_path=some_file, preserve_ownership=True, preserve_permissions=False, sudo=True) as fobj:
            fobj.write("test data")
    ...
    def test_remote_file_preserve_permissions(self):
        remoter = self.remoter_cls()
        some_file = "/some/path/some.file"
        with remote_file(remoter=remoter, remote_path=some_file, preserve_ownership=False, preserve_permissions=True, sudo=True) as fobj:
            fobj.write("test data")
