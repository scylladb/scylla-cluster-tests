...
class TestGeminiEvent(unittest.TestCase):
    def test_continuous_event_with_error(self):
        ...
        self.assertEqual(
            str(begin_event),
            "(GeminiStressEvent Severity.NORMAL) period_type=begin event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543: node=node gemini_cmd=gemini_cmd",
        )
        ...
        self.assertEqual(
            ...
            "(GeminiStressEvent Severity.ERROR) period_type=end event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543 duration=16m30s: node=node gemini_cmd=gemini_cmd\nerrors=['error1', 'error2']",
        )
    ...
    def test_continuous_event_with_result(self):
        ...
        self.assertEqual(
            str(begin_event),
            "(GeminiStressEvent Severity.NORMAL) period_type=begin event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543: node=node gemini_cmd=cat",
        )
        self.assertEqual(begin_event.event_timestamp, begin_event_timestamp)
        self.assertEqual(begin_event.timestamp, begin_event_timestamp)
    ...
    def test_continuous_event_without_result(self):
        ...
        self.assertEqual(
            str(gemini_stress_event),
            "(GeminiStressEvent Severity.NORMAL) period_type=end event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543 duration=1d3h46m40s: node=node gemini_cmd=cat",
        )
...
class TestCassandraStressEvent(unittest.TestCase):
    def test_continuous_event_with_errors(self):
        ...
        self.assertEqual(
            str(begin_event),
            "(CassandraStressEvent Severity.NORMAL) period_type=begin event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543: node=node\nstress_cmd=stress_cmd",
        )
        self.assertEqual(begin_event.event_timestamp, begin_event_timestamp)
        self.assertEqual(begin_event.timestamp, begin_event_timestamp)
        self.assertEqual(begin_event, pickle.loads(pickle.dumps(begin_event)))
        ...
        self.assertEqual(
            str(cs_event),
            "(CassandraStressEvent Severity.ERROR) period_type=end event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543 duration=0s: node=node\nstress_cmd=stress_cmd\nerrors:\n\nerror1\nerror2",
        )
        ...
        self.assertEqual(
            str(cs_event),
            "(CassandraStressEvent Severity.CRITICAL) period_type=one-time event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543 duration=0s: node=node\nstress_cmd=stress_cmd\nerrors:\n\nerror1\nerror2\nOne more error",
        )
    ...
    def test_continuous_event_without_errors(self):
        ...
        self.assertEqual(
            str(cs_event),
            "(CassandraStressEvent Severity.NORMAL) period_type=end event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543 duration=10s: node=node\nstress_cmd=stress_cmd",
        )
        self.assertEqual(cs_event.event_timestamp, end_event_timestamp)
        self.assertEqual(cs_event.timestamp, end_event_timestamp)
        self.assertEqual(cs_event, pickle.loads(pickle.dumps(cs_event)))
...
class TestScyllaBenchEvent(unittest.TestCase):
    def test_continuous_event_with_error(self):
        ...
        self.assertEqual(
            str(begin_event),
            "(ScyllaBenchEvent Severity.NORMAL) period_type=begin event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543: node=node\nstress_cmd=stress_cmd",
        )
        self.assertEqual(begin_event.event_timestamp, begin_event_timestamp)
        self.assertEqual(begin_event.timestamp, begin_event_timestamp)
        self.assertEqual(begin_event, pickle.loads(pickle.dumps(begin_event)))
        ...
        self.assertEqual(
            ...
            "(ScyllaBenchEvent Severity.ERROR) period_type=end event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543 duration=1m40s: node=node\nstress_cmd=stress_cmd\nerrors:\n\nStress command completed with bad status 1",
        )
    ...
    def test_continuous_event_without_error(self):
        ...
        self.assertEqual(
            str(scylla_bench_event),
            "(ScyllaBenchEvent Severity.NORMAL) period_type=end event_id=14f35b64-2fcc-4b6e-a09d-4aeaf4faa543 duration=1s: node=node\nstress_cmd=stress_cmd",
        )
        scylla_bench_event.log_file_name = "log_file_name"
        self.assertEqual(scylla_bench_event, pickle.loads(pickle.dumps(scylla_bench_event)))
...
class TestYcsbStressEvent(unittest.TestCase):
    ...
    def test_without_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(YcsbStressEvent Severity.ERROR) period_type=not-set event_id=68067fe2-4c9e-421c-97b5-12db8d7ba71d: type=error node=[]\nstress_cmd=c-s",
        )
    ...
    def test_with_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(YcsbStressEvent Severity.CRITICAL) period_type=not-set event_id=225676a7-ddd1-4f4d-bae8-1cf5b35d0955: type=failure node=node1\nerrors:\n\ne1\ne2",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
...
class TestCDCReaderStressEvent(unittest.TestCase):
    ...
    def test_without_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(CDCReaderStressEvent Severity.NORMAL) period_type=not-set event_id=aed3946d-33a8-4f68-b56c-1d09f71f5da9: type=start node=[]\nstress_cmd=c-s",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
    ...
    def test_with_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(CDCReaderStressEvent Severity.CRITICAL) period_type=not-set event_id=3c5deb9e-7a67-49ee-9295-c7e986b015a9: type=failure node=node1\nerrors:\n\ne1\ne2",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
...
class TestNdBenchStressEvent(unittest.TestCase):
    ...
    def test_without_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(NdBenchStressEvent Severity.ERROR) period_type=not-set event_id=a07b48fa-2706-465b-b139-698d35909cfa: type=error node=[]\nstress_cmd=c-s",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
    ...
    def test_with_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(NdBenchStressEvent Severity.CRITICAL) period_type=not-set event_id=e45b347e-c395-4583-9f19-6e1fcdf31fab: type=failure node=node1\nerrors:\n\ne1\ne2",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
...
class TestKclStressEvent(unittest.TestCase):
    ...
    def test_without_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(KclStressEvent Severity.ERROR) period_type=not-set event_id=1acd4202-3a38-4b0d-9464-62f4825ee148: type=failure node=[]\nstress_cmd=c-s",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
    ...
    def test_with_errors(self):
        ...
        self.assertEqual(
            str(event),
            "(KclStressEvent Severity.ERROR) period_type=not-set event_id=d169ca02-c119-49f2-9eb7-23f152098cb7: type=failure node=node1\nerrors:\n\ne1\ne2",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
...
class TestCassandraStressLogEvent(unittest.TestCase):
    ...
    def get_event(line, expected_type, expected_severity):
        ...
        raise ValueError(
            f"Event is not recognized in the line {line}. Expected exception type is {expected_type},expected severity is {expected_severity}"
        )
    ...
    def test_cs_all_events_list(self):
        self.assertSetEqual(
            set(dir(CassandraStressLogEvent)) - set(dir(LogEvent)),
    ...
    def test_cs_hint_in_flight_error(self):
        ...
        self.get_event(
            line="java.io.IOException: Operation x10 on key(s) [334f37384f4d32303430]: Error executing: (OverloadedException): Queried host (10.0.3.167/10.0.3.167:9042) ",
    ...
    def test_cs_normal_rack_awarnes_event(self):
        self.get_event(
            line="Using provided rack name '1a' for RackAwareRoundRobinPolicy (if this is incorrect, please provide the correct rack name with RackAwareRoundRobinPolicy constructor",
...
class TestScyllaBenchLogEvent(unittest.TestCase):
    ...
    def test_scylla_bench_error_events_list(self):
        self.assertSetEqual(
            set(dir(ScyllaBenchLogEvent)) - set(dir(LogEvent)), {ev.type for ev in SCYLLA_BENCH_ERROR_EVENTS}
        )
...
class TestGeminiLogEvent(unittest.TestCase):
    def test_json_line(self):
        ...
        self.assertEqual(
            str(event),
            '(GeminiStressLogEvent Severity.NORMAL) period_type=one-time event_id=3eaf9cb2-b54b-43f5-8472-d0fbf5c25f72: type=GeminiEvent line_number=1 node=node1\nline=Test run stopped. Exiting. (N="pump")',
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
