...
def get_k8s_endpoint_update(namespace, pod_name, ip, required_labels_in_place=True, as_str=True):
    labels = {}
    if required_labels_in_place:
        labels = {k: "fake_value" for k in (ScyllaPodsIPChangeTrackerThread.SCYLLA_PODS_EXPECTED_LABEL_KEYS)}
    as_dict = {
        ...
        "object": {
            ...
            "subsets": [{"addresses": [{"ip": ip, "hostname": "fake_hostname", "nodeName": "fake_nodeName", "targetRef": "fake_targetRef"}], "ports": "fake_ports"}],
        },
    }
    if not as_str:
        return as_dict
...
def test_scylla_pods_ip_change_tracker_01_positive_scenario():
    ...
    fake_ip1, fake_ip2, fake_ip1a, fake_ip3, fake_ip3a = ("fake-ip-1", "fake-ip-2", "fake-ip-1a", "fake-ip-3", "fake-ip-3a")
    ...
    # Register callbacks
    ip_tracker.register_callbacks(callbacks=ns1_p1_callbacks_as_list, namespace=namespace1, pod_name=pod_names1[0], add_pod_name_as_kwarg=False)
    ip_tracker.register_callbacks(callbacks=ns1_p2_callbacks_as_callable, namespace=namespace1, pod_name=pod_names1[1], add_pod_name_as_kwarg=True)
    ip_tracker.register_callbacks(callbacks=ns1_eachpod_callbacks_as_callable, namespace=namespace1, pod_name="__each__", add_pod_name_as_kwarg=False)

    # Verify registered callbacks
    assert len(ip_mapper) == 1, ip_mapper

    ip_tracker.register_callbacks(callbacks=ns2_eachpod_callbacks_as_list, namespace=namespace2, add_pod_name_as_kwarg=True)
    ...
    assert ip_mapper[namespace1][pod_names1[0]]["callbacks"] == [
        (ns1_p1_callbacks_as_list[0][0], ns1_p1_callbacks_as_list[0][1], ns1_p1_callbacks_as_list[0][2], False),
    ...
    ns1_p1_callbacks_as_list[0][0].assert_called_once_with(*ns1_p1_callbacks_as_list[0][1], **ns1_p1_callbacks_as_list[0][2])
