...
def test_scylla_pods_ip_change_tracker_01_positive_scenario():  # pylint: disable=too-many-statements
    ...
    ip_tracker.register_callbacks(
        callbacks=ns1_p1_callbacks_as_list, namespace=namespace1, pod_name=pod_names1[0], add_pod_name_as_kwarg=False
    )
    ip_tracker.register_callbacks(
        callbacks=ns1_p2_callbacks_as_callable, namespace=namespace1, pod_name=pod_names1[1], add_pod_name_as_kwarg=True
    )
    ip_tracker.register_callbacks(
        callbacks=ns1_eachpod_callbacks_as_callable,
        namespace=namespace1,
        pod_name="__each__",
        add_pod_name_as_kwarg=False,
    )
    ...
    ns1_p1_callbacks_as_list[0][0].assert_called_once_with(
        *ns1_p1_callbacks_as_list[0][1], **ns1_p1_callbacks_as_list[0][2]
    )
