...
class ConfigurationTests(unittest.TestCase):
    ...
    def test_12_scylla_version_repo_case2(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            r"AMIs for scylla_version='99.0.3' not found in eu-west-1 arch=x86_64",
    ...
    def test_12_scylla_version_repo_ubuntu(self):
        ...
        self.assertEqual(conf.get("scylla_repo"), expected_repo)
        self.assertEqual(
            conf.get("scylla_repo_loader"), "https://s3.amazonaws.com/downloads.scylladb.com/deb/ubuntu/scylla-5.2.list"
        )
    ...
    def test_12_scylla_version_repo_ubuntu_loader_centos(self):
        ...
        os.environ["SCT_SCYLLA_REPO_LOADER"] = (
        ...
        self.assertEqual(conf.get("scylla_repo"), expected_repo)
        self.assertEqual(
            conf.get("scylla_repo_loader"), "https://s3.amazonaws.com/downloads.scylladb.com/rpm/centos/scylla-5.2.repo"
        )
    ...
    def test_14_check_rackaware_config_false_loader(self):
        ...
        with self.assertRaisesRegex(
            ValueError, expected_regex="rack_aware_loader' must be set to True for rackaware validator."
        ):
            conf.verify_configuration()
    ...
    def test_14_check_rackaware_config_no_rack_without_loader(self):
        ...
        conf = sct_config.SCTConfiguration()
        with self.assertRaisesRegex(ValueError, expected_regex="Rack-aware validation requires zones without loaders."):
            conf.verify_configuration()
    ...
    def test_conf_check_required_files_negative(self):
        ...
        try:
            ...
            self.assertEqual(
                str(exc),
                "Stress command parameter 'stress_cmd' contains profile '/tmp/1232123123123123123.yaml' that does not exists under data_dir/",
            )
    ...
    def test_17_verify_scylla_bench_required_parameters_in_command(self):
        ...
        os.environ["SCT_STRESS_READ_CMD"] = (
    ...
    def test_29_number_of_nodes_per_az_may_not_be_divisable_by_number_of_az_on_k8s(self):
        ...
        os.environ["SCT_K8S_SCYLLA_OPERATOR_HELM_REPO"] = "https://storage.googleapis.com/scylla-operator-charts/latest"
    ...
    def test_31_check_network_config_aws(self):
        ...
        self.assertEqual(
            conf.get("scylla_network_config"),
            [
                {
                ...
                {"address": "broadcast_rpc_address", "ip_type": "ipv4", "public": False, "use_dns": False, "nic": 0},
    ...
    def test_31_check_network_config_aws_interface_param_public_not_primary(self):
        os.environ["SCT_CONFIG_FILES"] = (
            """["internal_test_data/network_config_interface_param_public_not_primary.yaml"]"""
        )
    ...
    def test_35_append_str_options():
        ...
        assert conf.get("append_scylla_args") == (
            "--blocked-reactor-notify-ms 25 --abort-on-lsa-bad-alloc 1 --abort-on-seastar-bad-alloc --abort-on-internal-error 1 --abort-on-ebadf 1 --enable-sstable-key-validation 1 --overprovisioned 5"
        )

    @staticmethod
    def test_35_append_list_options():
        os.environ["SCT_SCYLLA_D_OVERRIDES_FILES"] = '["++", "extra_file/scylla.d/io.conf"]'
