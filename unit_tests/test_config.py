...
def test_06a_docker_latest_no_loader(monkeypatch):
    ...
    with unittest.mock.patch.object(sct_config, "get_specific_tag_of_docker_image", return_value=docker_tag_after_processing):
        conf = sct_config.SCTConfiguration()
        conf.verify_configuration()
...
def test_12_scylla_version_repo_ubuntu(monkeypatch):
    ...
    with unittest.mock.patch.object(sct_config, "get_branch_version", return_value="4.7.dev", clear=True), unittest.mock.patch.object(sct_config, "find_scylla_repo", return_value=expected_repo, clear=True):
        conf = sct_config.SCTConfiguration()
        conf.verify_configuration()
...
def test_conf_check_required_files(monkeypatch):  # pylint: disable=no-self-use
    monkeypatch.setenv("SCT_CLUSTER_BACKEND", "aws")
    ami_id = get_ssm_ami("/aws/service/canonical/ubuntu/server/22.04/stable/current/amd64/hvm/ebs-gp2/ami-id", region_name="eu-west-1")
...
def test_conf_check_required_files_negative(monkeypatch):
    ...
    try:
        ...
        assert str(exc) == "Stress command parameter 'stress_cmd' contains profile '/tmp/1232123123123123123.yaml' that does not exists under data_dir/"
...
def test_15a_new_scylla_repo_by_scylla_version(monkeypatch):
    ...
    with unittest.mock.patch.object(sct_config, "get_branch_version", return_value="666.development", clear=True), unittest.mock.patch.object(sct_config, "find_scylla_repo", return_value=resolved_repo_link, clear=True):
        conf = sct_config.SCTConfiguration()
        conf.verify_configuration()
        conf._get_target_upgrade_version()
...
def test_15b_image_id_by_scylla_version(monkeypatch):
    ...
    resolved_image_link = "https://www.googleapis.com/compute/v1/projects/scylla-images/global/images/scylla-4-7-dev-0-20220113-8bcd23fa0-1-build-359"
    image = namedtuple("Image", "name self_link")(name="scylla-4-7-dev-0-20220113-8bcd23fa0-1-build-359", self_link=resolved_image_link)
...
def test_17_verify_scylla_bench_required_parameters_in_command(monkeypatch):
    monkeypatch.setenv("SCT_CLUSTER_BACKEND", "aws")
    monkeypatch.setenv("SCT_AMI_ID_DB_SCYLLA", "ami-dummy")
    monkeypatch.setenv("SCT_STRESS_CMD", "scylla-bench -workload=sequential -mode=write -replication-factor=3 -partition-count=100")
    monkeypatch.setenv("SCT_STRESS_READ_CMD", "scylla-bench -workload=uniform -mode=read -replication-factor=3 -partition-count=100")

    conf = sct_config.SCTConfiguration()
    assert conf["stress_cmd"] == ["scylla-bench -workload=sequential -mode=write -replication-factor=3 -partition-count=100"]
    assert conf["stress_read_cmd"] == ["scylla-bench -workload=uniform -mode=read -replication-factor=3 -partition-count=100"]
...
def test_17_2_raise_error_if_scylla_bench_command_dont_have_mode(monkeypatch):
    ...
    monkeypatch.setenv("SCT_AMI_ID_DB_SCYLLA", "ami-dummy")
    monkeypatch.setenv("SCT_STRESS_CMD", "scylla-bench -workload=sequential -mode=write -replication-factor=3 -partition-count=100")
    monkeypatch.setenv("SCT_STRESS_READ_CMD", "scylla-bench -workload=uniform -replication-factor=3 -partition-count=100")
...
def test_20_user_data_format_version_gce_3(monkeypatch):
    monkeypatch.setenv("SCT_CLUSTER_BACKEND", "gce")
    monkeypatch.setenv("SCT_GCE_IMAGE_DB", ("https://www.googleapis.com/compute/v1/projects/ubuntu-os-cloud/global/images/family/ubuntu-2404-lts-amd64"))
    conf = sct_config.SCTConfiguration()
    conf.verify_configuration()
    conf.verify_configuration_urls_validity()
...
def test_21_nested_values(monkeypatch):
    monkeypatch.setenv("SCT_CONFIG_FILES", ('["unit_tests/test_configs/minimal_test_case.yaml", "unit_tests/test_data/stress_image_extra_config.yaml"]'))
    monkeypatch.setenv("SCT_CLUSTER_BACKEND", "aws")
...
def test_23_1_include_nemesis_selector(monkeypatch):
    monkeypatch.setenv("SCT_CLUSTER_BACKEND", "aws")
    monkeypatch.setenv("SCT_AMI_ID_DB_SCYLLA", "ami-dummy")
    monkeypatch.setenv("SCT_CONFIG_FILES", '["unit_tests/test_configs/minimal_test_case.yaml", "unit_tests/test_configs/nemesis_selector_list.yaml"]')
...
def test_23_2_nemesis_include_selector_list(monkeypatch):
    ...
    monkeypatch.setenv("SCT_REGION_NAME", "eu-west-1")
    monkeypatch.setenv("SCT_AMI_ID_DB_SCYLLA", "ami-dummy")
    monkeypatch.setenv("SCT_CONFIG_FILES", '["unit_tests/test_configs/minimal_test_case.yaml", "unit_tests/test_configs/nemesis_selector_list_of_list.yaml"]')
...
def test_26_run_fullscan_params_validtion_positive(monkeypatch):
    monkeypatch.setenv("SCT_CONFIG_FILES", '["unit_tests/test_configs/minimal_test_case.yaml", "unit_tests/test_configs/positive_fullscan_param.yaml"]')
    sct_config.SCTConfiguration()


def test_27_run_fullscan_params_validtion_negative(monkeypatch):
    monkeypatch.setenv("SCT_CONFIG_FILES", '["unit_tests/test_configs/minimal_test_case.yaml", "unit_tests/test_configs/negative_fullscan_param.yaml"]')
    try:
        sct_config.SCTConfiguration()
...
def test_35_append_str_options(monkeypatch):
    ...
    conf = sct_config.SCTConfiguration()
    conf.verify_configuration()
    assert conf.get("append_scylla_args") == ("--blocked-reactor-notify-ms 25 --abort-on-lsa-bad-alloc 1 --abort-on-seastar-bad-alloc --abort-on-internal-error 1 --abort-on-ebadf 1 --enable-sstable-key-validation 1 --overprovisioned 5")
...
def test_37_validates_single_thread_count_for_all_throttle_steps(monkeypatch):
    ...
    monkeypatch.setenv("SCT_PERF_GRADUAL_THROTTLE_STEPS", '{"read": ["unthrottled", "unthrottled"], "mixed": [100, "unthrottled"], "write": [200, "unthrottled"]}')
    conf = sct_config.SCTConfiguration()
    conf.verify_configuration()
