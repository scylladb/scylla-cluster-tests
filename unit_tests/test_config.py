...
def test_21_3_single_underscore_field_names_not_mistaken_for_nesting(monkeypatch):
    ...
    SCT_INSTANCE_TYPE_DB is a plain scalar option (single underscores throughout its
    name) and must stay a literal string, even though SCT_INSTANCE_TYPE_DB_ORACLE
    shares it as a prefix -- the anchored split must not let either claim the other.
    """
    monkeypatch.setenv("SCT_CLUSTER_BACKEND", "aws")
    monkeypatch.setenv("SCT_AMI_ID_DB_SCYLLA", "ami-1234")
    monkeypatch.setenv("SCT_CONFIG_FILES", "unit_tests/test_configs/minimal_test_case.yaml")
    monkeypatch.setenv("SCT_INSTANCE_TYPE_DB", "i4i.large")
    monkeypatch.setenv("SCT_INSTANCE_TYPE_DB_ORACLE", "i4i.xlarge")

    conf = sct_config.SCTConfiguration()
    conf.verify_configuration()

    assert conf.get("instance_type_db") == "i4i.large"
    assert conf.get("instance_type_db_oracle") == "i4i.xlarge"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Anchored on `field_env + sep`, so an option whose env var name is a prefix of
        # another option's (instance_type_db vs. instance_type_db_oracle) never false-matches.
        pytest.param(
            "SCT_INSTANCE_TYPE_DB_ORACLE__arch", "SCT_INSTANCE_TYPE_DB", "__", None, id="anchors-on-full-field-prefix"
        ),
...
def test_21_6_no_option_env_name_contains_double_underscore():
    """`_check_unexpected_sct_variables` truncates env var names at the first "." or "__"
    on the assumption that no config option's env var name itself contains "__". This is a
    ...
    """
    assert not any("__" in opt["env"] for opt in sct_config.SCTConfiguration.config_options), (
        "a config option's env var name contains '__', which breaks the env var name truncation "
...
||||||| parent of bd2dbde48 (refactor(sct_config): repoint patch targets and docs at the package):unit_tests/unit/test_config.py
...
>>>>>>> bd2dbde48 (refactor(sct_config): repoint patch targets and docs at the package):unit_tests/unit/test_config.py
@pytest.mark.parametrize(
    "raw_value,expected",
    [
        ("  2025.1.0  ", "2025.1.0"),
