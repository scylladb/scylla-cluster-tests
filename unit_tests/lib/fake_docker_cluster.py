...
class RackAwareDummyDockerNode(DummyDockerNode):
    """``DummyDockerNode`` with methods needed by ``node_setup``.

    Provides stub implementations of ``is_scylla_installed``,
    ``is_docker_rack_arg_supported``, ``config_setup``, and
    ``restart_scylla`` that operate on the underlying ``RemoteDocker``
    container.
    """
    ...
    def restart_scylla(self, verify_up_before=True):
        ...
        rd.remoter.run(f"{rd.sudo_needed}docker restart {rd.docker_id}", timeout=60)
