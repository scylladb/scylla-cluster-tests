...
def _init_invoke_result(instance_type, children):
    return False, instance_type(connection=None, **{attr_name: children.get(attr_name, None) for attr_name in ["stderr", "stdout", "exited"]})
...
class Pickler:
    class_info = {
        "fabric.runners.Result": {
            "init_args": {"stderr": PicklerAction.PASSTHROUGH, "stdout": PicklerAction.PASSTHROUGH, "exited": PicklerAction.PASSTHROUGH, "connection": None},
        ...
        "invoke.exceptions.UnexpectedExit": {"init_args": {"result": PicklerAction.PASSTHROUGH, "reason": PicklerAction.PASSTHROUGH}},
    }
    ...
    def import_and_return(path):
        ...
        if tclass is not None:
            return tclass
        paths = path.split(".")
        global_vars[path] = lib = getattr(__import__(".".join(paths[0:-1]), globals(), locals(), fromlist=[paths[-1]], level=0), paths[-1])
        return lib
