...
class FakeRemoter(RemoteCmdRunnerBase):
    ...
    def run(self, cmd: str, timeout=None, ignore_status=False, verbose=True, new_session=False, log_file=None, retry=1, watchers=None, timestamp_logs=False, change_context=False) -> Result:
        for pattern, result in self.result_map.items():
            if re.match(pattern, cmd) is not None:
                if ignore_status is True:
                    ...
                    return result
        raise ValueError(f"No fake result specified for command: {cmd}.Set {self.__class__.__name__}.result_map variable with Dict[Pattern, Result] mapping")
