...
from sdcm.utils.loader_utils import STRESS_ROLE_NAME_TEMPLATE, STRESS_ROLE_PASSWORD_TEMPLATE, SERVICE_LEVEL_NAME_TEMPLATE
...
class FakePrometheus:
    ...
    def get_scylla_io_queue_total_operations(start_time, end_time, node_ip, irate_sample_sec="60s"):
        return {"127.0.0.1": {"sl:default": [410.5785714285715, 400.36428571428576], "sl:sl200_abc": [177.11428571428573, 182.02857142857144], "sl:sl50_abc": [477.11428571428573, 482.02857142857144]}}
...
class TestSlaUtilsTest(unittest.TestCase, SlaUtils):
    def test_less_operations_than_expected_error(self):
        ...
        with self.assertRaises(SchedulerRuntimeUnexpectedValue) as error:
            self.validate_io_queue_operations(start_time=time.time(), end_time=time.time() + 60, read_users=read_roles, prometheus_stats=prometheus_stats, db_cluster=db_cluster, publish_wp_error_event=False)
        assert str(error.exception) == str(
            "\n(Node 127.0.0.1) - Service level with higher shares got less resources unexpectedly. CPU%: 100. Runtime per service level group:\n  sl:sl50_abc (shares 50): 479.57\n  sl:sl200_abc (shares 200): 179.57"
        )

    @staticmethod
    def create_sla_auth(session, shares: int, index: str) -> Role:
        role = FakeRole(session=session, name=STRESS_ROLE_NAME_TEMPLATE % (shares or "", index), password=STRESS_ROLE_PASSWORD_TEMPLATE % shares or "", login=True).create()
        role.attach_service_level(FakeServiceLevel(session=session, name=SERVICE_LEVEL_NAME_TEMPLATE % (shares or "", index), shares=shares).create())
