...
def test_nothing_written_to_a_node_can_terminate_its_heredoc():
    """SCT installs these files with 'cat > path << 'SCTEOF'', so content is the only safeguard.

    A quoted heredoc is what keeps the script's '$' from being expanded by the shell writing it,
    which rules out a per-call unique delimiter - so a line equal to the delimiter would end the
    redirection early and silently write a truncated file to the node. Leading and trailing spaces
    are stripped before comparing: they do not terminate a '<<' heredoc, but they would a '<<-' one.
    """
    written = {
        "SECONDARY_NICS_SCRIPT": SECONDARY_NICS_SCRIPT,
        "SECONDARY_NICS_SERVICE_UNIT_TMPL": SECONDARY_NICS_SERVICE_UNIT_TMPL,
    }
    for name, content in written.items():
        offending = [number for number, line in enumerate(content.splitlines(), 1) if line.strip() == HEREDOC_DELIMITER]
        assert not offending, f"{name} line(s) {offending} would close the '{HEREDOC_DELIMITER}' heredoc early"
