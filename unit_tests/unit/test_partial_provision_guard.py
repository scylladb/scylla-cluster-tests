...
# ---------------------------------------------------------------------------
# GCE: add_nodes count validation (REUSE_CLUSTER path)
# ---------------------------------------------------------------------------


def _make_gce_cluster(instance_count, requested_count):
    """Create a minimally-mocked GCECluster and invoke add_nodes with REUSE_CLUSTER=True.

    In this branch, _get_instances is only called on the REUSE_CLUSTER path,
    so the partial-provision guard lives there.
