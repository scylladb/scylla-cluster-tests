...
@pytest.mark.parametrize(
    ...
    [
        ...
        # quoted value containing a space — the second word must NOT leak
        (
            'scylla-bench -password "My Secret" -nodes 1.2.3.4',
            "-password ****",
            "-nodes 1.2.3.4",
        ),
        # quoted KV style
        ('tool -p password="hunter two" -p hosts=node1', "password=****", "hosts=node1"),
    ],
)
def test_redact_cli_secrets_masks_known_patterns(cmd, expected_redacted_substring, expected_preserved_substring):
    ...
    for secret in ("hunter2", "secret", "supersecret", "abc123", "deadbeef", "My Secret", "hunter two"):
        if secret in cmd:
