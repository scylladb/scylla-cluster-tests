...
class TestCassandraStressExporter:
    ...
    def test_01(self):
        ...
        cs_exporter = CassandraStressExporter(
            "127.0.0.1", self.metrics, "write", tmp_file.name, loader_idx=1, cpu_idx=0
        )
...
class TestCassandraStressHDRExporter:
    ...
    def test_01_mixed(self):
        tmp_file = tempfile.NamedTemporaryFile(mode="w+")
        cs_exporter = CassandraStressHDRExporter(
            "127.0.0.1", self.metrics, "mixed", tmp_file.name, loader_idx=1, cpu_idx=0
        )
        ...
        assert (
            'collectd_cassandra_stress_hdr_mixed_gauge{cassandra_stress_hdr_mixed="READ",cpu_idx="0",instance="127.0.0.1",keyspace="",loader_idx="1",type="lat_perc_999"} 36.54'
            in expected_lines
        )
        time.sleep(1)
        cs_exporter.stop()
    ...
    def test_03_read(self):
        tmp_file = tempfile.NamedTemporaryFile(mode="w+")
        cs_exporter = CassandraStressHDRExporter(
            "127.0.0.1", self.metrics, "read", tmp_file.name, loader_idx=1, cpu_idx=0
        )
    ...
    def test_04_mixed_only_write(self):
        ...
        assert (
            'collectd_cassandra_stress_hdr_mixed_gauge{cassandra_stress_hdr_mixed="READ",cpu_idx="0",instance="127.0.0.1",keyspace="",loader_idx="1",type="lat_perc_999"} 36.54'
            not in expected_lines
        ), "Not expected log message was found"
        time.sleep(1)
        cs_exporter.stop()
...
class TestYcsbStressThread(BaseSCTEventsTest):
    def test_01(self):
        ...
        thread1 = YcsbStressThread(
            loader_set,
            "bin/ycsb load dynamodb -P workloads/workloada -threads 250 -p recordcount=50000000 -p fieldcount=10 -p fieldlength=1024 -s",
            70,
            "None",
            node_list=[DbNode()],
            params=params,
