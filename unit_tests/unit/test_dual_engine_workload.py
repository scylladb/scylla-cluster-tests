...
- _aggregate_ops_rate: round-robin slice semantics (avgxN) vs additive semantics (sumxN).
...
def test_aggregate_ops_rate_round_robin_slices():
    """Round-robin slice commands (num_commands == num_loaders): avg x num_loaders.
    ...
    avg x num_loaders must be preserved exactly to avoid Argus series drift.
    """
...
def test_aggregate_ops_rate_round_robin_equals_avg_times_loaders():
    """Round-robin path: result matches avg x num_loaders exactly (historical formula)."""
...
def test_aggregate_ops_rate_additive_commands():
    """Additive commands (num_commands != num_loaders): sum x num_loaders.
