...
from sdcm.exceptions import FirewallNotDisabled
...
def test_disable_firewall_drops_the_ruleset_restored_on_every_boot(firewall_node):
    """Flushing the live tables is not enough on the images which persist their rules.

    netfilter-persistent re-applies /etc/iptables/rules.v{4,6} on every boot, so a node which
    was rebooted by a nemesis comes back with everything but SSH REJECTed: locally healthy,
    unreachable for its peers, the loaders and `wait_db_up()` (SCT-479).
    """
    node = firewall_node()

    BaseNode.disable_firewall(node)

    commands = [call.args[0] for call in node.remoter.sudo.call_args_list]
    for service in ("ufw", "netfilter-persistent", "nftables"):
        assert f"systemctl disable --now {service}" in commands
    assert "rm -f /etc/iptables/rules.v4 /etc/iptables/rules.v6" in commands


@pytest.mark.parametrize("iptables", ["iptables", "ip6tables"])
def test_disable_firewall_clears_the_live_tables_of_both_families(firewall_node, iptables):
    ...
    commands = [call.args[0] for call in node.remoter.sudo.call_args_list]
    assert f"{iptables} -F" in commands
    assert f"{iptables} -P INPUT ACCEPT" in commands
...
def test_wait_db_up_diagnoses_a_node_it_waits_for_indefinitely(firewall_node):
    """`wait_for` waits forever on a falsey timeout - the time budget must cope with it."""
    node = firewall_node()
    node.stop_wait_db_up_event = threading.Event()
    node.db_up = unittest.mock.Mock(side_effect=[False, True])
    node.log_cql_unreachable_diagnostics = unittest.mock.Mock()

    with unittest.mock.patch("sdcm.wait.time.sleep"):
        BaseNode.wait_db_up(node, verbose=False, timeout=None, diagnose_after=0)
