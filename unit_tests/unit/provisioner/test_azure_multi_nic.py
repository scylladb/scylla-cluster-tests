...
def nic_specs(count: int) -> list[dict]:
    """NIC specs for `count` IPv4 interfaces, public IP on the primary one only."""
    return [
        {
...
def test_a_vm_named_after_another_vms_nic_keeps_its_own_interfaces(make_provisioner):
    """NICs are matched by name, and one VM name can be a prefix of another's.

    'nsvm' takes three NICs, the last two named 'nsvm-nic1' and 'nsvm-nic2'. A VM actually called
    'nsvm-nic1' then owns a primary NIC named 'nsvm-nic1-nic', which starts with 'nsvm-nic' - a
    plain prefix match would file it under 'nsvm' and hand teardown a NIC of a live node.
    """
    provisioner = make_provisioner(3)
    provisioner.get_or_create_instance(definition("nsvm"), PricingModel.ON_DEMAND)
    provisioner.get_or_create_instance(definition("nsvm-nic1"), PricingModel.ON_DEMAND)

    assert nic_names(provisioner, "nsvm") == ["nsvm-nic", "nsvm-nic1", "nsvm-nic2"]
    assert nic_names(provisioner, "nsvm-nic1") == ["nsvm-nic1-nic", "nsvm-nic1-nic1", "nsvm-nic1-nic2"]
...
def test_terminate_forgets_every_nic_and_public_ip(make_provisioner):
    ...
    What must hold is that *every* NIC and public IP is dropped, not only the primary one: a stale
    cache entry would make a re-provisioned node reuse a resource the resource group cleanup has
    removed. The addresses are enumerated from the NICs, so a teardown that drops the NICs first
    reaches index 0 and nothing beyond it.
    """
    specs = nic_specs(3)
    specs[1]["public_ip"] = True
    provisioner = make_provisioner(3, specs=specs)
    instance = provisioner.get_or_create_instance(definition("teardown-vm"), PricingModel.ON_DEMAND)
    assert provisioner._ip_provider.get(instance.name, index=1).id is not None  # noqa: SLF001
    ...
    for index in range(3):
        assert provisioner._ip_provider.get(instance.name, index=index).id is None, (  # noqa: SLF001
            f"public IP of nic {index} was left in the cache"
        )
