...
class TestListSctRunners(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gce_runner = SctRunnerInfo(
            sct_runner_class=GceSctRunner, cloud_service_instance=None, region_az="us-central1-a", instance=MagicMock(metadata=MagicMock()), instance_name="gce-runner-1", public_ips=["5.6.7.8"], test_id="test-id-2"
        )

        self.azure_runner = SctRunnerInfo(
            sct_runner_class=AzureSctRunner, cloud_service_instance=None, region_az="eastus-1", instance=MagicMock(tags={"RunByUser": "user2"}), instance_name="azure-runner-1", public_ips=["9.10.11.12"], test_id="test-id-1"
        )
...
class TestCleanSctRunners(unittest.TestCase):
    ...
    def setUp(self):
        self.mock_runner_with_keep = MagicMock(
            keep="24", keep_action="terminate", launch_time=datetime.now(timezone.utc), public_ips=["1.2.3.4"], cloud_provider="aws", instance_name="test-runner-1", region_az="us-east-1a", test_id="test-123"
        )

        self.mock_runner_no_keep = MagicMock(keep=None, keep_action=None, launch_time=datetime.now(timezone.utc), public_ips=["5.6.7.8"], cloud_provider="gce", instance_name="test-runner-2", region_az="us-central1-a", test_id="test-456")
    ...
    def test_clean_sct_runners_mixed_filters(self, mock_list_runners):
        ...
        mock_list_runners.assert_called_once_with(backend="aws", test_runner_ip=None, user="test_user", test_id="test-id-123")
    ...
    def test_clean_sct_runners_respect_keep_tags(self, mock_list_runners, mock_ssh_cmd):
        ...
        mock_runner_keep_alive = MagicMock(keep="alive", keep_action="terminate", launch_time=datetime.now(timezone.utc), public_ips=["1.2.3.4"], cloud_provider="aws", instance_name="test-runner-alive", region_az="us-east-1a")
