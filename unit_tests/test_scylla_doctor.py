...
class TestConfiguredEdition:
    """Tests for the configured_edition property."""

    def test_configured_edition_defaults_to_basic(self, mock_node, mock_test_config):
        """When scylla_doctor_edition is not set, configured_edition should default to 'basic'."""
        test_config, params = mock_test_config
        # Don't set scylla_doctor_edition in params — it returns None
        doc = ScyllaDoctor(node=mock_node, test_config=test_config)
        assert doc.configured_edition == "basic"

    def test_configured_edition_full(self, mock_node, mock_test_config):
    ...
    def test_configured_edition_basic_explicit(self, mock_node, mock_test_config):
...
class TestLocateFullScyllaDoctorPackage:
    """Tests for locate_full_scylla_doctor_package."""

    @patch("utils.scylla_doctor.boto3.client")
    @patch("utils.scylla_doctor.KeyStore")
    def test_locate_full_package_found(self, mock_keystore_cls, mock_boto_client, doctor):
    ...
    @patch("utils.scylla_doctor.boto3.client")
    @patch("utils.scylla_doctor.KeyStore")
    def test_locate_full_package_not_found(self, mock_keystore_cls, mock_boto_client, doctor):
    ...
    @patch("utils.scylla_doctor.boto3.client")
    @patch("utils.scylla_doctor.KeyStore")
    def test_locate_full_package_version_not_found(self, mock_keystore_cls, mock_boto_client, doctor):
    ...
    @patch("utils.scylla_doctor.boto3.client")
    @patch("utils.scylla_doctor.KeyStore")
    def test_locate_full_package_latest_when_no_version(self, mock_keystore_cls, mock_boto_client, doctor):
...
class TestDownloadFullScyllaDoctor:
    """Tests for download_full_scylla_doctor."""

    @patch("utils.scylla_doctor.boto3.client")
    @patch("utils.scylla_doctor.KeyStore")
    def test_download_full_generates_presigned_url(
        self, mock_keystore_cls, mock_boto_client, mock_node, mock_test_config
    ):
    ...
    @patch("utils.scylla_doctor.boto3.client")
    @patch("utils.scylla_doctor.KeyStore")
    def test_download_full_raises_when_package_not_found(
        self, mock_keystore_cls, mock_boto_client, mock_node, mock_test_config
    ):
...
class TestDownloadScyllaDoctorDispatch:
    """Tests for download_scylla_doctor edition dispatching."""

    @patch.object(ScyllaDoctor, "download_full_scylla_doctor")
    def test_download_dispatches_to_full_when_edition_is_full(self, mock_download_full, mock_node, mock_test_config):
    ...
    def test_download_fallback_to_basic_on_full_failure(
        self, mock_download_full, mock_boto_client, mock_node, mock_test_config
    ):
    ...
    @patch("utils.scylla_doctor.boto3.client")
    def test_download_basic_edition_skips_full(self, mock_boto_client, mock_node, mock_test_config):
