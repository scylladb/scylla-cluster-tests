...
class TestSdcmWait(unittest.TestCase):
    ...
    def test_02_throw_exc(self):
        ...
        self.assertRaisesRegex(Exception, r"error", wait_for, callback, throw_exc=True, timeout=2, step=0.5, arg1=1, arg2=3)
    ...
    def test_03_false_return_rerise(self):
        ...
        self.assertRaisesRegex(Exception, "callback: timeout - 2 seconds - expired", wait_for, callback, timeout=2, throw_exc=True, step=0.5, arg1=1, arg2=3)
        self.assertEqual(len(calls), 5)
...
class TestSdcmWaitWithEventStop(unittest.TestCase):
    ...
    def test_04_stop_by_event(self, throw_exc):
        ...
        th.start()
        if throw_exc:
            self.assertRaisesRegex(ExitByEventError, "callback: stopped by Event", wait_for, self.callback, timeout=3, throw_exc=throw_exc, stop_event=self.ev, step=0.5, arg1=1, arg2=3)
        else:
    ...
    def test_04_stop_by_event_in_main_thread(self):
        self.callback_return_true_after = 3
        th = ThreadPoolExecutor(max_workers=1).submit(wait_for, func=self.callback, timeout=self.callback_return_true_after, step=0.5, throw_exc=False, stop_event=self.ev, arg1=1, arg2=3)
    ...
    def test_04_raise_by_timeout_before_set_event(self):
        ...
        th.start()
        self.assertRaisesRegex(WaitForTimeoutError, "callback: timeout - 3 seconds - expired", wait_for, self.callback, timeout=3, throw_exc=True, stop_event=self.ev, step=0.5, arg1=1, arg2=3)
    ...
    def test_04_raise_exception_in_func_before_set_event(self, throw_exc):
        ...
        th.start()
        if throw_exc == True:
            self.assertRaisesRegex(ExitByEventError, "callback: stopped by Event", wait_for, callback, timeout=4, throw_exc=throw_exc, stop_event=self.ev, step=0.5, arg1=1, arg2=3)
        else:
    ...
    def test_04_set_event_timeout_at_same_time(self):
        ...
        th = threading.Thread(target=self.set_stop_in_timeout, kwargs={"ev": self.ev, "set_after": 4})
        th.start()
        self.assertRaisesRegex(ExitByEventError, "callback: stopped by Event", wait_for, self.callback, timeout=4, throw_exc=True, stop_event=self.ev, step=0.5, arg1=1, arg2=3)
...
class TestWaitForLogLines:
    def test_can_wait_for_log_start_line_and_end_line(self, tmp_path):
        ...
        lines = [" [shard  0] repair - rebuild_with_repair: started with keyspaces=drop_table_during_repair", "some line in between", "[shard  0] repair - rebuild_with_repair: finished with keyspaces=drop_table_during_repair"]
        write_thread = threading.Thread(target=write_to_file, args=(file_path, lines))
        write_thread.daemon = True
        file_path.touch()
        with wait_for_log_lines(node=node, start_line_patterns=["rebuild.*started with keyspaces=", "Rebuild starts"], end_line_patterns=["rebuild.*finished with keyspaces=", "Rebuild succeeded"], start_timeout=3, end_timeout=5):
            write_thread.start()
    ...
    def test_wait_for_log_timeout_when_no_start_line(self, tmp_path):
        ...
        t.daemon = True
        file_path.touch()
        with pytest.raises(TimeoutError, match="Timeout occurred while waiting for start log line"), wait_for_log_lines(node=node, start_line_patterns=["start"], end_line_patterns=["end"], start_timeout=0.4, end_timeout=1.2):
            t.start()
    ...
    def test_wait_for_log_timeout_when_no_end_line(self, tmp_path):
        ...
        t.daemon = True
        file_path.touch()
        with pytest.raises(TimeoutError, match="Timeout occurred while waiting for end log line"), wait_for_log_lines(node=node, start_line_patterns=["start"], end_line_patterns=["end"], start_timeout=0.5, end_timeout=0.7):
            t.start()
    ...
    def test_wait_for_log_reraises_exception(self, tmp_path):
        ...
        t.daemon = True
        file_path.touch()
        with pytest.raises(ValueError, match="dummy error"), wait_for_log_lines(node=node, start_line_patterns=["start"], end_line_patterns=["end"], start_timeout=0.5, end_timeout=0.7):
            t.start()
    ...
    def test_wait_for_log_reraises_exception_and_timeout_error(self, tmp_path):
        ...
        node = DummyNode(log_path=file_path)
        file_path.touch()
        with pytest.raises(TimeoutError, match="Timeout occurred while waiting for start log line") as exc_info, wait_for_log_lines(node=node, start_line_patterns=["start"], end_line_patterns=["end"], start_timeout=0.4, end_timeout=0.7):
            raise ValueError("dummy error")
    ...
    def test_wait_for_log_reraises_timeout_error_with_error_context(self, tmp_path):
        ...
        with pytest.raises(TimeoutError, match=expected_match), wait_for_log_lines(node=node, start_line_patterns=["start"], end_line_patterns=["end"], start_timeout=0.4, end_timeout=0.7, error_msg_ctx="Wait end line"):
            t.start()
