...
class ServicesIntegrationTest(unittest.TestCase):
    ...
    def test_integration(self):
        try:
            start_events_device(tempfile.mkdtemp(), test_mode=True)
            services = get_from_context()
            self.assertEqual(5, len(services.alive()))
            names = services.names()
            names.sort()
            self.assertEqual(
                ['EventsAnalyzer', 'EventsDevice', 'EventsFileLogger', 'GrafanaAnnotator', 'GrafanaEventAggragator'],
                names)
            self.assertEqual(
                ['EventsFileLogger', 'GrafanaAnnotator', 'EventsAnalyzer'],
                services.find_by_name('EventsDevice')[0].dependants.names())
            services.gradually_stop()
            self.assertEqual(0, len(services.alive()))
        finally:
