...
from sdcm.services.base import NodeService, ClusterService, DetachService, \
    ...
    DetachThreadService, DetachProcessService, find_serviecs_by_instance, find_detached_services, find_services_from_all_sources
...
class ServicesUnitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        find_services_from_all_sources().kill().cleanup()
    ...
    def _init_service(self, cls, raise_event):
        if issubclass(cls, NodeService):
            self.svc_attached_instance = FakeNode(remoter=None, logdir=tempfile.mkdtemp())
            self.svc = cls(self.svc_attached_instance)
        elif issubclass(cls, ClusterService):
            self.svc_attached_instance = FakeCluster()
            self.svc = cls(self.svc_attached_instance)
    ...
    def _get_services(self):
        if isinstance(self.svc, (NodeService, ClusterService)):
            return find_serviecs_by_instance(self.svc_attached_instance)
        elif isinstance(self.svc, DetachService):
            return find_detached_services()
        raise RuntimeError(f'Unexpected service type - {type(self.svc)}')
    ...
    def test_unit_test(self, name, raise_event, cls):
        ...
        self.assertEqual(1, len(registered_services.find_by_name(self.svc.service_name)),
                         msg=f"Result: {','.join(registered_services.find_by_name(self.svc.service_name).names())}")
        ...
        while not self.svc.job_completed and end_time >= time.perf_counter():
            pass
        self.svc.stop()
        self.assertEqual(True, self.svc.wait_till_stopped(0.5))
        ...
        self.assertEqual(0, len(registered_services.alive()),
                         msg=f"Result: {','.join(registered_services.alive().names())}")
        self.assertEqual(0, len(registered_services.find_by_name(self.svc.service_name)),
                         msg=f"Result: {','.join(registered_services.find_by_name(self.svc.service_name).names())}")
