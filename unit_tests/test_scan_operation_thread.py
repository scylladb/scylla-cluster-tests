...
DEFAULT_PARAMS = {"termination_event": Event(), "user": "sla_role_name", "user_password": "sla_role_password", "duration": 10, "interval": 0, "validate_data": True}
...
def test_negative_prometheus_validation_error(events, cluster):
    ...
    with patch.object(PrometheusDBStats, "__init__", return_value=None):
        with patch.object(PrometheusDBStats, "query", return_value=[{"values": [[0, "1"], [1, "1"]]}]):
            ...
            assert severity in all_events[1] and "period_type=end" in all_events[1] and "Fullscan failed - 'mapreduce_service_requests_dispatched_to_other_nodes' was not triggered" in all_events[1]
...
def test_scan_negative_execution_errors(mode, exception, events, node):
    ...
    node.parent_cluster = db_cluster
    default_params = ThreadParams(db_cluster=db_cluster, ks_cf="a.b", mode=mode, full_scan_aggregates_operation_limit=60 * 30, full_scan_operation_limit=300, **DEFAULT_PARAMS)
    with events.wait_for_n_events(events.get_events_logger(), count=2, timeout=10):
        ScanOperationThread(default_params)._run_next_operation()
    all_events = get_event_log_file(events)
    assert "Severity.NORMAL" in all_events[0] and "period_type=begin" in all_events[0]
...
@pytest.mark.parametrize(("mode", "execute_mock"), [["partition", "execute_async"], ["aggregate", "execute"], ["table", "execute"]])
