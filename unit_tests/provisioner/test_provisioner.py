...
def test_can_provision_scylla_vm(region, definition, provisioner, backend, provisioner_params):
    ...
    assert v_m == provisioner.list_instances()[0], "provisioner with the same params should rediscover created resources"
...
def test_can_run_command(provisioner, definition, backend, provisioner_params):
    assert provisioner.get_or_create_instance(definition).run_command('echo "hello instance"').stdout == "hello instance\n"
