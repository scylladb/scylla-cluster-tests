...
def test_user_data_builder_generates_valid_yaml_from_single_user_data_object():
    ...
    assert loaded_yaml["runcmd"][0] == "cd /var/lib/sct/cloud-init; bash -eux /var/lib/sct/cloud-init/0_ExampleUserDataObject.sh; test  $? = 0 || touch /var/lib/sct/cloud-init/0_ExampleUserDataObject.sh.failed"
...
def test_user_data_can_merge_user_data_objects_yaml():
    ...
    assert sorted(loaded_yaml["packages"]) == sorted(["some-pkg-to-install", "another-pkg-to-install", "pkg-from-empty"])
