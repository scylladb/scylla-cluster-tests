...
pytestmark = [
    ...
    pytest.mark.docker_scylla_args(scylla_docker_image="scylladb/scylla-nightly:2025.4.0-dev-0.20250811.e14c5e3890de", vs_docker_image="scylladb/vector-store:latest"),
...
def create_vector_table(db_cluster: "LocalScyllaClusterDummy") -> None:  # noqa: F821
    ...
    with db_cluster.cql_connection_patient(node) as session:
        session.execute("CREATE KEYSPACE IF NOT EXISTS vector_test WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}")
        session.execute("CREATE TABLE IF NOT EXISTS vector_test.embeddings (id UUID PRIMARY KEY, vector VECTOR<FLOAT, 5>, metadata text) WITH cdc = {'enabled': true}")
        session.execute("CREATE INDEX IF NOT EXISTS embeddings_vector_idx ON vector_test.embeddings (vector) USING 'vector_index' WITH OPTIONS = { 'similarity_function': 'DOT_PRODUCT' }")
...
def insert_test_vectors(db_cluster: "LocalScyllaClusterDummy", count: int = 100, vector_dim: int = 5) -> list:  # noqa: F821
    ...
    with db_cluster.cql_connection_patient(db_cluster.nodes[0]) as session:
        insert_stmt = session.prepare("INSERT INTO vector_test.embeddings (id, vector, metadata) VALUES (?, ?, ?)")
...
def wait_for_vector_indexing(vector_client: "VectorStoreClient", timeout: int = 300) -> None:  # noqa: F821
    ...
    while time.time() - start_time < timeout:
        ...
        vector_test_indexes = [idx for idx in indexes if idx.get("keyspace") == "vector_test" and idx.get("index") == "embeddings_vector_idx"]
...
def test_vector_search(docker_scylla, docker_vector_store, params):  # noqa: PLR0914
    ...
    search_results = vector_client.ann_search(keyspace="vector_test", index="embeddings_vector_idx", embedding=query_vector, limit=10)
    ...
    with db_cluster.cql_connection_patient(db_cluster.nodes[0]) as session:
        cql_query = f"SELECT id, metadata FROM vector_test.embeddings ORDER BY vector ANN OF {query_vector} LIMIT 10"
    ...
    for limit in [1, 5, 20]:
        results = vector_client.ann_search(keyspace="vector_test", index="embeddings_vector_idx", embedding=query_vector, limit=limit)
