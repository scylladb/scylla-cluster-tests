...
def create_vector_table(db_cluster: "LocalScyllaClusterDummy") -> None:  # noqa: F821
    ...
    with db_cluster.cql_connection_patient(node) as session:
        ...
        session.execute(
            "CREATE TABLE IF NOT EXISTS vector_test.embeddings "
            "(id UUID PRIMARY KEY, "
...
def wait_for_vector_indexing(vector_client: "VectorStoreClient", timeout: int = 300) -> None:  # noqa: F821
    ...
    while time.time() - start_time < timeout:
        ...
        vector_test_indexes = [
            idx
            for idx in indexes
            if idx.get("keyspace") == "vector_test" and idx.get("index") == "embeddings_vector_idx"
        ]
...
def test_vector_search(docker_scylla, docker_vector_store, params):  # noqa: PLR0914
    ...
    assert closest_distance < 0.001, (
        f"Self-search distance too high for API search: {closest_distance}. Expected near 0."
    )
