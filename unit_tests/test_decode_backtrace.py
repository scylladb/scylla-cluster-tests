    ...
    def test_02_reactor_stalls_is_decoded_if_enabled(self):
        ...
        for event in events:
            if event.get("backtrace") and event.get("raw_backtrace"):
                self.assertEqual(event["backtrace"].strip(), "addr2line -Cpife scylla_debug_info_file {}".format(" ".join(event["raw_backtrace"].split("\n"))))
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(False, None, lambda e: e.get("type") == "REACTOR_STALLED" and e.get("raw_backtrace"), False, id="reactor_stalls_not_decoded_when_stall_decoding_disabled"),
        pytest.param(False, None, lambda e: e.get("raw_backtrace") and e.get("type") != "REACTOR_STALLED", True, id="other_backtraces_decoded_when_stall_decoding_disabled"),
        pytest.param(
            True,
            "^REACTOR_STALLED$",
...
def test_backtrace_decoding_configuration(test_config, dummy_node, monitor_node, events_function_scope, stall_decoding, disable_regex, event_filter, should_decode):
    ...
    for event in filtered_events:
        if should_decode:
            assert event.get("backtrace") is not None, f"Event of type {event.get('type')} should have decoded backtrace"
        else:
            assert event.get("backtrace") is None, f"Event of type {event.get('type')} should not have decoded backtrace"
