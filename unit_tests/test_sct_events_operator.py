...
class TestOperatorEvents(unittest.TestCase):
    ...
    def test_scylla_operator_log_event_tls_handshake_error(self):
        log_record = "I0628 15:53:02.269804       1 operator/operator.go:133] http: TLS handshake error from 172.17.0.1:50882: EOF"
        ...
        expected_timestamp = datetime.datetime(year=datetime.datetime.now().year, month=6, day=28, hour=15, minute=53, second=2, microsecond=269804, tzinfo=datetime.timezone.utc).timestamp()
    ...
    def test_scylla_operator_log_event_operator_started_info(self):
        ...
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
        event.event_id = "9bb2980a-5940-49a7-8b08-d5c323b46aa9"
        expected_timestamp = datetime.datetime(year=datetime.datetime.now().year, month=6, day=28, hour=16, minute=7, second=43, microsecond=572294, tzinfo=datetime.timezone.utc).timestamp()
        assert event.timestamp == expected_timestamp
        self.assertEqual(
    ...
    def test_scylla_operator_log_event_wrong_scheduled_info(self):
        log_record = "I0830 12:35:39              Not allowed pods are scheduled on Scylla node found: kube-proxy-7kjnf (ip-10-0-1-200.eu-north-1.compute.internal node)"
