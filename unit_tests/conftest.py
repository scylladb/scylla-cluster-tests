...
def fixture_docker_scylla(request: pytest.FixtureRequest, params):  # noqa: PLR0914
    ...
    # TODO: use vector search capable dev image for vector store tests, until support of vectore store is added to scylla
    if docker_scylla_args.get('vector_store'):
        default_image = "docker.io/scylladb/scylladb-releng:2025.4.0-dev-0.20250718.d36c9b3119b7-x86_64"
    else:
        default_image = "docker.io/scylladb/scylla-nightly:2025.2.0-dev-0.20250302.0343235aa269"
    docker_version = docker_scylla_args.get('image', default_image)
    ...
    env_vars = '-e VECTOR_SEARCH_TEST=true' if docker_scylla_args.get('vector_store') else ''
...
def fixture_docker_vector_store(request: pytest.FixtureRequest, docker_scylla, params):
    ...
    if not docker_scylla_args.get('vector_store'):
        yield None
        return
