...
from sdcm.provision.helpers.certificate import create_ca, create_certificate, SCYLLA_SSL_CONF_DIR, CA_CERT_FILE, CA_KEY_FILE, TLSAssets
    ...
    try:
        for key, value in vars(scylla_yaml).items():
            if value is not None:
                ...
                remove_cmd = ["docker", "exec", container_id, "bash", "-c", f'sed -i "/^{yaml_key}:/d" /etc/scylla/scylla.yaml']
                subprocess.run(remove_cmd, capture_output=True, text=True, check=False)
                add_cmd = ["docker", "exec", container_id, "bash", "-c", f'echo "{yaml_key}: {yaml_value}" >> /etc/scylla/scylla.yaml']
...
def configure_scylla_node(docker_scylla_args: dict, params, ssl_dir: Path | None = None):  # noqa: PLR0914
    ...
    extra_docker_opts = f'-p {ALTERNATOR_PORT} -p {BaseNode.CQL_PORT} --cpus="1" -v {entryfile_path}:/entry.sh:z{ssl_mount} --user root {env_vars} --entrypoint /entry.sh'
    ...
    scylla = RemoteDocker(LocalNode("scylla", cluster), image_name=docker_version, command_line=f"--smp 1 {alternator_flags}{seeds}", extra_docker_opts=extra_docker_opts, docker_network=docker_network)
    ...
    if ssl_dir:
        ...
        create_certificate(ssl_dir / TLSAssets.CLIENT_CERT, ssl_dir / TLSAssets.CLIENT_KEY, cname="scylladb", ca_cert_file=CA_CERT_FILE, ca_key_file=CA_KEY_FILE)
    ...
    wait.wait_for(func=db_alternator_up, step=1, text="Waiting for DB services to be up alternator)", timeout=120, throw_exc=True)
...
def fixture_docker_vector_store(request: pytest.FixtureRequest, docker_scylla, params):
    ...
    vs_docker_image, vs_version = vs_docker_image_version.rsplit(":", 1) if ":" in vs_docker_image_version else (vs_docker_image_version, "latest")
    ...
    try:
        vector_store_cluster = VectorStoreSetDocker(params=params, vs_docker_image=params.get("vector_store_docker_image"), vs_docker_image_tag=params.get("vector_store_version"), cluster_prefix="test-vector-store", n_nodes=1)
        ...
        restart_result = subprocess.run(["docker", "restart", scylla_node.docker_id], capture_output=True, text=True, check=False)
        if restart_result.returncode != 0:
            raise RuntimeError(f"Container restart failed: {restart_result.stderr}")
        wait.wait_for(func=lambda: scylla_node.is_port_used(port=BaseNode.CQL_PORT, service_name="scylla-server"), step=2, timeout=60, text="Waiting for Scylla container to restart")
