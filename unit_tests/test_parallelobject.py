...
class ParallelObjectTester(unittest.TestCase):
    ...
    def test_successful_parallel_run_func_returning_tuple(self):
        parallel_object = ParallelObject(self.rand_timeouts, timeout=self.max_timout + 2, num_workers=len(self.rand_timeouts))
    ...
    def test_raised_exception_by_timeout(self):
        ...
        with self.assertRaises(ParallelObjectException) as exp:
            parallel_object = ParallelObject(self.rand_timeouts, timeout=test_timeout - 1, num_workers=len(self.rand_timeouts))
