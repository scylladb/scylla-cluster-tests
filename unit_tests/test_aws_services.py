...
def keystore_configure(moto_server):
    ...
    bucket.put_object(Key="aws_images_role.json", Body=b'{"role_arn": "arn:aws:iam::123456789012:role/role-name", "role_session_name": "role-session-name"}')
...
def test_04_get_scylla_ami_versions() -> None:
    ...
    assert {ami.id for ami in amis} == {"ami-03c6a218ada59a3c6", "ami-0c4b2f47376303de9", "ami-0e5174dc58971bf48", "ami-050a1c9be69353d38", "ami-0b676a2642ece0ba8", "ami-760aaa0f", "ami-04c1efb7a7322d71e"}
...
def test_05_ec2_client_spot(aws_region: AwsRegion) -> None:
    ...
    instances = ec2.create_spot_instances(
        ...
        tag_specifications=[{"ResourceType": "spot-instances-request", "Tags": tags_as_ec2_tags({"Name": "test-spot-instance"})}],
        network_if=[{"DeviceIndex": 0, "SubnetId": "subnet-0a1b2c3d4e5f6g7h8", "AssociatePublicIpAddress": True}],
    )
