...
class TestSeedSelector(unittest.TestCase):
    ...
    def setup_cluster(self, nodes_number):
        ...
        for i in range(1, nodes_number + 1):
            self.cluster.nodes.append(DummyNode(name="node%d" % i, parent_cluster=self.cluster, base_logdir=self.temp_dir, ssh_login_info=dict(key_file="~/.ssh/scylla-test")))
    ...
    def test_first_2_seeds(self):
        ...
        self.assertTrue(self.cluster.seed_nodes_addresses == [self.cluster.nodes[0].ip_address, self.cluster.nodes[1].ip_address])
