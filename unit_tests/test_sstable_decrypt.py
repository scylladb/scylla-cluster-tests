...
def setup_node(fake_remoter):
    ...
    Usage::
        ...
        def test_something(setup_node):
            ...
            assert decrypt_sstables_on_node(node, "/snap") is True
...
def test_decrypt_sstables_on_node_happy_path(setup_node):
    """decrypt_sstables_on_node runs upgrade for each Data.db and replaces originals."""
    data_file = "/var/lib/scylla/data/ks/tbl-uuid/snapshots/snap/me-big-Data.db"
    node, _ = setup_node(
        {
            re.compile(r"cat .*/schema\.cql"): ok(SCHEMA_WITH_ENCRYPTION),
            re.compile(r"ls .*/\*-Data\.db.*"): ok(data_file),
            re.compile(r"cat > /tmp/sct_decrypt_schema.*"): ok(),
            re.compile(r"rm -rf /tmp/sct_decrypt_out.* && mkdir -p.*"): ok(),
            re.compile(r".*scylla sstable upgrade.*"): ok("Upgraded."),
            re.compile(r"mv /tmp/sct_decrypt_out.* .*"): ok(),
            re.compile(r"rm -f /tmp/sct_decrypt_schema.*"): ok(),
        }
    )
    assert decrypt_sstables_on_node(node, "/var/lib/scylla/data/ks/tbl-uuid/snapshots/snap") is True


def test_decrypt_sstables_on_node_missing_schema_skips_gracefully(setup_node):
    """decrypt_sstables_on_node exits at the schema-read step, never proceeding to list Data.db files."""
    node, mock_run = setup_node(
        {
            re.compile(r"cat .*/schema\.cql"): fail("No such file"),
        }
    )
    assert decrypt_sstables_on_node(node, "/snapshots/snap") is False
    assert (
        call("ls /snapshots/snap/*-Data.db 2>/dev/null || true", ignore_status=True, verbose=False)
        not in mock_run.call_args_list
    )


def test_decrypt_sstables_on_node_no_data_files_skips_gracefully(setup_node):
    """decrypt_sstables_on_node exits after finding no Data.db files, never invoking the upgrade tool."""
    node, mock_run = setup_node(
        {
...
def test_decrypt_sstables_on_node_upgradefailure_does_not_raise(setup_node):
    """decrypt_sstables_on_node falls back to original files when upgrade fails."""
    ...
    node, _ = setup_node(
        {
            ...
            re.compile(r"rm -f /tmp/sct_decrypt_schema.*"): ok(),
            re.compile(r"rm -rf /tmp/sct_decrypt_out.*"): ok(),
        }
    )
    assert decrypt_sstables_on_node(node, "/snapshots/snap") is False
...
def test_decrypt_sstable_files_on_node_happy_path(setup_node):
    """decrypt_sstable_files_on_node decrypts individual files in-place."""
    ...
    assert decrypt_sstable_files_on_node(node=node, data_files=[data_file], schema_text=SCHEMA_WITH_ENCRYPTION) is True


def test_decrypt_sstable_files_on_node_empty_list_is_noop(setup_node):
    """decrypt_sstable_files_on_node returns immediately when given an empty file list."""
    node, _ = setup_node({})
    assert decrypt_sstable_files_on_node(node, data_files=[], schema_text=SCHEMA_NO_ENCRYPTION) is False


def test_decrypt_sstable_files_on_node_upgradefailure_does_not_raise(setup_node):
    """decrypt_sstable_files_on_node is non-blocking when upgrade fails."""
    ...
    assert decrypt_sstable_files_on_node(node=node, data_files=[data_file], schema_text=SCHEMA_NO_ENCRYPTION) is False
