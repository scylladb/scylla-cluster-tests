...
from sdcm.sct_events.database import get_pattern_to_event_to_func_mapping, CompactionEvent, IndexSpecialColumnErrorEvent, ScyllaServerStatusEvent, DatabaseLogEvent
...
class TestContinuousEventsRegistry:
    ...
    def test_add_nemesis_sub_context_to_continuous_event(self):
        ...
        assert event.msgfmt == ("({0.base} {0.severity}) period_type={0.period_type} event_id={0.event_id} during_nemesis=test1,test2 node={0.node}")
        ...
        assert event.msgfmt == ("({0.base} {0.severity}) period_type={0.period_type} event_id={0.event_id} during_nemesis=test2 duration={0.duration_formatted} node={0.node}")
    ...
    def test_add_nemesis_sub_context_to_information_event(self):
        ...
        assert event.msgfmt == ("({0.base} {0.severity}) period_type={0.period_type} event_id={0.event_id} during_nemesis=test0,test1: message={0.message}")
    ...
    def test_add_nemesis_sub_context_to_db_log_event(self):
        ...
        assert event.msgfmt == ("({0.base} {0.severity}) period_type={0.period_type} event_id={0.event_id} during_nemesis=test0,test1: type={0.type} regex={0.regex} line_number={0.line_number} node={0.node}\n{0.line}")
    ...
    def test_get_compact_events_by_continues_hash_from_log(self, populated_registry: ContinuousEventsRegistry):
        ...
        continues_hash = CompactionEvent.get_continuous_hash_from_dict({"node": "node1", "shard": "2", "table": "system.local", "compaction_process_id": "edc49670-2a65-11ec-a8b8-b62621e7624c"})
