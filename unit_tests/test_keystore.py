...
# Use a bucket name that does NOT exist in real AWS so that if moto ever fails
# to intercept boto3 calls the tests fail loudly instead of corrupting the
# production keystore bucket.
...
def _populate_bucket(s3_resource):
    ...
    s3_resource.create_bucket(Bucket=TEST_KEYSTORE_BUCKET)
    ...
    for key, data in json_objects.items():
        s3_resource.Object(TEST_KEYSTORE_BUCKET, key).put(Body=json.dumps(data).encode())

    s3_resource.Object(TEST_KEYSTORE_BUCKET, "scylla_test_id_ed25519").put(Body=FAKE_SSH_PRIVATE_KEY)
    s3_resource.Object(TEST_KEYSTORE_BUCKET, "scylla_test_id_ed25519.pub").put(Body=FAKE_SSH_PUBLIC_KEY)
...
def mocked_s3(monkeypatch):
    """Provide a moto-mocked S3 environment with a populated test bucket.

    Three layers of defence so a moto failure can never corrupt real AWS:
    1. TEST_KEYSTORE_BUCKET is patched into the module so KeyStore never
       references scylla-qa-keystore.
    2. AWS_ENDPOINT_URL is cleared so moto's URL-pattern matching sees
       *.amazonaws.com URLs (a custom endpoint bypasses moto interception).
    3. Fake credentials ensure any call that does escape moto is rejected
       by real AWS with an auth error rather than silently succeeding.
    """
    ...
    # Undo any session-level conftest patches on KeyStore (e.g. from
    # unit_tests/unit/conftest.py mock_cloud_services autouse fixture) so
    # moto intercepts the real boto3 calls.  The module-level shadow fixture
    # (mock_cloud_services below) handles this at module scope, but under
    # pytest-xdist a unit_tests/unit/ test can activate the session fixture
    # before this module is collected, leaving the class-level stubs active.
    for _name, _original in _KEYSTORE_ORIGINALS.items():
        monkeypatch.setattr(keystore_module.KeyStore, _name, _original, raising=False)
    with mock_aws():
...
class TestGetJson:
    ...
    def test_invalid_json_raises(self, ks, mocked_s3):
        mocked_s3.Object(TEST_KEYSTORE_BUCKET, "bad.json").put(Body=b"{not json")
        with pytest.raises(json.JSONDecodeError):
            ks.get_json("bad.json")

    def test_empty_object_is_valid(self, ks, mocked_s3):
        mocked_s3.Object(TEST_KEYSTORE_BUCKET, "empty.json").put(Body=b"{}")
...
class TestCredentialGetters:
    ...
    def test_gcp_custom_project(self, ks, mocked_s3):
        ...
        mocked_s3.Object(TEST_KEYSTORE_BUCKET, "my-project.json").put(Body=json.dumps(custom).encode())
    ...
    def test_gcp_service_accounts_no_duplicate_scope(self, ks, mocked_s3):
        ...
        mocked_s3.Object(TEST_KEYSTORE_BUCKET, "gcp-sct-project-1_service_accounts.json").put(
...
class TestCaching:
    ...
    def test_bypass_cache_updates_cached_value(self, ks, mocked_s3):
        ...
        mocked_s3.Object(TEST_KEYSTORE_BUCKET, "email_config.json").put(Body=new_data)
...
class TestErrorCases:
    ...
    def test_get_json_invalid_content(self, ks, mocked_s3):
        mocked_s3.Object(TEST_KEYSTORE_BUCKET, "bad.json").put(Body=b"{{bad")
        with pytest.raises(json.JSONDecodeError):
            ks.get_json("bad.json")
