...
#
# The conftest at unit_tests/unit/conftest.py has a session-scoped autouse
# fixture that patches KeyStore methods via patch.object().  Under xdist,
# pytest collects all conftest files for all workers, so even workers that
# only run test_keystore.py still activate the session patches.
#
# We reload the keystore module to get a fresh, unpatched KeyStore class,
# then copy its methods onto the original class for the duration of this
# module's tests.
# ---------------------------------------------------------------------------

# Methods that conftest's session fixture may have patched on KeyStore.
_PATCHED_NAMES = (
    "__init__",
    "s3",
    "s3_client",
...
def mock_cloud_services():
    ...
    Under xdist, the session-scoped conftest fixture may have already patched
    KeyStore by the time this module runs.  We reload the keystore module to
    get pristine originals that are guaranteed unpatched.
    """
    fresh_module = importlib.reload(keystore_module)
    FreshKeyStore = fresh_module.KeyStore

    saved = {name: KeyStore.__dict__.get(name) for name in _PATCHED_NAMES if name in KeyStore.__dict__}

    for name in _PATCHED_NAMES:
        if name in FreshKeyStore.__dict__:
            setattr(KeyStore, name, FreshKeyStore.__dict__[name])

    # Reload creates a new SSHKey NamedTuple; update reference for isinstance checks.
    this_module = sys.modules[__name__]
    this_module.SSHKey = fresh_module.SSHKey
    this_module.SecretsManagerEntryMissing = fresh_module.SecretsManagerEntryMissing
...
@pytest.fixture
def ks(mocked_s3, monkeypatch):
    """Return a fresh KeyStore wired to the moto-mocked S3 backend."""
...
class TestMaterializeSshKey:
    def test_writes_key_bytes(self, mocked_s3, monkeypatch):
        monkeypatch.setenv("SCT_KEYSTORE_BACKEND", "s3")
        with materialize_ssh_key("scylla_test_id_ed25519") as key_path:
            assert key_path.read_bytes() == FAKE_SSH_PRIVATE_KEY

    def test_sets_0600_perms(self, mocked_s3, monkeypatch):
        monkeypatch.setenv("SCT_KEYSTORE_BACKEND", "s3")
        with materialize_ssh_key("scylla_test_id_ed25519") as key_path:
            mode = key_path.stat().st_mode & 0o777
        assert mode == 0o600, f"expected 0o600, got {oct(mode)}"

    def test_removes_file_on_exit(self, mocked_s3, monkeypatch):
        monkeypatch.setenv("SCT_KEYSTORE_BACKEND", "s3")
        with materialize_ssh_key("scylla_test_id_ed25519") as key_path:
            captured = key_path
    ...
    def test_removes_file_on_exception(self, mocked_s3, monkeypatch):
        monkeypatch.setenv("SCT_KEYSTORE_BACKEND", "s3")
        key_path = None
        try:
            with materialize_ssh_key("scylla_test_id_ed25519") as kp:
                key_path = kp
    ...
    def test_prefers_tmpfs_when_available(self, mocked_s3, monkeypatch):
...
class TestSecretsManagerBackend:
    ...
    def test_missing_secret_raises(self, sm_ks):
        with pytest.raises(SecretsManagerEntryMissing) as exc_info:
            sm_ks.get_file_contents("nonexistent.json")
        message = str(exc_info.value)
        assert "sct/nonexistent.json" in message
        assert "validate-keystore-sm.sh" in message

    def test_missing_secret_raises_on_version_lookup(self, sm_ks, tmp_path):
        with pytest.raises(SecretsManagerEntryMissing) as exc_info:
            sm_ks.get_sm_version_id("nonexistent")
        assert "sct/nonexistent" in str(exc_info.value)

    def test_missing_secret_surfaces_from_sync(self, sm_ks, tmp_path):
        # Reproduces the Jenkins failure mode: sync() calls get_obj_if_needed(),
        # which calls get_sm_version_id() first when the backend is SM — so a
        # non-migrated key must bubble up as SecretsManagerEntryMissing, not as
        # the opaque botocore ClientError the original code exposed.
        with pytest.raises(SecretsManagerEntryMissing):
            sm_ks.sync(["nonexistent"], str(tmp_path), 0o600)
    ...
    def test_s3_backend_is_fallback_when_env_unset(self, mocked_s3):
        with patch.dict(os.environ, {}, clear=False):
