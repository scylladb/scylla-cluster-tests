...
@pytest.mark.skipif(True, reason="Skipping integration test for kernel panic detection., should be run manually as needed.")
@pytest.mark.integration
@pytest.mark.parametrize("create_sct_runner", [
...
def test_check_kernel_panic(create_sct_runner, events):
    ...
    if backend == "aws":
        ...
        checker = AzureKernelPanicChecker(node=instance, vm_name=instance_identifier, region=region,
                                          resource_group=sct_runner._instance._provisioner._resource_group_name)
    else:
        raise ValueError(f"Unsupported backend: {backend}")

    with checker:
        LOGGER.info("Started checking kernel panic for instance %s in %s.",
                    instance_identifier, location_value)
        ...
        if wait_for_kernel_panic_event(events, timeout=300):
            ...
            raise Exception(f"No kernel panic event detected within timeout for backend: {backend}")
