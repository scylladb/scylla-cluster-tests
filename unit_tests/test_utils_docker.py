...
from sdcm.utils.docker_utils import _Name, ContainerManager, DockerException, NotFound, ImageNotFound, NullResource, Retry, ContainerAlreadyRegistered
...
class TestContainerManager(unittest.TestCase):
    ...
    def test_get_docker_client(self):
        with self.subTest("Default Docker client"):
            self.assertEqual(ContainerManager.get_docker_client(self.node, "c2"), ContainerManager.default_docker_client)
        ...
        with self.subTest("Node-wide Docker client (callable, with member)"):
            ...
            self.assertEqual(ContainerManager.get_docker_client(self.node, "c2:blah"), sentinel.node_callable_docker_client)
            self.node.docker_client.assert_called_once_with()
        ...
        with self.subTest("Docker client per container family (callable, with member)"):
            self.node.docker_client = Mock(return_value=None)
            self.node.c2_docker_client = Mock(return_value=None)
            self.assertEqual(ContainerManager.get_docker_client(self.node, "c2:blah"), ContainerManager.default_docker_client)
    ...
    def test_run_container(self):
        ...
        with self.subTest("Verify that all containers are there"):
            ...
            set2 = {ContainerManager.get_container(self.node, name) for name in ("c1", "c2", "c2:another", "c3", "c3:another")}
    ...
    def test_ssh_copy_id(self):
        with self.subTest("Copy SSH pub key to non-existent container"):
            self.assertRaises(NotFound, ContainerManager.ssh_copy_id, self.node, "c2", user="root", key_file="/root/.ssh/id_rsa")
        ...
        with self.subTest("Check exec_run failure"):
            ...
            with patch("paramiko.rsakey.RSAKey.from_private_key_file") as rsa_key_mock:
                self.assertRaisesRegex(DockerException, "blah", ContainerManager.ssh_copy_id, self.node, "c1", user=sentinel.ssh_user, key_file="lala.key")

    def test_register_container(self):
        c2_container = DummyContainer()

        with self.subTest("Try to register registered container"):
            self.assertRaisesRegex(ContainerAlreadyRegistered, "container .* registered already", ContainerManager.register_container, self.node, "c2", container=self.container)

        with self.subTest("Try to register registered container with replace=True"):
            self.assertRaisesRegex(ContainerAlreadyRegistered, "container .* registered already", ContainerManager.register_container, self.node, "c2", container=self.container, replace=True)

        with self.subTest("Try to replace container"):
            self.assertRaisesRegex(ContainerAlreadyRegistered, "another container registered", ContainerManager.register_container, self.node, "c1", container=c2_container)
        ...
        with self.subTest("Try to force container replacement with container registered with another name"):
            self.assertRaisesRegex(ContainerAlreadyRegistered, "container .* registered already", ContainerManager.register_container, self.node, "c1", container=self.container, replace=True)
    ...
    def test_build_container_image(self):
        ...
        with self.subTest("Node-wide dockerfile args"):
            ...
            self.assertRaisesRegex(AssertionError, "Dockerfile", ContainerManager.build_container_image, self.node, "c2")
        ...
        with self.subTest("No build args"):
            ...
            self.assertEqual(image, ((), dict(tag="hello-world:latest", path=".", labels=self.node.tags, pull=True, rm=True, arg1="value1", arg2="value2")))
        ...
        with self.subTest("No build args"):
            ...
            image = ContainerManager.build_container_image(self.node, "c2:another", arg1="value1", arg2="value2")
            self.assertEqual(image, ((), dict(tag="hello-world:latest", path=".", labels=self.node.tags, pull=True, rm=True, value1="arg1", value2="arg2")))
            self.node.c2_container_image_tag.assert_called_once_with("another")
            self.node.c2_container_image_dockerfile_args.assert_called_once_with("another")
