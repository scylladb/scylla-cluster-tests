...
class TestSystemEvents(unittest.TestCase):
    ...
    def test_elasticsearch_event(self):
        ...
        self.assertEqual(
            str(event),
            "(ElasticsearchEvent Severity.ERROR) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: doc_id=d1 error=e1",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
    ...
    def test_spot_termination_event(self):
        ...
        self.assertEqual(
            str(event),
            "(SpotTerminationEvent Severity.CRITICAL) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: node=node1 message=m1",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
    ...
    def test_scylla_repo_event(self):
        ...
        self.assertEqual(
            str(event),
            "(ScyllaRepoEvent Severity.WARNING) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: url=u1 error=e1",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
    ...
    def test_thread_failed_event(self):
        ...
        self.assertEqual(
            str(event),
            "(ThreadFailedEvent Severity.ERROR) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: message=m1\nt1",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
