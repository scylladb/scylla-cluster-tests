...
class TestSystemEvents(unittest.TestCase):
    def test_startup_test_event(self):
        ...
        self.assertEqual(str(event), "(StartupTestEvent Severity.NORMAL) period_type=not-set event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e")
    ...
    def test_test_framework_event(self):
        ...
        self.assertEqual(
            str(event),
            "(TestFrameworkEvent Severity.ERROR) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e, source=s1.m1(args=('a1', 'a2'), kwargs={'k1': 'v1', 'k2': 'v2'}) message=msg1\nexception=e1",
        )
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
    ...
    def test_elasticsearch_event(self):
        ...
        event.event_id = "aff29bce-d75c-4f86-9890-c6d9c1c25d3e"
        self.assertEqual(str(event), "(ElasticsearchEvent Severity.ERROR) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: doc_id=d1 error=e1")
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))

    def test_spot_termination_event(self):
        event = SpotTerminationEvent(node="node1", message="m1")
        event.event_id = "aff29bce-d75c-4f86-9890-c6d9c1c25d3e"
        self.assertEqual(str(event), "(SpotTerminationEvent Severity.CRITICAL) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: node=node1 message=m1")
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))

    def test_scylla_repo_event(self):
        event = ScyllaRepoEvent(url="u1", error="e1")
        event.event_id = "aff29bce-d75c-4f86-9890-c6d9c1c25d3e"
        self.assertEqual(str(event), "(ScyllaRepoEvent Severity.WARNING) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: url=u1 error=e1")
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))

    def test_info_event(self):
        event = InfoEvent(message="m1")
        event.event_id = "aff29bce-d75c-4f86-9890-c6d9c1c25d3e"
        self.assertEqual(str(event), "(InfoEvent Severity.NORMAL) period_type=not-set event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: message=m1")
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))

    def test_thread_failed_event(self):
        event = ThreadFailedEvent(message="m1", traceback="t1")
        event.event_id = "aff29bce-d75c-4f86-9890-c6d9c1c25d3e"
        self.assertEqual(str(event), "(ThreadFailedEvent Severity.ERROR) period_type=one-time event_id=aff29bce-d75c-4f86-9890-c6d9c1c25d3e: message=m1\nt1")
        self.assertEqual(event, pickle.loads(pickle.dumps(event)))
...
class TestInstanceStatusEvent(unittest.TestCase):
    ...
    def test_instance_reboot_event(self):
        ...
        self.assertIs(event1, event1.add_info(node="n1", line="Stopped target Host and Network Name Lookups", line_number=0))
        self.assertEqual(event1.severity, Severity.WARNING)
        self.assertEqual(event1.node, "n1")
