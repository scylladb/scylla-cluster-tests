...
class Node:
    def __init__(self):
        self.ssh_login_info = {"hostname": "34.253.205.91", "user": "centos", "key_file": "~/.ssh/scylla_test_id_ed25519"}
...
class EC2ClientTests(unittest.TestCase):
    @staticmethod
    def test_01():
        network_if = [{"DeviceIndex": 0, "SubnetId": "subnet-ad3ce9f4", "AssociatePublicIpAddress": True, "Groups": ["sg-5e79983a"]}]
...
class TestCassandraStressHDRExporter(unittest.TestCase):
    ...
    def test_01_mixed(self):
        tmp_file = tempfile.NamedTemporaryFile(mode="w+")
        cs_exporter = CassandraStressHDRExporter("127.0.0.1", self.metrics, "mixed", tmp_file.name, loader_idx=1, cpu_idx=0)
    ...
    def test_02_write(self):
        tmp_file = tempfile.NamedTemporaryFile(mode="w+")
        cs_exporter = CassandraStressHDRExporter("127.0.0.1", self.metrics, "write", tmp_file.name, loader_idx=1, cpu_idx=0)
    ...
    def test_03_read(self):
        tmp_file = tempfile.NamedTemporaryFile(mode="w+")
        cs_exporter = CassandraStressHDRExporter("127.0.0.1", self.metrics, "read", tmp_file.name, loader_idx=1, cpu_idx=0)
...
class TestYcsbStressThread(BaseSCTEventsTest):
    def test_01(self):
        ...
        thread1 = YcsbStressThread(loader_set, "bin/ycsb load dynamodb -P workloads/workloada -threads 250 -p recordcount=50000000 -p fieldcount=10 -p fieldlength=1024 -s", 70, "None", node_list=[DbNode()], params=params)

        thread2 = YcsbStressThread(
            loader_set, "bin/ycsb run dynamodb -P workloads/workloada -threads 100 -p recordcount=50000000 -p fieldcount=10 -p fieldlength=1024 -p operationcount=10000 -s", 70, "None", node_list=[DbNode()], params=params
        )
