...
def fixture_cql_driver(docker_scylla):
    ...
    port = int(port)
    return Cluster([node_ip], port=port, auth_provider=PlainTextAuthProvider(username="cassandra", password="cassandra"))


@pytest.fixture(scope="function")
def create_table(docker_scylla, cql_driver, alternator_api, params):
    with cql_driver.connect() as session:
        session.execute("CREATE ROLE %s WITH PASSWORD = %s AND login = true AND superuser = true", (params.get("alternator_access_key_id"), params.get("alternator_secret_access_key")))
...
def create_cql_ks_and_table(cql_driver):
    ...
    session.execute("""create keyspace ycsb WITH REPLICATION = {'class' : 'NetworkTopologyStrategy', 'replication_factor': 1 };""")
    session.execute(
...
def test_01_dynamodb_api(request, docker_scylla, prom_address, params):
    ...
    ycsb_thread = YcsbStressThread(loader_set, cmd, node_list=[docker_scylla], timeout=5, params=params, cluster_tester=MagicMock())
...
def test_02_dynamodb_api_dataintegrity(request, docker_scylla, prom_address, events, params):
    ...
    ycsb_thread1 = YcsbStressThread(loader_set, cmd, node_list=[docker_scylla], timeout=30, params=params, cluster_tester=MagicMock())
    ...
    ycsb_thread2 = YcsbStressThread(loader_set, cmd, node_list=[docker_scylla], timeout=30, params=params, cluster_tester=MagicMock())
...
def test_03_cql(request, docker_scylla, prom_address, params):
    ...
    cmd = "bin/ycsb load scylla -P workloads/workloada -threads 5 -p recordcount=1000000 -p fieldcount=10 -p fieldlength=1024 -p operationcount=200200300 -s"
...
def test_04_insert_new_data(docker_scylla, alternator_api):
    ...
    assert {row[schema_keys[0]]: row[schema_keys[1]] for row in new_items} == {row[schema_keys[0]]: row[schema_keys[1]] for row in data}
