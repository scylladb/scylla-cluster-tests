...
def test_static_ips_azure_failure_continues_with_other_clouds():
    ...
    with (
        patch("sdcm.utils.cloud_monitor.resources.static_ips.AzureService") as mock_azure_service,
        ...
        mock_cloud_instances = {"aws": [], "gce": [], "azure": []}
...
def test_cloud_instances_azure_failure_continues_with_other_clouds():
    ...
    with (
        patch("sdcm.utils.cloud_monitor.resources.instances.AzureService") as mock_azure_service,
...
@pytest.mark.parametrize(
    "side_effect, expected_call_count, expected_sleep_calls",
    [
        (
            [HttpResponseError("RateLimiting - Client application has been throttled"), HttpResponseError("RateLimiting - Client application has been throttled"), Mock(data=[{"test": "data"}], skip_token=None, result_truncated="false")],
            3,
            2,
        ),
...
def test_azure_resource_graph_query_rate_limiting_retry(side_effect, expected_call_count, expected_sleep_calls):
    ...
    with patch("sdcm.utils.azure_utils.time.sleep") as mock_sleep:
        ...
        with patch.object(azure_service, "resource_graph", mock_resource_graph), patch.object(azure_service, "subscription_id", "test-subscription"):
            # Execute query
