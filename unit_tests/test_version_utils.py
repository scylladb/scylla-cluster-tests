...
DEB_URL = f"{BASE_S3_DOWNLOAD_URL}/enterprise/deb/unstable/stretch/9f724fedb93b4734fcfaec1156806921ff46e956-2bdfa9f7ef592edaf15e028faf3b7f695f39ebc1-525a0255f73d454f8f97f32b8bdd71c8dec35d3d/68/scylladb-2019.1/scylla.list"
RPM_URL = (
    f"{BASE_S3_DOWNLOAD_URL}/enterprise/rpm/unstable/centos/9f724fedb93b4734fcfaec1156806921ff46e956-2bdfa9f7ef592edaf15e028faf3b7f695f39ebc1-525a0255f73d454f8f97f32b8bdd71c8dec35d3d-a6b2b2355c666b1893f702a587287da978aeec22/71/scylla.repo"
)
...
class TestVersionUtils(unittest.TestCase):
    def check_multiple_urls(self, urls):
        ...
        for branch_version, expected_version in zip(get_branch_version_for_multiple_repositories(urls=repo_urls), expected_versions):
    ...
    def test_06_get_scylla_urls_from_repository_rpm_one_arch(self):
        ...
        self.assertEqual(
            {
                "http://downloads.scylladb.com/unstable/scylla/master/rpm/centos/2021-06-29T00:59:00Z/scylla/noarch/repodata/repomd.xml",
                "http://downloads.scylladb.com/unstable/scylla/master/rpm/centos/2021-06-29T00:59:00Z/scylla/x86_64/repodata/repomd.xml",
            },
            urls,
        )
        ...
        with unittest.mock.patch.object(sdcm.utils.version_utils, "get_url_content", return_value="", clear=True):
            urls = get_scylla_urls_from_repository(
                RepositoryDetails(
                    ...
                    urls=["deb [arch=amd64] http://downloads.scylladb.com/unstable/scylla/master/deb/unified/2021-06-23T10:53:35Z/scylladb-master stable main"],
                )
            )
        self.assertEqual(
            {
                "http://downloads.scylladb.com/unstable/scylla/master/deb/unified/2021-06-23T10:53:35Z/scylladb-master/dists/stable/main/binary-amd64/Packages",
            },
            urls,
        )
    ...
    def test_06_get_scylla_urls_from_repository_deb_two_archs(self):
        with unittest.mock.patch.object(sdcm.utils.version_utils, "get_url_content", return_value="", clear=True):
            urls = get_scylla_urls_from_repository(
                RepositoryDetails(
                    type=ScyllaFileType.DEBIAN,
                    urls=["deb [arch=amd64,arm64] http://downloads.scylladb.com/unstable/scylla/master/deb/unified/2021-06-23T10:53:35Z/scylladb-master stable main"],
                )
            )
        self.assertEqual(
            {
                "http://downloads.scylladb.com/unstable/scylla/master/deb/unified/2021-06-23T10:53:35Z/scylladb-master/dists/stable/main/binary-amd64/Packages",
                "http://downloads.scylladb.com/unstable/scylla/master/deb/unified/2021-06-23T10:53:35Z/scylladb-master/dists/stable/main/binary-arm64/Packages",
            },
            urls,
        )
    ...
    def test_07_get_all_versions(self):
        self.assertIn("4.5.3", get_all_versions("https://s3.amazonaws.com/downloads.scylladb.com/rpm/centos/scylla-4.5.repo"))
    ...
    def test_09_assume_versions(self):
        with unittest.mock.patch.object(os.environ, "get", return_value="branch-2022.1", clear=True):
            ...
            repo_url = "http://downloads.scylladb.com/unstable/scylla-enterprise/enterprise-2022.1/deb/unified/2022-05-10T22:12:50Z/scylladb-2022.1/scylla.list"
...
@pytest.mark.parametrize(
    ...
    + [
        (scylla_version, method)
        for scylla_version in ("2019.1", "2019.1.0", "2019.1.1", "2020.1", "2020.1.0", "2020.1.1", "2021.1", "2021.1.0", "2021.1.1", "2022.1", "2022.1.0", "2022.1.1", "2023:latest")
    ...
    + [(scylla_version, method) for scylla_version in ("2022.1.rc1", "2022.1", "2022.1.0", "2022.1.1", "2023:latest") for method in ("new_es_method", "new_mixed_method")],
)
...
def test_scylla_versions_decorator_negative(scylla_version, method):
    for nemesis_like_class in (True, False):
        try:
            ...
            assert "Method '{}' with version '{}' is not supported in '{}'!".format(method, scylla_version, cls_instance.__class__.__name__) in str(exc)
        else:
...
def test_scylla_versions_decorator_negative_latest_scylla_no_nodes():
    ...
    for nemesis_like_class in (True, False):
        try:
            ...
            assert "Method 'oss_method' with version 'n/a' is not supported in '{}'!".format(cls_instance.__class__.__name__) in str(exc)
        else:
            assert False, f"Versioned method must have been not found for the '{scylla_version}' scylla version"
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("2022.1.3-0.20220922.539a55e35 with build-id d1fb2faafd95058a04aad30b675ff7d2b930278d", (2022, 1, 3, "dev-0.20220922", "539a55e35")),
...
def _compare_versions(version_string_left, version_string_right, is_left_greater, is_equal, comparable_class):
    ...
    compare_expected_result_err_msg = "One of 'is_left_greater' and 'is_equal' must be 'True' and another one must be 'False'"
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("5.2.0-dev-0.20230109.8ff4717fd010", "5.2.0-dev-0.20230109.08b3a9c786d9", False, True, ComparableScyllaVersion),
    ),
)
def test_comparable_scylla_versions_compare(version_string_left, version_string_right, is_left_greater, is_equal, comparable_class):
...
def test_comparable_scylla_operator_versions_init_negative(version_string):
    try:
        ...
        assert False, f"'ComparableScyllaOperatorVersion' must raise a ValueError for the '{version_string}' provided input"
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("scylla-operator-v1.8.0-alpha.0-100-gf796b97", "1.8.0-alpha.0-99-g1234567", True, False, ComparableScyllaOperatorVersion),
        ("scylla-operator-v1.8.0-alpha.0-10-gf796b97", "1.8.0-alpha.0-9-g1234567", True, False, ComparableScyllaOperatorVersion),
        ...
        ("scylla-manager-v1.8.0-alpha.0-100-gf796b97", "1.8.0-alpha.0-100-gf796b97", False, True, ComparableScyllaOperatorVersion),
    ),
)
def test_comparable_scylla_operator_versions_compare(version_string_left, version_string_right, is_left_greater, is_equal, comparable_class):
    _compare_versions(version_string_left, version_string_right, is_left_greater, is_equal, comparable_class)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("enterprise-2024.2:latest", "centos", "unstable/scylla-enterprise/enterprise-2024.2/rpm/centos/latest/scylla.repo"),
        ("enterprise-2024.2:latest", "ubuntu", "unstable/scylla-enterprise/enterprise-2024.2/deb/unified/latest/scylladb-2024.2/scylla.list"),
        ("enterprise-2024.1:latest", "debian", "unstable/scylla-enterprise/enterprise-2024.1/deb/unified/latest/scylladb-2024.1/scylla.list"),
    ),
)
