...
def test_is_conflict_error_with_http_response_error(kms_provider):
...
def test_is_conflict_error_with_message(kms_provider):
...
def test_is_conflict_error_with_regular_error(kms_provider):
...
@patch('sdcm.provision.azure.kms_provider.LOGGER')
def test_get_or_create_keyvault_success(mock_logger, kms_provider, azure_service_mock):
...
def test_vm_creation_fails_when_kms_returns_none(vm_provider):
    ...
    with patch('sdcm.provision.azure.virtual_machine_provider.AzureKmsProvider') as mock_kms_class:
        ...
        # Create a minimal instance definition with properly mocked SSH key
        mock_ssh_key = MagicMock()
        mock_ssh_key.name = "test-key"
        mock_ssh_key.public_key = b"ssh-rsa AAAAB3..."
...
def test_vm_creation_succeeds_with_valid_kms(vm_provider):
