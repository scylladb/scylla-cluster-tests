...
class TestGceErrorLog(BaseEventsTest):
    ...
    def test_host_error_log_entry_creates_sct_error_event(self):
        ...
        assert (
            "compute.instances.hostError on node longevity-10gb-3h-master-db-node-fac7b27a-0-6 at 2022-06-30"
            in error_events
        )
        assert (
            "compute.instances.automaticRestart on node longevity-10gb-3h-master-db-node-fac7b27a-0-6 at 2022-06-30 "
            in error_events
        )
        warning_events = self.get_event_log_file("warning.log")
        assert (
            "compute.instances.some on node longevity-10gb-3h-master-db-node-fac7b27a-0-6 at 2022-06-30"
            in warning_events
        )
