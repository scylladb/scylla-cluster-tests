...
class FakeGceLogClient(GceLoggingClient):
    ...
    def get_system_events(self, from_: float, until: float):
        ...
        entry = {
            ...
            "resource": {"type": "gce_instance", "labels": {"zone": "us-east1-d", "project_id": "skilled-adapter-452", "instance_id": "2074079198322937303"}},
            "timestamp": "2022-06-30T14:07:23.102868Z",
            "severity": "INFO",
            "logName": "projects/skilled-adapter-452/logs/cloudaudit.googleapis.com%2Fsystem_event",
            "operation": {"id": "systemevent-1656598023967-5e2aac8c11261-3553f149-42656705", "producer": "compute.instances.hostError", "first": True, "last": True},
...
class TestGceErrorLog(BaseEventsTest):
    ...
    def test_host_error_log_entry_creates_sct_error_event(self):
        ...
        assert "compute.instances.hostError on node longevity-10gb-3h-master-db-node-fac7b27a-0-6 at 2022-06-30" in error_events
        assert "compute.instances.automaticRestart on node longevity-10gb-3h-master-db-node-fac7b27a-0-6 at 2022-06-30 " in error_events
        warning_events = self.get_event_log_file("warning.log")
        assert "compute.instances.some on node longevity-10gb-3h-master-db-node-fac7b27a-0-6 at 2022-06-30" in warning_events
