...
def test_check_ssm_prerequisites_success(mock_boto_client, region, instance_id, mock_boto_clients):
    ...
    # Mock SSM response
    mock_ssm.describe_instance_information.return_value = {"InstanceInformationList": [{"PingStatus": "Online"}]}
...
def test_run_success(mock_sleep, mock_boto_client, region, instance_id, mock_boto_clients):
    ...
    mock_ssm.get_command_invocation.return_value = {"Status": "Success", "StandardOutputContent": "Hello World", "StandardErrorContent": ""}
...
def test_run_with_log_file(mock_sleep, mock_file_open, mock_boto_client, region, instance_id, mock_boto_clients):
    ...
    mock_ssm.describe_instance_information.return_value = {"InstanceInformationList": [{"PingStatus": "Online"}]}
    mock_ssm.send_command.return_value = {"Command": {"CommandId": "cmd-123"}}
    mock_ssm.get_command_invocation.return_value = {"Status": "Success", "StandardOutputContent": "Test output", "StandardErrorContent": ""}

    runner = SSMCommandRunner(region, instance_id)
    result = runner.run(cmd='echo "Test"', log_file="/tmp/output.txt")
