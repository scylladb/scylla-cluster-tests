...
class TestNemesisClass(Nemesis):
    ...
    def __init__(self, tester_obj, termination_event, *args, nemesis_selector=None, nemesis_seed=None, **kwargs):
        super().__init__(tester_obj, termination_event, *args, nemesis_selector=nemesis_selector, nemesis_seed=nemesis_seed, **kwargs)
...
@pytest.mark.parametrize(
    "params, expected",
    [
        pytest.param({"nemesis_exclude_disabled": True}, {"disrupt_method_a", "disrupt_method_c", "disrupt_rnd_method"}, id="exclude_disabled"),
        pytest.param({"nemesis_exclude_disabled": False}, {"disrupt_method_a", "disrupt_method_c", "disrupt_rnd_method", "disrupt_method_b"}, id="disabled"),
    ],
)
...
def test_add_sisyphus_with_filter_in_parallel_nemesis_run(tmp_path):
    ...
    for i, nemesis_settings in enumerate(nemesises):
        assert nemesis_settings["nemesis"] == SisyphusMonkey, f"Wrong instance of nemesis class {nemesis_settings['nemesis']} expected SisyphusMonkey"
        assert nemesis_settings["nemesis_selector"] == expected_selectors[i], f"Wrong nemesis filter selecters {nemesis_settings['nemesis_selector']} expected {expected_selectors[i]}"
