...
def test_select_and_release_node(allocator_with_nodes):
    allocator, _, _ = allocator_with_nodes

    # select a node
    selected_node, _ = allocator.select_target_node(
        nemesis_name="TestNemesis", pool_type=NEMESIS_TARGET_POOLS.data_nodes, filter_seed=False)
    assert selected_node is not None
    assert selected_node in allocator.active_nemeses_on_nodes
    assert allocator.active_nemeses_on_nodes[selected_node] == "TestNemesis"

    # select another node
    second_node, _ = allocator.select_target_node(
    ...
    allocator.release_busy_node(selected_node, "TestNemesis")
    assert selected_node not in allocator.active_nemeses_on_nodes


def test_no_available_nodes_returns_none(allocator_with_nodes):
    ...
    for i, node in enumerate(data_nodes):
        allocator.mark_node_as_busy(node, f"Nemesis-{i}")
...
def test_switch_target_node(allocator_with_nodes):
    ...
    allocator.mark_node_as_busy(nodes[1], "SwitchNemesis")
    assert nodes[1] in allocator.active_nemeses_on_nodes

    switch_ok = allocator.switch_target_node(old_node=nodes[1], new_node=nodes[2], nemesis_name="SwitchNemesis")
    assert switch_ok
    assert nodes[1] not in allocator.active_nemeses_on_nodes
    assert nodes[2] in allocator.active_nemeses_on_nodes
    assert allocator.active_nemeses_on_nodes[nodes[2]] == "SwitchNemesis"


def test_switch_to_busy_node_fails(allocator_with_nodes):
    allocator, _, nodes = allocator_with_nodes

    allocator.mark_node_as_busy(nodes[1], "NemesisA")
    allocator.mark_node_as_busy(nodes[2], "NemesisB")

    switch_ok = allocator.switch_target_node(old_node=nodes[1], new_node=nodes[2], nemesis_name="NemesisA")
    assert not switch_ok
    assert allocator.active_nemeses_on_nodes[nodes[1]] == "NemesisA"
    assert allocator.active_nemeses_on_nodes[nodes[2]] == "NemesisB"
    ...
    def worker_thread(worker_id):
        try:
            for _ in range(10):
                ...
                node, _ = allocator.select_target_node(nemesis_name, NEMESIS_TARGET_POOLS.data_nodes, False)
                if node:
                    time.sleep(random.uniform(0.05, 0.1))
                    results.append(node)
                    allocator.release_busy_node(node, nemesis_name)
...
def test_context_manager_marks_and_releases_single_node(allocator_with_nodes):
    allocator, _, nodes = allocator_with_nodes
    node_to_mark = nodes[0]

    with allocator.mark_nodes_as_busy(node_to_mark, "CtxNemesis"):
        assert allocator.active_nemeses_on_nodes.get(node_to_mark) == "CtxNemesis"

    assert node_to_mark not in allocator.active_nemeses_on_nodes


def test_context_manager_marks_and_releases_multiple_nodes(allocator_with_nodes):
    allocator, _, nodes = allocator_with_nodes
    nodes_to_mark = [nodes[0], nodes[1]]

    with allocator.mark_nodes_as_busy(nodes_to_mark, "MultiCtxNemesis"):
        assert all(node in allocator.active_nemeses_on_nodes for node in nodes_to_mark)

    assert all(node not in allocator.active_nemeses_on_nodes for node in nodes_to_mark)


def test_context_manager_fails_if_node_is_busy(allocator_with_nodes):
    allocator, _, nodes = allocator_with_nodes
    nodes_to_mark = [nodes[0], nodes[1]]

    allocator.mark_node_as_busy(nodes[1], "PreexistingLock")

    with pytest.raises(RuntimeError, match="was already occupied"):
        with allocator.mark_nodes_as_busy(nodes_to_mark, "FailedAcquireNemesis"):
            pytest.fail("[THE TEST SHOULD NOT REACH HERE] Context manager should have failed to acquire all nodes.")

    assert nodes[0] not in allocator.active_nemeses_on_nodes
    assert allocator.active_nemeses_on_nodes[nodes[1]] == "PreexistingLock"
