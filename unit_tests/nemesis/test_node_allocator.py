...
def test_switch_to_reserved_node_fails(allocator_with_nodes):
    ...
    with pytest.raises(NemesisNodeAllocationError, match="already running nemesis"):
...
def test_context_manager_fails_if_node_is_reserved(allocator_with_nodes):
    allocator, _, nodes = allocator_with_nodes
    nodes_to_mark = [nodes[0], nodes[1]]

    allocator.set_running_nemesis(nodes[1], "PreexistingLock")

    with pytest.raises(NemesisNodeAllocationError, match="was already reserved"):
        with allocator.nodes_running_nemesis(nodes_to_mark, "FailedAcquireNemesis"):
            pytest.fail("[THE TEST SHOULD NOT REACH HERE] Context manager should have failed to reserve all nodes.")
