...
def create_client_error(error_code="InvalidAMIID.NotFound"):
    ...
    return ClientError(error_response={"Error": {"Code": error_code, "Message": "Not found"}}, operation_name="DescribeImages")
...
def test_find_equivalent_ami_multiple_regions(mock_get_scylla_resource, mock_boto3_resource):
    ...
    mock_get_scylla_resource.side_effect = [mock_scylla_source, create_empty_scylla_resource(), create_empty_scylla_resource()]

    # Execute
    results = find_equivalent_ami(ami_id="ami-source123", source_region="us-east-1", target_regions=["us-east-1", "eu-west-1"])
...
def test_find_equivalent_ami_real_scylla_ami():
    ...
    try:
        results = find_equivalent_ami(ami_id=source_ami, source_region=source_region, target_regions=["us-east-1", "us-west-2"])
...
def test_find_equivalent_ami_cross_architecture():
    ...
    try:
        results = find_equivalent_ami(ami_id=source_ami, source_region=source_region, target_regions=["us-east-1"], target_arch="arm64")
...
def test_integration_find_equivalent_ami():
    ...
    assert results[0]["scylla_version"] == "2025.4.0~rc2-0.20251015.83babc20e3f7", f"Expected 2025.4.0~rc2-0.20251015.83babc20e3f7, got {results[0]['scylla_version']}"
