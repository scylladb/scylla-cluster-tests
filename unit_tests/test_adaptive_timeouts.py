...
class MemoryAdaptiveTimeoutStore(AdaptiveTimeoutStore):
    ...
    def store(self, metrics: dict[str, Any], operation: str, duration: int, timeout: int, timeout_occurred: bool) -> None:
        metrics.update({"operation": operation, "duration": duration, "timeout": timeout, "timeout_occurred": timeout_occurred})
        key = str(uuid.uuid4())
...
def test_tablets_decommission_uses_predefined_timeouts(hard_timeout_mock, soft_timeout_mock, fake_node, adaptive_timeout_store, mock_tablets_feature):
...
@mock.patch("sdcm.sct_events.system.SoftTimeoutEvent.publish_or_dump")
@mock.patch("sdcm.sct_events.system.HardTimeoutEvent.publish_or_dump")
def test_tablets_new_node_uses_predefined_timeouts(hard_timeout_mock, soft_timeout_mock, fake_node, adaptive_timeout_store, mock_tablets_feature):
    mock_tablets_feature.return_value = True
    with adaptive_timeout(operation=Operations.NEW_NODE, node=fake_node, stats_storage=adaptive_timeout_store, timeout=9999) as timeout:
        assert timeout == TABLETS_HARD_TIMEOUT
