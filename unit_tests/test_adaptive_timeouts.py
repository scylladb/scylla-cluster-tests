...
from sdcm.utils.adaptive_timeouts.load_info_store import AdaptiveTimeoutStore, ArgusAdaptiveTimeoutStore
...
def test_decommission_timeout_is_calculated_and_stored(publish_or_dump, fake_node, adaptive_timeout_store):
    ...
    publish_or_dump.assert_not_called()
    metrics = MemoryAdaptiveTimeoutStore().get(operation=Operations.DECOMMISSION.name, timeout_occurred=False)
    assert metrics
...
def test_tablets_decommission_uses_predefined_timeouts(
    ...
    assert metrics[0]["hard_timeout"] == TABLETS_HARD_TIMEOUT
...
def test_argus_store_reports_hard_timeout_when_set(mock_submit):
    with mock.patch.object(TestConfig, "argus_client", return_value=mock.Mock()):
