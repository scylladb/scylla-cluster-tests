...
def ssh_docker_server(tmp_path_factory):
    ...
    cmd = ["docker", "run", "-d", "--name", container_name, "-p", f"{SSH_PORT}", "-e", f"USER_NAME={SSH_USER}", "-e", f"PUBLIC_KEY={pathlib.Path(ssh_pub_path).read_text().strip()}", SSH_IMAGE]
    ...
    inspect_cmd = ["docker", "inspect", "-f", "{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}", container_name]
    docker_address = subprocess.run(inspect_cmd, check=True, stdout=subprocess.PIPE).stdout.decode().strip()
    # Wait for SSH to be up
    for _ in range(20):
        try:
            output = subprocess.run(
                ["ssh", "-i", str(ssh_key_path), "-o", "IdentitiesOnly=yes", "-o", "UserKnownHostsFile=/dev/null", "-o", "StrictHostKeyChecking=no", f"{SSH_USER}@{docker_address}", "-p", str(SSH_PORT), "echo", "ok"],
                check=False,
    ...
    yield {"hostname": docker_address, "user": SSH_USER, "key_file": str(ssh_key_path), "port": SSH_PORT, "ssh_options": "-o IdentitiesOnly=yes -o StrictHostKeyChecking=no"}, container_name, tmp_path
    subprocess.run(["docker", "rm", "-f", container_name], check=False)
...
def test_upload_remote_files_directly_to_s3(ssh_docker_server, s3_test_key):
    ...
    # Run the upload
    s3_key = s3_test_key
    link = upload_remote_files_directly_to_s3(ssh_info=ssh_info, files=["/tmp/testfile.bin"], s3_bucket=TEST_S3_BUCKET, s3_key=s3_key)
