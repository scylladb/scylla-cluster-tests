...
class CleanInstanceAwsTest(unittest.TestCase):
    ...
    def test_sct_runner(self, ec2_client):
        with patch.object(resources_cleanup, "list_instances_aws", Mock(return_value={"eu-north-1": [SCT_RUNNER_AWS]})) as list_instances_aws:
            clean_instances_aws(
                {
                    "TestId": 1111,
                }
    ...
    def test_terminate(self, ec2_client):
        with patch.object(resources_cleanup, "list_instances_aws", Mock(return_value={"eu-north-1": [{"InstanceId": "i-1111"}]})) as list_instances_aws:
            clean_instances_aws(
                {
                    "TestId": 1111,
                }
...
class CleanElasticIpsAws(unittest.TestCase):
    ...
    def test_release(self, ec2_client):
        ...
        with patch.object(resources_cleanup, "list_elastic_ips_aws", Mock(return_value=return_value)) as list_elastic_ips_aws:
            clean_elastic_ips_aws(
                {
                    "TestId": 1111,
                }
...
class CleanInstancesGceTest(unittest.TestCase):
    ...
    def test_destroy(self):
        ...
        with patch.object(resources_cleanup, "list_instances_gce", Mock(return_value=[instance])) as list_instances_gce:
            with patch.object(resources_cleanup, "get_gce_compute_instances_client", Mock(return_value=(instance, dict(project_id="test")))):
                clean_instances_gce(
                    {
                        "TestId": 1111,
                    }
...
class CleanResourcesDockerTest(unittest.TestCase):
    ...
    def test_destroy():
        image = MagicMock()
        container = MagicMock()
        with patch.object(resources_cleanup, "list_resources_docker", Mock(return_value={"images": [image], "containers": [container]})) as list_resources_docker:
            clean_resources_docker(
                {
                    "TestId": 1111,
                }
