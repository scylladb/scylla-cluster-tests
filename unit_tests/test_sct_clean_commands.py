...
class TestCleanResourcesCommand(unittest.TestCase):
    ENV_VARS = {"SCT_REGION_NAME": "us-east-1", "SCT_GCE_DATACENTER": "us-central1-a", "SCT_AZURE_REGION_NAME": "eastus"}
    ...
    def test_clean_resources_with_clean_runners_flag_and_user(self, mock_logger, mock_clean_runners, mock_clean_resources):
        ...
        mock_clean_runners.assert_called_once_with(test_status="", test_runner_ip=None, backend=None, user="test.user", test_id=None, dry_run=True, force=True)
    ...
    def test_clean_resources_with_clean_runners_flag_and_test_id(self, mock_logger, mock_clean_runners, mock_clean_resources):
        ...
        with patch.dict(os.environ, self.ENV_VARS):
            result = self.runner.invoke(sct.clean_resources, ["--test-id", "test-123", "--clean-runners", "--backend", "aws"])

        self.assertEqual(result.exit_code, 0)
        mock_clean_runners.assert_called_once_with(test_status="", test_runner_ip=None, backend="aws", user=None, test_id="test-123", dry_run=False, force=True)
    ...
    def test_clean_resources_with_both_user_and_test_id(self, mock_logger, mock_clean_runners, mock_clean_resources):
        ...
        with patch.dict(os.environ, self.ENV_VARS):
            result = self.runner.invoke(sct.clean_resources, ["--user", "test.user", "--test-id", "test-123", "--clean-runners", "--backend", "gce"])

        self.assertEqual(result.exit_code, 0)
        mock_clean_runners.assert_called_once_with(test_status="", test_runner_ip=None, backend="gce", user="test.user", test_id="test-123", dry_run=False, force=True)
...
class TestCleanRunnerInstancesCommand(unittest.TestCase):
    ...
    def test_clean_runner_instances_with_user(self, mock_logger, mock_clean_runners):
        ...
        self.assertEqual(result.exit_code, 0)
        mock_clean_runners.assert_called_once_with(test_runner_ip="", test_status=None, backend=None, user="test.user", test_id=(), dry_run=False, force=False)
    ...
    def test_clean_runner_instances_with_test_id(self, mock_logger, mock_clean_runners):
        ...
        self.assertEqual(result.exit_code, 0)
        mock_clean_runners.assert_called_once_with(test_runner_ip="", test_status=None, backend=None, user=None, test_id=("test-123",), dry_run=False, force=False)
    ...
    def test_clean_runner_instances_with_runner_ip(self, mock_logger, mock_clean_runners):
        ...
        self.assertEqual(result.exit_code, 0)
        mock_clean_runners.assert_called_once_with(test_runner_ip="1.2.3.4", test_status=None, backend=None, user=None, test_id=(), dry_run=False, force=False)
    ...
    def test_clean_runner_instances_force(self, mock_logger, mock_clean_runners):
        ...
        self.assertEqual(result.exit_code, 0)
        mock_clean_runners.assert_called_once_with(test_runner_ip="1.2.3.4", test_status=None, backend=None, user=None, test_id=(), dry_run=False, force=True)
    ...
    def test_clean_runner_instances_all_options(self, mock_logger, mock_clean_runners):
        ...
        result = self.runner.invoke(sct.clean_runner_instances, ["--runner-ip", "1.2.3.4", "--test-status", "timeout", "--backend", "gce", "--user", "test.user", "--test-id", "test-123", "--dry-run", "--force"])

        self.assertEqual(result.exit_code, 0)
        mock_clean_runners.assert_called_once_with(test_runner_ip="1.2.3.4", test_status="timeout", backend="gce", user="test.user", test_id=("test-123",), dry_run=True, force=True)
