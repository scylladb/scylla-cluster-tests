...
@pytest.mark.docker_scylla_args(docker_network="kafka-stack-docker-compose_default", image="scylladb/scylla-nightly:latest")
...
def test_01_kafka_cdc_source_connector(request, docker_scylla, kafka_cluster, params, events):
    ...
    docker_scylla.run_cqlsh(f"CREATE KEYSPACE IF NOT EXISTS keyspace1 WITH REPLICATION = {{'class' : 'NetworkTopologyStrategy', 'replication_factor': 1 }} {disable_tablets};")
