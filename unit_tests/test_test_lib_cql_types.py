...
class CQLColumnTypeTest(unittest.TestCase):
    def test_get_random(self):
        already_used = {}
        EMBEDDABLE_NON_COLLECTION_COLUMN_TYPES = [e for e in ALL_COLUMN_TYPES if e not in NOT_EMBEDDABLE_COLUMN_TYPES and e not in COLLECTION_COLUMN_TYPES]
        ...
        assert set(already_used.keys()) == set(NON_COLLECTION_TYPES), f"{list(already_used.keys())} != {NON_COLLECTION_TYPES}"
        already_used = {}
    ...
    def test_str_and_remember_forget_variants(self):
        ...
        assert type_int._get_available_variants(already_created_info={}, avoid_types=["int", "map"], allowed_types=["int", "map", "list", "text"], allow_levels=1) == ["list", "text"]
        assert type_int._get_available_variants(already_created_info={}, avoid_types=["int"], allowed_types=["int", "map", "list", "text"], allow_levels=0) == ["text"]
        ...
        assert already_used_info == {"int": True, "map": {"key_type": {"int": True}, "value_type": {"int": True}}}, f"{already_used_info}"
        type_map_of_map_of_int_int.remember_variant(already_used_info)
        assert already_used_info == {
            "int": True,
            "map": {"key_type": {"int": True, "map": {"key_type": {"int": True}, "value_type": {"int": True}}}, "value_type": {"int": True, "map": {"key_type": {"int": True}, "value_type": {"int": True}}}},
        }, f"{already_used_info}"
