...
class TestBaseNode(unittest.TestCase, EventsUtilsMixin):
    ...
    def test_search_system_log_specific_log(self):
        errors = list(self.node.follow_system_log(patterns=["Failed to load schema version"], start_from_beginning=True))
    ...
    def test_search_power_off(self):
        ...
        InstanceStatusEvent.POWER_OFF().add_info(
            node="A", line_number=22, line=f"{datetime.utcfromtimestamp(time.time() + 1):%Y-%m-%dT%H:%M:%S+00:00} longevity-large-collections-12h-mas-db-node-c6a4e04e-1 !INFO    | systemd-logind: Powering Off..."
        ).publish()
...
class TestBaseNodeGetScyllaVersion(unittest.TestCase):
    ...
    def setUp(self):
        self.node = DummyNode(name="test_node", parent_cluster=None, base_logdir=self.temp_dir, ssh_login_info=dict(key_file="~/.ssh/scylla-test"))
    ...
    def test_get_scylla_version_from_second_attempt(self):
        ...
        self.node.remoter = VersionDummyRemote(
            ...
            (
                ("/usr/bin/scylla --version", (127, "", "bash: scylla: command not found\n")),
                ("dpkg-query --show --showformat '${Version}' scylla", (1, "", "dpkg-query: no packages found matching scylla\n")),
                ("/usr/bin/scylla --version", (127, "", "bash: scylla: command not found\n")),
                ("dpkg-query --show --showformat '${Version}' scylla", (1, "", "dpkg-query: no packages found matching scylla\n")),
            ),
        )
...
class TestNodetoolStatus(unittest.TestCase):
    def test_can_get_nodetool_status_typical(self):
        ...
        assert status == {
            "eastus": {
                "10.0.59.34": {"state": "UN", "load": "21.71GB", "tokens": "256", "owns": "?", "host_id": "e5bcb094-e4de-43aa-8dc9-b1bf74b3b346", "rack": "1a"},
                "10.0.198.153": {"state": "UN", "load": "?", "tokens": "256", "owns": "?", "host_id": "fba174cd-917a-40f6-ab62-cc58efaaf301", "rack": "1a"},
            }
        }
    ...
    def test_can_get_nodetool_status_typical_with_one_space_after_host_id(self):
        ...
        assert status == {
            "datacenter1": {
                "172.17.0.2": {"state": "UN", "load": "202.92KB", "tokens": "256", "owns": "?", "host_id": "7b8f86bf-c70c-4246-a273-146057e12431", "rack": "rack1"},
            }
        }
    ...
    def test_can_get_nodetool_status_ipv6(self):
        ...
        assert status == {
            "eu-north": {
                "2a05:d016:cf8:de00:e07d:5832:c5c0:36a0": {"state": "UN", "load": "774KB", "tokens": "256", "owns": "?", "host_id": "e2ed6943", "rack": "1a"},
                "2a05:d016:cf8:de00:339e:d0d:9446:1980": {"state": "UN", "load": "1.04MB", "tokens": "256", "owns": "?", "host_id": "d67e8502", "rack": "1a"},
            }
        }
    ...
    def test_can_get_nodetool_status_azure(self):
        ...
        assert status == {
            "eastus": {
                "10.0.0.4": {"host_id": "ed6af9a0-8c22-4813-ac9b-6fbeb462b687", "load": "431KB", "owns": "?", "rack": "", "state": "UN", "tokens": "256"},
                "10.0.0.5": {"host_id": "caa15869-cfb4-4229-85d7-0f4832986237", "load": "612KB", "owns": "?", "rack": "1", "state": "UN", "tokens": "256"},
                "10.0.0.6": {"host_id": "3046ded9-ce17-4a3a-ac44-a3ada6916972", "load": "806KB", "owns": "?", "rack": "", "state": "UN", "tokens": "256"},
            }
        }
...
def test_get_any_ks_cf_list(docker_scylla, params, events):
    ...
    with cluster.cql_connection_patient(docker_scylla) as session:
        session.execute("CREATE KEYSPACE mview WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}")
        session.execute("CREATE TABLE mview.users (username text, first_name text, last_name text, password text, email text, last_access timeuuid, PRIMARY KEY(username))")
        session.execute("CREATE MATERIALIZED VIEW mview.users_by_first_name AS SELECT * FROM mview.users WHERE first_name IS NOT NULL and username IS NOT NULL PRIMARY KEY (first_name, username)")
        session.execute("CREATE MATERIALIZED VIEW mview.users_by_last_name AS SELECT * FROM mview.users WHERE last_name IS NOT NULL and username IS NOT NULL PRIMARY KEY (last_name, username)")
        session.execute("INSERT INTO mview.users (username, first_name, last_name, password) VALUES ('fruch', 'Israel', 'Fruchter', '1111')")
        session.execute("CREATE KEYSPACE \"123_keyspace\" WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}")
        session.execute('CREATE TABLE "123_keyspace".users (username text, first_name text, last_name text, password text, email text, last_access timeuuid, PRIMARY KEY(username))')
        session.execute('CREATE TABLE "123_keyspace"."120users" (username text, first_name text, last_name text, password text, email text, last_access timeuuid, PRIMARY KEY(username))')
        session.execute("INSERT INTO \"123_keyspace\".users (username, first_name, last_name, password) VALUES ('fruch', 'Israel', 'Fruchter', '1111')")
        session.execute("INSERT INTO \"123_keyspace\".\"120users\" (username, first_name, last_name, password) VALUES ('fruch', 'Israel', 'Fruchter', '1111')")
    ...
    assert set(table_names) == {"mview.users_by_first_name", "mview.users_by_last_name", "mview.users", '"123_keyspace"."120users"', '"123_keyspace".users'}
...
def test_filter_out_ks_with_rf_one(docker_scylla, params, events):
    ...
    with cluster.cql_connection_patient(docker_scylla) as session:
        session.execute("CREATE KEYSPACE mview WITH replication = {'class': 'org.apache.cassandra.locator.SimpleStrategy', 'replication_factor': '1'} AND durable_writes = true AND tablets = {'enabled': false}")
        session.execute("CREATE TABLE mview.users (username text, first_name text, last_name text, password text, email text, last_access timeuuid, PRIMARY KEY(username))")
        session.execute("INSERT INTO mview.users (username, first_name, last_name, password) VALUES ('fruch', 'Israel', 'Fruchter', '1111')")
        docker_scylla.run_nodetool("flush")
...
def test_is_table_has_no_sstables(docker_scylla, params, events):
    ...
    with cluster.cql_connection_patient(docker_scylla) as session:
        ...
        if is_virtual_tables_get_snapshot():
            ks_cf = cluster.get_any_ks_cf_list(docker_scylla, filter_func=cluster.is_table_has_no_sstables, filter_empty_tables=False)
        else:
    ...
    for i in range(10):
        for node in cluster.nodes:
            with cluster.cql_connection_patient_exclusive(node) as session:
                ...
                assert local.rpc_address == node.cql_address, f"Local rpc_address: {local.rpc_address}, expected: {node.cql_address}"
                assert peers.rpc_address != node.cql_address, f"Peers rpc_address: {peers.rpc_address}, expected not: {node.cql_address}"
...
class TestNodetool(unittest.TestCase):
    def test_describering_parsing(self):
        ...
        assert ranges == [
            {"start_token": -9193109213506951143, "details": [{"host": "127.0.49.3", "datacenter": "datacenter1", "rack": "rack1"}], "end_token": 9202125676696964746, "endpoints": "127.0.49.3", "rpc_endpoints": "127.0.49.3"},
            {"start_token": 9154793403047166459, "details": [{"host": "127.0.49.3", "datacenter": "datacenter1", "rack": "rack1"}], "end_token": 9156354786201613199, "endpoints": "127.0.49.3", "rpc_endpoints": "127.0.49.3"},
        ]
