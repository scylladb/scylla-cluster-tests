...
class TestBaseNode(unittest.TestCase, EventsUtilsMixin):
    ...
    def test_search_power_off(self):
        ...
        InstanceStatusEvent.POWER_OFF().add_info(
            node="A",
            line_number=22,
            line=f"{datetime.utcfromtimestamp(time.time() + 1):%Y-%m-%dT%H:%M:%S+00:00} longevity-large-collections-12h-mas-db-node-c6a4e04e-1 !INFO    | systemd-logind: Powering Off...",
        ).publish()
...
def test_get_any_ks_cf_list(docker_scylla, params, events):
    ...
    with cluster.cql_connection_patient(docker_scylla) as session:
        ...
        session.execute(
            "INSERT INTO mview.users (username, first_name, last_name, password) VALUES ('fruch', 'Israel', 'Fruchter', '1111')"
        )
        session.execute(
            "CREATE KEYSPACE \"123_keyspace\" WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}"
        )
        session.execute(
            'CREATE TABLE "123_keyspace".users (username text, first_name text, last_name text, password text, email text, last_access timeuuid, PRIMARY KEY(username))'
        )
        session.execute(
            'CREATE TABLE "123_keyspace"."120users" (username text, first_name text, last_name text, password text, email text, last_access timeuuid, PRIMARY KEY(username))'
        )
        session.execute(
            "INSERT INTO \"123_keyspace\".users (username, first_name, last_name, password) VALUES ('fruch', 'Israel', 'Fruchter', '1111')"
        )
        session.execute(
            "INSERT INTO \"123_keyspace\".\"120users\" (username, first_name, last_name, password) VALUES ('fruch', 'Israel', 'Fruchter', '1111')"
        )
...
def test_filter_out_ks_with_rf_one(docker_scylla, params, events):
    ...
    with cluster.cql_connection_patient(docker_scylla) as session:
        session.execute(
            "CREATE KEYSPACE mview WITH replication = {'class': 'org.apache.cassandra.locator.SimpleStrategy', 'replication_factor': '1'} AND durable_writes = true AND tablets = {'enabled': false}"
        )
        session.execute(
            "CREATE TABLE mview.users (username text, first_name text, last_name text, password text, email text, last_access timeuuid, PRIMARY KEY(username))"
        )
