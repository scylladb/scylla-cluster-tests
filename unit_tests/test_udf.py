...
class TestUDF(TestCase):
    MOCK_LUA_UDF_VALS = {"name": "lua_var_length_counter", "args": "(var text)", "called_on_null_input_returns": "NULL", "return_type": "int", "language": "lua", "script": "return #var"}
...
(export "__table_base" (global 6)))""",
    ...
    def test_creating_udf_with_missing_required_argument(self):
        ...
        for arg_name in required_arg_names:
            ...
            with self.assertRaises(ValidationError, msg=f"Creating a udf without providing {arg_name} did not raise a ValidationError."):
                UDF(**udf_args)
    ...
    def test_creating_udf_class_with_invalid_language(self):
        ...
        with self.assertRaises(ValidationError, msg="Creating UDF class with invalid language did not raise ValidationError."):
            UDF(**udf_vals)
