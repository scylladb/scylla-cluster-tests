...
from sdcm.sct_events.database import DatabaseLogEvent, FullScanEvent, IndexSpecialColumnErrorEvent, TOLERABLE_REACTOR_STALL, SYSTEM_ERROR_EVENTS
...
class TestDatabaseLogEvent(unittest.TestCase):
    ...
    def test_system_error_events_list(self):
        self.assertSetEqual(set(dir(DatabaseLogEvent)) - set(dir(LogEvent)), {ev.type for ev in SYSTEM_ERROR_EVENTS})
...
class TestIndexSpecialColumnErrorEvent(unittest.TestCase):
    def test_msgfmt(self):
        ...
        self.assertEqual(str(event), "(IndexSpecialColumnErrorEvent Severity.ERROR) period_type=one-time event_id=ac449879-485a-4b06-8596-3fbe58881093: message=m1")
