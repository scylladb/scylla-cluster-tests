...
class CoredumpExportTestBase(unittest.TestCase):
    ...
    def _run_coredump_with_fake_remoter(self, test_name: str):
        ...
        expected_results = coredump_thread.load_expected_results(os.path.join(os.path.dirname(__file__), "test_data", "test_coredump", self.test_data_folder, test_name + "_results.json"))
        for coredump_status, expected_coredump_list in expected_results.items():
            ...
            try:
                ...
                raise AssertionError(f"Got unexpected results for {coredump_status}: {str(result_coredump_list)}\n{str(exc)}") from exc
...
class CoredumpExportExceptionTest(CoredumpExportTestBase):
    ...
    def _init_target_coredump_class(self, test_name: str) -> CoredumpExportSystemdTestThread:
        coredump_thread = CoredumpExportSystemdTestThread(FakeNode(MockRemoter(responses=os.path.join(os.path.dirname(__file__), "test_data", "test_coredump", self.test_data_folder, test_name + "_remoter.json")), tempfile.mkdtemp()), 5)
...
class CoredumpExportSystemdTest(CoredumpExportTestBase):
    ...
    def _init_target_coredump_class(self, test_name: str) -> CoredumpExportSystemdTestThread:
        return CoredumpExportSystemdTestThread(FakeNode(MockRemoter(responses=os.path.join(os.path.dirname(__file__), "test_data", "test_coredump", self.test_data_folder, test_name + "_remoter.json")), tempfile.mkdtemp()), 5)
...
class CoredumpExportFileTest(CoredumpExportTestBase):
    ...
    def _init_target_coredump_class(self, test_name: str) -> CoredumpExportFileTestThread:
        return CoredumpExportFileTestThread(
            FakeNode(MockRemoter(responses=os.path.join(os.path.dirname(__file__), "test_data", "test_coredump", self.test_data_folder, test_name + "_remoter.json")), tempfile.mkdtemp()),
            5,
