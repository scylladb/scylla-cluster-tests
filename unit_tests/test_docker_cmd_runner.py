...
class TestDockerCmdRunner(unittest.TestCase):
    ...
    def test_execute_command_success(self):
        ...
        mock_container.exec_run.assert_called_once_with(["sh", "-c", "echo hello"], tty=False, demux=True, stream=False)
    ...
    def test_sudo_with_non_root_user(self):
        with patch.object(self.runner, 'run', return_value=MagicMock()) as mock_run:
            ...
            mock_run.assert_called_once_with(
                cmd=f'su - testuser -c {quote(cmd)}', timeout=None, ignore_status=False, verbose=True,
