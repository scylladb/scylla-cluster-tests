...
class TestDockerCmdRunner(unittest.TestCase):
    ...
    def test_execute_command_success(self):
        ...
        mock_container.exec_run.assert_called_once_with(["sh", "-c", "echo hello"], tty=False, demux=True, stream=False, user="")
    ...
    def test_run(self, mock_retrying):
        ...
        with patch.object(self.runner, "_execute_command") as mock_exec, patch.object(self.runner, "_setup_watchers") as mock_setup, patch.object(self.runner, "_print_command_results") as mock_print:
    ...
    def test_receive_files(self, mock_retrying):
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=True) as temp_file, patch.object(self.runner, "_get_container", return_value=mock_container):
    ...
    def test_sudo_with_non_root_user(self):
        with patch.object(self.runner, "run", return_value=MagicMock()) as mock_run:
            ...
            mock_run.assert_called_once_with(cmd=f"{cmd}", timeout=None, ignore_status=False, verbose=True, new_session=False, log_file=None, retry=1, watchers=None, user="testuser")
