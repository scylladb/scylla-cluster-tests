...
class DummyNode(BaseNode):
    ...
    def __init__(self, node_num: int, parent_cluster, base_logdir: str):
        ...
        self._ipv6_ip_address = [
            "aaaa:db8:aaaa:aaaa:aaaa:aaaa:aaaa:aaa" + str(node_num),
...
class IntegrationTests(unittest.TestCase):
    ...
    def _run_test(self, config_path: str, expected_node_config: str, region_names: str):
        ...
        old_test_config = {
            key: value
            for key, value in TestConfig().__class__.__dict__.items()
            if is_builtin(type(value)) and not key.startswith("__")
        }
        while os.environ:
            os.environ.popitem()
        try:
