    ...
    def test_aws_multi_openldap(self):
        self._run_test(
            ...
            expected_as_dict={
                ...
                "ldap_url_template": "ldap://1.1.1.1:389/dc=scylla-qa,dc=com?cn?sub?(uniqueMember=uid={USER},ou=Person,dc=scylla-qa,dc=com)",
    ...
    def test_gce_multi_msldap(self):
        self._run_test(
            builder_params={
                "test_config": TestConfigWithLdap,
                "cluster_name": "test-cluster",
                "msldap_server_info": {"ldap_bind_dn": "SOMEDN", "admin_password": "PASSWORD", "server_address": "3.3.3.3"},
                "params": {
                    "cluster_backend": "gce",
    ...
    def test_aws_single_private_noextra(self):
        self._run_test(
            ...
            expected_as_dict={
                ...
                "rpc_address": "1.1.1.1",
                "seed_provider": [{"class_name": "org.apache.cassandra.locator.SimpleSeedProvider", "parameters": [{"seeds": "1.1.1.1"}]}],
...
class DummyNode(BaseNode):
    ...
    def __init__(self, node_num: int, parent_cluster, base_logdir: str):
        ...
        self._ipv6_ip_address = ["aaaa:db8:aaaa:aaaa:aaaa:aaaa:aaaa:aaa" + str(node_num), "aaaa:db8:aaaa:aaaa:aaaa:aaaa:aaaa:bbb" + str(node_num)]
    ...
    def network_interfaces(self):
        return [
            NetworkInterface(
                ipv4_public_address=self._public_ip_address[0],
    ...
    def validate_scylla_yaml(self, expected_node_config, seed_node_ips):
        with self.remote_scylla_yaml() as node_yaml:
            expected_node_yaml = expected_node_config.replace("__NODE_PRIVATE_ADDRESS_2__", self._private_ip_address[1])
            expected_node_yaml = expected_node_yaml.replace("__NODE_PRIVATE_ADDRESS__", self.private_ip_address)
            expected_node_yaml = expected_node_yaml.replace("__NODE_PUBLIC_ADDRESS__", self.public_ip_address)
            expected_node_yaml = expected_node_yaml.replace("__SEED_NODE_IPS__", seed_node_ips)
            expected_node_yaml = expected_node_yaml.replace("__NODE_IPV6_ADDRESS__", self.ipv6_ip_address)
            assert json.loads(expected_node_yaml) == node_yaml.model_dump(exclude_unset=True, exclude_defaults=True, exclude_none=True)
...
class IntegrationTests(unittest.TestCase):
    get_scylla_ami_version_output = ObjectDict(
        ...
        ImageLocation="797456418907/ScyllaDB Enterprise 2021.1.15",
        ImageType="machine",
        Public=True,
        ...
        State="available",
        BlockDeviceMappings=[
            {"DeviceName": "/dev/sda1", "Ebs": {"DeleteOnTermination": True, "SnapshotId": "snap-0717a2bee0a38bc84", "VolumeSize": 30, "VolumeType": "gp3", "Encrypted": False}},
        ...
        Description="ScyllaDB Enterprise 2021.1.15",
        EnaSupport=True,
        Hypervisor="xen",
        Name="ScyllaDB Enterprise 2021.1.15",
    ...
    def _run_test(self, config_path: str, expected_node_config: str, region_names: str):
        with patch("sdcm.sct_config.get_scylla_ami_versions", return_value=[self.get_scylla_ami_version_output]), patch("sdcm.provision.scylla_yaml.certificate_builder.install_client_certificate", return_value=None):
    ...
    def test_integration_node(self, _, config_path, result_path):
        ...
        self._run_test(config_path, expected_node_config=expected_node_config, region_names='["eu-west-1", "us-east-1"]')
