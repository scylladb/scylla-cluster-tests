...
class MicroBenchmarkingResultsAnalyzerMock(MicroBenchmarkingResultsAnalyzer):
    _mock_returns = None
    _mock_returns_path = os.path.join(
        os.path.dirname(__file__), "test_data/test_microbenchmarking/mock_response/{funct_name}/{hash}.zip"
    )
...
class TestMBM(unittest.TestCase):
    def setUp(self):
        self.mbra = MicroBenchmarkingResultsAnalyzerMock(
            email_recipients=("alex.bykov@scylladb.com",), es_index="microbenchmarking"
        )
    ...
    def test_get_result_with_2_datasets(self):
        ...
        self.assertRaises(
            LargeNumberOfDatasetsException, self.mbra.get_results, results_path=result_path, update_db=False
        )
    ...
    def test_get_result_for_empty_tests_folders(self):
        result_path = os.path.join(
            os.path.dirname(__file__), "test_data/test_microbenchmarking/PFF_empty_tests_folders"
        )
        self.assertRaises(EmptyResultFolder, self.mbra.get_results, results_path=result_path, update_db=False)

    def test_get_result_for_empty_dataset_folders(self):
        result_path = os.path.join(
            os.path.dirname(__file__), "test_data/test_microbenchmarking/PFF_empty_dataset_folders"
        )
        self.assertRaises(EmptyResultFolder, self.mbra.get_results, results_path=result_path, update_db=False)
    ...
    def test_check_regression_for_new_metrics_cpu_aio_and_empty_last_and_best(self):
        ...
        self.assertEqual(
            report_results["small-partition-slicing_500000-4096.1"]["avg cpu"]["Last, commit, date"][0], None
        )
        self.assertEqual(
            report_results["small-partition-slicing_500000-4096.1"]["avg cpu"]["Best, commit, date"][0], None
        )
...
<<<<<<< HEAD
        ...
        with subprocess.Popen(['./microbenchmarking.py', '--help'], cwd=self.cwd,
                              stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env) as ps:
...
<<<<<<< HEAD
        ...
        with subprocess.Popen(['./microbenchmarking.py', 'exclude', '--help'],
                              ...
                              stderr=subprocess.PIPE, env=env) as ps:
||||||| parent of e29892926 (improvement(treewide): Reformat using ruff)
        msg = b'usage: microbenchmarking.py exclude [-h]'
        with subprocess.Popen(['./microbenchmarking.py', 'exclude', '--help'],
                              cwd=self.cwd,
