...
class MicroBenchmarkingResultsAnalyzerMock(MicroBenchmarkingResultsAnalyzer):
    _mock_returns = None
    _mock_returns_path = os.path.join(os.path.dirname(__file__), "test_data/test_microbenchmarking/mock_response/{funct_name}/{hash}.zip")
    ...
    def _mock_function(self, target_fn, *args, **kwargs):
        ...
        if output is None:
            ...
            file_path = self._save_mock_return("_get_prior_tests", args=args, kwargs=kwargs, result=output)
...
class TestMBM(unittest.TestCase):
    def setUp(self):
        self.mbra = MicroBenchmarkingResultsAnalyzerMock(email_recipients=("alex.bykov@scylladb.com",), es_index="microbenchmarking")
    ...
    def test_get_result_with_2_datasets(self):
        ...
        self.assertRaises(LargeNumberOfDatasetsException, self.mbra.get_results, results_path=result_path, update_db=False)
    ...
    def test_get_result_for_empty_tests_folders(self):
        result_path = os.path.join(os.path.dirname(__file__), "test_data/test_microbenchmarking/PFF_empty_tests_folders")
        self.assertRaises(EmptyResultFolder, self.mbra.get_results, results_path=result_path, update_db=False)

    def test_get_result_for_empty_dataset_folders(self):
        result_path = os.path.join(os.path.dirname(__file__), "test_data/test_microbenchmarking/PFF_empty_dataset_folders")
        self.assertRaises(EmptyResultFolder, self.mbra.get_results, results_path=result_path, update_db=False)
    ...
    def test_main_help_message(self):
        with subprocess.Popen(["./microbenchmarking.py", "--help"], cwd=self.cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as ps:
    ...
    def test_help_parameter_for_exclude(self):
        ...
        with subprocess.Popen(["./microbenchmarking.py", "exclude", "--help"], cwd=self.cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as ps:
            stdout, stderr = ps.communicate()
            self.assertEqual(ps.returncode, 0)
    ...
    def test_help_parameter_for_check(self):
        ...
        with subprocess.Popen(["./microbenchmarking.py", "check", "--help"], cwd=self.cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as ps:
            stdout, stderr = ps.communicate()
            self.assertEqual(ps.returncode, 0)
            self.assertIn(msg, stdout)
            self.assertFalse(stderr)
    ...
    def test_execute_mbm_from_cli(self):
        ...
        with subprocess.Popen(
            ["./microbenchmarking.py", "check", "--results-path", result_path, "--email-recipients", "alex.bykov@scylladb.com", "--report-path", html_report, "--hostname", self.mbra.hostname, "--es-index", self.es_index],
            cwd=self.cwd,
