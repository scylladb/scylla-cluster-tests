...
class TestTriggerMatrix:
    def test_dry_run_produces_output(self, sample_matrix_yaml, caplog):
        ...
        assert len(results["triggered"]) == 4  # no version exclusion applies for "2025.4"
    ...
    def test_full_flow_with_mocked_api(self, mock_trigger, sample_matrix_yaml):
        ...
        assert mock_trigger.call_count == 4  # no version exclusion applies for "2025.4"
...
class TestBackendValidation:
    def test_invalid_backend_raises(self, tmp_path):
        ...
        with pytest.raises(MatrixValidationError, match="invalid backend 'gec'"):
            load_matrix_config(path)

    @pytest.mark.parametrize("backend", sorted(["aws", "gce", "azure", "docker", "oci"]))
    def test_valid_backends_accepted(self, tmp_path, backend):
...
class TestTriggerJenkinsJobErrors:
    def test_missing_jenkins_url_raises(self, monkeypatch):
        monkeypatch.delenv("JENKINS_URL", raising=False)
        monkeypatch.delenv("JENKINS_API_TOKEN", raising=False)
        with pytest.raises(JenkinsTriggerError, match="JENKINS_URL"):
            trigger_jenkins_job("test-job", {}, dry_run=False)

    def test_missing_jenkins_token_raises(self, monkeypatch):
        monkeypatch.setenv("JENKINS_URL", "https://jenkins.example.com")
        monkeypatch.delenv("JENKINS_API_TOKEN", raising=False)
        with pytest.raises(JenkinsTriggerError, match="JENKINS_API_TOKEN"):
            trigger_jenkins_job("test-job", {}, dry_run=False)

    @patch("sdcm.utils.trigger_matrix.requests.post")
    def test_4xx_returns_false_no_retry(self, mock_post, monkeypatch):
        monkeypatch.setenv("JENKINS_URL", "https://jenkins.example.com")
    ...
    @patch("sdcm.utils.trigger_matrix.requests.post")
    def test_5xx_retries_then_fails(self, mock_post, mock_sleep, monkeypatch):
        monkeypatch.setenv("JENKINS_URL", "https://jenkins.example.com")
        monkeypatch.setenv("JENKINS_API_TOKEN", "fake-token")
    ...
    @patch("sdcm.utils.trigger_matrix.time.sleep")
    @patch("sdcm.utils.trigger_matrix.requests.post")
    def test_5xx_then_success(self, mock_post, mock_sleep, monkeypatch):
        monkeypatch.setenv("JENKINS_URL", "https://jenkins.example.com")
        monkeypatch.setenv("JENKINS_API_TOKEN", "fake-token")
        fail_resp = type("Resp", (), {"status_code": 503})()
        ok_resp = type("Resp", (), {"status_code": 201})()
...
class TestZeroMatchWarning:
    def test_zero_match_logs_warning(self, sample_matrix_yaml, caplog):
...
class TestUnknownSkipJobsWarning:
    def test_unknown_skip_jobs_logs_warning(self, sample_matrix_yaml, caplog):
        with caplog.at_level(logging.WARNING):
            trigger_matrix(
                matrix_file=str(sample_matrix_yaml),
                scylla_version="2025.4",
