...
class TestNodetoolEvent(unittest.TestCase):
    def test_nodetool_cmd_no_options(self):
        ...
        event.begin_event()
        self.assertEqual(str(event), "(NodetoolEvent Severity.NORMAL) period_type=begin event_id=3c8e2362-a987-4eff-953f-9cd1ad2017e5: nodetool_command=scrub node=1.0.0.121")

        event.duration = 20.325
        event.end_event()
        self.assertEqual(str(event), "(NodetoolEvent Severity.NORMAL) period_type=end event_id=3c8e2362-a987-4eff-953f-9cd1ad2017e5 duration=20.325s: nodetool_command=scrub node=1.0.0.121")
    ...
    def test_nodetool_cmd_with_options(self):
        ...
        self.assertEqual(
            str(event), "(NodetoolEvent Severity.NORMAL) period_type=begin event_id=ef4aeb1a-c004-40e4-af14-9d87a0526408: nodetool_command=scrub node=1.0.0.121 options=--skip-corrupted drop_table_during_repair_ks_0 more options"
        )
        ...
        with mock.patch("time.time", return_value=begin_event_timestamp + 1):
            ...
            self.assertEqual(
                str(event),
                "(NodetoolEvent Severity.NORMAL) period_type=end event_id=ef4aeb1a-c004-40e4-af14-9d87a0526408 duration=1s: nodetool_command=scrub node=1.0.0.121 options=--skip-corrupted drop_table_during_repair_ks_0 more options",
            )
