...
| `-max-rate` | A few hundred ops/s. **Shrink it, do not remove it** — scylla-bench requires a nonzero rate on `-workload=timeseries`, and every timeseries config in the repo carries one |
| `-write-rate` (on `-mode=read`) | **Derived, not chosen:** `write-rate = -max-rate / -concurrency` of the *write* command. Shrink the writer and leave this alone and the readers hunt rows nobody wrote — the same trap as `data_validation` below |
| `-connection-count` | Match it to `-concurrency`, roughly one connection per worker. Production runs 100; that many against a 1-vCPU guest starves it |
| `-duration` | Minutes |

Keep: `-workload`, `-mode`, `-replication-factor`, `-consistency-level`, and the
`SET_WRITE_TIMESTAMP` / `GET_WRITE_TIMESTAMP` markers that tie a read command to its writer.
Data-validation configs reference `scylla_bench.test` and a partition range by number — shrink
`-partition-count` and the `data_validation` block **together**, or validation reads
partitions that were never written.

The three cross-command couplings are what make scylla-bench easy to get wrong: `-write-rate`
tracks the writer's rate and concurrency, `data_validation` tracks `-partition-count`, and the
timestamp markers tie the pair together. Shrink either side alone and the readers look for data
that does not exist.
