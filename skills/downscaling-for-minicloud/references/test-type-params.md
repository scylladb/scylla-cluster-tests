...
| `prepare_write_cmd` | **The preload, and the easiest thing to miss.** `preload_data=True` for the read, mixed and disk-only sub-tests, so the production 650M-row load runs before the steps do. With `round_robin` it holds one command per loader, so dropping to one loader means dropping to one command |
| `prepare_stress_cmd` | Schema creation. Only the write sub-test sets `prepare_schema=True`; for the others the latte thread creates the schema from `latte_schema_parameters` |
| `prepare_stress_duration` | The preload's timeout in minutes, 300 in production. A 50k-row preload needs a fraction of that, and leaving it long hides a stuck preload |
