...
GUESTS=5   # n_db_nodes + n_loaders + n_monitor_nodes, from the Phase 2 budget
...
phase_check() {
    ...
    local avail per_guest
    avail=$(awk '/MemAvailable/ {printf "%.1f", $2/1048576}' /proc/meminfo)
    per_guest="${SCT_MINICLOUD_LIGHTWEIGHT_MEMORY%GiB}"
    echo "    $GUESTS guests x ${per_guest}GiB + 2GiB = $(( GUESTS * per_guest + 2 ))GiB needed, ${avail}GiB available"
}
