...
Unit tests must **never** make real network calls. The `mock_cloud_services` autouse fixture blocks most AWS/GCE/Azure calls, and `fake_remoter` blocks SSH — but custom HTTP calls (e.g., `requests.get`, direct `boto3` for uncommon services) are not auto-blocked.
