...
`from sdcm.sct_config.types import dict_or_str`, so the effective patch target is
`sdcm.sct_config.config.dict_or_str` — patching `...types.dict_or_str` replaces an
