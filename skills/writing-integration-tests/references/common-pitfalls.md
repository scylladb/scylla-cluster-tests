...
**Rule:** If a test fundamentally requires real cloud credentials (not just mocked by `conftest.py` fixtures), mark it as integration. The `mock_cloud_services` autouse fixture handles most cases, but tests that call cloud APIs directly bypass those mocks.
