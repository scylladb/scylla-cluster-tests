...
class ThroughputLimitFunctionalTest(ClusterTester):
    def test_per_partition_limit(self):
        ...
        assert limited_write_result.latency_99 < base_write_result.latency_99 * 1.05, (
            f"Per partition limit caused more than 5% write latency degradation {limited_write_result.latency_99}<{base_write_result.latency_99 * 1.05}"
        )
        assert limited_read_result.latency_99 < base_read_result.latency_99 * 1.05, (
            f"Per partition limit caused more than 5% read latency degradation{limited_read_result.latency_99}<{base_read_result.latency_99 * 1.05}"
        )
        assert limited_write_result.ops > base_write_result.ops * 0.95, (
            f"Per partition limit caused more than 5% write op/s degradation{limited_write_result.ops}>{base_write_result.ops * 0.95}"
        )
        assert limited_read_result.ops > base_read_result.ops * 0.95, (
            f"Per partition limit caused more than 5% read op/s degradation{limited_read_result.ops}>{base_read_result.ops * 0.95}"
        )
    ...
    def test_compaction_throughput_limit(self):
        ...
        for limit in [0.45, 0.2, 0.01]:
            ...
            if limited_write_result.latency_99 > base_write_result.latency_99:
                InfoEvent(
                    f"Compaction throughput limit didn't bring write latency improvements when limiting compaction to {limit} of disk write bandwidth {limited_write_result.latency_99}<{base_write_result.latency_99}",
                    severity=Severity.ERROR,
                ).publish()
            if limited_write_result.ops < base_write_result.ops:
                InfoEvent(
                    f"Compaction throughput limit didn't bring write op/s improvements when limiting compaction to {limit} of disk write bandwidth {limited_write_result.ops}>{base_write_result.ops}",
                    severity=Severity.ERROR,
                ).publish()
