...
clustering-key + 3 value columns; column names are arbitrary).
"""
...
class LWTPaxosGrowthTest(LongevityTest):
    ...
    def test_paxos_growth(self):
        ...
        try:
            ...
            if user_final >= user_after_populate:
                self.fail(
                    "Base table did not shrink after plain DELETE + compaction "
                    f"(user {user_after_populate} -> {user_final} bytes). Tombstones not "
                    "purged - verify plain DELETE op and tombstone_gc=immediate."
                )
            ...
            for report in (
                self._report_table_sizes_all_nodes,
                lambda: self._report_cas_metrics(label="FINAL"),
                self._log_table_properties,
            ):
                try:
                    report()
                except Exception as exc:  # pylint: disable=broad-except  # noqa: BLE001
                    self.log.warning("Final report step failed: %s", exc)
