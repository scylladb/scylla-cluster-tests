...
def test_deploy_quasi_multidc_db_cluster(db_cluster: ScyllaPodCluster):  # noqa: PLR0914
    ...
    def get_pod_names_and_ips(cluster_name: str, namespace: str):
        pod_names_and_ips = kubectl(f"get pods --no-headers -o=custom-columns=':.metadata.name,:.status.podIP' -l scylla/cluster={cluster_name},app.kubernetes.io/name=scylla", namespace=namespace).stdout.split("\n")
    ...
    try:
        log.debug(k8s_cluster.helm_install(target_chart_name=target_chart_name, source_chart_name="scylla-operator/scylla", version=operator_version, use_devel=True, values=values, namespace=namespace))
        ...
        svc_ips = kubectl(f"get svc --no-headers -o=custom-columns=':.spec.clusterIP' -l scylla/cluster={cluster_name} -l scylla-operator.scylladb.com/scylla-service-type=member", namespace=namespace).stdout.split()
        assert svc_ips
        ...
        try:
            ...
            log.debug(k8s_cluster.helm_install(target_chart_name=target_chart_name2, source_chart_name="scylla-operator/scylla", version=operator_version, use_devel=True, values=values, namespace=namespace2))
            ...
            for current_namespace in pods_data.keys():
                for pod_name, pod_ip in pods_data[current_namespace].items():
                    ...
                    cqlsh_results = kubectl(f'exec {pod_name} -- /bin/cqlsh -e "{cqlsh_cmd}"', namespace=current_namespace).stdout.split("---\n")[-1].split("\n")
...
def test_rolling_restart_cluster(db_cluster):
    ...
    assert old_force_redeployment_reason != new_force_redeployment_reason, f"'{old_force_redeployment_reason}' must be different than '{new_force_redeployment_reason}'"
    ...
    for pod_name_and_status in pods_name_and_status:
        scylla_log = db_cluster.k8s_cluster.kubectl(f"logs {pod_name_and_status['name']} -c scylla", namespace=db_cluster.namespace)
        assert "scylla_io_setup" not in scylla_log.stdout, f"iotune was run after reboot on {pod_name_and_status['name']}"
...
def test_add_new_node_and_check_old_nodes_are_cleaned_up(db_cluster):
    ...
    for node in db_cluster.nodes:
        for keyspace in db_cluster.nodes[0].run_cqlsh("describe keyspaces").stdout.split():
            log_followers[f"{node.name}--{keyspace}"] = node.follow_system_log(patterns=[f"api - force_keyspace_cleanup: keyspace={keyspace} ", f"api - Keyspace {keyspace} does not require cleanup"])
    ...
    def wait_for_cleanup_logs(log_follower_name, log_follower, db_cluster):
        ...
        while not (list(log_follower) or stop_ks_creation):
            ...
            cql_create_ks_cmd = f"CREATE KEYSPACE IF NOT EXISTS {current_ks_name} WITH replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor' : {db_rf}}}"
            try:
    ...
    try:
        ...
        if need_to_collect_logs:
            KubernetesOps.gather_k8s_logs(logdir_path=logdir, kubectl=k8s_cluster.kubectl, namespaces=[SCYLLA_NAMESPACE])
...
def _scylla_cluster_monitoring_ckecks(db_cluster: ScyllaPodCluster, monitoring_type: str):
    ...
    try:
        ...
        k8s_cluster.deploy_scylla_cluster_monitoring(cluster_name=cluster_name, namespace=namespace, monitoring_type=monitoring_type)

        # NOTE: add SCT dashboard to the Grafana service
        api_call_return_code = k8s_cluster.register_sct_grafana_dashboard(cluster_name=cluster_name, namespace=namespace)
...
@pytest.mark.parametrize("manager_version", ("3.5.1",))
...
def test_listen_address(db_cluster):
    ...
    all_errors = []
    for node in db_cluster.nodes:
        result = node.remoter.run("ps aux | grep docker-entrypoint.py | grep -Po '\\-\\-listen-address=[^ ]+' | sed -r 's/--listen-address[= ]([^ ]+)/\\1/'", ignore_status=True)
...
def test_check_operator_operability_when_scylla_crd_is_incorrect(db_cluster):
    ...
    db_cluster.k8s_cluster.helm_install(target_chart_name=target_chart_name, source_chart_name="scylla-operator/scylla", version=db_cluster.k8s_cluster.scylla_operator_chart_version, use_devel=True, values=values, namespace=namespace)
    try:
        ...
        time.sleep(30)
        invalid_cluster_sts = db_cluster.k8s_cluster.kubectl(f"get sts -n {namespace} -l scylla/cluster={cluster_name}", ignore_status=True)
        assert "No resources found" in invalid_cluster_sts.stderr, f"Expected {cluster_name} not to have statefulset created.\nstdout: {invalid_cluster_sts.stdout}\nstderr: {invalid_cluster_sts.stderr}"
...
def test_ha_update_spec_while_rollout_restart(db_cluster: ScyllaPodCluster):
    ...
    try:
        ...
        db_cluster.k8s_cluster.kubectl("wait -l app.kubernetes.io/name=scylla-operator --for=delete pod", namespace=SCYLLA_OPERATOR_NAMESPACE, timeout=300)
        ...
        db_cluster.k8s_cluster.kubectl("rollout status deployment webhook-server --watch=true", namespace=SCYLLA_OPERATOR_NAMESPACE)
        ...
        db_cluster.k8s_cluster.kubectl_wait("--for=condition=Ready pod -l app.kubernetes.io/name=scylla-operator", namespace=SCYLLA_OPERATOR_NAMESPACE, timeout=300)

    try:
        assert expected_aio_max_nr_value == current_aio_max_nr_value, f"Cluster specification has not been updated correctly. Expected value for '{sysctl_name}' sysctl is {expected_aio_max_nr_value}, actual is {current_aio_max_nr_value}"
    finally:
...
def test_scylla_operator_pods(db_cluster: ScyllaPodCluster):
    scylla_operator_pods = get_pods_and_statuses(db_cluster=db_cluster, namespace=SCYLLA_OPERATOR_NAMESPACE, label="app.kubernetes.io/instance=scylla-operator")

    assert len(scylla_operator_pods) == 2, f"Expected 2 scylla-operator pods, but exists {len(scylla_operator_pods)}"

    not_running_pods = ",".join([pods_info["name"] for pods_info in scylla_operator_pods if pods_info["status"] != "Running"])
    ...
    pods = db_cluster.k8s_cluster.kubectl(f"get pods {scylla_operator_pods[0]['name']} -o yaml", namespace=SCYLLA_OPERATOR_NAMESPACE)
    for owner_reference in yaml.safe_load(pods.stdout)["metadata"].get("ownerReferences", []):
        assert owner_reference["kind"] == "ReplicaSet", f"Expected 'ReplicaSet' kind as owner, but got '{owner_reference['Kind']}'"


@pytest.mark.readonly
def test_startup_probe_exists_in_scylla_pods(db_cluster: ScyllaPodCluster):
    pods = get_pods_without_probe(db_cluster=db_cluster, probe_type="startupProbe", selector="app.kubernetes.io/name=scylla", container_name="scylla")
...
def test_readiness_probe_exists_in_mgmt_pods(db_cluster: ScyllaPodCluster):
    ...
    """
    pods = get_pods_without_probe(db_cluster=db_cluster, probe_type="readinessProbe", selector="app.kubernetes.io/name=scylla-manager", container_name="scylla-manager")
...
def test_deploy_helm_with_default_values(db_cluster: ScyllaPodCluster):
    ...
    try:
        ...
        pods_name_and_status = get_pods_and_statuses(db_cluster, namespace=namespace, label=db_cluster.pod_selector)
        ...
        assert len(pods_name_and_status) == 3, f"Expected 3 pods to be created in {namespace} namespace but actually {len(pods_name_and_status)}: {pods_name_and_status}"

        for pod_name_and_status in pods_name_and_status:
            assert pod_name_and_status["status"] == PodStatuses.RUNNING.value, f"Expected '{PodStatuses.RUNNING.value}' status of pod '{pod_name_and_status['name']}' but actually it's {pod_name_and_status['status']}"

            storage_capacity = get_pod_storage_capacity(db_cluster, namespace=namespace, pod_name=pod_name_and_status["name"], label="app.kubernetes.io/name=scylla")
            assert storage_capacity[0]["capacity"] == expected_capacity, f"Expected capacity is {expected_capacity}, actual capacity of pod '{pod_name_and_status['name']}' is {storage_capacity[0]['capacity']}"

            scylla_version = k8s_cluster.kubectl(f"exec {pod_name_and_status['name']} -c scylla -- scylla --version", namespace=namespace)
            assert not scylla_version.stderr, f"Failed to get scylla version from {pod_name_and_status['name']}. Error: {scylla_version.stderr}"
            assert scylla_version.stdout, f"Failed to get scylla version from {pod_name_and_status['name']}. Output of command 'scylla --version' is empty"
        need_to_collect_logs = False
...
def test_scylla_yaml_override(db_cluster, scylla_yaml):
    ...
    assert isinstance(original_hinted_handoff, bool), f"configMap scylla.yaml have unexpected '{hh_enabled_option_name}' type: {type(original_hinted_handoff)}. Expected 'bool'"
    new_hinted_handoff = not original_hinted_handoff

    if original_hinted_handoff_throttle_in_kb:
        assert isinstance(original_hinted_handoff_throttle_in_kb, int), f"Node scylla.yaml have unexpected '{hh_throttle_option_name}' type: {type(original_hinted_handoff_throttle_in_kb)}. Expected 'int'"
...
def test_default_dns_policy(db_cluster: ScyllaPodCluster):
    ...
    pods = db_cluster.k8s_cluster.kubectl(f"get pods -l scylla/cluster={db_cluster.params.get('k8s_scylla_cluster_name')} -o yaml", namespace=SCYLLA_NAMESPACE)
    ...
    assert not pods_with_wrong_dns_policy, f"Found pods that have unexpected dnsPolicy.\nExpected is '{expected_policy}'.\nPods: {yaml.safe_dump(pods_with_wrong_dns_policy, indent=2)}"
...
def test_operator_managed_tls(db_cluster: ScyllaPodCluster, tmp_path: path.Path):
    ...
    crt_data = db_cluster.k8s_cluster.kubectl(rf"get secret/{cluster_name}-local-client-ca -o jsonpath='{{.data.tls\.crt}}'", namespace=db_cluster.namespace)
    key_data = db_cluster.k8s_cluster.kubectl(rf"get secret/{cluster_name}-local-client-ca -o jsonpath='{{.data.tls\.key}}'", namespace=db_cluster.namespace)
    ca_data = db_cluster.k8s_cluster.kubectl(rf"get secret/{cluster_name}-local-serving-ca -o jsonpath='{{.data.tls\.crt}}'", namespace=db_cluster.namespace)
    ...
    execution_profile = ExecutionProfile(load_balancing_policy=WhiteListRoundRobinPolicy([db_cluster.nodes[0].cql_address]))
    cluster = Cluster(contact_points=[db_cluster.nodes[0].cql_address], port=db_cluster.nodes[0].CQL_SSL_PORT, execution_profiles={EXEC_PROFILE_DEFAULT: execution_profile})
...
def test_can_recover_from_fatal_pod_termination(db_cluster):
    ...
    experiment.start()
    db_cluster.k8s_cluster.kubectl(f"wait --timeout=1m --for=condition=Ready=false pod {target_node.name} -n {db_cluster.namespace}")
