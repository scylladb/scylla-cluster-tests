...
def scylla_pod_names(db_cluster: ScyllaPodCluster) -> list:
    pods = db_cluster.k8s_cluster.kubectl(
        f"get pods -n {db_cluster.namespace} --no-headers -l scylla/cluster={db_cluster.params.get('k8s_scylla_cluster_name')} -o=custom-columns='NAME:.metadata.name'"
    )
...
def scylla_services_names(db_cluster: ScyllaPodCluster) -> list:
    ...
    services = db_cluster.k8s_cluster.kubectl(
        f"get svc -n {db_cluster.namespace} --no-headers -l scylla/cluster={scylla_cluster_name} -o=custom-columns='NAME:.metadata.name'"
    )
...
def get_pod_storage_capacity(db_cluster: ScyllaPodCluster, namespace: str, pod_name: str = None, label: str = None):
    ...
    for pod in yaml.safe_load(persistent_volume_info.stdout)["items"]:
        ...
        pods_storage_capacity.append(
            {"name": pod["metadata"]["name"], "capacity": pod["status"]["capacity"]["storage"]}
        )
...
def reinstall_scylla_manager(db_cluster: ScyllaPodCluster, manager_version: str):
    values = HelmValues(
        yaml.safe_load(
            db_cluster.k8s_cluster.helm("get values scylla-manager -o yaml", namespace=SCYLLA_MANAGER_NAMESPACE)
        )
    )
...
def verify_resharding_on_k8s(db_cluster: ScyllaPodCluster, cpus: Union[str, int, float]):
    ...
    log.info(
        "Update the cpu count to '%s' CPUs to make Scylla start the resharding process on all the nodes 1 by 1", cpus
    )
    ...
    for node, liveness_probe_failures, resharding_start, resharding_finish in nodes_data:
        ...
        if resharding_time < 120:
            log.warning("Resharding was too fast - '%s's (<120s) on the '%s' node", resharding_time, node.name)
        else:
            log.info("Resharding has taken '%s's on the '%s' node", resharding_time, node.name)
        ...
        assert not liveness_probe_failures_list, f"liveness probe has failures: {liveness_probe_failures_list}"
