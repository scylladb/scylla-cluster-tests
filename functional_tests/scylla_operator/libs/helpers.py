...
from sdcm.cluster import DB_LOG_PATTERN_RESHARDING_START, DB_LOG_PATTERN_RESHARDING_FINISH, UnexpectedExit
...
@log_run_info
@retrying(n=10, sleep_time=2, allowed_exceptions=(k8s_exceptions.ApiException,), message="Failed to set scylla sysctl value...")
...
def get_pods_without_probe(db_cluster: ScyllaPodCluster, probe_type: str, selector: str, container_name: str) -> list:
    ...
    for pod in yaml.safe_load(pods.stdout)["items"]:
        for container in pod.get("spec", {}).get("containers", []):
            if container["name"] == container_name and not container.get(probe_type):
                pods_without_probes.append(f"pod: {pod['metadata']['name']}, container: {container['name']}")
    return pods_without_probes


def scylla_pod_names(db_cluster: ScyllaPodCluster) -> list:
    pods = db_cluster.k8s_cluster.kubectl(f"get pods -n {db_cluster.namespace} --no-headers -l scylla/cluster={db_cluster.params.get('k8s_scylla_cluster_name')} -o=custom-columns='NAME:.metadata.name'")
...
def scylla_services_names(db_cluster: ScyllaPodCluster) -> list:
    ...
    services = db_cluster.k8s_cluster.kubectl(f"get svc -n {db_cluster.namespace} --no-headers -l scylla/cluster={scylla_cluster_name} -o=custom-columns='NAME:.metadata.name'")
    return [name for name in services.stdout.split() if name not in ("NAME", f"{scylla_cluster_name}-client")]


def wait_for_resource_absence(
    db_cluster: ScyllaPodCluster,  # pylint: disable=too-many-arguments
    resource_type: str,
    resource_name: str,
...
def get_pods_and_statuses(db_cluster: ScyllaPodCluster, namespace: str, label: str = None):
    ...
    return [{"name": pod["metadata"]["name"], "status": pod["status"]["phase"]} for pod in yaml.safe_load(pods.stdout)["items"] if pod]
...
def get_pod_storage_capacity(db_cluster: ScyllaPodCluster, namespace: str, pod_name: str = None, label: str = None):
    ...
    for pod in yaml.safe_load(persistent_volume_info.stdout)["items"]:
        ...
        pods_storage_capacity.append({"name": pod["metadata"]["name"], "capacity": pod["status"]["capacity"]["storage"]})

    return pods_storage_capacity


def reinstall_scylla_manager(db_cluster: ScyllaPodCluster, manager_version: str):
    values = HelmValues(yaml.safe_load(db_cluster.k8s_cluster.helm("get values scylla-manager -o yaml", namespace=SCYLLA_MANAGER_NAMESPACE)))
    if values.get("image.tag") != manager_version:
        log.info("Scylla Manager '%s' is going to be installed instead of the '%s' one.", manager_version, values.get("image.tag"))
        ...
        db_cluster.k8s_cluster.helm("uninstall scylla-manager --wait", namespace=SCYLLA_MANAGER_NAMESPACE)
        wait_for_resource_absence(db_cluster=db_cluster, step=1, timeout=120, resource_name="", resource_type="pod", namespace=SCYLLA_MANAGER_NAMESPACE)
        db_cluster.k8s_cluster.kubectl("delete pvc --all --wait=true", namespace=SCYLLA_MANAGER_NAMESPACE)
        ...
        time.sleep(5)
        db_cluster.k8s_cluster.kubectl_wait("--all --for=condition=Ready pod", namespace=SCYLLA_MANAGER_NAMESPACE, timeout=600)
...
def verify_resharding_on_k8s(db_cluster: ScyllaPodCluster, cpus: Union[str, int, float]):
    ...
    log.info("Update the cpu count to '%s' CPUs to make Scylla start the resharding process on all the nodes 1 by 1", cpus)
