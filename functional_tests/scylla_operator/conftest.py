...
def fixture_tester() -> ScyllaOperatorFunctionalClusterTester:
    ...
    start_events_device(log_dir=tester_inst.logdir, _registry=getattr(tester_inst, "_registry", None) or tester_inst.events_processes_registry)
    tester_inst.setUp()
    yield tester_inst
...
def fixture_db_cluster(tester: ScyllaOperatorFunctionalClusterTester):
    ...
    if dataset_name := tester.db_cluster.params.get("k8s_functional_test_dataset"):
        tester.db_cluster.wait_for_nodes_up_and_normal(nodes=tester.db_cluster.nodes, verification_node=tester.db_cluster.nodes[0])
    ...
    if tester.healthy_flag:
        _bring_cluster_back_to_original_state(tester, config_map=original_scylla_config_map, original_scylla_cluster_spec=original_scylla_cluster_spec)


def _bring_cluster_back_to_original_state(tester: ScyllaOperatorFunctionalClusterTester, config_map: dict, original_scylla_cluster_spec: dict):
    restart = False
    ...
    try:
        ...
        cluster_spec_diff = DeepDiff(original_scylla_cluster_spec, current_cluster_spec, ignore_order=True, exclude_paths=["forceRedeploymentReason", "repairs", "backups"])
...
def skip_if_scylla_not_semver(request, tester: ScyllaOperatorFunctionalClusterTester) -> None:
    if request.node.get_closest_marker("restart_is_used") and not version_utils.SEMVER_REGEX.match(tester.db_cluster.params.get("scylla_version")):
...
def skip_if_no_tls(request, tester: ScyllaOperatorFunctionalClusterTester) -> None:
    if request.node.get_closest_marker("requires_tls_and_sni") and not tester.params.get("k8s_enable_tls") and not tester.params.get("k8s_enable_sni"):
        pytest.skip("test requires k8s_enable_tls to be enabled")


def pytest_collection_modifyitems(items):
    start_tests = ["test_scylla_yaml_override", "test_orphaned_services_multi_rack"]
    end_tests = ["test_deploy_helm_with_default_values", "test_nodetool_flush_and_reshard"]
