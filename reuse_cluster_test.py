...
class ReuseClusterTest(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.cluster = None
        self._registry = create_default_events_process_registry(log_dir=TestConfig().logdir())
        start_events_device(_registry=self._registry)
    ...
    def provision_cluster(self, reuse_cluster_id: str = None) -> None:
        ...
        self.cluster = self.ReusableCluster(_registry=self._registry)
        self.cluster.setUp()

    def check_scylla(self) -> None:
        stress_cmd = (
            "cassandra-stress mixed 'ratio(write=1,read=2)' no-warmup cl=QUORUM duration=1m -schema "
            "'replication(strategy=NetworkTopologyStrategy,replication_factor=3)' "
            "-mode cql3 native -rate threads=10 -pop seq=1..10000 -log interval=5")
        self.cluster.run_stress(stress_cmd)

    def test_reuse_cluster(self):
        InfoEvent(message="Step 1 - Initial cluster provisioning").publish()
        ...
        initial_db_nodes = self.cluster.db_cluster.nodes

        InfoEvent(message="Step 2 - Pre-populate DB with data").publish()
        self.cluster.run_prepare_write_cmd()

        InfoEvent(message="Step 3 - Reuse the previously provisioned cluster").publish()
        self.provision_cluster(reuse_cluster_id=test_id)
        self.cluster.db_cluster.check_nodes_up_and_normal()
        self.cluster.db_cluster.check_cluster_health()
        self.check_scylla()

        InfoEvent(message="Step 4 - Verify that re-provisioned cluster consists of the initial DB nodes").publish()
        self.assertEquals(set(self.cluster.db_cluster.nodes), set(initial_db_nodes),
