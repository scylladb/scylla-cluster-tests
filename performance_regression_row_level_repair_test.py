...
class PerformanceRegressionRowLevelRepairTest(ClusterTester):
    ...
    def _pre_create_schema_large_scale(self, keyspace_num=1, scylla_encryption_options=None):
        ...
        for i in range(1, keyspace_num + 1):
            ...
            self.create_table(name=table_name, key_type="blob", read_repair=0.0, columns={'"C0"': "blob"}, scylla_encryption_options=scylla_encryption_options)
    ...
    def preload_data(self, consistency_level=None):
        ...
        if prepare_write_cmd:
            ...
            # One stress cmd command
                ...
                stress_cmd = prepare_write_cmd if not consistency_level else self._update_cl_in_stress_cmd(str_stress_cmd=prepare_write_cmd, consistency_level=consistency_level)
                stress_queue.append(self.run_stress_thread(stress_cmd=stress_cmd, stress_num=1, prefix="preload-", stats_aggregate_cmds=False))
    ...
    def test_row_level_repair_3_nodes_small_diff(self):
        ...
        :return:
        """
        base_distinct_write_cmd = "cassandra-stress write no-warmup cl=ONE n=1000000 -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor=3)' -mode cql3 native -rate threads=200 -col 'size=FIXED(1024) n=FIXED(1)'"
    ...
    def _populate_scylla_bench_data_in_parallel(self, base_cmd, partition_count, clustering_row_count, consistency_level="all"):
        n_loaders = int(self.params.get("n_loaders"))
        partitions_per_loader = partition_count // n_loaders
        str_additional_args = "-partition-count={} -clustering-row-count={} -consistency-level={}".format(partitions_per_loader, clustering_row_count, consistency_level)
        write_queue = []
        offset = 0
        for _ in range(n_loaders):
            str_offset = "-partition-offset {}".format(offset)
            stress_cmd = " ".join([base_cmd, str_additional_args, str_offset])
            self.log.debug("Scylla-bench stress command to execute: {}".format(stress_cmd))
            write_queue.append(self.run_stress_thread_bench(stress_cmd=stress_cmd, stats_aggregate_cmds=False, round_robin=True))
    ...
    def test_row_level_repair_large_partitions(self):
        ...
        self._populate_scylla_bench_data_in_parallel(base_cmd=scylla_bench_base_cmd, partition_count=partition_count, clustering_row_count=clustering_row_count)

        self._wait_no_compactions_running()

        offset = 0  # per node increased with interval of: partition_count_per_node * clustering_row_count_per_node * 10
        str_additional_args = "-partition-count={} -clustering-row-count={} -consistency-level=ALL".format(partition_count_per_node, clustering_row_count_per_node)

        for node in self.db_cluster.nodes:
            str_offset = "-partition-offset {}".format(offset)
            stress_cmd = " ".join([scylla_bench_base_cmd, str_additional_args, str_offset])
    ...
    def test_row_level_repair_during_load(self, preload_data=True):
        ...
        background_stress_cmds = [
            "cassandra-stress write no-warmup cl=QUORUM duration=140m -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor=3)' -mode cql3 native -rate threads=25 -col 'size=FIXED(1024) n=FIXED(1)'",
            "cassandra-stress read no-warmup cl=QUORUM duration=140m -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor=3)' -mode cql3 native -rate threads=6 -col 'size=FIXED(1024) n=FIXED(1)'",
        ]
