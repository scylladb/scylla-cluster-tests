...
class PerformanceRegressionRowLevelRepairTest(ClusterTester):
    ...
    def _run_repair(self, node):
        self.log.info(f"Running nodetool repair on {node.name}")
    ...
    def _pre_create_schema_large_scale(self, keyspace_num=1, scylla_encryption_options=None):
        ...
        for i in range(1, keyspace_num + 1):
            ...
            table_name = f"{keyspace_name}.standard1"
            self.create_table(
                name=table_name,
                key_type="blob",
                read_repair=0.0,
    ...
    def _update_cl_in_stress_cmd(self, str_stress_cmd, consistency_level):
        for param in str_stress_cmd.split():
            if param.startswith("cl="):
                return str_stress_cmd.replace(param, f"cl={consistency_level}")
        self.log.debug(f"Could not find a 'cl' parameter in stress command: {str_stress_cmd}")
            ...
            # One stress cmd command
                ...
                stress_queue.append(
                    self.run_stress_thread(
                        stress_cmd=stress_cmd, stress_num=1, prefix="preload-", stats_aggregate_cmds=False
                    )
                )
    ...
    def _wait_no_compactions_running(self):
        ...
        if results:
            assert any(float(v[1]) for v in results[0]["values"]) is False, "Waiting until all compactions settle down"
    ...
    def _disable_hinted_handoff(self):
        ...
        disable_hinted_handoff = dedent(
            f"""
                                                    grep -v hinted_handoff_enabled {yaml_file} > {tmp_yaml_file}
                                                    echo hinted_handoff_enabled: false >> {tmp_yaml_file}
    ...
    def _run_scylla_bench_on_single_node(self, node, stress_cmd):
        self.log.info(f"Stopping all other nodes before updating {node.name}")
        self.stop_all_nodes_except_for(node=node)
        self.log.info(f"Updating cluster data only for {node.name}")
        self.log.info(f"Run stress command of: {stress_cmd}")
        stress_queue = self.run_stress_thread_bench(stress_cmd=stress_cmd, stats_aggregate_cmds=False, round_robin=True)
    ...
    def test_row_level_repair_single_node_diff(self):
        ...
        for node in self.db_cluster.nodes:
            ...
            self.log.debug(f"Node {node.public_ip_address} initial used capacity is: {used_capacity}")

        self._disable_hinted_handoff()

        self.log.info(f"Stopping node-3 ({node1.name}) before updating cluster data")
        node1.stop_scylla_server()
        self.log.info(f"Updating cluster data when node3 ({node1.name}) is down")
        self.log.info("Starting c-s/s-b write workload")
        self.preload_data()
        self.wait_no_compactions_running()

        self.log.info(f"Starting node-3 ({node1.name}) after updated cluster data")
        node1.start_scylla_server()

        for node in self.db_cluster.nodes:
            used_capacity = self.get_used_capacity(node=node)
            self.log.debug(f"Node {node.public_ip_address} used capacity after pre-load data is: {used_capacity}")
    ...
    def test_row_level_repair_3_nodes_small_diff(self):
        ...
        :return:
        """
        base_distinct_write_cmd = (
            "cassandra-stress write no-warmup cl=ONE n=1000000 "
        ...
        for node in [node1, node2, node3]:
            ...
            self.stop_all_nodes_except_for(node=node)
            self.log.info(f"Updating cluster data only for {node.name}")
            distinct_write_cmd = f"{base_distinct_write_cmd} -pop seq={sequence_current_index + 1}..{sequence_current_index + sequence_range} -node {node.private_ip_address}"
            self.log.info(f"Run stress command of: {distinct_write_cmd}")
        ...
        self.log.info(f"Repair (99.8% synced) time on node: {node3.name} is: {repair_time}")
    ...
    def _populate_scylla_bench_data_in_parallel(
        ...
        for _ in range(n_loaders):
            str_offset = f"-partition-offset {offset}"
            stress_cmd = f"{base_cmd} {str_additional_args} {str_offset}"
            self.log.debug(f"Scylla-bench stress command to execute: {stress_cmd}")
            write_queue.append(
                self.run_stress_thread_bench(stress_cmd=stress_cmd, stats_aggregate_cmds=False, round_robin=True)
            )
    ...
    def test_row_level_repair_large_partitions(self):
        ...
        self._populate_scylla_bench_data_in_parallel(
            base_cmd=scylla_bench_base_cmd, partition_count=partition_count, clustering_row_count=clustering_row_count
        )

        self._wait_no_compactions_running()

        offset = 0  # per node increased with interval of: partition_count_per_node * clustering_row_count_per_node * 10
        str_additional_args = f"-partition-count={partition_count_per_node} -clustering-row-count={clustering_row_count_per_node} -consistency-level=ALL"

        for node in self.db_cluster.nodes:
            str_offset = f"-partition-offset {offset}"
            stress_cmd = f"{scylla_bench_base_cmd} {str_additional_args} {str_offset}"
        ...
        self.log.info(f"Repair (with large partitions) time on node: {node3.name} is: {repair_time}")
    ...
    def test_row_level_repair_during_load(self, preload_data=True):
        ...
        self.log.info(f"Repair (during r/w load) time on node: {node1.name} is: {repair_time}")
