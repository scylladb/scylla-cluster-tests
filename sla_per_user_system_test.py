...
class SlaPerUserTest(LongevityTest):
    ...
    STRESS_WRITE_CMD = "cassandra-stress write cl=QUORUM n={n} -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor=3)' -mode cql3 native user={user} password={password} -rate threads={threads}"
    ...
    STRESS_READ_CMD = "cassandra-stress read cl=ALL duration={duration} -mode connectionsPerHost=16 cql3 native user={user} password={password} -rate threads={threads} -pop {pop}"
    STRESS_MIXED_CMD = (
    ...
    def prepare_schema(self):
        ...
        self.connection_cql = self.db_cluster.cql_connection_patient(node=self.db_cluster.nodes[0], user=self.DEFAULT_USER, password=self.DEFAULT_USER_PASSWORD)
    ...
    def create_test_data_and_wait_no_compaction(self, rows_amount=None):
        ...
        write_cmd = self.STRESS_WRITE_CMD.format(n=self.num_of_partitions, user=self.DEFAULT_USER, password=self.DEFAULT_USER_PASSWORD, threads=250)
        self.run_stress_and_verify_threads(params={"stress_cmd": write_cmd, "prefix": "preload-", "stats_aggregate_cmds": False})
    ...
    def validate_ratio(expected_ratio, actual_ratio, msg):
        if not (expected_ratio and actual_ratio):
            WorkloadPrioritisationEvent.RatioValidationEvent(message=f"Can't compare expected and actual shares ratio. Expected: {expected_ratio}. Actual: {actual_ratio}", severity=Severity.ERROR).publish()

        elif expected_ratio <= actual_ratio:
            WorkloadPrioritisationEvent.RatioValidationEvent(message=f"{msg}. Actual ratio ({actual_ratio}) is as expected (more or equal then expected ratio {expected_ratio})", severity=Severity.NORMAL).publish()
        else:
            WorkloadPrioritisationEvent.RatioValidationEvent(message=f"{msg}. Actual ratio ({actual_ratio}) is less then expected ratio ({expected_ratio})", severity=Severity.ERROR).publish()
    ...
    def validate_deviation(self, expected_ratio, actual_ratio, msg):
        ...
        if dev is None:
            ...
            WorkloadPrioritisationEvent.RatioValidationEvent(message=f"{msg}. Actual ratio ({actual_ratio}) is not as expected ({expected_ratio})", severity=Severity.ERROR).publish()
            return False
        else:
            WorkloadPrioritisationEvent.RatioValidationEvent(message=f"{msg}. Actual ratio ({actual_ratio}) is as expected ({expected_ratio})", severity=Severity.NORMAL).publish()
            return True
    ...
    def get_c_s_stats(self, read_queue, users, statistic_name):
        ...
        for i, read in enumerate(read_queue):
            stat_rate, username = self.one_run_c_s_stats(read_run=read, user_name=role_names[i], statistic_name=statistic_name)

            if stat_rate is None:
                return stat_rate

            self.assertEqual(username, role_names[i], msg=f'Expected that stress was run with user "{role_names[i]}" but it was "{username}"')
    ...
    def validate_if_scylla_load_high_enough(self, start_time, wait_cpu_utilization):
        ...
        if scylla_load < wait_cpu_utilization:
            WorkloadPrioritisationEvent.CpuNotHighEnough(f"Load {scylla_load} isn't high enough(expected at least {wait_cpu_utilization}). The test results may be not correct.", severity=Severity.ERROR).publish()
            return False
    ...
    def warm_up_cache_before_test(self, max_key_for_read, stress_duration):
        read_cmds = [self.STRESS_READ_CMD.format(n=self.num_of_partitions, user=self.DEFAULT_USER, password=self.DEFAULT_USER, pop="seq=1..%d" % max_key_for_read, duration="%dm" % stress_duration, threads=200)]
        self.run_stress_and_verify_threads(params={"stress_cmd": read_cmds})

    def define_read_cassandra_stress_command(
        self, role: Role, load_type: str, c_s_workload_type: str, threads: int, stress_duration_min: int, max_rows_for_read: int = None, stress_command: str = STRESS_READ_CMD, throttle: int = 20000, **kwargs
    ):
        """
        ...
        def mixed(max_rows_for_read):
            ...
            return "'dist=gauss(1..%d, %d, %d)'" % (max_rows_for_read, int(max_rows_for_read / 2), int(max_rows_for_read * 0.05))
        ...
        params = {"n": self.num_of_partitions, "user": role.name, "password": role.password, "pop": pop, "duration": "%dm" % stress_duration_min, "threads": rate}
        if kwargs:
    ...
    def test_read_throughput_1to5_ratio(self):
        ...
        self._two_users_load_througput_workload(shares=[190, 950], load=self.MIXED_LOAD, expected_shares_ratio=expected_shares_ratio)
    ...
    def _two_users_load_througput_workload(self, shares, load, expected_shares_ratio=None):
        ...
        for share in shares:
            read_users.append(
                {
                    "user": User(session=session, name="user%d" % share, password="user%d" % share, superuser=True).create(),
                    "role": Role(session=session, name="role%d" % share, password="role%d" % share, login=True, superuser=True).create(),
        ...
        expected_shares_ratio = expected_shares_ratio or self.calculate_metrics_ratio_per_user(two_users_list=read_users)

        stress_duration = 10  # minutes
        read_cmds = [
            self.define_read_cassandra_stress_command(role=read_users[0]["role"], load_type=load, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=1000, stress_duration_min=stress_duration),
            self.define_read_cassandra_stress_command(role=read_users[1]["role"], load_type=load, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=1000, stress_duration_min=stress_duration),
        ]
        ...
        try:
            ...
            self.validate_if_scylla_load_high_enough(start_time=start_time, wait_cpu_utilization=self.MIN_CPU_UTILIZATION)
            ...
            self.validate_ratio(expected_ratio=expected_shares_ratio, actual_ratio=actual_shares_ratio, msg="Validate cassandra-stress ops")
    ...
    def test_read_throughput_vs_latency_cache_and_disk(self):
        ...
        read_cmds = {
            "throughput": self.define_read_cassandra_stress_command(
                role=read_users[0]["role"], load_type=self.MIXED_LOAD, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=200, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
            "latency": self.define_read_cassandra_stress_command(
                role=read_users[1]["role"], load_type=self.MIXED_LOAD, c_s_workload_type=self.WORKLOAD_LATENCY, threads=250, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
            "latency_throughput": self.define_read_cassandra_stress_command(
                role=read_users[1]["role"], load_type=self.MIXED_LOAD, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=1000, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
        }
        ...
        self._throughput_latency_tests_run(read_users=read_users, read_cmds=read_cmds, latency_user=read_users[1], improvement_expected=improvement_expected)
    ...
    def test_read_throughput_vs_latency_cache_only(self):
        ...
        read_cmds = {
            "throughput": self.define_read_cassandra_stress_command(
                role=read_users[0]["role"], load_type=self.CACHE_ONLY_LOAD, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=950, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
            "latency": self.define_read_cassandra_stress_command(
                role=read_users[1]["role"], load_type=self.CACHE_ONLY_LOAD, c_s_workload_type=self.WORKLOAD_LATENCY, threads=1000, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
            "latency_throughput": self.define_read_cassandra_stress_command(
                role=read_users[1]["role"], load_type=self.CACHE_ONLY_LOAD, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=1000, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
        }
    ...
    def test_read_throughput_vs_latency_disk_only(self):
        ...
        read_cmds = {
            "throughput": self.define_read_cassandra_stress_command(
                role=read_users[0]["role"], load_type=self.DISK_ONLY_LOAD, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=200, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read * 2
            ),
            "latency": self.define_read_cassandra_stress_command(
                role=read_users[1]["role"], load_type=self.DISK_ONLY_LOAD, c_s_workload_type=self.WORKLOAD_LATENCY, threads=250, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read * 3
            ),
            "latency_only": self.define_read_cassandra_stress_command(
                role=read_users[1]["role"], load_type=self.DISK_ONLY_LOAD, c_s_workload_type=self.WORKLOAD_LATENCY, threads=250, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
            "latency_throughput": self.define_read_cassandra_stress_command(
                role=read_users[1]["role"], load_type=self.DISK_ONLY_LOAD, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=1000, stress_duration_min=stress_duration, max_rows_for_read=max_key_for_read
            ),
        }
    ...
    def _throughput_latency_tests_run(self, read_cmds, read_users, latency_user, improvement_expected):
        ...
        read_queue = self.run_stress_and_verify_threads(params={"stress_cmd": [read_cmds.get("latency_only") or read_cmds["latency"]], "round_robin": True})

        latency_99_for_latency_workload = self.get_c_s_stats(read_queue=read_queue, users=[latency_user], statistic_name="latency 99th percentile")

        self.assertTrue(latency_99_for_latency_workload, msg="Not received cassandra-stress results for latency workload")

        # Run throughput (user950) and latency (user950) workloads
        latency_workload_same_user, throughput_user950_workload, user950_result_print_str = self._throughput_latency_parallel_run(
            read_cmds=read_cmds,
        ...
        latency_workload_mixed_users, throughput_user150_workload, user150_result_print_str = self._throughput_latency_parallel_run(
            read_cmds=read_cmds,
            test_start_time=test_start_time,
            latency_99_for_latency_workload=latency_99_for_latency_workload,
            latency_user=latency_user,
        ...
        self.log.info("Result of run with user950 throughput and user950 latency workloads: %s", user950_result_print_str)

        self.log.info("Result of run with user150 throughput and user950 latency workloads: %s", user150_result_print_str)

        improvement_actual = (throughput_user950_workload * latency_workload_mixed_users) / (throughput_user150_workload * latency_workload_same_user)
        if improvement_actual >= improvement_expected:
            WorkloadPrioritisationEvent.SlaTestResult(message=f"Actual improvement is {improvement_actual} more/equal then {improvement_expected} as expected.", severity=Severity.NORMAL).publish()

        else:
            WorkloadPrioritisationEvent.SlaTestResult(message=f"Actual improvement is {improvement_actual} less then expected {improvement_expected}", severity=Severity.ERROR).publish()
    ...
    def test_workload_types(self):
        ...
        # Define Service Levels/Roles/Users
        interactive_role = Role(session=session, name="interactive", password="interactive", login=True, verbose=True, superuser=True).create()
        batch_role = Role(session=session, name="batch1", password="batch1", login=True, verbose=True, superuser=True).create()
        interactive_sla = ServiceLevel(session=session, name="interactive", shares=None, workload_type="interactive").create()
        ...
        read_cmds = {
            ...
            "throughput_batch": self.define_read_cassandra_stress_command(
                role=batch_role, load_type=self.MIXED_LOAD, c_s_workload_type=self.WORKLOAD_THROUGHPUT, threads=120, stress_duration_min=stress_duration_min, stress_command=self.STRESS_MIXED_CMD, kwargs={"write_ratio": 1, "read_ratio": 1}
            ),
        }
    ...
    def _compare_workloads_c_s_metrics(self, workloads_queue: list) -> dict:
        ...
        assert len(workloads_results) == 2, "Expected workload_results length to be 2, got: %s. workload_results: %s" % (len(workloads_results), workloads_results)
        comparison_results = {}
        try:
            for item, target_margin in comparison_axis.items():
                ...
                within_margin = self.validate_deviation(expected_ratio=target_margin, actual_ratio=ratio, msg=f'Validate workload ration for "{item}" item. ')

                comparison_results.update({item: {"interactive": interactive, "batch": batch, "diff": batch - interactive, "ratio": ratio, "within_margin": within_margin}})
    ...
    def upload_c_s_comparison_to_es(self) -> None:
        ...
        es_body = {self.db_cluster.get_node().db_node_instance_type: {"test_id": self.test_id, "backend": self.db_cluster.params.get("cluster_backend"), "scylla_version": self.get_scylla_versions(), **self._comparison_results}}
    ...
    def get_email_data(self):
        ...
        email_data.update({"scylla_ami_id": self.params.get("ami_id_db_scylla") or "-", "region": self.params.get("region_name") or "-", "workload_comparison": self._comparison_results if self._comparison_results else {}})
    ...
    def _throughput_latency_parallel_run(self, read_cmds, test_start_time, latency_99_for_latency_workload, latency_user, throughput_user, throughput_cmd_name, latency_cmd_name):
        def __get_stat_for_user(read, user_name):
            ...
            stat_rate, _ = self.one_run_c_s_stats(read_run=read, user_name=user_name, statistic_name="latency 99th percentile")

            if stat_rate:
                latency_99_for_mixed_workload[user_name] = float(stat_rate)

        self.log.debug("Start latency workload (user %s) in parallel with throughput workload (user %s)", latency_user, throughput_user)
        read_queue = self.run_stress_and_verify_threads(params={"stress_cmd": [read_cmds[throughput_cmd_name], read_cmds[latency_cmd_name]], "round_robin": True})
