...
class SlaPerUserTest(LongevityTest):
    ...
    def validate_scheduler_runtime(self, start_time, end_time, read_users, expected_ratio):
        ...
        for node_ip in self.db_cluster.get_node_private_ips():
            ...
            test_users_to_sg = self.role_to_scheduler_group(
                test_users=roles_with_shares, scheduler_shares=scheduler_shares
            )
            ...
            for rolename, val in test_users_to_sg.items():
                if val[1] in shards_time_per_sla[node_ip]:
                    runtime_per_role[rolename] = sum(shards_time_per_sla[node_ip][val[1]]) / len(
                        shards_time_per_sla[node_ip][val[1]]
                    )
                else:
            ...
            actual_shares_ratio = self.calculate_metrics_ratio_per_user(
                two_users_list=read_users, metrics=runtime_per_role
            )
            self.validate_deviation(
                expected_ratio=expected_ratio,
                actual_ratio=actual_shares_ratio,
                msg=f"Validate scheduler CPU runtime on the node {node_ip}. Run time per role: {runtime_per_role}",
            )
    ...
    def validate_ratio(expected_ratio, actual_ratio, msg):
        if not (expected_ratio and actual_ratio):
            ...
            WorkloadPrioritisationEvent.RatioValidationEvent(
                message=f"{msg}. Actual ratio ({actual_ratio}) is as expected (more or equal then expected ratio {expected_ratio})",
    ...
    def validate_if_scylla_load_high_enough(self, start_time, wait_cpu_utilization):
        ...
        if scylla_load < wait_cpu_utilization:
            WorkloadPrioritisationEvent.CpuNotHighEnough(
                f"Load {scylla_load} isn't high enough(expected at least {wait_cpu_utilization}). The test results may be not correct.",
                severity=Severity.ERROR,
            ).publish()
            return False
    ...
    def _two_users_load_througput_workload(self, shares, load, expected_shares_ratio=None):
        ...
        try:
            ...
            self.validate_scheduler_runtime(
                start_time=start_time, end_time=end_time, read_users=read_users, expected_ratio=expected_shares_ratio
            )
    ...
    def _throughput_latency_tests_run(self, read_cmds, read_users, latency_user, improvement_expected):
        ...
        if improvement_actual >= improvement_expected:
            WorkloadPrioritisationEvent.SlaTestResult(
                message=f"Actual improvement is {improvement_actual} more/equal then {improvement_expected} as expected.",
                severity=Severity.NORMAL,
            ).publish()
