...
class PlatformMigrationTest(LongevityTest):
    ...
    def setUp(self):
        ...
        target_image_param = self.params.get("ami_id_db_scylla_target")
        self.target_images = target_image_param.split() if target_image_param else []
    ...
    def _get_target_image_for_dc(self, dc_idx: int) -> str:
        ...
        if dc_idx >= len(self.target_images):
            raise ValueError(f"No target image for DC index {dc_idx}. Available: {self.target_images}")
    ...
    def _decommission_nodes(self, nodes: list) -> None:
        """Decommission a list of nodes sequentially"""
        for i, node in enumerate(nodes, 1):
            self.log.info("Decommissioning node %d/%d: %s", i, len(nodes), node.name)
            ...
            self.log.info("Decommissioned %s", node.name)
        ...
        while dc_iter:
            ...
            if batch:
                ...
                with ThreadPoolExecutor(max_workers=len(batch)) as executor:
                    for node in batch:
                        future = executor.submit(self.db_cluster.decommission, node)
                        future.result()
                        self.log.info("Decommissioned %s", node.name)
