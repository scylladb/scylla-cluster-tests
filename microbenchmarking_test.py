...
class PerfSimpleQueryTest(ClusterTester):
    def test_perf_simple_query(self):
        ...
        result = self.db_cluster.nodes[0].remoter.run(f"scylla perf-simple-query --json-result=perf-simple-query-result.txt --smp 1 -m 1G {perf_simple_query_extra_command}")
        if result.ok:
            output = self.db_cluster.nodes[0].remoter.run("cat perf-simple-query-result.txt").stdout
            results = json.loads(output)
            self.create_test_stats(specific_tested_stats={"perf_simple_query_result": results}, doc_id_with_timestamp=True)
            if self.create_stats:
                is_gce = self.params.get("cluster_backend") == "gce"
                PerfSimpleQueryAnalyzer(self._test_index).check_regression(self._test_id, is_gce=is_gce, extra_jobs_to_compare=self.params.get("perf_extra_jobs_to_compare"))
