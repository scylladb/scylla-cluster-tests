...
class PerformanceRegressionAlternatorTest(PerformanceRegressionTest):
    ...
    def _prepare_and_execute_workload_with_latency_calculator_decorator(self, *, test_name, row_name, target_ops_text, stress_num=1, **kwargs):
        ...
        try:
            ...
            self.log.error(f"Error while running workload with test_name={test_name}, stress_num={stress_num}, kwargs={kwargs}: {e}")
            raise
        finally:
            self.log.info(f"Finished workload with test_name={test_name}, stress_num={stress_num}, kwargs={kwargs}")

    def _workload(self, stress_cmd, stress_num, test_name=None, sub_type=None, keyspace_num=1, prefix="", debug_message="", save_stats=True, is_alternator=True):
        ...
        if save_stats:
            self.create_test_stats(test_name=test_name, sub_type=sub_type, doc_id_with_timestamp=True, append_sub_test_to_name=False)
    ...
    def create_cql_ks_and_table(self, field_number):
        ...
        with self.db_cluster.cql_connection_patient(node) as session:
            session.execute("""create keyspace ycsb WITH REPLICATION = {'class' : 'NetworkTopologyStrategy', 'replication_factor': 3 };""")
    ...
    def run_test_suite_by_configuration_name(self, mode):  # noqa: PLR0914
        ...
        def run_read_cql():
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text=total_target_ops_per_sec_text, test_name=self.id() + "_read", sub_type="cql", stress_cmd=base_cmd_r + cmd_add_params, stress_num=1, keyspace_num=1, is_alternator=False, row_name="cql"
            )
        ...
        def run_read_alternator_no_lwt():
            ...
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text=total_target_ops_per_sec_text, test_name=self.id() + "_read", sub_type="without-lwt", stress_cmd=base_cmd_r + cmd_add_params, stress_num=1, keyspace_num=1, row_name="alternator-no-lwt"
            )

        def run_write_cql():
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text=total_target_ops_per_sec_text, test_name=self.id() + "_write", sub_type="cql", stress_cmd=base_cmd_w + cmd_add_params, stress_num=1, keyspace_num=1, is_alternator=False, row_name="cql"
            )

        def run_write_alternator_no_lwt():
            self.alternator.set_write_isolation(node=node, isolation=alternator.enums.WriteIsolation.FORBID_RMW)
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text=total_target_ops_per_sec_text, test_name=self.id() + "_write", sub_type="without-lwt", stress_cmd=base_cmd_w + cmd_add_params, stress_num=1, keyspace_num=1, row_name="alternator-no-lwt"
            )
        ...
        def run_mixed_cql():
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text=total_target_ops_per_sec_text, test_name=self.id() + "_mixed", sub_type="cql", stress_cmd=base_cmd_m + cmd_add_params, stress_num=1, keyspace_num=1, is_alternator=False, row_name="cql"
            )

        def run_mixed_alternator_no_lwt():
            self.alternator.set_write_isolation(node=node, isolation=alternator.enums.WriteIsolation.FORBID_RMW)
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text=total_target_ops_per_sec_text, test_name=self.id() + "_mixed", sub_type="without-lwt", stress_cmd=base_cmd_m + cmd_add_params, stress_num=1, keyspace_num=1, row_name="alternator-no-lwt"
            )
        ...
        def run_read_throughput_cql():
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text="unthrottled", test_name=self.id() + "_throughput_read", sub_type="cql", stress_cmd=base_cmd_r + cmd_add_throughput_params, stress_num=1, keyspace_num=1, is_alternator=False, row_name="cql"
            )

        def run_read_throughput_alternator_no_lwt():
            self.alternator.set_write_isolation(node=node, isolation=alternator.enums.WriteIsolation.FORBID_RMW)
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text="unthrottled", test_name=self.id() + "_throughput_read", sub_type="without-lwt", stress_cmd=base_cmd_r + cmd_add_throughput_params, stress_num=1, keyspace_num=1, row_name="alternator-no-lwt"
            )

        def run_write_throughput_cql():
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text="unthrottled", test_name=self.id() + "_throughput_write", sub_type="cql", stress_cmd=base_cmd_w + cmd_add_throughput_params, stress_num=1, keyspace_num=1, is_alternator=False, row_name="cql"
            )

        def run_write_throughput_alternator_no_lwt():
            self.alternator.set_write_isolation(node=node, isolation=alternator.enums.WriteIsolation.FORBID_RMW)
            self._prepare_and_execute_workload_with_latency_calculator_decorator(
                target_ops_text="unthrottled", test_name=self.id() + "_throughput_write", sub_type="without-lwt", stress_cmd=base_cmd_w + cmd_add_throughput_params, stress_num=1, keyspace_num=1, row_name="alternator-no-lwt"
            )
