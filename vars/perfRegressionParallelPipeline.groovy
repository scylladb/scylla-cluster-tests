...
def call(Map pipelineParams) {
    ...
    pipeline {
        ...
        parameters {
            ...
            string(defaultValue: "${groovy.json.JsonOutput.toJson(pipelineParams.get('sub_tests', ["test_write"]))}",
                   description: 'subtests',
                   name: 'sub_tests')
        ...
        stages {
            stage('Run SCT Performance Tests') {
                steps {
                    script {
                        def tasks = [:]
                        def sub_tests = new JsonSlurper().parseText(params.sub_tests)
