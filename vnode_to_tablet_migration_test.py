...
class VnodeToTabletMigrationTest(LongevityTest):
    ...
    def _perform_migration_steps(self) -> None:
        ...
        # Start nemesis only after migration has been issued and all nodes confirmed USES_VNODES.
        # Starting earlier risks a nemesis stopping a node during the status check, causing
        # node.host_id to return None (it is not cached) and raising KeyError on migration_status[None].
    ...
    def run_pre_migration_benchmark(self):
        ...
        for cmd in self.params.get("stress_cmd"):
            stress_queue.append(self.run_stress_thread(stress_cmd=cmd, duration=10))
