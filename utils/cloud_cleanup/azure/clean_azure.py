...
def get_vm_creation_time(v_m, resource_group_name):
    try:
        ...
        try:
            ...
            LOGGER.info("Failed to update VM tags: %s in resource group: %s with exception: %s", v_m.name, resource_group_name, exc)
...
def delete_virtual_machine(resource_group_name, vm_name, test_id, dry_run=False):
    if dry_run:
        ...
        try:
            ...
            LOGGER.info("Failed to delete VM: %s in resource group: %s with exception: %s", vm_name, resource_group_name, exc)
            return
...
def stop_virtual_machine(resource_group_name, vm_name, test_id, dry_run=False):
    if dry_run:
        ...
        try:
            ...
            LOGGER.info("Failed to stop VM: %s in resource group: %s with exception: %s", vm_name, resource_group_name, exc)
            return
...
def clean_azure_instances(dry_run=False):
    resource_groups = [resource_group for resource_group in resource_client.resource_groups.list() if (resource_group.tags.get("keep", "") != "alive" if resource_group.tags else True)]
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--dry-run", action=argparse.BooleanOptionalAction, help="do not stop or terminate anything", default=os.environ.get("DRY_RUN"))
