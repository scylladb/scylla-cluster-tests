    ...
    for zone in zones:
        for instance in zone[1].instances:
            ...
            if should_keep(vm_creation_time, get_keep_hours(instance_metadata)):
                LOGGER.info("keeping instance %s, keep: %s, creation time: %s ", instance.name, instance_metadata.get("keep", "not set"), vm_creation_time)
                continue
            if instance_metadata.get("keep_action", "terminate") in ("terminate", ""):  # terminate by default if not set
                if not dry_run:
                    ...
                    try:
                        res = instances_client.delete(instance=instance.name, project=project_id, zone=instance.zone.split("/")[-1])
                        res.done()
                    ...
                    LOGGER.info("dry run: would terminate instance %s, creation time: %s", instance.name, vm_creation_time)
                ...
                if not dry_run:
                    ...
                    try:
                        res = instances_client.stop(instance=instance.name, project=project_id, zone=instance.zone.split("/")[-1])
                        res.done()
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--duration", type=int, help="duration to keep non-tagged instances running in hours", default=os.environ.get("DURATION", str(DEFAULT_KEEP_HOURS)))
