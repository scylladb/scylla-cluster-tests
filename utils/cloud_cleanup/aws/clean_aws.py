...
def print_instance(instance, msg):
    print("instance %s type %s launched at %s keys %s %s" % (instance.id, instance.instance_type, instance.launch_time, instance.key_name, msg))
...
def scan_region_instances(region_name, duration):
    ...
    for instance in instances:
        debug("checking instance %s, type %s, tags %s, placement %s, launch_time %s" % (instance.id, instance.instance_type, instance.tags, instance.placement, instance.launch_time))
...
def clean_instances(region_name, duration):
    ...
    print("region %s: stopped %d, terminated %d, kept alive %d and expiring %d, running %d instances" % (region_name, len(expired) - terminated, terminated, keep_alive, expiring, running))
...
def clean_volumes(region_name):
    ...
    for volume in volumes:
        debug("checking volume %s %s %s %s %s" % (volume.id, volume.volume_id, volume.state, volume.tags, volume.create_time))
...
def clean_capacity_reservations(region_name):
    ...
    for cr in response["CapacityReservations"]:
        ...
        print(
            ...
            % (cr["CapacityReservationId"], cr["StartDate"], cr["InstanceType"], cr["TotalInstanceCount"], cr["AvailableInstanceCount"], test_id)
        )
        if (
...
def clean_unattached_security_groups(region_name: str):
    ...
    for group in groups:
        ...
        if group["GroupName"] in ("default", "SCT-2-sg", "SCT-builder-ssh-sg") or {"Key": "keep", "Value": "alive"} in tags:
            continue
        if group["GroupId"] not in attached_group_ids:
            deleted += 1
            if VERBOSE:
                print(f"Found unattached security group: {group['GroupId']} ({group['GroupName']}) in {region_name}\ntags: {tags}")
            if not DRY_RUN:
                try:
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--duration", type=int, help="duration to keep non-tagged instances running in hours", default=os.environ.get("DURATION", DEFAULT_KEEP_HOURS))
    arg_parser.add_argument("--verbose", action="store_true", help="print processing instances details", default=os.environ.get("VERBOSE"))
    arg_parser.add_argument("--trace", action="store_true", help="trace every AWS call", default=os.environ.get("TRACE"))
    arg_parser.add_argument("--wait", type=int, help="blind wait for instances to stop timeout", default=os.environ.get("WAIT", "60"))
    arg_parser.add_argument("--dry-run", action="store_true", help="do not stop or terminate anything", default=os.environ.get("DRY_RUN"))
    arg_parser.add_argument("--default-action", help="The default action when stopping an image (stop/terminate)", default=os.environ.get("DEFAULT_ACTION", "terminate"))
