...
def migrate(creds, job_name, days=7, index_name="performancestatsv2", dry_run=True):
    ...
    query = {
        "query": {
            "bool": {
                "must": [
                    {"range": {"test_details.start_time": {"gte": (datetime.datetime.utcnow() - datetime.timedelta(days=days)).timestamp(), "lte": datetime.datetime.utcnow().timestamp(), "boost": 2.0}}},
    ...
    for hit in res["hits"]["hits"]:
        ...
        client = ArgusSCTClient(document_id, auth_token=creds["token"], base_url=creds["baseUrl"], extra_headers=creds.get("extra_headers"))
        try:
        ...
        for operation in ordered_latency_during_ops:
            if operation == "Steady State":
                ...
                if dry_run:
                    ...
                    continue
                try:
                    start_time = result["hdr_summary"].get("READ", result["hdr_summary"].get("WRITE"))["start_time"] / 1000
                    send_result_to_argus(argus_client=client, workload=workload, name=operation, description=description, cycle=0, result=result, start_time=start_time)
                except argus.client.base.ArgusClientError:
                    print(f"Failed to send {operation} - {workload} - latencies to Argus: {hit['_source']['test_details']['job_url']}")
                continue
            for idx, cycle in enumerate(latency_during_ops[operation]["cycles"], start=1):
                ...
                if dry_run:
                    ...
                    continue
                try:
                    send_result_to_argus(argus_client=client, workload=workload, name=operation, description=latency_during_ops[operation]["legend"] or "", cycle=idx, result=cycle, start_time=start_time)
                except RuntimeError:
                    # happens when no EventsDevice is running and trying to raise SCT event
                    pass
                except argus.client.base.ArgusClientError:
                    print(f"Failed to send {operation} - {workload} - latencies - cycle {idx} to Argus: {hit['_source']['test_details']['job_url']}")
