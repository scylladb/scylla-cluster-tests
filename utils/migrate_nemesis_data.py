...
def migrate(old_index_name, dry_run, new_index, days):
    ...
    res = scan(
        ...
        query={"query": {"range": {"test_details.start_time": {"gte": (datetime.datetime.utcnow() - datetime.timedelta(days=days)).timestamp(), "lte": datetime.datetime.utcnow().timestamp(), "boost": 2.0}}}},
        size=300,
        scroll="3h",
    )
    ...
    for num, hit in enumerate(res):
        ...
        if "scylla-server" not in test_data["versions"]:
            LOGGER.debug("%s: No version for %s - %s", num, test_data["test_details"]["test_id"], test_data["test_details"]["job_name"])
