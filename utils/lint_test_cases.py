...
class TestCaseAnalyzer:
    ...
    def run_yaml_test_for_config(
        ...
        """
        # Run the test
        error, output = self._run_yaml_test(
    ...
    def analyze_file(self, file_path: Path) -> TestCaseAnalysis:
        ...
        is_multi_dc = self._is_multi_dc(filepath_str, content)
        if is_multi_dc:
            dc_count = self._get_dc_count(filepath_str, content)
            reason_parts.append(f"multi-DC with {dc_count} DCs" if dc_count else "multi-DC")
    ...
    def _is_multi_dc(self, filepath: str, content: Optional[Dict]) -> bool:
        """Check if test case is multi-DC."""

        # Check content for multi-DC indicators
        if content:
            # Check for multiple regions/datacenters
            if "n_db_nodes" in content:
                n_db_nodes = str(content["n_db_nodes"])
                # Format like "3 3 3" indicates multi-DC
                if " " in n_db_nodes:
                    return True

            # Check for GCE datacenter specification
            if "gce_datacenter" in content:
                gce_dc = content["gce_datacenter"]
                if isinstance(gce_dc, str) and " " in gce_dc:
                    return True

            # Check for region specifications
            for key in ["region_name", "gce_datacenter"]:
                if key in content:
                    value = content[key]
    ...
    def _get_dc_count(self, filepath: str, content: Optional[Dict]) -> Optional[int]:
        """Determine the number of datacenters."""
        # Check filename for explicit DC count
        match = re.search(r"(\d+)dcs", filepath)
        if match:
    ...
    def _get_aws_configs(self, analysis: TestCaseAnalysis) -> List[BackendConfig]:
        ...
        if analysis.is_multi_dc and analysis.dc_count:
            if analysis.dc_count == 5:
                # 5 DC configuration
                configs.append(BackendConfig(
                    backend=Backend.AWS,
                    env_vars={
                        "SCT_AMI_ID_DB_SCYLLA": "ami-1 ami-2 ami-3 ami-4 ami-5",
                        "SCT_REGION_NAME": "eu-central-1 eu-north-1 eu-west-1 eu-west-2 us-east-1",
                        "SCT_SCYLLA_REPO": "http://downloads.scylladb.com.s3.amazonaws.com/rpm/centos/scylla-2021.1.repo",
                    },
                    description="AWS multi-DC with 5 regions",
                    multi_region=True,
                    region_count=5,
                ))
        else:
    ...
    def _get_gce_configs(self, analysis: TestCaseAnalysis) -> List[BackendConfig]:
        ...
        if analysis.is_multi_dc and analysis.dc_count:
            if analysis.dc_count == 2:
                env = base_env.copy()
                env["SCT_GCE_DATACENTER"] = "us-east1 us-west1"
                configs.append(BackendConfig(
                    backend=Backend.GCE,
                    env_vars=env,
                    description="GCE multi-DC with 2 regions",
                    multi_region=True,
                    region_count=2,
                ))
            elif analysis.dc_count == 3:
                env = base_env.copy()
                env["SCT_GCE_DATACENTER"] = "us-east1 us-west1 eu-north1"
                configs.append(BackendConfig(
                    backend=Backend.GCE,
                    env_vars=env,
                    description="GCE multi-DC with 3 regions",
                    multi_region=True,
                    region_count=3,
                ))
            else:
    ...
    def _get_k8s_config(self, analysis: TestCaseAnalysis, backend: Backend) -> BackendConfig:
        ...
        if backend == Backend.K8S_EKS:
            ...
            if analysis.is_multi_dc and analysis.dc_count:
                if analysis.dc_count == 2:
                    env_vars["SCT_REGION_NAME"] = "eu-north-1 eu-west-1"
                    description = f"Kubernetes EKS multi-DC with {analysis.dc_count} regions"
                elif analysis.dc_count == 3:
                    env_vars["SCT_REGION_NAME"] = "eu-north-1 eu-west-1 us-east-1"
                    description = f"Kubernetes EKS multi-DC with {analysis.dc_count} regions"
                else:
                    env_vars["SCT_REGION_NAME"] = "eu-north-1 eu-west-1"
                    description = f"Kubernetes EKS multi-DC with {analysis.dc_count} regions"
            ...
            if analysis.is_multi_dc and analysis.dc_count:
                if analysis.dc_count == 2:
                    env_vars["SCT_GCE_DATACENTER"] = "us-east1 us-west1"
                    description = f"Kubernetes GKE multi-DC with {analysis.dc_count} regions"
                elif analysis.dc_count == 3:
                    env_vars["SCT_GCE_DATACENTER"] = "us-east1 us-west1 eu-north1"
                    description = f"Kubernetes GKE multi-DC with {analysis.dc_count} regions"
                else:
                    env_vars["SCT_GCE_DATACENTER"] = "us-east1 us-west1"
                    description = f"Kubernetes GKE multi-DC with {analysis.dc_count} regions"
    ...
    def group_by_backend_config(
        self,
        test_cases: Optional[List[Path]] = None
    ) -> Dict[str, List[Path]]:
        """
...
def _process_tests_parallel(
    ...
    with multiprocessing.Pool(processes=workers) as pool:
        ...
        for exit_code, failures, file_path in pool.imap_unordered(_worker_process_file, test_cases):
            ...
            # Print results immediately as each file completes
            if exit_code == 0:
                # Success - compact output
                click.secho(f"✅ {rel_path}", fg='green')
            else:
                # Failure - show details immediately
                for failure in failures:
                    _display_test_failure(failure, rel_path)
...
def _display_analysis_result(
    ...
    if verbose:
        click.echo(f"  Multi-DC: {analysis.is_multi_dc}")
...
@click.option(
    '--group',
    is_flag=True,
    help='Group test cases by backend configuration'
)
@click.option(
    '--verbose', '-v',
    is_flag=True,
...
@click.option(
    ...
    'run_tests',
    is_flag=True,
    help='Run YAML validation tests for the analyzed files'
)
@click.option(
    '--test',
    'run_tests_alias',
