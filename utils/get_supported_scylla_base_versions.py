...
class UpgradeBaseVersion:
    ...
    def set_start_support_version(self, backend: str = None) -> None:
        ...
        if self.dist_type not in ["debian", "ubuntu"] and self.linux_distro != "centos":
            ...
            if versions_dict is None:
                raise Exception(f"We can't detect the support versions for this distro, which shares the repo with {self.dist_type}.")
        ...
        LOGGER.info("Support start versions set: oss=%s enterprise=%s", self.oss_start_support_version, self.ent_start_support_version)
    ...
    def get_version_list(self) -> tuple[list, list]:
        ...
        for version_prefix, _ in self.repo_maps.items():
            ...
            if self.oss_start_support_version and not is_enterprise(version_prefix) and ComparableScyllaVersion(version_prefix) < self.oss_start_support_version:
                continue
            # Enterprise: the major version is smaller than the start support version
            if self.ent_start_support_version and is_enterprise(version_prefix) and ComparableScyllaVersion(version_prefix) < self.ent_start_support_version:
                continue
