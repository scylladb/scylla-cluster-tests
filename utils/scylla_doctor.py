...
class ScyllaDoctor:
    ...
    def download_scylla_doctor(self):
        ...
        if not latest_package:
            TestFrameworkEvent(source=self.__class__.__name__, severity=Severity.ERROR,
                               message="Unable to find latest scylla-doctor package for offline install").publish()
        ...
        self.node.remoter.run(f"curl -JLO {self.SCYLLA_DOCTOR_OFFLINE_DOWNLOAD_URI}{package_path}")
        self.node.remoter.run(f"tar -xvzf {self.current_dir}/{package_filename}")
