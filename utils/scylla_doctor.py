...
class ScyllaDoctor:
    ...
    def locate_newest_scylla_doctor_package(self):
        s3 = boto3.client("s3")
        packages = s3.list_objects(Bucket=self.SCYLLA_DOCTOR_OFFLINE_BUCKET_NAME, Prefix=self.SCYLLA_DOCTOR_OFFLINE_BUCKET_PREFIX, MaxKeys=5000)
        latest = next(iter(sorted(packages["Contents"], key=lambda package: package["LastModified"], reverse=True)), None)
        return latest
    ...
    def install_scylla_doctor(self):
        ...
        if self.offline_install or self.node.parent_cluster.cluster_backend == "docker":
            ...
            if self.node.is_nonroot_install:
                ...
                self.update_scylla_doctor_config(self.current_dir, additional_config=self.SCYLLA_DOCTOR_DISABLED_OFFLINE_COLLECTORS)
        else:
    ...
    def find_local_python3_binary(self, user_home: str):
        if not (python3_path := self.node.remoter.run(f"ls {user_home}/scylladb/python3/bin/python3", verbose=False).stdout.strip()):
            python3_path = self.node.remoter.run("for i in `find scylladb -name python3`;do [ -f $i ] && echo $i;done").stdout.strip()
    ...
    def run_scylla_doctor_and_collect_results(self):
        ...
        assert self.json_result_file, f"Vitals result json file {json_name} has not been created. Scylla doctor version: {self.version}"
        ...
        if self.node.parent_cluster.cluster_backend != "docker":
            ...
            assert self.scylla_logs_file, f"Scylla log archive {self.scylla_logs_file} has not been created. Scylla doctor version: {self.version}"
    ...
    def filter_out_failed_collectors(self, collector):
        ...
        if self.node.parent_cluster.cluster_backend == "docker" and collector in ["StorageConfigurationCollector", "PerftuneSystemConfigurationCollector"]:
            return True

        if self.node.distro.is_debian and self.offline_install and collector in ["RAIDSetupCollector", "SysctlCollector"]:
