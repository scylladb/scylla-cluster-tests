...
class ScyllaDoctor:
    ...
    def run_analysis_phase(self):
        ...
        findings and recommendations about the cluster health.  The raw
        analysis output (stdout) is written to a file on the remote
        node so it can be downloaded for post-mortem inspection.

        Note: scylla-doctor ``--load-vitals`` always produces human-readable
        output (ASCII banner + tabular report) regardless of the ``--verbose``
        flag.  The ``--verbose`` flag only adds extra details.  We save the
        output to a ``.txt`` file and parse the table format in
        ``analyze_and_verify_analysis_results()``.
        """
        ...
        analysis_name = f"{self.node.public_dns_name}.analysis.txt"
        LOGGER.info("Running Scylla Doctor analysis phase (full edition)...")

        # scylla-doctor --load-vitals always outputs a human-readable report
        # (ASCII banner, table of collectors/analyzers with PASSED/FAILED/etc.).
        # The --verbose flag adds extra detail but does NOT switch to JSON mode.
        sd_cmd = f"{self.scylla_doctor_exec} --load-vitals {self.json_result_file}"
        tee_cmd = f"{sd_cmd} | tee {analysis_name}"

        if not self.node.is_nonroot_install:
            result = self.node.remoter.sudo(tee_cmd, verbose=False)
        else:
            result = self.node.remoter.run(f"bash -lce '{self.python3_path} {tee_cmd}'", verbose=False)

        LOGGER.info("Analysis phase output:\n%s", result.stdout.strip())

        # Verify the analysis report was created
        check = self.node.remoter.run(f"test -s {analysis_name}", verbose=False, ignore_status=True)
        if check.ok:
            self.analysis_report_file = analysis_name
            LOGGER.info("Analysis report saved to: %s", self.analysis_report_file)
        else:
            raise ScyllaDoctorException(
                f"Analysis report file {analysis_name} was not created. Scylla doctor version: {self.version}"
            )
    ...
    def _parse_human_readable_analysis(content: str) -> dict:
        """Parse the human-readable scylla-doctor analysis output.

        Scylla-doctor ``--load-vitals`` always produces a tabular report with
        ...
        returns a dict compatible with the JSON-based verification:
        ...
        results = {}
        # Matches lines like:
        #   - CPUScalingAnalyzer: CPU scaling setup    FAILED    CPU does not support scaling
        ...
        for line in content.splitlines():
            ...
            if in_analysis_section and stripped.startswith("+ "):
                # New section started — we've left the analysis section
                break
    ...
    def analyze_and_verify_analysis_results(self):
        """Parse the analysis report and verify that no analyzer failed.

        The analysis report may be in one of two formats:

        1. **JSON** — each top-level key is an analyzer name mapped to a dict
           with at least a ``status`` field (0 = success, 1 = failed, 2 = skipped).
        2. **Human-readable** — a tabular report with PASSED/FAILED/WARNING/SKIPPED
           columns, produced by ``scylla-doctor --load-vitals``.

        The method first tries JSON parsing; if that fails it falls back to
        ...
        # Try JSON first (future-proofing in case scylla-doctor adds JSON mode)
        analysis_result = None
        try:
            analysis_result = self._extract_json_from_output(raw_content)
            LOGGER.debug("Parsed analysis report as JSON")
        except ScyllaDoctorException:
            LOGGER.info(
                "Analysis report is not JSON — parsing human-readable format. Report file: %s",
                self.analysis_report_file,
            )

        # Fall back to human-readable format
        if analysis_result is None:
            analysis_result = self._parse_human_readable_analysis(raw_content)
            if not analysis_result:
                LOGGER.warning(
                    "Could not extract any analyzer results from the analysis report. "
                    "The report is saved on the node at %s for manual inspection. "
                    "First 300 chars: %s",
                    self.analysis_report_file,
                    raw_content[:300],
                )
                return
            LOGGER.info("Parsed %d analyzer results from human-readable report", len(analysis_result))
        ...
        assert not failed_analyzers, f"Failed analyzers: {failed_analyzers}. Scylla doctor version: {self.version}"
