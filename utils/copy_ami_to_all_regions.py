...
def copy_ami_to_all_regions(ami_id, ami_name, source_region, target_regions, dry_run):
...
@click.option("--dry-run", is_flag=True, default=False, help="Show what would be done without making changes")
@click.option(
    "--copy-permissions", is_flag=True, default=False, help="Copy launch permissions from source AMI to target AMIs"
)
def copy_ami_to_all_regions(ami_id, ami_name, source_region, target_regions, dry_run, copy_permissions):  # noqa: PLR0914
>>>>>>> e29892926 (improvement(treewide): Reformat using ruff)
    ...
    tags_response = ec2_source.describe_tags(Filters=[{"Name": "resource-id", "Values": [ami_id]}])
    ...
    for target_region in target_regions_list:
        ...
        check_response = ec2_target.describe_images(Filters=[{"Name": "name", "Values": [ami_name]}], Owners=["self"])
...
=======
            new_ami_id = next(
                iter([img["ImageId"] for img in check_response["Images"] if img["Name"] == ami_name]), None
            )
        else:
            click.echo(f"Copying AMI to {target_region}")
            copy_response = ec2_target.copy_image(Name=ami_name, SourceRegion=source_region, SourceImageId=ami_id)
            new_ami_id = copy_response["ImageId"]
...
=======
        if copy_permissions:
            click.echo(f"Copying launch permissions from {ami_id} to {new_ami_id} in {target_region}")
            # Get launch permissions from source AMI
            launch_permissions = ec2_source.describe_image_attribute(ImageId=ami_id, Attribute="launchPermission").get(
                "LaunchPermissions", []
            )
            if launch_permissions:
                ec2_target.modify_image_attribute(ImageId=new_ami_id, LaunchPermission={"Add": launch_permissions})
