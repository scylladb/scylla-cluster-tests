...
def copy_ami_to_all_regions(ami_id, ami_name, source_region, target_regions, dry_run, copy_permissions):  # noqa: PLR0914
    ...
    for target_region in target_regions_list:
        if dry_run:
            click.echo(f"[Dry Run] Would copy AMI '{ami_id}' from {source_region} to {target_region} with name '{ami_name}'")
        ...
        if check_response.get("Images"):
            ...
            new_ami_id = next(iter([img["ImageId"] for img in check_response["Images"] if img["Name"] == ami_name]), None)
        else:
        ...
        if copy_permissions:
            ...
            launch_permissions = ec2_source.describe_image_attribute(ImageId=ami_id, Attribute="launchPermission").get("LaunchPermissions", [])
