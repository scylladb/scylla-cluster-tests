...
def scan_issue_skips():
    ...
    for file_path in Path(get_sct_root_path()).glob("**/*.py"):
        if file_path.name.startswith("test_") or file_path.name == Path(__file__).name or any(subdir in str(file_path) for subdir in (".tox/", ".venv/", ".env/")):
        ...
        for node in ast.walk(ast.parse(file_path.read_text())):
            if isinstance(node, ast.Call):
                ...
                if name == "SkipPerIssues":
                    ...
                    issues_labeled = any(label.name for label in issues_labels if label.name.startswith("sct-") and label.name.endswith("-skip"))
