...
class PerformanceRegressionCDCTest(PerformanceRegressionTest):
    ...
    def test_write_with_cdc(self):
        ...
        self._workload_cdc(write_cmd, stress_num=2, test_name="test_write", sub_type="cdc_disabled")
    ...
    def cdc_workflow(self, use_cdclog_reader=False):
        ...
        self._workload_cdc(write_cmd, stress_num=2, test_name="test_write", sub_type="cdc_enabled", read_cdclog_cmd=self.cdclog_reader_cmd, update_cdclog_stats=self.update_es, enable_batching=self.enable_batching)
        ...
        self._workload_cdc(write_cmd, stress_num=2, test_name="test_write", sub_type="cdc_preimage_enabled", read_cdclog_cmd=self.cdclog_reader_cmd, update_cdclog_stats=self.update_es, enable_batching=self.enable_batching)
        ...
        self._workload_cdc(write_cmd, stress_num=2, test_name="test_write", sub_type="cdc_postimage_enabled", read_cdclog_cmd=self.cdclog_reader_cmd, update_cdclog_stats=self.update_es, enable_batching=self.enable_batching)

        self.wait_no_compactions_running()

        self.check_regression_with_baseline(subtest_baseline="cdc_disabled")

    def _workload_cdc(self, stress_cmd, stress_num, test_name, sub_type=None, save_stats=True, read_cdclog_cmd=None, update_cdclog_stats=False, enable_batching=True):
        ...
        if read_cdclog_cmd:
            cdc_stress_queue = self.run_cdclog_reader_thread(stress_cmd=read_cdclog_cmd, stress_num=1, keyspace_name=self.keyspace, base_table_name=self.table, enable_batching=enable_batching)
