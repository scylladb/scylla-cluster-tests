...
// Or on failure: ['master', 'staging', 'no_billing_project']
```
...
**Authentication**: Uses Jenkins credential `github-bot-token`
...
5. **Authentication Errors**: Logged and fallback list is used

All errors are logged to the Jenkins console for debugging.
...
The following pipelines currently use the `billing_project` parameter and can be migrated:
...
2. Verify `github-bot-token` credential exists and is valid
