...
## Step 1 — Name the instance type literally

Perf jobs pin the instance with `instance_type_db` / `gce_instance_type_db` /
`azure_instance_type_db` (and the matching `*_loader` / `*_monitor` params).

Do **not** use constraint-based sizing (`sizing_db` and friends) here. It resolves a
*sufficient* instance per cloud, which is the right behaviour for functional suites and the
wrong behaviour for performance: the hardware is the variable under test, results are only
comparable across runs on identical hardware, and a resolver that silently picks a different
instance invalidates the Argus baseline.
...
- [ ] Instance type named literally in the config
