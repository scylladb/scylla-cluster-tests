...
Subclass `DockerBasedStressThread` (`sdcm/stress/base.py`) in `sdcm/<tool>_thread.py` or
...
`ClusterTester.run_stress_thread` (`sdcm/tester.py`) selects the stress thread class by
inspecting the command string. Add a `run_<tool>_thread` method and a branch.
...
(`sdcm/utils/hdrhistogram.py`) resolves a tag to READ or WRITE with a substring match, in this
order:
...
`current_throttle()` in `performance_regression_gradual_grow_throughput.py` formats the rate
...
You are not editing the test class. You are producing a parallel config + job set.
