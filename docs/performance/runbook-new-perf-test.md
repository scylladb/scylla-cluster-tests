...
| Option | When |
|---|---|
| New test-case YAML on an existing test class | Same measurement, different workload, dataset or topology. **Most new perf tests are this.** |
...
| New test class | Genuinely new lifecycle (new preload semantics, new phase structure) |

Check [test-catalog.md](test-catalog.md) before writing a class — including its list of
unmaintained modules, which may already hold something close to what you need.
...
| `ClusterTester` | Only when none of the perf lifecycle applies (e.g. `ScaleUpTest`) |
...
| Label | Cadence | Suits |
|---|---|---|
| `master-2weeks` | every 14 days | Core regression coverage you want to bisect against |
| `master-3weeks` | every 21 days | Nemesis and upgrade tests (long, expensive) |
| `master-monthly` | 1st of month | Secondary topologies (vnodes) |
| `gce-custom-monthly` | 2nd Tuesday | GCE customer scenarios |
| no label | version-gated only | Release-branch runs |

Only labels that have a `cron_triggers` entry actually fire — check the YAML before inventing a
new one. Full picture: [pipelines-and-scheduling.md](pipelines-and-scheduling.md#1-cadence).
