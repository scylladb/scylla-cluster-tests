...
`n_monitor_nodes`, and the instance type. Perf jobs always name the instance literally
(`instance_type_db` and friends) — see
[runbook-new-instance-type.md](runbook-new-instance-type.md#step-1--name-the-instance-type-literally)
for why constraint-based sizing is not used here.
...
`preload_data()` — defined in `performance_regression_test.py` and overridden in
`performance_regression_gradual_grow_throughput.py` — runs `prepare_write_cmd`.
...
self.run_fstrim_on_all_db_nodes()
```
This triplet is repeated at the head of every measuring method in
...
`warmup_cache(stress_cmd_templ, params_dict)` in
`performance_regression_gradual_grow_throughput.py`, driven by the `stress_cmd_cache_warmup`
config param.
...
| Customer steady state | the whole steady-state window |
| Nemesis | one disruption |
| Upgrade | one node's upgrade |

The staircase loop, `run_gradual_increase_load` in
`performance_regression_gradual_grow_throughput.py`, is the clearest example:
