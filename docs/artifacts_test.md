...
```
<<<<<<< HEAD
||||||| parent of 8d3fdc208 (docs(unified-package): add usage examples for relocatable:latest and unified_package)
...
hydra run-test artifacts_test --backend docker --config test-cases/artifacts/docker.yaml
```
>>>>>>> 8d3fdc208 (docs(unified-package): add usage examples for relocatable:latest and unified_package)
