...
- ARM pattern: `configurations/arm_instance_types/i8g_2xlarge.yaml` (NOTE: includes --no-ec2-check flag for artifact tests, not needed for performance tests)
- Target for migration: `configurations/arm_instance_types/i8g_4xlarge.yaml` (newly created for i8g.4xlarge - minimal config without --no-ec2-check as performance tests install from SMI)
...
6. **Are there ARM64-specific Scylla configuration considerations?**
   - Note: `--no-ec2-check` flag is NOT needed for performance tests (only for artifact tests installing not from SMI)
   - Performance regression tests install from SMI, so no special append_scylla_setup_args required
   - Need to verify if any other ARM64-specific tuning is needed
