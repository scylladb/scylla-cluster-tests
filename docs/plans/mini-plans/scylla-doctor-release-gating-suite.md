...
This pipeline:
1. Accepts `sd_tarball_url` as a **required** parameter — the direct URL to the SD tarball under test
...
4. Passes `scylla_doctor_tarball_url` and `run_scylla_doctor_only=true` as **direct parameters** to each artifact job (no `extra_environment_variables`)
...
|-----------|------|----------|-------------|
| `sd_tarball_url` | string | **Yes** | Direct URL to a Scylla Doctor tarball in S3 |
...
string(defaultValue: '', name: 'scylla_doctor_tarball_url',
       description: 'Direct URL to a Scylla Doctor tarball in S3.')
...
| `vars/artifactsPipeline.groovy` | Modify | Add `scylla_doctor_tarball_url` and `run_scylla_doctor_only` as direct pipeline parameters |
