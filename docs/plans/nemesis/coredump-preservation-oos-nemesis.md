...
**`scylla_coredump_setup`** ([source](https://github.com/scylladb/scylladb/blob/master/dist/common/scripts/scylla_coredump_setup)), invoked by `scylla_setup` from `BaseNode.scylla_setup()` (`sdcm/cluster.py:3182`):
...
**`sdcm/coredump.py`** — `CoredumpExportSystemdThread` (line 354), started per node from `sdcm/cluster.py:1582`:
...
- `disrupt_nodetool_enospc()` (line 1826) — restricted to Scylla ≤ 2025.3; K8s takes `_k8s_fake_enospc_error()`; otherwise `reach_enospc_on_node()` fills the disk and a `finally` runs `clean_enospc_on_node()`.
- `disrupt_end_of_quota_nemesis()` (line 1857) — a *different* mechanism: an `xfs_quota` block limit on the **scylla user**, filled by `fallocate` run as `su - scylla` (`sdcm/utils/quota.py:99`).

**`sdcm/utils/common.py`** — `reach_enospc_on_node()` (line 2563) fills to ~100% with `fallocate` in a `wait_for` loop; `clean_enospc_on_node()` (line 2585) removes `occupy_90percent.*` and restarts Scylla.
...
- [ ] **First**, extract `CoredumpExportSystemdThread._wait_for_pending_coredumps()` into a standalone
  `wait_for_pending_coredumps(node)` in `sdcm/coredump.py` and have the method delegate to it. The body
  touches only `self.node` and `self.log`, so the move is mechanical. The context manager calls the helper —
  it must **not** reach into `node._coredump_thread._wait_for_pending_coredumps()`, which would couple
  `context_managers.py` to the thread's internals and break on nodes where the thread was never started.
- [ ] Restore in a `finally`, and log-and-continue on restore failure rather than masking the nemesis's own exception.
- [ ] Log a warning when the pre-restore move fails for any reason other than "no cores were written" — never
  redirect its stderr to `/dev/null`. A swallowed failure here strands exactly the core this plan exists to keep.
...
*(a) Cores in flight.* `systemd-coredump@.service` is configured with `RuntimeMaxSec=infinity` and a multi-GB core can take minutes to write. Stopping the mount underneath a running dump truncates it. `CoredumpExportSystemdThread._wait_for_pending_coredumps()` (`sdcm/coredump.py:403`) already implements exactly this wait — reuse it by extracting it into a module-level `wait_for_pending_coredumps(node)` in `sdcm/coredump.py` (see Phase 1 Definition of Done), not by duplicating it and not by calling the private method through `node._coredump_thread`.
    ...
    try:
        ...
        try:
            ...
            result = node.remoter.sudo(
                "sh -c 'mv -f /var/lib/systemd/coredump/core.* /var/lib/scylla/coredump/'",
                ignore_status=True)
            if not result.ok and "No such file or directory" not in result.stderr:
                # a silent failure here strands exactly the core this plan exists to keep
                node.log.warning("Failed to move coredumps before restoring %s: %s", unit, result.stderr)
...
**Owner:** whoever picks up Phase 1 runs this measurement *first*, before writing the context manager. It is a
measurement on an existing test run, not a code change, so it does not block starting Phase 1 — but it can invalidate
Phases 1–2 outright, so it must not be left until after they are written.

**Dependencies:** none — the measurement gates the *value* of Phases 1–2, not their development. Phase 1 may be
developed and merged in parallel (it is a no-op on any node without the mount unit), but **Phase 2 must not merge
before the measured `root_disk_size_db` lands** — wiring the context manager into the nemesis while the root disk is
still 30 GB trades a lost core for a full root filesystem, which is the worse failure.
...
| `test_coredump_to_root_warns_when_move_fails` | a failing move logs a warning and still restores the mount; an empty-glob "No such file" result logs nothing |
...
1. **Compressed core size on ENOSPC instance types** — drives the `root_disk_size_db` value in Phase 3. Must be measured, not guessed; owned by the Phase 1 implementer and taken first (see Phase 3).
...
3. **`--no-raid-setup` / offline-install nodes** (`sdcm/cluster.py:6156`) — presumed no bind mount, so the no-op path applies; worth confirming on one run.
