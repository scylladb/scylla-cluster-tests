...
region-chunked, fail-soft), plus `rank_az_letters()` and `rank_regions()`. Only a non-empty answer enters the
cache: caching the `[]` returned on failure would turn one throttled or briefly-unauthorized call into a whole
TTL of unscored provisioning, which is exactly when the scores are worth having.
...
survives it. Configured AZ letters keep priority unless `spot_score_overrides_configured_az` is set. When
`spot_placement_score_min` is raised above its default `0` and leaves fewer qualifying AZs than the config asks
for, provisioning fails rather than quietly spanning fewer AZs than the test was written for. Multi-region
...
fail. Useful for `region: random` jobs that land badly. Any failure to relocate degrades to staying put — the
configured region is always a valid place to start, so an opt-in tuning knob must not become a new way for runs
to fail.
...
load — including on backends without spot, and when no duration is known at all, where it becomes `on_demand` —
so nothing downstream sees it. An unknown duration is not evidence of a short test, and the two wrong guesses
are not symmetric: guessing spot risks losing a multi-day run to a reclamation, guessing on_demand only costs
money.

**Debug CLI.** `hydra spot-placement-scores` (AWS) and `hydra gce-capacity-advice` (GCE) print the raw
signal for manual inspection, per region or per AZ/zone. A CLI that printed a single best region for a
Jenkins step to consume was considered and dropped: see the builder-region limitation below.
