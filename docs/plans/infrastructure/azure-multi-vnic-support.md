...
1. **Provision up to `MaxNetworkInterfaces` NICs per Azure DB VM**, driven by the existing
   `scylla_network_config` option, with each secondary NIC in its own dedicated subnet of the
   test's VNet.
2. **Make `scylla_network_config` fully functional on the Azure backend** — a shared
   ...
   AWS, with no Azure-side companion file to write.
3. **Support IPv6 on Azure DB nodes as an opt-in**, both private (VNet ULA prefix) and public
   ...
   so nothing IPv6 is created unless an `ip_type: ipv6` entry asks for it.
...
### Phase 1: derive the Azure NIC layout from `scylla_network_config`

**Importance**: Critical
**Description**: Translate `scylla_network_config` into the Azure NIC layout, and validate what
Azure cannot honour. No provisioning behaviour changes yet, so the phase is independently
mergeable and testable.

`scylla_network_config` already says how many interfaces the node has, which address family each
one carries and which addresses must be reachable from outside the VNet. Everything Azure needs to
build the NICs follows from it, so no Azure-only companion option is introduced:

| Derived | Rule |
|---|---|
| number of interfaces | the number of distinct `nic` indexes |
| subnet | `default` for nic 0, `nic<index>` for the rest |
...
**Deliverables**:
- `azure_network_interfaces(params) -> list[dict]` helper in
  `sdcm/provision/network_configuration.py`, one dict per device index, built from the table above.
  The NIC count reuses the existing `network_interfaces_count()`, so the provisioner and the
  node-side routing setup cannot drift apart.
- `azure_ipv6_enabled(params) -> bool` helper beside it, `any(nic["ipv6"] for nic in ...)`. This is
  the single predicate every later phase gates IPv6 provisioning on — the providers (Phase 6), the
  shared region resources and the runner (Phase 7), and the monitor node (Phase 8).
- The `scylla_network_config` docstring updated from *"Supported for AWS and GCE meanwhile"* to
  name OCI and Azure too.
- New validation, run only when `cluster_backend == "azure"`, in validation block 17. Deriving the
  layout makes most mismatches unrepresentable; what is left is what Azure cannot place where the
  profile asks for it:
  - Contiguous `nic` indexes starting at 0, so no address targets an interface never built.
  - An `ip_type: ipv4, public: true` entry only on nic 0 — SCT attaches the IPv4 Public IP to the
    primary NIC only (complements the existing "public IPv4 only on nic 0" rule, for nic 2 and up).
  - `ip_ssh_connections: ipv6` requires an `ip_type: ipv6, public: true` `test_communication` on
    nic 0 — the SCT runner is outside the test VNet, so it can only reach a node over a routable
    IPv6.
...
- New `scylla_network_config` → NIC-spec plumbing: `InstanceDefinition` gains
...
- `AzureSctRunner._network_interfaces()`: one NIC, dual-stack with a routable IPv6 Public IP when
  `azure_ipv6_enabled(self.params)`, passed to the runner's provisioner. `scylla_network_config`
...
**Deliverables**:
- `configurations/azure/network_config/*.yaml` — an override only for a shared
  `configurations/network_config/` profile Azure cannot honour as written. The NIC layout itself
  is derived, so a profile needs a file here only when Azure changes what Scylla may bind to.
...
- `unit_tests/unit/test_network_config.py` — the Phase 1 derivation table and validation rules,
  positive and negative, plus `network_interfaces_count()` against Azure configs.
