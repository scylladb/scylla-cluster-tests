...
1. **Provision a standalone Cassandra cluster as a first-class SCT cluster type** — introduce a `CassandraCluster` hierarchy (backed by `BaseCassandraCluster`) that SCT can provision independently of any specific test tool, with Gemini oracle as the first validation use case, replacing the dead legacy `CassandraAWSCluster`
2. **Support all backends** — Cassandra clusters on AWS, GCE, Azure, and Docker, using the same install-on-boot pattern as other cluster types
3. **Enable Cassandra as a Gemini oracle (initial use case)** — wire the new Cassandra cluster into Gemini's `--oracle-cluster` flow via a new `db_type: "mixed_cassandra"`, fulfilling Gemini's original design intent and validating the Cassandra cluster implementation end-to-end
