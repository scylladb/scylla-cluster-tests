...
owner: fruch
---
# Cluster Layout Topology - Usage Examples

This document provides practical examples for configuring clusters using the unified cluster topology format (Option 3). Examples cover per-node instance types, disk configuration, per-rack availability zones, per-datacenter networking, and other topology-aware parameters.
...
## Topology-Aware Configuration Examples

### Example 10: Per-Node Disk Configuration

**Use Case**: Test nodes with different storage configurations in the same rack
    ...
    racks:
      - rack:
        ...
        nodes:
          ...
          - instance_type: i4i.2xlarge
            root_disk_size: 100
            data_volume_disk_size: 1000
            data_volume_disk_type: gp3
            data_volume_disk_iops: 6000
            data_volume_disk_throughput: 500
```

**Result**:
- 1 datacenter, 1 rack, 2 nodes
- Each node has different disk size and IOPS settings
- Useful for testing storage-sensitive workloads on heterogeneous hardware

---

### Example 11: Per-Rack AZ and Per-DC Network Configuration

**Use Case**: Explicit control over availability zones, subnets, and VPCs
...
**Result**:
- 1 datacenter with explicit VPC
- 2 racks pinned to different AZs and subnets
- 4 total nodes with per-rack network isolation

---

### Example 12: GCE with Local SSDs Per Node

**Use Case**: Test nodes with different local SSD counts on GCE
...
**Result**:
- 1 datacenter, 1 rack, 2 nodes
- Different local SSD counts per node
- Tests storage performance with asymmetric local storage

---

### Example 13: Combined Instance Type, Disk, and Network Configuration

**Use Case**: Full topology-aware configuration combining all parameter types
      ...
      - rack:
        ...
        nodes:
          ...
          - instance_type: i4i.4xlarge
            root_disk_size: 100
            data_volume_disk_size: 2000
            data_volume_disk_type: io2
            data_volume_disk_iops: 10000
...
**Result**:
- 2 datacenters in different regions with explicit VPCs
- Per-rack AZ and subnet placement
- Per-node instance type, disk size, disk type, and IOPS configuration
- Tests cross-region replication with fully heterogeneous topology
...
- **Implementation Plan**: See `docs/plans/infrastructure/cluster-layout-topology.md`
