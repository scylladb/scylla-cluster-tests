...
- [cassandra-stress](https://github.com/scylladb/cassandra-stress/) - the most commonly used stress tool, simulating various workloads on Scylla with predefined columns count, row size and user profiles for custom ones.
- [scylla-bench](https://github.com/scylladb/scylla-bench/) - a newer stress tool designed for Scylla, providing more advanced features especially for testing large partitions.
- [gemini](https://github.com/scylladb/gemini/) - tool for data consistency validation across various schemas and mixed workloads (read, write, delete). Verifies data integrity against 'oracle' cluster.
- [latte](https://github.com/scylladb/latte/) - advanced stress tool, mostly used for custom schemas (supports many data types and is very flexible). `rust` based.
- [cql-stress](https://github.com/scylladb/cql-stress/) - `rust` based equivalent of `cassandra-stress` and `scylla-bench` - supports the same command line options.
- [ycsb](https://github.com/scylladb/ycsb/) - only tool supporting ScyllaDB Alternator API, used for testing ScyllaDB in AWS DynamoDB compatible mode.
...
- **HDR Histogram**: if `use_hdrhistogram` is set to `true`, proper arguments are added and SCT handles collection HDR histograms. Supported by `cassandra-stress`, `cql-stress`, `latte` and (possibly) `scylla-bench`; (`ycsb` WIP).
...
- **Round Robin (`true`)**: Each stress command runs sequentially on loaders, rotating through available loaders.
