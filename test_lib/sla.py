...
=======
    def __hash__(self):
        return hash(
            (
                self.name,
                tuple((field.name, getattr(self._sl_attributes, field.name)) for field in fields(self._sl_attributes)),
            )
        )
...
=======
            raise ValueError(
                f"No Service Level attached to the role '{self.name}'. But it is expected that Service Level "
                f"'{self._attached_service_level_name}' is attached to this role. Validate if it is test or "
                "Scylla issue"
            )
>>>>>>> e29892926 (improvement(treewide): Reformat using ruff)
