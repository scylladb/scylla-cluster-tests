...
class ServiceLevel:
    ...
    def __init__(self, session, name: str, shares: Optional[int] = 1000, timeout: str = None, workload_type: str = None):
    ...
    def from_row(cls, session, row):
        ...
        return ServiceLevel(session=session, name=row_dict["service_level"], shares=row_dict["shares"], timeout=row_dict["timeout"], workload_type=row_dict["workload_type"])
    ...
    def __hash__(self):
        return hash((self.name, tuple((field.name, getattr(self._sl_attributes, field.name)) for field in fields(self._sl_attributes))))
    ...
    def create(self, if_not_exists=True) -> ServiceLevel:
        query = "CREATE SERVICE_LEVEL{if_not_exists} {service_level_name}{query_attributes}".format(if_not_exists=" IF NOT EXISTS" if if_not_exists else "", service_level_name=self.name, query_attributes=self._sl_attributes.query_string)
        if self.verbose:
    ...
    def alter(self, new_shares: int = None, new_timeout: str = None, new_workload_type: str = None):
        ...
        query = "ALTER SERVICE_LEVEL {service_level_name} {query_string}".format(service_level_name=self.name, query_string=sla.query_string)
        if self.verbose:
    ...
    def drop(self, if_exists=True):
        query = "DROP SERVICE_LEVEL{if_exists} {service_level_name}".format(service_level_name=self.name, if_exists=" IF EXISTS" if if_exists else "")
        if self.verbose:
...
class Role(UserRoleBase):
    ...
    def role_full_info_dict(self) -> dict:
        return {"service_level": self.attached_service_level, "service_level_shares": self.attached_service_level_shares, "service_level_name": self.attached_service_level_name, "sl_group": self.attached_scheduler_group_name}
    ...
    def validate_role_service_level_attributes_against_db(self):
        ...
        if not service_level and self.attached_service_level:
            raise ValueError(f"No Service Level attached to the role '{self.name}'. But it is expected that Service Level '{self._attached_service_level_name}' is attached to this role. Validate if it is test or Scylla issue")
        elif not self.attached_service_level and service_level:
            raise ValueError(f"Found attached Service Level '{service_level[0].service_level}' to the role '{self.name}'. But it is expected that no attached Service Level. Validate if it is test or Scylla issue")
        ...
        if service_level[0].service_level == self.attached_service_level_name:
            ...
            if db_service_level.shares != self.attached_service_level_shares:
                raise ValueError(
                    f"Found attached Service Level '{service_level[0].service_level}' to the role '{self.name}' "
...
class User(UserRoleBase):
    ...
    def create(self) -> User:
        user_options_str = "{password}{superuser}".format(password=" PASSWORD '{}'".format(self.password) if self.password else "", superuser="" if self.superuser is None else " SUPERUSER" if self.superuser else " NOSUPERUSER")
        if user_options_str:
...
def create_sla_auth(session, shares: int, index: str, superuser: bool = True, attach_service_level: bool = True) -> Role:
    role = Role(session=session, name=STRESS_ROLE_NAME_TEMPLATE % (shares or "", index), password=STRESS_ROLE_PASSWORD_TEMPLATE % shares or "", login=True, superuser=superuser).create()
    if attach_service_level:
        role.attach_service_level(ServiceLevel(session=session, name=SERVICE_LEVEL_NAME_TEMPLATE % (shares or "", index), shares=shares).create())
