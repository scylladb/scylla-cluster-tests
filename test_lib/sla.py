...
class ServiceLevel:
    ...
    def __init__(
        self, session, name: str, shares: Optional[int] = 1000, timeout: str = None, workload_type: str = None
    ):
    ...
    def create(self, if_not_exists=True) -> ServiceLevel:
        query = "CREATE SERVICE_LEVEL{if_not_exists} {service_level_name}{query_attributes}".format(
    ...
    def drop(self, if_exists=True):
        query = "DROP SERVICE_LEVEL{if_exists} {service_level_name}".format(
            service_level_name=self.name, if_exists=" IF EXISTS" if if_exists else ""
        )
        if self.verbose:
...
class Role(UserRoleBase):
    ...
    def role_full_info_dict(self) -> dict:
        return {
            "service_level": self.attached_service_level,
    ...
    def validate_role_service_level_attributes_against_db(self):
        ...
        if not service_level and self.attached_service_level:
            raise ValueError(
                f"No Service Level attached to the role '{self.name}'. But it is expected that Service Level '{self._attached_service_level_name}' is attached to this role. Validate if it is test or Scylla issue"
            )
        elif not self.attached_service_level and service_level:
            raise ValueError(
                f"Found attached Service Level '{service_level[0].service_level}' to the role '{self.name}'. But it is expected that no attached Service Level. Validate if it is test or Scylla issue"
            )
...
def create_sla_auth(
    session, shares: int, index: str, superuser: bool = True, attach_service_level: bool = True
) -> Role:
    role = Role(
        session=session,
