...
def get_gc_mode(node: BaseNode, keyspace: str, table: str) -> str | GcMode:
    ...
    :Arguments:
        ...
        table
    """
    table_gc_mode_result = node.run_cqlsh(f"SELECT extensions FROM system_schema.tables where keyspace_name = '{keyspace}' and table_name = '{table}'", split=True)
...
def get_compaction_strategy(node, keyspace, table):
    ...
    Arguments:
        ...
        table
    """
    list_tables_compaction = node.run_cqlsh("SELECT keyspace_name, table_name, compaction FROM system_schema.tables", split=True)
    compaction = "N/A"
...
def get_compaction_random_additional_params(strategy: CompactionStrategy):
    ...
    list_additional_params_options = {
        ...
        CompactionStrategy.SIZE_TIERED: [{"bucket_high": bucket_high}, {"bucket_low": bucket_low}, {"min_sstable_size": min_sstable_size}, {"min_threshold": min_threshold}, {"max_threshold": max_threshold}],
