...
class GrowClusterTest(ClusterTester):
    ...
    def get_stress_cmd_profile(self):
        ...
        for node in self.loaders.nodes:
            node.remoter.send_files(cs_custom_config, "/tmp/cassandra-stress-custom-mixed-narrow-wide-row.yaml", verbose=True)
    ...
    def get_stress_cmd(self, mode="write", duration=None):
        ...
        return "cassandra-stress %s cl=QUORUM duration=%sm -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor=3)' -col 'size=FIXED(2) n=FIXED(1)' -mode cql3 native -rate threads=%s -pop seq=1..%s -node %s" % (
            mode,
            duration,
            threads,
            population_size,
    ...
    def test_grow_x_to_y(self):
        ...
        """
        self.grow_cluster(cluster_target_size=self._cluster_target_size, stress_cmd=self.get_stress_cmd())
    ...
    def test_grow_3_to_4_large_partition(self):
        ...
        3) Add a new node
        """
        self.grow_cluster(cluster_target_size=4, stress_cmd=self.get_stress_cmd_profile())
    ...
    def test_add_remove_nodes(self):
        ...
        while duration <= self._duration:
            ...
            node_cnt = len(self.db_cluster.nodes)
            max_add_cnt = max_random_cnt if max_random_cnt <= self._cluster_target_size - node_cnt else self._cluster_target_size - node_cnt
            if max_add_cnt >= 1:
            ...
            if rm_cnt > 0:
                ...
                for _ in range(rm_cnt):
                    decommision_nemesis = nemesis.DecommissionMonkey(tester_obj=self, termination_event=self.db_cluster.nemesis_termination_event)
