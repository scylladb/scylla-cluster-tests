...
class GrowClusterTest(ClusterTester):
    ...
    def get_stress_cmd_profile(self):
        ...
        for node in self.loaders.nodes:
            node.remoter.send_files(
                cs_custom_config, "/tmp/cassandra-stress-custom-mixed-narrow-wide-row.yaml", verbose=True
            )
    ...
    def get_stress_cmd(self, mode="write", duration=None):
        ...
        return (
            "cassandra-stress %s cl=QUORUM duration=%sm -schema 'replication(strategy=NetworkTopologyStrategy,replication_factor=3)' -col 'size=FIXED(2) n=FIXED(1)' -mode cql3 native -rate threads=%s -pop seq=1..%s -node %s"
            % (
                mode,
                duration,
                threads,
    ...
    def test_add_remove_nodes(self):
        ...
        while duration <= self._duration:
            ...
            max_add_cnt = (
                max_random_cnt
                if max_random_cnt <= self._cluster_target_size - node_cnt
                else self._cluster_target_size - node_cnt
            )
            if max_add_cnt >= 1:
